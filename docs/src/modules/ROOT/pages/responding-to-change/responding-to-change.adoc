...
* Use xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariableAllowingUnassigned[planning variables with unassigned values],
...
. Make the planning variable xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariableAllowingUnassigned[allow unassigned values].
...
Use xref:constraints-and-score/score-calculation.adoc#constraintStreamsForEach[`forEachIncludingUnassigned`] and check if the xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariable[planning variable] or xref:using-greycos-solver/modeling-planning-problems.adoc#bidirectionalVariable[inverse relation shadow variable] is `null`:
...
There are cases where pinning only a part of xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariable[planning list variable] is necessary.
...
it is important to understand the behavior of xref:using-greycos-solver/modeling-planning-problems.adoc#cloningASolution[a planning clone].
...
. The `workingSolution` is xref:using-greycos-solver/modeling-planning-problems.adoc#cloningASolution[a planning clone] of the ``BestSolutionChangedEvent``'s ``bestSolution``.
...
This requires xref:using-greycos-solver/modeling-planning-problems.adoc#planningId[annotating a property of that class as the @PlanningId].
...
(unless you have a xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariableAllowingUnassigned[planning variable with unassigned values]),
...
If there are multiple construction heuristics phases in the xref:using-greycos-solver/configuration.adoc#solverConfiguration[solver configuration],
