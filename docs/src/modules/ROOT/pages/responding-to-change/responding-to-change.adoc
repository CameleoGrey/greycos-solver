...
GreyCOS Solver allows you to start planning earlier, despite unforeseen changes,
...
By default, GreyCOS Solver assigns all planning entities, overloads the planning values, and therefore breaks hard constraints.
There are two ways to avoid this:

* Use xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariableAllowingUnassigned[planning variables with unassigned values],
...
. Make the planning variable xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariableAllowingUnassigned[allow unassigned values].
...
Use xref:constraints-and-score/score-calculation.adoc#constraintStreamsForEach[`forEachIncludingUnassigned`] and check if the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariable[planning variable] or xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#bidirectionalVariable[inverse relation shadow variable] is `null`:
...
This is commonly used by users to pin down one or more specific assignments and force GreyCOS Solver to schedule around those fixed assignments.
...
There are cases where pinning only a part of xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[planning list variable] is necessary.
...
In that case, the value of `@PlanningPinToIndex` must be zero (`0`).
...
WARNING: `PinningFilter` is deprecated for removal in a future version of GreyCOS Solver.
...
GreyCOS Solver can handle such scenarios with `ProblemChange` (in combination with <<pinnedPlanningEntities,pinned planning entities>>).
...
it is important to understand the behavior of xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#cloningASolution[a planning clone].
...
. The `workingSolution` is xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#cloningASolution[a planning clone] of the ``BestSolutionChangedEvent``'s ``bestSolution``.
...
This requires xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningId[annotating a property of that class as the @PlanningId].
...
(unless you have a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariableAllowingUnassigned[planning variable with unassigned values]),
...
If there are multiple construction heuristics phases in the xref:xref:using-greycos-solver/configuration.adoc.adoc#solverConfiguration[solver configuration],
