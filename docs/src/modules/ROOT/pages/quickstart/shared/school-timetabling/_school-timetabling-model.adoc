...
Such classes do not require any GreyCOS Solver specific annotations.
...
During solving, GreyCOS Solver changes the `timeslot` and `room` fields of the `Lesson` class,
to assign each lesson to a time slot and a room.
Because GreyCOS Solver changes these fields, `Lesson` is a _planning entity_:
...
GreyCOS Solver changes these fields during solving.
Such fields are called planning variables.
In order for GreyCOS Solver to recognize them,
...
data class Lesson (
    ...
    // No-arg constructor required for GreyCOS
    constructor() : this("0", "", "", "")
...
so GreyCOS Solver knows that this class changes during solving
because it contains one or more planning variables.

The `timeslot` field has an `@PlanningVariable` annotation,
so GreyCOS Solver knows that it can change its value.
In order to find potential `Timeslot` instances to assign to this field,
GreyCOS Solver uses the variable type to connect to a xref:using-greycos-solver/modeling-planning-problems.adoc#planningValueRangeProvider[value range provider]
