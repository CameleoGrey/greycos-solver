= Gather the Cotwin objects in a planning solution
:imagesdir: ../..
...
so GreyCOS Solver knows that this class contains all of the input and output data.
...
It holds the `Timeslot` instances which GreyCOS Solver can pick from to assign to the `timeslot` field of `Lesson` instances.
The `timeslots` field has an `@ValueRangeProvider` annotation to connect the `@PlanningVariable` with the `@ValueRangeProvider`,
by matching the type of the planning variable with the type returned by the xref:using-greycos-solver/modeling-planning-problems.adoc#planningValueRangeProvider[value range provider].
...
Furthermore, GreyCOS Solver needs to know which `Lesson` instances it can change
...
so GreyCOS Solver can change them during solving
