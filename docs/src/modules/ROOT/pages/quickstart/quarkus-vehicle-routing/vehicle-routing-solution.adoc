...
it is a xref:using-greycos-solver/modeling-planning-problems.adoc#planningProblemAndPlanningSolution[_planning solution_]
and it has a score:
...
by matching the type of the planning list variable with the type returned by the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueRangeProvider[value range provider].
