...
* GreyCOS Solver (`greycos-solver-spring-boot-starter`)
...
greycos.solver.termination.spent-limit=5s
----
...
First add the `greycos-solver-test` dependency to your build file:
...
@SpringBootTest(properties = {
        ...
        "greycos.solver.termination.spent-limit=1h",
        "greycos.solver.termination.best-score-limit=0hard/*soft" },
...
[source,properties]
----
logging.level.ai.greycos.solver=debug
----
...
Congratulations!
You have just developed a Spring application with https://github.com/CameleoGrey/greycos-solver[GreyCOS]!

Next Steps:

- For a full implementation with a web UI, check out {spring-boot-quickstart-url}[the Spring-boot quickstart source code].
- Check out more information about GreyCOS's integration with Spring in our xref:../integration/integration.adoc#integrationWithSpringBoot[Integration documentation].
