...
By default, each constraint is justified with `greycos.solver.core.api.score.stream.DefaultConstraintJustification`,
...
The only limitation is that it must implement the `greycos.solver.core.api.score.stream.ConstraintJustification` marker interface.
...
|3          |`greycos.solver.core.api.function.TriPredicate<A, B, C>`
|4          |`greycos.solver.core.api.function.QuadPredicate<A, B, C, D>`
|===
...
[source,java,options="nowrap"]
----
    import static greycos.solver.core.api.score.stream.Joiners.*;
...
[source,java,options="nowrap"]
----
import static greycos.solver.core.api.score.stream.ConstraintCollectors.*;
...
`greycos.solver.core.api.score.stream.uni.UniConstraintCollector` interface,
