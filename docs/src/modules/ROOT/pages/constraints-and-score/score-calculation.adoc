...
This constraint stream iterates over all instances of class `Shift` in the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#problemFacts[problem facts] and
xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningEntity[planning entities]
in the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningProblemAndPlanningSolution[planning problem].
...
is in a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#problemFacts[problem fact collection]
or a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningEntitiesOfASolution[planning entity collection]
that is xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#detectingInconsistencies[consistent]
and has no `null` genuine planning variables.

To include instances with a `null` xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariable[genuine planning variable],
or planning values
not assigned to any xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[planning list variable],
...
In cases utilizing the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[planning list variable],
you may want
to include an xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#listVariableShadowVariablesInverseRelation[inverse relation shadow variable]
...
`from()`
behaves unexpectedly for xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariableAllowingUnassigned[variables with unassigned values].
...
The purpose of constraint streams is to build up a xref:constraints-and-score/overview.adoc#whatIsAScore[score] for a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningProblemAndPlanningSolution[solution].
...
To use it, first add a test scoped dependency to the `greycos-solver-test` JAR.
...
that calling `givenSolution(...)` does not update any xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowVariable[shadow variables].
...
To test all constraints while updating all xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowVariable[shadow variables],
...
add the `easyScoreCalculatorCustomProperties` element and use xref:xref:using-greycos-solver/configuration.adoc.adoc#customPropertiesConfiguration[custom properties]:
...
(so the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] can tweak those parameters),
add the `incrementalScoreCalculatorCustomProperties` element and use xref:xref:using-greycos-solver/configuration.adoc.adoc#customPropertiesConfiguration[custom properties]:
