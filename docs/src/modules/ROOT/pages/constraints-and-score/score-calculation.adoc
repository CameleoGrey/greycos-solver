...
This constraint stream iterates over all instances of class `Shift` in the xref:using-greycos-solver/modeling-planning-problems.adoc#problemFacts[problem facts] and
xref:using-greycos-solver/modeling-planning-problems.adoc#planningEntity[planning entities]
in the xref:using-greycos-solver/modeling-planning-problems.adoc#planningProblemAndPlanningSolution[planning problem].
...
GreyCOS Solver currently does not support constraint stream cardinalities higher than 4.
...
[NOTE]
====
GreyCOS Solver does not provide any tuple implementations out of the box.
...
is in a xref:using-greycos-solver/modeling-planning-problems.adoc#problemFacts[problem fact collection]
or a xref:using-greycos-solver/modeling-planning-problems.adoc#planningEntitiesOfASolution[planning entity collection]
that is xref:using-greycos-solver/modeling-planning-problems.adoc#detectingInconsistencies[consistent]
and has no `null` genuine planning variables.

To include instances with a `null` xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariable[genuine planning variable],
or planning values
not assigned to any xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariable[planning list variable],
...
In cases utilizing the xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariable[planning list variable],
you may want
to include an xref:using-greycos-solver/modeling-planning-problems.adoc#listVariableShadowVariablesInverseRelation[inverse relation shadow variable]
...
`from()`
behaves unexpectedly for xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariableAllowingUnassigned[variables with unassigned values].
...
`from()`, `fromUnfiltered()` and `fromUniquePair()` are now deprecated and will be removed in a future major version of GreyCOS Solver.
====

[#constraintStreamsPenaltiesRewards]
=== Penalties and rewards

The purpose of constraint streams is to build up a xref:constraints-and-score/overview.adoc#whatIsAScore[score] for a xref:using-greycos-solver/modeling-planning-problems.adoc#planningProblemAndPlanningSolution[solution].
...
This is an example of using a constraint match weight.
...
- Constraint ID is a short name which must be unique within the entire `ConstraintProvider` implementation.
It may not contain most special characters.
- Constraint weight is a constant score value indicating how much every match affects the score.
Valid examples include `SimpleScore.ONE`, `HardSoftScore.ONE_HARD` and `HardMediumSoftScore.of(1, 2, 3)`.
- Constraint match weight is an optional function indicating how many times the constraint weight is applied.
The penalty or reward is the constraint weight multiplied by the match weight; the default match weight is `1`.

Constraints with zero weight are disabled and impose no runtime cost.
Weights can be overridden through the `ConstraintWeightOverrides` member of the planning solution.

[#constraintMetadata]
===== Attaching metadata to constraints

`asConstraint(String id)` creates a constraint whose metadata contains only the constraint ID.
For richer metadata, implement `ConstraintMetadata` and pass it to `asConstraint(ConstraintMetadata)`:
...
[source,java,options="nowrap"]
----
public record MyConstraintMetadata(String id, String displayName) implements ConstraintMetadata {
}

private Constraint penalizeEveryShift(ConstraintFactory factory) {
    return factory.forEach(Shift.class)
            .penalize(HardSoftScore.ONE_SOFT)
            .asConstraint(new MyConstraintMetadata("penalizeEveryShift", "Penalize every shift"));
}
----
====

The solver snapshots `ConstraintMetadata.id()` while building the constraint,
but retains the complete metadata object for `Constraint.getConstraintMetadata()`.
The ID must remain stable for the lifetime of that object;
accessing metadata after its ID changes fails immediately instead of exposing inconsistent constraint identities.

[#constraintStreamsCustomizingJustifications]
==== Customizing justifications

One of important GreyCOS Solver features is its ability to xref:constraints-and-score/understanding-the-score.adoc[explain the score] of solutions it produced
through justifications.
By default, each constraint is justified with `ai.greycos.solver.core.api.score.stream.DefaultConstraintJustification`,
which captures the matched tuple and its score impact.
For example:
...
The only limitation is that it must implement the `ai.greycos.solver.core.api.score.stream.ConstraintJustification` marker interface.
...
|3          |`ai.greycos.solver.core.api.function.TriPredicate<A, B, C>`
|4          |`ai.greycos.solver.core.api.function.QuadPredicate<A, B, C, D>`
|===
...
[source,java,options="nowrap"]
----
    import static ai.greycos.solver.core.api.score.stream.Joiners.*;
...
It is tempting to replace filters with joiners where applicable, expecting better performance.
However, joiners carry the overhead of maintaining indexes,
and if a joiner does not significantly reduce the number of matches,
it may perform worse than a filter.
Some advanced joiners require large datasets before they outperform filtering.

Follow these guidelines:

* First implement the constraint in the most readable way; do not optimize prematurely.
* Measure instead of assuming how an API performs, because performance characteristics may change between versions.
* Use xref:performance.adoc#constraintProfiling[constraint profiling] to locate the most expensive constraint,
  optimize that constraint, and repeat until performance is satisfactory.

NOTE: To maximize performance, index evaluation order may differ from joiner declaration order.
Typically all `equal()` joiners are fused into one key and evaluated first,
followed by the remaining indexing joiners and finally the filtering joiners.
...
[NOTE]
====
Objects used as group key must obey the https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/Object.html#hashCode()[general contract of `hashCode`].
...
[source,java,options="nowrap"]
----
import static ai.greycos.solver.core.api.score.stream.ConstraintCollectors.*;
...
`ai.greycos.solver.core.api.score.stream.uni.UniConstraintCollector` interface,
...
The count is collected in a `long`.
...
The distinct count is collected in a `long`.
...
The sum is collected in a `long`.
Other variants of this collector:

* `sumBigDecimal()` collects a `java.math.BigDecimal` value.
* `sumBigInteger()` collects a `java.math.BigInteger` value.
* `sumDuration()` collects a `java.time.Duration` value.
* `sumPeriod()` collects a `java.time.Period` value.
...
* `averageBigDecimal()` collects a `java.math.BigDecimal` value, resulting in a `BigDecimal` average.
* `averageBigInteger()` collects a `java.math.BigInteger` value, resulting in a `BigDecimal` average.
* `averageDuration()` collects a `java.time.Duration` value, resulting in a `Duration` average.
...
See https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/Comparable.html[Javadoc for `Comparable`] to learn more.
====
...
<A> UniConstraintCollector<A, ?, Double>
    average(ToLongFunction<A> groupValueMapping) {
...
[source,java,options="nowrap"]
----
UniConstraintCollector<A, ?, Triple<Long, Integer, Integer>> collector =
    compose(count(),
            min(),
            max(),
...
[NOTE]
====
GreyCOS Solver does not provide any `Pair`, `Triple` or `Quadruple` implementation out of the box.
====
...
or its `Bi...`, `Tri...` or `Quad...` counterpart.
The supplier creates a mutable container for each group,
the specialized accumulator creates one value handle for every tuple entering that group,
and the finisher converts the container into the result.

[source,java,options="nowrap"]
----
public final class IncrementalToListCollector<A>
...
For every tuple, the solver calls `add()` exactly once,
then `replaceWith()` zero or more times while the tuple changes,
and `remove()` at most once when it leaves the group.
Implement updates directly instead of rebuilding the entire accumulated value;
that is the main performance benefit of the incremental collector contract.

NOTE: This simplified example uses `List.indexOf()` and `List.remove(Object)`,
which are linear-time operations and identify elements by equality.
Use it only when equal duplicate elements cannot occur.
Production collectors that accept duplicates need a stable per-handle identity in their result container.
...
that calling `givenSolution(...)` does not update any xref:using-greycos-solver/modeling-planning-problems.adoc#shadowVariable[shadow variables].
...
[NOTE]
====
You can also use `justifiesWith(...)` and `justifiesWithExactly(...)`
to validate the expected constraint justifications mapped by the constraint definition.
====
...
When executing `justifiesWith(...)` and `justifiesWithExactly(...)`,
comparisons are made using the standard method `equals` on the fact problem instances.
====

[#constraintStreamsTestingCustomAssertions]
=== Employing custom assertions

`getScore()` returns the score produced by a single constraint,
while `getImpact()` returns its raw aggregate match weight.
Use these values with an assertion library when a comparison is clearer than a fixed expected value:
...
This is useful when comparing the severity of a constraint across scenarios.
...
To test all constraints while updating all xref:using-greycos-solver/modeling-planning-problems.adoc#shadowVariable[shadow variables],
...
If you are using the `greycos-solver-quarkus` extension, inject the `ConstraintVerifier` in your tests:
...
If you are using the `greycos-solver-spring-boot-starter` module, autowire the `ConstraintVerifier` in your tests:
...
GreyCOS Solver supports two other types of score calculation.
...
(so the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] can tweak those parameters),
add the `easyScoreCalculatorCustomProperties` element and use xref:using-greycos-solver/configuration.adoc#customPropertiesConfiguration[custom properties]:
...
Advantages::
- Very fast and scalable; potentially faster than Constraint Streams if implemented correctly.
Disadvantages::
- Difficult to write; a scalable implementation heavily uses maps, indexes and other specialized data structures.
You have to learn, design, write and improve all these performance optimizations yourself.
- Difficult to maintain; regular constraint changes can lead to high maintenance costs.

Use <<constraintStreams,Constraint Streams>> unless measurements show that a custom incremental calculator is necessary.
...
[IMPORTANT]
====
Incremental score calculator code can be difficult to write and review.
xref:constraints-and-score/overview.adoc#invalidScoreDetection[Assert its correctness]
against an equivalent Constraint Streams implementation in an asserted environment mode.
====

To configure values of an `IncrementalScoreCalculator` dynamically in the solver configuration
(so the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] can tweak those parameters),
add the `incrementalScoreCalculatorCustomProperties` element and use xref:using-greycos-solver/configuration.adoc#customPropertiesConfiguration[custom properties]:
...
==== Incremental score calculation with score analysis

To add support for xref:constraints-and-score/understanding-the-score.adoc[score analysis],
implement `AnalyzableIncrementalScoreCalculator` instead of `IncrementalScoreCalculator`:
...
The solver calls `enableConstraintMatch()` once before analysis starts.
Register each match through the supplied `ConstraintMatchRegistry` and retain the returned
`ConstraintMatchRegistration` so that the match can be updated or canceled incrementally.
All registrations are canceled automatically when the working solution is reset.
