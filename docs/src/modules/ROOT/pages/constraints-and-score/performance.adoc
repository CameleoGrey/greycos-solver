...
then turn them into xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#cachedProblemFact[cached problem facts].
...
Use xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#valueRangeProviderOnPlanningEntity[a ValueRangeProvider on the planning entity]
...
* When you use xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowVariable[Shadow Variables], you will provide a `supplier` method that computes the value of the shadow variable.
...
JVM performance may differ by as much as 20% between runs. To decide whether your changes helped or made things worse, make sure to always average the move evaluation speed from multiple runs on the same machine with the same solver configuration. We provide a xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] to help you with that.

[#fullAssert]
== Validate the implementation using FULL_ASSERT
When you are done optimizing your score calculation, make sure to validate it using xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#environmentModeFullAssert[`FULL_ASSERT`] mode. Make sure not to run with this mode in production.
