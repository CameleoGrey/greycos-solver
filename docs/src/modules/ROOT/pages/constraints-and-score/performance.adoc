...
then turn them into xref:using-greycos-solver/modeling-planning-problems.adoc#cachedProblemFact[cached problem facts].
...
If a non-zero constraint can never contribute for the current problem,
the runtime may deactivate its node network after proving that no input can reach it.
For example, a constraint starting with `forEach(Visit.class)` can be deactivated when the solution contains no visits.
An explicit zero weight is still preferable because it communicates intent and lets the solver omit the network immediately.
...
Use xref:using-greycos-solver/modeling-planning-problems.adoc#valueRangeProviderOnPlanningEntity[a ValueRangeProvider on the planning entity]
...
* When you use xref:using-greycos-solver/modeling-planning-problems.adoc#shadowVariable[Shadow Variables], you will provide a `supplier` method that computes the value of the shadow variable.
...
You should xref:advanced-features/advanced-features.adoc#automaticNodeSharing[enable automatic node sharing] as it can significantly speed up score calculation.

[#benchmark]
== Benchmark
Whatever you do, benchmark on a large and diverse set of inputs.

JVM performance may differ by as much as 20% between runs.
To decide whether a change helped, run measurements sequentially on the same otherwise-idle machine,
using the same solver configuration, and average the move evaluation speed across multiple runs.
We provide a xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] to help with that.

[#constraintProfiling]
== Constraint profiling

Constraint profiling identifies which constraints consume the most score-calculation time.
Enable it only while diagnosing performance, because collecting timings adds overhead.
...
----
greycos.solver.constraint-stream-profiling-enabled=true
----
...
[source,properties,options="nowrap"]
----
quarkus.greycos.solver.constraint-stream-profiling-enabled=true
----
====

At the end of solving, GreyCOS Solver logs a summary ordered from the most expensive constraint to the least expensive.
The timings are wall-clock measurements and may include garbage collection pauses,
so use them to locate bottlenecks rather than as exact measurements.

Constraint profiling supports xref:score-calculation.adoc#constraintStreams[Constraint Streams]
and requires xref:advanced-features/advanced-features.adoc#multithreadedSolving[multi-threaded solving] to be disabled.

[#fullAssert]
== Validate the implementation using FULL_ASSERT
When you are done optimizing your score calculation, make sure to validate it using xref:using-greycos-solver/running-the-solver.adoc#environmentModeFullAssert[`FULL_ASSERT`] mode. Make sure not to run with this mode in production.
