...
GreyCOS Solver cannot automatically know which solution is best for your business,
...
Luckily, defining constraints in GreyCOS Solver is very flexible through the following score techniques:
...
[NOTE]
====
By default, GreyCOS Solver will always assign all planning variables a planning value.
...
even though GreyCOS Solver can handle many score levels.
====
...
It is left up to a human to choose the better out of a set of best solutions (with equal scores) found by GreyCOS Solver.
...
It is guaranteed that GreyCOS Solver has not found another solution which has more apples or more oranges or even a better combination of both (such as two apples and three oranges).

Pareto scoring is currently not supported in GreyCOS Solver.
...
but GreyCOS Solver's systems are __pareto comparison safe__, unless explicitly stated otherwise in this documentation.
====
...
GreyCOS Solver has several built-in `Score` implementations.
...
* `SimpleScore` uses a `long` value instead of an `int` value.
...
* `HardSoftScore` uses `long` values instead of `int` values.
...
* `HardMediumSoftScore` uses `long` values instead of `int` values.
...
* `BendableScore` uses `long` values instead of `int` values.
...
GreyCOS Solver does not recalculate the score of a solution if it can predict it (unless an xref:using-greycos-solver/running-the-solver.adoc#environmentMode[environmentMode assertion] is enabled).
...
To update planning entities when the planning variable change, use xref:using-greycos-solver/modeling-planning-problems.adoc#shadowVariable[shadow variables] instead.
====
...
When you put the xref:using-greycos-solver/running-the-solver.adoc#environmentMode[`environmentMode`] in `FULL_ASSERT` (or ``STEP_ASSERT``),
