...
GreyCOS Solver does not recalculate the score of a solution if it can predict it (unless an xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#environmentMode[environmentMode assertion] is enabled).
...
To update planning entities when the planning variable change, use xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowVariable[shadow variables] instead.
====
...
When you put the xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#environmentMode[`environmentMode`] in `FULL_ASSERT` (or ``STEP_ASSERT``),
