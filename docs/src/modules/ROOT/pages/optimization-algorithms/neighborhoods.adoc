...
This is only necessary when the solver is configured to use xref:advanced-features/advanced-features.adoc#multithreadedSolving[multi-threaded solving].
`String describe()`::
This method returns an identifier for the move type, which is used by parts of xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[the benchmarker]
...
It further specializes into `PlanningVariableMetaModel` for a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariable[basic planning variable],
`PlanningListVariableMetaModel` for a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[list variable],
and `ShadowVariableMetaModel` for a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowPlanningVariables[shadow variable].
...
The following example shows a simple move implementation which changes the value of a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariable[basic planning variable],
...
`ChangeMove`:: Changes the value of a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariable[basic planning variable] on a planning entity.
`SwapMove`:: Swaps the values of a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningVariable[basic planning variable] between two planning entities.
`ListChangeMove`:: Changes the position of a value in the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[list variable] of a planning entity.
`ListSwapMove`:: Swaps xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[list variable] elements between planning entities.
