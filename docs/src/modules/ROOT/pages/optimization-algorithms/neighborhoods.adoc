...
In technical terms, a move is an implementation of the `ai.greycos.solver.core.preview.api.move.Move` interface.
...
These moves are available through the `ai.greycos.solver.core.preview.api.move.builtin.Moves` class.
...
We encourage you to explore the `ai.greycos.solver.core.preview.api.move.builtin.Moves` API
...
in an implementation of the `ai.greycos.solver.core.preview.api.neighborhood.MoveProvider` interface.
...
found in the `ai.greycos.solver.core.preview.api.neighborhood.stream.joiner.NeighborhoodsJoiners` class.
...
you need to implement the `ai.greycos.solver.core.preview.api.neighborhood.NeighborhoodProvider` interface
...
This means that the solver will first pick whether to use the Neighborhoods API or the move selectors to generate the next move, and only then use that particular method to generate the move.
