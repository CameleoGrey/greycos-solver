...
In order for a metaheuristics-based solver such as GreyCOS Solver to explore the solution space effectively,
...
The Neighborhoods API is a new way to define moves in GreyCOS Solver.
...
In technical terms, a move is an implementation of the `greycos.solver.core.preview.api.move.Move` interface.
...
This is necessary when the solver is configured to use xref:advanced-features/advanced-features.adoc#multithreadedSolving[multi-threaded solving].
`String describe()`::
This method returns an identifier for the move type, which is used by parts of xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[the benchmarker]
...
`timeslotVariable` comes from the Cotwin MetaModel API,
...
we use the Cotwin MetaModel API.
...
It further specializes into `PlanningVariableMetaModel` for a xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariable[basic planning variable],
`PlanningListVariableMetaModel` for a xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariable[list variable],
and `ShadowVariableMetaModel` for a xref:using-greycos-solver/modeling-planning-problems.adoc#shadowPlanningVariables[shadow variable].
...
NOTE: Long-time users of GreyCOS Solver may be familiar with the concept of variable descriptors.
The Cotwin MetaModel API is a modern replacement for variable descriptors,
offering a more type-safe and user-friendly way to interact with the planning solution structure.
It is recommended to use the Cotwin MetaModel API for all new developments,
...
The following example shows a simple move implementation which changes the value of a xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariable[basic planning variable],
...
GreyCOS Solver provides several built-in move implementations
that cover the most common use cases.
These moves are available through the `greycos.solver.core.preview.api.move.builtin.Moves` class.
...
`ChangeMove`:: Changes the value of a xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariable[basic planning variable] on a planning entity.
`SwapMove`:: Swaps the values of a xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariable[basic planning variable] between two planning entities.
`ListChangeMove`:: Changes the position of a value in the xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariable[list variable] of a planning entity.
`ListSwapMove`:: Swaps xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariable[list variable] elements between planning entities.

We encourage you to explore the `greycos.solver.core.preview.api.move.builtin.Moves` API
...
in an implementation of the `greycos.solver.core.preview.api.neighborhood.MoveProvider` interface.
...
[#neighborhoodsEnumerateVsSample]
==== When to enumerate and when to sample?

Operations such as `join()` are available both on <<#neighborhoodsEnumeration,enumerating stream>> and <<#neighborhoodsSampling,sampling streams>>.
The decision between enumeration and sampling is effectively
...
- Sampling streams, on the other hand,
...
In those cases,
just-in-time capabilities provided by sampling streams are the way to go,
even if they result in comparatively slower solver performance;
a slower optimization beats no optimization at all.

Some capabilities (such as pillars) may only be provided as sampling streams,
...
and convert to sampling if/when you see the solver exceeding available memory.
...
to the <<neighborhoodsSampling,sampling layer>> below, or `asCachedDataset()`, handing you the dataset handle to resolve and
sample yourself. Both terminal operations keep the dataset in memory and up to date;
...
found in the `greycos.solver.core.preview.api.neighborhood.stream.joiner.NeighborhoodsJoiners` class.
More advanced selection strategies,
such as xref:advanced-features/advanced-features.adoc#nearbySelection[nearby selection],
are likely to materialize here in the future as well.

Should these methods of sampling be too restrictive for you,
refer to <<neighborhoodsIteratorBased,iterator-based neighborhoods>>
which give you full control over what is sampled and how,
...
public class BestShiftMoveProvider implements MoveProvider<Roster> {
    ...
    public BestShiftMoveProvider(PlanningVariableMetaModel<Roster, Shift, Employee> variableMetaModel) {
...
mirroring `pick()` but without committing to the declarative sampling approach.
...
which exposes `size()`, `iterator(RandomGenerator)` and `exhaustiveIterator(RandomGenerator)` -
 the same random-walk sampling used throughout Move Streams.
...
and application code must not rely on an exact move sequence.
This does not remove solver reproducibility with fixed seeds, identical inputs and configuration,
and count-based termination; sampling changes between versions may change the resulting trajectory.
...
to enumerate and sample moves from a move provider in isolation,
...
var entityMetaModel = solutionMetaModel.genuineEntity(Lesson.class);
var evaluator = NeighborhoodTester.build(new SwapMoveProvider<>(entityMetaModel), solutionMetaModel);
var context = evaluator.using(solution);
context.producesAllOf(Moves.swap(entityMetaModel.basicVariable(), lessonA, lessonB)); // Assert a specific move exists.
context.producesNoneOf(Moves.swap(entityMetaModel.basicVariable(), lessonA, lessonA)); // Assert a move never exists.
----

The API requires:

1. **Move provider**: An implementation of `greycos.solver.core.preview.api.neighborhood.MoveProvider`
...
[source,java,options="nowrap"]
----
context.within(10_000).producesAllOf(Moves.swap(entityMetaModel.basicVariable(), lessonA, lessonB));
----

If you need to inspect a move's fields directly rather than compare it via `equals()`,
`getMovesAsStream()`/`getMovesAsIterator()` expose random sampling which may never end.
Bound stream consumption with `limit(...)`, or stop an iterator after a chosen number of draws.
Use the membership assertions to check expected moves within a sampling budget;
failure to observe a move does not prove that it is impossible.
...
[source,java,options="nowrap"]
----
var tester = NeighborhoodTester.build(new SwapMoveProvider<>(entityMetaModel), solutionMetaModel);
...
you need to implement the `greycos.solver.core.preview.api.neighborhood.NeighborhoodProvider` interface
...
The Neighborhoods API is part of Local Search and therefore operates on an initialized solution.
For an initially unassigned solution, configure a construction heuristic before Local Search.
...
GreyCOS enables neighborhood generation only when `neighborhoodProviderClass` is configured.
Enabling the preview feature alone preserves the existing move selectors.
A provider without an explicit move selector configuration supplies all Local Search moves;
configuring both combines the provider with those move selectors.
The combined selector weights each side by its number of neighborhood providers or leaf move selectors,
then draws within the selected side.

Sampling can repeatedly produce the same move. Empty datasets, pairs rejected by every filter,
and exhausted finite custom iterators stop producing moves;
active sampling must be bounded by the solver's termination and forager settings.
Indexed pairs without an additional filter are sampled uniformly.
Additional filters and overlapping `containingAnyOf` keys may retain selection bias;
filtered sampling also uses bounded retries and therefore cannot prove exhaustive coverage.
...
- Solver integration currently supports one genuine entity class with one genuine variable.
...
such as 2-opt, 3-opt and xref:optimization-algorithms/move-selector-reference.adoc#nearbySelection[nearby selection].
