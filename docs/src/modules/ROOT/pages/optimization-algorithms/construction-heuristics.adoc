...
Like <<firstFit,First Fit>>, but analyzes the "more difficult" planning entities first,
because they are less likely to fit in the leftovers.
It sorts the planning entities in descending order by any given metric,
meaning that "more difficult" planning entities are added earlier in the list while "less difficult" values are included later.

image::optimization-algorithms/construction-heuristics/firstFitDecreasingNQueens04.png[align="center"]

Requires the model
to support xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningEntitySorting[planning entity sorting].
...
Like <<firstFit,First Fit>>, but it analyzes the "weaker" planning values first,
because the "stronger" planning values are more likely to be able to accommodate later planning entities.
It sorts the planning values in ascending order by any given metric,
meaning that "weaker" planning values are added earlier in the list while "stronger" values are included later.

Requires the model
to support xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueSorting[planning value sorting].
...
Combines <<firstFitDecreasing,First Fit Decreasing>> and <<weakestFit,Weakest Fit>>.
It sorts the planning entities in descending order and the planning values in ascending order based on defined metrics.

Requires the model
to support xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningEntitySorting[planning entity sorting]
and xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueSorting[planning value sorting].
...
Like <<firstFit, First Fit>>, but it analyzes the "stronger" planning values first,
because the "stronger" planning values are more likely to have a lower soft cost to use.
So it sorts the planning values in descending order by any given metric,
meaning that "stronger" planning values are added earlier in the list.
...
Combines <<firstFitDecreasing, First Fit Decreasing>> and <<strongestFit, Strongest Fit>>.
It sorts the planning entities and the planning values in descending order based on defined metrics.
...
Allocate Entity From Queue is a versatile generic form of <<firstFit,First Fit>>,
...
----
  <constructionHeuristic>
    ...
    <valueSorterManner>ASCENDING_IF_AVAILABLE</valueSorterManner>
  </constructionHeuristic>
----

The `entitySorterManner` options are:

* ``DESCENDING``: Evaluate the planning entities in descending order based on a given metric.
This usually increases pruning (and therefore improves scalability).
Requires the model
to support xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningEntitySorting[planning entity sorting].
* `DESCENDING_IF_AVAILABLE` (default): If the model supports xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningEntitySorting[planning entity sorting], behave like ``DESCENDING``, else like ``NONE``.
* ``NONE``: Initialize the planning entities in original order.

The `valueSorterManner` options are:

* ``ASCENDING``: Evaluate the planning values in ascending order based on a given metric.
Requires the model
to support xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueSorting[planning value sorting].
* `ASCENDING_IF_AVAILABLE` (default): If the model supports xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueSorting[planning value sorting], behave like ``ASCENDING``, else like ``NONE``.
* ``DESCENDING``: Evaluate the planning values in descending order based on a given metric.
Requires the model to support xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueSorting[planning value sorting].
* ``DESCENDING_IF_AVAILABLE``: If the model supports xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueSorting[planning value sorting], behave like ``DESCENDING``, else like ``NONE``.
...
The default behavior of the construction heuristic for xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#mixedModels[mixed models] is
...
If the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] proves that this is not the case, there's a number of improvements that can be done:
...
Let the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] guide you.
====
...
xref:advanced-features/advanced-features.adoc#partitionedSearch[Partitioned Search] reduces the number of moves per step.
