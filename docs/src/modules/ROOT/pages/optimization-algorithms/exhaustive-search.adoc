...
GreyCOS Solver maximizes the score (because it supports combining negative and positive constraints). Therefore, for clarity, it uses different terms, as it would be confusing to use the term lower bound for a bound which is always higher.
====
...
* ``DESCENDING``: Evaluate the planning values in descending order based on a given metric. Requires the model to support xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueSorting[planning value sorting].
...
As shown in these time spent graphs from the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker],
