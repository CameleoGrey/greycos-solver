...
* ``DESCENDING``: Evaluate the planning values in descending order based on a given metric. Requires the model to support xref:using-greycos-solver/modeling-planning-problems.adoc#planningValueSorting[planning value sorting].
...
As shown in these time spent graphs from the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker],
