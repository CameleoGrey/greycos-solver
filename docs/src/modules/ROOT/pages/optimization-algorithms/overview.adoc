...
[#doesGreyCOSFindTheOptimalSolution]
=== Does GreyCOS Solver find the optimal solution?
...
Therefore, GreyCOS Solver focuses on finding the best solution in available time.
...
GreyCOS Solver supports three _families_ of optimization algorithms: Exhaustive Search, Construction Heuristics and Metaheuristics.
...
6+|  xref:optimization-algorithms/alns.adoc#alns[Adaptive Large Neighborhood Search] (ALNS): adaptive destroy/repair search from an initialized solution.
|===
...
. Next,
implement xref:using-greycos-solver/modeling-planning-problems.adoc#planningEntitySorting[planning entity sorting]
...
Use the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker]
...
Also benchmark xref:optimization-algorithms/alns.adoc#alns[Adaptive Large Neighborhood Search (ALNS)]
when changing several assignments together and rebuilding part of the solution suits the problem,
such as removing customers from routes and reinserting them.
ALNS is a separate phase that learns which destroy/repair operators earn rewards;
it can follow construction heuristics and be combined with Local Search in a phase sequence.
Compare solution quality and resource use on representative inputs under the same budget.
...
GreyCOS Solver combines optimization algorithms (metaheuristics, ...)
...
GreyCOS Solver applies __configuration by exception__, so all optimization algorithms have default parameter values.
...
If no phases are configured, GreyCOS Solver will default to a Construction Heuristic phase followed by a Local Search phase.
...
Configure xref:using-greycos-solver/running-the-solver.adoc#logging[logging] to display the log messages of each scope.
...
Switching xref:using-greycos-solver/running-the-solver.adoc#environmentMode[EnvironmentMode] can heavily impact when this termination ends.
...
You can use the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] to tweak them.
====
...
add the `customProperties` element and use xref:using-greycos-solver/configuration.adoc#customPropertiesConfiguration[custom properties]:
...
----
    <unionMoveSelector>
      <changeMoveSelector/>
      <swapMoveSelector/>
      <selectorProbabilityWeightFactoryClass>greycos.solver.core.impl.heuristic.selector.common.decorator.FairSelectorProbabilityWeightFactory</selectorProbabilityWeightFactoryClass>
    </unionMoveSelector>
----
...
** ``DESCENDING``: Sorts the planning entities in descending order based on a give metric (xref:using-greycos-solver/modeling-planning-problems.adoc#planningEntitySorting[planning entity sorting]). Requires that planning entity is annotated on the domain model.
+
[source,xml,options="nowrap"]
----
    <entitySelector>
...
** ``ASCENDING``: Sorts the planning values in ascending order based on a given metric (xref:using-greycos-solver/modeling-planning-problems.adoc#planningValueSorting[planning value sorting]). Requires that planning value is annotated on the domain model.
+
[source,xml,options="nowrap"]
----
    <valueSelector>
...
[NOTE]
====
Nearby selection is an advanced feature of GreyCOS Solver.
https://github.com/CameleoGrey/greycos-solver[Learn more about GreyCOS].
====

Read about nearby selection in the xref:advanced-features/advanced-features.adoc#nearbySelection[Nearby selection section]
of the xref:advanced-features/advanced-features.adoc[Advanced Features manual].
...
run a xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] __for a short amount of time__
and xref:using-greycos-solver/benchmarking-and-tweaking.adoc#writeTheOutputSolutionOfBenchmarkRuns[configure it to write the best solutions to disk].
...
GreyCOS Solver calls `AbstractMove.doMove(ScoreDirector)`, which calls `doMoveOnGenuineVariables(ScoreDirector)`.
...
GreyCOS Solver automatically filters out _non doable moves_ by calling the `isMoveDoable(ScoreDirector)` method on each selected move.
...
Always implement the `toString()` method to keep GreyCOS Solver's logs readable.
...
xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarkReportPickedMoveTypeBestScoreDiffOverTimeStatistic[picked move statistics]:
...
For xref:advanced-features/advanced-features.adoc#multithreadedIncrementalSolving[multi-threaded incremental solving],
...
Internally it often uses a mapping technique based on the xref:using-greycos-solver/modeling-planning-problems.adoc#planningId[planning ID].
...
(so the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] can tweak those parameters),
add the `moveListFactoryCustomProperties` element and use xref:using-greycos-solver/configuration.adoc#customPropertiesConfiguration[custom properties].
...
(so the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] can tweak those parameters),
add the `moveIteratorFactoryCustomProperties` element and use xref:using-greycos-solver/configuration.adoc#customPropertiesConfiguration[custom properties].
