...
. Next,
implement xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningEntitySorting[planning entity sorting]
...
Use the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker]
...
Configure xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#logging[logging] to display the log messages of each scope.
...
Switching xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#environmentMode[EnvironmentMode] can heavily impact when this termination ends.
...
You can use the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] to tweak them.
====
...
add the `customProperties` element and use xref:xref:using-greycos-solver/configuration.adoc.adoc#customPropertiesConfiguration[custom properties]:
...
----
    <unionMoveSelector>
      <changeMoveSelector/>
      <swapMoveSelector/>
      <selectorProbabilityWeightFactoryClass>ai.greycos.solver.core.impl.heuristic.selector.common.decorator.FairSelectorProbabilityWeightFactory</selectorProbabilityWeightFactoryClass>
    </unionMoveSelector>
----
...
** ``DESCENDING``: Sorts the planning entities in descending order based on a give metric (xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningEntitySorting[planning entity sorting]). Requires that planning entity is annotated on the domain model.
+
[source,xml,options="nowrap"]
----
    <entitySelector>
...
** ``ASCENDING``: Sorts the planning values in ascending order based on a given metric (xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningValueSorting[planning value sorting]). Requires that planning value is annotated on the domain model.
+
[source,xml,options="nowrap"]
----
    <valueSelector>
...
run a xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] __for a short amount of time__
and xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#writeTheOutputSolutionOfBenchmarkRuns[configure it to write the best solutions to disk].
...
xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarkReportPickedMoveTypeBestScoreDiffOverTimeStatistic[picked move statistics]:
...
Internally it often uses a mapping technique based on the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningId[planning ID].
...
(so the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] can tweak those parameters),
add the `moveListFactoryCustomProperties` element and use xref:xref:using-greycos-solver/configuration.adoc.adoc#customPropertiesConfiguration[custom properties].
...
(so the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] can tweak those parameters),
add the `moveIteratorFactoryCustomProperties` element and use xref:xref:using-greycos-solver/configuration.adoc.adoc#customPropertiesConfiguration[custom properties].
