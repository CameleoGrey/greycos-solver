...
Greycos Solver public APIs are backwards compatible,
...
and to take advantage of the newest features in Greycos Solver.
...
[%collapsible%open]
====
The `SelectionSorterWeightFactory` sorter class has been deprecated in favor of `ComparatorFactory`.
...
[%collapsible%open]
====
Previously, `flattenLast` mistakenly used object identity to determine when to create or retract tuples.
This was particularly inefficient when a mapping function was used with Java records, 
or value-based types such as `LocalDate`:
...
factory.forEach(Shift.class)
    .map(shift -> new Interval(shift.getStart(), shift.getEnd()))
    .flattenLast(interval -> List.of(interval))
    // ...
----

Since the `map` creates a new interval instance on every update, 
the `flattenLast` did a retract of the old instance,
followed by an insert of the new instance.

Now, `flattenLast` use object equality and `hashCode` to determine when to create or retract tuples.
For the example given above, since the different intervals are `equals` to each other, 
`flattenLast` will reuse the old interval and do an `update` on it.
For the vast majority of cases, this change is not noticeable;
if two objects `equals` each other, they are equivalent and therefore interchangeable.

However, if an object overrides `equals` and `hashCode` 
such that some property is not considered for `equals` or `hashCode`, 
and your constraints check that property after `flattenLast`, 
score corruption can occur since a different instance with a different value for that property can be used.
For example:

[source,java]
----
record IntervalEqualByStart(int start, int end) {
    @Override
...
factory.forEach(Shift.class)
    .map(shift -> new IntervalEqualByStart(shift.getStart(), shift.getEnd()))
    .flattenLast(interval -> List.of(interval))
    // "end" is not considered in IntervalEqualByStart,
    // so IntervalEqualByStart(1, 2) and IntervalEqualByStart(1, 3)
    // are equals to one another, causing flattenLast to reuse
    // IntervalEqualByStart(1, 2) (which is not accepted by this filter)
    // instead of using IntervalEqualByStart(1, 3) (which would be accepted by this filter)
    // if shift.getEnd() was changed from 2 to 3.
    .filter(interval -> interval.end() > 2)
    // ...
----

To fix this, ensure the objects returned by the iterable passed to `flattenLast` 
are not `equals` if any checked properties downstream are different.
(In other words: make sure that `equals` means equal.)
For instance:
...
xref:using-greycos-solver/modeling-planning-problems.adoc#planningModelSolutionInheritance[solution] and xref:using-greycos-solver/modeling-planning-problems.adoc#planningModelEntiyInheritance[entity] models.
...
All such methods were deprecated and will be removed in a future major version of Greycos Solver.
...
and will be removed in a future major version of Greycos Solver.
====
...
[%collapsible%open]
====
In Greycos Solver's logging, the term "Score calculation speed" has been replaced by "Move evaluation speed".
...
Before Greycos Solver 1.10.0, this would result in Construction Heuristics finishing with a negative `init` score.
Starting with Greycos Solver 1.10.0, this situation will result in a runtime exception.

Read more about xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariableAllowingUnassigned[explicitly allowing unassigned values].
====
...
[%collapsible%open]
====
Users of Enterprise Edition will soon need to authenticate to access Greycos's Maven Repository.

If you are a Greycos customer, a Greycos representative will reach out to you
to give you the necessary credentials, as well as sufficient time to make the necessary changes.

If you are not a Greycos customer and you wish to retain your access to the Enterprise Edition artifacts,
you can https://greycos.ai/contact[contact us] to start your evaluation.
There are https://greycos.ai/pricing[many benefits] to being a Greycos customer.
...
that the solver will look up entities by their xref:using-greycos-solver/modeling-planning-problems.adoc#planningId[planning ID].
If the solver doesn't find anything with that ID, it will throw an exception.

In a future version of _Greycos Solver_, we will remove the option of configuring the lookup strategy.
...
For example, the old artifact name `greycos-solver-quarkus-vehicle-routing-quickstart` became `vehicle-routing`.
...
.icon:info-circle[role=yellow] Several internal modules folded into `greycos-solver-core`
[%collapsible%open]
====
The following JAR files have been merged into `greycos-solver-core`:

- `greycos-solver-core-impl`,
- `greycos-solver-constraint-streams`.

`greycos-solver-core` was previously an empty module that served as an aggregator for the above modules.
Now it contains the source code for both modules directly.
The automatic module name for this module is `ai.greycos.solver.core`.
...
you will need to update the imports to point `ai.greycos.solver.core.impl.score.stream.bavet`.

Finally, with the folding of these modules into `greycos-solver-core`,
...
[%collapsible%open]
====
In some cases, especially if you've reused our https://github.com/CameleoGrey/greycos-quickstarts/tree/stable/java/food-packaging[Food Packaging quickstart], you may see your tests failing after the upgrade.
...
xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariableAllowingUnassigned[unassigned values in list variables], several methods in xref:constraints-and-score/score-calculation.adoc#constraintStreams[Constraint Streams]
...
====
To better align with the newly introduced support for
xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariableAllowingUnassigned[unassigned values in list variables], the `nullable` attribute of `@PlanningVariable` has been renamed to `allowsUnassigned`.
