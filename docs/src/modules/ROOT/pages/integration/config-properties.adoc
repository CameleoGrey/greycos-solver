...
A classpath resource to read the xref:xref:using-greycos-solver/configuration.adoc.adoc#solverConfiguration[solver configuration XML].
...
Enable xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#environmentMode[runtime assertions] to detect common bugs in your
...
{property_prefix}greycos.solver.{solver_name_prefix}domain-access-type::
How GreyCOS Solver should access the domain model.
See xref:xref:using-greycos-solver/configuration.adoc.adoc#domainAccess[the domain access section] for more details.
