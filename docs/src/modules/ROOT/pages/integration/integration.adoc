...
GreyCOS Solver's input and output data (the planning problem and the best solution) are plain old JavaBeans (POJOs), so integration with other Java technologies is straightforward.
For example:
...
* To expose the Solver as a REST service that reads the planning problem and responds with the best solution, annotate the domain POJOs with JAXB or Jackson annotations and hook the `Solver` into RESTEasy or a similar framework.

[#compatibilityMatrix]
== Compatibility matrix

GreyCOS Solver 2.x uses Java 21 as its bytecode baseline,
integrates with Quarkus 3.x, and targets Spring Boot 4.1.
Use Java 25 for native-image builds and CI parity with the project build.
...
[NOTE]
====
Do not confuse JPA's `@Entity` annotation with GreyCOS Solver's `@PlanningEntity` annotation.
They can appear both on the same class:

[source,java,options="nowrap"]
----
@PlanningEntity // GreyCOS Solver annotation
...
The `greycos-solver-jpa` jar provides a JPA score converter for every built-in score type.
...
which implies that when the solution is xref:using-greycos-solver/modeling-planning-problems.adoc#cloningASolution[planning cloned], they need to be cloned too.
Use an `@DeepPlanningClone` on each such problem fact class to enforce that:

[source,java,options="nowrap"]
----
@PlanningSolution // GreyCOS Solver annotation
@Entity // JPA annotation
...
[source,java,options="nowrap"]
----
@DeepPlanningClone // GreyCOS Solver annotation: Force the default planning cloner to planning clone this class too
@Entity // JPA annotation
public class Room {
...
Add a dependency to the `greycos-solver-jaxb` jar to take advantage of these extra integration features:
...
Add a dependency to the `greycos-solver-jackson` jar and register `GreyCOSJacksonModule`:
...
The standalone persistence and Spring integrations use Jackson 3 (`tools.jackson.*`).
The Quarkus extension uses the Jackson 2 API supplied by Quarkus (`com.fasterxml.jackson.*`)
and registers its module through the Quarkus integration.
Keep object mappers from these two API generations inside their respective module boundaries.
...
The `GreyCOSJacksonModule` fixes that, by using `HardSoftScoreJacksonSerializer`,
...
To use GreyCOS Solver with Quarkus, read the xref:quickstart/quarkus/quarkus-quickstart.adoc#quarkusQuickStart[Quarkus Java quick start].
If you are starting a new project, visit the https://code.quarkus.io/[code.quarkus.io] and select
the _GreyCOS AI constraint solver_ extension before generating your application.

[#integrationWithQuarkusVersionPolicy]
=== Supported Quarkus versions

GreyCOS tracks the latest Quarkus 3 release and validates its native integration on Java 25.
Quarkus supplies its own Jackson 2 stack; keep it separate from the standalone Jackson 3 persistence modules.
...
The `SolverConfig` resource is constructed by reading `application.properties` and the classpath. Therefore, Cotwin types
(solution, entities and constraint classes) and customized properties such as `spent-limit` for the
...
GreyCOS provides all the necessary resources for problem-solving and analysis. However, it is still possible to manually create and override the default managed resources. To create a custom `SolverManager`, use the `SolverFactory` resource.
...
application. GreyCOS identifies each setting and provides a specific managed resource, `SolverManager`.

==== Solver configuration properties

The configuration properties for multiple solvers are defined using the namespace `quarkus.greycos.solver.<solverName>`,
where `<solverName>` is the name of the related solver. The `<solverName>` property is only necessary when using multiple
solvers. For defining a single solver, refer to the xref:#integrationWithQuarkusProperties[GreyCOS configuration properties section].
...
quarkus.greycos.solver."fastSolver".termination.spent-limit=5s // <1>
quarkus.greycos.solver."regularSolver".termination.spent-limit=10s // <2>
----
...
[source,properties,options="nowrap"]
----
quarkus.greycos.solver."teacherSolver".solver-config-xml=teachersSolverConfig.xml // <1>
quarkus.greycos.solver."roomSolver".solver-config-xml=roomsSolverConfig.xml // <2>
----
...
To use GreyCOS Solver on Spring Boot, add the `greycos-solver-spring-boot-starter` dependency
and read the xref:quickstart/spring-boot/spring-boot-quickstart.adoc#springBootQuickStart[Spring Boot Java quick start].

[NOTE]
====
GreyCOS Solver Spring Boot Starter targets Spring Boot version 4.1.x.
====
...
import greycos.solver.core.api.solver.SolverManager;
import greycos.solver.core.config.solver.SolverManagerConfig;
import greycos.solver.spring.boot.autoconfigure.GreyCOSAutoConfiguration;
...
@Configuration
@Import(GreyCOSAutoConfiguration.class)
public class BeanProducer {
...
@Configuration
@Import(
    GreyCOSAutoConfiguration::class
)
class BeanProducer {
    @Bean
    @Primary
...
The configuration properties for multiple solvers are defined using the namespace `greycos.solver.<solverName>`,
where `<solverName>` is the name of the related solver. The `<solverName>` property is only necessary when using multiple
solvers. For defining a single solver, refer to the xref:#integrationWithSpringBootProperties[GreyCOS configuration properties section].
Following properties are supported:
...
----
# The solver "fastSolver" runs only for 5 seconds, and "regularSolver" runs for 10s
greycos.solver.fastSolver.termination.spent-limit=5s // <1>
greycos.solver.regularSolver.termination.spent-limit=10s // <2>
----
<1> Define a solver config named *fastSolver*.
<2> Define a solver config named *regularSolver*.
...
[source,properties,options="nowrap"]
----
greycos.solver.teacherSolver.solver-config-xml=teachersSolverConfig.xml // <1>
greycos.solver.roomSolver.solver-config-xml=roomsSolverConfig.xml // <2>
----
<1> Define a solver config named *teacherSolver*.
<2> Define a solver config named *roomSolver*.
...
GreyCOS Solver supports the https://openjdk.org/projects/jigsaw/spec/[Java Platform Module System (JPMS)].

When using GreyCOS Solver from code on the module path,
_export_ the packages that contain your domain objects, constraints and solver configuration, and
_open_ them to the GreyCOS Solver module(s) in your `module-info.java` file:

[source,java,options="nowrap"]
----
module org.acme.vehiclerouting {
    requires greycos.solver.core;
    ...

    exports org.acme.vehiclerouting.domain; // Domain classes
    exports org.acme.vehiclerouting.solver; // Constraints

    opens org.acme.vehiclerouting.domain to greycos.solver.core; // Domain classes
    opens org.acme.vehiclerouting.solver to greycos.solver.core; // Constraints
    ...
}
----

If this is not set up correctly, you will get errors. These usually mention the `unnamed module`
and explain which package export or opening is missing.

[source,options="nowrap"]
----
class org.acme.schooltimetabling.domain.Timetable$GreyCOS$MemberAccessor$Field$lessons
(in unnamed module @0x273444fe) cannot access class org.acme.schooltimetabling.domain.Timetable
(in module hello.world.school.timetabling) because module hello.world.school.timetabling does not
export org.acme.schooltimetabling.domain to unnamed module @0x273444fe
----
...
====
If you access non-exported classes, for example by running on the classpath, and GreyCOS later
changes or removes them, that is not considered a breaking change.

You can opt out of JPMS by running all JAR files on the classpath instead of using the module path.


[#integrationWithHumanPlanners]
== Integration with human planners (politics)

A good GreyCOS Solver implementation beats any good human planner for non-trivial datasets.
Many human planners fail to accept this, often because they feel threatened by an automated system.

But despite that, both can benefit if the human planner becomes the supervisor of GreyCOS Solver:
...
* *The human planner is always in control of GreyCOS Solver.*
...
GreyCOS Solver does not change them: it optimizes the planning around the enforcements made by the human.
If the human planner pins down all planning variables, he/she sidelines GreyCOS Solver completely.
...
Before sizing a GreyCOS Solver service, first understand the typical behaviour of a `Solver.solve()` call:

image::integration/sizingHardware.png[align="center"]

Understand these guidelines to decide the hardware for a GreyCOS Solver service:

* **RAM memory**: Provision plenty, but no need to provide more.
** The problem dataset, loaded before GreyCOS Solver is called, often consumes the most memory. It depends on the problem scale.
*** If this is a problem, review the domain class structure: remove classes or fields that GreyCOS Solver doesn't need during solving.
*** GreyCOS Solver usually has up to three solution instances: the internal working solution, the best solution and the old best solution (when it's being replaced). However, these are all a xref:using-greycos-solver/modeling-planning-problems.adoc#cloningASolution[planning clone] of each other, so many problem fact instances are shared between those solution instances.
...
except with xref:advanced-features/advanced-features.adoc#nearbySelection[nearby selection] and caching move selector,
...
** GreyCOS Solver is not a web server: a solver thread does not block (unlike a servlet thread), each one fully drains a CPU.
...
See https://github.com/CameleoGrey/greycos-solver[our blog archive] for the details of our experiments, which use our diverse set of examples.
Your mileage may vary.

* Operating System
** No experimentally proven advice yet (but prefer Linux anyway).
* JDK
** Version: Our benchmarks have consistently shown improvements in performance when comparing new JDK releases with their predecessors. It is therefore recommended using the latest available JDK. If you're interested in the performance comparisons of GreyCOS Solver running of different JDK releases, you can find them in the form of blog posts in https://github.com/CameleoGrey/greycos-solver[our blog archive].
** Garbage Collector: ParallelGC can be potentially between 5% and 35% faster than G1GC (the default). Unlike web servers, GreyCOS Solver needs a GC focused on throughput, not latency. Use `-XX:+UseParallelGC` to turn on ParallelGC.
* Logging can have a severe impact on performance.
** Debug logging `greycos.solver` can be between 0% and 15% slower than info logging. Trace logging can be between 5% and 70% slower than info logging.
