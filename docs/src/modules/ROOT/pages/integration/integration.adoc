...
[NOTE]
====
GreyCOS Solver Spring Boot Starter only supports Spring Boot version 3.2.x and up.
====
...
When using GreyCOS Solver from code on the modulepath (Java 9 and higher),
...
Otherwise GreyCOS Solver can't reach those classes or files, even if they are exported.
