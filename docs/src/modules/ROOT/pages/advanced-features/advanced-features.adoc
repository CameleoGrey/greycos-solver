...
captureless lambdas are accessed via a cached static field.
...
----
public class MyConstraintProvider implements ConstraintProvider {
    private static Predicate<Shift> $predicate1 = shift -> shift.getEmployee().getName().equals("Ann");
...
----
...
import ai.greycos.solver.core.api.ThrottlingBestSolutionEventConsumer;
