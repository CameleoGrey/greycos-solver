...
GreyCOS Solver design patterns are generic reusable solutions to common challenges in the model or architecture of projects that perform constraint solving. The design patterns in this section list and solve common design challenges.
...
because GreyCOS Solver cannot assign an extra skill to an employee.

. *If there are multiple relationships (or fields), check for xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowVariable[shadow variables]*.
...
The other side will become an xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#bidirectionalVariable[inverse relation shadow variable] later on.
...
[NOTE]
====
GreyCOS Solver does not currently support a `@PlanningVariable` annotation on a collection.
xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[Planning list variable] is not a means of achieving a many-to-one relationship;
...
it is impossible to know in advance how many shifts each employee will have before GreyCOS Solver solves the model
...
[#domainCotwinPattern]
== Domain/Cotwin pattern

The Domain/Cotwin pattern is a architectural approach that separates business logic from optimization logic.

This pattern splits your application into two distinct layers:

* *Domain model*: A Java object hierarchy that mirrors your business domain, handles persistence, validation, and business logic. It remains solver-agnostic with no GreyCOS annotations.

* *Cotwin model*: Plain Java POJOs with GreyCOS annotations (`@PlanningSolution`, `@PlanningEntity`, `@PlanningVariable`) optimized for solving.
...
Input files / API / DB
        |
        v
Domain model (Java object hierarchy)
        |
        |  Domain -> Cotwin mapper
        v
Cotwin model (PlanningSolution + PlanningEntities)
        |
        v
GreyCOS Solver
        |
        |  Best-solution event -> Domain updater
        v
Domain model (updated) -> persistence / API output
----

Key benefits:

* *Separation of concerns*: Business logic stays in the domain, optimization logic in the cotwin

* *Flexibility*: Domain can compute complex business metrics that are easy to interpret and verify

* *Maintainability*: Each layer has a single responsibility; the cotwin stays minimal, the domain stays solver-agnostic

* *Integration*: External systems interact with the domain model, while the solver works with the optimized cotwin

The domain model stores children in maps (`Map<Id, Child>`) for lookup and lists (`List<Id>`) for ordering, supporting replanning, pinning, continuous planning, and fact changes. Each object has a stable `id` and exposes parent IDs for aggregation in constraints.

The cotwin preserves hierarchy by storing parent IDs as immutable fields, enabling constraints to use `groupBy` and joins while maintaining a flat structure.

[NOTE]
====
GreyCOS does not provide a built-in Domain/Cotwin mapper; create explicit mappers in your application: one to convert Domain → Cotwin before solving, and another to apply Cotwin → Domain updates when best solutions arrive.
====
...
use a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowVariable[shadow variable] to calculate the time.

This pattern is implemented using either the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#chainedPlanningVariable[chained planning variable]
or the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[planning list variable].
The two modeling approaches are equivalent because they both allow GreyCOS Solver to order elements in sequences of variable lengths.
...
Using the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#chainedPlanningVariable[chained planning variable], planning entities are arranged in a recursive data structure, forming a chain, that ends with an anchor.
...
The starting time of an entity is usually a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowVariable[shadow variable].
...
GreyCOS Solver distributes planning values into planning entities’ xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningListVariable[planning list variable].
...
It lasts 52 minutes, therefore her second task starts at 8:52.
The starting time of an element is usually a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#shadowVariable[shadow variable].
...
Implement the automatic collapse in the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#customVariableListener[custom variable listener]
...
Implement the automatic delay in the xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#customVariableListener[custom variable listener]
...
There are two common usage patterns of GreyCOS Solver in the cloud:
