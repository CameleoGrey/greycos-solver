...
Explore our documentation and available models https://github.com/CameleoGrey/greycos-solver[here].
====

[#isThisClassAProblemFactOrPlanningEntity]
== Introduction

GreyCOS Solver gives you access to a range of building blocks to model the domain of your planning problem.
...
[NOTE]
====
If you have never tried GreyCOS Solver before, we recommend following our xref:quickStartOverview[Getting Started guides] for a practical introduction.
====
...
image:using-greycos-solver/modeling-planning-problems/classDiagramSchoolTimetabling.svg[align="center"]
////
...
*In GreyCOS Solver, all elements of the planning domain model are plain objects.* Load them from a database, an XML file, a data repository, a REST service, a noSQL cloud, ... (see xref:integration/integration.adoc#integration[integration]): it doesn't matter.
...
From an optimization perspective, GreyCOS Solver effectively only optimizes _genuine variables_.
...
image::using-greycos-solver/modeling-planning-problems/shadowVariableExample.svg[align="center"]
...
In addition, GreyCOS Solver also provides a way to xref:shadowVariable[implement shadow variables yourself].
...
A problem fact class does _not_ require any GreyCOS Solver specific code.
...
Therefore, when dealing with a messy (denormalized) legacy system, it can sometimes be worthwhile to convert the messy domain model into a GreyCOS Solver specific model first.
...
(such as xref:advanced-features/advanced-features.adoc#multithreadedIncrementalSolving[multi-threaded incremental solving]
and xref:responding-to-change/responding-to-change.adoc#realTimePlanning[real-time planning]),
GreyCOS Solver needs to map problem facts and planning entities to a unique ID.
GreyCOS Solver uses that ID to _rebase_ a move from one thread's solution state to another's.
...
or of a primitive type whose boxed type implements `Comparable`.
** The type must also provide a stable `equals()` and `hashCode()` contract.
** Prefer immutable identifier types such as `Long`, `String` or `UUID`, or primitive alternatives such as `long`.
...
----
<solver xmlns="https://greycos.ai/xsd/solver" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
xsi:schemaLocation="https://greycos.ai/xsd/solver https://greycos.ai/xsd/solver/solver.xsd">
...
...
NOTE: When GreyCOS Solver is xref:integration/integration.adoc#integration[integrated in Quarkus or Spring], the XML file is optional.
...
Consequently, this will also increase the time it takes GreyCOS Solver to find optimized solutions.
...
Planning entity equality must remain constant:

- Entities equal at the start of solving must remain equal.
- Entities not equal at the start of solving must remain different.
- Equal objects are treated as the same planning entity.

Therefore, `equals()` and `hashCode()` must not depend on planning variables, which change during solving.
Use <<planningId,`@PlanningId`>> to distinguish planning entities.
Pay special attention to generated `equals()` and `hashCode()` implementations,
such as Kotlin data classes or Lombok's `@EqualsAndHashCode`.

==== Planning entity implementations must be mutable

As GreyCOS Solver will mutate the <<planningVariable,genuine>> or <<shadowVariable,shadow>> _planning variables_ of the planning entity, the planning entity must be mutable.

As Java's `enum` and `record` types are immutable by design, these can not be used as planning entities.


[#planningEntitySorting]
=== [[planningEntityDifficulty]]Planning entity sorting

Some optimization algorithms are more efficient when planning entities are sorted based on a specific metric,
which helps estimate the relative difficulty of each planning entity.
For example: in bin packing bigger items are harder to fit.

[NOTE]
====
*Do not try to use planning entity order to implement a business constraint.*
...
Only consider adding planning entity sort order if it can make the solver more efficient.
====
...
[IMPORTANT]
====
Entities should be sorted in ascending order: "easy" entities are lower, "difficult" entities are higher.
For example, in bin packing: small item < medium item < big item.

Although most algorithms start in descending order, they just reverse the ordering.
====

_None of the current planning variable states should be used to compare planning entities._
...
If none are provided, GreyCOS Solver will attempt to auto-detect matching ``@ValueRangeProvider``s.
...
xref:using-greycos-solver/configuration.adoc#annotationAlternatives[Annotating the field] instead of the property works too:
...
GreyCOS Solver will automatically add the value `null` to the value range.
...
Furthermore, it must be a mutable `Collection` because once GreyCOS Solver starts initializing or changing genuine planning variables,
...
- <<shadowVariable,Register this class as a planning entity>>, otherwise GreyCOS Solver won't detect it and the shadow variable won't update.
...
otherwise GreyCOS Solver won't detect it and the shadow variable won't update.
...
GreyCOS Solver triggers a user-defined logic after all events are processed.
...
GreyCOS Solver triggers the user-defined logic in `updateWeightAndArrivalTime` at the end of the event lifecycle.
...
The value range annotated method can also be defined with a single parameter,
which must be of the same type as the solution class.
The method will have access to the current solution and allow for the dynamic generation of a list of values.
...
[NOTE]
====
A `ValueRangeProvider` that uses the solution as a parameter for the planning entity consumes less memory
compared to the previous approach,
as it does not require storing information at the entity level.
This also helps reduce the overhead of the xref:using-greycos-solver/modeling-planning-problems.adoc#cloningASolution[solution cloning],
which can become significant for huge data sets.
====
...
GreyCOS Solver will go over every ``@ValueRangeProvider``-annotated method or field which does not have an ``id`` property set,
...
[#planningValueSorting]
=== [[planningValueStrength]]Planning value sorting

Some optimization algorithms work a bit more efficiently
if the planning values are sorted according to a given metric,
which means they are more likely to satisfy a planning entity requirement.
For example: in bin packing bigger containers are more likely to fit an item.

[NOTE]
====
*Do not try to use planning value order to implement a business constraint.*
It will not affect the score function: if we have infinite solving time, the returned solution will be the same.
...
Only consider adding planning value sort order if it can make the solver more efficient.
====
...
the `comparatorClass` needs to implement a `Comparator` of a common superclass (for example ``Comparator<Object>``)
and be able to handle comparing instances of those different classes.
====

If the model uses a xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariable[list variable],
...
[IMPORTANT]
====
Values should be sorted in ascending order: "weaker" values are lower, and "stronger" values are higher.
...
image::using-greycos-solver/modeling-planning-problems/chainPrinciples.png[align="center"]
...
image::using-greycos-solver/modeling-planning-problems/chainCorrection.png[align="center"]
...
GreyCOS Solver will not violate that principle during planning, but the input must not violate it either.
====
...
Real-world planning problems often involve multiple types of decisions simultaneously.
A production line must both assign an operator (a single value) and sequence a list of jobs (an ordered collection).
A vehicle route must both plan the sequence of visits and, for each visit, independently select a fallback delivery point.

When a solution model combines xref:using-greycos-solver/modeling-planning-problems.adoc#planningVariable[basic] and xref:using-greycos-solver/modeling-planning-problems.adoc#planningListVariable[list] planning variables,
it is referred to as a *mixed model*.

Mixed models support three distinct patterns.

[#mixedModelsBothOnSingleEntity]
=== Both variable types on a single entity

A single planning entity can declare both a basic variable and a list variable.
This is appropriate when a resource requires both a single-value assignment and an ordered workload.

In the following example, each production `Line` must be assigned an `Operator` (basic variable)
and must also maintain an ordered list of `Job` objects to process (list variable).
The operator assignment drives skill-based constraints, while the job list drives throughput and scheduling constraints.
...
NOTE: With both variables on the same entity, you can pin the entire entity (using `@PlanningPin`) or partially pin the list (using `@PlanningPinToIndex`), but you cannot pin the basic assignment independently; if you need that, use xref:#mixedModelsMultipleEntities[multiple entities].

[#mixedModelsMultipleEntities]
=== Multiple entities with independent variable types

Different planning entities can each define their own variable type.
This is appropriate when decisions have different lifecycles or ownership, and need to be re-solved independently.

In the following example, each `Line` manages its own job sequence (list variable),
while `LineOperation` records the assigned operator (basic variable).
Separating these into distinct entities allows staff assignments to be re-solved
without disturbing the existing job schedules, and vice versa.
...
[#mixedModelsChildEntities]
=== Entities inside a list variable with their own basic variables

Items within a list variable can themselves be planning entities that carry additional basic variables.
This handles decisions that are scoped to individual list elements rather than to the list as a whole.

In the following example, each `Vehicle` maintains an ordered list of `Delivery` objects (list variable).
Each `Delivery` is also a planning entity that independently selects a `DropoffPoint` location (basic variable).
The vehicle route will optimize both the order of the visits and switch out the different potential drop-off locations.
...
GreyCOS Solver effectively only optimizes the genuine variables, and it assures that when a genuine variable changes, any dependent shadow variables are changed accordingly.
...
GreyCOS Solver provides various built-in shadow variable annotation that make configuring common shadow variables trivial.
...
These paths must follow 1 of 4 syntactic forms:
...
|"variableName"
|Refers to a variable (genuine or shadow) on the same planning entity. A planning list variable requires the element access form below.
...
|Planning List Element Access
|"values[].endTime"
|Refers to a declarative shadow variable (`endTime`) on each element currently assigned to the planning list variable (`values`) directly on the entity. The supplier is recomputed when list membership or an element's source shadow changes, including when the list becomes empty. The element shadow may be accessed through a fact.
|===
...
image::using-greycos-solver/modeling-planning-problems/alignmentShadowVariable.png[align="center"]
...
To update a shadow variable, GreyCOS Solver uses a ``VariableListener``.
...
image::using-greycos-solver/modeling-planning-problems/shadowVariableOrder.png[align="center"]
...
GreyCOS Solver guarantees that:
...
GreyCOS Solver does not guarantee the order in which the `after*()` methods are called for the _same_``VariableListener`` with different parameters (such as A1 and A2 in the example above), although they are likely to be in the order in which they were affected.

By default, GreyCOS Solver does not guarantee that the events are unique.
...
The GreyCOS Solver API includes a facility method that automatically updates shadow variables from a given model,
...
GreyCOS Solver needs to extract the entity instances from the solution instance.
...
GreyCOS Solver needs to extract the problem fact instances from the solution instance.
...
image::using-greycos-solver/modeling-planning-problems/solutionCloning.png[align="center"]
