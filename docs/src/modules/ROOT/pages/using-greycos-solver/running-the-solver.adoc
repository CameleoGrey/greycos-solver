[#useTheSolver]
[#runningGreyCOSSolver]
= Running GreyCOS Solver
:doctype: book
:sectnums:
:icons: font
...
xref:optimization-algorithms/overview.adoc#doesGreyCOSFindTheOptimalSolution[that best solution might or might not be an optimal solution].
...
is most likely xref:using-greycos-solver/modeling-planning-problems.adoc#cloningASolution[a planning clone] of the instance
...
* *xref:advanced-features/advanced-features.adoc#multithreadedIncrementalSolving[Multi-threaded incremental solving]*:
...
* *xref:advanced-features/advanced-features.adoc#partitionedSearch[Partitioned search]*:
...
** Use the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] during development to determine the algorithm that is the most appropriate on average.
* *Multitenancy*: solve different datasets in parallel.
** The xref:using-greycos-solver/running-the-solver.adoc#solverManager[`SolverManager`] can help with this.

image::using-greycos-solver/running-the-solver/multiThreadingStrategies.png[align="center"]
...
[NOTE]
====
**If an error happens, GreyCOS Solver normally fails fast**: it throws a subclass of `RuntimeException` with a detailed message to the calling code.
...
Both trace logging and debug logging cause congestion in xref:using-greycos-solver/running-the-solver.adoc#multithreadedSolving[multi-threaded solving] with most appenders,
see below.
...
----
+
Configure the logging level on the `ai.greycos.solver` package in your `logback.xml` file:
+
[source,xml,options="nowrap"]
----
<configuration>

  <logger name="ai.greycos.solver" level="debug"/>
...
[NOTE]
====
When running multiple solvers or a xref:using-greycos-solver/running-the-solver.adoc#multithreadedSolving[multi-threaded solver],
  ...
  <appender name="fileAppender" class="ch.qos.logback.classic.sift.SiftingAppender">
    ...
    <sift>
      <appender name="fileAppender.${tenant.name}" class="...FileAppender">
        <file>local/log/greycos-solver-${tenant.name}.log</file>
        ...
      </appender>
    </sift>
  </appender>
...
GreyCOS Solver exposes metrics through https://micrometer.io/[Micrometer] which you can use to monitor the solver. GreyCOS automatically connects to configured registries when it is used in Quarkus or Spring Boot. If you use GreyCOS with plain Java, you must add the metrics registry to the global registry.

.Prerequisites
* You have a plain Java GreyCOS Solver project.
...
. Open your monitoring system to view the metrics for your GreyCOS Solver project. The following metrics are exposed:
+
[NOTE]
====
...
====
+
* `greycos.solver.errors.total`: the total number of errors that occurred while solving since the start
of the measuring.
* `greycos.solver.solve.duration.active-count`: the number of solvers currently solving.
* `greycos.solver.solve.duration.seconds-max`: run time of the
longest-running currently active solver.
* `greycos.solver.solve.duration.seconds-duration-sum`: the sum of each active solver's solve duration. For example, if there are two active solvers, one running for three minutes and the other for one minute, the total solve time is four minutes.

=== Additional metrics

For more detailed monitoring, GreyCOS Solver can be configured to monitor additional metrics at a performance cost.
...
<solver xmlns="https://greycos.ai/xsd/solver" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <monitoring>
    ...
    <constraintMatchMetricSampleInterval>1</constraintMatchMetricSampleInterval>
    ...
  </monitoring>
  ...
</solver>
...
For heavy constraint match monitoring (`CONSTRAINT_MATCH_TOTAL_BEST_SCORE` and
`CONSTRAINT_MATCH_TOTAL_STEP_SCORE`), set `constraintMatchMetricSampleInterval`
to collect these metrics every N local search steps.
The default value is `1` (collect every step), and the solver always flushes
the latest value at local search phase end.

The following metrics are available:

- `SOLVE_DURATION` (default, Micrometer meter id: "greycos.solver.solve.duration"):
...
- `ERROR_COUNT` (default, Micrometer meter id: "greycos.solver.errors"):
Measures the number of errors that occur while solving.

- `SCORE_CALCULATION_COUNT` (default, Micrometer meter id: "greycos.solver.score.calculation.count"):
Measures the number of score calculations GreyCOS Solver performed.

- `MOVE_EVALUATION_COUNT` (default, Micrometer meter id: "greycos.solver.move.evaluation.count"):
Measures the number of move evaluations GreyCOS Solver performed.

- `PROBLEM_ENTITY_COUNT` (default, Micrometer meter id: "greycos.solver.problem.entities"):
Measures the number of entities in the problem submitted to GreyCOS Solver.

- `PROBLEM_VARIABLE_COUNT` (default, Micrometer meter id: "greycos.solver.problem.variables"):
Measures the number of genuine variables in the problem submitted to GreyCOS Solver.

- `PROBLEM_VALUE_COUNT` (default, Micrometer meter id: "greycos.solver.problem.values"):
Measures the approximate number of planning values in the problem submitted to GreyCOS Solver.

- `PROBLEM_SIZE_LOG` (default, Micrometer meter id: "greycos.solver.problem.size.log"):
Measures the approximate log 10 of the search space size for the problem submitted to GreyCOS Solver.

- `BEST_SCORE` (Micrometer meter id: "greycos.solver.best.score.*"):
Measures the score of the best solution GreyCOS Solver found so far.
There are separate meters for each level of the score.
For instance, for a `HardSoftScore`, there are `greycos.solver.best.score.hard.score` and `greycos.solver.best.score.soft.score` meters.

- `STEP_SCORE` (Micrometer meter id: "greycos.solver.step.score.*"):
Measures the score of each step GreyCOS Solver takes.
There are separate meters for each level of the score.
For instance, for a `HardSoftScore`, there are `greycos.solver.step.score.hard.score` and `greycos.solver.step.score.soft.score` meters.

- `BEST_SOLUTION_MUTATION` (Micrometer meter id: "greycos.solver.best.solution.mutation"):
Measures the number of changed planning variables between consecutive best solutions.

- `MOVE_COUNT_PER_STEP` (Micrometer meter id: "greycos.solver.step.move.count"):
Measures the number of moves evaluated in a step.

- `MOVE_COUNT_PER_TYPE` (Micrometer meter id: "greycos.solver.move.type.count"):
...
- `CONSTRAINT_MATCH_TOTAL_BEST_SCORE` (Micrometer meter id: "greycos.solver.constraint.match.best.score.*"):
Measures the score impact of each constraint on the best solution GreyCOS Solver found so far.
...
there are `greycos.solver.constraint.match.best.score.hard.score` and `greycos.solver.constraint.match.best.score.soft.score` meters with a tag "constraint.name=Minimize Cost".

- `CONSTRAINT_MATCH_TOTAL_STEP_SCORE` (Micrometer meter id: "greycos.solver.constraint.match.step.score.*"):
Measures the score impact of each constraint on the current step.
There are separate meters for each level of the score, with tags for each constraint.
For instance, for a `HardSoftScore` for a constraint "Minimize Cost",
there are `greycos.solver.constraint.match.step.score.hard.score` and `greycos.solver.constraint.match.step.score.soft.score` meters with a tag "constraint.name=Minimize Cost".

- `PICKED_MOVE_TYPE_BEST_SCORE_DIFF` (Micrometer meter id: "greycos.solver.move.type.best.score.diff.*"):
...
there are `greycos.solver.move.type.best.score.diff.hard.score` and `greycos.solver.move.type.best.score.diff.soft.score` meters with the tag `move.type=ChangeMove(Lesson.room)`.

- `PICKED_MOVE_TYPE_STEP_SCORE_DIFF` (Micrometer meter id: "greycos.solver.move.type.step.score.diff.*"):
Measures how much a particular move type improves the best solution.
There are separate meters for each level of the score, with a tag for the move type.
For instance, for a `HardSoftScore` and a `ChangeMove` for the room of a lesson,
there are `greycos.solver.move.type.step.score.diff.hard.score` and `greycos.solver.move.type.step.score.diff.soft.score` meters with the tag `move.type=ChangeMove(Lesson.room)`.

[#randomNumberGenerator]
== Random number generator

Many heuristics and metaheuristics depend on pseudorandom values for move selection,
score-tie resolution and probability-based acceptance.
The solver derives independent streams for move generation, move selection and acceptance from one root seed.
This prevents one component's random consumption from changing another component's sequence.

To change the root seed, specify `randomSeed`:
...
With a reproducible environment mode, the same seed and solver configuration produce the same stream assignment.
Child solvers and worker threads receive streams by stable index before they start,
so their random sequences do not depend on thread scheduling.
...
to simplify solving planning problems in REST and other server applications.
...
regardless of the xref:advanced-features/advanced-features.adoc#multithreadedSolving[`moveThreadCount`] of the solvers.
...
Additionally,
GreyCOS Solver provides xref:advanced-features/advanced-features.adoc#throttlingBestSolutionEvents[throttling of best solution events]
to help manage event frequency.
...
xref:using-greycos-solver/configuration.adoc[a configuration step] is necessary.
