...
is most likely xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#cloningASolution[a planning clone] of the instance
...
** Use the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] during development to determine the algorithm that is the most appropriate on average.
* *Multitenancy*: solve different datasets in parallel.
** The xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#solverManager[`SolverManager`] can help with this.
...
Both trace logging and debug logging cause congestion in xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#multithreadedSolving[multi-threaded solving] with most appenders,
see below.
...
[NOTE]
====
When running multiple solvers or a xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#multithreadedSolving[multi-threaded solver],
...
If you want to confirm this on your use case, use the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[benchmarker].
...
regardless of the xref:advanced-features/advanced-features.adoc#enterpriseMultithreadedSolving[`moveThreadCount`] of the solvers.
