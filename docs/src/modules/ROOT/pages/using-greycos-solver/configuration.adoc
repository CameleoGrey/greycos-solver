...
or a property on a `$$*$$Config` class in the package namespace ``greycos.solver.core.config``.
These `$$*$$Config` classes are the Java representation of the XML format.
They build the runtime components (of the package namespace ``greycos.solver.core.impl``)
