...
representing a xref:using-greycos-solver/modeling-planning-problems.adoc#planningProblemAndPlanningSolution[planning problem and solution].
...
*Greycos Solver makes it relatively easy to switch optimization algorithm(s) just by changing the configuration.* There is even a xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] which allows you to play out different configurations against each other and report the most appropriate configuration for your use case.
...
Custom properties are useful to tweak dynamic values through the xref:using-greycos-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker].
