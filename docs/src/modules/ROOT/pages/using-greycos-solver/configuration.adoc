...
representing a xref:xref:using-greycos-solver/modeling-planning-problems.adoc.adoc#planningProblemAndPlanningSolution[planning problem and solution].
...
*GreyCOS Solver makes it relatively easy to switch optimization algorithm(s) just by changing the configuration.* There is even a xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker] which allows you to play out different configurations against each other and report the most appropriate configuration for your use case.
...
or a property on a `$$*$$Config` class in the package namespace ``ai.greycos.solver.core.config``.
These `$$*$$Config` classes are the Java representation of the XML format.
They build the runtime components (of the package namespace ``ai.greycos.solver.core.impl``)
...
GreyCOS Solver needs to be told which classes in your domain model are planning entities,
...
GreyCOS Solver by default accesses your domain using reflection, which
will always work, but is slow compared to direct access.
Alternatively, you can configure GreyCOS Solver to access your domain
...
These restrictions do not apply when using GreyCOS Solver with Quarkus,
...
Custom properties are useful to tweak dynamic values through the xref:xref:using-greycos-solver/benchmarking-and-tweaking.adoc.adoc#benchmarker[Benchmarker].
