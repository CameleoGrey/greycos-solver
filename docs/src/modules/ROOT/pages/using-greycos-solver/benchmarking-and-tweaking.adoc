...
GreyCOS Solver supports several optimization algorithms, so you're probably wondering which is the best one?
...
Luckily, GreyCOS Solver includes a benchmarker, which allows you to play out different solver phases with different settings
against each other in development, so you can use the best configuration for your planning problem in production.

image::using-greycos-solver/benchmarking-and-tweaking/benchmarkOverview.png[align="center"]
...
=== Add a dependency on `greycos-solver-benchmark`

The benchmarker is in a separate artifact called ``greycos-solver-benchmark``.
...
----
    <dependency>
      <groupId>ai.greycos.solver</groupId>
      <artifactId>greycos-solver-benchmark</artifactId>
    </dependency>
----

This is similar for Gradle, Ivy and Buildr.
The version must be exactly the same as the `greycos-solver-core` version used (which is automatically the case if you import ``greycos-solver-bom``).
...
<plannerBenchmark xmlns="https://greycos.ai/xsd/benchmark" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    xsi:schemaLocation="https://greycos.ai/xsd/benchmark https://greycos.ai/xsd/benchmark/benchmark.xsd">
...
the entire Benchmarker does not fail-fast (unlike everything else in GreyCOS Solver).
...
The `SolutionFileIO` interface is in the `greycos-persistence-common` jar (which is a dependency of the `greycos-solver-benchmark` jar).
...
Benchmark logging is configured like xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#logging[solver logging].
  ...
  <appender name="fileAppender" class="ch.qos.logback.classic.sift.SiftingAppender">
    ...
    <sift>
      <appender name="fileAppender.${subSingleBenchmark.name}" class="...FileAppender">
        <file>local/log/greycosBenchmark-${subSingleBenchmark.name}.log</file>
        ...
      </appender>
    </sift>
  </appender>
...
image::using-greycos-solver/benchmarking-and-tweaking/bestScoreSummary.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/bestScoreDistributionSummary.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/bestScoreStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/letTheBestScoreStatisticGuideYou.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/stepScoreStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/scoreCalculationSpeedStatistic.png[align="center"]


NOTE: The initial high calculation speed is typical during solution initialization:
...
image::using-greycos-solver/benchmarking-and-tweaking/moveEvaluationSpeedStatistic.png[align="center"]


NOTE: The initial high evaluation speed is typical during solution initialization:
it's far easier to calculate the score of a solution if only a handful planning entities have been initialized,
than when all the planning entities are initialized.
...
image::using-greycos-solver/benchmarking-and-tweaking/moveCountPerTypeStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/bestSolutionMutationStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/moveCountPerStepStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/memoryUseStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/constraintMatchTotalBestScoreStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/constraintMatchTotalStepScoreStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/pickedMoveTypeBestScoreDiffStatistic.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/pickedMoveTypeStepScoreDiffStatistic.png[align="center"]
...
* ``AUTO``: Let GreyCOS Solver decide how many benchmarks to run in parallel. This formula is based on experience. It's recommended to prefer this over the other parallel enabling options.
...
[NOTE]
====
This feature is independent of xref:advanced-features/advanced-features.adoc#multithreadedIncrementalSolving[multi-threaded incremental solving] (an advanced feature).
====
...
the benchmarker will automatically use a _different_ xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#randomNumberGenerator[`Random` seed] for every sub single run,
without losing reproducibility (for each sub single index) in xref:xref:using-greycos-solver/running-the-solver.adoc.adoc#environmentMode[EnvironmentMode] ``NO_ASSERT`` and lower.
====
...
image::using-greycos-solver/benchmarking-and-tweaking/benchmarkAggregator.png[align="center"]
...
image::using-greycos-solver/benchmarking-and-tweaking/benchmarkAggregatorScreenshot.png[align="center"]
...
[NOTE]
====
All the input reports which are being merged should have been generated with the same GreyCOS Solver version (excluding hotfix differences) as the ``BenchmarkAggregator``.
Using reports from different GreyCOS Solver major or minor versions are not guaranteed to succeed and deliver correct information,
because the benchmark report data structure often changes.
====

==== Using the Benchmark Aggregator programmatically

For automated or integrated workflows, you can use the BenchmarkAggregator methods to create aggregated reports programmatically. This approach is particularly useful in CI/CD pipelines, data science analytics, research studies, or when you need a comprehensive view of multiple benchmark runs as part of a larger application workflow.

===== Aggregating All Benchmark Results

The simplest approach is to aggregate all benchmark results found in the configured benchmark directory:
[source,java,options="nowrap"]
----
public File aggregateAllBenchmarks() {
    ...
    // Create and use the benchmark aggregator
    BenchmarkAggregator aggregator = new BenchmarkAggregator();

    // Aggregate all benchmark results - returns HTML report file
    return aggregator.aggregateBenchmarks(benchmarkConfig);
}
----

===== Discovering Available Benchmark Directories

To see what benchmark results are available for aggregation:
[source,java,options="nowrap"]
----
public List<String> getAvailableBenchmarks() {
    PlannerBenchmarkConfig benchmarkConfig = PlannerBenchmarkConfig.createFromXmlResource("benchmarkConfig.xml");
    BenchmarkAggregator aggregator = new BenchmarkAggregator();
    // Returns list of directory names that contain valid benchmark results
    return aggregator.getAvailableBenchmarkDirectories(benchmarkConfig);
}
----

===== Aggregating Selected Benchmark Results

For more control, you can aggregate specific benchmark results by selecting particular subdirectories:
[source,java,options="nowrap"]
----
public File aggregateSelectedBenchmarks() {
    ...
    List<String> availableDirectories =
            aggregator.getAvailableBenchmarkDirectories(benchmarkConfig);

    // Select specific directories to aggregate (example: latest two benchmarks)
    List<String> selectedDirectories = availableDirectories.stream()
            .sorted(Collections.reverseOrder())
            .limit(2)
            .collect(Collectors.toList());

    // Aggregate only selected benchmark results
    return aggregator.aggregateSelectedBenchmarks(benchmarkConfig, selectedDirectories);
}
----

===== Aggregating and Opening in Browser

For interactive analysis, you can aggregate benchmarks and automatically open the report in the default browser:
[source,java,options="nowrap"]
----
public File aggregateAndView() {
    PlannerBenchmarkConfig benchmarkConfig = PlannerBenchmarkConfig.createFromXmlResource("benchmarkConfig.xml");
    BenchmarkAggregator aggregator = new BenchmarkAggregator();

    // Aggregate all benchmarks and open HTML report in browser
    return aggregator.aggregateAndShowReportInBrowser(benchmarkConfig);
}
----

===== Method Overview

The BenchmarkAggregator provides the following public methods:

* `aggregateBenchmarks(PlannerBenchmarkConfig)` - Aggregates all benchmark results found in the configured directory
* `aggregateSelectedBenchmarks(PlannerBenchmarkConfig, List<String>)` - Aggregates benchmark results from specific subdirectories by name
* `getAvailableBenchmarkDirectories(PlannerBenchmarkConfig)` - Returns list of available benchmark subdirectories with valid results
* `aggregateAndShowReportInBrowser(PlannerBenchmarkConfig)` - Aggregates all benchmarks and opens the HTML report in the default browser

All aggregation methods return a `File` pointing to the generated HTML overview report that contains the aggregated benchmark results.

NOTE: All the input reports which are being merged should have been generated with the same GreyCOS Solver version as the `BenchmarkAggregator`.
Using reports from different GreyCOS Solver major or minor versions are not guaranteed to succeed and deliver correct information,
because the benchmark report data structure might change.
