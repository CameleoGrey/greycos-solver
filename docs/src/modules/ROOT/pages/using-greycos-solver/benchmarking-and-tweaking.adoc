...
----
    <dependency>
      <groupId>io.github.cameleogrey</groupId>
...
Benchmark logging is configured like xref:using-greycos-solver/running-the-solver.adoc#logging[solver logging].
...
the benchmarker will automatically use a _different_ xref:using-greycos-solver/running-the-solver.adoc#randomNumberGenerator[`Random` seed] for every sub single run,
without losing reproducibility (for each sub single index) in xref:using-greycos-solver/running-the-solver.adoc#environmentMode[EnvironmentMode] ``NO_ASSERT`` and lower.
====
