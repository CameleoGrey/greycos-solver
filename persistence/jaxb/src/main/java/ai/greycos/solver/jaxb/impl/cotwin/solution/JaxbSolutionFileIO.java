package ai.greycos.solver.jaxb.impl.cotwin.solution;
...
public class JaxbSolutionFileIO<Solution_> implements SolutionFileIO<Solution_> {
  ...
  public JaxbSolutionFileIO(Class... jaxbAnnotatedClasses) {
    try {
      ...
      throw new IllegalArgumentException(
          "JAXB creation for classes (" + Arrays.toString(jaxbAnnotatedClasses) + ") has failed.",
          e);
    }
  }
  ...
  public void write(Solution_ solution, File outputSolutionFile) {
    try (Writer writer =
        new OutputStreamWriter(new FileOutputStream(outputSolutionFile), "UTF-8")) {
      marshaller.marshal(solution, writer);
    } catch (IOException | JAXBException e) {
      throw new IllegalArgumentException(
          "Failed writing outputSolutionFile (" + outputSolutionFile + ").", e);
    }
  }
}
