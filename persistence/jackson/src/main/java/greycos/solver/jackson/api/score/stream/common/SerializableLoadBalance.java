...
record SerializableLoadBalance(BigDecimal unfairness) {
  ...
  static <Value_> SerializableLoadBalance of(LoadBalance<Value_> loadBalance) {
    ...
    // - It's possibly a very large map, that would end up in the JSON through default
    // justifications.
    // - Deserializing the map would be tricky, because the type of <Value_> is not known at
    // runtime.
