...
import static ai.greycos.solver.spring.boot.autoconfigure.util.LambdaUtils.rethrowFunction;
...
public class IncludeAbstractClassesEntityScanner extends EntityScanner {
            ...
            candidate -> {
              try {
                return (Class<? extends T>)
                    ClassUtils.forName(candidate.getBeanClassName(), context.getClassLoader())
                        .asSubclass(targetClass);
              } catch (ClassNotFoundException e) {
                throw new IllegalStateException(
                    "The %s class (%s) cannot be found."
                        .formatted(targetClass.getSimpleName(), candidate.getBeanClassName()),
                    e);
              }
            })
  ...
  @SafeVarargs
  public final List<Class<?>> findClassesWithAnnotation(
    ...
    return packages.stream()
        .flatMap(
            rethrowFunction(
                basePackage ->
                    findAllClassesUsingClassLoader(this.context.getClassLoader(), basePackage)
                        .stream()))
        .filter(clazz -> hasAnyFieldOrMethodWithAnnotation(clazz, annotations))
        .toList();
  }

  private boolean hasAnyFieldOrMethodWithAnnotation(
    ...
    return List.of(annotations).stream()
        .anyMatch(
            a ->
                fieldList.stream().anyMatch(f -> f.getAnnotation(a) != null)
  ...
  public boolean hasSolutionOrEntityClasses() {
    try {
      ...
      throw new IllegalStateException(
          "Scanning for @%s and @%s annotations failed."
              .formatted(
    ...
    try {
      ...
      while (!childEntityList.isEmpty()) {
        ...
        var childEntityClassList =
            findImplementingClassList(entityClass).stream()
      ...
      throw new IllegalStateException(
          "Scanning for @%s failed.".formatted(PlanningEntity.class.getSimpleName()), e);
    }
  }
