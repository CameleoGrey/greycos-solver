...
public class GreyCOSProperties {
  ...
  @NestedConfigurationProperty private SolverManagerProperties solverManager;

  /**
   * A classpath resource to read the solver configuration XML. Defaults to solverConfig.xml. If
   * this property isn't specified, that file is optional.
   */
  private String solverConfigXml;

  @NestedConfigurationProperty private Map<String, SolverProperties> solver;

  @NestedConfigurationProperty private BenchmarkProperties benchmark;
  ...
  public void setSolver(Map<String, Object> solver) {
    // Solver properties can be configured for a single solver or multiple solvers. The namespace
    // greycos.solver.*
    // defines the default properties for a single solver and greycos.solver.solver-name.* allows
    // configuring
    // multiple solvers
      ...
      if (!invalidKeySet.isEmpty()) {
        throw new IllegalStateException(
            """
                ...
                .formatted(
                    invalidKeySet, String.join(", ", SolverProperty.getValidPropertyNames())));
      }
