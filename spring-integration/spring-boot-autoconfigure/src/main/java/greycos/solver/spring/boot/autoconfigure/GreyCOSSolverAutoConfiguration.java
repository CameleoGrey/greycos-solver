...
import greycos.solver.spring.boot.autoconfigure.config.DiminishedReturnsProperties;
import greycos.solver.spring.boot.autoconfigure.config.GreyCOSProperties;
import greycos.solver.spring.boot.autoconfigure.config.SolverProperties;
import greycos.solver.spring.boot.autoconfigure.config.TerminationProperties;
...
public class GreyCOSSolverAutoConfiguration
  ...
  static final Predicate<Class<?>> UNIQUENESS_PREDICATE =
      clazz -> {
        ...
        return !pkg.startsWith("greycos.solver.core") || pkg.contains(".test");
      };
  ...
  private Map<String, SolverConfig> getSolverConfigMap() {
      ...
      LOG.warn(
          """
              ...
              .formatted(
                  ...
                  SpringBootApplication.class.getSimpleName(),
                  EntityScan.class.getSimpleName()));
      return Map.of();
    }
    ...
    if (greycosProperties.getSolver() == null || greycosProperties.getSolver().isEmpty()) {
      ...
      greycosProperties
          ...
          .forEach(
              solverName ->
                  solverConfigMap.put(
                      solverName, createSolverConfig(greycosProperties, solverName)));
    }
    ...
    solverConfigMap.forEach(
        (solverName, solverConfig) ->
            loadSolverConfig(entityScanner, greycosProperties, solverName, solverConfig));
  ...
  public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry)
    ...
    if (solverConfigMap.isEmpty()) {
      ...
      rootBeanDefinition
          ...
          .addGenericArgumentValue(solverXmlOutput.toString());
  ...
  private SolverConfig createSolverConfig(GreyCOSProperties greycosProperties, String solverName) {
    ...
    if (solverConfigXml.isEmpty()) {
      solverConfigXml = Optional.ofNullable(greycosProperties.getSolverConfigXml());
    }
  ...
  private void applyScoreDirectorFactoryProperties(
    ...
    if (solverProperties.getEnabledPreviewFeatures() != null) {
      solverConfig.setEnablePreviewFeatureSet(
          new HashSet<>(solverProperties.getEnabledPreviewFeatures()));
    }
  ...
  private static ScoreDirectorFactoryConfig defaultScoreDirectoryFactoryConfig(
      IncludeAbstractClassesEntityScanner entityScanner) {
    var scoreDirectorFactoryConfig = new ScoreDirectorFactoryConfig();
    scoreDirectorFactoryConfig.setEasyScoreCalculatorClass(
        entityScanner.findFirstImplementingClass(EasyScoreCalculator.class));
    scoreDirectorFactoryConfig.setConstraintProviderClass(
        entityScanner.findFirstImplementingClass(ConstraintProvider.class));
    scoreDirectorFactoryConfig.setIncrementalScoreCalculatorClass(
        entityScanner.findFirstImplementingClass(IncrementalScoreCalculator.class));

    return scoreDirectorFactoryConfig;
  }

  static void applyTerminationProperties(
      SolverConfig solverConfig, TerminationProperties terminationProperties) {
    ...
    if (terminationProperties != null) {
      ...
      if (terminationProperties.getUnimprovedSpentLimit() != null) {
        terminationConfig.overwriteUnimprovedSpentLimit(
            terminationProperties.getUnimprovedSpentLimit());
      }
      ...
      if (terminationProperties.getDiminishedReturns() != null) {
        applyDiminishedReturnsProperties(
            solverConfig, terminationProperties.getDiminishedReturns());
      }
    }
  }

  static void applyDiminishedReturnsProperties(
      SolverConfig solverConfig, DiminishedReturnsProperties diminishedReturnsProperties) {
    ...
    terminationConfig.setDiminishedReturnsConfig(
  ...
  private static void assertValidaPlanningVariables(
      IncludeAbstractClassesEntityScanner entityScanner) {
    var greycosFieldAnnotationList =
        entityScanner.findClassesWithAnnotation(PLANNING_ENTITY_FIELD_ANNOTATIONS);
    for (var clazz : greycosFieldAnnotationList) {
      assertValidPlanningVariables(clazz);
    }
  }

  private static void assertSolverConfigSolutionClasses(
      IncludeAbstractClassesEntityScanner entityScanner,
    ...
    try {
      ...
      throw new IllegalStateException(
          "Scanning for @%s annotations failed.".formatted(PlanningSolution.class.getSimpleName()),
          e);
    }
  }
  ...
  private static void assertSolverConfigEntityClasses(Map<String, SolverConfig> solverConfigMap) {
    ...
    for (var config : solverConfigMap.values()) {
      ...
      if (entityList == null || entityList.isEmpty()) {
        throw new IllegalStateException(
            "No classes were found with a @%s annotation."
                .formatted(PlanningEntity.class.getSimpleName()));
      }
  ...
  private static void assertSolverConfigConstraintClasses(
    ...
    assertSolverConfigsSpecifyScoreCalculatorWhenAmbigious(
        solverConfigMap, simpleScoreClassList, constraintScoreClassList, incrementalScoreClassList);
    assertNoUnusedScoreClasses(
        solverConfigMap, simpleScoreClassList, constraintScoreClassList, incrementalScoreClassList);
  }

  private static void assertSolverConfigsSpecifyScoreCalculatorWhenAmbigious(
      Map<String, SolverConfig> solverConfigMap,
      List<Class<? extends EasyScoreCalculator>> simpleScoreClassList,
      List<Class<? extends ConstraintProvider>> constraintScoreClassList,
      List<Class<? extends IncrementalScoreCalculator>> incrementalScoreClassList) {
    ...
    var errorMessage =
        "Multiple score calculator classes (%s) that implements %s were found in the classpath.";
  ...
  private static void assertNoUnusedScoreClasses(
      Map<String, SolverConfig> solverConfigMap,
      List<Class<? extends EasyScoreCalculator>> simpleScoreClassList,
      List<Class<? extends ConstraintProvider>> constraintScoreClassList,
      List<Class<? extends IncrementalScoreCalculator>> incrementalScoreClassList) {
    ...
    solverConfigWithUnusedSolutionClassList =
        constraintScoreClassList.stream()
            .map(Class::getName)
            .filter(
                className ->
                    solverConfigMap.values().stream()
    ...
    solverConfigWithUnusedSolutionClassList =
        incrementalScoreClassList.stream()
            .map(Class::getName)
            .filter(
                className ->
                    solverConfigMap.values().stream()
                        ...
                        .noneMatch(
                            c ->
                                c.getScoreDirectorFactoryConfig()
                                    ...
                                    .equals(className)))
            .toList();
    if (incrementalScoreClassList.size() > 1
