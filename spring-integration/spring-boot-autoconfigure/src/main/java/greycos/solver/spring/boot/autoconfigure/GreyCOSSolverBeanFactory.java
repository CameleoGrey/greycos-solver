...
import greycos.solver.core.api.score.stream.test.SingleConstraintVerification;
...
import greycos.solver.spring.boot.autoconfigure.config.SolverManagerProperties;
...
public class GreyCOSSolverBeanFactory implements ApplicationContextAware, EnvironmentAware {
  ...
  public <Solution_, Score_ extends Score<Score_>>
      SolutionManager<Solution_, Score_> solutionManager() {
  ...
  public <Solution_> ConstraintMetaModel constraintMetaModel() {
    ...
    if (scoreDirectorFactory
        instanceof
        AbstractConstraintStreamScoreDirectorFactory<Solution_, ?, ?> castScoreDirectorFactory) {
  ...
  class GreyCOSConstraintVerifierConfiguration {
    ...
    private record UnsupportedConstraintVerifier<
            ConstraintProvider_ extends ConstraintProvider, SolutionClass_>(
        String errorMessage) implements ConstraintVerifier<ConstraintProvider_, SolutionClass_> {

      @NonNull
      @Override
      public SingleConstraintVerification<SolutionClass_> verifyThat(
          @NonNull BiFunction<ConstraintProvider_, ConstraintFactory, Constraint>
              constraintFunction) {
