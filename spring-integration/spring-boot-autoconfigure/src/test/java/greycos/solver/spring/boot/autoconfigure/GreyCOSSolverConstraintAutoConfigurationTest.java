...
import greycos.solver.spring.boot.autoconfigure.config.SolverProperty;
import greycos.solver.spring.boot.autoconfigure.multiple.MultipleConstraintSpringTestConfiguration;
...
class GreyCOSSolverConstraintAutoConfigurationTest {
  ...
  void constraintVerifier() {
    contextRunner
        ...
        .run(
            context -> {
              ConstraintVerifier<TestdataSpringConstraintProvider, TestdataSpringSolution>
                  constraintVerifier = context.getBean(ConstraintVerifier.class);
  ...
  void constraintVerifierBavet() {
    contextRunner
        .withClassLoader(testFilteredClassLoader)
        .withPropertyValues(
            "greycos.solver-config-xml=greycos/solver/spring/boot/autoconfigure/bavetSolverConfig.xml")
        .run(
            context -> {
              ConstraintVerifier<TestdataSpringConstraintProvider, TestdataSpringSolution>
                  constraintVerifier = context.getBean(ConstraintVerifier.class);
  ...
  void readOnlyConcreteProviderClass() {
    AssertionsForClassTypes.assertThatCode(
            () ->
                multiConstraintProviderRunner.run(context -> context.getBean(SolverFactory.class)))
  ...
  void nodeSharingFailFastInNativeImage() {
    try (var nativeDetectorMock = Mockito.mockStatic(NativeDetector.class)) {
      ...
      fakeNativeWithNodeSharingContextRunner.run(
          context -> {
  ...
  void nodeSharingDisabledWorksInNativeImage() {
    try (var nativeDetectorMock = Mockito.mockStatic(NativeDetector.class)) {
      nativeDetectorMock.when(NativeDetector::inNativeImage).thenReturn(true);
      fakeNativeWithoutNodeSharingContextRunner.run(
          context -> {
