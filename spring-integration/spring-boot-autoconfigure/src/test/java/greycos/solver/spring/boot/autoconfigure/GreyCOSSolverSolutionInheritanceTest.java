...
import greycos.solver.core.testcotwin.inheritance.solution.baseannotated.childtooabstract.TestdataBothAnnotatedAbstractExtendedSolution;
import greycos.solver.spring.boot.autoconfigure.inheritance.solution.BothAnnotatedAbstractSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.solution.BothAnnotatedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.solution.MultipleInheritanceSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.solution.OnlyBaseAnnotatedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.solution.OnlyChildAnnotatedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.solution.ReplaceAnnotatedMemberSpringTestConfiguration;
...
class GreyCOSSolverSolutionInheritanceTest {
  ...
  public GreyCOSSolverSolutionInheritanceTest() {
    ...
    bothClassesAnnotatedConfiguredXmlContextRunner =
        new ApplicationContextRunner()
            .withPropertyValues(
                "greycos.solver.solver-config-xml=greycos/solver/spring/boot/autoconfigure/inheritance/bothClassAnnotatedConfig.xml")
            .withConfiguration(
                AutoConfigurations.of(
                    GreyCOSSolverAutoConfiguration.class, GreyCOSSolverBeanFactory.class))
            .withUserConfiguration(BothAnnotatedSpringTestConfiguration.class);
  ...
  void testBothClassesAnnotated() {
    assertThatCode(
        ...
        .hasMessageContaining(
            "Multiple classes", "found in the classpath with a @PlanningSolution annotation.");
  }

  /**
   * This test validates the behavior of the solver when both child and parent solution classes are
   * annotated with {@code @PlanningSolution}, and the base class is abstract.
   */
  @Test
  void testBothClassesAnnotatedBaseAbstract() {
    assertThatCode(
            () ->
                bothClassesAnnotatedAbstractContextRunner.run(
                    context -> {
                      var solverFactory = context.getBean(SolverFactory.class);
                      var problem =
                          TestdataBothAnnotatedAbstractExtendedSolution.generateSolution(3, 2);
                      var solution =
                          (TestdataBothAnnotatedAbstractExtendedSolution)
  ...
  void testBothClassesAnnotatedConfiguredXml() {
    assertThatCode(
            () ->
                bothClassesAnnotatedConfiguredXmlContextRunner.run(
                    context -> {
                      var solverFactory = context.getBean(SolverFactory.class);
                      var problem = TestdataBothAnnotatedExtendedSolution.generateSolution(3, 2);
                      var solution =
                          (TestdataBothAnnotatedExtendedSolution)
                              solverFactory.buildSolver().solve(problem);
                      assertNotNull(solution);
  ...
  /**
   * This test validates the behavior of the solver when only child class is annotated with
  ...
  void testOnlyChildClassAnnotated() {
    assertThatCode(
        ...
        .hasMessageContaining(
            "Maybe remove the annotated members ([entityList, score, valueList])");
  }
            ...
            () ->
                onlyBaseClassAnnotatedContextRunner.run(
                    context -> {
                      var solverFactory = context.getBean(SolverFactory.class);
                      var problem =
                          TestdataOnlyBaseAnnotatedExtendedSolution.generateSolution(3, 2);
                      var solution =
                          (TestdataOnlyBaseAnnotatedExtendedSolution)
                              solverFactory.buildSolver().solve(problem);
                      assertNotNull(solution);
  ...
  /**
   * This test validates the behavior of the solver when the child {@code @PlanningSolution}
   * replaces an existing annotated member.
   */
  @Test
  void testReplaceAnnotatedMember() {
    assertThatCode(
            () -> replaceMemberContextRunner.run(context -> context.getBean(SolverFactory.class)))
        .rootCause()
        .hasMessageContaining("Multiple classes")
        .hasMessageContaining("found in the classpath with a @PlanningSolution annotation.");
  }

  /** This test validates the behavior of the solver when multiple inheritance is applied. */
  @Test
  void testMultipleInheritance() {
    assertThatCode(
            () ->
                multipleInheritanceContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
        .hasMessageContaining("Multiple classes")
        .hasMessageContaining("found in the classpath with a @PlanningSolution annotation.");
