...
import greycos.solver.core.testcotwin.inheritance.entity.single.baseannotated.interfaces.addvar.TestdataAddVarInterfaceSolution;
import greycos.solver.core.testcotwin.inheritance.entity.single.baseannotated.interfaces.childnot.TestdataChildNotAnnotatedInterfaceSolution;
...
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.AddVarInterfaceSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.AddVarSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.BothAnnotatedInterfaceSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.BothAnnotatedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.MultipleBothAnnotatedInterfaceSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.MultipleBothAnnotatedMixedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.MultipleBothAnnotatedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.MultipleOnlyBaseAnnotatedInterfaceSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.MultipleOnlyBaseAnnotatedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.OnlyBaseAnnotatedInterfaceSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.OnlyBaseAnnotatedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.OnlyChildAnnotatedInterfaceSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.OnlyChildAnnotatedSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.ReplaceVarInterfaceSpringTestConfiguration;
import greycos.solver.spring.boot.autoconfigure.inheritance.entity.ReplaceVarSpringTestConfiguration;
...
class GreyCOSSolverEntityInheritanceTest {
  ...
  public GreyCOSSolverEntityInheritanceTest() {
    ...
    bothClassesAnnotatedBaseInterfaceContextRunner =
        new ApplicationContextRunner()
            .withConfiguration(
                AutoConfigurations.of(
                    GreyCOSSolverAutoConfiguration.class, GreyCOSSolverBeanFactory.class))
    ...
    onlyBaseClassAnnotatedBaseInterfaceContextRunner =
        new ApplicationContextRunner()
            .withConfiguration(
                AutoConfigurations.of(
                    GreyCOSSolverAutoConfiguration.class, GreyCOSSolverBeanFactory.class))
  ...
  void testBothClassesAnnotated() {
    assertThatCode(
            () ->
                bothClassesAnnotatedContextRunner.run(
                    context -> {
                      ...
                      var solution =
                          (TestdataBothAnnotatedSolution)
                              solverFactory.buildSolver().solve(problem);
  ...
  void testBothClassesAnnotatedAndBaseIsInterface() {
    assertThatCode(
            () ->
                bothClassesAnnotatedBaseInterfaceContextRunner.run(
                    context -> {
                      var solverFactory = context.getBean(SolverFactory.class);
                      var problem =
                          TestdataBothAnnotatedInterfaceSolution.generateSolution(3, 2, false);
                      var solution =
                          (TestdataBothAnnotatedInterfaceSolution)
                              solverFactory.buildSolver().solve(problem);
                      assertNotNull(solution);
                      assertThat(solution.getScore()).isEqualTo(SimpleScore.of(2));
                    }))
  ...
  /**
   * This test validates the behavior of the solver when only the parent class is annotated with
   * {@code @PlanningEntity}.
   */
  @Test
  void testOnlyBaseClassAnnotated() {
    assertThatCode(
            () ->
                onlyBaseClassAnnotatedContextRunner.run(
                    context -> {
                      var solverFactory = context.getBean(SolverFactory.class);
                      var problem = TestdataChildNotAnnotatedSolution.generateSolution(3, 2, false);
                      var solution =
                          (TestdataChildNotAnnotatedSolution)
                              solverFactory.buildSolver().solve(problem);
                      assertNotNull(solution);
                      assertThat(solution.getScore()).isEqualTo(SimpleScore.of(2));
                    }))
  ...
  /**
   * This test validates the behavior of the solver when only the parent class is annotated with
   * {@code @PlanningEntity} and the base entity is an interface.
   */
  @Test
  void testOnlyBaseClassAnnotatedBaseIsInterface() {
    assertThatCode(
            () ->
                onlyBaseClassAnnotatedBaseInterfaceContextRunner.run(
                    context -> {
                      var solverFactory = context.getBean(SolverFactory.class);
                      var problem =
                          TestdataChildNotAnnotatedInterfaceSolution.generateSolution(1, 2, false);
                      var solution =
  ...
  /**
   * This test validates the behavior of the solver when only the child class is annotated with
   * {@code @PlanningEntity}.
   */
  @Test
  void testOnlyChildClassAnnotated() {
    assertThatCode(
            () ->
                onlyChildClassAnnotatedContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
  ...
  void testOnlyChildClassAnnotatedBaseIsInterface() {
    assertThatCode(
            () ->
                onlyChildClassAnnotatedBaseInterfaceContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
        .hasMessageContaining(
            "is not annotated with @PlanningEntity but defines genuine or shadow variables.");
  ...
  /**
   * This test validates the behavior of the solver when the annotated child class adds new
   * variables.
   */
  @Test
  void testBothClassesAnnotatedAddNewVariable() {
    assertThatCode(
            () ->
                bothAnnotatedAddNewVarContextRunner.run(
                    context -> {
                      var solverFactory = context.getBean(SolverFactory.class);
                      var problem = TestdataAddVarSolution.generateSolution(3, 2, false);
                      var solution =
                          (TestdataAddVarSolution) solverFactory.buildSolver().solve(problem);
                      assertNotNull(solution);
                      assertThat(solution.getScore())
                          .isEqualTo(SimpleScore.of(4)); // value of 2 for each constraint
                    }))
        .doesNotThrowAnyException();
  }
  ...
  /**
   * This test validates the behavior of the solver when the annotated child class adds new
   * variables and the base entity is an interface.
   */
  @Test
  void testBothClassesAnnotatedAddNewVariableBaseIsInterface() {
    assertThatCode(
            () ->
                bothAnnotatedAddNewVarBaseInterfaceContextRunner.run(
                    context -> {
                      var solverFactory = context.getBean(SolverFactory.class);
                      var problem = TestdataAddVarInterfaceSolution.generateSolution(3, 2, false);
                      var solution =
                          (TestdataAddVarInterfaceSolution)
                              solverFactory.buildSolver().solve(problem);
                      assertNotNull(solution);
                      assertThat(solution.getScore())
                          .isEqualTo(SimpleScore.of(4)); // value of 2 for each constraint
  ...
  /**
   * This test validates the behavior of the solver when the annotated child class replaces an
   * existing variable.
   */
  @Test
  void testBothClassesAnnotatedReplaceVariable() {
    assertThatCode(
            () ->
                bothClassAnnotatedReplaceVarContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
  ...
  /**
   * This test validates the behavior of the solver when the annotated child class replaces an
   * existing variable and the base entity is an interface.
   */
  @Test
  void testBothClassesAnnotatedReplaceVariableBaseIsInterface() {
    assertThatCode(
            () ->
                bothClassAnnotatedReplaceVarBaseInterfaceContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
        .hasMessageContaining("redefines the genuine variables ([value]), which is not permitted.");
  }
   ...
   * {@code @PlanningEntity} and multiple inheritance is used.
   */
  @Test
  void testMultipleBothClassesAnnotated() {
    assertThatCode(
            () ->
                multipleBothClassesAnnotatedContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
        .hasMessageContaining(
  ...
  void testMultipleBothClassesAnnotatedBaseIsInterface() {
    assertThatCode(
            () ->
                multipleBothClassesAnnotatedBaseInterfaceContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
        .hasMessageContaining(
            "Remove either the entity classes or entity interfaces from the inheritance chain to create a single-level inheritance structure");
  ...
  /**
   * This test validates the behavior of the solver when only the base class is annotated with
   * {@code @PlanningEntity} and multiple inheritance is used.
   */
  @Test
  void testMultipleOnlyBaseClassAnnotated() {
    assertThatCode(
            () ->
                multipleOnlyBaseClassAnnotatedContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
        .hasMessageContaining(
            "Remove either the entity classes or entity interfaces from the inheritance chain to create a single-level inheritance structure");
  ...
  void testMultipleOnlyBaseClassAnnotatedBaseIsInterface() {
    assertThatCode(
            () ->
                multipleOnlyBaseClassAnnotatedBaseInterfaceContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
        .hasMessageContaining(
            "Remove either the entity classes or entity interfaces from the inheritance chain to create a single-level inheritance structure");
  ...
  void testMultipleBothClassesAnnotatedMixedPattern() {
    assertThatCode(
            () ->
                multipleBothClassesAnnotatedMixedPatternContextRunner.run(
                    context -> context.getBean(SolverFactory.class)))
        .rootCause()
