...
public class BeanUtil {
  ...
  public static ConstraintMetaModel buildConstraintMetaModel(SolverFactory<?> solverFactory) {
    if (Objects.requireNonNull(solverFactory)
        instanceof DefaultSolverFactory<?> defaultSolverFactory) {
      ...
      throw new IllegalStateException(
          "%s is not supported by the solver factory (%s)."
              .formatted(
                  ConstraintMetaModel.class.getSimpleName(), solverFactory.getClass().getName()));
    }
  }
