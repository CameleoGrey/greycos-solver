...
class GreyCOSBenchmarkProcessorEmptyAppWithInjectionTest {
  ...
  static final QuarkusUnitTest config =
      new QuarkusUnitTest()
          ...
          .overrideConfigKey(
              "quarkus.arc.unremovable-types", PlannerBenchmarkFactory.class.getName())
          .setArchiveProducer(() -> ShrinkWrap.create(JavaArchive.class).addClasses());

  @Test
  void emptyAppDoesNotCrash() {
    assertThatIllegalStateException()
        .isThrownBy(() -> Arc.container().instance(PlannerBenchmarkFactory.class).get())
        .withMessageContaining(
            "The " + PlannerBenchmarkFactory.class.getName() + " is not available as there are no");
  }
}
