== Contributing to GreyCOS Solver

This is an open source project, and you are more than welcome to contribute!

* Found an issue? https://github.com/CameleoGrey/greycos-solver/issues[Submit an issue.]
* Want to fix an issue or contribute an improvement? https://github.com/CameleoGrey/greycos-solver/discussions[Talk to us about your ideas] or just start coding:

. https://github.com/CameleoGrey/greycos-solver/fork[Fork it.]
...
. https://github.com/CameleoGrey/greycos-solver/compare[Create a new Pull Request.]
...
=== Build the GreyCOS Solver project

Use one of the following ways to build this project:

* :rocket: *build-fast*: `./mvnw clean install -Dquickly` skips any checks and code analysis (~1 min)

* :hammer: *build-normally*: `./mvnw clean install` runs tests, checks code style, skips documentation (~17 min)

* :receipt: *build-doc*: `./mvnw clean install` in the `docs` directory creates asciidoctor documentation `docs/target/html_single/index.html` (~2 min)

* :mechanical_arm: *build-all*: `./mvnw clean install -Dfull` runs all checks and creates documentation and distribution files (~20 min). It requires JDK 25+ to build properly.
...
+
* Type: Application
* Main class: `ai.greycos.solver.examples.app.GreyCOSExamplesApp`
...
* Use classpath of module: `greycos-solver-examples`
...
./mvnw clean install
----
For information about how to set up code style checks, see https://github.com/CameleoGrey/greycos-solver/blob/main/build/ide-config/ide-configuration.adoc[IDE Setup Instructions].
...
GreyCOS Solver is developed in accordance with the following general principles:
...
The codebase of GreyCOS Solver is structured into three conceptual parts:
...
Compatibility-breaking changes are only allowed in major versions of GreyCOS Solver, such as 2.0.0.
...
. *Fail Fast at runtime in assertion mode* if the detection performance cost is high. For example: If, after every low level iteration, the variable A needs to be equal to the square root of B, check it if and only if an assert flag is set to true (usually controlled by the xref:using-greycos-solver/running-the-solver.adoc#environmentMode[EnvironmentMode]).
...
One of the biggest challenges in multi-algorithm implementations (such as GreyCOS Solver)
