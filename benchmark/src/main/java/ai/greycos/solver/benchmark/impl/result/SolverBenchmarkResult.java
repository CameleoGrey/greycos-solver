...
public class SolverBenchmarkResult {
  ...
  // The average of the average is not just the overall average if the SingleBenchmarkResult's
  // timeMillisSpent differ
  ...
  /**
   * Does not call {@link SingleBenchmarkResult#accumulateResults(BenchmarkReport)}, because {@link
   * PlannerBenchmarkResult#accumulateResults(BenchmarkReport)} does that already on {@link
  ...
  public void accumulateResults(BenchmarkReport benchmarkReport) {
    ...
    standardDeviationDoubles =
        StatisticUtils.determineStandardDeviationDoubles(
            singleBenchmarkResultList, averageScore, getSuccessCount());
  }
  ...
  protected void determineTotalsAndAverages() {
    ...
    if (!firstNonFailure) {
      ...
      averageWorstScoreCalculationSpeedDifferencePercentage =
          totalWorstScoreCalculationSpeedDifferencePercentage / successCount;
    }
  }
  ...
  protected static Map<SolverBenchmarkResult, SolverBenchmarkResult> createMergeMap(
    ...
    for (Map.Entry<SolverBenchmarkResult, SolverBenchmarkResult> entry : mergeMap.entrySet()) {
      ...
      if (nameCountMap.get(oldResult.name) > 1) {
        newResult.name =
            oldResult.name + " (" + oldResult.getPlannerBenchmarkResult().getName() + ")";
      } else {
