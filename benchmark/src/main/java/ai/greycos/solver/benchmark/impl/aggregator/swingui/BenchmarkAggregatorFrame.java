package ai.greycos.solver.benchmark.impl.aggregator.swingui;
...
import ai.greycos.solver.benchmark.impl.aggregator.BenchmarkAggregator;
import ai.greycos.solver.benchmark.impl.aggregator.swingui.MixedCheckBox.MixedCheckBoxStatus;
...
import ai.greycos.solver.benchmark.impl.statistic.common.MillisecondsSpentNumberFormat;
import ai.greycos.solver.swing.impl.SwingUncaughtExceptionHandler;
import ai.greycos.solver.swing.impl.SwingUtils;

public class BenchmarkAggregatorFrame extends JFrame {

  /**
   * Reads an XML benchmark configuration from the classpath and uses that {@link
   * PlannerBenchmarkConfig} to do an aggregation.
   *
   * @param benchmarkConfigResource never null, same one as in {@link
  ...
  public static void createAndDisplayFromXmlResource(String benchmarkConfigResource) {
    PlannerBenchmarkConfig benchmarkConfig =
        PlannerBenchmarkConfig.createFromXmlResource(benchmarkConfigResource);
   ...
   *
   * @param templateResource never null, same one as in {@link
  ...
  public static void createAndDisplayFromFreemarkerXmlResource(String templateResource) {
    PlannerBenchmarkConfig benchmarkConfig =
        PlannerBenchmarkConfig.createFromFreemarkerXmlResource(templateResource);
    createAndDisplay(benchmarkConfig);
  }
  ...
  public static void createAndDisplay(PlannerBenchmarkConfig benchmarkConfig) {
    ...
    BenchmarkAggregatorFrame benchmarkAggregatorFrame =
        new BenchmarkAggregatorFrame(benchmarkAggregator);
  ...
  private JComponent createNoPlannerFoundTextField() {
    String infoMessage =
        "No planner benchmarks have been found in the benchmarkDirectory ("
  ...
  private JComponent createBenchmarkTreePanel() {
    ...
    benchmarkTreePanel.add(
        new JScrollPane(
            plannerBenchmarkResultList.isEmpty()
                ? createNoPlannerFoundTextField()
                : createCheckBoxTree(),
            JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
            JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED),
        BorderLayout.CENTER);
  ...
  private CheckBoxTree createCheckBoxTree() {
    ...
    resultCheckBoxTree.addTreeSelectionListener(
        e -> {
          ...
          if (treeSelectionPath != null) {
            ...
            renameNodeButton.setEnabled(
                checkBox.getBenchmarkResult() instanceof PlannerBenchmarkResult
                    || checkBox.getBenchmarkResult() instanceof SolverBenchmarkResult);
          }
        });
    resultCheckBoxTree.addMouseListener(
        new MouseAdapter() {
          @Override
          public void mousePressed(MouseEvent e) {
            // Enable button if checked singleBenchmarkResults exist
            generateReportButton.setEnabled(
                !resultCheckBoxTree.getSelectedSingleBenchmarkNodes().isEmpty());
          }
        });
  ...
  private void initPlannerBenchmarkResultList() {
    plannerBenchmarkResultList =
        benchmarkResultIO.readPlannerBenchmarkResultList(
            benchmarkAggregator.getBenchmarkDirectory());
    BenchmarkReportFactory reportFactory =
        new BenchmarkReportFactory(benchmarkAggregator.getBenchmarkReportConfig());
    for (PlannerBenchmarkResult plannerBenchmarkResult : plannerBenchmarkResultList) {
      plannerBenchmarkResult.accumulateResults(
          reportFactory.buildBenchmarkReport(plannerBenchmarkResult));
    }
  }
  ...
  private class GenerateReportAction extends AbstractAction {
    ...
    private void generateReport() {
      ...
      for (Map.Entry<SingleBenchmarkResult, DefaultMutableTreeNode> entry :
          resultCheckBoxMapping.entrySet()) {
        if (((MixedCheckBox) entry.getValue().getUserObject()).getStatus()
            == MixedCheckBoxStatus.CHECKED) {
      ...
      if (singleBenchmarkResultList.isEmpty()) {
        JOptionPane.showMessageDialog(
            parentFrame,
            "No single benchmarks have been selected.",
            "Warning",
        ...
        GenerateReportWorker worker =
            new GenerateReportWorker(parentFrame, singleBenchmarkResultList);
        worker.execute();
      }
    }
  }
  ...
  private class MoveNodeAction extends AbstractAction {
    ...
    public MoveNodeAction(boolean directionUp) {
      super(
          directionUp ? "Move up" : "Move down",
          new ImageIcon(
              BenchmarkAggregatorFrame.class.getResource(
    ...
    public void actionPerformed(ActionEvent e) {
      if (checkBoxTree.getSelectionPath() != null) {
        DefaultMutableTreeNode selectedNode =
            (DefaultMutableTreeNode) checkBoxTree.getSelectionPath().getLastPathComponent();
        DefaultMutableTreeNode parentNode = (DefaultMutableTreeNode) selectedNode.getParent();
        if (parentNode != null) {
          DefaultMutableTreeNode immediateSiblingNode =
              directionUp
                  ? (DefaultMutableTreeNode) parentNode.getChildBefore(selectedNode)
  ...
  private class SwitchLevelsAction extends AbstractAction {
    ...
    public SwitchLevelsAction(boolean solverLevelFirst) {
      super(
          "Switch levels",
          new ImageIcon(BenchmarkAggregatorFrame.class.getResource("switchTree.png")));
  ...
  private class RenameNodeDialog extends JDialog {
    ...
    public RenameNodeDialog(final DefaultMutableTreeNode treeNode) {
      ...
      final JTextField benchmarkResultNameTextField =
          new JTextField(
              benchmarkResultTextFieldText == null
                  ? benchmarkResult.toString()
                  : benchmarkResultTextFieldText,
              30);
      mainPanel.add(benchmarkResultNameTextField, BorderLayout.WEST);
      AbstractAction renamedAction =
            ...
            public void actionPerformed(ActionEvent e) {
              ...
              if (newBenchmarkResultName == null || newBenchmarkResultName.isEmpty()) {
                ...
                if (benchmarkResult instanceof PlannerBenchmarkResult result) {
                  ...
                  ((DefaultTreeModel) checkBoxTree.getModel()).nodeChanged(treeNode);
                  solverBenchmarkResultNameMapping.put(result, newBenchmarkResultName);
                }
                dispose();
              }
            }
  ...
  private DefaultMutableTreeNode initBenchmarkHierarchy(boolean solverFirst) {
    DefaultMutableTreeNode parentNode =
        new DefaultMutableTreeNode(new MixedCheckBox("Planner benchmarks"));
    for (PlannerBenchmarkResult plannerBenchmarkResult : plannerBenchmarkResultList) {
      DefaultMutableTreeNode plannerNode =
          new DefaultMutableTreeNode(createPlannerBenchmarkCheckBox(plannerBenchmarkResult));
      ...
      if (solverFirst) {
        for (SolverBenchmarkResult solverBenchmarkResult :
          ...
          DefaultMutableTreeNode solverNode =
              new DefaultMutableTreeNode(createSolverBenchmarkCheckBox(solverBenchmarkResult));
          ...
          for (ProblemBenchmarkResult problemBenchmarkResult :
            ...
            DefaultMutableTreeNode problemNode =
                new DefaultMutableTreeNode(createProblemBenchmarkCheckBox(problemBenchmarkResult));
  ...
  private void initSingleBenchmarkNodes(
    ...
    for (SingleBenchmarkResult singleBenchmarkResult :
      ...
      if (singleBenchmarkResult.getProblemBenchmarkResult().equals(problemBenchmarkResult)) {
        DefaultMutableTreeNode singleBenchmarkNode =
            resultCheckBoxMapping.get(singleBenchmarkResult);
        if (singleBenchmarkNode != null) {
          problemNode.add(singleBenchmarkNode);
        } else {
          DefaultMutableTreeNode singleNode =
              new DefaultMutableTreeNode(createSingleBenchmarkCheckBox(singleBenchmarkResult));
  ...
  private MixedCheckBox createPlannerBenchmarkCheckBox(
      PlannerBenchmarkResult plannerBenchmarkResult) {
    String plannerBenchmarkDetail =
        String.format(
            "Average score: %s%n" + "Average problem scale: %d",
            plannerBenchmarkResult.getAverageScore(),
            plannerBenchmarkResult.getAverageProblemScale());
    return new MixedCheckBox(
        plannerBenchmarkResult.getName(), plannerBenchmarkDetail, plannerBenchmarkResult);
  }

  private MixedCheckBox createSolverBenchmarkCheckBox(SolverBenchmarkResult solverBenchmarkResult) {
    String solverCheckBoxName =
        solverBenchmarkResult.getName() + " (" + solverBenchmarkResult.getRanking() + ")";
    String solverBenchmarkDetail =
        String.format(
            "Total score: %s%n"
                + "Average score: %s%n"
            ...
            solverBenchmarkResult.getAverageTimeMillisSpent() == null
                ? ""
                : millisecondsSpentNumberFormat.format(
                    solverBenchmarkResult.getAverageTimeMillisSpent()));
  ...
  private MixedCheckBox createProblemBenchmarkCheckBox(
      ProblemBenchmarkResult problemBenchmarkResult) {
    String problemBenchmarkDetail =
        String.format(
            "Entity count: %d%n" + "Problem scale: %d%n" + "Used memory: %s",
  ...
  private MixedCheckBox createSingleBenchmarkCheckBox(SingleBenchmarkResult singleBenchmarkResult) {
    String singleCheckBoxName =
        singleBenchmarkResult.getName() + " (" + singleBenchmarkResult.getRanking() + ")";
    String singleBenchmarkDetail =
        String.format(
            "Score: %s%n" + "Used memory: %s%n" + "Time spent: %s",
  ...
  private class GenerateReportWorker extends SwingWorker<File, Void> {
    ...
    public GenerateReportWorker(
        BenchmarkAggregatorFrame parentFrame,
    ...
    protected File doInBackground() {
      return benchmarkAggregator.aggregateSelectedBenchmarksInUi(
          singleBenchmarkResultList, solverBenchmarkResultNameMapping);
    }
  ...
  private class ReportFinishedDialog extends JDialog {
    ...
    private JPanel createButtonPanel() {
      ...
      AbstractAction openBrowserAction =
            ...
            public void actionPerformed(ActionEvent event) {
              try {
                ...
                throw new IllegalStateException(
                    "Failed showing reportFile (" + reportFile + ") in the default browser.", e);
              }
              finishDialog();
            }
          };
      ...
      AbstractAction openFileAction =
          new AbstractAction("Show in files") {
            @Override
            public void actionPerformed(ActionEvent event) {
              try {
                ...
                throw new IllegalStateException(
                    "Failed showing reportFile (" + reportFile + ") in the file explorer.", e);
              }
              finishDialog();
            }
          };
