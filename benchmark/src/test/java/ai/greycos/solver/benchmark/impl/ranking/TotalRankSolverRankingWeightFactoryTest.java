...
import static ai.greycos.solver.core.testutil.PlannerAssert.assertCompareToEquals;
import static ai.greycos.solver.core.testutil.PlannerAssert.assertCompareToOrder;
...
class TotalRankSolverRankingWeightFactoryTest extends AbstractSolverRankingComparatorTest {
  ...
  void uninitializedSingleBenchmarks() {
    ...
    b.accumulateResults(benchmarkReport);
    // ranks, uninitialized variable counts and total scores are equal, A loses on worst score
    // (tie-breaker)
    assertCompareToOrder(aWeight, bWeight);
  }
  ...
  void differentScoreTypeOfSingleBenchmarks() {
    ...
    a.accumulateResults(benchmarkReport);
    solverBenchmarkResultList.add(a);
    // Scores with different number of levels are compared from the highest level, see
    // ResilientScoreComparator.compare
    addSingleBenchmarkWithHardSoftLongScore(
        b, bSingleBenchmarkResultList, -1000, 0, -30, 0, -1000, -1000);
    addSingleBenchmarkWithHardSoftLongScore(
        b, bSingleBenchmarkResultList, -400, 0, -30, 0, -1000, -1000);
    addSingleBenchmarkWithHardSoftLongScore(
        b, bSingleBenchmarkResultList, -30, 0, -30, 0, -1000, -1000);
    b.setSingleBenchmarkResultList(bSingleBenchmarkResultList);
    b.accumulateResults(benchmarkReport);
    solverBenchmarkResultList.add(b);
    var totalSingleBenchmarkResultList = new ArrayList<>(aSingleBenchmarkResultList);
