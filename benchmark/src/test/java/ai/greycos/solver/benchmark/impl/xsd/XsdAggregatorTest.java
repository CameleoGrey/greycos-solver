...
class XsdAggregatorTest {
  ...
  void validateByMergedXsd() throws IOException {
    ...
    String[] args =
        new String[] {
          solverXsd.getAbsolutePath(),
          benchmarkXsd.getAbsolutePath(),
    ...
    GenericJaxbIO<PlannerBenchmarkConfig> jaxbIO =
        new GenericJaxbIO<>(PlannerBenchmarkConfig.class);
    PlannerBenchmarkConfig plannerBenchmarkConfig;
    try (Reader reader =
        new InputStreamReader(getClass().getResourceAsStream(VALID_BENCHMARK_CONFIG_XML))) {
      plannerBenchmarkConfig =
          jaxbIO.readAndValidate(reader, readXmlSchemaFromFile(mergedBenchmarkXsd));
    }
  ...
  private File getResourceAsFile(String classPathResource) {
    try {
      ...
      throw new IllegalArgumentException(
          "Class path resource " + classPathResource + " is not a valid URI.", e);
    }
  }
  ...
  private Schema readXmlSchemaFromFile(File schemaFile) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          String.format("Failed to read schema from a file (%s).", schemaFile), e);
    }
  }
}
