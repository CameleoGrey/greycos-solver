...
public final class DefaultConstraintVerifier<
  ...
  /**
   * {@link ConstraintVerifier} is mutable, due to {@link
   * #withConstraintStreamImplType(ConstraintStreamImplType)}. Since this method can be run at any
   * time, possibly invalidating the pre-built score director factories, the easiest way of dealing
   * with the issue is to keep an internal immutable constraint verifier instance and clearing it
   * every time the configuration changes. The code that was using the old configuration will
   * continue running on the old instance, which will eventually be garbage-collected. Any new code
   * will get a new instance with the new configuration applied.
   */
