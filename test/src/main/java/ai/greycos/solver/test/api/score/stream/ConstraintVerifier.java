...
import ai.greycos.solver.test.impl.score.stream.DefaultConstraintVerifier;
...
public interface ConstraintVerifier<ConstraintProvider_ extends ConstraintProvider, Solution_> {
   ...
   * @param planningSolutionClass {@link PlanningSolution}-annotated class associated with the
   *     constraints
   * @param entityClasses at least one, {@link PlanningEntity} types used by the {@link
   *     PlanningSolution}
  ...
  static <ConstraintProvider_ extends ConstraintProvider, Solution_>
      @NonNull ConstraintVerifier<ConstraintProvider_, Solution_> build(
          ...
          @NonNull Class<Solution_> planningSolutionClass,
          @NonNull Class<?> @NonNull ... entityClasses) {
    requireNonNull(constraintProvider);
    var solutionDescriptor =
        SolutionDescriptor.buildSolutionDescriptor(
            requireNonNull(planningSolutionClass), entityClasses);
    return new DefaultConstraintVerifier<>(constraintProvider, solutionDescriptor);
  }

  /**
   * Uses a {@link SolverConfig} to build a {@link ConstraintVerifier}. Alternative to {@link
   * #build(ConstraintProvider, Class, Class[])}.
   *
  ...
  static <ConstraintProvider_ extends ConstraintProvider, Solution_>
      @NonNull ConstraintVerifier<ConstraintProvider_, Solution_> create(
    ...
    var scoreDirectorFactoryConfig =
        requireNonNull(nonNullSolverConfig.getScoreDirectorFactoryConfig());
    var constraintProviderClass =
        requireNonNull(scoreDirectorFactoryConfig.getConstraintProviderClass());
    var constraintProvider =
        ConfigUtils.newInstance(
            scoreDirectorFactoryConfig::toString,
            "constraintProviderClass",
            constraintProviderClass);
    ConfigUtils.applyCustomProperties(
        constraintProvider,
        "constraintProviderClass",
        scoreDirectorFactoryConfig.getConstraintProviderCustomProperties(),
  ...
  /**
   * All subsequent calls to {@link #verifyThat(BiFunction)} and {@link #verifyThat()} use the given
   * {@link ConstraintStreamImplType}.
   *
   * @return this
   ...
   */
  @NonNull SingleConstraintVerification<Solution_> verifyThat(
  ...
  @NonNull MultiConstraintVerification<Solution_> verifyThat();
}
