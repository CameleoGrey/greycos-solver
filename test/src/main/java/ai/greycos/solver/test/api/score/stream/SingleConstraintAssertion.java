...
public interface SingleConstraintAssertion {
   ...
   * the score.
   *
   * <p>This is equivalent to checking that there are neither penalties nor rewards.
   *
   ...
   * penalty.
   *
   * <p>Ignores the constraint weight: it only asserts the match weights. For example: a match with
   ...
   *
   * <p>An {@code int matchWeightTotal} automatically casts to {@code long} for {@link
   * HardSoftLongScore long scores}.
   *
   ...
   * number of penalties.
   *
   * <p>Ignores the constraint and match weights: it only asserts the number of matches For example:
   * if there are two matches with weight of {@code 10} each, this assertion will check for 2
   * matches.
   *
  ...
  /**
   * Asserts that the {@link Constraint} being tested, given a set of facts, results in any number
   * of penalties.
   *
   * <p>Ignores the constraint and match weights: it only asserts the number of matches For example:
   * if there are two matches with weight of {@code 10} each, this assertion will succeed. If there
   * are no matches, it will fail.
   *
   ...
   * rewards larger than given.
   *
   * <p>Ignores the constraint and match weights: it only asserts the number of matches. For
   * example: if there are two matches with weight of {@code 10} each, this assertion will check for
   * 2 matches.
   *
   * @param times at least 0, expected number of times that the constraint will penalize
   * @throws AssertionError when the expected penalty is not observed
