package greycos.solver.core.impl.bavet.quad.joiner;
...
@NullMarked
public final class DefaultQuadJoiner<A, B, C, D> extends AbstractJoiner<D>
  ...
  public DefaultQuadJoiner(
      TriFunction<A, B, C, ?> leftMapping, JoinerType joinerType, Function<D, ?> rightMapping) {
    super(rightMapping, joinerType);
    this.leftMappings = new TriFunction[] {leftMapping};
  }

  private DefaultQuadJoiner(
      TriFunction<A, B, C, ?>[] leftMappings,
  ...
  public static <A, B, C, D> DefaultQuadJoiner<A, B, C, D> merge(
      List<DefaultQuadJoiner<A, B, C, D>> joinerList) {
  ...
  public int hashCode() {
    return Objects.hash(
        Arrays.hashCode(joinerTypes),
        Arrays.hashCode(leftMappings),
