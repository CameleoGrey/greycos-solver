...
/**
 * Implements the concat operation. Concat cannot be implemented as a pass-through operation because
 * of two caveats:
 *
 * <ul>
 *   <li>It is possible to have the same {@link TupleSource} for both parent streams, in which case
 *       the exact same tuple can be inserted twice. Such a tuple should be counted twice
 *       downstream, and thus need to be cloned.
 *   <li>Because concat has two parent nodes, it must be a {@link TupleSource} (since all nodes have
 *       exactly one {@link TupleSource}, and the source tuple can come from either parent). {@link
 *       TupleSource} must produce new tuples and not reuse them, since if tuples are reused, the
 *       stores inside them get corrupted.
 * </ul>
 *
 * The {@link AbstractConcatNode} works by creating a copy of the source tuple and putting it into
 * the tuple's store. If the same tuple is inserted twice (i.e. when the left and right parent have
