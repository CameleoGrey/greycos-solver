 ...
 * PlanningEntity}s in the working {@link PlanningSolution}.
 *
 * <p>Usually the move holds a direct reference to each {@link PlanningEntity} of the {@link
 * PlanningSolution} which it will change when {@link #doMoveOnly(ScoreDirector)} is called. On that
 * change it should also notify the {@link ScoreDirector} accordingly.
 *
 * <p>A Move should implement {@link Object#equals(Object)} and {@link Object#hashCode()} for {@link
 * MoveTabuAcceptor}.
 *
 * <p>An implementation must extend {@link AbstractMove} to ensure backwards compatibility in future
 * versions. It is highly recommended to override {@link #getPlanningEntities()} and {@link
 * #getPlanningValues()}, otherwise the resulting move will throw an exception when used with Tabu
 * search.
 *
 * <p>To ease interoperability with Neighborhoods API, this interface extends {@link
 * greycos.solver.core.preview.api.move.Move}, giving the user an option to override certain methods
 * which they should not. Specifically, the following methods must not be overridden by the user, as
 * suitable default implementations are provided:
 *
 * <ul>
 ...
 * @see AbstractMove
 */
public interface Move<Solution_> extends greycos.solver.core.preview.api.move.Move<Solution_> {

  /**
   * Called before a move is evaluated to decide whether the move can be done and evaluated. A Move
   * is not doable if:
   *
   * <ul>
   *   <li>Either doing it would change nothing in the {@link PlanningSolution}.
   *   <li>Either it's simply not possible to do (for example due to built-in hard constraints).
   * </ul>
   *
   * <p>It is recommended to keep this method implementation simple: do not use it in an attempt to
   * satisfy normal hard and soft constraints.
   *
   * <p>Although you could also filter out non-doable moves in for example the {@link MoveSelector}
   ...
   * @return true if the move achieves a change in the solution and the move is possible to do on
   *     the solution.
   */
  boolean isMoveDoable(ScoreDirector<Solution_> scoreDirector);

  /**
   * Does the move (which indirectly affects the {@link ScoreDirector#getWorkingSolution()}). When
   * the {@link PlanningSolution working solution} is modified, the {@link ScoreDirector} must be
   * correctly notified (through {@link ScoreDirector#beforeVariableChanged(Object, String)} and
   ...
   * @deprecated Prefer {@link #doMoveOnly(ScoreDirector)} instead, undo moves no longer have any
   *     effect.
   */
  @Deprecated(forRemoval = true, since = "1.16.0")
  default Move<Solution_> doMove(ScoreDirector<Solution_> scoreDirector) {
    throw new UnsupportedOperationException(
        "Operation requires an undo move, which is no longer supported.");
  }
   ...
   * ScoreDirector} which is usually on another {@link Thread} or JVM. The new move returned by this
   * method translates the entities and problem facts to the destination {@link PlanningSolution} of
   * the destination {@link ScoreDirector}, That destination {@link PlanningSolution} is a deep
   * planning clone (or an even deeper clone) of the origin {@link PlanningSolution} that this move
   ...
   *
   * <p>Generally speaking, an implementation of this method iterates through every entity and fact
   * instance in this move, translates each one to the destination {@link ScoreDirector} with {@link
   * ScoreDirector#lookUpWorkingObject(Object)} and creates a new move instance of the same move
   * type, using those translated instances.
   *
   ...
   * planning entities} and {@link ProblemFactProperty problem facts}, which are usually declared by
   * a {@link PlanningId} on those classes. It must not depend on the state of the {@link
   ...
   *
   * @param destinationScoreDirector never null, the {@link ScoreDirector#getWorkingSolution()} that
   *     the new move should change the planning entity instances of.
   * @return never null, a new move that does the same change as this move on another solution
   *     instance
   */
  ...
  /** Do not override this default implementation. */
  @Override
  default greycos.solver.core.preview.api.move.Move<Solution_> rebase(Lookup lookup) {
   ...
   *
   * <p>The format is not formalized. Never parse the {@link String} returned by this method.
   *
   * @return never null
   */
