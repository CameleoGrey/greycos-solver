...
final class NumberEqualityUtil {
  ...
  /**
   * Return the correct predicate to compare two unknown subtypes of {@link Number}. Supports {@link
   * Integer}, {@link Long} and {@link BigDecimal}. If two numbers are not equal but still represent
   * the same point on the number line (such as int 1, long 1 and {@link BigDecimal#ONE} the
  ...
  public static <Score_ extends Score<Score_>> BiPredicate<Number, Number> getEqualityPredicate(
    ...
    if (expectedImpact instanceof Integer) {
      ...
      throw new IllegalStateException(
          "Impossible state: unknown impact type class ("
  ...
  private static <Score_ extends Score<Score_>> BiPredicate<Number, Number> getIntEqualityPredicate(
    ...
    if (actualImpactType == int.class) {
      ...
      return (Number expected, Number actual) ->
          (BigDecimal.valueOf(expected.intValue())).compareTo((BigDecimal) actual) == 0;
    } else {
      throw new IllegalStateException(
          "Impossible state: unknown numeric type ("
              + actualImpactType
              + ") for score definition ("
              + scoreDefinition.getClass()
              + ").");
    }
  }

  private static <Score_ extends Score<Score_>>
      BiPredicate<Number, Number> getLongEqualityPredicate(
          ScoreDefinition<Score_> scoreDefinition) {
    Class<?> actualImpactType = scoreDefinition.getNumericType();
    if (actualImpactType == int.class) {
      ...
      return (Number expected, Number actual) ->
          (BigDecimal.valueOf(expected.longValue())).compareTo((BigDecimal) actual) == 0;
    } else {
      throw new IllegalStateException(
          "Impossible state: unknown numeric type ("
              + actualImpactType
  ...
  private static <Score_ extends Score<Score_>>
      BiPredicate<Number, Number> getBigDecimalEqualityPredicate(
          ScoreDefinition<Score_> scoreDefinition) {
    Class<?> actualImpactType = scoreDefinition.getNumericType();
    if (actualImpactType == int.class) {
      return (Number expected, Number actual) ->
          ((BigDecimal) expected).compareTo(BigDecimal.valueOf(actual.intValue())) == 0;
    } else if (actualImpactType == long.class) {
      return (Number expected, Number actual) ->
          ((BigDecimal) expected).compareTo(BigDecimal.valueOf(actual.longValue())) == 0;
    } else if (actualImpactType == BigDecimal.class) {
      return (Number expected, Number actual) ->
  ...
  public static Comparator<Number> getComparison(Number expectedImpact) {
    ...
    throw new IllegalStateException(
        "Impossible state: unknown impact type class (%s) for impact (%s)."
