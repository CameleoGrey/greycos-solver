...
import greycos.solver.core.impl.constructionheuristic.placer.QueuedValuePlacer;
...
/**
 * The decorator returns a list of reachable entities for a specific value. It enables the creation
 * of a filtering tier when using entity-provided value ranges, ensuring only valid and reachable
 * entities are returned. An entity is considered reachable to a value if its value range includes
 * that value.
 *
 ...
 * </code>
 *
 * <p>This node is currently used by the {@link QueuedValuePlacer} to build an initial solution. To
 * illustrate its usage, let’s assume how moves are generated. First, a value is selected using a
 * value selector. Then, a change move selector generates all possible moves for that value to the
 * available entities and selects the entity and position with the best score.
 *
 * <p>Considering the previous process and the current goal of this node, we can observe that once a
 * value is selected, only change moves to reachable entities will be generated. This ensures that
 * entities that do not accept the currently selected value will not produce any change moves.
 *
...
public final class FilteringEntityByValueSelector<Solution_>
  ...
  public void phaseStarted(AbstractPhaseScope<Solution_> phaseScope) {
    ...
    this.entitiesSize =
        childEntitySelector
            .getEntityDescriptor()
            .extractEntities(phaseScope.getWorkingSolution())
            .size();
  ...
  private static class OriginalFilteringValueRangeIterator<Entity_, Value_>
    ...
    protected void initialize() {
      ...
      if (currentUpcomingValue == null) {
        ...
        var allValues =
            reachableValues.extractEntitiesAsList(Objects.requireNonNull(currentUpcomingValue));
