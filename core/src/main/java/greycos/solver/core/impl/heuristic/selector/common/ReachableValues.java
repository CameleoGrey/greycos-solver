...
/**
 * This class records the relationship between each planning value and all entities that include the
 * related value within its value range.
 *
...
public final class ReachableValues<Entity_, Value_> {
  ...
  public ReachableValues(
      ReachableValuesIndex<Entity_, Entity_> entitiesIndex,
      ReachableValuesIndex<Value_, ReachableItemValue<Entity_, Value_>> valuesIndex,
      Class<?> expectedSupertypeOfValue,
  ...
  public ReachableValues<Entity_, Value_> copy(ValueRangeSorter<Value_> sorterAdapter) {
    return new ReachableValues<>(
        entitiesIndex, valuesIndex, expectedSupertypeOfValue, sorterAdapter, acceptsNullValue);
  }
  ...
  public static final class ReachableItemValue<Entity_, Value_> {
    ...
    List<Value_> getRandomAccessValueList(
        List<ReachableItemValue<Entity_, Value_>> allValues,
        @Nullable ValueRangeSorter<Value_> valueRangeSorter) {
  ...
  @NullMarked
  public record ReachableValuesIndex<Value_, Type_>(
      Map<Value_, Integer> indexMap, List<Type_> allItems) {}
  ...
  private static final class BitSetIndexedList<Type_, Value_> extends AbstractList<Value_> {
    ...
    @SuppressWarnings("unchecked")
    private BitSetIndexedList(
        List<Type_> availableValueList,
        BitSet containedValueIndex,
