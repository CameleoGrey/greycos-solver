...
import greycos.solver.core.impl.bavet.common.tuple.TupleList;
import greycos.solver.core.impl.bavet.common.tuple.TupleState;
...
/**
 * This class has two direct children: {@link AbstractIndexedIfExistsNode} and {@link
 * AbstractUnindexedIfExistsNode}. The logic in either is identical, except that the latter removes
 * all indexing work. Therefore any time that one of the classes changes, the other should be
 * inspected if it could benefit from applying the change there too.
 *
 * @param <LeftTuple_>
 * @param <Right_>
 */
public abstract class AbstractIfExistsNode<LeftTuple_ extends Tuple, Right_>
    extends AbstractTwoInputNode<LeftTuple_, UniTuple<Right_>> implements DeferredSettleAware {

  protected final boolean shouldExist;

  // When isFiltering, these slots hold the head FilteringTracker of a hidden intrusive
  // doubly-linked list
  // (null = empty list). Links are stored in FilteringTracker's own prev/next fields — there is no
  // list object.
   ...
   * read) from "whenever a parent propagates in" to this node's own layer turn (see {@link
   * #prepareForSettle()}), closing the stale-activity race at its root instead of guarding against
   * it per read; the same mechanism {@code AbstractJoinNode} uses for filtering joins.
   ...
   *
   * <p>{@code pendingLeft}/{@code pendingRight} hold tuples whose cross-match is due; the marker
   * slots exist purely to make enqueueing idempotent. Both lists are drained in {@link
   * #prepareForSettle()}, calling {@link #reconcilePendingLeft(Tuple)}/{@link
   * #reconcilePendingRight(UniTuple)}; a brand-new counter has an empty tracker list and {@code
   * countRight == 0}, so it doubles as the insert path too.
   *
   * <p>Unlike the join sibling, which drains left before right, this node drains <b>right before
   ...
   * did for the same left tuple in between. That in turn is what makes {@link
   * #updateCounterRight(ExistsCounter, UniTuple)}'s pending-left skip safe: skipping a left tuple's
   * counter during the right pass only ever discards work the left tuple's own reconcile would have
   * superseded anyway. The skip is self-protecting either way; {@link #prepareForSettle()} clears
   * each side's marker as it drains, so swapping the two drains would just make the skip check see
   * no pending markers left to skip, degrading to correct-but-redundant work rather than an
  ...
  protected AbstractIfExistsNode(
      boolean shouldExist,
    ...
    this.inputStoreIndexLeftTrackerList =
        isFiltering ? tupleStorePositionTracker.reserveNextLeft() : -1;
    this.inputStoreIndexRightTrackerList =
        isFiltering ? tupleStorePositionTracker.reserveNextRight() : -1;
  ...
  /**
   * Removes {@code leftTuple} from the pending queue, if it is on it: a tuple can be retracted in
   * the same round it was enqueued, before its turn to reconcile ever comes. Must run before the
   * tuple's own store entries (composite key, counter entry, ...) are cleared, since a
   * still-pending entry left dangling would be read by {@link #prepareForSettle()} after those are
   * gone.
   */
   ...
   * reset {@code countRight} to zero, re-walk the whole right side, then fire one aggregate {@link
   * #updateCounterLeft(ExistsCounter)}. Reusing that logic is what makes this correct for a tuple
   * that was actually a fresh insert too: a brand-new counter has an empty tracker list, so every
   * match found is necessarily new. Implemented by each subclass because only it knows how to walk
   * the opposite side (indexed: the shared index/bucket; unindexed: the plain tuple list).
   */
  ...
  protected void updateCounterLeft(ExistsCounter<LeftTuple_> counter) {
    ...
    if (shouldExist ? counter.countRight > 0 : counter.countRight == 0) {
      ...
      switch (state) {
        ...
        default ->
            throw new IllegalStateException(
                "Impossible state: the counter (%s) has an impossible insert state (%s)."
      ...
      switch (state) {
        ...
        default ->
            throw new IllegalStateException(
                "Impossible state: The counter (%s) has an impossible retract state (%s)."
                    .formatted(counter, state));
      }
    }
  }
   ...
   * cross-removing each tracker from its right tuple's hidden list. No-op when !isFiltering. Walk
   * safety: {@link #removeRight(FilteringTracker)} only touches right-side links, so {@code
   * leftNext} is stable across the call.
   */
   ...
   * Walk safety: removeFromLeft only touches left-side links, so rightNext is stable across the
   * call.
   */
  ...
  protected void updateCounterLeft(ExistsCounter<LeftTuple_> counter, UniTuple<Right_> rightTuple) {
    ...
    // after every ancestor on both sides has completed its retract/update/insert turn for this
    // round;
  ...
  /**
   * Prepends tracker into the left tuple's hidden intrusive tracker list. The left tuple's store at
   * {@link #inputStoreIndexLeftTrackerList} holds the list head (null = empty).
   */
  ...
  /**
   * Prepends tracker into the right tuple's hidden intrusive tracker list. The right tuple's store
   * at {@link #inputStoreIndexRightTrackerList} holds the list head (null = empty).
   */
  ...
  protected void updateCounterRight(
      ExistsCounter<LeftTuple_> counter, UniTuple<Right_> rightTuple) {
    ...
    if (isPendingLeft(leftTuple)) {
      ...
      // recomputes its entire tracker set against the settled right side, including this right
      // tuple.
    ...
    // (the pending-left skip above already handles the one case where leftTuple's own reconcile
    // hasn't run yet this round),
  ...
  /**
   * A node in two hidden intrusive doubly-linked lists at once: one keyed on its left tuple
   * (counter.leftTuple) and one on its right tuple. The list heads live in the tuples'
   * inputStoreIndexLeftTrackerList / inputStoreIndexRightTrackerList store slots (null = empty
   * list). These fields are the links — no ElementAwareLinkedList or Entry is allocated.
   */
  @NullMarked
  protected static final class FilteringTracker<LeftTuple_ extends Tuple> {

    final ExistsCounter<LeftTuple_>
        counter; // -> leftTuple, for the left-keyed list and counter decrement
    final Tuple
        rightTuple; // for the right-keyed list; typed as Tuple (not UniTuple<Right_>) — only
    // getStore/setStore needed
    @Nullable
