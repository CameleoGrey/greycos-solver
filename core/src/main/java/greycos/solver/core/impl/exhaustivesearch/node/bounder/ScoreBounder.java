package greycos.solver.core.impl.exhaustivesearch.node.bounder;
...
public interface ScoreBounder<Score_ extends Score<Score_>> {
   ...
   *
   * @param scoreDirector never null, use {@link ScoreDirector#getWorkingSolution()} to get the
   *     working {@link PlanningSolution}
   * @param score never null, the {@link Score} of the working {@link PlanningSolution}
   * @return never null, never worse than the best possible {@link Score} we can get by initializing
   *     the uninitialized variables of the working {@link PlanningSolution}.
   * @see ScoreDefinition#buildOptimisticBound(InitializingScoreTrend, Score)
   */
  InnerScore<Score_> calculateOptimisticBound(
   ...
   *     working {@link PlanningSolution}
   * @param score never null, the {@link Score} of the working {@link PlanningSolution}
   * @return never null, never better than the worst possible {@link Score} we can get by
   *     initializing the uninitialized variables of the working {@link PlanningSolution}.
   * @see ScoreDefinition#buildPessimisticBound(InitializingScoreTrend, Score)
   */
  InnerScore<Score_> calculatePessimisticBound(
      ScoreDirector<?> scoreDirector, InnerScore<Score_> score);
}
