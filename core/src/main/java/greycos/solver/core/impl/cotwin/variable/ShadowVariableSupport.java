...
public final class ShadowVariableSupport<Solution_> implements SupplyManager {
  ...
  public static <Solution_> ShadowVariableSupport<Solution_> create(
      InnerScoreDirector<Solution_, ?> scoreDirector) {
    return new ShadowVariableSupport<>(scoreDirector, DefaultTopologicalOrderGraph::new);
  }
  ...
  // Supplies and trackers may also register after the working solution has been initialized.
  ...
  private final List<BasicVariableStateDemand<Solution_>> basicVariableStateDemandList =
      new ArrayList<>();
      ...
      if (inverseRelationShadowVariableDescriptor.getSourceVariableDescriptor()
        ...
        if (listVariableStateSupply != null) {
          processShadowVariableDescriptorWithListVariable(
              inverseRelationShadowVariableDescriptor, listVariableStateSupply);
        }
      } else {
  ...
  private @Nullable ListVariableStateSupply<Solution_, Object, Object>
      getListVariableStateSupply() {
    if (listVariableChangeHandlerList.isEmpty()) {
      return null;
    }
    return (ListVariableStateSupply<Solution_, Object, Object>)
        listVariableChangeHandlerList.getFirst();
  }
  ...
  private Supply createSupply(Demand<?> demand) {
    ...
    if (supply instanceof BasicVariableChangeHandler<?> handler) {
      ...
      resetWorkingSolutionIfSet(
          () -> basicVariableChangeHandler.resetWorkingSolution(scoreDirector));
