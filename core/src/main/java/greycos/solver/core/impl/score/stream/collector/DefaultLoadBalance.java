...
public final class DefaultLoadBalance<Balanced_> implements LoadBalance<Balanced_> {
  ...
  private void updateSquaredDeviation(long oldValue, long newValue) {
    // o' = o + (x_0'^2 - x_0^2) + (2 * (x_0s - x_0's') + 2 * (x_1 + x_2 + x_3 + ... + x_n)(s - s')
    // + (s'^2 - s^2))/n
    ...
    var squaredDeviationSecondTermNumerator =
  ...
  public @NonNull BigDecimal unfairness() {
    ...
    return switch (totalToBalanceCount) {
      ...
      case 1 ->
          BigDecimal.valueOf(squaredDeviationFractionNumerator + squaredDeviationIntegralPart)
              .sqrt(RESULT_MATH_CONTEXT);
      default -> { // Only do the final sqrt as BigDecimal, fast floating point math is good enough
        // for the rest.
        var tmp =
            (squaredDeviationFractionNumerator / (double) totalToBalanceCount)
                + squaredDeviationIntegralPart;
