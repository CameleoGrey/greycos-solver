...
public final class RandomUtils {
  ...
  /**
   * Return a value between 0 and {@code distribution.length}, with indices with larger values in
   * {@code distribution} being more likely.
   *
   ...
   * @param distribution Relative weight of the index being chosen. If one index's value is twice
   *     another index, that index is twice as likely to be chosen. Each value must be non-negative
   *     (0 is allowed).
   * @return An index between 0 and {@code distribution.length}, biased towards indices in
   *     distribution with larger values.
   */
  public static int sampleWithDistribution(
      RandomGenerator random, int distributionSum, int[] distribution) {
    ...
    var current = distribution[index];
    ...
    // [sum(i=0...index - 1)(distribution[i]), sum(i=0...index - 1)(distribution[i]) +
    // distribution[i]]
    // (start exclusive, end inclusive)
    // which is an interval of length distribution[index],
    // and thus have a distribution[index]/distributionSum chance of being chosen.
    return index;
  }
