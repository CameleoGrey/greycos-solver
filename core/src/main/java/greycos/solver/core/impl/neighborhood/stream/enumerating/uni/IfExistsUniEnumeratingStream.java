...
import greycos.solver.core.impl.bavet.common.AbstractIfExistsNode;
...
import greycos.solver.core.impl.neighborhood.stream.enumerating.common.IfExistsEnumeratingStream;
...
final class IfExistsUniEnumeratingStream<Solution_, A, B>
  ...
  private AbstractIfExistsNode<UniTuple<A>, B> getNode(
    ...
    if (indexerFactory.hasJoiners()) {
      if (isFiltering) {
        return new IndexedIfExistsUniNode<>(
            ...
            indexerFactory,
            downstream,
            filtering.toBiPredicate(sessionContext.solutionView()),
            tupleStorePositionTracker);
      } else {
        return new IndexedIfExistsUniNode<>(
            shouldExist, indexerFactory, downstream, tupleStorePositionTracker);
      }
    } else if (isFiltering) {
      return new UnindexedIfExistsUniNode<>(
          shouldExist,
          downstream,
          filtering.toBiPredicate(sessionContext.solutionView()),
          tupleStorePositionTracker);
