...
/**
 * Provide access to a {@link RandomGenerator}. Random state is reset each step in {@link
 * AbstractSolver#stepStarted} and {@link AbstractSolver#stepEnded}. As such, it does not matter how
 * many random calls is performed in a step, provided usage of the {@link RandomGenerator} is fully
 * deterministic. <br>
 * Moves and acceptors use different {@link RandomGenerator} since an acceptor is called when a move
 * has been evaluated, and in a multithreaded context, we can generate moves while waiting for a
 * prior move to be evaluated. Thus, if they used the same {@link RandomGenerator}, the moves
 * generated would not be deterministic, since sometimes there will be another random call since an
 * earlier move was evaluated, and sometimes there won't be.
 */
