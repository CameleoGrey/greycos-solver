...
/**
 * The implementation moves tuples directly into an either retract, update or insert queue, without
 * any option of moving between the queues. This is the most efficient implementation. It will throw
 * exceptions if a tuple is in the wrong queue, based on its state.
 *
 * @param <Tuple_>
 */
public final class StaticPropagationQueue<Tuple_ extends Tuple>
  ...
  public void propagateInserts() {
    ...
    if (!retractQueue.isEmpty()) {
      ...
      throw new IllegalStateException(
          "Impossible state: The update queue (%s) is not empty.".formatted(updateQueue));
    }
  }
}
