...
public final class UpdateShadowVariablesAction<Solution_> implements ChangeAction<Solution_> {
  ...
  private static final UpdateShadowVariablesAction<?> INSTANCE =
      new UpdateShadowVariablesAction<>();

  /**
   * The action is stateless ({@code Solution_} only occurs in a parameter position), so the single
   * shared instance serves every solution type.
   */
