...
/**
 * A unified index for an equal-bearing join/ifExists node, replacing the two parallel left/right
 * {@link EqualIndexer}s with ONE shared map from the equal-prefix key to a {@link Bucket}. It
 * assumes that all equal() joiners have been moved to the start of the query, also called
 * "equal-prefix".
 *
 * <p>An EQUAL match is {@code Objects.equals(leftKey, rightKey)} (exactly how a {@link HashMap}
 * groups entries) so matching left and right tuples land in the SAME bucket; co-location IS the
 * match. One top-level lookup per insert then replaces the second (cross-side) lookup the
 * two-indexer path does.
 *
 * <p>The map is keyed by {@code topEqualKeyUnpacker.apply(compositeKey)}: the whole composite key
 * when the join is pure-equal (a single merged level), or the key's first component when there is a
 * comparison/containing suffix. Callers pass the FULL composite key to every method; the unpacking
 * to the equal key happens here, and the suffix levels are handled by the bucket's per-side
 * downstream indexers.
 *
...
public final class FusedEqualIndex<L, R> {
  ...
  /**
   * @return true when this index has a comparison/containing suffix below the equal prefix (so
   *     different composite keys can still share a bucket); false for a pure-equal index (the
   *     bucket key is the whole composite key). The node uses this to gate {@link #isSameBucket}.
   */
  ...
  /**
   * Whether two composite keys resolve to the same bucket, i.e. share the same equal prefix. Used
   * by the node on a changed-key update to reuse the cached bucket (skipping a top-level lookup and
   * a bucket drop/recreate) when only the suffix changed.
   *
   * <p>Only meaningful for {@link #hasSuffix()} indexes (the node gates on it); for a pure-equal
   * index the equal key is the whole composite key, so this coincides with the full-key equality
   * the caller has already ruled out. {@code Objects.equals} because the equal-prefix component may
   * be null (a nullable planning variable feeding the equal joiner; the bucket map permits a null
   * key).
   */
  ...
  /**
   * Returns the bucket for the equal key of the given composite key, creating it if absent. This is
   * the single top-level lookup of an insert.
   */
  ...
  /**
   * Drops the bucket from the map iff both of its sides are empty. A bucket holding a live tuple on
   * either side is never reclaimed, so a cached reference to it (kept on a tuple by the node) never
   * goes stale.
   */
  ...
  /**
   * Co-locates the left and right tuples that share an equal-prefix key inside a {@link
   * FusedEqualIndex}. Each side's downstream {@link Indexer} is allocated lazily on first {@link
   * #putLeft}/{@link #putRight}, so one-sided keys (e.g. unmatched ifExists counters) pay no
   * allocation for the unused side.
   *
   * <p>The {@code compositeKey} passed to the per-side methods is the FULL composite key; a backend
   * downstream ignores it, a suffix sub-chain extracts its levels from it (via {@code
   * KeyUnpacker.composite(1)}, ...). The left downstream holds left tuples and is queried with a
   * right tuple's key; the right downstream holds right tuples and is queried with a left tuple's
   * key (mirroring the two-indexer cross-side query).
   *
   * @param <L> the left element type (a left tuple, or {@code ExistsCounter} for ifExists)
   * @param <R> the right element type (a right {@code UniTuple})
   */
