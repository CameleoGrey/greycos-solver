...
import greycos.solver.core.impl.bavet.quad.IndexedIfExistsQuadNode;
import greycos.solver.core.impl.bavet.quad.UnindexedIfExistsQuadNode;
import greycos.solver.core.impl.score.stream.bavet.BavetConstraintFactory;
import greycos.solver.core.impl.score.stream.bavet.common.BavetIfExistsConstraintStream;
...
final class BavetIfExistsQuadConstraintStream<Solution_, A, B, C, D, E>
  ...
  public BavetIfExistsQuadConstraintStream(
  ...
  public <Score_ extends Score<Score_>> void buildNode(
    ...
    var positionTracker =
        buildHelper.getTupleStorePositionTracker(
            this, parentABCD.getTupleSource(), parentBridgeE.getTupleSource());
    var node =
        indexerFactory.hasJoiners()
            ? (filtering == null
                ? new IndexedIfExistsQuadNode<>(
                    shouldExist, indexerFactory, downstream, positionTracker)
                : new IndexedIfExistsQuadNode<>(
                    shouldExist, indexerFactory, downstream, filtering, positionTracker))
            : (filtering == null
                ? new UnindexedIfExistsQuadNode<>(shouldExist, downstream, positionTracker)
                : new UnindexedIfExistsQuadNode<>(
                    shouldExist, downstream, filtering, positionTracker));
  ...
  public boolean equals(Object object) {
    ...
    BavetIfExistsQuadConstraintStream<?, ?, ?, ?, ?, ?> that =
        (BavetIfExistsQuadConstraintStream<?, ?, ?, ?, ?, ?>) object;
    /*
     * Bridge streams do not implement equality because their equals() would have to point back to this stream,
     * resulting in StackOverflowError.
     * Therefore we need to check bridge parents to see where this ifExists node comes from.
     */
    return shouldExist == that.shouldExist
        && Objects.equals(parentABCD, that.parentABCD)
        && Objects.equals(parentBridgeE.getParent(), that.parentBridgeE.getParent())
