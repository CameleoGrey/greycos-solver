...
import static greycos.solver.core.api.score.stream.Joiners.lessThan;
...
public abstract class InnerConstraintFactory<Solution_, Constraint_ extends Constraint>
  ...
  @Override
  public <A> @NonNull BiConstraintStream<A, A> forEachUniquePair(
      @NonNull Class<A> sourceClass, BiJoiner<A, A> @NonNull ... joiners) {
    return forEachUniquePair(this::forEach, sourceClass, joiners);
  }

  public <A> @NonNull BiConstraintStream<A, A> forEachUniquePair(
      Function<Class<A>, UniConstraintStream<A>> streamFunction,
  ...
  public <A> void assertValidFromType(Class<A> fromType) {
    ...
    if (!hasMatchingType) {
      List<String> canonicalClassNameList =
  ...
  public List<Constraint_> buildConstraints(ConstraintProvider constraintProvider) {
    ...
    constraintsPerIdMap.forEach(
        (constraintRef, duplicateConstraintList) -> {
          if (duplicateConstraintList.size() > 1) {
            throw new IllegalStateException(
                "There are multiple constraints with the same ID (%s).".formatted(constraintRef));
          }
        });
    return Arrays.stream(constraints).map(c -> (Constraint_) c).collect(Collectors.toList());
  }
