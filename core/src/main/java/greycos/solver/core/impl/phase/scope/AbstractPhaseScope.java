package greycos.solver.core.impl.phase.scope;
...
public abstract class AbstractPhaseScope<Solution_> {
  ...
  /**
   * As defined by #AbstractPhaseScope(SolverScope, int, boolean) with the
   * phaseSendingBestSolutionEvents parameter set to true.
   */
   ...
   * @param phaseSendingBestSolutionEvents set to false if the phase only sends one best solution
   *     event at the end, or none at all; this is typical for construction heuristics, whose result
   *     only matters when it reached its natural end.
   */
  protected AbstractPhaseScope(
      SolverScope<Solution_> solverScope, int phaseIndex, boolean phaseSendingBestSolutionEvents) {
  ...
  public <Score_ extends Score<Score_>> void assertExpectedWorkingScore(
  ...
  public <Score_ extends Score<Score_>> void assertWorkingScoreFromScratch(
  ...
  public <Score_ extends Score<Score_>> void assertPredictedScoreFromScratch(
      InnerScore<Score_> workingScore, Object completedAction) {
    InnerScoreDirector<Solution_, Score_> innerScoreDirector = getScoreDirector();
    innerScoreDirector.assertPredictedScoreFromScratch(workingScore, completedAction);
  }

  public <Score_ extends Score<Score_>> void assertShadowVariablesAreNotStale(
      InnerScore<Score_> workingScore, Object completedAction) {
    InnerScoreDirector<Solution_, Score_> innerScoreDirector = getScoreDirector();
