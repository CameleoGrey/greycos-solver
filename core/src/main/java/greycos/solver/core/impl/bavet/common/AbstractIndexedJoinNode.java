...
import greycos.solver.core.impl.bavet.common.index.FusedEqualIndex;
import greycos.solver.core.impl.bavet.common.index.FusedEqualIndex.Bucket;
...
import greycos.solver.core.impl.bavet.common.index.IndexerFactory.KeysExtractor;
import greycos.solver.core.impl.bavet.common.index.IndexerFactory.UniKeysExtractor;
import greycos.solver.core.impl.bavet.common.tuple.InOutTupleStorePositionTracker;
import greycos.solver.core.impl.bavet.common.tuple.LeftTupleLifecycle;
import greycos.solver.core.impl.bavet.common.tuple.RightTupleLifecycle;
 ...
 *
 * <p>Indexing takes one of two forms, chosen once at construction (see {@link
 * IndexerFactory#isFusedEqualIndexEligible()}):
 *
 * <ul>
 *   <li>the non-unified path keeps two parallel {@link Indexer}s ({@code indexerLeft}/{@code
 *       indexerRight}); a tuple inserted on one side queries the OPPOSITE indexer with its OWN key
 *       (a second hash navigation of that key);
 *   <li>the unified path (equal-bearing joins) keeps ONE {@link FusedEqualIndex}: a tuple looks up
 *       its bucket ONCE, adds itself to its side, and iterates the other side of the SAME bucket —
 *       co-location is the equal match. The resolved bucket is cached on the tuple so same-key
 *       updates and retracts need no lookup at all.
 * </ul>
 *
...
public abstract class AbstractIndexedJoinNode<
  ...
  // Unified path (useFusedEqualIndex == true): one shared join index, plus per-side cached-bucket
  // store slots.
  ...
  // True only for an equal+suffix unified index: a changed-key update whose equal prefix is
  // unchanged can reuse the
  // cached bucket. Pure-equal nodes are false, so the dominant path never even evaluates
  // isSameBucket.
  private final boolean reuseBucketEligible;

  protected AbstractIndexedJoinNode(
      KeysExtractor<LeftTuple_> keysExtractorLeft,
      IndexerFactory<Right_> indexerFactory,
    ...
    this.inputStoreIndexLeftBucket =
        useFusedEqualIndex ? tupleStorePositionTracker.reserveNextLeft() : -1;
    this.inputStoreIndexRightBucket =
        useFusedEqualIndex ? tupleStorePositionTracker.reserveNextRight() : -1;
  ...
  public final void updateLeft(LeftTuple_ leftTuple) {
    ...
    if (oldCompositeKey.equals(newCompositeKey)) {
      ...
      innerUpdateLeft(
          leftTuple, consumer -> forEachRightMatch(leftTuple, oldCompositeKey, consumer));
    } else {
      TupleList<OutTuple_> outTupleListLeft = leftTuple.getStore(inputStoreIndexLeftOutTupleList);
      var reuseBucket =
          reuseBucketEligible && fusedEqualIndex.isSameBucket(oldCompositeKey, newCompositeKey);
      if (reuseBucket) {
        // Equal prefix unchanged ⇒ same bucket: move within the cached bucket, no top lookup or
        // drop/recreate.
        ...
        if (useFusedEqualIndex) {
          Bucket<LeftTuple_, UniTuple<Right_>> oldBucket =
              leftTuple.getStore(inputStoreIndexLeftBucket);
  ...
  private void indexAndPropagateLeft(
      LeftTuple_ leftTuple, Object compositeKey, boolean reuseCachedBucket) {
    ...
    if (useFusedEqualIndex) {
      ...
      // no top-level
      // lookup and no re-cache; otherwise resolve the bucket (the single top-level lookup) and
      // cache it.
    ...
    if (!leftTuple.getState().isActive()) {
      ...
      // and requires adding null checks to the filter for something that should intuitively be
      // impossible.
    ...
    forEachRightMatch(
        leftTuple, compositeKey, rightTuple -> insertOutTupleFiltered(leftTuple, rightTuple));
  }
  ...
  public final void updateRight(UniTuple<Right_> rightTuple) {
    ...
    if (oldCompositeKey.equals(newCompositeKey)) {
      ...
      // Prefer an update over retract-insert if possible
      innerUpdateRight(
          rightTuple, consumer -> forEachLeftMatch(rightTuple, oldCompositeKey, consumer));
    } else {
  ...
  private void indexAndPropagateRight(
      UniTuple<Right_> rightTuple, Object compositeKey, boolean reuseCachedBucket) {
    ...
    forEachLeftMatch(
        rightTuple,
        compositeKey,
        leftTuple -> insertOutTupleFilteredFromLeft(leftTuple, rightTuple));
  }
  ...
  public final void retractRight(UniTuple<Right_> rightTuple) {
    ...
    TupleList<OutTuple_> outTupleListRight =
        rightTuple.removeStore(inputStoreIndexRightOutTupleList);
    ListEntry<UniTuple<Right_>> entry = rightTuple.removeStore(inputStoreIndexRightEntry);
    if (useFusedEqualIndex) {
      Bucket<LeftTuple_, UniTuple<Right_>> bucket =
          rightTuple.removeStore(inputStoreIndexRightBucket);
  ...
  /**
   * Iterates the right tuples matching the given left composite key: the right side of the left
   * tuple's cached bucket (unified), or {@code indexerRight} queried with that key (non-unified).
   */
  private void forEachRightMatch(
      LeftTuple_ leftTuple, Object compositeKey, Consumer<UniTuple<Right_>> consumer) {
    if (useFusedEqualIndex) {
      Bucket<LeftTuple_, UniTuple<Right_>> bucket = leftTuple.getStore(inputStoreIndexLeftBucket);
  ...
  /**
   * Iterates the left tuples matching the given right composite key: the left side of the right
   * tuple's cached bucket (unified), or {@code indexerLeft} queried with that key (non-unified).
   */
  private void forEachLeftMatch(
      UniTuple<Right_> rightTuple, Object compositeKey, Consumer<LeftTuple_> consumer) {
    if (useFusedEqualIndex) {
