...
import greycos.solver.core.impl.bavet.common.tuple.OutTupleStorePositionTracker;
 ...
 * AbstractUnindexedJoinNode}. The logic in either is identical, except that the latter removes all
 * indexing work. Therefore any time that one of the classes changes, the other should be inspected
 * if it could benefit from applying the change there too.
 *
 * @param <LeftTuple_>
 * @param <Right_>
 */
public abstract class AbstractJoinNode<LeftTuple_ extends Tuple, Right_, OutTuple_ extends Tuple>
   ...
   * the stale-activity race at its root instead of guarding against it per read. Non-filtering
   * joins never dereference a fact through a user predicate, so a stale-but-"active" read can't
   * corrupt anything there (confirmed by dedicated regression tests); these fields stay {@code
   * null}/{@code -1} and cost nothing for them.
   *
   * <p>{@code pendingLeft}/{@code pendingRight} hold tuples whose cross-match is due; the marker
   * slots exist purely to make enqueueing idempotent (a tuple already awaiting its turn isn't
   * re-added). Both lists are drained, left before right, in {@link #prepareForSettle()}, calling
   * {@link #reconcilePendingLeft(Tuple)}/{@link #reconcilePendingRight(UniTuple)}. That logic
   * already treats "no existing out-tuple for this pair" as "insert if the predicate passes", so it
   * doubles as the insert path too: whichever side's tuple is reconciled *second* always sees the
   * *first* side's just-created out-tuple (since {@link #insertOutTuple} links it into both sides'
   * out-tuple lists immediately), and correctly treats it as an existing match instead of creating
   * a duplicate. The two sides are therefore never both examined at once; consistently draining
   * left before right merely fixes which side's view is "first" without affecting anything's
   * correctness.
   */
   ...
   * tuple's own store entries (composite key, out-tuple list, ...) are cleared, since a
   ...
   * opposite side, exactly as an eager filtering update already would have ({@link
   * #innerUpdateLeft}); reusing that method is what makes this correct for a tuple that was
   * actually a fresh insert too: with no pre-existing out-tuples to mark, every match it finds is
   * necessarily new, so {@code innerUpdateLeft}'s own "no existing out-tuple ⇒ insert" branch
   * handles it. Implemented by each subclass because only it knows how to walk the opposite side
  ...
  /**
   * Only ever called from the non-filtering path (filtering joins enqueue and defer to {@link
   * #reconcilePendingLeft}/{@link #reconcilePendingRight} instead), where {@code
   * testFiltering(...)} is never even consulted; see {@link #testFiltering}'s callers.
   */
  protected final void insertOutTupleIfActiveFiltered(
  ...
  protected final void innerUpdateLeft(
    ...
    if (!isFiltering) {
      ...
      rightTupleConsumer.accept(
          rightTuple -> processOutTupleUpdateRight(leftTuple, rightTuple, version));
    }
  }
  ...
  protected final void innerUpdateRight(
    ...
    if (!isFiltering) {
      ...
      leftTupleConsumer.accept(
          leftTuple -> processOutTupleUpdateLeft(leftTuple, rightTuple, version));
    }
  }

  private void processOutTupleUpdateLeft(
      LeftTuple_ leftTuple, UniTuple<Right_> rightTuple, long version) {
