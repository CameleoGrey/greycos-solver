...
public class ListRuinRecreateMove<Solution_> extends AbstractMove<Solution_> {
      ...
      for (var entry : entityToInsertedValuesMap.entrySet()) {
        if (!entityToOriginalPositionMap.containsKey(entry.getKey())) {
          ...
          onlyRecordingChangesScoreDirector.beforeListVariableChanged(
              listVariableDescriptor, entry.getKey(), 0, originalElementList.size());
          listVariableDescriptor.getValue(entry.getKey()).clear();
  ...
  public Move<Solution_> rebase(ScoreDirector<Solution_> destinationScoreDirector) {
    ...
    var rebasedAffectedEntitySet =
        AbstractMove.rebaseSet(affectedEntitySet, destinationScoreDirector);
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof ListRuinRecreateMove<?> that)) return false;
  ...
  public String toString() {
    return "ListRuinMove{"
        + "values="
        + ruinedValueList
        + ", newPositionsByEntity="
        + (!entityToNewPositionMap.isEmpty() ? entityToNewPositionMap : "?")
        + '}';
  }
}
