...
public final class DefaultUniDatasetInstance<Solution_, A> implements UniDatasetInstance<A> {
  ...
  /**
   * Maps a tuple iterator to its fact. Uniqueness or endlessness are entirely a property of the
   * wrapped {@code tupleIterator}; this class neither removes nor limits anything itself.
   */
  private record FactIterator<A>(Iterator<UniTuple<A>> tupleIterator)
      implements Iterator<@Nullable A> {
