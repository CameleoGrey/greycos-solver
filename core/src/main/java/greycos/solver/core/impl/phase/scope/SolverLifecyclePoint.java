...
/**
 * Identifies which move thread, which phase, step and move/move tree the solver is currently
 * executing.
 *
 ...
 */
public record SolverLifecyclePoint(
    int moveThreadIndex, int phaseIndex, int stepIndex, int moveIndex, String treeId) {

  public static SolverLifecyclePoint of(AbstractMoveScope<?> moveScope) { // General purpose.
    var stepScope = moveScope.getStepScope();
    return new SolverLifecyclePoint(
        -1,
        stepScope.getPhaseScope().getPhaseIndex(),
        stepScope.getStepIndex(),
        moveScope.moveIndex,
        null);
  }

  public static SolverLifecyclePoint of(
      AbstractStepScope<?> stepScope, String treeId) { // Used in exhaustive search.
    return new SolverLifecyclePoint(
        -1,
        stepScope.getPhaseScope().getPhaseIndex(),
        stepScope.getStepIndex(),
  ...
  public static SolverLifecyclePoint of(
      int moveThreadIndex, int phaseIndex, int stepIndex, int moveIndex) {
