...
import greycos.solver.core.impl.bavet.common.joiner.AbstractJoiner;
...
/**
 * {@link Indexer Indexers} form a parent-child hierarchy, each child has exactly one parent. {@link
 * LeafIndexer} is always at the bottom of the hierarchy, never a parent unless it is the only
 ...
 *
 * <p>Comparison joiners result in a single indexer each. The comber reorders joiners equal-first,
 * so all equal joiners form a single leading run; that run is merged into a single (top-most)
 * indexer. In that case, a composite keyFunction is created of type {@link Pair}, {@link Triple},
 * {@link Quadruple} or {@link IndexerKey}, based on the length of the equal prefix (number of
 * leading equal joiners).
 *
 * <ul>
 *   <li>Example 2: For an EQUAL+LESS_THAN joiner, there are two indexers in the chain with
 *       keyFunction length of 1 each.
 *   <li>Example 3: For an EQUAL+EQUAL+LESS_THAN joiner, there are still two indexers, but the first
 *       (equal) indexer's keyFunction length is 2.
 *   <li>Example 4: For an EQUAL+EQUAL+LESS_THAN+GREATER_THAN joiner, there are three indexers in
 *       the chain, and the first (equal) one's keyFunction length is 2.
 * </ul>
 *
 * @param <Right_>
 */
public final class IndexerFactory<Right_> {
   ...
   * (see {@code reorderedEqualsFirst()}), so all equal joiners form a single run at the front; this
   * is its length (0 if the joiner starts with a non-equal joiner). The equal run is merged into
   * one indexer level (the composite key); every remaining joiner becomes its own single-key level.
   */
   ...
   * (length {@link #equalPrefixLength}, when {@code >= 1}) followed by one single-joiner level
   * each. Empty when the joiner has no joiners. For {@code [EQUAL, EQUAL, LESS_THAN]} this is
   * {@code [2, 3]} (one merged level spanning indices 0..1, then a single level at index 2).
   */
  ...
  public <A> UniKeysExtractor<A> buildUniLeftKeysExtractor() {
    return buildUniTupleKeysExtractor(getMappingExtractor());
  }
  ...
  private <A> Function<A, Object> buildUniFactKeysExtractor(
      IntFunction<Function<A, Object>> mappingExtractor) {
    var joinerCount = joiner.getJoinerCount();
    if (joinerCount == 0) {
    ...
    return combineFactKeysExtractor(buildLevelKeyFunctionList(mappingExtractor));
  }

  /**
   * Tuple-typed counterpart of {@link #buildUniFactKeysExtractor(IntFunction)}. Kept separate
   * (instead of wrapping the fact extractor with a {@code tuple ->
   * factExtractor.apply(tuple.getA())} layer) because this feeds the indexed join/if-exists hot
   * path, where every tuple insert, update, and retract pays for the extra call.
   */
  private <A> UniKeysExtractor<A> buildUniTupleKeysExtractor(
      IntFunction<Function<A, Object>> mappingExtractor) {
    var joinerCount = joiner.getJoinerCount();
    if (joinerCount == 0) {
    ...
    return combineTupleKeysExtractor(buildLevelKeyFunctionList(mappingExtractor));
  }

  @SuppressWarnings("unchecked")
  private <A> List<Function<A, Object>> buildLevelKeyFunctionList(
      IntFunction<Function<A, Object>> mappingExtractor) {
    ...
    for (var endIndexExclusive : levelEndIndices()) {
      ...
      // The leading EQUAL run is merged into a single composite keyFunction; every other level has
      // length 1.
      Function<A, Object> keyFunction =
          switch (keyFunctionLength) {
            ...
            case 4 -> {
              ...
              yield a ->
                  new Quadruple<>(
                      mapping1.apply(a), mapping2.apply(a), mapping3.apply(a), mapping4.apply(a));
            }
            default -> {
  ...
  private static <A> Function<A, Object> combineFactKeysExtractor(
      List<Function<A, Object>> keyFunctionList) {
  ...
  private static <A> UniKeysExtractor<A> combineTupleKeysExtractor(
      List<Function<A, Object>> keyFunctionList) {
    var keyFunctionCount = keyFunctionList.size();
    return switch (keyFunctionCount) {
      case 1 -> {
      ...
      BiFunction<A, B, Object> keyFunction =
          switch (keyFunctionLength) {
            ...
            case 3 -> {
              ...
              yield (a, b) ->
                  new Triple<>(mapping1.apply(a, b), mapping2.apply(a, b), mapping3.apply(a, b));
            }
            case 4 -> {
              var mapping1 = castJoiner.getLeftMapping(levelStart);
              var mapping2 = castJoiner.getLeftMapping(levelStart + 1);
      ...
      TriFunction<A, B, C, Object> keyFunction =
          switch (keyFunctionLength) {
            ...
            case 3 -> {
              ...
              yield (a, b, c) ->
                  new Triple<>(
                      mapping1.apply(a, b, c), mapping2.apply(a, b, c), mapping3.apply(a, b, c));
            }
            case 4 -> {
              var mapping1 = castJoiner.getLeftMapping(levelStart);
              var mapping2 = castJoiner.getLeftMapping(levelStart + 1);
              ...
              yield (a, b, c) ->
                  new Quadruple<>(
                      mapping1.apply(a, b, c),
                      mapping2.apply(a, b, c),
      ...
      QuadFunction<A, B, C, D, Object> keyFunction =
          switch (keyFunctionLength) {
            ...
            case 2 -> {
              ...
              var mapping2 = castJoiner.getLeftMapping(levelStart + 1);
              yield (a, b, c, d) ->
                  new Pair<>(mapping1.apply(a, b, c, d), mapping2.apply(a, b, c, d));
            }
            case 3 -> {
              var mapping1 = castJoiner.getLeftMapping(levelStart);
              var mapping2 = castJoiner.getLeftMapping(levelStart + 1);
  ...
  private static <A, B, C, D> QuadKeysExtractor<A, B, C, D> toKeysExtractor(
      QuadFunction<A, B, C, D, Object> keyFunction) {
    return tuple ->
        CompositeKey.of(keyFunction.apply(tuple.getA(), tuple.getB(), tuple.getC(), tuple.getD()));
  }
  ...
  /**
   * Builds the indexer chain for levels {@code [fromLevelInclusive, last]}, children-first, and
   * returns a supplier of its top indexer. Level ids (hence the {@link KeyUnpacker#composite(int)}
   * ids) are anchored to the FULL chain; {@code fromLevelInclusive} only bounds where the build
   * stops. Therefore a suffix built with {@code fromLevelInclusive == 1} still uses {@code
   * KeyUnpacker.composite(1)} for its top level and never takes the {@code indexPropertyId == 0}
   * leaf-fuse / single-key shortcut (that would hand it the whole composite key instead of
   * component 1).
   *
   * <p>{@code buildIndexer(isLeftBridge)} is {@code buildIndexerChain(isLeftBridge, 0,
   * backendSupplier)}; {@link #buildFusedEqualIndex()} uses {@code fromLevelInclusive == 1} to
   * build the per-side suffix sub-chains.
   */
  private <T> Supplier<Indexer<T>> buildIndexerChain(
      boolean isLeftBridge, int fromLevelInclusive, Supplier<Indexer<T>> backendSupplier) {
    // Build children-first, so iterate the levels bottom-up (the leaf is the highest
    // indexPropertyId).
    // A level's joiner type is the type of any joiner it spans; the merged equal prefix (level 0)
    // is EQUAL.
    var endIndices = levelEndIndices();
    var downstreamIndexerSupplier = backendSupplier;
    for (var indexPropertyId = endIndices.length - 1;
        indexPropertyId >= fromLevelInclusive;
        indexPropertyId--) {
      ...
      if (downstreamIndexerSupplier == backendSupplier && indexPropertyId == 0) {
        // Leaf-most level whose index key equals the whole composite key: no KeyUnpacker
        // indirection.
  ...
  /**
   * Whether this join/ifExists can use a unified {@link FusedEqualIndex} instead of two parallel
   * indexers. Eligible iff the joiner has a leading EQUAL run (always true iff it has any equal,
   * since the comber reorders equal-first) and random access is not required.
   */
  ...
  /**
   * Builds a unified {@link FusedEqualIndex} for an {@link #isFusedEqualIndexEligible() eligible}
   * join/ifExists. The top map is keyed by the equal prefix; each bucket holds the per-side
   * downstream for the remaining comparison/containing suffix (built right-flipped on the right
   * side), or a bare tuple-list backend when the join is pure-equal.
   *
  ...
  public <L, R> FusedEqualIndex<L, R> buildFusedEqualIndex() {
    ...
    var topEqualKeyUnpacker =
        equalPrefixLength == joinerCount ? KeyUnpacker.single() : KeyUnpacker.composite(0);
    if (equalPrefixLength == joinerCount) {
      // Pure equal: the per-side downstream is just the tuple list; the bucket is the equal-key
      // group.
      return new FusedEqualIndex<>(
          topEqualKeyUnpacker, false, LinkedListLeafIndexer::new, LinkedListLeafIndexer::new);
    } else {
      // Equal prefix + suffix: build the per-side suffix sub-chain (the right side flips
      // comparisons).
      ...
      return new FusedEqualIndex<>(
          topEqualKeyUnpacker, true, leftDownstreamSupplier, rightDownstreamSupplier);
    }
  }

  /**
   * A right bridge node stores B, keyed by rightMapping, and is queried using A's
   * leftMapping-derived key. The joiner type has to be flipped to translate "leftMapping(A) OP
   * rightMapping(B)" into the equivalent "storedKey OP' queryKey" relation, regardless of whether a
   * left bridge also exists to pair with it.
   */
  private static <T> Indexer<T> buildIndexerPart(
