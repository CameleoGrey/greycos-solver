...
public final class ConditionalTupleLifecycle<Tuple_ extends Tuple>
  ...
  public ConditionalTupleLifecycle(
      TupleLifecycle<Tuple_> downstreamLifecycle, TuplePredicate<Tuple_> predicate) {
  ...
  public void afterAllFactsInserted(boolean upstreamCanProduceTuples) {
    // It is possible the predicate will always filter everything out, but we cannot know that for
    // certain.
    // We must pass the upstream information downstream, and be active if upstream can send anything
    // to us.
