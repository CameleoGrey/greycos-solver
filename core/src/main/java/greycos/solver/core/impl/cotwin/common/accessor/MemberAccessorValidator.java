...
final class MemberAccessorValidator {
  ...
  public static void verifyIsValidMember(
      @Nullable Class<?> annotationClass, Member member, MemberAccessorType memberAccessorType) {
    var memberType = (member instanceof Field) ? "field" : "method";
    var messagePrefix =
        (annotationClass == null)
            ? "The %s (%s) in class (%s)"
                .formatted(
                    memberType, member.getName(), member.getDeclaringClass().getCanonicalName())
            : "The @%s annotated %s (%s) in class (%s)"
                .formatted(
  ...
  private static void verifyIsVoidMethod(Member member, String messagePrefix) {
    ...
    if (!method.getReturnType().equals(void.class)) {
      throw new IllegalArgumentException(
          "%s must be a void method, but it returns (%s)."
              .formatted(messagePrefix, method.getReturnType().getCanonicalName()));
    }
  ...
  private static void verifyGetterName(Method method, String messagePrefix) {
    if (!method.getName().startsWith("get")
        && !(method.getReturnType().equals(boolean.class) && method.getName().startsWith("is"))) {
      throw new IllegalArgumentException(
          """
  ...
  private static void verifyFieldOrGetter(Member member, String messagePrefix) {
    switch (member) {
      ...
      default ->
          throw new IllegalArgumentException(
              "Unhandled member type (%s).".formatted(member.getClass().getCanonicalName()));
    }
  }
      ...
      if (getterMethod.getReturnType().equals(void.class)) {
        throw new IllegalArgumentException(
            "%s has a public getter method that returns void. Maybe make the method (%s) return (%s) instead?"
                .formatted(
                    messagePrefix, getterMethod.getName(), field.getType().getCanonicalName()));
      }
    }
  }

  private static void verifyIsPublicFieldOrHasReadMethod(
      Member member, String messagePrefix, boolean acceptOptionalParameter) {
    switch (member) {
      case Field field -> verifyIsPublicFieldOrHasPublicGetter(field, messagePrefix);
      case Method method -> {
        if (!Modifier.isPublic(method.getModifiers())) {
  ...
  private static void verifyIsPublicFieldOrHasPublicSetter(Member member, String messagePrefix) {
    switch (member) {
      case Field field -> {
        ...
        var getterMethod =
            ReflectionHelper.getGetterMethod(field.getDeclaringClass(), field.getName());
        var setterMethod =
            ReflectionHelper.getSetterMethod(
                field.getDeclaringClass(), memberType, field.getName());
      ...
      case Method getterMethod -> {
        ...
        var setterMethod =
            ReflectionHelper.getSetterMethod(
                getterMethod.getDeclaringClass(), memberType, memberName);
        if (setterMethod == null) {
          throw new IllegalArgumentException(
              """
  ...
  private static void verifyGetterSetterProperties(
      Method getterMethod, Method setterMethod, String messagePrefix) {
    if (!Modifier.isPublic(getterMethod.getModifiers())) {
      throw new IllegalArgumentException(
          "%s has a non-public getter method. Maybe make the method (%s) public?"
              .formatted(messagePrefix, getterMethod.getName()));
    ...
    if (setterMethod.getParameterCount() != 1) {
      throw new IllegalArgumentException(
          ...
          Maybe make the method (%s) take exactly one parameter?"""
              .formatted(messagePrefix, setterMethod.getParameterCount(), setterMethod.getName()));
    }
