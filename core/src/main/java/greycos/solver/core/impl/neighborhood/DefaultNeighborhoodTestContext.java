...
import greycos.solver.core.preview.api.neighborhood.test.NeighborhoodMoveAsserter;
...
final class DefaultNeighborhoodTestContext<Solution_>
  ...
  DefaultNeighborhoodTestContext(
      NeighborhoodsBasedMoveRepository<Solution_> moveRepository,
  ...
  private DefaultNeighborhoodTestContext(
      NeighborhoodsBasedMoveRepository<Solution_> moveRepository,
      MoveTestContext<Solution_> moveTestContext,
      LocalSearchPhaseScope<Solution_> phaseScope,
      int iterationLimit) {
  ...
  public NeighborhoodMoveAsserter<Solution_> within(int iterationLimit) {
    return new DefaultNeighborhoodTestContext<>(
        moveRepository, moveTestContext, phaseScope, iterationLimit);
  }
  ...
  /**
   * Repeatedly draws moves from a fresh, seeded {@link
   * NeighborhoodsBasedMoveRepository#iterator(java.util.random.RandomGenerator)} (restarting
   * whenever it exhausts) until either {@code stopCondition} returns true, the iteration limit is
  ...
  @Override
  public <Move_ extends Move<Solution_>> Iterator<Move_> getMovesAsIterator(
      Function<Move<Solution_>, Move_> moveCaster) {
    var stepScope = new LocalSearchStepScope<>(phaseScope);
    moveRepository.stepStarted(stepScope);
    var iterator =
        new CastingIterator<>(
            moveRepository.iterator(), Objects.requireNonNull(moveCaster, "moveCaster"));
    moveRepository.stepEnded(stepScope);
    return iterator;
  }
  ...
  private record CastingIterator<Solution_, Move_ extends Move<Solution_>>(
      Iterator<Move<Solution_>> childIterator, Function<Move<Solution_>, Move_> moveCaster)
