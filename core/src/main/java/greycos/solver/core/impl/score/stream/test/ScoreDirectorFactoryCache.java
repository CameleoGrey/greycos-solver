...
/**
 * Designed for access from a single thread. Callers are responsible for ensuring that instances are
 * never run from a thread other than that which created them.
 *
 * @param <ConstraintProvider_>
 * @param <Solution_>
 * @param <Score_>
 */
final class ScoreDirectorFactoryCache<
    ConstraintProvider_ extends ConstraintProvider, Solution_, Score_ extends Score<Score_>> {

  /**
   * Score director factory creation is expensive; we cache it. The cache needs to be recomputed
   * every time that the parent's configuration changes.
   */
  private final Map<
          ConstraintRef, AbstractConstraintStreamScoreDirectorFactory<Solution_, Score_, ?>>
      scoreDirectorFactoryMap = new HashMap<>();
  ...
  /**
   * Retrieve {@link AbstractConstraintStreamScoreDirectorFactory} from the cache, or create and
   * cache a new instance. Cache key is the ID of the single constraint returned by calling the
   * constraintFunction.
   *
   * @param constraintFunction never null, determines the single constraint to be used from the
   *     constraint provider
  ...
  public AbstractConstraintStreamScoreDirectorFactory<Solution_, Score_, ?> getScoreDirectorFactory(
    ...
    List<Constraint> constraints =
        (List<Constraint>) fullConstraintFactory.buildConstraints(constraintProvider);
    Constraint expectedConstraint =
        constraintFunction.apply(constraintProvider, fullConstraintFactory);
    Constraint result =
        constraints.stream()
            .filter(
                c -> Objects.equals(c.getConstraintRef(), expectedConstraint.getConstraintRef()))
            .findFirst()
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "Impossible state: Constraint provider ("
                            + constraintProvider
                            + ") has no constraint ("
                            + expectedConstraint
                            + ")."));
    return getScoreDirectorFactory(
        result.getConstraintRef(), constraintFactory -> new Constraint[] {result}, environmentMode);
  }
   ...
   * @param constraintProvider never null, constraint provider to create the factory from; ignored
   *     on cache hit
   * @return never null
   */
  public AbstractConstraintStreamScoreDirectorFactory<Solution_, Score_, ?> getScoreDirectorFactory(
      ...
      EnvironmentMode environmentMode) {
    return scoreDirectorFactoryMap.computeIfAbsent(
        constraintRef,
        k ->
            new BavetConstraintStreamScoreDirectorFactory<>(
                solutionDescriptor, constraintProvider, environmentMode, false));
  }
}
