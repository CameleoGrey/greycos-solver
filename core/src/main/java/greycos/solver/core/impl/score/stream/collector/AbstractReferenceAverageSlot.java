...
public abstract class AbstractReferenceAverageSlot<Input_, Output_> {
  ...
  public static final class State<Input_, Output_> {
    ...
    State(
        Input_ zero,
        BinaryOperator<Input_> adder,
        BinaryOperator<Input_> subtractor,
  ...
  private static final Supplier<State<BigDecimal, BigDecimal>> BIG_DECIMAL =
      () ->
          new State<>(
              BigDecimal.ZERO,
              BigDecimal::add,
  ...
  private static final Supplier<State<BigInteger, BigDecimal>> BIG_INTEGER =
      () ->
          new State<>(
              BigInteger.ZERO,
              BigInteger::add,
              BigInteger::subtract,
              (sum, count) ->
                  new BigDecimal(sum).divide(BigDecimal.valueOf(count), RoundingMode.HALF_EVEN));

  private static final Supplier<State<Duration, Duration>> DURATION =
      () ->
          new State<>(
              Duration.ZERO,
              Duration::plus,
