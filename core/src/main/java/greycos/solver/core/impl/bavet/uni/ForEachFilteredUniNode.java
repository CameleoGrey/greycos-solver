...
@NullMarked
public final class ForEachFilteredUniNode<A> extends AbstractForEachUniNode<A> {

  private final TupleLifecycle<UniTuple<A>> nextNodesTupleLifecycle;
  private final Predicate<A> filter;

  /**
   * Counts every fact currently inserted into this node, including those which do not pass the
   * filter. Only {@link #insert(Object)} and {@link #retract(Object)} may mutate it; the filter
   * transitions inside {@link #update(Object)} concern facts already counted.
   */
  private int tupleCountWithoutFiltering = 0;

  public ForEachFilteredUniNode(
