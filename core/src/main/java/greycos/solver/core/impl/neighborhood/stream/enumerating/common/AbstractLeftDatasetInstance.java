...
public abstract class AbstractLeftDatasetInstance<Solution_, Tuple_ extends Tuple>
  ...
  /**
   * As defined by {@link #exhaustiveIterator(RandomGenerator)}, but the caller must call {@link
   * RetiringRandomIterator#retire()} itself after each {@link Iterator#next()} to permanently drop
   * an element. Only meant for a caller which must decide by itself when an element is no longer
   * needed, such as the left side of a join, which must not retire a tuple until its right side is
   * confirmed empty.
   */
