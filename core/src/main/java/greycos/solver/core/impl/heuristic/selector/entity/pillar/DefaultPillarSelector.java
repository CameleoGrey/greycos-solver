...
import greycos.solver.core.impl.heuristic.selector.move.generic.PillarDemand;
...
public final class DefaultPillarSelector<Solution_> extends AbstractSelector<Solution_>
  ...
  public DefaultPillarSelector(
    ...
    this.pillarDemand =
        new PillarDemand<>(entitySelector, variableDescriptors, subpillarConfigPolicy);
    ...
    if (!randomSelection && subPillarEnabled) {
      throw new IllegalStateException(
          "The selector ("
              ...
              + randomSelection
              + ") does not support non random selection with "
              + "sub pillars because the number of sub pillars scales exponentially.\n"
              + "Either set subPillarType to "
              + SubPillarType.NONE
              + " or use JIT random selection.");
    }
  }
  ...
  public void constructCache(SolverScope<Solution_> solverScope) {
    ...
    cachedBasePillarList =
        solverScope.getScoreDirector().getSupplyManager().demand(pillarDemand).read();
  }
  ...
  public void disposeCache(SolverScope<Solution_> solverScope) {
     ...
     */
    solverScope.getScoreDirector().getSupplyManager().cancel(pillarDemand);
  ...
  public long getSize() {
    if (!subpillarConfigPolicy.isSubPillarEnabled()) {
      ...
      // For each pillar, the number of combinations is: the sum of every (n! / (k! (n-k)!)) for
      // which n is
      // pillar.getSize() and k iterates from minimumSubPillarSize to maximumSubPillarSize. This
      // implies that a
      ...
      throw new UnsupportedOperationException(
          "The selector ("
              ...
              + ") with randomSelection  ("
              + randomSelection
              + ") and sub pillars does not support getSize() "
  ...
  private String getSubPillarExceptionMessage() {
    return "Impossible state because the constructors fails with randomSelection ("
        + randomSelection
        + ") and sub pillars.";
  }
  ...
  private String getListIteratorExceptionMessage() {
    return "The selector ("
        + this
        + ") does not support a ListIterator with randomSelection ("
        + randomSelection
        + ").";
  }
