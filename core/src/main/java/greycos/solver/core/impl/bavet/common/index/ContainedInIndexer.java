...
final class ContainedInIndexer<T, Key_, KeyCollection_ extends SequencedCollection<Key_>>
  ...
  private Indexer<T> getDownstreamIndexer(
      Object compositeKey, Key_ indexerKey, ListEntry<T> entry) {
  ...
  /**
   * {@link #queryKeyUnpacker}'s output can contain a duplicated key; nothing upstream guarantees
   * uniqueness. Buckets in this indexer are disjoint by construction (one key per tuple, see {@link
   * #modifyKeyUnpacker}), so a duplicated query key would otherwise double-count in {@link
   * #size(Object)} and double-visit its bucket in {@link DefaultIterator}, returning the same
   * tuples twice from a {@link #uniqueRandomIterator(Object, RandomGenerator)} drain and breaking
   * its documented "never returned again" contract.
   */
  ...
  /**
   * Every {@code next()} independently picks a key by its downstream size, with replacement, so
   * this never ends: there is no shrinking distribution and no removed set to maintain.
   */
  private RepeatingRandomIterator<T> multiIndexerRepeatingIterator(
      Object queryCompositeKey,
      SequencedCollection<Key_> indexKeyCollection,
      RandomGenerator workingRandom) {
    return ContainingAnyOfIndexer.multiIndexerRepeatingIterator(
  ...
  /**
   * Buckets here are disjoint by construction (one key per tuple, see {@link #modifyKeyUnpacker}),
   * so {@link MultiBucketUniqueRandomIterator} applies directly: every matching, non-empty bucket
   * is walked upfront to learn its size, and weighted, uniform-without-replacement sampling takes
   * care of the rest.
   */
  private UniqueRandomIterator<T> uniqueRandomIteratorManyKeys(
    ...
    for (var indexKey : indexKeyCollection) {
      ...
      bucketIteratorList.add(
          downstreamIndexer.uniqueRandomIterator(queryCompositeKey, workingRandom));
    }
    if (distributionSum == 0) {
      return UniqueRandomIterator.empty();
    }
    return new MultiBucketUniqueRandomIterator<>(
        bucketIteratorList, distribution, distributionSum, workingRandom);
  }
  ...
  private final class DefaultIterator implements Iterator<T> {
    ...
    @Nullable private Iterator<T> downstreamIterator = null;
