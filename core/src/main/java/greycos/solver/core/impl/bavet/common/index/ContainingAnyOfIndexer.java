...
@NullMarked
final class ContainingAnyOfIndexer<T, Key_, KeyCollection_ extends SequencedCollection<Key_>>
  ...
  public ContainingAnyOfIndexer(
      KeyUnpacker<Key_> keyUnpacker, Supplier<Indexer<T>> downstreamIndexerSupplier) {
  ...
  public ListEntry<T> put(Object modifyCompositeKey, T tuple) {
    ...
    return put(
        modifyCompositeKey,
        downstreamIndexerMap,
        downstreamIndexerSupplier,
        tuple,
        indexKeyCollection);
  }

  static <T, Key_, KeyCollection_ extends SequencedCollection<Key_>> ListEntry<T> put(
      Object modifyCompositeKey,
      Map<Key_, Indexer<T>> downstreamIndexerMap,
    ...
    for (var indexKey : indexKeyCollection) {
      ...
      // Even though this method puts a tuple in multiple downstreamIndexers, it does not break
      // size() or forEach()
  ...
  static <T, Key_, KeyCollection_ extends SequencedCollection<Key_>> void remove(
      Object modifyCompositeKey,
      Map<Key_, Indexer<T>> downstreamIndexerMap,
      ListEntry<T> entry,
      KeyCollection_ indexKeyCollection) {
  ...
  private int sizeManyKeys(Object queryCompositeKey, KeyCollection_ indexKeyCollection) {
    ...
    var iterator =
        new DefaultIterator(
            queryCompositeKey, indexKeyCollection); // Avoid duplicating iteration logic
  ...
  private void forEachSingleKey(
      Object queryCompositeKey, KeyCollection_ indexKeyCollection, Consumer<T> tupleConsumer) {
  ...
  private Iterator<T> iteratorSingleKey(
      Object queryCompositeKey, KeyCollection_ indexKeyCollection) {
    var indexKey = indexKeyCollection.iterator().next();
    var downstreamIndexer = downstreamIndexerMap.get(indexKey);
    if (downstreamIndexer == null) {
      return Collections.emptyIterator();
  ...
  /**
   * Picks a key by its downstream bucket size, then a random element from within that bucket, with
   * replacement; never ends and never needs to track duplicates, so there is no shrinking
   * distribution and no removed set to maintain.
   */
  private RepeatingRandomIterator<T> multiIndexerRepeatingIterator(
      Object queryCompositeKey, KeyCollection_ indexKeyCollection, RandomGenerator workingRandom) {
    return multiIndexerRepeatingIterator(
        queryCompositeKey, downstreamIndexerMap, indexKeyCollection, workingRandom);
  }

  static <T, Key_, KeyCollection_ extends SequencedCollection<Key_>>
      RepeatingRandomIterator<T> multiIndexerRepeatingIterator(
    ...
    for (var indexKey : indexKeyCollection) {
      ...
      if (downstreamIndexer == null) {
        ...
        downstreamIteratorList.add(
            downstreamIndexer.randomIterator(queryCompositeKey, workingRandom));
        distributionSum += distribution[index];
      }
      index++;
    }
    return new WeightedRepeatingIterator<>(
        downstreamIteratorList, distribution, distributionSum, workingRandom);
  }
  ...
  private UniqueRandomIterator<T> uniqueRandomIteratorSingleKey(
      Object queryCompositeKey, KeyCollection_ indexKeyCollection, RandomGenerator workingRandom) {
    var downstreamIndexer = downstreamIndexerMap.get(indexKeyCollection.iterator().next());
    if (downstreamIndexer == null) {
      return UniqueRandomIterator.empty();
  ...
  /**
   * Unlike {@link ComparisonIndexer} or {@code ContainedInIndexer}, whose buckets for different
   * query keys are disjoint, a single tuple here can be reachable under more than one query key
   * (that is the whole point of {@link Joiners#containingAnyOf(Function, Function)}). A
   * bucket-then-uniform-inside-bucket sampler cannot be made uniform over the distinct tuples when
   * buckets overlap: for buckets {@code X = {t1, t2}}, {@code Y = {t2, t3}}, whatever weight is put
   * on each bucket, {@code P(t1) + P(t3) = 1/2} always, since each bucket contributes half of its
   * own probability mass to its non-shared element; but uniformity over the 3 distinct tuples needs
   * {@code P(t1) + P(t3) = 2/3}. No weight assignment closes that gap.
   *
   * <p>So instead of weighting buckets, this drains the already tuple-deduplicating {@link
   * DefaultIterator} into a plain list and delegates to {@link
   * UniqueRandomIterator#of(ElementAwareArrayList, RandomGenerator)}, which is exact over any list.
   */
  private UniqueRandomIterator<T> uniqueRandomIteratorManyKeys(
      Object queryCompositeKey, KeyCollection_ indexKeyCollection, RandomGenerator workingRandom) {
