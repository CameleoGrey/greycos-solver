...
/**
 * Accepts a single dataset coming from one enumerating stream, and provides {@link Move} iterators
...
public final class UniMoveStream<Solution_, A> implements InnerMoveStream<Solution_> {
  ...
  public MoveIterable<Solution_> getMoveIterable(NeighborhoodSession neighborhoodSession) {
    var context =
        new UniMoveStreamContext<>(
            (DefaultNeighborhoodSession<Solution_>) neighborhoodSession, dataset, moveConstructor);
