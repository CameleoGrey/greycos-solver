...
import greycos.solver.core.impl.bavet.common.AbstractNodeBuildHelper;
import greycos.solver.core.impl.bavet.common.BavetStream;
...
import greycos.solver.core.impl.neighborhood.stream.enumerating.collector.NeighborhoodsCollectorUtils;
...
public sealed interface NeighborhoodsGroupNodeConstructor<Solution_, Tuple_ extends Tuple>
  ...
  static <Solution_, A, GroupKey_>
      NeighborhoodsGroupNodeConstructor<Solution_, UniTuple<GroupKey_>> uniOneKeyGroupBy(
          UniNeighborhoodsMapper<Solution_, A, GroupKey_> key) {
    return new NeighborhoodsGroupNodeConstructorWithoutAccumulate<>(
        key,
        view ->
            GroupNodeConstructor.oneKeyGroupBy(
                key.toFunction(view), Group1Mapping0CollectorUniNode::new));
  }

  @SuppressWarnings("unchecked")
  static <Solution_, A, Result_>
      NeighborhoodsGroupNodeConstructor<Solution_, UniTuple<Result_>> uniZeroKeysGroupBy(
          UniNeighborhoodsCollector<Solution_, A, ?, Result_> collector) {
    return new NeighborhoodsGroupNodeConstructorWithAccumulate<>(
        collector,
        view ->
  ...
  static <Solution_, A, GroupKey_, Result_>
      NeighborhoodsGroupNodeConstructor<Solution_, BiTuple<GroupKey_, Result_>>
          uniOneKeyAndCollectorGroupBy(
              UniNeighborhoodsMapper<Solution_, A, GroupKey_> key,
              UniNeighborhoodsCollector<Solution_, A, ?, Result_> collector) {
    return new NeighborhoodsGroupNodeConstructorWithAccumulate<>(
        new Pair<>(key, collector),
        view ->
            GroupNodeConstructor.oneKeyGroupBy(
                key.toFunction(view),
                NeighborhoodsCollectorUtils.toConstraintCollector(
  ...
  static <Solution_, A, B, GroupKey_>
      NeighborhoodsGroupNodeConstructor<Solution_, UniTuple<GroupKey_>> biOneKeyGroupBy(
          BiNeighborhoodsMapper<Solution_, A, B, GroupKey_> key) {
    return new NeighborhoodsGroupNodeConstructorWithoutAccumulate<>(
        key,
        view ->
            GroupNodeConstructor.oneKeyGroupBy(
                key.toBiFunction(view), Group1Mapping0CollectorBiNode::new));
  }
    ...
    return new NeighborhoodsGroupNodeConstructorWithoutAccumulate<>(
        ...
        view ->
            GroupNodeConstructor.twoKeysGroupBy(
                keyA.toBiFunction(view),
                keyB.toBiFunction(view),
                Group2Mapping0CollectorBiNode::new));
  }

  @SuppressWarnings("unchecked")
  static <Solution_, A, B, Result_>
      NeighborhoodsGroupNodeConstructor<Solution_, UniTuple<Result_>> biZeroKeysGroupBy(
