...
import greycos.solver.core.impl.bavet.bi.IndexedIfExistsBiNode;
import greycos.solver.core.impl.bavet.bi.UnindexedIfExistsBiNode;
...
final class BavetIfExistsBiConstraintStream<Solution_, A, B, C>
  ...
  public BavetIfExistsBiConstraintStream(
  ...
  public <Score_ extends Score<Score_>> void buildNode(
    ...
    var positionTracker =
        buildHelper.getTupleStorePositionTracker(
            this, parentAB.getTupleSource(), parentBridgeC.getTupleSource());
    var node =
        indexerFactory.hasJoiners()
            ? (filtering == null
                ? new IndexedIfExistsBiNode<>(
                    shouldExist, indexerFactory, downstream, positionTracker)
                : new IndexedIfExistsBiNode<>(
                    shouldExist, indexerFactory, downstream, filtering, positionTracker))
            : (filtering == null
                ? new UnindexedIfExistsBiNode<>(shouldExist, downstream, positionTracker)
                : new UnindexedIfExistsBiNode<>(
  ...
  public boolean equals(Object object) {
    ...
    BavetIfExistsBiConstraintStream<?, ?, ?, ?> that =
        (BavetIfExistsBiConstraintStream<?, ?, ?, ?>) object;
    /*
    ...
    return shouldExist == that.shouldExist
        && Objects.equals(parentAB, that.parentAB)
        && Objects.equals(parentBridgeC.getParent(), that.parentBridgeC.getParent())
