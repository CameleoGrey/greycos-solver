...
/**
 * Adds a per-A index on top of the plain flat tuple list {@link AbstractLeftDatasetInstance}
 * already maintains, so {@code CachedBiDatasetInstance}'s per-A queries do not need to scan every
 * materialized pair and reject non-matches, and therefore need no bail-out at all.
 */
@NullMarked
public final class BiLeftDatasetInstance<Solution_, A, B>
  ...
  /**
   * Built again after each change of the dataset, in {@link #indexByA()}; it is not incremental. It
   * does not need to be: the dataset only changes in {@code NeighborhoodSession.settle()}, which
   * the solver calls at the start and at the end of a step, but never during move iteration. One
   * step therefore does one rebuild at the most, and all queries of that step share it.
   *
   * <p>Can easily be made incremental, if profiling shows the rebuild as a significant issue.
   */
  ...
  private Map<@Nullable A, ElementAwareArrayList<BiTuple<A, B>>> indexByA() {
    if (indexByA == null) {
      ...
      for (var tuple : this) {
        freshIndexByA
            .computeIfAbsent(tuple.getA(), ignored -> new ElementAwareArrayList<>())
            .add(tuple);
      }
  ...
  /**
   * As defined by {@link #iterator(RandomGenerator)}, restricted to tuples paired with the given
   * left value. No filter, no bail-out needed: {@link #bucketOrEmpty} is already exactly that
   * restriction.
   */
  public RepeatingRandomIterator<BiTuple<A, B>> randomIterator(
      @Nullable A a, RandomGenerator workingRandom) {
    return RepeatingRandomIterator.of(bucketOrEmpty(a), workingRandom);
  }

  /**
   * As defined by {@link #exhaustiveIterator(RandomGenerator)}, restricted to tuples paired with
   * the given left value.
   */
  public UniqueRandomIterator<BiTuple<A, B>> exhaustiveRandomIterator(
      @Nullable A a, RandomGenerator workingRandom) {
