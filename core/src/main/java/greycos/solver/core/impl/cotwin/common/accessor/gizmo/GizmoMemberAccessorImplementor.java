...
public final class GizmoMemberAccessorImplementor {
  /**
   * Generates the constructor and implementations of {@link AbstractGizmoMemberAccessor} methods
   * for the given {@link Member}.
   *
   ...
   */
  public static void defineAccessorFor(
      String className, ClassOutput classOutput, GizmoMemberInfo memberInfo) {
    ...
    gizmo.class_(
        ...
        classCreator -> {
          ...
          if (memberInfo.readMethodWithParameter()) {
            methodParameterType =
                classCreator.field(
                    "readMethodParameterType",
                    fieldCreator -> {
                      fieldCreator.final_();
                      fieldCreator.setType(Type.class);
                      fieldCreator.setInitial(
                          (Class<?>) memberInfo.descriptor().getMethodParameterType());
                    });
          }
          ...
          var generatedClassInfo =
              new GeneratedClassInfo(
                  classCreator, genericType, methodParameterType, annotatedElement, memberInfo);
  ...
  private static Class<? extends AbstractGizmoMemberAccessor> getCorrectSuperclass(
    ...
    if (supportsSetter.get()) {
      return methodWithParameter.get()
          ? AbstractReadWriteExtendedGizmoMemberAccessor.class
          : AbstractReadWriteGizmoMemberAccessor.class;
    } else {
      return methodWithParameter.get()
          ? AbstractReadOnlyExtendedGizmoMemberAccessor.class
   ...
   * @param annotationClass The annotation it was annotated with (used for error reporting)
   * @param accessorInfo additional information of the accessor
   * @param gizmoClassLoader never null
   * @return A new MemberAccessor that uses Gizmo generated bytecode. Will generate the bytecode the
   *     first type it is called for a member, unless a classloader has been set, in which case no
   *     Gizmo code will be generated.
   */
  static MemberAccessor createAccessorFor(
      Member member,
  ...
  private static MemberAccessor createInstance(
      String className, GizmoClassLoader gizmoClassLoader) {
    try {
        ...
        constructorCreator ->
            constructorCreator.body(
                blockCreator -> {
                  ...
                  blockCreator.invokeSpecial(
                      ConstructorDesc.of(classCreator.superClass()), thisObj);

                  var declaringClass =
                      blockCreator.localVar(
                          "declaringClass",
                          Const.of(ClassDesc.of(memberInfo.descriptor().getDeclaringClassName())));
                  memberInfo
                      .descriptor()
                          ...
                          md -> {
                            ...
                            var type =
                                blockCreator.invokeVirtual(
                                    ...
                                    field);
                            blockCreator.set(
                                thisObj.field(generatedClassInfo.genericTypeField), type);
                            blockCreator.set(
                                thisObj.field(generatedClassInfo.annotatedElementField), field);
                          });
                  ...
                  memberInfo
                      ...
                      .whenMetadataIsOnMethod(
                          md -> {
                            ...
                            // we must specify the type of that parameter to locate the declared
                            // method
                            var parameterArray =
                                generatedClassInfo.readMethodParameterTypeField() != null
                                    ? blockCreator.newArray(
                                        Class.class,
                                        Const.of(
                                            (Class<?>)
                                                generatedClassInfo
                            ...
                            if (memberInfo.returnTypeRequired()) {
                              // We create a field to store the result, only if the called method
                              // has a return type.
                            ...
                            blockCreator.set(
                                thisObj.field(generatedClassInfo.annotatedElementField), method);
                          });
   ...
   */
  private static void assertIsGoodMethod(
      MethodDesc method, boolean returnTypeRequired, boolean readMethodWithParameter) {
    // V = void return type
    ...
    if (!readMethodWithParameter && method.parameterCount() != 0) {
      ...
      throw new IllegalStateException(
          "The getterMethod (%s) must not have any parameters, but has parameters (%s)."
              .formatted(methodName, Arrays.toString(method.parameterTypes().toArray())));
    }
    if (methodName.startsWith("get")) {
      if (method.returnType().equals(ConstantDescs.CD_void)) {
        throw new IllegalStateException(
            "The getterMethod (%s) must have a non-void return type.".formatted(methodName));
      }
      ...
      if (returnTypeRequired && method.returnType().equals(ConstantDescs.CD_void)) {
        throw new IllegalStateException(
            "The readMethod (%s) must have a non-void return type.".formatted(methodName));
      }
    }
  }
  ...
  private static void assertIsGoodMethod(
    ...
    if (methodName.startsWith("get")) {
        ...
        throw new IllegalStateException(
            """
                ...
                .formatted(
                    methodName,
                    annotationClass.getSimpleName(),
                    method.returnType().descriptorString(),
      ...
      if (returnTypeRequired && method.returnType().equals(ConstantDescs.CD_void)) {
        throw new IllegalStateException(
            "The readMethod (%s) with a %s annotation must have a non-void return type."
                .formatted(methodName, annotationClass.getSimpleName()));
      }
    }
  }
   ...
   * </pre>
   *
   * If it is a getter method, "get" is removed and the first letter become lowercase
   */
                    ...
                    .whenIsMethod(
                        method -> {
                          ...
                          if (annotationClass == null) {
                            assertIsGoodMethod(
                                method,
                                memberInfo.returnTypeRequired(),
                                memberInfo.readMethodWithParameter());
                          } else {
                            assertIsGoodMethod(
                                method,
                                memberInfo.returnTypeRequired(),
   ...
   * </pre>
   *
   * We are unable to load a non-primitive object constant, so we need to store it in the
   * implementor, which then can return us the Type when needed. The type is stored in
   * gizmoMemberAccessorNameToGenericType when this method is called.
   */
    ...
    classCreator.method(
        ...
        builder -> {
          ...
          builder.body(
              blockCreator ->
                  blockCreator.return_(
                      classCreator.this_().field(generatedClassInfo.genericTypeField)));
        });
  }
    ...
    classCreator.method(
        ...
        builder -> {
          ...
          builder.body(
              blockCreator ->
                  blockCreator.return_(
                      classCreator.this_().field(generatedClassInfo.readMethodParameterTypeField)));
        });
  }
   ...
   * </pre>
   *
   * The member MUST be public if not called in Quarkus (i.e. we don't delegate to the field
   * getter/setter). In Quarkus, we generate simple getter/setter for the member if it is private
   * (which get passed to the MemberDescriptor).
   */
          ...
          builder.body(
              blockCreator -> {
                var castedBean =
                    blockCreator.localVar(
                        "castedBean",
                        ClassDesc.of(memberInfo.descriptor().getDeclaringClassName()),
                        bean);
                if (memberInfo.returnTypeRequired()) {
                  blockCreator.return_(
   ...
   * Generates the following code:
   *
   * <p>For a method with returning type
   *
   * <pre>
              ...
              blockCreator -> {
                ...
                var castedValue =
                    blockCreator.localVar(
                        "castedValue",
                        ClassDesc.of(
                            memberInfo.descriptor().getMethodParameterType().getTypeName()),
                        value);
                if (memberInfo.returnTypeRequired()) {
                  blockCreator.return_(
                      memberInfo
                          .descriptor()
                          .readMemberValue(blockCreator, castedBean, castedValue));
                } else {
                  memberInfo.descriptor().readMemberValue(blockCreator, castedBean);
                  // Returns null as the called method has no return type
                  blockCreator.returnNull();
          ...
          builder.body(
              blockCreator -> {
                ...
                if (memberInfo.descriptor().writeMemberValue(blockCreator, castedBean, value)) {
                  ...
                  blockCreator.throw_(
                      blockCreator.new_(
                          UnsupportedOperationException.class, Const.of("Setter not supported")));
                }
              });
        });
  }

  private static MethodDesc getAnnotationMethod(
      Class<?> returnType, String methodName, Class<?>... parameters) {
    ...
    classCreator.method(
        ...
        builder -> {
          ...
          builder.body(
              blockCreator -> {
                var annotatedElement =
                    classCreator.this_().field(generatedClassInfo.annotatedElementField);
                blockCreator.return_(
                    blockCreator.invokeInterface(
                        getAnnotationMethod(Annotation.class, "getAnnotation", Class.class),
                        annotatedElement,
                        query));
              });
        });
          ...
          builder.body(
              blockCreator -> {
                ...
                blockCreator.return_(
                    blockCreator.invokeInterface(
                        getAnnotationMethod(
                            Annotation[].class, "getDeclaredAnnotationsByType", Class.class),
                        annotatedElement,
                        query));
              });
        });
  ...
  private record GeneratedClassInfo(
      ClassCreator classCreator,
      FieldDesc genericTypeField,
      FieldDesc readMethodParameterTypeField,
      FieldDesc annotatedElementField,
      GizmoMemberInfo memberInfo) {}
}
