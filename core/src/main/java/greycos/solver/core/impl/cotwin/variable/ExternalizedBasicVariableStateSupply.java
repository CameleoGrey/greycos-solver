...
@NullMarked
public final class ExternalizedBasicVariableStateSupply<Solution_>
    implements BasicVariableStateSupply<Solution_> {

  private final VariableDescriptor<Solution_> sourceVariableDescriptor;
  private final Consumer<Object> notifier;

  @Nullable private InverseRelationShadowVariableDescriptor<Solution_> shadowVariableDescriptor;
  @Nullable private Map<Object, Set<Object>> inverseEntitySetMap;

  public ExternalizedBasicVariableStateSupply(
  ...
  public void resetWorkingSolution(InnerScoreDirector<Solution_, ?> scoreDirector) {
    if (shadowVariableDescriptor == null) {
      ...
      sourceVariableDescriptor
          .getEntityDescriptor()
          .visitAllEntities(
              scoreDirector.getWorkingSolution(), entity -> insert(scoreDirector, entity));
    } else {
      forEachEntity(
          scoreDirector,
  ...
  private static <Solution_, Entity_> void forEachEntity(
  ...
  private void insert(InnerScoreDirector<Solution_, ?> scoreDirector, Object entity) {
    if (shadowVariableDescriptor == null) {
      ...
      var inverseEntitySet =
          inverseEntitySetMap.computeIfAbsent(
              value, k -> Collections.newSetFromMap(new IdentityHashMap<>()));
  ...
  public String toString() {
    return "%s(%s)"
        .formatted(getClass().getSimpleName(), sourceVariableDescriptor.getVariableName());
  }
}
