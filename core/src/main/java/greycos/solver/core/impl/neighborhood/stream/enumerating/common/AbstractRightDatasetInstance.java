...
public abstract class AbstractRightDatasetInstance<Solution_, Right_>
  ...
  /**
   * The number of tuples currently live in this dataset, across every composite key. Maintained
   * incrementally in {@link #insert}/{@link #retract} so {@link #totalSize()} is O(1); {@link
   * Indexer} has no key-independent size query to recompute it from.
   */
  private int totalSize;

  protected AbstractRightDatasetInstance(
  ...
  /** As defined by {@link UniDatasetInstance#size()}, only accepts a key for joins. */
  public int size(Object compositeKey) {
    return indexer.size(compositeKey);
  }

  /**
   * The number of tuples currently live in this dataset, across every composite key. Used as the
   * denominator that makes {@code BiRandomMoveIterator}'s per-pair probability uniform: an indexed
   * key's {@link #size(Object)} is always a subset of this total.
   */
  ...
  /**
   * As defined by {@link UniDatasetInstance#iterator(RandomGenerator)}, only accepts a key for
   * joins.
   */
  ...
  /**
   * As defined by {@link UniDatasetInstance#exhaustiveIterator(RandomGenerator)}, only accepts a
   * key for joins.
   */
  public UniqueRandomIterator<UniTuple<Right_>> uniqueRandomIterator(
      Object compositeKey, RandomGenerator workingRandom) {
    return indexer.uniqueRandomIterator(compositeKey, workingRandom);
  }

  /**
   * As defined by {@link #uniqueRandomIterator(Object, RandomGenerator)}, but only returning
   * elements matching the given predicate.
   */
  public Iterator<UniTuple<Right_>> uniqueRandomIterator(
      Object compositeKey, RandomGenerator workingRandom, Predicate<UniTuple<Right_>> predicate) {
    return new FilteringIterator<>(
        indexer.uniqueRandomIterator(compositeKey, workingRandom), predicate);
  }
}
