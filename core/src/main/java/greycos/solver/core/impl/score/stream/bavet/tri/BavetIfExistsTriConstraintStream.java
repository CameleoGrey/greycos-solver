...
import greycos.solver.core.impl.bavet.tri.IndexedIfExistsTriNode;
import greycos.solver.core.impl.bavet.tri.UnindexedIfExistsTriNode;
...
final class BavetIfExistsTriConstraintStream<Solution_, A, B, C, D>
  ...
  public BavetIfExistsTriConstraintStream(
  ...
  public <Score_ extends Score<Score_>> void buildNode(
    ...
    var positionTracker =
        buildHelper.getTupleStorePositionTracker(
            this, parentABC.getTupleSource(), parentBridgeD.getTupleSource());
    var node =
        indexerFactory.hasJoiners()
            ? (filtering == null
                ? new IndexedIfExistsTriNode<>(
                    shouldExist, indexerFactory, downstream, positionTracker)
                : new IndexedIfExistsTriNode<>(
                    shouldExist, indexerFactory, downstream, filtering, positionTracker))
            : (filtering == null
                ? new UnindexedIfExistsTriNode<>(shouldExist, downstream, positionTracker)
                : new UnindexedIfExistsTriNode<>(
  ...
  public boolean equals(Object object) {
    ...
    BavetIfExistsTriConstraintStream<?, ?, ?, ?, ?> that =
        (BavetIfExistsTriConstraintStream<?, ?, ?, ?, ?>) object;
    /*
    ...
    return shouldExist == that.shouldExist
        && Objects.equals(parentABC, that.parentABC)
        && Objects.equals(parentBridgeD.getParent(), that.parentBridgeD.getParent())
