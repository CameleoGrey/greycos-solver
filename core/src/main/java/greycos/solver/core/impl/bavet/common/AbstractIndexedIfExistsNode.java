 ...
 * IndexerFactory#isFusedEqualIndexEligible()}). The non-unified path keeps two parallel {@link
 * Indexer}s; the unified path (equal-bearing) keeps one {@link FusedEqualIndex} whose buckets
 * co-locate the left counters and the right tuples sharing an equal key, with the resolved bucket
 * cached on the tuple so same-key updates and retracts need no lookup. The counter /
 * filtering-tracker logic in {@link AbstractIfExistsNode} is identical for both.
 *
 * @param <LeftTuple_>
 * @param <Right_>
 */
public abstract class AbstractIndexedIfExistsNode<LeftTuple_ extends Tuple, Right_>
  ...
  // store slots.
  private final @Nullable FusedEqualIndex<ExistsCounter<LeftTuple_>, UniTuple<Right_>>
      fusedEqualIndex;
  ...
  public final void insertLeft(LeftTuple_ leftTuple) {
    ...
    updateCounterRight(
        leftTuple, compositeKey, counter, putLeftCounter(leftTuple, compositeKey, counter, false));
    initCounterLeft(counter);
  }

  private void updateCounterRight(
    ...
    if (!isFiltering) {
      ...
      forEachRightFromLeft(
          leftTuple, compositeKey, rightTuple -> updateCounterFromLeft(counter, rightTuple));
    }
  }
  ...
  public final void updateLeft(LeftTuple_ leftTuple) {
    ...
    if (oldCompositeKey.equals(newCompositeKey)) {
      ...
      if (!isFiltering) {
        ...
        forEachRightFromLeft(
            leftTuple, oldCompositeKey, rightTuple -> updateCounterFromLeft(counter, rightTuple));
        updateCounterLeft(counter);
      }
    } else {
      // sameBucket: equal prefix unchanged ⇒ keep & reuse the cached bucket (no top lookup, no
      // drop/recreate).
      var sameBucket =
  ...
  private void removeFromIndexerLeft(
      Object compositeKey,
      LeftTuple_ leftTuple,
      ListEntry<ExistsCounter<LeftTuple_>> counterEntry,
      boolean keepBucket) {
    if (useFusedEqualIndex) {
      ...
      if (!keepBucket) { // keepBucket: a same-bucket changed-key update re-adds immediately, so
        // don't drop it.
  ...
  private void updateIndexerLeft(
      Object compositeKey,
      ListEntry<ExistsCounter<LeftTuple_>> counterEntry,
      LeftTuple_ leftTuple,
      boolean keepBucket) {
  ...
  private void updateCounterLeft(UniTuple<Right_> rightTuple, Object compositeKey) {
    if (!isFiltering) {
      ...
      // No list object is needed; the slot starts null and the first tracker becomes the head.
      forEachLeftCounter(
          rightTuple, compositeKey, counter -> updateCounterFromRight(counter, rightTuple));
    }
  }
  ...
  public final void updateRight(UniTuple<Right_> rightTuple) {
    ...
    if (oldCompositeKey.equals(newCompositeKey)) {
      ...
      if (isFiltering) {
        ...
        forEachLeftCounter(
            rightTuple, oldCompositeKey, counter -> updateCounterFromRight(counter, rightTuple));
      }
    } else {
      // sameBucket: equal prefix unchanged ⇒ keep & reuse the cached bucket (no top lookup, no
      // drop/recreate).
      ...
      if (useFusedEqualIndex) {
        ...
        if (!sameBucket) { // keepBucket: a same-bucket changed-key update re-adds immediately, so
          // don't drop it.
      ...
      rightTuple.setStore(
          inputStoreIndexRightEntry, putRightTuple(rightTuple, newCompositeKey, sameBucket));
  ...
  /**
   * Adds a left counter to its side of the index, caching the resolved bucket on the left tuple
   * (unified path).
   */
  private ListEntry<ExistsCounter<LeftTuple_>> putLeftCounter(
      LeftTuple_ leftTuple,
      Object compositeKey,
      ExistsCounter<LeftTuple_> counter,
  ...
  /**
   * Adds a right tuple to its side of the index, caching the resolved bucket on the right tuple
   * (unified path).
   */
  private ListEntry<UniTuple<Right_>> putRightTuple(
  ...
  /**
   * Iterates the right tuples matching a left counter's composite key (its cached bucket, or {@code
   * indexerRight}).
   */
  private void forEachRightFromLeft(
  ...
  /**
   * Iterates the left counters matching a right tuple's composite key (its cached bucket, or {@code
   * indexerLeft}).
   */
  private void forEachLeftCounter(
