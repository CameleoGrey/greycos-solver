...
public class ListValueRangeStatistics<Solution_> {
  @Nullable private final ListVariableDescriptor<Solution_> listVariableDescriptor;
  private final ValueRangeManager<Solution_> valueRangeManager;

  public ListValueRangeStatistics(
      @Nullable ListVariableDescriptor<Solution_> listVariableDescriptor,
  ...
  public long computeListProblemScaleLog(long logBase) {
    ...
    if (allowsUnassignedValues) {
      // Unassigned values are treated as if they are assigned to a virtual entity to simplify
      // calculations
      entityCount++;
    }
    ...
    for (var value : valueSet) {
      validPercentageLog +=
          MathUtils.getScaledApproximateLog(
              MathUtils.LOG_PRECISION,
              logBase,
              ((double) reachableValues.getReachableEntitiesSize(value) + additionalCount)
                  / entityCount);
    }

    return MathUtils.getPossibleArrangementsScaledApproximateLog(
            MathUtils.LOG_PRECISION, logBase, valueCount, entityCount)
        + validPercentageLog;
  }
}
