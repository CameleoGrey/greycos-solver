...
/**
 * An {@link Indexer} for LT/LTE/GT/GTE joins, keyed by a single {@link Comparable} value and backed
 * by a {@link ScalingNavigableMap} of key to downstream {@link Indexer}.
 *
 * <p>GT/GTE are handled by iterating the map in reverse ({@link #reverseOrder}) rather than by
 * using a reversed comparator or sub-map views, so both directions can share the same ascending,
 * comparator-free {@link ScalingNavigableMap} and its fast, non-comparator {@code TreeMap} lookup
 * path. {@link #boundaryReached} folds that direction, plus LTE/GTE inclusivity ({@link
 * #hasOrEquals}), into a single check used to stop a range scan.
 *
 * <p>Every query operation ({@link #size}, {@link #forEach}, {@link #iterator}, {@link
 * #randomIterator}) has separate array-mode and tree-mode implementations, dispatched on {@link
 * ScalingNavigableMap#isArrayBased()}.
 *
 * <p>This class was heavily benchmarked; it is recommended to assume that most decisions made here
 * are performance-driven and not necessarily obvious or intuitive. Any changes should also be based
 * on benchmarks.
 *
 * @param <T> the element type, see {@link Indexer}
 * @param <Key_> the type of the comparison key, unpacked from the composite key by {@link
 *     #keyUnpacker}
 */
@NullMarked
final class ComparisonIndexer<T, Key_ extends Comparable<Key_>> implements Indexer<T> {
  ...
  // Package-private: tests in this package read arrayBased via this field, same as
  // ScalingNavigableMap's own.
  ...
  public ComparisonIndexer(
    ...
    this.reverseOrder =
        comparisonJoinerType == JoinerType.GREATER_THAN
            || comparisonJoinerType == JoinerType.GREATER_THAN_OR_EQUAL;
    this.hasOrEquals =
        comparisonJoinerType == JoinerType.GREATER_THAN_OR_EQUAL
  ...
  /**
   * Looks up the entry once (comparisonMap.indexOf(), one binary search) and reuses the same index
   * to remove it if needed, instead of a second indexOf()-equivalent lookup for the same key (as
   * comparisonMap.remove(key) alone would do).
   */
  ...
  private int sizeSingleIndexer(Object compositeKey) {
    return comparisonMap.isArrayBased()
        ? sizeSingleIndexerArray(compositeKey)
        : sizeSingleIndexerTree(compositeKey);
  }
  ...
  /**
   * Whether {@code entryKey}, and every key past it in iteration order, fails to match {@code
   * indexKey} and the range scan (in {@code size}/{@code forEach}/{@code iterator}/{@code
   * randomIterator}) should stop.
   *
   * <p>{@code comparisonMap} is always iterated ascending by natural order (see {@link
   * ScalingNavigableMap}); {@link #reverseOrder} instead flips the sign of the comparison here, so
   * GT/GTE effectively scan the same ascending storage from the other end, without needing a
   * reversed comparator or sub-map view.
   */
  private boolean boundaryReached(Key_ entryKey, Key_ indexKey) {
    ...
    // so negation is not safe here for a Comparable whose compareTo() can return exactly
    // Integer.MIN_VALUE.
    var comparison = reverseOrder ? indexKey.compareTo(entryKey) : entryKey.compareTo(indexKey);
    if (comparison >= 0) {
      // Boundary condition reached when we're out of bounds entirely, or when GTE/LTE is not
      // allowed.
  ...
  private int sizeManyIndexers(Object compositeKey) {
    return comparisonMap.isArrayBased()
        ? sizeManyIndexersArray(compositeKey)
        : sizeManyIndexersTree(compositeKey);
  }
  ...
  /**
   * Resolves the single bucket this indexer holds when {@link #comparisonMap} has exactly one
   * entry, or null if {@code queryCompositeKey}'s boundary is already reached at that one bucket
   * (nothing matches). Shared by {@link #iteratorSingleIndexer}, {@link
   * #singleIndexerRepeatingIterator} and {@link #singleIndexerUniqueIterator}: only the accessor
   * each calls on the result, and the {@code empty()} marker it returns instead, differ between
   * them.
   */
  ...
  private RepeatingRandomIterator<T> singleIndexerRepeatingIterator(
      Object queryCompositeKey, RandomGenerator workingRandom) {
    var indexer = singleIndexer(queryCompositeKey);
    return indexer == null
        ? RepeatingRandomIterator.empty()
  ...
  /**
   * Every matching bucket is disjoint from every other (each is a distinct map key), so {@link
   * MultiBucketUniqueRandomIterator} applies directly: {@link #collectMatchingBuckets} walks every
   * matching, non-empty bucket upfront to learn its size, and weighted uniform-without-replacement
   * sampling takes care of the rest.
   */
  private UniqueRandomIterator<T> multiIndexerUniqueIterator(
      Object queryCompositeKey, RandomGenerator workingRandom) {
    ...
    return new MultiBucketUniqueRandomIterator<>(
        bucketIteratorList, distribution, sum, workingRandom);
  }

  /**
   * Walks every bucket matching {@code queryCompositeKey}'s boundary (in {@link #comparisonMap}'s
   * iteration order, respecting {@link #reverseOrder}), handing each one to {@code
   * matchingBucketConsumer}, empty or not. Shared by {@link RepeatingRandomIterator} and {@link
   * #multiIndexerUniqueIterator}, both of which need every matching bucket upfront: there is no
   * boundary-ordered walk to fall back on for either, since a never-ending downstream would never
   * let the walk advance past the first bucket.
   */
  private void collectMatchingBuckets(
      Object queryCompositeKey, Consumer<Indexer<T>> matchingBucketConsumer) {
    var indexKey = keyUnpacker.apply(queryCompositeKey);
    if (comparisonMap.isArrayBased()) {
  ...
  private void collectMatchingBucketsFromArray(
      Key_ indexKey, Consumer<Indexer<T>> matchingBucketConsumer) {
    var arraySize = comparisonMap.size();
    var i = reverseOrder ? arraySize - 1 : 0;
    var step = reverseOrder ? -1 : 1;
    while (i >= 0 && i < arraySize) {
  ...
  private void collectMatchingBucketsFromTree(
      Key_ indexKey, Consumer<Indexer<T>> matchingBucketConsumer) {
    var entryIterator = comparisonMap.iterator(reverseOrder);
    while (entryIterator.hasNext()) {
      var entry = entryIterator.next();
  ...
  private UniqueRandomIterator<T> singleIndexerUniqueIterator(
      Object queryCompositeKey, RandomGenerator workingRandom) {
    var indexer = singleIndexer(queryCompositeKey);
    return indexer == null
        ? UniqueRandomIterator.empty()
  ...
  /**
   * Handles both array-mode and tree-mode iteration in a single class, branching internally ({@link
   * #advanceFromArray}/{@link #advanceFromTree}) rather than splitting into two {@code Iterator}
   * implementations behind a shared type.
   */
  private final class DefaultIterator implements Iterator<T> {
    ...
    // Tree mode: entries pulled from here. Array mode: this is null and the array cursor fields are
    // used instead.
  ...
  /**
   * Every matching bucket is disjoint from every other (each is a distinct map key), so {@link
   * WeightedRepeatingIterator} applies directly: {@link #collectMatchingBuckets} walks every
   * matching, non-empty bucket upfront to learn its size, and weighted, with-replacement sampling
   * takes care of the rest.
   */
  private RepeatingRandomIterator<T> multiIndexerRepeatingIterator(
      Object queryCompositeKey, RandomGenerator workingRandom) {
    ...
    // (sampleWithDistribution() only ever walks its non-zero prefix), so no trim to the real count
    // is needed.
    var distribution = new int[comparisonMap.size()];
    var bucketCount = new MutableInt();
    var distributionSum = new MutableInt();
    ...
    return new WeightedRepeatingIterator<>(
        downstreamIteratorList, distribution, distributionSum.intValue(), workingRandom);
  }
}
