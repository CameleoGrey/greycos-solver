...
import greycos.solver.core.impl.heuristic.selector.move.composite.CompositeMoveSelector;
...
/**
 * Mixes a legacy {@link MoveSelector} with a {@link NeighborhoodsMoveSelector}, drawing from each
 * side in proportion to how many movers it holds: the number of leaf move selectors on one side,
 * the number of neighborhoods on the other.
 *
...
public final class MixedMoveSelector<Solution_> extends CompositeMoveSelector<Solution_> {
  ...
  /**
   * Counts the leaf move selectors reachable from the given selector, recursing through nested
   * unions and through the decorators (filtering, caching/sorting/shuffling, and
   * selected-count-limit) that may wrap them. A {@code ProbabilityMoveSelector} is deliberately
   * left as an opaque leaf: it weights a single selector's own moves, which is unrelated to
   * weighting one selector against another.
   */
    ...
    if (moveSelector instanceof UnionMoveSelector<?> unionMoveSelector) {
      if (unionMoveSelector.getSelectorProbabilityWeightFactory() != null) {
        throw new UnsupportedOperationException(
            "Probability-weighted move selectors are not supported together with the Neighborhoods"
                + " API.");
      }
      var count = 0;
  ...
  public Iterator<Move<Solution_>> iterator() {
    return new MixedMoveIterator<>(
        moveSelector.iterator(),
        moveSelectorWeight,
        neighborhoodsMoveSelector.iterator(),
        neighborhoodWeight,
        workingRandom);
  }
  ...
  public String toString() {
    return "Mixed(%s, %s)".formatted(moveSelector, neighborhoodsMoveSelector);
  }
  ...
  private static final class MixedMoveIterator<Solution_> implements Iterator<Move<Solution_>> {
    ...
    MixedMoveIterator(
        Iterator<Move<Solution_>> moveSelectorIterator,
        int moveSelectorWeight,
        Iterator<Move<Solution_>> neighborhoodIterator,
        int neighborhoodWeight,
