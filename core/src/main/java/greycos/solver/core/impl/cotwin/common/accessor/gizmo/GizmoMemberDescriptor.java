...
public final class GizmoMemberDescriptor {
  ...
  /**
   * The name of a member. For a field, it is the field name. For a method, if it is a getter, the
   * method name without "get"/"is" and the first letter lowercase; otherwise, the method name.
   */
    ...
    if (member instanceof Field field) {
      ...
      this.name =
          ReflectionHelper.isGetterMethod(method)
              ? ReflectionHelper.getGetterPropertyName(member)
              : member.getName();
      this.methodParameterType =
          getMethodParameterType(method, accessorInfo.readMethodWithParameter());
      this.memberHandler =
  ...
  public GizmoMemberDescriptor(
      String name, FieldDesc fieldDescriptor, Class<?> declaringClass, AccessorInfo accessorInfo) {
    this.name = name;
  ...
  public GizmoMemberDescriptor(
      String name,
      MethodDesc memberDescriptor,
      MethodDesc metadataDescriptor,
    ...
    this.name = name;
    this.memberHandler =
        GizmoMemberHandler.of(declaringClass, (Class<?>) methodParameterType, memberDescriptor);
    this.metadataHandler =
        memberDescriptor == metadataDescriptor
            ? this.memberHandler
  ...
  public static Type getMethodParameterType(Method method, boolean methodWithParameter) {
    ...
    if (methodWithParameter && parameterCount > 1) {
      ...
      throw new IllegalStateException(
          "The getterMethod (%s) must have only one parameter (%s)."
   ...
   * Otherwise, do nothing. Returns self for chaining.
   *
  ...
  /**
   * If the member accessor is a method, pass the member's method descriptor to the provided
   * consumer. Otherwise, do nothing. Returns self for chaining.
   *
  ...
  /**
   * Write the bytecode for writing to this member. If there is no setter, it write the bytecode for
   * throwing the exception. Return true if it was able to write the member value.
   *
  ...
  /**
   * If the member metadata is on a field, pass the member's field descriptor to the provided
   * consumer. Otherwise, do nothing. Returns self for chaining.
   *
   * @param fieldDescriptorConsumer What to do if the member a field.
   * @return this
  ...
  /**
   * If the member metadata is on a method, pass the member's method descriptor to the provided
   * consumer. Otherwise, do nothing. Returns self for chaining.
   *
   * @param methodDescriptorConsumer What to do if the member a method.
   * @return this
   */
  public GizmoMemberDescriptor whenMetadataIsOnMethod(
      Consumer<MethodDesc> methodDescriptorConsumer) {
    metadataHandler.whenIsMethod(methodDescriptorConsumer);
    return this;
  }

  /**
   * Returns the declaring class name of the member in descriptor format. For instance, the
   * declaring class name of Object.toString() is "java/lang/Object".
   *
  ...
  private static Optional<MethodDesc> lookupSetter(
      Object memberDescriptor, Class<?> declaringClass, String name) {
  ...
  /**
   * Returns the member type (for fields) / return type (for methods) name. The name does not
