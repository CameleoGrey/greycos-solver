...
public abstract class AbstractJoiner<Right_> {
  ...
  /**
   * Whether indexers built from this joiner must support random access into their contents. Only
   * the Neighborhoods API needs it, to draw a random tuple from an indexer bucket;
   ...
   * preserving the original relative order within the equal group and within the rest. Subclasses
   * use it in {@code reorderedEqualsFirst()} so the comber can emit an equal-first merged joiner;
   * reordering ANDed joiners cannot change which pairs match.
   *
