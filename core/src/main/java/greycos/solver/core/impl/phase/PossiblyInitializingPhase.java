...
/**
 * Describes a phase that can be used to initialize a solution. {@link ConstructionHeuristicPhase}
 * is automatically an initializing phase. {@link CustomPhase} can be an initializing phase, if it
 * comes before the first {@link LocalSearchPhase}.
 *
...
public interface PossiblyInitializingPhase<Solution_> extends Phase<Solution_> {
  ...
  /**
   * Check if a phase should trigger the first initialized solution event. The first initialized
   * solution immediately precedes the first {@link LocalSearchPhase}.
   *
   ...
   * @param early If the phase terminated early without completing all steps. If true, this
   *     signifies a solution that is not fully initialized.
