...
import greycos.solver.core.impl.heuristic.selector.move.generic.list.ListSwapMoveSelector;
...
/**
 * The decorator returns a list of reachable values for a specific value. It enables the creation of
 * a filtering tier when using entity-provided value ranges, ensuring only valid and reachable
 * values are returned. A value is considered reachable to another value if both exist within their
 * respective entity value ranges.
 *
 ...
 * that once a value is selected using the left value selector, the right node can filter out all
 * non-reachable values and generate a valid move. A move is considered valid only if both entities
 * accept each other's values. The filtering process of invalid values allows the solver to explore
...
public final class FilteringValueRangeSelector<Solution_>
  ...
  public FilteringValueRangeSelector(
      IterableValueSelector<Solution_> nonReplayingValueSelector,
  ...
  /**
   * The expected replayed value corresponds to the selected value when the replaying selector has
   * the next value. Once it is selected, it will be reused until a new value is replayed by the
   * recorder selector.
   */
  ...
  private abstract class AbstractFilteringValueRangeIterator<Entity_, Value_>
    ...
    @Nullable private Value_ currentUpcomingValue;
    @Nullable private Entity_ currentUpcomingEntity;
    @Nullable private List<Value_> currentUpcomingValueList;
    ...
    /**
     * This method initializes the basic structure required for the child iterators, including the
     * upcoming entity and the upcoming list.
     *
    ...
    private void loadValues(@Nullable Value_ upcomingValue) {
      ...
      this.hasData = !currentUpcomingValueList.isEmpty();
      this.initialized = true;
    }

    abstract void processUpcomingValue(Value_ upcomingValue, List<Value_> upcomingList);
    ...
    boolean isReachable(Value_ destinationValue) {
      ...
      if (checkSourceAndDestination) {
        return reachableValues.isEntityReachable(
            ...
            && reachableValues.isEntityReachable(
                Objects.requireNonNull(destinationValue), currentUpcomingEntity);
      } else {
        return reachableValues.isEntityReachable(
            Objects.requireNonNull(currentUpcomingValue), destinationEntity);
      }
