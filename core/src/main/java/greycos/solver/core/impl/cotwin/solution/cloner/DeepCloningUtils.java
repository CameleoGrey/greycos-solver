...
public final class DeepCloningUtils {
  ...
  /**
   * Gets the deep cloning decision for a particular value assigned to a field, memoizing the
   * result.
   *
   * @param field the field to get the deep cloning decision of
   * @param owningClass the class that owns the field; can be different from the field's declaring
   *     class (ex: subclass)
   * @param actualValueClass the class of the value that is currently assigned to the field; can be
   *     different from the field type (ex: for the field "List myList", the actual value class
   *     might be ArrayList).
   * @return true iff the field should be deep cloned with a particular value.
   */
  public static boolean isDeepCloned(
   ...
   */
  public static boolean isFieldDeepCloned(
      SolutionDescriptor<?> solutionDescriptor, Field field, Class<?> owningClass) {
  ...
  public static boolean needsDeepClone(
      SolutionDescriptor<?> solutionDescriptor, Field field, Class<?> owningClass) {
  ...
  /**
   * Return true only if a field represents an entity property on the solution class. An entity
   * property is one who type is a PlanningEntity or a collection of PlanningEntity.
   *
   * @param field The field to get the deep cloning decision of
   * @param owningClass The class that owns the field; can be different from the field's declaring
   *     class (ex: subclass).
   * @return True only if the field is an entity property on the solution class. May return false if
   *     the field getter/setter is complex.
   */
  static boolean isFieldAnEntityPropertyOnSolution(
      SolutionDescriptor<?> solutionDescriptor, Field field, Class<?> owningClass) {
   ...
   */
  private static boolean isFieldAnEntityOrSolution(
      SolutionDescriptor<?> solutionDescriptor, Field field) {
  ...
  private static boolean isTypeArgumentDeepCloned(
      SolutionDescriptor<?> solutionDescriptor, Type genericType) {
