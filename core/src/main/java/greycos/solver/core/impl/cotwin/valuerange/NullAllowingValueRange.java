...
public final class NullAllowingValueRange<T> extends AbstractValueRange<T> {
  ...
  public NullAllowingValueRange(ValueRange<T> childValueRange) {
    this.childValueRange = (AbstractValueRange<T>) childValueRange;
    if (childValueRange instanceof NullAllowingValueRange<T>) {
      throw new IllegalArgumentException(
          "Impossible state: The childValueRange (%s) must not be a %s, because it is already wrapped in one."
              .formatted(childValueRange, NullAllowingValueRange.class.getSimpleName()));
    }
  ...
  public boolean equals(Object o) {
    ...
    return o instanceof NullAllowingValueRange<?> that
        && size == that.size
