...
public record RootVariableSource<Entity_, Value_>(
  ...
  private record VariablePath(
      Class<?> variableEntityClass,
      String variableName,
  ...
  public static <Entity_, Value_> RootVariableSource<Entity_, Value_> from(
    ...
    for (var iterator = pathIterator(rootEntityClass, variablePath); iterator.hasNext(); ) {
      ...
      if (pathPart.isCollection()) {
        var isListVariable =
            getAnnotation(
                        pathPart.member().getDeclaringClass(),
                        pathPart.name(),
                        PlanningListVariable.class)
                    != null
                && isVariable(
                    solutionMetaModel, pathPart.member().getDeclaringClass(), pathPart.name());
        if (!isListVariable
            && isVariable(
                solutionMetaModel, pathPart.member().getDeclaringClass(), pathPart.name())) {
        ...
        var memberAccessor =
            getMemberAccessor(pathPart.member(), memberAccessorFactory, descriptorPolicy);
        ...
        if (isListVariable) {
          ...
          listVariableMetaModel =
              solutionMetaModel
                  .entity(memberAccessor.getDeclaringClass())
        ...
        if (isVariable) {
          ...
          if (parentVariableType == null) {
            parentVariableType =
                determineParentVariableType(
                    rootEntityClass, variablePath, chainToVariable, memberAccessor);
          }
    ...
    List<MemberAccessor> chainToVariableEntity =
        chainToVariable.subList(0, chainToVariable.size() - 1);
    if (!hasListMemberAccessor) {
      valueEntityFunction = getRegularSourceEntityVisitor(chainToVariableEntity);
      listMemberAccessors.clear();
    } else {
      valueEntityFunction =
          getCollectionSourceEntityVisitor(listMemberAccessors, chainToVariableEntity);
    }
    ...
    for (var i = 0; i < chainStartingFromSourceVariableList.size(); i++) {
      ...
      var newSourceReference =
          createVariableSourceReferenceFromChain(
              variablePath,
              listMemberAccessors,
              solutionMetaModel,
              rootEntityClass,
              targetVariableName,
              chainStartingFromSourceVariable,
              chainToVariable,
              factCountAfterSourceVariableToNextVariableList.get(i),
              i == 0,
    ...
    if (parentVariableType == ParentVariableType.LIST_ELEMENT
        && (variableSourceReferences.size() != 1
            || !variableSourceReferences.get(0).isDeclarative())) {
      throw new IllegalArgumentException(
    ...
    if (variableSourceReferences.size() > 2) {
      throw new IllegalArgumentException(
          """
              ...
              .formatted(
                  variablePath,
                  rootEntityClass.getCanonicalName(),
                  variableSourceReferences.size()));
    }

    return new RootVariableSource<>(
        rootEntityClass,
        listMemberAccessors,
        valueEntityFunction,
  ...
  public @NonNull BiConsumer<Object, Consumer<Object>> getEntityVisitor(
      List<MemberAccessor> chainToEntity) {
  ...
  private static <Value_>
      @NonNull BiConsumer<Object, Consumer<Value_>> getRegularSourceEntityVisitor(
  ...
  private static <Value_>
      @NonNull BiConsumer<Object, Consumer<Value_>> getCollectionSourceEntityVisitor(
          List<MemberAccessor> listMemberAccessors, List<MemberAccessor> finalChainToVariable) {
    var entityListMemberAccessor =
        RootVariableSource.<Iterable<Object>>getRegularSourceEntityVisitor(listMemberAccessors);
    var elementSourceEntityVisitor =
        RootVariableSource.<Value_>getRegularSourceEntityVisitor(finalChainToVariable);
    return (entity, consumer) ->
        entityListMemberAccessor.accept(
            entity,
            iterable -> {
  ...
  private static <Entity_> @NonNull VariableSourceReference createVariableSourceReferenceFromChain(
      ...
      Class<? extends Entity_> rootEntityClass,
      String targetVariableName,
      List<MemberAccessor> afterChain,
      List<MemberAccessor> chainToVariable,
      int factCountTillNextVariable,
      boolean isTopLevel,
    ...
    if (isDeclarativeShadowVariable(maybeDownstreamVariable)) {
      downstreamDeclarativeVariable =
          solutionMetaModel
              .entity(maybeDownstreamVariable.getDeclaringClass())
    ...
    return new VariableSourceReference(
        solutionMetaModel
            .entity(variableMemberAccessor.getDeclaringClass())
            .variable(variableMemberAccessor.getName()),
        sourceVariablePath.memberAccessorsBeforeEntity,
        isTopLevel
            && sourceVariablePath.memberAccessorsBeforeEntity.isEmpty()
            && listMemberAccessors.isEmpty(),
        isTopLevel,
        isBottomLevel,
        isDeclarative,
  ...
  private static void assertIsValidVariableReference(
  ...
  public static Member getMember(
      Class<?> rootClass, String sourcePath, Class<?> declaringClass, String memberName) {
  ...
  public static boolean isVariable(
      PlanningSolutionMetaModel<?> metaModel, Class<?> declaringClass, String memberName) {
  ...
  private static ParentVariableType determineParentVariableType(
      Class<?> rootClass,
      String variablePath,
      List<MemberAccessor> chain,
    ...
    if (getAnnotation(declaringClass, memberName, InverseRelationShadowVariable.class) != null) {
      ...
      var inverseVariable =
          Objects.requireNonNull(
              getAnnotation(declaringClass, memberName, InverseRelationShadowVariable.class));
      var sourceClass = memberAccessor.getType();
      var variableName = inverseVariable.sourceVariableName();
      PlanningVariable sourcePlanningVariable =
          getAnnotation(sourceClass, variableName, PlanningVariable.class);
  ...
  private static <T extends Annotation> T getAnnotation(
      Class<?> declaringClass, String memberName, Class<? extends T> annotationClass) {
  ...
  private static boolean isDeclarativeShadowVariable(MemberAccessor memberAccessor) {
    var shadowVariable =
        getAnnotation(
            memberAccessor.getDeclaringClass(), memberAccessor.getName(), ShadowVariable.class);
