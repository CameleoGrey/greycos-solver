 ...
 * ({@code i--}). This class performs the same steps, split across two calls, over a virtual
 * permutation layer instead of the real list:
 *
 * <ul>
 *   <li>{@link #hasNext()} draws the random {@code j}: {@code nextSlot =
 *       workingRandom.nextInt(activeCount)}.
 *   <li>{@link #retire()} performs the swap-and-shrink: it moves the item that lives in the last
 *       live slot into the retired slot's position (see {@link #slotMap} for this "reservation"),
 *       then shrinks {@link #activeCount} so the last slot drops out of the live range.
 * </ul>
 *
 * Only one side of the swap is ever written, because the slot that drops out of range is never read
 * again.
 *
 * @param <T>
 */
