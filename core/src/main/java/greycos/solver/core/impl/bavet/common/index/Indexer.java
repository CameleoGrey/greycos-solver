 ...
 *
 * <p>Some indexer types (such as contain, containedIn, ...) have two different key types (modify
 * key vs query key), depending on the operation type (modify operation vs query operation). For
 * example, for a contain indexer the modify key is a collection, but the query key is not.
 *
...
public sealed interface Indexer<T>
  ...
  /**
   * As defined by {@link #randomIterator(Object, RandomGenerator)}, but only returning elements
