...
public final class QuadJoinerComber<A, B, C, D> {
    ...
    for (var i = 0; i < joiners.length; i++) {
      ...
      if (joiner instanceof FilteringQuadJoiner) {
        ...
        throw new IllegalArgumentException(
            "The joiner class (%s) is not supported.".formatted(joiner.getClass()));
      }
    }
  ...
  private static <A, B, C, D> @Nullable QuadPredicate<A, B, C, D> mergeFiltering(
      List<QuadPredicate<A, B, C, D>> filteringList) {
  ...
  public QuadJoinerComber(
      DefaultQuadJoiner<A, B, C, D> mergedJoiner,
      @Nullable QuadPredicate<A, B, C, D> mergedFiltering) {
