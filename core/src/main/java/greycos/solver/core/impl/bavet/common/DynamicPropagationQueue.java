 ...
 * @param <Tuple_>
 */
final class DynamicPropagationQueue<
        Tuple_ extends Tuple, Carrier_ extends AbstractPropagationMetadataCarrier<Tuple_>>
  ...
  private DynamicPropagationQueue(
      TupleLifecycle<Tuple_> nextNodesTupleLifecycle, Consumer<Carrier_> preprocessor, int size) {
    ...
    // Updates tend to be dominant; update queue isn't stored, it's deduced as neither insert nor
    // retract.
  ...
  public DynamicPropagationQueue(
      TupleLifecycle<Tuple_> nextNodesTupleLifecycle, Consumer<Carrier_> preprocessor) {
      ...
      switch (carrier.getState()) {
        ...
        default ->
            throw new IllegalStateException(
                "Impossible state: Cannot insert (%s), already inserting.".formatted(carrier));
      }
    }
  ...
  public void retract(Carrier_ carrier, TupleState state) {
    if (state.isActive() || state == TupleState.DEAD) {
      throw new IllegalArgumentException(
          "Impossible state: The state (%s) is not a valid retract state.".formatted(state));
    }
    var positionInDirtyList = carrier.positionInDirtyList;
    if (positionInDirtyList < 0) {
      ...
      switch (carrier.getState()) {
        ...
        default ->
            throw new IllegalStateException(
                "Impossible state: Cannot retract (%s), already retracting.".formatted(carrier));
      }
    }
    ...
    while (i != -1) {
      ...
      switch (state) {
        case DYING -> {
          clean(
              carrier,
              TupleState.DEAD); // Hide original state from the next node by doing this before
          // propagation.
  ...
  private void propagateInsertOrUpdate(Carrier_ carrier, boolean isUpdate) {
    ...
    clean(
        carrier,
        TupleState.OK); // Hide original state from the next node by doing this before propagation.
    if (isUpdate) {
