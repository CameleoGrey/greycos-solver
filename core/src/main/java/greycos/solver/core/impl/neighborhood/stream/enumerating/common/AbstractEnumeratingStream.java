...
@NullMarked
public abstract class AbstractEnumeratingStream<Solution_> implements BavetStream {
  ...
  protected AbstractEnumeratingStream(
  ...
  public final <Stream_ extends AbstractEnumeratingStream<Solution_>> Stream_ shareAndAddChild(
      Stream_ stream) {
  ...
  public AbstractEnumeratingStream<Solution_> getTupleSource() {
    if (this instanceof TupleSource) {
      ...
      throw new IllegalStateException(
          "Impossible state: the stream (" + this + ") does not have a parent.");
    }
  ...
  protected void assertEmptyChildStreamList() {
    if (!childStreamList.isEmpty()) {
      throw new IllegalStateException(
          "Impossible state: the stream ("
              + this
              + ") has a non-empty childStreamList ("
              + childStreamList
              + ").");
    }
  }
