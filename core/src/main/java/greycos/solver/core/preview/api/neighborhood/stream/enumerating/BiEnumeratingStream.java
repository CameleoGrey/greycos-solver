...
public interface BiEnumeratingStream<Solution_, A, B> extends EnumeratingStream {
  ...
  /**
   * As defined by {@link #map(BiNeighborhoodsMapper)}, only resulting in {@link
   * BiEnumeratingStream}.
   */
  ...
  /**
   * As defined by {@link UniEnumeratingStream#groupBy(UniNeighborhoodsMapper)}, only for {@link
   * BiEnumeratingStream} sources.
   */
  <GroupKey_> UniEnumeratingStream<Solution_, GroupKey_> groupBy(
      BiNeighborhoodsMapper<Solution_, A, B, GroupKey_> key);

  /**
   * As defined by {@link UniEnumeratingStream#groupBy(UniNeighborhoodsCollector)}, only for {@link
   * BiEnumeratingStream} sources.
   */
   ...
   * UniNeighborhoodsCollector)}, only for {@link BiEnumeratingStream} sources.
   */
  ...
  /**
   * Terminal operation: materializes this stream as a {@link BiDataset}, kept up to date in memory
   * as the working solution changes. Use this to consume the dataset from a custom {@link
   * MoveIteratorProvider}, as opposed to being consumed by {@link
   * MoveStreamFactory#pick(UniEnumeratingStream)} Resolve the returned handle against a {@link
   * MoveIteratorSession} inside {@link MoveStreamFactory#buildMoveStream(MoveIteratorProvider)}.
   *
