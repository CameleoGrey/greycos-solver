...
/**
 * Assigns each unassigned value to a position in some entity's list variable. Only generates moves
 * where the value is in range for the destination entity.
 *
 * <p>For moving an already-assigned value to a different position, see {@link
 * ListChangeMoveProvider}. For removing an already-assigned value from its position (unassigning),
 * see {@code ListUnassignMoveProvider}.
 *
 * <p>Requires that the variable {@link PlanningListVariableMetaModel#allowsUnassignedValues()
 * allows unassigned values}; otherwise the constructor throws {@link IllegalArgumentException}.
 */
@NullMarked
public final class ListAssignMoveProvider<Solution_, Entity_, Value_>
  ...
  public MoveStream<Solution_> build(MoveStreamFactory<Solution_> moveStreamFactory) {
    ...
    return moveStreamFactory
        .pick(moveStreamFactory.forEachUnassignedValue(variableMetaModel))
        .pick(
            moveStreamFactory.forEachDestination(variableMetaModel),
            NeighborhoodsJoiners.filtering(
                nodeSharingSupportFunctions.valueInRangeFilterForPosition()))
        .asMove(
            (view, value, pos) ->
                Moves.assign(variableMetaModel, Objects.requireNonNull(value), pos));
  }
}
