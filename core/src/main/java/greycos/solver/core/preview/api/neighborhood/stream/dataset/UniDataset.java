...
/**
 * An opaque handle to a dataset created via {@link UniEnumeratingStream#asCachedDataset()}. Carries
 * no behavior of its own; resolve it against a {@link MoveIteratorSession} to obtain a {@link
 * UniDatasetInstance}.
 *
...
public interface UniDataset<Solution_, A> {
  ...
  @SuppressWarnings("unchecked")
  default <B> BiDataset<Solution_, A, B> join(
      UniEnumeratingStream<Solution_, B> other, BiNeighborhoodsJoiner<A, B> joiner) {
    return join(other, new BiNeighborhoodsJoiner[] {joiner});
  }

  @SuppressWarnings("unchecked")
  default <B> BiDataset<Solution_, A, B> join(
      UniEnumeratingStream<Solution_, B> other,
   ...
   * @return never null
   */
  <B> BiDataset<Solution_, A, B> join(
      UniEnumeratingStream<Solution_, B> other, BiNeighborhoodsJoiner<A, B>... joiners);
}
