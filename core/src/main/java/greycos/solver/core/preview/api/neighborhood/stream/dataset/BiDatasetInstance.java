...
public interface BiDatasetInstance<A, B> {
  ...
  /** As defined by {@link #size()}, but restricted to rows paired with the given left value. */
  int size(@Nullable A a);

  /**
   * As defined by {@link #iterator(RandomGenerator)}, but restricted to rows paired with the given
   * left value.
   */
  Iterator<@Nullable B> iterator(@Nullable A a, RandomGenerator random);

  /**
   * As defined by {@link #exhaustiveIterator(RandomGenerator)}, but restricted to rows paired with
