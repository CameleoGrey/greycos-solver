...
/**
 * Entry point for evaluating {@link MoveProvider}s on a given solution. Given a planning solution,
 * it produces an {@link NeighborhoodTestContext} for sampling moves generated from that solution
 ...
 *         .using(solution);
 * context.getMovesAsStream().limit(100).forEach(move -> {
 *     // Run assertions on the sampled move here.
 * });
 * }</pre>
 *
...
public interface NeighborhoodTester<Solution_> {
  ...
  /**
   * Creates a new {@link NeighborhoodTester} for the given move provider and the given solution and
   * entity classes.
   *
   * <p>This method validates inputs, and initializes many internal structures. These are heavy
   * operations performed once and cached for reuse.
   *
   * <p>Shadow variables are initialized later when a solution is bound via {@link #using(Object)}.
   *
   * @param moveProvider the move provider to generate moves
   * @param solutionMetaModel the planning solution meta-model; use {@link
   *     PlanningSolutionMetaModel#of(Class, Class[])} to build one.
   * @param <Solution_> the planning solution type
  ...
  /**
   * Creates an evaluation context for the given solution instance. Once you have the context, you
   * can retrieve the moves via methods such as {@link NeighborhoodTestContext#getMovesAsStream()}.
   *
   * <p>Different evaluation contexts can be created, each bound to a different solution instance.
