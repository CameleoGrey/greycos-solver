...
import greycos.solver.core.impl.bavet.bi.joiner.FilteringBiJoiner;
import greycos.solver.core.impl.bavet.common.joiner.JoinerType;
import greycos.solver.core.impl.bavet.penta.joiner.DefaultPentaJoiner;
import greycos.solver.core.impl.bavet.penta.joiner.FilteringPentaJoiner;
import greycos.solver.core.impl.bavet.quad.joiner.DefaultQuadJoiner;
import greycos.solver.core.impl.bavet.quad.joiner.FilteringQuadJoiner;
import greycos.solver.core.impl.bavet.tri.joiner.DefaultTriJoiner;
import greycos.solver.core.impl.bavet.tri.joiner.FilteringTriJoiner;
import greycos.solver.core.impl.util.ConstantLambdaUtils;

import org.jspecify.annotations.NonNull;

/**
 * Creates an {@link BiJoiner}, {@link TriJoiner}, ... instance for use in {@link
 * UniConstraintStream#join(Class, BiJoiner)}, ...
 */
public final class Joiners {

  // TODO Support using non-natural comparators, such as lessThan(leftMapping, rightMapping,
  // comparator).
  ...
  /**
   * Joins every A and B that share a property. These are exactly the pairs where {@code
   * leftMapping.apply(A).equals(rightMapping.apply(B))}. For example, on a cartesian product of
   * list {@code [Ann(age = 20), Beth(age = 25), Eric(age = 20)]} with both leftMapping and
   * rightMapping being {@code Person::getAge}, this joiner will produce pairs {@code (Ann, Ann),
   * (Ann, Eric), (Beth, Beth), (Eric, Ann), (Eric, Eric)}.
   *
   ...
   */
  public static <A, B, Property_> @NonNull BiJoiner<A, B> equal(
   ...
   * @param <Property_> the type of the property to compare
   */
  public static <A, Property_ extends Comparable<Property_>> @NonNull BiJoiner<A, A> lessThan(
   ...
   * These are exactly the pairs where {@code leftMapping.apply(A).compareTo(rightMapping.apply(B))
   * < 0}.
   *
   * <p>For example, on a cartesian product of list {@code [Ann(age = 20), Beth(age = 25), Eric(age
   * = 20)]} with both leftMapping and rightMapping being {@code Person::getAge}, this joiner will
   * produce pairs {@code (Ann, Beth), (Eric, Beth)}.
   *
   * @param leftMapping mapping function to apply to A
   * @param rightMapping mapping function to apply to B
  ...
  /**
   * As defined by {@link #lessThanOrEqual(Function, Function)} with both arguments using the same
   * mapping.
   *
   * @param mapping mapping function to apply
   * @param <A> the type of both objects
   * @param <Property_> the type of the property to compare
   */
  public static <A, Property_ extends Comparable<Property_>>
      @NonNull BiJoiner<A, A> lessThanOrEqual(Function<A, Property_> mapping) {
    return lessThanOrEqual(mapping, mapping);
  }

  /**
   * Joins every A and B where a value of property on A is less than or equal to the value of a
   * property on B. These are exactly the pairs where {@code
   * leftMapping.apply(A).compareTo(rightMapping.apply(B)) <= 0}.
   *
   * <p>For example, on a cartesian product of list {@code [Ann(age = 20), Beth(age = 25), Eric(age
   * = 20)]} with both leftMapping and rightMapping being {@code Person::getAge}, this joiner will
   * produce pairs {@code (Ann, Ann), (Ann, Beth), (Ann, Eric), (Beth, Beth), (Eric, Ann), (Eric,
   * Beth), (Eric, Eric)}.
   *
   * @param leftMapping mapping function to apply to A
   * @param rightMapping mapping function to apply to B
   ...
   * @param <Property_> the type of the property to compare
   */
  public static <A, B, Property_ extends Comparable<Property_>>
      @NonNull BiJoiner<A, B> lessThanOrEqual(
          Function<A, Property_> leftMapping, Function<B, Property_> rightMapping) {
    return new DefaultBiJoiner<>(leftMapping, JoinerType.LESS_THAN_OR_EQUAL, rightMapping);
  }

  /**
   * As defined by {@link #greaterThan(Function, Function)} with both arguments using the same
   * mapping.
   *
   * @param mapping mapping function to apply
   * @param <A> the type of both objects
  ...
  /**
   * Joins every A and B where a value of property on A is greater than the value of a property on
   * B. These are exactly the pairs where {@code
   * leftMapping.apply(A).compareTo(rightMapping.apply(B)) > 0}.
   *
   * <p>For example, on a cartesian product of list {@code [Ann(age = 20), Beth(age = 25), Eric(age
   * = 20)]} with both leftMapping and rightMapping being {@code Person::getAge}, this joiner will
   * produce pairs {@code (Beth, Ann), (Beth, Eric)}.
   *
   * @param leftMapping mapping function to apply to A
   * @param rightMapping mapping function to apply to B
   * @param <A> the type of object on the left
  ...
  /**
   * As defined by {@link #greaterThanOrEqual(Function, Function)} with both arguments using the
   * same mapping.
   *
   * @param mapping mapping function to apply
   * @param <A> the type of both objects
   ...
   */
  public static <A, Property_ extends Comparable<Property_>>
      @NonNull BiJoiner<A, A> greaterThanOrEqual(Function<A, Property_> mapping) {
    return greaterThanOrEqual(mapping, mapping);
  }

  /**
   * Joins every A and B where a value of property on A is greater than or equal to the value of a
   * property on B. These are exactly the pairs where {@code
   * leftMapping.apply(A).compareTo(rightMapping.apply(B)) >= 0}.
   *
   * <p>For example, on a cartesian product of list {@code [Ann(age = 20), Beth(age = 25), Eric(age
   * = 20)]} with both leftMapping and rightMapping being {@code Person::getAge}, this joiner will
   * produce pairs {@code (Ann, Ann), (Ann, Eric), (Beth, Ann), (Beth, Beth), (Beth, Eric), (Eric,
   * Ann), (Eric, Eric)}.
   *
   * @param leftMapping mapping function to apply to A
   * @param rightMapping mapping function to apply to B
   ...
   */
  public static <A, B, Property_ extends Comparable<Property_>>
      @NonNull BiJoiner<A, B> greaterThanOrEqual(
          Function<A, Property_> leftMapping, Function<B, Property_> rightMapping) {
    return new DefaultBiJoiner<>(leftMapping, JoinerType.GREATER_THAN_OR_EQUAL, rightMapping);
  }

  /**
   * Applies a filter to the joined tuple, with the semantics of {@link
   * BiConstraintStream#filter(BiPredicate)}.
   *
   * <p>For example, on a cartesian product of list {@code [Ann(age = 20), Beth(age = 25), Eric(age
   * = 20)]} with filter being {@code age == 20}, this joiner will produce pairs {@code (Ann, Ann),
   * (Ann, Eric), (Eric, Ann), (Eric, Eric)}.
   *
  ...
  /**
   * Joins every A and B that overlap for an interval which is specified by a start and end property
   * on both A and B. These are exactly the pairs where {@code A.start < B.end} and {@code A.end >
   * B.start}.
   *
   * <p>For example, on a cartesian product of list {@code [Ann(start=08:00, end=14:00),
   * Beth(start=12:00, end=18:00), Eric(start=16:00, end=22:00)]} with startMapping being {@code
   * Person::getStart} and endMapping being {@code Person::getEnd}, this joiner will produce pairs
   * {@code (Ann, Ann), (Ann, Beth), (Beth, Ann), (Beth, Beth), (Beth, Eric), (Eric, Beth), (Eric,
   * Eric)}.
   *
  ...
  /**
   * Joins every A and B where a value of property on B is contained in the collection of properties
   * on A.
   *
   * <p>For example:
   *
   * <ul>
   *   <li>{@code ["A", "B"]} containing {@code "A"} is {@code true}
   *   <li>{@code ["A"]} containing {@code "A"} is {@code true}
   *   <li>{@code ["X", "Y"]} containing {@code "A"} is {@code false}
   *   <li>{@code []} containing {@code "A"} is {@code false}
   *   <li>{@code ["A", "B"]} containing {@code null} is {@code false}
   *   <li>{@code []} containing {@code null} is {@code false}
   * </ul>
   *
   * @param leftMapping mapping function to apply to A
   * @param rightMapping mapping function to apply to B
  ...
  /**
   * Joins every A and B where a value of property on A is contained in the collection of properties
   * on B.
   *
   * <p>For example:
   *
   * <ul>
   *   <li>{@code "A"} contained in {@code ["A", "B"]} is {@code true}
   *   <li>{@code "A"} contained in {@code ["A"]} is {@code true}
   *   <li>{@code "A"} contained in {@code ["X", "Y"]} is {@code false}
   *   <li>{@code "A"} contained in {@code []} is {@code false}
   *   <li>{@code null} contained in {@code ["A", "B"]} is {@code false}
   *   <li>{@code null} contained in {@code []} is {@code false}
   * </ul>
   *
   * @param leftMapping mapping function to apply to A
   * @param rightMapping mapping function to apply to B
  ...
  /**
   * As defined by {@link #containingAnyOf(Function, Function)} with both arguments using the same
   * mapping.
   *
   * @param mapping mapping function to apply to both A and B
   * @param <A> the type of both objects
  ...
  /**
   * Joins every A and B where a collection of properties on A overlaps with a collection of
   * properties on B.
   *
   * <p>For example:
   *
   * <ul>
   *   <li>{@code ["A", "B"]} intersecting {@code ["A", "B"]} is {@code true}
   *   <li>{@code ["A", "B"]} intersecting {@code ["A"]} is {@code true}
   *   <li>{@code ["A"]} intersecting {@code ["A", "B"]} is {@code true}
   *   <li>{@code ["A", "B"]} intersecting {@code ["X", "Y"]} is {@code false}
   *   <li>{@code ["A", "B"]} intersecting {@code []} is {@code false}
   *   <li>{@code []} intersecting {@code ["A", "B"]} is {@code false}
   *   <li>{@code []} intersecting {@code []} is {@code false}
   * </ul>
   *
   * @param leftMapping mapping function to apply to A
   * @param rightMapping mapping function to apply to B
   ...
   */
  public static <A, B, C, Property_> @NonNull TriJoiner<A, B, C> equal(
   ...
   * @param <Property_> the type of the property to compare
   */
  public static <A, B, C, Property_ extends Comparable<Property_>>
      @NonNull TriJoiner<A, B, C> lessThan(
          BiFunction<A, B, Property_> leftMapping, Function<C, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, Property_ extends Comparable<Property_>>
      @NonNull TriJoiner<A, B, C> lessThanOrEqual(
          BiFunction<A, B, Property_> leftMapping, Function<C, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, Property_ extends Comparable<Property_>>
      @NonNull TriJoiner<A, B, C> greaterThan(
          BiFunction<A, B, Property_> leftMapping, Function<C, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, Property_ extends Comparable<Property_>>
      @NonNull TriJoiner<A, B, C> greaterThanOrEqual(
          BiFunction<A, B, Property_> leftMapping, Function<C, Property_> rightMapping) {
   ...
   */
  public static <A, B, C> @NonNull TriJoiner<A, B, C> filtering(
      @NonNull TriPredicate<A, B, C> filter) {
   ...
   * As defined by {@link #overlapping(Function, Function)}.
   *
   * @param leftStartMapping maps the first and second arguments to their interval start point
   *     (inclusive)
   * @param leftEndMapping maps the first and second arguments to their interval end point
   *     (exclusive)
   ...
   */
  public static <A, B, C, Property_ extends Comparable<Property_>>
      @NonNull TriJoiner<A, B, C> overlapping(
   ...
   * @param <Property_> the type of the property to compare
   */
  public static <A, B, C, D, Property_ extends Comparable<Property_>>
      @NonNull QuadJoiner<A, B, C, D> lessThan(
          TriFunction<A, B, C, Property_> leftMapping, Function<D, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, D, Property_ extends Comparable<Property_>>
      @NonNull QuadJoiner<A, B, C, D> lessThanOrEqual(
          TriFunction<A, B, C, Property_> leftMapping, Function<D, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, D, Property_ extends Comparable<Property_>>
      @NonNull QuadJoiner<A, B, C, D> greaterThan(
          TriFunction<A, B, C, Property_> leftMapping, Function<D, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, D, Property_ extends Comparable<Property_>>
      @NonNull QuadJoiner<A, B, C, D> greaterThanOrEqual(
          TriFunction<A, B, C, Property_> leftMapping, Function<D, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, D> @NonNull QuadJoiner<A, B, C, D> filtering(
      @NonNull QuadPredicate<A, B, C, D> filter) {
   ...
   * As defined by {@link #overlapping(Function, Function)}.
   *
   * @param leftStartMapping maps the first, second and third arguments to their interval start
   *     point (inclusive)
   * @param leftEndMapping maps the first, second and third arguments to their interval end point
   *     (exclusive)
   ...
   */
  public static <A, B, C, D, Property_ extends Comparable<Property_>>
      @NonNull QuadJoiner<A, B, C, D> overlapping(
   ...
   * @param <Property_> the type of the property to compare
   */
  public static <A, B, C, D, E, Property_ extends Comparable<Property_>>
      @NonNull PentaJoiner<A, B, C, D, E> lessThan(
          QuadFunction<A, B, C, D, Property_> leftMapping, Function<E, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, D, E, Property_ extends Comparable<Property_>>
      @NonNull PentaJoiner<A, B, C, D, E> lessThanOrEqual(
          QuadFunction<A, B, C, D, Property_> leftMapping, Function<E, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, D, E, Property_ extends Comparable<Property_>>
      @NonNull PentaJoiner<A, B, C, D, E> greaterThan(
          QuadFunction<A, B, C, D, Property_> leftMapping, Function<E, Property_> rightMapping) {
   ...
   */
  public static <A, B, C, D, E, Property_ extends Comparable<Property_>>
      @NonNull PentaJoiner<A, B, C, D, E> greaterThanOrEqual(
          QuadFunction<A, B, C, D, Property_> leftMapping, Function<E, Property_> rightMapping) {
   ...
   * @param <E> the type of object on the right
   */
  public static <A, B, C, D, E> @NonNull PentaJoiner<A, B, C, D, E> filtering(
      @NonNull PentaPredicate<A, B, C, D, E> filter) {
   ...
   * As defined by {@link #overlapping(Function, Function)}.
   *
   * @param leftStartMapping maps the first, second, third and fourth arguments to their interval
   *     start point (inclusive)
   * @param leftEndMapping maps the first, second, third and fourth arguments to their interval end
   *     point (exclusive)
   ...
   */
  public static <A, B, C, D, E, Property_ extends Comparable<Property_>>
      @NonNull PentaJoiner<A, B, C, D, E> overlapping(
