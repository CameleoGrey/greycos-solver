...
/**
 * Includes settings to override default {@link greycos.solver.core.api.solver.Solver}
 * configuration.
 *
 * <p>This class provides an API to override solver termination settings. The following options are
 * available:
 *
 * <ul>
 *   <li>Use {@link #withTerminationConfig(TerminationConfig)} to set a complete termination
 *       configuration
 *   <li>Use {@link #withTerminationSpentLimit(Duration)} and/or {@link
 *       #withTerminationUnimprovedSpentLimit(Duration)} to set specific time limits independently
 *       or in combination
 * </ul>
 *
 * <p><strong>Important ordering constraint:</strong> If {@link
 * #withTerminationConfig(TerminationConfig)} is used, it must be called before any specific
...
public final class SolverConfigOverride {
   ...
   * #withTerminationUnimprovedSpentLimit(Duration)}.
   *
  ...
  public SolverConfigOverride withTerminationConfig(TerminationConfig terminationConfig) {
    ...
    this.terminationConfig =
        Objects.requireNonNull(
            terminationConfig, "Invalid terminationConfig (null) given to SolverConfigOverride.");
    return this;
  }

  /**
   * Sets a time limit for the solver to run, creating or updating the default termination
   * configuration.
   *
   * <p>This method sets the maximum duration the solver is allowed to run before terminating. It
   * can be used independently or in combination with {@link
   * #withTerminationUnimprovedSpentLimit(Duration)}.
   *
   * <p>If no {@link TerminationConfig} has been set via {@link
   * #withTerminationConfig(TerminationConfig)}, this method will create a new one. If a
   * TerminationConfig already exists, this method will update its spent limit setting.
   *
   * <p>Usage examples:
   *
   * {@snippet :
  ...
  /**
   * Sets a time limit for the solver to run without finding an improved solution, creating or
   * updating the default termination configuration.
   *
   * <p>This method sets the maximum duration the solver is allowed to run without improvement
   * before terminating. The solver will stop if it hasn't found a better solution within this time
   * limit, even if the total spent limit (if set) hasn't been reached yet.
   *
   * <p>This method can be used independently or in combination with {@link
   * #withTerminationSpentLimit(Duration)}. When used together, the solver will terminate when
   * either condition is met (whichever comes first).
   *
   * <p>If no {@link TerminationConfig} has been set via {@link
   * #withTerminationConfig(TerminationConfig)}, this method will create a new one. If a
   * TerminationConfig already exists, this method will update its unimproved spent limit setting.
   *
   * <p>Usage examples:
   *
   * {@snippet :
