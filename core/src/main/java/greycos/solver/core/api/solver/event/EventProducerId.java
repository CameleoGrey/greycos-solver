...
import greycos.solver.core.impl.solver.event.SolveEventProducerId;
...
public interface EventProducerId {
  /**
   * An unique string identifying what produced the event, either of the form "Event" where "Event"
   * is a string describing the event that cause the update (like "Solving started") or "Phase
   * (index)", where "Phase" is a string identifying the type of phase (like "Construction
   * Heuristics") and index is the index of the phase in the {@link
  ...
  /**
   * A (non-unique) string describing what produced the event. Events from different phases of the
   * same type (for example, when multiple Construction Heuristics are configured) will return the
   * same value.
   *
  ...
  /**
   * If present, the index of the phase that produced the event in the {@link
   * SolverConfig#getPhaseConfigList()}. Is absent when the producer does not correspond to a phase,
   * for instance, an event triggered after {@link ProblemChange} were processed.
   *
   * @return The index of the corresponding phase in {@link SolverConfig#getPhaseConfigList()}, or
   *     {@link OptionalInt#empty()} if there is no corresponding phase.
   */
