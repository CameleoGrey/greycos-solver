...
/**
 * A ScoreDefinition knows how to compare {@link Score}s and what the perfect maximum/minimum {@link
 * Score} is.
 *
...
public interface ScoreDefinition<Score_ extends Score<Score_>> {
  ...
  /**
   * Returns the length of {@link Score#toLevelNumbers()} for every {@link Score} of this
   * definition. For example: returns 2 on {@link HardSoftScoreDefinition}.
   *
   * @return at least 1
   */
  int getLevelsSize();
  ...
  /**
   * Returns the number of levels of {@link Score#toLevelNumbers()}. that are used to determine
   * {@link Score#isFeasible()}.
   *
  ...
  /**
   * Returns a label for each score level. Each label includes the suffix "score" and must start in
   * lower case. For example: returns {@code {"hard score", "soft score "}} on {@link
  ...
  /**
   * Returns the {@link Class} of the actual {@link Score} implementation. For example: returns
   * {@link HardSoftScore HardSoftScore.class} on {@link HardSoftScoreDefinition}.
   *
   * @return never null
   */
  ...
  /**
   * Builds a {@link Score} which is equal or better than any other {@link Score} with more
   * variables initialized (while the already variables don't change).
   *
  ...
  /**
   * Builds a {@link Score} which is equal or worse than any other {@link Score} with more variables
   * initialized (while the already variables don't change).
   *
   * @param initializingScoreTrend never null, with {@link InitializingScoreTrend#getLevelsSize()}
   *     equal to {@link #getLevelsSize()}.
   * @param score never null, considered initialized
  ...
  /**
   * @param score never null
   * @return true if the otherScore is accepted as a parameter of {@link Score#add(Score)}, {@link
   *     Score#subtract(Score)} and {@link Score#compareTo(Object)} for scores of this score
   *     definition.
   */
   ...
   *
   * <ul>
   *   <li>int.class for {@link SimpleScore}
   *   <li>BigDecimal.class for {@link SimpleBigDecimalScore}
   * </ul>
   *
   * @return never null
   */
