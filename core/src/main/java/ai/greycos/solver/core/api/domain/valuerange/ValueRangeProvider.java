 ...
 *
 * <p>This is specified on a getter of a java bean property (or directly on a field) which returns a
 * {@link Collection} or {@link ValueRange}. A {@link Collection} is implicitly converted to a
 * {@link ValueRange}. For solver reproducibility, the collection must have a deterministic, stable
 ...
 * that entity's variables, or on any other entity's variables. If you need to change a value range
 * defined on an entity, trigger a {@link ProblemChange} for that entity or restart the solver with
 * an updated planning solution. If you need to change a value range defined on a planning solution,
...
public @interface ValueRangeProvider {
  ...
  /**
   * Used by {@link PlanningVariable#valueRangeProviderRefs()} to map a {@link PlanningVariable} to
   * a {@link ValueRangeProvider}. If not provided, an attempt will be made to find a matching
   * {@link PlanningVariable} without a ref.
   *
