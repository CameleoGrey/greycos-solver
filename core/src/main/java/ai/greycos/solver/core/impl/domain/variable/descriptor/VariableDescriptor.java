...
public abstract class VariableDescriptor<Solution_> {
  ...
  protected VariableDescriptor(
    ...
    this.simpleEntityAndVariableName =
        entityDescriptor.getEntityClass().getSimpleName() + "." + variableName;
  }

  /**
   * A number unique within an {@link EntityDescriptor}, increasing sequentially from zero. Used for
   * indexing in arrays to avoid object hash lookups in maps.
   *
  ...
  public <Entity_, Value_> VariableMetaModel<Solution_, Entity_, Value_> getVariableMetaModel() {
    ...
    cachedMetamodel = entityDescriptor.getEntityMetaModel().variable(variableName);
    return (VariableMetaModel<Solution_, Entity_, Value_>) cachedMetamodel;
  }
}
