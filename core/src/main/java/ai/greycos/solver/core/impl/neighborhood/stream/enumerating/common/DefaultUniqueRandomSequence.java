...
/**
 * Keeps an exact track of which items were already removed from the sequence, so that no item is
 * ever returned twice. It accepts a list of unique items on input, and does not copy or modify it.
 *
 * @param <T>
 */
@NullMarked
public final class DefaultUniqueRandomSequence<T> implements UniqueRandomSequence<T> {
  ...
  private static final DefaultUniqueRandomSequence<?> EMPTY =
      new DefaultUniqueRandomSequence<>(Collections.emptyList());
  ...
  private int determineActiveIndex(RandomGenerator workingRandom, int randomIndex) {
    ...
    var nextIndexDistance =
        nextClearIndex >= length ? Integer.MAX_VALUE : nextClearIndex - randomIndex;
    var previousIndexDistance =
        previousClearIndex == -1 ? Integer.MAX_VALUE : randomIndex - previousClearIndex;
  ...
  public T remove(int index) {
    if (removed.get(index)) {
      throw new IllegalArgumentException(
          "The index (%s) has already been removed.".formatted(index));
    }
    removed.set(index);
    removedCount++;
