package ai.greycos.solver.core.impl.heuristic.selector.common.iterator;
...
/**
 * IMPORTANT: The constructor of any subclass of this abstract class, should never call any of its
 * child {@link Selector}'s {@link Iterator#hasNext()} or {@link Iterator#next()} methods, because
 * that can cause descendant {@link Selector}s to be selected too early (which breaks {@link
...
public abstract class UpcomingSelectionIterator<S> extends SelectionIterator<S> {
   ...
   */
  public static ElementPosition findUnpinnedDestination(
  ...
  public static boolean isPinned(
      ElementPosition destination, ListVariableDescriptor<?> listVariableDescriptor) {
    if (destination instanceof PositionInList positionInList) {
      return listVariableDescriptor.isElementPinned(
          null, positionInList.entity(), positionInList.index());
