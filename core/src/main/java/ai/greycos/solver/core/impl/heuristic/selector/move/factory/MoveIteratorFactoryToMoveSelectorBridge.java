...
public class MoveIteratorFactoryToMoveSelectorBridge<Solution_>
  ...
  public MoveIteratorFactoryToMoveSelectorBridge(
  ...
  public long getSize() {
    ...
    if (size < 0L) {
      throw new IllegalStateException(
          "The moveIteratorFactoryClass ("
              + moveIteratorFactory.getClass()
              + ") has size ("
              + size
  ...
  public Iterator<Move<Solution_>> iterator() {
    if (!randomSelection) {
      return (Iterator<Move<Solution_>>)
          moveIteratorFactory.createOriginalMoveIterator(scoreDirector);
    } else {
      return (Iterator<Move<Solution_>>)
          moveIteratorFactory.createRandomMoveIterator(scoreDirector, workingRandom);
    }
  }
