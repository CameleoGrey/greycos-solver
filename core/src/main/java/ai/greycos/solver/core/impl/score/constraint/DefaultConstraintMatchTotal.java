package ai.greycos.solver.core.impl.score.constraint;
...
public final class DefaultConstraintMatchTotal<Score_ extends Score<Score_>>
  ...
  /**
   * Creates a {@link ConstraintMatch} and adds it to the collection returned by {@link
   * #getConstraintMatchSet()}. It will use {@link DefaultConstraintJustification}, whose {@link
   * DefaultConstraintJustification#getFacts()} method will return the given list of justifications.
  ...
  public ConstraintMatch<Score_> addConstraintMatch(List<Object> justifications, Score_ score) {
    return addConstraintMatch(
        DefaultConstraintJustification.of(score, justifications), justifications, score);
  }

  /**
   * Creates a {@link ConstraintMatch} and adds it to the collection returned by {@link
   * #getConstraintMatchSet()}. It will be justified with the provided {@link
   * ConstraintJustification}. Additionally, the constraint match will indict the objects in the
   * given list of indicted objects.
   *
   * @param indictedObjects never null, may be empty
   * @param score never null
   * @return never null
   */
  public ConstraintMatch<Score_> addConstraintMatch(
    ...
    ConstraintMatch<Score_> constraintMatch =
        new ConstraintMatch<>(constraintRef, justification, indictedObjects, score);
    ...
    if (!removed) {
      throw new IllegalStateException(
          "The constraintMatchTotal ("
              + this
              + ") could not remove constraintMatch ("
              + constraintMatch
              + ") from its constraintMatchSet ("
              + constraintMatchSet
              + ").");
    }
  }
