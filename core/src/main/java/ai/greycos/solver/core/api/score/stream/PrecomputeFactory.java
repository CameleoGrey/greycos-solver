...
/**
 * Similar to a {@link ConstraintFactory}, except its methods (and the {@link ConstraintStream}s
 * they return) do not apply any automatic filters (like those mentioned in {@link
 * ConstraintFactory#forEach(Class)}).
 */
public interface PrecomputeFactory {
  /**
   * As defined by {@link ConstraintFactory#forEachUnfiltered(Class)}, with the additional change of
   * any joining stream will also be unfiltered.
   *
   * <p>For example,
   *
   * <p>
   ...
   * </pre>
   *
   * <p>Would roughly be equivalent to
   *
   * <p>
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>Important: no variables can be referenced in any operations performed by the returned {@link
   * ConstraintStream}, otherwise a score corruption will occur. See the note in {@link
   * ConstraintFactory#precompute(Function)} for more details.
   *
  ...
  /**
   * As defined by {@link ConstraintFactory#forEachUniquePair(Class)}, with the additional change
   * that the problem facts/entities are unfiltered.
   *
   * <p>For example,
   *
   * <p>
  ...
  /**
   * As defined by {@link ConstraintFactory#forEachUniquePair(Class, BiJoiner)}, with the additional
   * change that the problem facts/entities are unfiltered.
   *
   * <p>For example,
   *
   * <p>
   ...
   */
  @SuppressWarnings("unchecked")
  default <A> BiConstraintStream<A, A> forEachUnfilteredUniquePair(
  ...
  default <A> BiConstraintStream<A, A> forEachUnfilteredUniquePair(
    ...
    return forEachUnfilteredUniquePair(
        sourceClass, new BiJoiner[] {joiner1, joiner2, joiner3, joiner4});
  }
   ...
   *     {@link BiJoiner joiners} are true
   */
  <A> BiConstraintStream<A, A> forEachUnfilteredUniquePair(
