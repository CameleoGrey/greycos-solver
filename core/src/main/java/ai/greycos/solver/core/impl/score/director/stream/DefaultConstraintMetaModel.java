...
record DefaultConstraintMetaModel(
    ...
    Map<String, List<Constraint>> constraintPerGroupMap)
    implements ConstraintMetaModel {
  ...
  public static ConstraintMetaModel of(List<? extends Constraint> constraints) {
    ...
    for (var constraint : constraints) {
      ...
      perGroupMap
          .computeIfAbsent(constraint.getConstraintGroup(), k -> new ArrayList<>(constraintCount))
          .add(constraint);
    }
    return new DefaultConstraintMetaModel(
        Collections.unmodifiableMap(perRefMap), Collections.unmodifiableMap(perGroupMap));
  }
