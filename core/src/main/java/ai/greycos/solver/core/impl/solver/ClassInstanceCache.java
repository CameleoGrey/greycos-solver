 ...
 *
 * <p>Case in point, {@link NearbyDistanceMeter}. If two places in the config reference the same
 * implementation of this interface, we can be sure that they represent the same logic. And since it
 * is us who instantiate them and they require no-arg constructors, we can be reasonably certain
 * that they will not contain any context-specific state. Therefore it is safe to have all of these
 * references served by a single instance, allowing for all sorts of beneficial caching. (Such as,
 * in this case, nearby distance matrix caching across the solver.)
 *
...
public final class ClassInstanceCache {
  ...
  public <T> T newInstance(Object configBean, String propertyName, Class<T> clazz) {
    return (T)
        singletonMap.computeIfAbsent(
            clazz, key -> ConfigUtils.newInstance(configBean, propertyName, key));
  }
}
