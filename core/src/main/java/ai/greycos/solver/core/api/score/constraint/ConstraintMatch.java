...
/**
 * Retrievable from {@link ConstraintMatchTotal#getConstraintMatchSet()} and {@link
 * Indictment#getConstraintMatchSet()}.
 *
 * <p>This class implements {@link Comparable} for consistent ordering of constraint matches in
 * visualizations. The details of this ordering are unspecified and are subject to change.
 *
...
public final class ConstraintMatch<Score_ extends Score<Score_>>
  ...
  public ConstraintMatch(
    ...
    this.indictedObjectList =
        requireNonNull(indictedObjectList) instanceof List<Object> list
            ? list
   ...
   *
   * <ul>
   *   <li>For constraint streams, it returns {@link DefaultConstraintJustification} from the
   *       matching tuple (eg. [A, B] for a bi stream), unless a custom justification mapping was
   *       provided, in which case it returns the return value of that function.
   ...
   *
   * <ul>
   *   <li>For constraint streams, it returns the facts from the matching tuple (eg. [A, B] for a bi
   *       stream), unless a custom indictment mapping was provided, in which case it returns the
   *       return value of that function.
   *   <li>For incremental score calculation, it returns what the calculator is implemented to
   *       return.
   *   <li>It may return an empty list, if justification support was disabled altogether.
   * </ul>
   *
  ...
  public int compareTo(ConstraintMatch<Score_> other) {
    ...
    return Integer.compare(
        System.identityHashCode(justification), System.identityHashCode(other.justification));
  }
