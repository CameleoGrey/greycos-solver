...
public final class BendableLongScoreInliner extends AbstractScoreInliner<BendableLongScore> {
  ...
  BendableLongScoreInliner(
      Map<Constraint, BendableLongScore> constraintWeightMap,
  ...
  @Override
  public WeightedScoreImpacter<BendableLongScore, ?> buildWeightedScoreImpacter(
    ...
    if (singleLevel != null) {
      ...
      BendableLongScoreContext context =
          new BendableLongScoreContext(
              ...
              softScores.length,
              level,
              constraintWeight.hardOrSoftScore(singleLevel));
      if (isHardScore) {
        return WeightedScoreImpacter.of(
            context,
            (BendableLongScoreContext ctx,
                long impact,
                ConstraintMatchSupplier<BendableLongScore> constraintMatchSupplier) ->
                ctx.changeHardScoreBy(impact, constraintMatchSupplier));
      } else {
        return WeightedScoreImpacter.of(
            context,
            (BendableLongScoreContext ctx,
                long impact,
                ConstraintMatchSupplier<BendableLongScore> constraintMatchSupplier) ->
                ctx.changeSoftScoreBy(impact, constraintMatchSupplier));
      }
    } else {
      BendableLongScoreContext context =
          new BendableLongScoreContext(
      ...
      return WeightedScoreImpacter.of(
          ...
          (BendableLongScoreContext ctx,
              long impact,
              ConstraintMatchSupplier<BendableLongScore> constraintMatchSupplier) ->
              ctx.changeScoreBy(impact, constraintMatchSupplier));
    }
  }
  ...
  public BendableLongScore extractScore() {
    return BendableLongScore.of(
        Arrays.copyOf(hardScores, hardScores.length), Arrays.copyOf(softScores, softScores.length));
  }
