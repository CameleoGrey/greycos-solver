...
public abstract class AbstractMoveSelectorFactory<
   ...
   *     pointless.
   * @param randomSelection true is equivalent to {@link SelectionOrder#RANDOM}, false is equivalent
   *     to {@link SelectionOrder#ORIGINAL}
   * @return never null
   */
  protected abstract MoveSelector<Solution_> buildBaseMoveSelector(
  ...
  public MoveSelector<Solution_> buildMoveSelector(
    ...
    MoveSelectorConfig<?> unfoldedMoveSelectorConfig =
        buildUnfoldedMoveSelectorConfig(configPolicy);
    if (unfoldedMoveSelectorConfig != null) {
      return buildMoveSelector(
          configPolicy,
          unfoldedMoveSelectorConfig,
          minimumCacheType,
    ...
    SelectionCacheType resolvedCacheType =
        SelectionCacheType.resolve(config.getCacheType(), minimumCacheType);
  ...
  protected static <T> T checkUnfolded(String configPropertyName, T configProperty) {
    if (configProperty == null) {
      throw new IllegalStateException(
          "The %s (%s) should haven been initialized during unfolding."
              .formatted(configPropertyName, configProperty));
    }
    return configProperty;
  }

  private void validateResolvedCacheType(
      SelectionCacheType resolvedCacheType, MoveSelector<Solution_> moveSelector) {
    if (!moveSelector.supportsPhaseAndSolverCaching()
        && resolvedCacheType.compareTo(SelectionCacheType.PHASE) >= 0) {
      throw new IllegalArgumentException(
          "The moveSelectorConfig ("
              + config
              + ") has a resolvedCacheType ("
              + resolvedCacheType
    ...
    return switch (resolvedSelectionOrder) {
      ...
      case RANDOM ->
          ...
          resolvedCacheType.isNotCached()
              || isBaseInherentlyCached() && config.getFilterClass() == null;
      default ->
          throw new IllegalStateException(
  ...
  private MoveSelector<Solution_> applyFiltering(
     ...
     */
    SelectionFilter<Solution_, Move<Solution_>> baseFilter =
        skipNonDoableMoves ? DoableMoveSelectionFilter.INSTANCE : null;
    ...
    if (filterClass != null) {
      ...
      SelectionFilter<Solution_, Move<Solution_>> finalFilter =
          baseFilter == null
              ? selectionFilter
              : SelectionFilter.compose(baseFilter, selectionFilter);
  ...
  protected void validateSorting(SelectionOrder resolvedSelectionOrder) {
    ...
    if ((comparatorClass != null
      ...
      throw new IllegalArgumentException(
          "The moveSelectorConfig (%s) with comparatorClass (%s) and comparatorFactoryClass (%s) and sorterOrder (%s) and sorterClass (%s) has a resolvedSelectionOrder (%s) that is not %s."
              .formatted(
                  config,
                  comparatorClass,
    ...
    if (comparatorClass != null && comparatorFactoryClass != null) {
      throw new IllegalArgumentException(
          "The moveSelectorConfig (%s) has both a comparatorClass (%s) and a comparatorFactoryClass (%s)."
              .formatted(config, comparatorClass, comparatorFactoryClass));
    }
    if (comparatorClass != null && config.getSorterClass() != null) {
      throw new IllegalArgumentException(
          "The moveSelectorConfig (%s) has both a comparatorClass (%s) and a sorterClass (%s)."
              .formatted(config, comparatorClass, config.getSorterClass()));
    }
    if (comparatorFactoryClass != null && config.getSorterClass() != null) {
      throw new IllegalArgumentException(
          "The moveSelectorConfig (%s) has both a comparatorFactoryClass (%s) and a sorterClass (%s)."
              .formatted(config, comparatorFactoryClass, config.getSorterClass()));
    }
    if (config.getSorterClass() != null && config.getSorterOrder() != null) {
      throw new IllegalArgumentException(
          "The moveSelectorConfig (%s) with sorterClass (%s) has a non-null sorterOrder (%s)."
              .formatted(config, config.getSorterClass(), config.getSorterOrder()));
    }
  }

  protected MoveSelector<Solution_> applySorting(
      SelectionCacheType resolvedCacheType,
      SelectionOrder resolvedSelectionOrder,
      MoveSelector<Solution_> moveSelector) {
    if (resolvedSelectionOrder == SelectionOrder.SORTED) {
      ...
      if (comparatorClass != null) {
        var sorterComparator = ConfigUtils.newInstance(config, "comparatorClass", comparatorClass);
        sorter =
        ...
        var comparatorFactory =
            ConfigUtils.newInstance(config, "comparatorFactoryClass", comparatorFactoryClass);
        sorter =
        ...
        throw new IllegalArgumentException(
            "The moveSelectorConfig (%s) with resolvedSelectionOrder (%s) needs a comparatorClass (%s) or a comparatorFactoryClass (%s) or a sorterClass (%s)."
                .formatted(
                    config,
                    resolvedSelectionOrder,
                    comparatorClass,
  ...
  private MoveSelector<Solution_> applyProbability(
      SelectionCacheType resolvedCacheType,
      SelectionOrder resolvedSelectionOrder,
      MoveSelector<Solution_> moveSelector) {
    if (resolvedSelectionOrder == SelectionOrder.PROBABILISTIC) {
      ...
      SelectionProbabilityWeightFactory<Solution_, Move<Solution_>> probabilityWeightFactory =
          ConfigUtils.newInstance(
              config, "probabilityWeightFactoryClass", probabilityWeightFactoryClass);
      moveSelector =
          new ProbabilityMoveSelector<>(moveSelector, resolvedCacheType, probabilityWeightFactory);
    }
    return moveSelector;
  }

  private MoveSelector<Solution_> applyShuffling(
      SelectionCacheType resolvedCacheType,
      SelectionOrder resolvedSelectionOrder,
      MoveSelector<Solution_> moveSelector) {
  ...
  private MoveSelector<Solution_> applyCaching(
      SelectionCacheType resolvedCacheType,
      SelectionOrder resolvedSelectionOrder,
      MoveSelector<Solution_> moveSelector) {
    if (resolvedCacheType.isCached()
        && resolvedCacheType.compareTo(moveSelector.getCacheType()) > 0) {
      moveSelector =
          new CachingMoveSelector<>(
              moveSelector, resolvedCacheType, resolvedSelectionOrder.toRandomSelectionBoolean());
    }
    return moveSelector;
  }
  ...
  private MoveSelector<Solution_> applySelectedLimit(MoveSelector<Solution_> moveSelector) {
    if (config.getSelectedCountLimit() != null) {
      moveSelector =
          new SelectedCountLimitMoveSelector<>(moveSelector, config.getSelectedCountLimit());
    }
    return moveSelector;
  }
}
