package ai.greycos.solver.core.impl.score.buildin;
...
import ai.greycos.solver.core.impl.score.definition.AbstractScoreDefinition;
...
public class HardMediumSoftScoreDefinition extends AbstractScoreDefinition<HardMediumSoftScore> {
  ...
  public HardMediumSoftScore fromLevelNumbers(Number[] levelNumbers) {
    ...
    return HardMediumSoftScore.of(
        (Integer) levelNumbers[0], (Integer) levelNumbers[1], (Integer) levelNumbers[2]);
  }

  @Override
  public HardMediumSoftScore buildOptimisticBound(
      InitializingScoreTrend initializingScoreTrend, HardMediumSoftScore score) {
    var trendLevels = initializingScoreTrend.trendLevels();
    return HardMediumSoftScore.of(
        trendLevels[0] == InitializingScoreTrendLevel.ONLY_DOWN
            ? score.hardScore()
            : Integer.MAX_VALUE,
        trendLevels[1] == InitializingScoreTrendLevel.ONLY_DOWN
            ? score.mediumScore()
            : Integer.MAX_VALUE,
        trendLevels[2] == InitializingScoreTrendLevel.ONLY_DOWN
            ? score.softScore()
            : Integer.MAX_VALUE);
  }

  @Override
  public HardMediumSoftScore buildPessimisticBound(
      InitializingScoreTrend initializingScoreTrend, HardMediumSoftScore score) {
    var trendLevels = initializingScoreTrend.trendLevels();
    return HardMediumSoftScore.of(
        trendLevels[0] == InitializingScoreTrendLevel.ONLY_UP
            ? score.hardScore()
            : Integer.MIN_VALUE,
        trendLevels[1] == InitializingScoreTrendLevel.ONLY_UP
            ? score.mediumScore()
            : Integer.MIN_VALUE,
        trendLevels[2] == InitializingScoreTrendLevel.ONLY_UP
            ? score.softScore()
            : Integer.MIN_VALUE);
  }

  @Override
  public HardMediumSoftScore divideBySanitizedDivisor(
      HardMediumSoftScore dividend, HardMediumSoftScore divisor) {
