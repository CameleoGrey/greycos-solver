...
final class AffectedEntitiesUpdater<Solution_> implements Consumer<BitSet> {
  ...
  static <Solution_> int[][] createNodeToEntityNodes(
    ...
    record EntityIdPair(Object entity, int entityId) {
      ...
      public boolean equals(Object o) {
        if (!(o instanceof EntityIdPair that)) return false;
    ...
    for (var entityIdPair : entityIdPairSet) {
      entityToNodes.put(
          entityIdPair.entityId(),
          entityToContainingNode.apply(entityIdPair.entity).stream()
  ...
  public void accept(BitSet changed) {
    ...
    while (!changeQueue.isEmpty()) {
      ...
      var isChanged =
          updateEntityShadowVariables(shadowVariable, graph.isLooped(loopedTracker, nextNode));
  ...
  private boolean updateEntityShadowVariables(
      GraphNode<Solution_> entityVariable, boolean isVariableInconsistent) {
  ...
  private <Entity_> boolean updateLoopedStatusOfEntity(
      Entity_ entity,
      int entityId,
    ...
    if (!Objects.equals(oldLooped, isEntityLooped)) {
      entityConsistencyState.setEntityIsInconsistent(
          changedVariableNotifier, entity, isEntityLooped);
    }
  ...
  private boolean updateShadowVariable(
      boolean isLooped, VariableUpdaterInfo<Solution_> shadowVariableReference, Object entity) {
    if (isLooped) {
    ...
    for (var node : nodeList) {
      ...
      if (node.groupEntityIds() != null) {
        for (var i = 0; i < node.groupEntityIds().length; i++) {
          ...
          updateLoopedStatusOfEntitySkippingProcessor(
              groupEntity, groupEntityId, entityConsistencyState);
        }
      } else {
        updateLoopedStatusOfEntitySkippingProcessor(
            node.entity(), node.entityId(), entityConsistencyState);
      }
    }
  }

  private <Entity_> void updateLoopedStatusOfEntitySkippingProcessor(
      Entity_ entity,
      int entityId,
      EntityConsistencyState<Solution_, Entity_> entityConsistencyState) {
