...
import static ai.greycos.solver.core.config.util.ConfigUtils.newInstance;
...
import ai.greycos.solver.core.impl.domain.valuerange.descriptor.FromSolutionPropertyValueRangeDescriptor;
...
public abstract class GenuineVariableDescriptor<Solution_> extends VariableDescriptor<Solution_> {
  ...
  protected void processValueRangeRefs(
      DescriptorPolicy descriptorPolicy, String[] valueRangeProviderRefs) {
    ...
    if (valueRangeProviderRefs == null || valueRangeProviderRefs.length == 0) {
      ...
      valueRangeProviderMemberAccessors =
          Arrays.stream(valueRangeProviderRefs)
    ...
    for (var valueRangeProviderMemberAccessor : valueRangeProviderMemberAccessors) {
      valueRangeDescriptorList.add(
          buildValueRangeDescriptor(descriptorPolicy, valueRangeProviderMemberAccessor));
    }
    if (valueRangeDescriptorList.size() == 1) {
      valueRangeDescriptor = valueRangeDescriptorList.get(0);
    } else {
      valueRangeDescriptor =
          descriptorPolicy.buildCompositeValueRangeDescriptor(this, valueRangeDescriptorList);
    }
  }

  private MemberAccessor[] findAnonymousValueRangeMemberAccessors(
        ...
        .filter(
            valueRangeProviderAccessor -> {
              ...
              var variableType =
                  isListVariable()
                      ? (Class<?>)
                          ((ParameterizedType) variableMemberAccessor.getGenericType())
              ...
              if (valueRangeType instanceof ParameterizedType parameterizedValueRangeType) {
                return ConfigUtils.extractGenericTypeParameter(
                        ...
                        valueRangeProviderAccessor.getType(),
                        parameterizedValueRangeType,
                        ValueRangeProvider.class,
                        valueRangeProviderAccessor.getName())
  ...
  private MemberAccessor findValueRangeMemberAccessor(
      DescriptorPolicy descriptorPolicy, String valueRangeProviderRef) {
    if (descriptorPolicy.hasFromSolutionValueRangeProvider(valueRangeProviderRef)) {
      ...
      throw new IllegalArgumentException(
          "The entityClass ("
              ...
              + valueRangeProviderRef
              + ") does not appear in the valueRangeProvideIds ("
              + providerIds
              + ")."
              + (!providerIds.isEmpty()
                  ? ""
  ...
  private ValueRangeDescriptor<Solution_> buildValueRangeDescriptor(
      DescriptorPolicy descriptorPolicy, MemberAccessor valueRangeProviderMemberAccessor) {
    if (descriptorPolicy.isFromSolutionValueRangeProvider(valueRangeProviderMemberAccessor)) {
      return descriptorPolicy.buildFromSolutionPropertyValueRangeDescriptor(
          this, valueRangeProviderMemberAccessor);
    } else if (descriptorPolicy.isFromEntityValueRangeProvider(valueRangeProviderMemberAccessor)) {
      return descriptorPolicy.buildFromEntityPropertyValueRangeDescriptor(
          this, valueRangeProviderMemberAccessor);
    } else {
      throw new IllegalStateException(
          "Impossible state: member accessor (%s) is not a value range provider."
  ...
  protected void processSorting(
      String comparatorPropertyName,
      Class<? extends Comparator> comparatorClass,
      String comparatorFactoryPropertyName,
      Class<? extends ComparatorFactory> comparatorFactoryClass) {
    if (comparatorClass != null
        && PlanningVariable.NullComparator.class.isAssignableFrom(comparatorClass)) {
    ...
    if (comparatorClass != null && comparatorFactoryClass != null) {
      throw new IllegalStateException(
          "The entityClass (%s) property (%s) cannot have a %s (%s) and a %s (%s) at the same time."
              .formatted(
                  entityDescriptor.getEntityClass(),
                  variableMemberAccessor.getName(),
    ...
    if (comparatorClass != null) {
      Comparator<Object> comparator =
          newInstance(this::toString, comparatorPropertyName, comparatorClass);
      ascendingSorter = new ComparatorSelectionSorter<>(comparator, SelectionSorterOrder.ASCENDING);
      descendingSorter =
  ...
  public void linkVariableDescriptors(DescriptorPolicy descriptorPolicy) {
    ...
    // This way, if this method ever gets any content, it will be called by all subclasses,
    // preventing bugs.
  }
  ...
  /**
   * Decides whether an entity is eligible for initialization. This is not an opposite of {@code
   * isInitialized()} because even a {@link PlanningVariable#allowsUnassigned() variable that allows
   * unassigned}, which is always considered initialized, is reinitializable if its value is {@code
   * null}.
   */
