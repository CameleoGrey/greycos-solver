...
final class BendableScoreContext extends ScoreContext<BendableScore, BendableScoreInliner> {
  ...
  public BendableScoreContext(
      BendableScoreInliner parent,
      AbstractConstraint<?, ?, ?> constraint,
      BendableScore constraintWeight,
      int hardScoreLevelCount,
      int softScoreLevelCount,
      int scoreLevel,
  ...
  public ScoreImpact<BendableScore> changeSoftScoreBy(
      long matchWeight, @Nullable ConstraintMatchSupplier<BendableScore> constraintMatchSupplier) {
  ...
  public ScoreImpact<BendableScore> changeHardScoreBy(
      long matchWeight, @Nullable ConstraintMatchSupplier<BendableScore> constraintMatchSupplier) {
  ...
  public ScoreImpact<BendableScore> changeScoreBy(
      long matchWeight, @Nullable ConstraintMatchSupplier<BendableScore> constraintMatchSupplier) {
    ...
    for (var hardScoreLevel = 0; hardScoreLevel < hardScoreLevelCount; hardScoreLevel++) {
      ...
      inliner.hardScores[hardScoreLevel] =
          Math.addExact(inliner.hardScores[hardScoreLevel], hardImpact);
    }
    for (var softScoreLevel = 0; softScoreLevel < softScoreLevelCount; softScoreLevel++) {
      ...
      inliner.softScores[softScoreLevel] =
          Math.addExact(inliner.softScores[softScoreLevel], softImpact);
    }
  ...
  private record SingleSoftImpact(BendableScoreContext ctx, long impact)
      implements ScoreImpact<BendableScore> {

    @Override
    public void undo() {
      ctx.inliner.softScores[ctx.scoreLevel] =
          Math.subtractExact(ctx.inliner.softScores[ctx.scoreLevel], impact);
    }

    @Override
    public BendableScore toScore() {
      return BendableScore.ofSoft(
          ctx.hardScoreLevelCount, ctx.softScoreLevelCount, ctx.scoreLevel, impact);
    }
  }
  ...
  private record SingleHardImpact(BendableScoreContext ctx, long impact)
    ...
    @Override
    public void undo() {
      ctx.inliner.hardScores[ctx.scoreLevel] =
          Math.subtractExact(ctx.inliner.hardScores[ctx.scoreLevel], impact);
    }

    @Override
    public BendableScore toScore() {
      return BendableScore.ofHard(
          ctx.hardScoreLevelCount, ctx.softScoreLevelCount, ctx.scoreLevel, impact);
    }
  }
  ...
  private record ComplexImpact(BendableScoreContext ctx, long[] hardImpacts, long[] softImpacts)
    ...
    public boolean equals(Object o) {
      ...
      if (!(o
          instanceof
          ComplexImpact(
              BendableScoreContext otherCtx,
              long[] otherHardImpacts,
              long[] otherSoftImpacts))) {
        return false;
      }
      return Objects.equals(ctx, otherCtx)
          && Objects.deepEquals(hardImpacts, otherHardImpacts)
