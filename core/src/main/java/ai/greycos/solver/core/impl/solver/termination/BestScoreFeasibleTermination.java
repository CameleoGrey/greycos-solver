...
final class BestScoreFeasibleTermination<Solution_>
  ...
  public BestScoreFeasibleTermination(
      ScoreDefinition<?> scoreDefinition, double[] timeGradientWeightFeasibleNumbers) {
    ...
    if (timeGradientWeightFeasibleNumbers.length != this.feasibleLevelsSize - 1) {
      throw new IllegalStateException(
          "The timeGradientWeightNumbers (%s)'s length (%d) is not 1 less than the feasibleLevelsSize (%d)."
              .formatted(
                  Arrays.toString(timeGradientWeightFeasibleNumbers),
                  timeGradientWeightFeasibleNumbers.length,
                  scoreDefinition.getFeasibleLevelsSize()));
    }
  }
  ...
  public double calculateSolverTimeGradient(SolverScope<Solution_> solverScope) {
    return calculateFeasibilityTimeGradient(
        InnerScore.fullyAssigned((Score) solverScope.getStartingInitializedScore()),
  ...
  public double calculatePhaseTimeGradient(AbstractPhaseScope<Solution_> phaseScope) {
    return calculateFeasibilityTimeGradient(
        phaseScope.getStartingScore(), phaseScope.getBestScore().raw());
  }

  <Score_ extends Score<Score_>> double calculateFeasibilityTimeGradient(
      @Nullable InnerScore<Score_> innerStartScore, Score_ score) {
