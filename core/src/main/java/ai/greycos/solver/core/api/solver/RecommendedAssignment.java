...
public interface RecommendedAssignment<Proposition_, Score_ extends Score<Score_>> {
  ...
  /**
   * Returns the proposition as returned by the proposition function. This is the actual assignment
   * recommended to the user.
   *
   * @return null if proposition function returned null
   */
  @Nullable Proposition_ proposition();

  /**
   * Difference between the original score and the score of the solution with the recommendation
   * applied.
   *
   * <p>If {@link SolutionManager#recommendAssignment(Object, Object, Function,
   * ScoreAnalysisFetchPolicy)} was called with {@link ScoreAnalysisFetchPolicy#FETCH_ALL}, the
   * analysis will include {@link MatchAnalysis constraint matches} inside its {@link
   * ConstraintAnalysis constraint analysis}; otherwise it will not.
   *
   * @return {@code fittedScoreAnalysis - originalScoreAnalysis} as defined by {@link
   *     ScoreAnalysis#diff(ScoreAnalysis)}
   */
