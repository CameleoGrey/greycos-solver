...
public class DefaultSolver<Solution_> extends AbstractSolver<Solution_> {
            ...
            entity -> {
              ...
              if (entityDescriptor.isMovable(
                ...
                if (pinIndex > listSize) {
                  // pinIndex == listSize is allowed, as that says the pin is at the end of the
                  // list,
                ...
                if (pinIndex != 0) {
                  throw new IllegalStateException(
                      "The immovable planning entity (%s) has a pin index (%s) which is not 0."
                          .formatted(entity, pinIndex));
                }
              }
            });
    ...
    if (id == null) { // Fail fast as planning ID is null.
      throw new IllegalStateException(
          "The planningId ("
              ...
              + fact
              + ") must not be null.\n"
              + "Maybe initialize the planningId of the class ("
              + planningIdAccessor.getDeclaringClass()
              + ") instance ("
              + fact
              + ") before solving.\n"
              + "Maybe remove the @"
  ...
  public void outerSolvingEnded(SolverScope<Solution_> solverScope) {
    logger.info(
        "Solving ended: time spent ({}), best score ({}), move evaluation speed ({}/sec), "
