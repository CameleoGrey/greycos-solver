...
/**
 * Circular buffer implementation for managing late scores, enabling simpler reset logic. When
 * {@link #tryReset} is called, instead of filling all slots, an epoch counter is incremented,
...
final class LateAcceptanceScoreBuffer {
  ...
  /**
   * Lazily resets all slots to {@code newScore}. Updating the score array is unnecessary since the
   * related counter ensures the new score is returned if no changes have occurred.
   *
   * @param newScore the score to be used to reset the buffer
   */
  void tryReset(InnerScore<?> newScore) {
    // Skips the reset action when no slot has been written since the last reset and the score is
    // unchanged
