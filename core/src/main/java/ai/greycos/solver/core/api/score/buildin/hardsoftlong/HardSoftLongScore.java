package ai.greycos.solver.core.api.score.buildin.hardsoftlong;
...
/**
 * This {@link Score} is based on 2 levels of long constraints: hard and soft. Hard constraints have
...
public final class HardSoftLongScore implements Score<HardSoftLongScore> {
  ...
  public static HardSoftLongScore parseScore(String scoreString) {
    var scoreTokens =
        ScoreUtil.parseScoreTokens(HardSoftLongScore.class, scoreString, HARD_LABEL, SOFT_LABEL);
    var hardScore =
        ScoreUtil.parseLevelAsLong(HardSoftLongScore.class, scoreString, scoreTokens[0]);
    var softScore =
        ScoreUtil.parseLevelAsLong(HardSoftLongScore.class, scoreString, scoreTokens[1]);
  ...
  public HardSoftLongScore add(HardSoftLongScore addend) {
    return of(hardScore + addend.hardScore(), softScore + addend.softScore());
  }

  @Override
  public HardSoftLongScore subtract(HardSoftLongScore subtrahend) {
    return of(hardScore - subtrahend.hardScore(), softScore - subtrahend.softScore());
  }

  @Override
  public HardSoftLongScore multiply(double multiplicand) {
    return of(
        (long) Math.floor(hardScore * multiplicand), (long) Math.floor(softScore * multiplicand));
  }

  @Override
  public HardSoftLongScore divide(double divisor) {
    return of((long) Math.floor(hardScore / divisor), (long) Math.floor(softScore / divisor));
  }
  ...
  public boolean equals(Object o) {
    if (o instanceof HardSoftLongScore other) {
      return hardScore == other.hardScore() && softScore == other.softScore();
    }
    return false;
  }
