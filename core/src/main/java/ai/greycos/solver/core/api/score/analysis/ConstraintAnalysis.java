...
import static ai.greycos.solver.core.api.score.analysis.ScoreAnalysis.DEFAULT_SUMMARY_CONSTRAINT_MATCH_LIMIT;
...
/**
 * Note: Users should never create instances of this type directly. It is available transitively via
 * {@link SolutionManager#analyze(Object)}.
 *
 * @param <Score_>
 * @param matches null if analysis not available; empty if constraint has no matches, but still
 *     non-zero constraint weight; non-empty if constraint has matches. This is a {@link List} to
 *     simplify access to individual elements, but it contains no duplicates just like {@link
 *     HashSet} wouldn't.
 * @param matchCount
 *     <ul>
 *       <li>For regular constraint analysis: -1 if analysis not available, 0 if constraint has no
 *           matches, positive if constraint has matches. Equal to the size of the {@link #matches}
 *           list.
 *       <li>For a {@link ScoreAnalysis#diff(ScoreAnalysis) diff of constraint analyses}: positive
 *           if the constraint has more matches in the new analysis, zero if the number of matches
 *           is the same in both, negative otherwise. Need not be equal to the size of the {@link
 *           #matches} list.
 *     </ul>
 */
public record ConstraintAnalysis<Score_ extends Score<Score_>>(
  ...
  @NonNull ConstraintAnalysis<Score_> negate() {
    ...
    if (matches == null) {
      ...
      return new ConstraintAnalysis<>(
          constraintRef, weight.negate(), score.negate(), null, matchCount);
    } else {
      // Within these semantics, match count == list size.
      var negatedMatchAnalysesList = matches.stream().map(MatchAnalysis::negate).toList();
      return new ConstraintAnalysis<>(
          constraintRef, weight.negate(), score.negate(), negatedMatchAnalysesList, matchCount);
    }
  }
  ...
  static <Score_ extends Score<Score_>> @NonNull ConstraintAnalysis<Score_> diff(
    ...
    if ((matchAnalyses == null && otherMatchAnalyses != null)
        || (matchAnalyses != null && otherMatchAnalyses == null)) {
      throw new IllegalStateException(
    ...
    // Compute the diff.
    var constraintWeightDifference =
        constraintAnalysis.weight().subtract(otherConstraintAnalysis.weight());
    ...
    if (matchAnalyses == null) {
      ...
      if ((!leftHasMatchCount && rightHasMatchCount)
          || (leftHasMatchCount && !rightHasMatchCount)) {
        throw new IllegalStateException(
    ...
    var matchAnalysesList =
        Stream.concat(matchAnalysisMap.keySet().stream(), otherMatchAnalysisMap.keySet().stream())
            .distinct()
            .flatMap(
                justification -> {
                  ...
                  if (matchAnalysis == null) {
                    ...
                    // No need to compute diff; this match is not present in the other score
                    // explanation.
                    ...
                    return Stream.of(
                        new MatchAnalysis<>(
                            constraintRef,
                            matchAnalysis.score().subtract(otherMatchAnalysis.score()),
                            justification));
  ...
  private static int getMatchCount(
      ConstraintAnalysis<?> analysis, ConstraintAnalysis<?> otherAnalysis) {
  ...
  /**
   * Returns a diagnostic text that explains part of the score quality through the {@link
  ...
  /**
   * Return a diagnostic text that explains part of the score quality using the {@link
   * #summarize(int)} method in a similar way to {@link #summarize()}. It is possible to specify the
   * maximum number of constraint matches to display per constraint by passing the desired limit as
   * an argument.
   *
  ...
  public String toString() {
    if (matches == null) {
      if (matchCount == -1) {
        return "(%s at %s, constraint matching disabled)".formatted(score, weight);
      } else {
