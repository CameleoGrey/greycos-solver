...
public class BendableLongScoreDefinition
    ...
    if (score.hardLevelsSize() != hardLevelsSize) {
      throw new IllegalArgumentException(
          "The scoreString ("
              + scoreString
              + ") for the scoreClass ("
              + BendableLongScore.class.getSimpleName()
              + ") doesn't follow the correct pattern:"
              + " the hardLevelsSize ("
              + score.hardLevelsSize()
              + ") doesn't match the scoreDefinition's hardLevelsSize ("
              + hardLevelsSize
              + ").");
    }
    if (score.softLevelsSize() != softLevelsSize) {
      throw new IllegalArgumentException(
          "The scoreString ("
              ...
              + ") doesn't follow the correct pattern:"
              + " the softLevelsSize ("
              + score.softLevelsSize()
              + ") doesn't match the scoreDefinition's softLevelsSize ("
              + softLevelsSize
              + ").");
    }
    return score;
  ...
  @Override
  public BendableLongScore buildOptimisticBound(
      InitializingScoreTrend initializingScoreTrend, BendableLongScore score) {
    var trendLevels = initializingScoreTrend.trendLevels();
    var hardScores = new long[hardLevelsSize];
    for (var i = 0; i < hardLevelsSize; i++) {
      hardScores[i] =
          (trendLevels[i] == InitializingScoreTrendLevel.ONLY_DOWN)
              ? score.hardScore(i)
              : Long.MAX_VALUE;
    }
    var softScores = new long[softLevelsSize];
    for (var i = 0; i < softLevelsSize; i++) {
      softScores[i] =
          (trendLevels[hardLevelsSize + i] == InitializingScoreTrendLevel.ONLY_DOWN)
              ? score.softScore(i)
              : Long.MAX_VALUE;
    }
    return BendableLongScore.of(hardScores, softScores);
  ...
  @Override
  public BendableLongScore buildPessimisticBound(
      InitializingScoreTrend initializingScoreTrend, BendableLongScore score) {
    var trendLevels = initializingScoreTrend.trendLevels();
    var hardScores = new long[hardLevelsSize];
    for (var i = 0; i < hardLevelsSize; i++) {
      hardScores[i] =
          (trendLevels[i] == InitializingScoreTrendLevel.ONLY_UP)
              ? score.hardScore(i)
              : Long.MIN_VALUE;
    }
    var softScores = new long[softLevelsSize];
    for (var i = 0; i < softLevelsSize; i++) {
      softScores[i] =
          (trendLevels[hardLevelsSize + i] == InitializingScoreTrendLevel.ONLY_UP)
              ? score.softScore(i)
              : Long.MIN_VALUE;
    }
    return BendableLongScore.of(hardScores, softScores);
  ...
  @Override
  public BendableLongScore divideBySanitizedDivisor(
