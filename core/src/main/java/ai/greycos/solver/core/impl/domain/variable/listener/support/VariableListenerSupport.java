...
import static ai.greycos.solver.core.impl.domain.variable.listener.support.ShadowVariableType.BASIC;
import static ai.greycos.solver.core.impl.domain.variable.listener.support.ShadowVariableType.CASCADING_UPDATE;
import static ai.greycos.solver.core.impl.domain.variable.listener.support.ShadowVariableType.CUSTOM_LISTENER;
import static ai.greycos.solver.core.impl.domain.variable.listener.support.ShadowVariableType.DECLARATIVE;
...
import ai.greycos.solver.core.impl.domain.variable.custom.LegacyCustomShadowVariableBasicVariableListener;
import ai.greycos.solver.core.impl.domain.variable.custom.LegacyCustomShadowVariableListVariableListener;
import ai.greycos.solver.core.impl.domain.variable.declarative.ConsistencyTracker;
import ai.greycos.solver.core.impl.domain.variable.declarative.DefaultShadowVariableSession;
import ai.greycos.solver.core.impl.domain.variable.declarative.DefaultShadowVariableSessionFactory;
...
import ai.greycos.solver.core.impl.domain.variable.listener.SourcedVariableListener;
import ai.greycos.solver.core.impl.domain.variable.listener.support.violation.ShadowVariablesAssert;
...
public final class VariableListenerSupport<Solution_> implements SupplyManager {
  ...
  public static <Solution_> VariableListenerSupport<Solution_> create(
  ...
  private final List<ListVariableChangedNotification<Solution_>>
      listVariableChangedNotificationList;
  ...
  @NonNull private final IntFunction<TopologicalOrderGraph> shadowVariableGraphCreator;
  ...
  public void linkVariableListeners() {
    ...
    scoreDirector.getSolutionDescriptor().getEntityDescriptors().stream()
        ...
        .forEach(
            descriptor -> {
              // All information about elements in all shadow variables is tracked in a centralized
              // place.
              // Therefore, all list-related shadow variables need to be connected to that
              // centralized place.
              ...
              if (listVariableStateSupply == null) {
                ...
                if (descriptor.isListVariableSource()) {
                  processShadowVariableDescriptorWithListVariable(
                      descriptor, listVariableStateSupply);
                } else {
                  processShadowVariableDescriptorWithoutListVariable(descriptor);
                }
              }
  ...
  private void processShadowVariableDescriptorWithListVariable(
    ...
    if (shadowVariableDescriptor
        instanceof IndexShadowVariableDescriptor<Solution_> indexShadowVariableDescriptor) {
      listVariableStateSupply.externalize(indexShadowVariableDescriptor);
    } else if (shadowVariableDescriptor
        instanceof
      ...
      listVariableStateSupply.externalize(previousElementShadowVariableDescriptor);
        ...
        instanceof
        NextElementShadowVariableDescriptor<Solution_> nextElementShadowVariableDescriptor) {
  ...
  private void processShadowVariableDescriptorWithoutListVariable(
      ShadowVariableDescriptor<Solution_> shadowVariableDescriptor) {
    ...
    if (!scoreDirector.getSolutionDescriptor().getDeclarativeShadowVariableDescriptors().isEmpty()
      ...
      shadowVariableSession =
          shadowVariableSessionFactory.forSolution(
              consistencyTracker, scoreDirector.getWorkingSolution());
    }
  }
  ...
  public void beforeListVariableChanged(
    ...
    if (!notifiables.isEmpty()) {
      ListVariableNotification<Solution_> notification =
          Notification.listVariableChanged(entity, fromIndex, toIndex);
      for (var notifiable : notifiables) {
        notifiable.notifyBefore(notification);
      }
      notificationQueuesAreEmpty = false;
  ...
  public void triggerVariableListenersInNotificationQueues() {
    ...
    if (listVariableDescriptor != null) {
      ...
      if (!cascadingUpdateShadowVarDescriptorList.isEmpty()
          && !(listVariableChangedNotificationList.isEmpty()
  ...
  private void triggerCascadingUpdateShadowVariableUpdate() {
    if (listVariableChangedNotificationList.isEmpty()
        || cascadingUpdateShadowVarDescriptorList.isEmpty()) {
      return;
    }
  ...
  private void cascadeListVariableChangedNotifications(
    ...
    for (var notification : listVariableChangedNotificationList) {
      cascadeListVariableValueUpdates(
          ...
          notification.getFromIndex(),
          notification.getToIndex(),
  ...
  /**
   * Clear all variable listeners without triggering any logic. The goal is to clear all queues and
   * avoid executing custom listener logic.
   */
  ...
  private void simulateGenuineVariableChange(Object entity) {
    ...
    for (var variableDescriptor : entityDescriptor.getGenuineVariableDescriptorList()) {
      if (variableDescriptor.isListVariable()) {
        ...
        // Triggering before...() is enough, as that will add the after...() call to the queue
        // automatically.
