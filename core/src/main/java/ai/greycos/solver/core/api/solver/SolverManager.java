 ...
 * planning problem, call {@link #solve(Object, Object, Consumer)} or {@link #solveAndListen(Object,
 * Object, Consumer)}.
 *
 * <p>These methods are thread-safe unless explicitly stated otherwise.
 *
 * <p>Internally a SolverManager manages a thread pool of solver threads (which call {@link
 * Solver#solve(Object)}) and consumer threads (to handle the {@link BestSolutionChangedEvent}s).
 *
 * <p>To learn more about problem change semantics, please refer to the {@link ProblemChange}
 * Javadoc.
 *
...
public interface SolverManager<Solution_, ProblemId_> extends AutoCloseable {
   ...
   *
   * <p>When using {@link SolutionManager} too, use {@link #create(SolverFactory)} instead so they
   * reuse the same {@link SolverFactory} instance.
   *
   * @param <Solution_> the solution type, the class with the {@link PlanningSolution} annotation
   * @param <ProblemId_> the ID type of a submitted problem, such as {@link Long} or {@link UUID}
   */
   ...
   *
   * <p>When using {@link SolutionManager} too, use {@link #create(SolverFactory,
   * SolverManagerConfig)} instead so they reuse the same {@link SolverFactory} instance.
   *
   * @param <Solution_> the solution type, the class with the {@link PlanningSolution} annotation
   * @param <ProblemId_> the ID type of a submitted problem, such as {@link Long} or {@link UUID}.
   */
  ...
  static <Solution_, ProblemId_> @NonNull SolverManager<Solution_, ProblemId_> create(
      @NonNull SolverFactory<Solution_> solverFactory,
      @NonNull SolverManagerConfig solverManagerConfig) {
  ...
  @NonNull SolverJobBuilder<Solution_, ProblemId_> solveBuilder();
   ...
   * a solver {@link Thread}, as soon as one is available. To retrieve the final best solution, use
   * {@link SolverJob#getFinalBestSolution()}.
   *
   * <p>In server applications, it's recommended to use {@link #solve(Object, Object, Consumer)}
   * instead, to avoid loading the problem going stale if solving can't start immediately. To listen
   * to intermediate best solutions too, use {@link #solveAndListen(Object, Object, Consumer)}
   * instead.
   *
   * <p>Defaults to logging exceptions as an error.
   *
   * <p>To stop a solver job before it naturally terminates, call {@link
   * SolverJob#terminateEarly()}.
   *
   * @param problemId a ID for each planning problem. This must be unique. Use this problemId to
   *     {@link #terminateEarly(Object) terminate} the solver early,
   * @param problem a {@link PlanningSolution} usually with uninitialized planning variables
   */
  default @NonNull SolverJob<Solution_, ProblemId_> solve(
      @NonNull ProblemId_ problemId, @NonNull Solution_ problem) {
   ...
   */
  default @NonNull SolverJob<Solution_, ProblemId_> solve(
      @NonNull ProblemId_ problemId,
      @NonNull Solution_ problem,
      @Nullable Consumer<? super Solution_> finalBestSolutionConsumer) {
    SolverJobBuilder<Solution_, ProblemId_> builder =
        solveBuilder().withProblemId(problemId).withProblem(problem);
    if (finalBestSolutionConsumer != null) {
      builder.withFinalBestSolutionEventConsumer(
   ...
   *
   * <p>When the solver terminates, the {@code finalBestSolutionConsumer} is called once with the
   * final best solution, on a consumer {@link Thread}, as soon as one is available. To listen to
   * intermediate best solutions too, use {@link #solveAndListen(Object, Object, Consumer)} instead.
   *
   * <p>Defaults to logging exceptions as an error.
   *
   * <p>To stop a solver job before it naturally terminates, call {@link #terminateEarly(Object)}.
   *
   * @param problemId an ID for each planning problem. This must be unique. Use this problemId to
   *     {@link #terminateEarly(Object) terminate} the solver early, {@link #getSolverStatus(Object)
   *     to get the status} or if the problem changes while solving.
  ...
  default @NonNull SolverJob<Solution_, ProblemId_> solve(
      ...
      @Nullable Consumer<? super Solution_> finalBestSolutionConsumer) {
    SolverJobBuilder<Solution_, ProblemId_> builder =
        solveBuilder().withProblemId(problemId).withProblemFinder(problemFinder);
    if (finalBestSolutionConsumer != null) {
      builder.withFinalBestSolutionConsumer(finalBestSolutionConsumer);
    }
    return builder.run();
   ...
   *     {@link #terminateEarly(Object) terminate} the solver early, {@link #getSolverStatus(Object)
   *     to get the status} or if the problem changes while solving.
   * @param problemFinder function that returns a {@link PlanningSolution}, usually with
   *     uninitialized planning variables
   * @param finalBestSolutionConsumer called only once, at the end, on a consumer thread
   * @param exceptionHandler called if an exception or error occurs. If null it defaults to logging
   *     the exception as an error.
   ...
   * As defined by {@link #solveAndListen(Object, Function, Consumer)}.
   *
   * <p>The final best solution is delivered twice: first to the {@code bestSolutionConsumer} when
   * it is found and then again to the {@code finalBestSolutionConsumer} when the solver terminates.
   * Do not store the solution twice. This allows for use cases that only process the {@link Score}
   * first (during best solution changed events) and then store the solution upon termination.
   *
   * @param problemId an ID for each planning problem. This must be unique. Use this problemId to
   *     {@link #terminateEarly(Object) terminate} the solver early, {@link #getSolverStatus(Object)
   *     to get the status} or if the problem changes while solving.
   ...
   *     uninitialized planning variables
   * @param bestSolutionConsumer called multiple times, on a consumer thread
   * @param finalBestSolutionConsumer called only once, at the end, on a consumer thread. That final
   *     best solution is already consumed by the bestSolutionConsumer earlier.
   * @param exceptionHandler called if an exception or error occurs. If null it defaults to logging
   *     the exception as an error.
   * @deprecated It is recommended to use {@link #solveBuilder()} while also providing a consumer
   *     for the best solution
   ...
   *
   * <p>Returns {@link SolverStatus#NOT_SOLVING} if the solver already terminated or if the
   * problemId was never added. To distinguish between both cases, use {@link
   * SolverJob#getSolverStatus()} instead. Here, that distinction is not supported because it would
   * cause a memory leak.
   *
   * @param problemId a value given to {@link #solve(Object, Object, Consumer)} or {@link
   *     #solveAndListen(Object, Object, Consumer)}
   */
  @NonNull SolverStatus getSolverStatus(@NonNull ProblemId_ problemId);

  /**
   * As defined by {@link #addProblemChanges(Object, List)}, only with a single {@link
   * ProblemChange}. Prefer to submit multiple {@link ProblemChange}s at once to reduce the
   * considerable overhead of multiple calls.
   */
  @NonNull
  default CompletableFuture<Void> addProblemChange(
      @NonNull ProblemId_ problemId, @NonNull ProblemChange<Solution_> problemChange) {
   ...
   * {@link Solver} and returns immediately. If the solver already terminated or the problemId was
   * never added, throws an exception. The same applies if the underlying {@link Solver} is not in
   * the {@link SolverStatus#SOLVING_ACTIVE} state.
   *
   * @param problemId a value given to {@link #solve(Object, Object, Consumer)} or {@link
   *     #solveAndListen(Object, Object, Consumer)}
   ...
   * @throws IllegalStateException if there is no solver actively solving the problem associated
   *     with the problemId
   * @see ProblemChange Learn more about problem change semantics.
   */
  @NonNull CompletableFuture<Void> addProblemChanges(
      @NonNull ProblemId_ problemId, @NonNull List<ProblemChange<Solution_>> problemChangeList);
  ...
  /**
   * Terminates all solvers, cancels all solver jobs that haven't (re)started yet and discards all
   * queued {@link ProblemChange}s. Releases all thread pool resources.
   *
   * <p>No new planning problems can be submitted after calling this method.
   */
  @Override
  void close();
}
