...
/**
 * The value range for list variables requires the selector to be entity-independent, as it needs to
 * fetch the entire list of values. Fetching the list of values is not a problem when the value
 * range is located within the solution class, serving as a single source of truth. In cases where
 * it is entity-dependent, the list of entities must be read to generate the corresponding list of
 * values.
 *
 * <p>This selector adapts {@link FromEntityPropertyValueSelector} to behave like an
 * entity-independent selector and meets the requirement to retrieve the complete list of values.
 *
...
public final class IterableFromEntityPropertyValueSelector<Solution_>
  ...
  public IterableFromEntityPropertyValueSelector(
      FromEntityPropertyValueSelector<Solution_> childValueSelector, boolean randomSelection) {
    this(childValueSelector, SelectionCacheType.JUST_IN_TIME, randomSelection);
  }

  public IterableFromEntityPropertyValueSelector(
      FromEntityPropertyValueSelector<Solution_> childValueSelector,
      SelectionCacheType minimumCacheType,
      boolean randomSelection) {
    this.childValueSelector = childValueSelector;
    this.minimumCacheType = minimumCacheType;
    this.randomSelection = randomSelection;
    this.valueRangeDescriptor =
        childValueSelector.getVariableDescriptor().getValueRangeDescriptor();
  }
  ...
  public long getSize() {
    return innerScoreDirector
        ...
        .countOnSolution(valueRangeDescriptor, innerScoreDirector.getWorkingSolution());
  }
