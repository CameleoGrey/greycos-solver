...
import static ai.greycos.solver.core.impl.util.ConstantLambdaUtils.biConstantNull;
import static ai.greycos.solver.core.impl.util.ConstantLambdaUtils.quadConstantOne;
import static ai.greycos.solver.core.impl.util.ConstantLambdaUtils.quadConstantOneBigDecimal;
import static ai.greycos.solver.core.impl.util.ConstantLambdaUtils.quadConstantOneLong;
import static ai.greycos.solver.core.impl.util.ConstantLambdaUtils.triConstantNull;
 ...
 *
 * @param <A> the type of the first matched fact (either a problem fact or a {@link PlanningEntity
 *     planning entity})
 * @param <B> the type of the second matched fact (either a problem fact or a {@link PlanningEntity
 *     planning entity})
 * @param <C> the type of the third matched fact (either a problem fact or a {@link PlanningEntity
 *     planning entity})
 * @param <D> the type of the fourth matched fact (either a problem fact or a {@link PlanningEntity
 *     planning entity})
...
public interface QuadConstraintStream<A, B, C, D> extends ConstraintStream {
  ...
  /**
   * Exhaustively test each tuple of facts against the {@link QuadPredicate} and match if {@link
   * QuadPredicate#test(Object, Object, Object, Object)} returns true.
   *
   * <p>Important: This is slower and less scalable than {@link
   * TriConstraintStream#join(UniConstraintStream, QuadJoiner)} with a proper {@link QuadJoiner}
   * predicate (such as {@link Joiners#equal(TriFunction, Function)}, because the latter applies
   * hashing and/or indexing, so it doesn't create every combination just to filter it out.
   */
  @NonNull QuadConstraintStream<A, B, C, D> filter(@NonNull QuadPredicate<A, B, C, D> predicate);
  ...
  /**
   * Create a new {@link QuadConstraintStream} for every tuple of A, B, C and D where E exists for
   * which the {@link PentaJoiner} is true (for the properties it extracts from the facts).
   *
   * <p>This method has overloaded methods with multiple {@link PentaJoiner} parameters.
   *
   ...
   * @return a stream that matches every tuple of A, B, C and D where E exists for which the {@link
   *     PentaJoiner} is true
   */
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifExists(
      @NonNull Class<E> otherClass, @NonNull PentaJoiner<A, B, C, D, E> joiner) {
    return ifExists(otherClass, new PentaJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifExists(Class, PentaJoiner)}. For performance reasons, indexing joiners
   * must be placed before filtering joiners.
   *
   * @param <E> the type of the fifth matched fact
   * @return a stream that matches every tuple of A, B, C and D where E exists for which the {@link
   *     PentaJoiner}s are true
   */
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifExists(
      @NonNull Class<E> otherClass,
  ...
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifExists(
      ...
      @NonNull PentaJoiner<A, B, C, D, E> joiner2,
      @NonNull PentaJoiner<A, B, C, D, E> joiner3,
      @NonNull PentaJoiner<A, B, C, D, E> joiner4) {
   ...
   *     PentaJoiner}s are true
   */
  <E> @NonNull QuadConstraintStream<A, B, C, D> ifExists(
      @NonNull Class<E> otherClass, @NonNull PentaJoiner<A, B, C, D, E>... joiners);
   ...
   */
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifExists(
      @NonNull UniConstraintStream<E> otherStream, @NonNull PentaJoiner<A, B, C, D, E> joiner) {
    return ifExists(otherStream, new PentaJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifExists(UniConstraintStream, PentaJoiner)}. For performance reasons,
   ...
   */
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifExists(
      @NonNull UniConstraintStream<E> otherStream,
      @NonNull PentaJoiner<A, B, C, D, E> joiner1,
      @NonNull PentaJoiner<A, B, C, D, E> joiner2) {
   ...
   */
  <E> @NonNull QuadConstraintStream<A, B, C, D> ifExists(
      @NonNull UniConstraintStream<E> otherStream, @NonNull PentaJoiner<A, B, C, D, E>... joiners);

  /**
   * Create a new {@link QuadConstraintStream} for every tuple of A, B, C and D where E exists for
   * which the {@link PentaJoiner} is true (for the properties it extracts from the facts). For
   ...
   *     PentaJoiner} is true
   */
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifExistsIncludingUnassigned(
      @NonNull Class<E> otherClass, @NonNull PentaJoiner<A, B, C, D, E> joiner) {
    return ifExistsIncludingUnassigned(otherClass, new PentaJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifExistsIncludingUnassigned(Class, PentaJoiner)}. For performance
  ...
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifExistsIncludingUnassigned(
    ...
    return ifExistsIncludingUnassigned(
        otherClass, new PentaJoiner[] {joiner1, joiner2, joiner3, joiner4});
  }

  /**
   * As defined by {@link #ifExistsIncludingNullVars(Class, PentaJoiner)}. For performance reasons,
   * indexing joiners must be placed before filtering joiners.
   *
   * <p>This method causes <i>Unchecked generics array creation for varargs parameter</i> warnings,
   * but we can't fix it with a {@link SafeVarargs} annotation because it's an interface method.
   ...
   *     PentaJoiner}s are true
   */
  <E> @NonNull QuadConstraintStream<A, B, C, D> ifExistsIncludingUnassigned(
      @NonNull Class<E> otherClass, @NonNull PentaJoiner<A, B, C, D, E>... joiners);

  /**
   * Create a new {@link QuadConstraintStream} for every tuple of A, B, C and D where E does not
   * exist for which the {@link PentaJoiner} is true (for the properties it extracts from the
   * facts).
   *
   * <p>This method has overloaded methods with multiple {@link PentaJoiner} parameters.
   *
   ...
   */
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifNotExists(
      @NonNull Class<E> otherClass, @NonNull PentaJoiner<A, B, C, D, E> joiner) {
    return ifNotExists(otherClass, new PentaJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifNotExists(Class, PentaJoiner)}. For performance reasons, indexing
   ...
   *     {@link PentaJoiner}s are true
   */
  <E> @NonNull QuadConstraintStream<A, B, C, D> ifNotExists(
      @NonNull Class<E> otherClass, @NonNull PentaJoiner<A, B, C, D, E>... joiners);
  ...
  /**
   * As defined by {@link #ifNotExists(UniConstraintStream, PentaJoiner)}. For performance reasons,
   * indexing joiners must be placed before filtering joiners.
   *
   * @param <E> the type of the fifth matched fact
   * @return a stream that matches every tuple of A, B, C and D where E does not exist for which the
   ...
   * Create a new {@link QuadConstraintStream} for every tuple of A, B, C and D where E does not
   * exist for which the {@link PentaJoiner} is true (for the properties it extracts from the
   * facts). For classes annotated with {@link PlanningEntity}, this method also includes entities
   * with null variables, or entities that are not assigned to any list variable.
   *
   * <p>This method has overloaded methods with multiple {@link PentaJoiner} parameters.
   *
   * @param <E> the type of the fifth matched fact
   ...
   *     {@link PentaJoiner} is true
   */
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifNotExistsIncludingUnassigned(
      @NonNull Class<E> otherClass, @NonNull PentaJoiner<A, B, C, D, E> joiner) {
    return ifNotExistsIncludingUnassigned(otherClass, new PentaJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifNotExistsIncludingUnassigned(Class, PentaJoiner)}. For performance
   * reasons, indexing joiners must be placed before filtering joiners.
   *
   * @param <E> the type of the fifth matched fact
   * @return a stream that matches every tuple of A, B, C and D where E does not exist for which the
  ...
  default <E> @NonNull QuadConstraintStream<A, B, C, D> ifNotExistsIncludingUnassigned(
    ...
    return ifNotExistsIncludingUnassigned(
        otherClass, new PentaJoiner[] {joiner1, joiner2, joiner3});
  }
   ...
   *     {@link PentaJoiner}s are true
   */
  <E> @NonNull QuadConstraintStream<A, B, C, D> ifNotExistsIncludingUnassigned(
      @NonNull Class<E> otherClass, @NonNull PentaJoiner<A, B, C, D, E>... joiners);
  ...
  /**
   * Convert the {@link QuadConstraintStream} to a {@link UniConstraintStream}, containing only a
   * single tuple, the result of applying {@link QuadConstraintCollector}. {@link
   * UniConstraintStream} which only has a single tuple, the result of applying {@link
  ...
  /**
   * Convert the {@link QuadConstraintStream} to a {@link BiConstraintStream}, containing only a
   * single tuple, the result of applying two {@link QuadConstraintCollector}s.
   *
  ...
  /**
   * Convert the {@link QuadConstraintStream} to a {@link TriConstraintStream}, containing only a
   * single tuple, the result of applying three {@link QuadConstraintCollector}s.
   *
   * @param collectorA the collector to perform the first grouping operation with See {@link
   *     ConstraintCollectors} for common operations, such as {@code count()}, {@code sum()} and
   *     others.
  ...
  /**
   * Convert the {@link QuadConstraintStream} to a {@link QuadConstraintStream}, containing only a
   * single tuple, the result of applying four {@link QuadConstraintCollector}s.
   *
   * @param collectorA the collector to perform the first grouping operation with See {@link
   *     ConstraintCollectors} for common operations, such as {@code count()}, {@code sum()} and
   *     others.
  ...
  /**
   * Convert the {@link QuadConstraintStream} to a {@link UniConstraintStream}, containing the set
   * of tuples resulting from applying the group key mapping function on all tuples of the original
  ...
  /**
   * Convert the {@link QuadConstraintStream} to a {@link BiConstraintStream}, consisting of unique
   * tuples.
   *
   * <p>The first fact is the return value of the group key mapping function, applied on the
   * incoming tuple. The second fact is the return value of a given {@link QuadConstraintCollector}
  ...
  /**
   * Convert the {@link QuadConstraintStream} to a {@link TriConstraintStream}, consisting of unique
   * tuples with three facts.
   *
   * <p>The first fact is the return value of the group key mapping function, applied on the
   * incoming tuple. The remaining facts are the return value of the respective {@link
   * QuadConstraintCollector} applied on all incoming tuples with the same first fact.
   *
   * @param groupKeyMapping function to convert the fact in the original tuple to a different fact
  ...
  /**
   * Convert the {@link QuadConstraintStream} to a {@link QuadConstraintStream}, consisting of
   * unique tuples with four facts.
   *
   * <p>The first fact is the return value of the group key mapping function, applied on the
   * incoming tuple. The remaining facts are the return value of the respective {@link
   * QuadConstraintCollector} applied on all incoming tuples with the same first fact.
  ...
  /**
   * Combines the semantics of {@link #groupBy(QuadFunction, QuadFunction)} and {@link
   * #groupBy(QuadConstraintCollector)}. That is, the first and second facts in the tuple follow the
   * {@link #groupBy(QuadFunction, QuadFunction)} semantics, and the third fact is the result of
   * applying {@link QuadConstraintCollector#finisher()} on all the tuples of the original {@link
   ...
   * Combines the semantics of {@link #groupBy(QuadFunction, QuadFunction)} and {@link
   * #groupBy(QuadConstraintCollector)}. That is, the first and second facts in the tuple follow the
   * {@link #groupBy(QuadFunction, QuadFunction)} semantics. The third fact is the result of
   * applying the first {@link QuadConstraintCollector#finisher()} on all the tuples of the original
   * {@link QuadConstraintStream} that belong to the group. The fourth fact is the result of
   * applying the second {@link QuadConstraintCollector#finisher()} on all the tuples of the
   * original {@link QuadConstraintStream} that belong to the group
   *
   * @param groupKeyAMapping function to convert the original tuple into a first fact
   * @param groupKeyBMapping function to convert the original tuple into a second fact
  ...
  /**
   * Combines the semantics of {@link #groupBy(QuadFunction, QuadFunction)} and {@link
   * #groupBy(QuadConstraintCollector)}. That is, the first three facts in the tuple follow the
   * {@link #groupBy(QuadFunction, QuadFunction)} semantics. The final fact is the result of
   * applying the first {@link QuadConstraintCollector#finisher()} on all the tuples of the original
   * {@link QuadConstraintStream} that belong to the group.
   *
   * @param groupKeyAMapping function to convert the original tuple into a first fact
   * @param groupKeyBMapping function to convert the original tuple into a second fact
   * @param groupKeyCMapping function to convert the original tuple into a third fact
  ...
  /**
   * Convert the {@link TriConstraintStream} to a {@link QuadConstraintStream}, consisting of unique
  ...
  <ResultA_> @NonNull UniConstraintStream<ResultA_> map(
      @NonNull QuadFunction<A, B, C, D, ResultA_> mapping);
  ...
  <ResultA_, ResultB_, ResultC_, ResultD_>
      @NonNull QuadConstraintStream<ResultA_, ResultB_, ResultC_, ResultD_> map(
          ...
          @NonNull QuadFunction<A, B, C, D, ResultB_> mappingB,
          @NonNull QuadFunction<A, B, C, D, ResultC_> mappingC,
          @NonNull QuadFunction<A, B, C, D, ResultD_> mappingD);

  /**
   * As defined by {@link BiConstraintStream#flattenLast(Function)}.
   *
   * @param <ResultD_> the type of the last fact in the resulting tuples. It is recommended that
   ...
   */
  <ResultD_> @NonNull QuadConstraintStream<A, B, C, ResultD_> flattenLast(
      @NonNull Function<D, Iterable<ResultD_>> mapping);
   ...
   * as {@link #map(QuadFunction)} may create a stream which breaks that promise. By calling this
   * method on such a stream, duplicate copies of the same tuple will be omitted at a performance
   * cost.
   */
  @NonNull QuadConstraintStream<A, B, C, D> distinct();

  /**
   * Returns a new {@link QuadConstraintStream} containing all the tuples of both this {@link
   * QuadConstraintStream} and the provided {@link UniConstraintStream}. The {@link
   * UniConstraintStream} tuples will be padded from the right by null.
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4), (C1,
   * C2, C3, C4)]} and the other stream consists of {@code [C, D, E]}, {@code this.concat(other)}
   * will consist of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4), (C1, C2, C3, C4), (C, null, null,
   * null), (D, null, null, null), (E, null, null, null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   */
  default @NonNull QuadConstraintStream<A, B, C, D> concat(
      @NonNull UniConstraintStream<A> otherStream) {
   ...
   * Returns a new {@link QuadConstraintStream} containing all the tuples of both this {@link
   * QuadConstraintStream} and the provided {@link UniConstraintStream}. The {@link
   * UniConstraintStream} tuples will be padded from the right by the results of the padding
   * functions.
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4), (C1,
   * C2, C3, C4)]} and the other stream consists of {@code [C, D, E]}, {@code this.concat(other, a
   * -> null, a -> null, a -> null)} will consist of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4),
   * (C1, C2, C3, C4), (C, null, null, null), (D, null, null, null), (E, null, null, null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   *
   ...
   */
  @NonNull QuadConstraintStream<A, B, C, D> concat(
      @NonNull UniConstraintStream<A> otherStream,
  ...
  /**
   * Returns a new {@link QuadConstraintStream} containing all the tuples of both this {@link
   * QuadConstraintStream} and the provided {@link BiConstraintStream}. The {@link
   * BiConstraintStream} tuples will be padded from the right by null.
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4), (C1,
   * C2, C3, C4)]} and the other stream consists of {@code [(C1, C2), (D1, D2), (E1, E2)]}, {@code
   * this.concat(other)} will consist of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4), (C1, C2, C3,
   * C4), (C1, C2, null, null), (D1, D2, null, null), (E1, E2, null, null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   */
  default @NonNull QuadConstraintStream<A, B, C, D> concat(
   ...
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4), (C1,
   * C2, C3, C4)]} and the other stream consists of {@code [(C1, C2), (D1, D2), (E1, E2)]}, {@code
   * this.concat(other, (a, b) -> null, (a, b) -> null)} will consist of {@code [(A1, A2, A3, A4),
   * (B1, B2, B3, B4), (C1, C2, C3, C4), (C1, C2, null, null), (D1, D2, null, null), (E1, E2, null,
   * null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   *
  ...
  /**
   * Returns a new {@link QuadConstraintStream} containing all the tuples of both this {@link
   * QuadConstraintStream} and the provided {@link TriConstraintStream}. The {@link
   * TriConstraintStream} tuples will be padded from the right by null.
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4), (C1,
   * C2, C3, C4)]} and the other stream consists of {@code [(C1, C2, C3), (D1, D2, D3), (E1, E2,
   * E3)]}, {@code this.concat(other)} will consist of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4),
   * (C1, C2, C3, C4), (C1, C2, C3, null), (D1, D2, D3, null), (E1, E2, E3, null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   */
  default @NonNull QuadConstraintStream<A, B, C, D> concat(
   ...
   * Returns a new {@link QuadConstraintStream} containing all the tuples of both this {@link
   * QuadConstraintStream} and the provided {@link TriConstraintStream}. The {@link
   * TriConstraintStream} tuples will be padded from the right by the result of the padding
   * function.
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3, A4), (B1, B2, B3, B4), (C1,
   * C2, C3, C4)]} and the other stream consists of {@code [(C1, C2, C3), (D1, D2, D3), (E1, E2,
   * E3)]}, {@code this.concat(other, (a, b, c) -> null)} will consist of {@code [(A1, A2, A3, A4),
   * (B1, B2, B3, B4), (C1, C2, C3, C4), (C1, C2, C3, null), (D1, D2, D3, null), (E1, E2, E3,
   * null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   *
  ...
  /**
   * Returns a new {@link QuadConstraintStream} containing all the tuples of both this {@link
   * QuadConstraintStream} and the provided {@link QuadConstraintStream}. Tuples in both this {@link
   * QuadConstraintStream} and the provided {@link QuadConstraintStream} will appear at least twice.
   *
   * <p>For instance, if this stream consists of {@code [(A, 1, -1, a), (B, 2, -2, b), (C, 3, -3,
   * c)]} and the other stream consists of {@code [(C, 3, -3, c), (D, 4, -4, d), (E, 5, -5, e)]},
   * {@code this.concat(other)} will consist of {@code [(A, 1, -1, a), (B, 2, -2, b), (C, 3, -3, c),
   * (C, 3, -3, c), (D, 4, -4, d), (E, 5, -5,e)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   */
  @NonNull QuadConstraintStream<A, B, C, D> concat(
  ...
  /**
   * As defined by {@link #complement(Class, Function, Function, Function)}, where the padding
   * function pads with null.
   */
   ...
   * instance. The rest of the tuple will be padded with the results of the padding functions,
   ...
   * @param paddingFunctionD function to find the padding for the fourth fact
   */
  default @NonNull QuadConstraintStream<A, B, C, D> complement(
    ...
    if (firstStream instanceof AbstractConstraintStream<?> abstractConstraintStream) {
      ...
      return firstStream.concat(
          secondStream.ifNotExists(remapped, Joiners.equal()),
          paddingFunctionB,
          paddingFunctionC,
          paddingFunctionD);
    } else {
  ...
  default <Score_ extends Score<Score_>>
      @NonNull QuadConstraintBuilder<A, B, C, D, Score_> penalize(
          @NonNull Score_ constraintWeight) {
    return penalize(constraintWeight, quadConstantOne());
  }

  /**
   * As defined by {@link #penalizeLong(Score, ToLongQuadFunction)}, where the match weight is one
   * (1).
   */
  default <Score_ extends Score<Score_>>
      @NonNull QuadConstraintBuilder<A, B, C, D, Score_> penalizeLong(
          @NonNull Score_ constraintWeight) {
    return penalizeLong(constraintWeight, quadConstantOneLong());
  }

  /**
   * As defined by {@link #penalizeBigDecimal(Score, QuadFunction)}, where the match weight is one
   * (1).
   */
  default <Score_ extends Score<Score_>>
      @NonNull QuadConstraintBuilder<A, B, C, D, Score_> penalizeBigDecimal(
          @NonNull Score_ constraintWeight) {
   ...
   *
   * <p>For non-int {@link Score} types use {@link #penalizeLong(Score, ToLongQuadFunction)} or
   ...
   *     constraintWeight
   */
  <Score_ extends Score<Score_>> @NonNull QuadConstraintBuilder<A, B, C, D, Score_> penalize(
  ...
  /**
   * As defined by {@link #penalize(Score, ToIntQuadFunction)}, with a penalty of type {@link
   * BigDecimal}.
   */
   ...
   *     constraintWeight
   * @return never null
   * @deprecated Prefer {@link #penalize(Score, ToIntQuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> penalizeConfigurable(
   ...
   *
   * @deprecated Prefer {@link #penalizeLong(Score, ToLongQuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> penalizeConfigurableLong(
      ToLongQuadFunction<A, B, C, D> matchWeigher);

  /**
   * As defined by {@link #penalizeConfigurable(ToIntQuadFunction)}, with a penalty of type {@link
   * BigDecimal}.
   *
   * @deprecated Prefer {@link #penalizeBigDecimal(Score, QuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> penalizeConfigurableBigDecimal(
   ...
   * ConstraintWeightOverrides} on the {@link PlanningSolution}-annotated class
   *
   * <p>For non-int {@link Score} types use {@link #rewardLong(Score, ToLongQuadFunction)} or {@link
   * #rewardBigDecimal(Score, QuadFunction)} instead.
   *
   * @param matchWeigher the result of this function (matchWeight) is multiplied by the
   *     constraintWeight
   */
  <Score_ extends Score<Score_>> @NonNull QuadConstraintBuilder<A, B, C, D, Score_> reward(
      @NonNull Score_ constraintWeight, @NonNull ToIntQuadFunction<A, B, C, D> matchWeigher);
  ...
  /**
   * As defined by {@link #reward(Score, ToIntQuadFunction)}, with a penalty of type {@link
   * BigDecimal}.
   */
  <Score_ extends Score<Score_>>
      @NonNull QuadConstraintBuilder<A, B, C, D, Score_> rewardBigDecimal(
          @NonNull Score_ constraintWeight,
          @NonNull QuadFunction<A, B, C, D, BigDecimal> matchWeigher);
   ...
   *     constraintWeight
   * @return never null
   * @deprecated Prefer {@link #reward(Score, ToIntQuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> rewardConfigurable(
      ToIntQuadFunction<A, B, C, D> matchWeigher);

  /**
   * As defined by {@link #rewardConfigurable(ToIntQuadFunction)}, with a penalty of type long.
   *
   * @deprecated Prefer {@link #rewardLong(Score, ToLongQuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> rewardConfigurableLong(
      ToLongQuadFunction<A, B, C, D> matchWeigher);

  /**
   * As defined by {@link #rewardConfigurable(ToIntQuadFunction)}, with a penalty of type {@link
   * BigDecimal}.
   *
   * @deprecated Prefer {@link #rewardBigDecimal(Score, QuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> rewardConfigurableBigDecimal(
      QuadFunction<A, B, C, D, BigDecimal> matchWeigher);
   ...
   *     constraintWeight
   */
  <Score_ extends Score<Score_>> @NonNull QuadConstraintBuilder<A, B, C, D, Score_> impact(
      @NonNull Score_ constraintWeight, @NonNull ToIntQuadFunction<A, B, C, D> matchWeigher);
  ...
  /**
   * As defined by {@link #impact(Score, ToIntQuadFunction)}, with an impact of type {@link
   * BigDecimal}.
   */
  <Score_ extends Score<Score_>>
      @NonNull QuadConstraintBuilder<A, B, C, D, Score_> impactBigDecimal(
          @NonNull Score_ constraintWeight,
          @NonNull QuadFunction<A, B, C, D, BigDecimal> matchWeigher);
   ...
   *
   * @return never null
   * @deprecated Prefer {@link #impact(Score, ToIntQuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> impactConfigurable(
      ToIntQuadFunction<A, B, C, D> matchWeigher);

  /**
   * As defined by {@link #impactConfigurable(ToIntQuadFunction)}, with an impact of type long.
   *
   * @deprecated Prefer {@link #impactLong(Score, ToLongQuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> impactConfigurableLong(
      ToLongQuadFunction<A, B, C, D> matchWeigher);

  /**
   * As defined by {@link #impactConfigurable(ToIntQuadFunction)}, with an impact of type
   * BigDecimal.
   *
   * @deprecated Prefer {@link #impactBigDecimal(Score, QuadFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  QuadConstraintBuilder<A, B, C, D, ?> impactConfigurableBigDecimal(
      QuadFunction<A, B, C, D, BigDecimal> matchWeigher);
  ...
  default <E> QuadConstraintStream<A, B, C, D> ifExistsIncludingNullVars(
      Class<E> otherClass, PentaJoiner<A, B, C, D, E> joiner) {
    return ifExistsIncludingUnassigned(otherClass, new PentaJoiner[] {joiner});
  }
  ...
  @Deprecated(forRemoval = true, since = "1.8.0")
  default <E> QuadConstraintStream<A, B, C, D> ifExistsIncludingNullVars(
      Class<E> otherClass, PentaJoiner<A, B, C, D, E> joiner1, PentaJoiner<A, B, C, D, E> joiner2) {
    return ifExistsIncludingUnassigned(otherClass, new PentaJoiner[] {joiner1, joiner2});
  }
   ...
   */
  @Deprecated(forRemoval = true, since = "1.8.0")
  default <E> QuadConstraintStream<A, B, C, D> ifNotExistsIncludingNullVars(
      Class<E> otherClass, PentaJoiner<A, B, C, D, E> joiner) {
    return ifNotExistsIncludingUnassigned(otherClass, new PentaJoiner[] {joiner});
  }
   ...
   *
   * <p>For non-int {@link Score} types use {@link #penalizeLong(String, Score, ToLongQuadFunction)}
   * or {@link #penalizeBigDecimal(String, Score, QuadFunction)} instead.
   *
   ...
   * ToLongQuadFunction)} or {@link #penalizeConfigurableBigDecimal(String, QuadFunction)} instead.
   *
   * @param constraintName never null, shows up in {@link ConstraintMatchTotal} during score
   *     justification
   * @param matchWeigher never null, the result of this function (matchWeight) is multiplied by the
  ...
  default Constraint penalizeConfigurable(
      String constraintName, ToIntQuadFunction<A, B, C, D> matchWeigher) {
  ...
  @Deprecated(forRemoval = true)
  default Constraint penalizeConfigurable(
      String constraintPackage, String constraintName, ToIntQuadFunction<A, B, C, D> matchWeigher) {
  ...
  default Constraint penalizeConfigurableLong(
      String constraintName, ToLongQuadFunction<A, B, C, D> matchWeigher) {
  ...
  default Constraint penalizeConfigurableBigDecimal(
      String constraintName, QuadFunction<A, B, C, D, BigDecimal> matchWeigher) {
   ...
   *
   * <p>For non-int {@link Score} types use {@link #rewardLong(String, Score, ToLongQuadFunction)}
   * or {@link #rewardBigDecimal(String, Score, QuadFunction)} instead.
   *
   ...
   *
   * <p>For non-int {@link Score} types use {@link #rewardConfigurableLong(String,
   * ToLongQuadFunction)} or {@link #rewardConfigurableBigDecimal(String, QuadFunction)} instead.
   *
   * @param constraintName never null, shows up in {@link ConstraintMatchTotal} during score
   *     justification
   * @param matchWeigher never null, the result of this function (matchWeight) is multiplied by the
   ...
   * have positive and negative weights.
   *
   * <p>For non-int {@link Score} types use {@link #impactLong(String, Score, ToLongQuadFunction)}
   * or {@link #impactBigDecimal(String, Score, QuadFunction)} instead.
   *
  ...
  /**
   * Positively or negatively impact the {@link Score} by the {@link ConstraintWeight} for each
   * match.
   *
   ...
   *
   * <p>For non-int {@link Score} types use {@link #impactConfigurableLong(String,
   * ToLongQuadFunction)} or {@link #impactConfigurableBigDecimal(String, QuadFunction)} instead.
   *
   * <p>The constraintWeight comes from an {@link ConstraintWeight} annotated member on the {@link
   * ConstraintConfiguration}, so end users can change the constraint weights dynamically. This
   * constraint may be deactivated if the {@link ConstraintWeight} is zero.
