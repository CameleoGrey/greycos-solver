...
@NullMarked
public sealed interface PhaseTermination<Solution_> extends Termination<Solution_>
  ...
  /**
   * Called by the {@link Phase} after every step and every move to determine if the search should
   * stop.
   *
   ...
   *
   * <p>Clients that use a timeGradient should cache it at the start of a single step because some
   * implementations are not time-stable.
   *
   * <p>If a timeGradient cannot be calculated, it should return -1.0. Several implementations (such
   * a {@link SimulatedAnnealingAcceptor}) require a correctly implemented timeGradient.
   *
   * <p>A Termination's timeGradient can be requested after they are terminated, so implementations
   * should be careful not to return a timeGradient above 1.0.
   *
   * @return timeGradient t for which {@code 0.0 <= t <= 1.0 or -1.0} when it is not supported. At
   *     the start of a solver t is 0.0 and at the end t would be 1.0.
   */
