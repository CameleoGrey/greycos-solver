package ai.greycos.solver.core.impl.domain.score.descriptor;
...
public final class ScoreDescriptor<Score_ extends Score<Score_>> {
  ...
  // Used to obtain default @PlanningScore attribute values from a score member that was
  // auto-discovered,
  ...
  public ScoreDescriptor(
      MemberAccessor scoreMemberAccessor, ScoreDefinition<Score_> scoreDefinition) {
  ...
  public void failFastOnDuplicateMember(
      DescriptorPolicy descriptorPolicy, Member member, Class<?> solutionClass) {
    MemberAccessor memberAccessor = descriptorPolicy.buildScoreMemberAccessor(member);
    // A solution class cannot have more than one score field or bean property (name check), and the
    // @PlanningScore
    ...
    if (!scoreMemberAccessor.getName().equals(memberAccessor.getName())
      ...
      throw new IllegalStateException(
          "The solutionClass ("
              ...
              + " annotated member ("
              + memberAccessor
              + ") that is duplicated by another member ("
              + scoreMemberAccessor
              + ").\n"
