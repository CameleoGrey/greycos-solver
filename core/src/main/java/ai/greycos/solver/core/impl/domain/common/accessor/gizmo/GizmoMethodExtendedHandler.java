...
final class GizmoMethodExtendedHandler extends GizmoMethodHandler {
  ...
  GizmoMethodExtendedHandler(
      Class<?> declaringClass, Class<?> methodParameterType, MethodDesc methodDescriptor) {
  ...
  public Expr readMemberValue(BlockCreator bytecodeCreator, Expr thisObj, Expr parameter) {
    return invokeMemberMethod(
        declaringClass, bytecodeCreator, methodDescriptor, thisObj, parameter);
  }
  ...
  public Type getType() {
    try {
      ...
      throw new IllegalStateException(
          "Cannot find method (%s) on class (%s)."
              .formatted(methodDescriptor.name(), declaringClass),
          e);
    }
  }
}
