...
public final class TerminationFactory<Solution_> {
  ...
  public static <Solution_> TerminationFactory<Solution_> create(
  ...
  public <Termination_ extends Termination<Solution_>> Termination_ buildTermination(
      HeuristicConfigPolicy<Solution_> configPolicy, Termination_ chainedTermination) {
  ...
  public <Score_ extends Score<Score_>> @Nullable Termination<Solution_> buildTermination(
    ...
    if (terminationConfig.getBestScoreLimit() != null) {
      ...
      terminationList.add(
          new BestScoreTermination<>(scoreDefinition, bestScoreLimit_, timeGradientWeightNumbers));
    }
    ...
    if (bestScoreFeasible != null) {
      ...
      if (!bestScoreFeasible) {
        throw new IllegalArgumentException(
            "The termination bestScoreFeasible (%s) cannot be false.".formatted(bestScoreFeasible));
      }
      ...
      terminationList.add(
          new BestScoreFeasibleTermination<>(scoreDefinition, timeGradientWeightFeasibleNumbers));
    }
    ...
    if (terminationConfig.getUnimprovedStepCountLimit() != null) {
      terminationList.add(
          new UnimprovedStepCountTermination<>(terminationConfig.getUnimprovedStepCountLimit()));
    }
  ...
  @SuppressWarnings("unchecked")
  <Score_ extends Score<Score_>> List<Termination<Solution_>> buildTimeBasedTermination(
      HeuristicConfigPolicy<Solution_> configPolicy) {
    ...
    Long unimprovedTimeMillisSpentLimit =
        terminationConfig.calculateUnimprovedTimeMillisSpentLimit();
    if (unimprovedTimeMillisSpentLimit != null) {
      if (terminationConfig.getUnimprovedScoreDifferenceThreshold() == null) {
        terminationList.add(
            new UnimprovedTimeMillisSpentTermination<>(unimprovedTimeMillisSpentLimit));
      } else {
        ScoreDefinition<Score_> scoreDefinition = configPolicy.getScoreDefinition();
        ...
        if (scoreDefinition.isNegativeOrZero(unimprovedScoreDifferenceThreshold_)) {
          throw new IllegalStateException(
              "The unimprovedScoreDifferenceThreshold ("
    ...
    if (terminationConfig.getDiminishedReturnsConfig() != null) {
      ...
      terminationList.add(
          new DiminishedReturnsTermination<>(gracePeriodMillis, minimumImprovementRatio));
    }

    return terminationList;
  }

  List<Termination<Solution_>> buildInnerTermination(
      HeuristicConfigPolicy<Solution_> configPolicy) {
    ...
    return terminationConfigList.stream()
        .map(config -> TerminationFactory.<Solution_>create(config).buildTermination(configPolicy))
