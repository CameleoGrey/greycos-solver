...
import ai.greycos.solver.core.impl.exhaustivesearch.event.ExhaustiveSearchPhaseLifecycleListener;
...
public final class ExhaustiveSearchDecider<Solution_>
  ...
  public ExhaustiveSearchDecider(
  ...
  public void expandNode(ExhaustiveSearchStepScope<Solution_> stepScope) {
    ...
    for (var move : moveRepository) {
      ...
      // Do not filter out pointless moves, because the original value of the entity(s) is
      // irrelevant.
  ...
  @SuppressWarnings("unchecked")
  private <Score_ extends Score<Score_>> void doMove(
      ...
      if (lastLayer) {
        ...
        if (optimisticBound.compareTo(bestPessimisticBound) > 0) {
          ...
          var pessimisticBound =
              castScoreBounder.calculatePessimisticBound(scoreDirector, innerScore);
