  ...
  public DiminishedReturnsTermination(long slidingWindowMillis, double minimumImprovementRatio) {
    if (slidingWindowMillis < 0L) {
      throw new IllegalArgumentException(
          "The slidingWindowMillis (%d) cannot be negative.".formatted(slidingWindowMillis));
    }

    if (minimumImprovementRatio <= 0.0) {
      throw new IllegalArgumentException(
          "The minimumImprovementRatio (%f) must be positive.".formatted(minimumImprovementRatio));
    }
