...
import static ai.greycos.solver.core.impl.heuristic.selector.move.generic.chained.SubChainReversingChangeMove.reverseChain;
...
public class SubChainReversingSwapMove<Solution_> extends AbstractMove<Solution_> {
  ...
  public SubChainReversingSwapMove(
    ...
    leftTrailingLastEntity =
        inverseVariableSupply.getInverseSingleton(leftSubChain.getLastEntity());
    this.rightSubChain = rightSubChain;
    rightTrailingLastEntity =
        inverseVariableSupply.getInverseSingleton(rightSubChain.getLastEntity());
  }
  ...
  public boolean isMoveDoable(ScoreDirector<Solution_> scoreDirector) {
    // Because leftFirstEntity and rightFirstEntity are unequal, chained guarantees their values are
    // unequal too.
    ...
    if (leftTrailingLastEntity != null) {
      if (leftTrailingLastEntity != rightFirstEntity) {
        castScoreDirector.changeVariableFacade(
            variableDescriptor, leftTrailingLastEntity, rightFirstEntity);
      } else {
        castScoreDirector.changeVariableFacade(
            variableDescriptor, leftLastEntity, rightFirstEntity);
      }
    }
    if (rightTrailingLastEntity != null) {
      if (rightTrailingLastEntity != leftFirstEntity) {
        castScoreDirector.changeVariableFacade(
            variableDescriptor, rightTrailingLastEntity, leftFirstEntity);
      } else {
        castScoreDirector.changeVariableFacade(
            variableDescriptor, rightLastEntity, leftFirstEntity);
      }
    }
  }

  @Override
  public SubChainReversingSwapMove<Solution_> rebase(
      ScoreDirector<Solution_> destinationScoreDirector) {
    return new SubChainReversingSwapMove<>(
        variableDescriptor,
  ...
  public String toString() {
    ...
    return leftSubChain.toDottedString()
        + " {"
        + oldLeftValue
        + "} <-reversing-> "
        + rightSubChain.toDottedString()
        + " {"
        + oldRightValue
        + "}";
  }
