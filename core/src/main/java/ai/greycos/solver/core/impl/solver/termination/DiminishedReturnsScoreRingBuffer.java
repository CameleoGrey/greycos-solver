...
/**
 * A heavily specialized pair of <a href="https://en.wikipedia.org/wiki/Circular_buffer">ring
 * buffers</a> that acts as a {@link NavigableMap} for getting the scores at a prior time. <br>
 * The ring buffers operate with the following assumptions:
 *
 * <ul>
 *   <li>Keys (i.e. {@link System#nanoTime()} are inserted in ascending order
 *   <li>Times are queried in ascending order
 *   <li>The queried time is probably near the start of the ring buffer
 * </ul>
 *
 * <br>
 * {@link DiminishedReturnsScoreRingBuffer} automatically clear entries prior to the queried time
 * when polled (if the queried time is not in the {@link DiminishedReturnsScoreRingBuffer}, then the
 * largest entry prior to the queried time is kept and returned). The ring buffers will
 * automatically resize when filled past their capacity.
 *
 * @param <Score_> The score type
 */
final class DiminishedReturnsScoreRingBuffer<Score_ extends Score<Score_>> {
  /** Arbitrary; 4096 is a common buffer size and is the default page size in the linux kernel. */
  ...
  DiminishedReturnsScoreRingBuffer(
      int readIndex,
      int writeIndex,
      long[] nanoTimeRingBuffer,
      @Nullable InnerScore<Score_>[] scoreRingBuffer) {
  ...
  record RingBufferState(
      int readIndex,
      int writeIndex,
      long[] nanoTimeRingBuffer,
      @Nullable InnerScore<?>[] scoreRingBuffer) {
    @Override
    public boolean equals(Object o) {
      if (!(o instanceof RingBufferState that)) {
        return false;
      }
      return readIndex == that.readIndex
          && writeIndex == that.writeIndex
          && Objects.deepEquals(nanoTimeRingBuffer, that.nanoTimeRingBuffer)
          && Objects.deepEquals(scoreRingBuffer, that.scoreRingBuffer);
    }
    ...
    public int hashCode() {
      return Objects.hash(
          readIndex,
          writeIndex,
          Arrays.hashCode(nanoTimeRingBuffer),
          Arrays.hashCode(scoreRingBuffer));
    }
    ...
    public String toString() {
      return "RingBufferState{"
          ...
          + writeIndex
          + ", nanoTimeRingBuffer="
          + Arrays.toString(nanoTimeRingBuffer)
          + ", scoreRingBuffer="
          + Arrays.toString(scoreRingBuffer)
          + '}';
    }
  }
  ...
  /**
   * Returns the first element of the score ring buffer. Does not remove the element. Raises an
  ...
  /**
   * Adds a time-score pairing to the ring buffers, resizing the ring buffers if necessary.
   *
  ...
  /**
   * Removes count elements from both ring buffers, and returns the next element (which remains in
   * the buffers).
   *
   * @param count the number of items to remove from both buffers.
   * @return the first element in the score ring buffer after the count elements were removed.
   */
  ...
  /**
   * Returns the latest score prior or at the given time, and removes all time-score pairings prior
   * to it.
   *
