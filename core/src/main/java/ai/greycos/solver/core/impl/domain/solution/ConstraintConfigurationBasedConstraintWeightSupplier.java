...
public final class ConstraintConfigurationBasedConstraintWeightSupplier<
  ...
  private final Map<ConstraintRef, Function<Solution_, Score_>> constraintWeightExtractorMap =
      new LinkedHashMap<>();
  ...
  private ConstraintConfigurationBasedConstraintWeightSupplier(
    ...
    this.constraintConfigurationDescriptor =
        Objects.requireNonNull(constraintConfigurationDescriptor);
  }
  ...
  public void initialize(
    ...
    constraintConfigurationDescriptor.processAnnotations(
        memberAccessorFactory, domainAccessType, scoreDescriptor.getScoreDefinition());
    constraintConfigurationDescriptor
        .getSupportedConstraints()
        .forEach(
            constraintRef -> {
              var descriptor =
                  constraintConfigurationDescriptor.findConstraintWeightDescriptor(constraintRef);
              var weightExtractor =
                  (Function<Solution_, Score_>)
                      descriptor.createExtractor(
  ...
  public void validate(Solution_ workingSolution, Set<ConstraintRef> userDefinedConstraints) {
    var missingConstraints =
        userDefinedConstraints.stream()
  ...
  private void validateConstraintWeight(ConstraintRef constraintRef, Score_ constraintWeight) {
    ...
    var scoreDescriptor =
        constraintConfigurationDescriptor.getSolutionDescriptor().<Score_>getScoreDescriptor();
  ...
  public String toString() {
    return "Constraint weights based on "
        + constraintConfigurationDescriptor.getConstraintConfigurationClass()
        + ".";
  }
}
