...
/**
 * A class representing a non-negative int-to-int map that is compacted so the backing int arrays
 * are as small as possible.
 *
 * <p>IMPORTANT: As the class does a linear probe, it is only appropriate to use when counting up to
 * a few dozen keys. When using more keys, use a {@link java.util.Map} instead.
 */
@NullMarked
public final class DynamicLinearProbeNonNegativeIntCounter {
  ...
  /**
   * Creates an entry for key and set its count to 1. Does a linear probe to find a {@link #MISSING}
   * entry in the keys/counts arrays, or expand the arrays if all entries have a key.
   *
