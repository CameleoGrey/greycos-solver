...
public abstract class AbstractTabuAcceptor<Solution_> extends AbstractAcceptor<Solution_> {
    ...
    for (var it = tabuSequenceDeque.iterator(); it.hasNext(); ) {
      ...
      if (oldTabuStepIndexInteger == null) {
        throw new IllegalStateException(
            "HashCode stability violation: the hashCode() of tabu ("
                + oldTabu
                + ") of class ("
                + oldTabu.getClass()
        ...
        for (var tabu : tabuSequenceDeque) {
          ...
          if (tabu != null && tabu.equals(checkingTabu)) {
            if (tabu.hashCode() != checkingTabu.hashCode()) {
              throw new IllegalStateException(
                  "HashCode/equals contract violation: tabu ("
                      + tabu
                      + ") of class ("
                      + tabu.getClass()
                      + ") and checkingTabu ("
  ...
  protected abstract Collection<? extends Object> findTabu(
      LocalSearchMoveScope<Solution_> moveScope);

  protected abstract Collection<? extends Object> findNewTabu(
