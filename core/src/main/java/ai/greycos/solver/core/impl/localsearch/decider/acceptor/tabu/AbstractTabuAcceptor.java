...
import ai.greycos.solver.core.impl.localsearch.decider.acceptor.AbstractAcceptor;
import ai.greycos.solver.core.impl.localsearch.decider.acceptor.Acceptor;
import ai.greycos.solver.core.impl.localsearch.decider.acceptor.tabu.size.TabuSizeStrategy;
 ...
 */
@NullMarked
public abstract sealed class AbstractTabuAcceptor<Solution_> extends AbstractAcceptor<Solution_>
  ...
  private Map<@Nullable Object, Integer> tabuToStepIndexMap =
      Collections.emptyMap(); // Avoid @Nullable.
  ...
  public void phaseStarted(LocalSearchPhaseScope<Solution_> phaseScope) {
    ...
    // Tabu sizes do not change during stepStarted(), because they must be in sync with the
    // tabuSequenceList.size()
    workingTabuSize =
        tabuSizeStrategy == null ? 0 : tabuSizeStrategy.determineTabuSize(lastCompletedStepScope);
  ...
  public void stepEnded(LocalSearchStepScope<Solution_> stepScope) {
    ...
    workingFadingTabuSize =
        fadingTabuSizeStrategy == null ? 0 : fadingTabuSizeStrategy.determineTabuSize(stepScope);
  ...
  public boolean isAccepted(LocalSearchMoveScope<Solution_> moveScope) {
    ...
    if (aspirationEnabled
        && moveScope.getScore().compareTo(moveScope.getStepScope().getPhaseScope().getBestScore())
            > 0) {
      logger.trace(
          "{}        Proposed move ({}) is tabu, but is accepted anyway due to aspiration.",
          logIndentation,
          moveScope.getMove());
      return true;
    }
    var tabuStepCount =
        moveScope.getStepScope().getStepIndex() - maximumTabuStepIndex; // at least 1
    if (tabuStepCount <= workingTabuSize) {
      logger.trace(
          "{}        Proposed move ({}) is tabu and is therefore not accepted.",
          logIndentation,
          moveScope.getMove());
      return false;
    }
    ...
    if (accepted) {
      logger.trace(
          "{}        Proposed move ({}) is fading tabu with acceptChance ({}) and is accepted.",
          logIndentation,
          moveScope.getMove(),
          Math.abs(decision));
    } else {
      logger.trace(
          "{}        Proposed move ({}) is fading tabu with acceptChance ({}) and is not accepted.",
          logIndentation,
          moveScope.getMove(),
          Math.abs(decision));
    }
    ...
    for (var checkingTabu : checkingTabus) {
      ...
      if (assertTabuHashCodeCorrectness) {
        for (var tabu : tabuToStepIndexMap.keySet()) {
          // tabu and checkingTabu can be null with a planning variable which allows unassigned
          // values
   ...
   * @return in absolute value, the accept chance; negative signum or 0 means not accepted, positive
   *     signum means accepted. This is hacky, but we can represent 2 things with one number and
   *     avoid allocation new types for this multiple return.
   */
  private double decideFadingTabuAcceptance(
      LocalSearchMoveScope<Solution_> moveScope, int fadingTabuStepCount) {
    // Invert the chance; the longer the element is in the tabu list, the higher the chance should
    // be.
   ...
   */
  protected abstract Collection<@Nullable Object> findTabu(
   ...
   * @return some tabu lists allow null, some don't; children will override
   */
  protected abstract Collection<@Nullable Object> findNewTabu(
