package ai.greycos.solver.core.api.domain.solution;
 ...
 * solution of that problem. A possible solution does not need to be optimal or even feasible. A
 * solution's planning variables might not be initialized (especially when delivered as a problem).
 *
 * <p>A solution is mutable. For scalability reasons (to facilitate incremental score calculation),
 * the same solution instance (called the working solution per move thread) is continuously
 * modified. It's cloned to recall the best solution.
 *
 * <p>Each planning solution must have exactly 1 {@link PlanningScore} property.
 *
 * <p>Each planning solution must have at least 1 {@link PlanningEntityCollectionProperty} or {@link
 * PlanningEntityProperty} property.
 *
 * <p>Each planning solution is recommended to have 1 {@link ConstraintWeightOverrides} property
 * too. This will make it easy for a solution to override constraint weights provided in {@link
 * ConstraintProvider}, in turn making it possible to run different solutions with a different
 * balance of constraint weights.
 *
 * <p>Each planning solution used with ConstraintStream score calculation must have at least 1
 * {@link ProblemFactCollectionProperty} or {@link ProblemFactProperty} property.
 *
...
public @interface PlanningSolution {
   ...
   *
   * <p>If this is not specified, then the default reflection-based {@link SolutionCloner} is used,
  ...
  /**
   * @deprecated When multi-threaded solving, ensure your domain classes use @{@link PlanningId}
   *     instead.
   */
