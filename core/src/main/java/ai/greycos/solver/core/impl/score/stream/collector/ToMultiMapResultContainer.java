...
public final class ToMultiMapResultContainer<
  ...
  public ToMultiMapResultContainer(
  ...
  public ToMultiMapResultContainer(
      IntFunction<Result_> resultFunction, IntFunction<Set_> setFunction) {
    IntFunction<Set_> nonNullSetFunction = Objects.requireNonNull(setFunction);
    this.setSupplier = () -> nonNullSetFunction.apply(0);
  ...
  public void add(Key_ key, Value_ value) {
    ToMapPerKeyCounter<Value_> counter =
        valueCounts.computeIfAbsent(key, k -> new ToMapPerKeyCounter<>());
    counter.add(value);
