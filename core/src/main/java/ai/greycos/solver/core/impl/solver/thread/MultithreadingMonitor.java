 ...
 * view of all monitoring components including memory usage, performance metrics, thread pool
 * health, and error recovery status.
 *
 * @since 1.0.0
 */
public class MultithreadingMonitor {
  ...
  public HealthStatus getHealthStatus() {
    ...
    ErrorRecoveryManager.RecoveryStatistics recoveryStats =
    ...
    boolean performanceHealthy =
        perfStats.getOverallEfficiency() > 50.0; // 50% efficiency threshold
    boolean recoveryHealthy =
        recoveryStats.getRecoveryState() == ErrorRecoveryManager.RecoveryState.NORMAL;
    boolean threadsHealthy =
        adjustmentStats.getTargetThreads() == adjustmentStats.getCurrentThreads();

    HealthLevel overallHealth =
        determineOverallHealth(memoryHealthy, performanceHealthy, recoveryHealthy, threadsHealthy);
  ...
  public PerformanceSuggestions getPerformanceSuggestions() {
    ...
    if (perfStats.getOverallEfficiency() < 70.0) {
      builder.addSuggestion(
          "Low overall efficiency ("
              + perfStats.getOverallEfficiency()
    ...
    if (memoryPressure != MemoryMonitor.MemoryPressureLevel.NORMAL) {
      builder.addSuggestion(
          "High memory pressure detected. Consider reducing move thread count or buffer sizes.");
    }

    // Analyze thread count
    if (currentThreads > 4 && perfStats.getOverallEfficiency() < 80.0) {
      builder.addSuggestion(
          "High thread count with low efficiency. Consider reducing thread count.");
    }

    // Analyze calculation throughput
    if (perfStats.getCalculationsPerSecond() < 1000) {
      builder.addSuggestion(
          "Low calculation throughput. Consider optimizing score calculation or move evaluation.");
    }
  ...
  private void logHealthStatus(HealthStatus healthStatus) {
    LOGGER.info(
        "Multithreading health check: {} - Memory: {}, Performance: {}, Recovery: {}, Threads: {}",
  ...
  private void logDetailedReport(MonitoringReport report) {
    ...
    LOGGER.info(
        "Performance: {:.2f} calc/sec, {:.2f} moves/sec, {:.2f}% efficiency",
  ...
  public static class HealthStatus {
    ...
    public boolean isRecoveryHealthy() {
      return recoveryHealthy;
    }

    public boolean isThreadsHealthy() {
      return threadsHealthy;
    }
    ...
    public boolean isErrorRecoveryNeeded() {
      return errorCount > 0;
    }

    public boolean isThreadAdjustmentNeeded() {
      return threadAdjustmentNeeded;
    }
  }
  ...
  public static class MonitoringReport {
    ...
    // Getters
    public MemoryMonitor.MemoryStatistics getMemoryStatistics() {
      return memoryStatistics;
    }

    public MemoryMonitor.MemoryPressureStatistics getPressureStatistics() {
      return pressureStatistics;
    }

    public PerformanceMetrics.PerformanceStatistics getPerformanceStatistics() {
      return performanceStatistics;
    }

    public ErrorRecoveryManager.RecoveryStatistics getRecoveryStatistics() {
      return recoveryStatistics;
    }

    public AdaptiveThreadPoolManager.AdjustmentStatistics getAdjustmentStatistics() {
