...
public final class BigDecimalValueRange extends AbstractCountableValueRange<BigDecimal> {
  ...
  public BigDecimalValueRange(BigDecimal from, BigDecimal to, BigDecimal incrementUnit) {
    ...
    if (scale != to.scale()) {
      throw new IllegalArgumentException(
          "The "
              ...
              + ") scale ("
              + to.scale()
              + ") which is different than its from ("
              + from
              + ") scale ("
              + scale
              + ").");
    ...
    if (incrementUnit.compareTo(BigDecimal.ZERO) <= 0) {
      throw new IllegalArgumentException(
          "The "
              + getClass().getSimpleName()
              + " must have strictly positive incrementUnit ("
              + incrementUnit
              + ").");
    }

    if (!to.unscaledValue()
        .subtract(from.unscaledValue())
        .remainder(incrementUnit.unscaledValue())
      ...
      throw new IllegalArgumentException(
          "The "
              ...
              + "'s incrementUnit ("
              + incrementUnit
              + ") must fit an integer number of times between from ("
              + from
              + ") and to ("
              + to
              + ").");
  ...
  @Override
  public long getSize() {
    return to.unscaledValue()
        .subtract(from.unscaledValue())
        .divide(incrementUnit.unscaledValue())
        .longValue();
  }
  ...
  public BigDecimal get(long index) {
    if (index < 0L || index >= getSize()) {
      throw new IndexOutOfBoundsException(
          "The index (" + index + ") must be >= 0 and < size (" + getSize() + ").");
    }
