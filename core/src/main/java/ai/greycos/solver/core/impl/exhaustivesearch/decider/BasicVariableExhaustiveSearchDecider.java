...
public final class BasicVariableExhaustiveSearchDecider<Solution_, Score_ extends Score<Score_>>
  ...
  public boolean isEntityReinitializable(Object entity) {
    ...
    var reinitializeVariableCount =
        sourceEntitySelector.getEntityDescriptor().countReinitializableVariables(entity);
  ...
  public void restoreWorkingSolution(
    ...
    while (oldNode != newNode) {
      ...
      if (oldDepth < newDepth) {
        newMoveArray[newMoveArray.length - newMoveCount++ - 1] =
            newNode.getMove(); // Build this in reverse.
    ...
    System.arraycopy(
        newMoveArray, newMoveArray.length - newMoveCount, moves, oldMoveCount, newMoveCount);
