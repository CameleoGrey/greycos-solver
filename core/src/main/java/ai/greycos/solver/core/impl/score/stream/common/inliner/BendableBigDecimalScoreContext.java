...
final class BendableBigDecimalScoreContext
  ...
  public UndoScoreImpacter changeSoftScoreBy(
    ...
    return impactWithConstraintMatch(
        ...
        BendableBigDecimalScore.ofSoft(
            hardScoreLevelCount, softScoreLevelCount, scoreLevel, softImpact),
  ...
  public UndoScoreImpacter changeHardScoreBy(
    ...
    return impactWithConstraintMatch(
        ...
        BendableBigDecimalScore.ofHard(
            hardScoreLevelCount, softScoreLevelCount, scoreLevel, hardImpact),
        constraintMatchSupplier);
  }
  ...
  public UndoScoreImpacter changeScoreBy(
    ...
    UndoScoreImpacter undoScoreImpact =
        () -> {
          for (int hardScoreLevel = 0; hardScoreLevel < hardScoreLevelCount; hardScoreLevel++) {
            parent.hardScores[hardScoreLevel] =
                parent.hardScores[hardScoreLevel].subtract(hardImpacts[hardScoreLevel]);
          }
          for (int softScoreLevel = 0; softScoreLevel < softScoreLevelCount; softScoreLevel++) {
            parent.softScores[softScoreLevel] =
                parent.softScores[softScoreLevel].subtract(softImpacts[softScoreLevel]);
          }
        };
    if (!constraintMatchPolicy.isEnabled()) {
      return undoScoreImpact;
