...
public final class ConcatQuadTriNode<A, B, C, D>
  ...
  protected QuadTuple<A, B, C, D> getOutTupleFromLeft(QuadTuple<A, B, C, D> leftTuple) {
    return new QuadTuple<>(
        leftTuple.factA, leftTuple.factB, leftTuple.factC, leftTuple.factD, outputStoreSize);
  }
  ...
  protected QuadTuple<A, B, C, D> getOutTupleFromRight(TriTuple<A, B, C> rightTuple) {
    ...
    return new QuadTuple<>(
        factA, factB, factC, paddingFunction.apply(factA, factB, factC), outputStoreSize);
  }

  @Override
  protected void updateOutTupleFromLeft(
      QuadTuple<A, B, C, D> leftTuple, QuadTuple<A, B, C, D> outTuple) {
  ...
  protected void updateOutTupleFromRight(
      TriTuple<A, B, C> rightTuple, QuadTuple<A, B, C, D> outTuple) {
