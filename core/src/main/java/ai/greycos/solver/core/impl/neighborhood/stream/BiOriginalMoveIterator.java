...
/**
 * Original iterators build moves based on all applicable pairs of A and B instances, in the order
 * in which they appear. This is a simple behavior that just enumerates all possible combinations.
 * First an instance of A is fixed, and then all instances of B are iterated. Then the next instance
 * of A is fixed, and all instances of B are iterated again. This continues until all instances of A
 * have been fixed.
 */
@NullMarked
  ...
  public boolean hasNext() {
    ...
    while (true) {
      if (rightTupleIterator == null || !rightTupleIterator.hasNext()) {
        if (leftTupleIterator
            .hasNext()) { // The second iterator is exhausted or the first one was not yet created.
          leftTuple = leftTupleIterator.next();
          var filter = rightDatasetInstance.getFilter();
          rightTupleIterator =
              rightDatasetInstance.iterator(rightDatasetInstance.produceCompositeKey(leftTuple));
          if (filter != null) {
            rightTupleIterator =
                new FilteringIterator<>(
                    filter,
                    context.neighborhoodSession().getSolutionView(),
                    leftTuple,
                    rightTupleIterator);
          }
