...
/**
 * A list that is composed of multiple sublists. If an element from any particular sublist is never
 * {@link #get(int) accessed}, it need not be stored in memory.
 *
 * <p>Note that this implementation does not support modifications of the list, or any of its
 * sublists.
 *
...
final class ComposingList<E> extends AbstractList<E> {
  ...
  /**
   * Instead of copying all elements into a single list, we keep a map of sublists. This is more
   * memory-efficient and faster to initialize, especially when some sublists are large. Key is the
   * starting index of the sublist in the overall list.
   */
   ...
   * @param listSize the size of the sublist, if we can determine it beforehand; otherwise create
   *     the list and use {@link #addSubList(List)}
   */
