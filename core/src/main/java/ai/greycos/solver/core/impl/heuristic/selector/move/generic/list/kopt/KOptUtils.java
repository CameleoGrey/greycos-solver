...
final class KOptUtils {
   ...
   * <br>
   * Any permutation can be expressed as combination of k-cycles. A k-cycle is a sequence of unique
   * elements (p_1, p_2, ..., p_k) where
   *
   * <ul>
   *   <li>p_1 maps to p_2 in the permutation
   *   <li>p_2 maps to p_3 in the permutation
   *   <li>p_(k-1) maps to p_k in the permutation
   *   <li>p_k maps to p_1 in the permutation
   *   <li>In general: p_i maps to p_(i+1) in the permutation
   * </ul>
   *
   ...
   *
   * @return The {@link KOptCycle} corresponding to the permutation described by {@link
   *     KOptDescriptor#removedEdgeIndexToTourOrder()}.
  ...
  static <Node_> List<Pair<Node_, Node_>> getAddedEdgeList(KOptDescriptor<Node_> kOptDescriptor) {
    ...
    while (currentEndpoint != 2 * k + 1) {
      out.add(
          new Pair<>(
              removedEdges[currentEndpoint],
              removedEdges[addedEdgeToOtherEndpoint[currentEndpoint]]));
