...
import ai.greycos.solver.core.impl.util.ListBasedScalingOrderedSet;

import org.jspecify.annotations.NullMarked;

/**
 * Generic notifiable that receives and triggers {@link Notification}s for a specific variable
 * listener of the type {@code T}.
 *
...
abstract class AbstractNotifiable<
        ...
        Listener_ extends InnerVariableListener<Solution_, ChangeEvent_>>
  ...
  static <
    ...
    if (variableListener instanceof InnerBasicVariableListener<?, ?> basicVariableListener) {
      return new VariableListenerNotifiable<>(
          ...
          variableListener.requiresUniqueEntityEvents()
              ? new ListBasedScalingOrderedSet<>()
              : new ArrayDeque<>(),
          globalOrder);
    } else if (variableListener
        instanceof InnerListVariableListener<?, ?, ?> listVariableListener) {
      ...
      throw new IllegalArgumentException(
          "Impossible state: InnerVariableListener (%s) must be an instance of %s or %s."
              .formatted(
                  variableListener.getClass().getCanonicalName(),
                  InnerBasicVariableListener.class.getSimpleName(),
