...
public abstract class AbstractIndexedIfExistsNode<LeftTuple_ extends AbstractTuple, Right_>
  ...
  private void updateCounterRight(
    ...
    if (!isFiltering) {
      ...
      indexerRight.forEach(
          compositeKey, rightTuple -> updateCounterFromLeft(counter, rightTuple, leftTrackerList));
  ...
  public final void updateLeft(LeftTuple_ leftTuple) {
    ...
    if (oldCompositeKey.equals(newCompositeKey)) {
      ...
      updateCounterRight(
          leftTuple, newCompositeKey, counter, indexerLeft.put(newCompositeKey, counter));
      updateCounterLeft(counter);
    }
  }
  ...
  private void updateCounterLeft(UniTuple<Right_> rightTuple, Object compositeKey) {
    if (!isFiltering) {
      ...
      indexerLeft.forEach(
          compositeKey, counter -> updateCounterFromRight(counter, rightTuple, rightTrackerList));
