...
import ai.greycos.solver.core.impl.bavet.common.AbstractTwoInputNode;
...
public abstract class AbstractBavetNodeNetwork {
  ...
  protected static AbstractNode[][] buildLayeredNodes(List<AbstractNode> nodeList) {
    ...
    nodeList.forEach(
        node ->
            layerMap.computeIfAbsent(node.getLayerIndex(), unused -> new ArrayList<>()).add(node));
  ...
  /**
   * A subset of {@code layeredNodes}. Once non-null, only contains propagators of nodes which are
   ...
   */
  protected AbstractBavetNodeNetwork(
   ...
   *     otherwise returns all root nodes. This means that if this information was ever read before
   *     activation checks were complete, it should be re-read after to make sure no inactive nodes
   *     are included.
   */
  ...
  public void settle() {
    if (layeredActivePropagators == null) {
      ...
      declaredClassToNodeMap.forEach(
          (declaredClass, rootNodes) ->
              rootNodes.forEach(
                  rootNode -> {
                  ...
                  layer ->
                      Arrays.stream(layer)
                          .filter(
                              s ->
                                  switch (s) {
                                    case ActivitySupport activityEnabled ->
                                        activityEnabled.isActive();
                                    case AbstractTwoInputNode<?, ?> twoInputNode ->
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof AbstractBavetNodeNetwork that)) return false;
