...
/**
 * Caches value ranges for the current working solution, allowing to quickly access these cached
 * value ranges when needed.
 *
 * <p>Outside a {@link ProblemChange}, value ranges are not allowed to change. Call {@link
 * #reset(Object)} every time the working solution changes through a problem fact, so that all
 * caches can be invalidated.
 *
 * <p>Two score directors can never share the same instance of this class; this class contains state
 * that is specific to a particular instance of a working solution. Even a clone of that same
 * solution must not share the same instance of this class, unless {@link #reset(Object)} is called
...
public final class ValueRangeManager<Solution_> {
  ...
  public static <Solution_> ValueRangeManager<Solution_> of(
  ...
  /**
   * It is not recommended for code other than {@link ScoreDirector} to create instances of this
   * class. See class-level documentation for more details. For safety, prefer using {@link
   * #of(SolutionDescriptor, Object)} to create an instance of this class with a solution already
   * set.
   */
  ...
  public SolutionInitializationStatistics computeInitializationStatistics(
      Solution_ solution, @Nullable Consumer<Object> finisher) {
  ...
  private <Entity_, Value_> ValueRangeState<Solution_, Entity_, Value_> fromDescriptor(
      ValueRangeDescriptor<Solution_> descriptor) {
    if (multipleValueRangeState == null) {
      ...
      if (singleValueRangeState == null) {
        singleValueRangeState =
            new ValueRangeState<>(descriptor, Objects.requireNonNull(cachedWorkingSolution));
      }
  ...
  public <Entity_> long countOnEntity(
      ValueRangeDescriptor<Solution_> valueRangeDescriptor, Entity_ entity) {
  ...
  public <Entity_, Value_> ReachableValues<Entity_, Value_> getReachableValues(
      GenuineVariableDescriptor<Solution_> variableDescriptor,
      @Nullable SelectionSorter<Solution_, Value_> sorter) {
    ValueRangeState<Solution_, Entity_, Value_> descriptor =
        fromDescriptor(variableDescriptor.getValueRangeDescriptor());
