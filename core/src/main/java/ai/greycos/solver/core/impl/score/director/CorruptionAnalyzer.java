...
final class CorruptionAnalyzer<Solution_, Score_ extends Score<Score_>> {
  ...
  public String analyzeScore(
    ...
    if (!scoreDirector.getConstraintMatchPolicy().isEnabled()
      ...
      """
          .formatted(
              scoreDirector.getConstraintMatchPolicy(),
    ...
    uncorruptedMap.forEach(
        (key, uncorruptedMatches) -> {
          ...
          updateExcessAndMissingConstraintMatches(
              uncorruptedMatches, corruptedMatches, excessSet, missingSet);
        });

    corruptedMap.forEach(
        (key, corruptedMatches) -> {
  ...
  private static <Score_ extends Score<Score_>>
      Map<Object, Set<ConstraintMatch<Score_>>> createConstraintMatchMap(
          Collection<ConstraintMatchTotal<Score_>> constraintMatchTotals) {
    var constraintMatchMap =
        LinkedHashMap.<Object, Set<ConstraintMatch<Score_>>>newLinkedHashMap(
            constraintMatchTotals.size() * 16);
    for (var constraintMatchTotal : constraintMatchTotals) {
      ...
      for (var constraintMatch : constraintMatchTotal.getConstraintMatchSet()) {
        ...
        // Constraint Streams with indistinct tuples may produce two different match instances for
        // the same key.
        var key = keyStream.add(constraintMatch.getScore()).build().collect(Collectors.toList());
        var added =
            constraintMatchMap
                .computeIfAbsent(key, k -> new LinkedHashSet<>(0))
                .add(constraintMatch);
        if (!added) {
  ...
  private static <Score_ extends Score<Score_>> void updateExcessAndMissingConstraintMatches(
      Set<ConstraintMatch<Score_>> uncorruptedSet,
      Set<ConstraintMatch<Score_>> corruptedSet,
      Set<ConstraintMatch<Score_>> excessSet,
  ...
  private static <Score_ extends Score<Score_>> void appendAnalysis(
      StringBuilder analysis,
      String workingLabel,
      String suffix,
      Set<ConstraintMatch<Score_>> matches) {
    if (matches.isEmpty()) {
      analysis.append(
          """
