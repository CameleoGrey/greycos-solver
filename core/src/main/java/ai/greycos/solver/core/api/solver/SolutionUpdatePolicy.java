...
/**
 * To fully de-normalize a planning solution freshly loaded from persistent storage, two operations
 * need to happen:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Each of these operations has its own performance cost, and for certain use cases, only one of
 * them may be actually necessary. Advanced users therefore get a choice of which to perform.
 *
 * <p>If unsure, pick {@link #UPDATE_ALL}.
 */
public enum SolutionUpdatePolicy {
   ...
   * Does not update shadow variables, making the user responsible for ensuring that all shadow
   * variables are mutually consistent. Otherwise the results of the computation are undefined, and
   * may range from the wrong score being computed to runtime exceptions being thrown. To avoid this
   * issue, use {@link #UPDATE_ALL} instead, which will update shadow variables to their correct
   * values first.
   */
