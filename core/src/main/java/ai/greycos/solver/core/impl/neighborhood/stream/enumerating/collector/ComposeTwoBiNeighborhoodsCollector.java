...
import ai.greycos.solver.core.preview.api.neighborhood.stream.enumerating.collector.BiNeighborhoodsCollectorAccumulator;
...
@NullMarked
public final class ComposeTwoBiNeighborhoodsCollector<
        Solution_, A, B, ResultHolder1_, ResultHolder2_, Result1_, Result2_, Result_>
    implements BiNeighborhoodsCollector<
        Solution_, A, B, Pair<ResultHolder1_, ResultHolder2_>, Result_> {
  ...
  private final BiFunction<@Nullable Result1_, @Nullable Result2_, @Nullable Result_>
      composeFunction;

  private final Supplier<ResultHolder1_> firstSupplier;
  private final Supplier<ResultHolder2_> secondSupplier;
  private final BiNeighborhoodsCollectorAccumulator<Solution_, A, B, ResultHolder1_>
      firstAccumulator;
  private final BiNeighborhoodsCollectorAccumulator<Solution_, A, B, ResultHolder2_>
      secondAccumulator;
  ...
  @Override
  public BiNeighborhoodsCollectorAccumulator<Solution_, A, B, Pair<ResultHolder1_, ResultHolder2_>>
      accumulator() {
  ...
  public Function<Pair<ResultHolder1_, ResultHolder2_>, @Nullable Result_> finisher() {
    return pair ->
        composeFunction.apply(firstFinisher.apply(pair.key()), secondFinisher.apply(pair.value()));
  }
