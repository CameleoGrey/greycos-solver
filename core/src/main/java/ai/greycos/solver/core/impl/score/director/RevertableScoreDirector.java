...
public interface RevertableScoreDirector<Solution_>
  ...
  /**
   * Use this method to get a representation of the operation that will be performed by {@link
   * #undoChanges()}. This operation will keep accumulating uncommitted changes until {@link
   * #undoChanges()} is actually called. After that happens, any subsequent call to this method will
  ...
  /**
   * Use this method to revert all changes made by moves. The score director that implements this
   * logic must be able to track every single change in the solution and restore it to its original
   * state.
   */
  void undoChanges();
}
