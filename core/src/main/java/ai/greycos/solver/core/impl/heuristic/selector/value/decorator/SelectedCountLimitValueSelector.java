...
public final class SelectedCountLimitValueSelector<Solution_>
  ...
  /**
   * Unlike most of the other {@link ValueSelector} decorations, this one works for an entity
   * dependent {@link ValueSelector} too.
   *
   * @param childValueSelector never null, if any of the {@link IterableValueSelector} specific
   *     methods are going to be used, this parameter must also implement that interface
   * @param selectedCountLimit at least 0
   */
  public SelectedCountLimitValueSelector(
      ValueSelector<Solution_> childValueSelector, long selectedCountLimit) {
    ...
    if (selectedCountLimit < 0L) {
      throw new IllegalArgumentException(
          "The selector ("
              + this
              + ") has a negative selectedCountLimit ("
              + selectedCountLimit
              + ").");
    }
  ...
  public Iterator<Object> iterator() {
    return new SelectedCountLimitValueIterator(
        ((IterableValueSelector<Solution_>) childValueSelector).iterator());
  }
  ...
  public boolean equals(Object other) {
    ...
    return selectedCountLimit == that.selectedCountLimit
