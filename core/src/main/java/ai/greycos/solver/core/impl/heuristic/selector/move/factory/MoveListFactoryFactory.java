  ...
  public MoveSelector<Solution_> buildBaseMoveSelector(
    ...
    if (moveListFactoryClass == null) {
      throw new IllegalArgumentException(
          "The moveListFactoryConfig (%s) lacks a moveListFactoryClass (%s)."
    ...
    ConfigUtils.applyCustomProperties(
        ...
        config.getMoveListFactoryCustomProperties(),
    ...
    if (minimumCacheType.compareTo(SelectionCacheType.STEP) < 0) {
      // cacheType upgrades to SelectionCacheType.STEP (without shuffling) because JIT is not
      // supported
      minimumCacheType = SelectionCacheType.STEP;
    }
    return new MoveListFactoryToMoveSelectorBridge<>(
        moveListFactory, minimumCacheType, randomSelection);
  }
