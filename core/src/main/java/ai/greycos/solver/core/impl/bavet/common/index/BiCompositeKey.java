...
record BiCompositeKey<A, B>(A propertyA, B propertyB) implements CompositeKey {
  ...
  public <Key_> Key_ get(int id) {
    return (Key_)
        switch (id) {
          ...
          case 1 -> propertyB;
          default ->
              throw new IllegalArgumentException("Impossible state: index (%d) > 1".formatted(id));
        };
  }
  ...
  public boolean equals(Object o) {
    ...
    return o instanceof BiCompositeKey<?, ?> that
        && ((propertyA == that.propertyA)
            || (propertyA != null && propertyA.equals(that.propertyA)))
        && ((propertyB == that.propertyB)
            || (propertyB != null && propertyB.equals(that.propertyB)));
  }
