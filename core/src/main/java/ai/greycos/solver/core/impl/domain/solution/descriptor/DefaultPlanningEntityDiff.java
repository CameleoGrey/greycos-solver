...
@NullMarked
record DefaultPlanningEntityDiff<Solution_, Entity_>(
    ...
    implements PlanningEntityDiff<Solution_, Entity_> {
  ...
  DefaultPlanningEntityDiff(PlanningSolutionDiff<Solution_> solutionDiff, Entity_ entity) {
    this(
        ...
        (PlanningEntityMetaModel<Solution_, Entity_>)
            solutionDiff.solutionMetaModel().entity(entity.getClass()));
  }

  DefaultPlanningEntityDiff(
      PlanningSolutionDiff<Solution_> solutionDiff,
      Entity_ entity,
      PlanningEntityMetaModel<Solution_, Entity_> entityMetaModel) {
    this(
        ...
        entity,
        entityMetaModel,
        new LinkedHashMap<>(entityMetaModel.variables().size()));
  }
    ...
    if (variableDiffMap.size() != 1) {
      throw new IllegalStateException(
          """
                    ...
                    Use variableDiff(String) instead."""
              .formatted(variableDiffMap.keySet(), entity.getClass()));
    }
  ...
  @SuppressWarnings("unchecked")
  @Override
  public @Nullable <Value_> PlanningVariableDiff<Solution_, Entity_, Value_> variableDiff(
      String variableRef) {
  ...
  private static String entityDiffToString(PlanningEntityDiff<?, ?> entityDiff) {
    ...
    if (variableDiffs.size() == 1) {
      ...
      return "  %s: %s -> %s"
          .formatted(diff.variableMetaModel().name(), diff.oldValue(), diff.newValue());
    }
