...
public final class PiggybackShadowVariableDescriptor<Solution_>
  ...
  private void linkShadowSources(DescriptorPolicy descriptorPolicy) {
    PiggybackShadowVariable piggybackShadowVariable =
        variableMemberAccessor.getAnnotation(PiggybackShadowVariable.class);
    ...
    if (shadowEntityClass.equals(PiggybackShadowVariable.NullEntityClass.class)) {
      ...
      shadowEntityDescriptor =
          entityDescriptor.getSolutionDescriptor().findEntityDescriptor(shadowEntityClass);
      if (shadowEntityDescriptor == null) {
        throw new IllegalArgumentException(
            "The entityClass ("
                + entityDescriptor.getEntityClass()
                + ") has a @"
                + PiggybackShadowVariable.class.getSimpleName()
                + " annotated property ("
                + variableMemberAccessor.getName()
                + ") with a shadowEntityClass ("
                + shadowEntityClass
    ...
    if (uncastShadowVariableDescriptor == null) {
      throw new IllegalArgumentException(
          "The entityClass ("
              ...
              + " annotated property ("
              + variableMemberAccessor.getName()
              + ") with shadowVariableName ("
              + shadowVariableName
              + ") which is not a valid planning variable on entityClass ("
              + shadowEntityDescriptor.getEntityClass()
              + ").\n"
              + shadowEntityDescriptor.buildInvalidVariableNameExceptionMessage(
                  shadowVariableName));
    }
    ...
    shadowVariableDescriptor =
        (CustomShadowVariableDescriptor<Solution_>) uncastShadowVariableDescriptor;
