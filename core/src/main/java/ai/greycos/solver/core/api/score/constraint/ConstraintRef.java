 ...
 * ConstraintRef#of(String, String)} and not the record's constructors.
 *
 * @param packageName The constraint package is the namespace of the constraint. When using a {@link
 *     ConstraintConfiguration}, it is equal to the {@link ConstraintWeight#constraintPackage()}. It
 *     is not recommended for the user to set this, or to read its value; instead, the user should
 *     use whatever the solver provided as default and not rely on this information at all. The
 *     entire concept of constraint package is likely to be removed in a future version of the
 *     solver.
 * @param constraintName The constraint name. It might not be unique, but {@link #constraintId()} is
 *     unique. When using a {@link ConstraintConfiguration}, it is equal to the {@link
 *     ConstraintWeight#value()}.
 * @param constraintId Always derived from {@code packageName} and {@code constraintName}.
 */
public record ConstraintRef(
    @NonNull String packageName, @NonNull String constraintName, String constraintId)
  ...
  public ConstraintRef {
    ...
    if (constraintId != null && !constraintId.equals(expectedConstraintId)) {
      throw new IllegalArgumentException(
          "Specifying custom constraintId (%s) is not allowed.".formatted(constraintId));
    }
  ...
  private static String validate(String identifier, String type) {
    ...
    if (sanitized.isEmpty()) {
      ...
      throw new IllegalArgumentException(
          "The %s (%s) cannot contain a package separator (%s)."
