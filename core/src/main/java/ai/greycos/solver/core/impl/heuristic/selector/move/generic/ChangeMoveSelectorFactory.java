...
public class ChangeMoveSelectorFactory<Solution_>
        ...
        if (onlyEntityDescriptor != null) {
          if (onlyVariableDescriptor.isListVariable()) {
            return buildListChangeMoveSelectorConfig(
                (ListVariableDescriptor<?>) onlyVariableDescriptor, true);
          }
    ...
    for (var variableDescriptor : variableDescriptorList) {
      if (variableDescriptor.isListVariable()) {
        if (configPolicy.getSolutionDescriptor().hasBothBasicAndListVariables()) {
          ...
          // The strategy aims to provide a more normalized move selector collection for mixed
          // models.
          continue;
        }
        // No childMoveSelectorConfig.inherit() because of
        // unfoldedMoveSelectorConfig.inheritFolded()
        var childMoveSelectorConfig =
            buildListChangeMoveSelectorConfig(
                (ListVariableDescriptor<?>) variableDescriptor, false);
        ...
        if (childEntitySelectorConfig.getMimicSelectorRef() == null) {
          childEntitySelectorConfig.setEntityClass(
  ...
  private ListChangeMoveSelectorConfig buildListChangeMoveSelectorConfig(
      ListVariableDescriptor<?> variableDescriptor, boolean inheritFoldedConfig) {
    LOGGER.warn(
        """
        ...
        config,
        ListChangeMoveSelectorConfig.class.getSimpleName());
  ...
  private DestinationSelectorConfig createDestinationSelectorConfig() {
    ...
    if (entitySelectorConfig == null) {
      if (valueSelectorConfig == null) {
        ...
        return new DestinationSelectorConfig().withValueSelectorConfig(valueSelectorConfig);
      }
    } else {
      if (valueSelectorConfig == null) {
        return new DestinationSelectorConfig().withEntitySelectorConfig(entitySelectorConfig);
      } else {
        return new DestinationSelectorConfig()
