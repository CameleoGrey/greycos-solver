...
final class BestSolutionHolder<Solution_> {
  ...
  /**
   * @return the last best solution together with problem changes the solution contains. If there is
   *     no new best solution, returns null.
   */
  @Nullable BestSolutionContainingProblemChanges<Solution_> take() {
    ...
    if (latestProcessedVersion.compareTo(bestSolutionVersion) > 0) {
      ...
      // This causes the later take() to return the latest best solution and all the problem
      // changes,
      // and the earlier best solution to be skipped entirely.
      return null;
    }
    // The map is replaced by a map containing only the problem changes that are not contained in
    // the best solution.
    ...
    // It is not practical to submit a problem change every second, as that gives the solver no time
    // to react.
  ...
  /**
   * Sets the new best solution if all known problem changes have been processed and thus are
   * contained in this best solution.
   *
   ...
   * @param isEveryProblemChangeProcessed a supplier that tells if all problem changes have been
   *     processed
   */
  void set(
  ...
  /**
   * Adds a batch of problem changes to a solver and registers them to be later retrieved together
   * with a relevant best solution by the {@link #take()} method.
   *
   * @return CompletableFuture that will be completed after the best solution containing this change
   *     is passed to a user-defined Consumer.
   */
  ...
  void cancelPendingChanges() {
    ...
    // Then we process the old map unsynchronized, which is safe because no one can access it
    // anymore.
