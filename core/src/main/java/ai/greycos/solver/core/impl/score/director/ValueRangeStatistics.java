...
import ai.greycos.solver.core.impl.domain.solution.descriptor.ProblemScaleTracker;
...
final class ValueRangeStatistics<Solution_> {
  ...
  SolutionInitializationStatistics computeInitializationStatistics(
      @Nullable Consumer<Object> finisher, boolean useCache) {
    ...
    var statistics =
        new SolutionInitializationStatistics(
            genuineEntityCount.intValue(),
            shadowEntityCount.intValue(),
            uninitializedEntityCount.intValue(),
            uninitializedVariableCount.intValue(),
            unassignedValueCount.intValue(),
  ...
  /**
   * Calculates an indication of the size of the problem instance. This is approximately the base 10
   * logarithm of the search space size.
   *
   * <p>The method uses a logarithmic scale to estimate the problem size, where the base of the
   * logarithm is determined by the maximum value range size. It accounts for both basic variables
   * and list variables in the solution, considering pinned values and value ranges on both entity
   * and solution.
   *
   * @return A non-negative double value representing the approximate base 10 logarithm of the
