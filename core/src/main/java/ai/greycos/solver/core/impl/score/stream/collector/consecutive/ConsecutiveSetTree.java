 ...
 * <i>x<sub>1</sub>,&nbsp;x<sub>2</sub>,&nbsp;x<sub>3</sub>,&nbsp;...,&nbsp;x<sub>n</sub></i> is
 * understood to be consecutive by <i>d</i> iff <i>x<sub>2</sub> &minus; x<sub>1</sub> &le; d,
 * x<sub>3</sub> &minus; x<sub>2</sub> &le; d, ..., x<sub>n</sub> &minus; x<sub>n-1</sub> &le;
 * d</i>. This data structure can be thought as an interval tree that maps the point <i>p</i> to the
 * interval <i>[p, p + d]</i>.
 *
 ...
 */
public final class ConsecutiveSetTree<
        Value_, Point_ extends Comparable<Point_>, Difference_ extends Comparable<Difference_>>
  ...
  public ConsecutiveSetTree(
      ...
      BinaryOperator<Difference_> sumFunction,
      Difference_ maxDifference,
      Difference_ zeroDifference) {
    this.differenceFunction = differenceFunction;
    this.sequenceLengthFunction =
        (first, last) -> sumFunction.apply(maxDifference, differenceFunction.apply(first, last));
    ...
    if (valueCount != null) { // Item already in bag.
      ...
      if (!Objects.equals(addingItem.index(), valueIndex)) {
        throw new IllegalStateException(
            "Impossible state: the item ("
                + value
                + ") is already in the bag with a different index ("
                + addingItem.index()
                + " vs "
                + valueIndex
                + ").\n"
  ...
  // Protected API
  private void removeItemFromBag(
      SequenceImpl<Value_, Point_, Difference_> bag,
      ComparableValue<Value_, Point_> item,
  ...
  NavigableMap<ComparableValue<Value_, Point_>, Value_> getComparableItems(
      ComparableValue<Value_, Point_> firstKey, ComparableValue<Value_, Point_> lastKey) {
