package ai.greycos.solver.core.impl.domain.solution;
...
final class ConstraintConfigurationDescriptor<Solution_> {
  ...
  public ConstraintConfigurationDescriptor(
  ...
  public void processAnnotations(
    ...
    for (var lineageClass :
        ConfigUtils.getAllAnnotatedLineageClasses(
            constraintConfigurationClass, ConstraintConfiguration.class)) {
      ...
      for (var member : memberList) {
        ...
        processParameterAnnotation(
            memberAccessorFactory, domainAccessType, member, scoreDefinition);
      }
    ...
    if (constraintWeightDescriptorMap.isEmpty()) {
      throw new IllegalStateException(
          "The constraintConfigurationClass ("
              + constraintConfigurationClass
              + ") must have at least 1 member with a "
              + ConstraintWeight.class.getSimpleName()
              + " annotation.");
    }
  }
  ...
  private void processPackAnnotation() {
    ...
    // If a @ConstraintConfiguration extends a @ConstraintConfiguration, their constraintPackage
    // might differ.
  ...
  private void processParameterAnnotation(
      MemberAccessorFactory memberAccessorFactory,
      DomainAccessType domainAccessType,
      Member member,
      ScoreDefinition<?> scoreDefinition) {
    if (((AnnotatedElement) member).isAnnotationPresent(ConstraintWeight.class)) {
      var memberAccessor =
          memberAccessorFactory.buildAndCacheMemberAccessor(
              member, FIELD_OR_READ_METHOD, ConstraintWeight.class, domainAccessType);
      if (constraintWeightDescriptorMap.containsKey(memberAccessor.getName())) {
        var duplicate =
            constraintWeightDescriptorMap.get(memberAccessor.getName()).getMemberAccessor();
        throw new IllegalStateException(
            "The constraintConfigurationClass ("
                + constraintConfigurationClass
                + ") has a @"
                ...
                + " annotated member ("
                + memberAccessor
                + ") that is duplicated by a member ("
                + duplicate
                + ").\n"
      ...
      if (!scoreDefinition.getScoreClass().isAssignableFrom(memberAccessor.getType())) {
        throw new IllegalStateException(
            "The constraintConfigurationClass ("
                ...
                + ") that is not assignable to the score class ("
                + scoreDefinition.getScoreClass()
                + ").\n"
                + "Maybe make that member ("
                + memberAccessor.getName()
                + ") return the score class ("
                + scoreDefinition.getScoreClass().getSimpleName()
                + ") instead.");
      }
    ...
    return constraintWeightDescriptorMap.values().stream()
        .filter(
            constraintWeightDescriptor ->
                constraintWeightDescriptor.getConstraintRef().equals(constraintRef))
        .findFirst()
        .orElse(null);
  }
