package ai.greycos.solver.core.config.heuristic.selector.entity.pillar;
...
public final class SubPillarConfigPolicy {
  ...
  private SubPillarConfigPolicy(
      int minimumSubPillarSize, int maximumSubPillarSize, Comparator<?> entityComparator) {
    this.subPillarEnabled = true;
    this.minimumSubPillarSize = minimumSubPillarSize;
    this.maximumSubPillarSize = maximumSubPillarSize;
    validateSizes();
  ...
  public static @NonNull SubPillarConfigPolicy sequential(
      int minSize, int maxSize, @NonNull Comparator<?> entityComparator) {
    return new SubPillarConfigPolicy(minSize, maxSize, entityComparator);
  }

  public static @NonNull SubPillarConfigPolicy sequentialUnlimited(
      @NonNull Comparator<?> entityComparator) {
  ...
  /**
   * @return Not null if the subpillars are to be treated as sequential. Always null if {@link
   *     #subPillarEnabled} is false.
   */
  ...
  public int hashCode() {
    return Objects.hash(
        subPillarEnabled, minimumSubPillarSize, maximumSubPillarSize, entityComparator);
  }
}
