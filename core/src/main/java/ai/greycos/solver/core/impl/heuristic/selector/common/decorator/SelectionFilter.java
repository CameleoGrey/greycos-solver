 ...
 * planning value, a {@link Move} or a {@link Selector}). For example, a pinned {@link
 * PlanningEntity} is rejected and therefore never used in a {@link Move}.
 *
 * <p>A filtered selection is considered as not selected, it does not count as an unaccepted
 * selection.
 *
 * <p>Implementations are expected to be stateless. The solver may choose to reuse instances.
 *
 * @param <Solution_> the solution type, the class with the {@link PlanningSolution} annotation
 * @param <T> the selection type. On problems using multiple planning variables on a single entity
 *     without specifying single variable name, this needs to be {@link Object} as variables of both
...
public interface SelectionFilter<Solution_, T> {
   ...
   */
  @SafeVarargs
  static <Solution_, T> SelectionFilter<Solution_, T> compose(
      SelectionFilter<Solution_, T>... filterArray) {
                    ...
                    filter -> {
                      if (filter == CompositeSelectionFilter.NOOP) {
                        return Stream.empty();
                          ...
                          instanceof
                          CompositeSelectionFilter<Solution_, T> compositeSelectionFilter) {
   ...
   *     {@link Selector}
   * @return true if the selection is accepted (for example it is movable), false if the selection
   *     will be discarded (for example it is pinned)
   */
