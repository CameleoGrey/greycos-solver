...
import ai.greycos.solver.core.impl.domain.valuerange.AbstractCountableValueRange;
import ai.greycos.solver.core.impl.domain.valuerange.util.ValueRangeIterator;
...
public final class TemporalValueRange<Temporal_ extends Temporal & Comparable<? super Temporal_>>
  ...
  /**
   * We could not use a {@link TemporalAmount} as {@code incrementUnit} due to lack of calculus
   * functions.
   */
   ...
   * @param incrementUnitType never null, must be {@link Temporal#isSupported(TemporalUnit)
   *     supported} by {@code from} and {@code to}
   */
  public TemporalValueRange(
    ...
    if (incrementUnitAmount <= 0) {
      throw new IllegalArgumentException(
          "The "
              + getClass().getSimpleName()
              + " must have strictly positive incrementUnitAmount ("
              + incrementUnitAmount
              + ").");
    }
    if (!from.isSupported(incrementUnitType) || !to.isSupported(incrementUnitType)) {
      throw new IllegalArgumentException(
          "The "
              + getClass().getSimpleName()
              + " must have an incrementUnitType ("
              + incrementUnitType
              + ") that is supported by its from ("
              + from
              + ") class ("
    ...
    if (from.compareTo(to) > 0) {
      throw new IllegalArgumentException(
          "The "
              ...
              + " cannot have a from ("
              + from
              + ") which is strictly higher than its to ("
              + to
              + ").");
    }
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "The "
                ...
                + "'s incrementUnitType ("
                + incrementUnitType
                + ") must fit an integer number of times in the space ("
                + space
                + ") between from ("
                + from
                + ") and to ("
  ...
  public boolean contains(Temporal_ value) {
    ...
    // Only checking the modulus is not enough: 1-MAR + 1 month doesn't include 7-MAR but the
    // modulus is 0 anyway
  ...
  private class OriginalTemporalValueRangeIterator extends ValueRangeIterator<Temporal_> {
    ...
    public Temporal_ next() {
      ...
      // Do not use upcoming += incrementUnitAmount because 31-JAN + 1 month + 1 month returns
      // 28-MAR
