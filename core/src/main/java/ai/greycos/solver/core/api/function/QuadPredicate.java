package ai.greycos.solver.core.api.function;
...
/**
 * Represents a predicate (boolean-valued function) of four arguments. This is the four-arity
...
public interface QuadPredicate<A, B, C, D> {
   ...
   */
  default @NonNull QuadPredicate<A, B, C, D> and(
      @NonNull QuadPredicate<? super A, ? super B, ? super C, ? super D> other) {
   ...
   * @throws NullPointerException if other is null
   */
  default @NonNull QuadPredicate<A, B, C, D> or(
      @NonNull QuadPredicate<? super A, ? super B, ? super C, ? super D> other) {
    Objects.requireNonNull(other);
    return (A a, B b, C c, D d) -> test(a, b, c, d) || other.test(a, b, c, d);
  }
