...
public class MutationCounter<Solution_> {
   ...
   * @param a never null
   * @param b never null
   * @return {@code >= 0}, the number of planning variables that have a different value in {@code a}
   *     and {@code b}.
   */
  public int countMutations(Solution_ a, Solution_ b) {
    ...
    for (EntityDescriptor<Solution_> entityDescriptor :
      ...
      for (Iterator<Object> aIt = aEntities.iterator(), bIt = bEntities.iterator();
      ...
      if (aEntities.size() != bEntities.size()) {
        mutationCount +=
            Math.abs(aEntities.size() - bEntities.size())
