...
public class RandomSubListSelector<Solution_> extends AbstractSelector<Solution_>
  ...
  public RandomSubListSelector(
    ...
    if (minimumSubListSize > maximumSubListSize) {
      throw new IllegalArgumentException(
          "The minimumSubListSize ("
              + minimumSubListSize
              + ") must be less than or equal to the maximumSubListSize ("
              + maximumSubListSize
              + ").");
    }
  ...
  public Iterator<SubList> iterator() {
    ...
    for (Object entity : ((Iterable<Object>) entitySelector::endingIterator)) {
      biggestListSize =
          Math.max(biggestListSize, listVariableDescriptor.getUnpinnedSubListSize(entity));
    }
  ...
  private final class RandomSubListIterator extends UpcomingSelectionIterator<SubList> {
    ...
    protected SubList createUpcomingSelection() {
      ...
      while (listSize < minimumSubListSize) {
        if (!valueIterator.hasNext()) {
          throw new IllegalStateException(
              "The valueIterator (" + valueIterator + ") should never end.");
        }
      ...
      TriangleElementFactory.TriangleElement triangleElement =
          triangleElementFactory.nextElement(listSize);
      int subListLength = listSize - triangleElement.level() + 1;
      if (subListLength < 1) {
        throw new IllegalStateException(
            "Impossible state: The subListLength (%s) must be greater than 0."
