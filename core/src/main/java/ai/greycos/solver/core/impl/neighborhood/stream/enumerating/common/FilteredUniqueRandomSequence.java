...
/**
 * Unlike {@link DefaultUniqueRandomSequence}, this class only returns elements that match the given
 * filter. Because we can't predict how many elements will be filtered out, and because we don't
 * want to pre-filter the entire list, this class may need to try multiple times to find a matching
 * element. It also can't provide an emptiness check and has to rely on {@link
 * NoSuchElementException} to be thrown when everything's been finally filtered out. Other than
 * that, it relies on {@link DefaultUniqueRandomSequence} to keep track of removed elements.
 *
 * @param <T>
 */
@NullMarked
public final class FilteredUniqueRandomSequence<T> implements UniqueRandomSequence<T> {
  ...
  FilteredUniqueRandomSequence(
      List<? extends ListEntry<T>> listOfUniqueItems, Predicate<T> filter) {
   ...
   *
   * @return true if the underlying sequence has no more elements; otherwise true, even if all of
   *     those elements may later be filtered out.
   */
  @Override
