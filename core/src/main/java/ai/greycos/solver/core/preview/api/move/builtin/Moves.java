 ...
 *
 * <p>This class provides static methods to create the standard moves used in optimization:
 *
 * <ul>
 *   <li>{@link #compose(Move[])} - combines multiple moves into one
 *   <li>{@link #change(PlanningVariableMetaModel, Object, Object)} - changes a basic planning
 *       variable's value
 *   <li>{@link #swap(PlanningVariableMetaModel, Object, Object)} - swaps values between two
 *       entities
 *   <li>{@link #assign(PlanningListVariableMetaModel, Object, Object, int)} - assigns a value to a
 *       list variable
 *   <li>{@link #unassign(PlanningListVariableMetaModel, Object, int)} - removes a value from a list
 *       variable
 *   <li>{@link #change(PlanningListVariableMetaModel, PositionInList, PositionInList)} - moves an
 *       element within or between list variables
 *   <li>{@link #swap(PlanningListVariableMetaModel, PositionInList, PositionInList)} - swaps two
 *       elements in list variables
 * </ul>
 *
...
public final class Moves {
   ...
   *
   * <p>When executed, the composite move executes all its child moves in order. If the list
   * contains only one move, that move is returned directly without wrapping.
   *
   * @param moves the list of moves to combine; must not be empty
   * @param <Solution_> the solution type
   * @return a single move that executes all the given moves, or the single move if the list
   *     contains only one
   ...
   *
   * <p>When executed, the composite move executes all its child moves in order. If the array
   * contains only one move, that move is returned directly without wrapping.
   *
   * @param moves the array of moves to combine; must not be empty
   * @param <Solution_> the solution type
   * @return a single move that executes all the given moves, or the single move if the array
   *     contains only one
   ...
   *
   * <p>This move is the fundamental building block for optimizing basic planning variables. It sets
   * the variable on the entity to a new value.
   *
   ...
   *
   * <p>Both entities must be different instances. After execution, the left entity will have the
   * value that the right entity had, and vice versa.
   *
   * <p>Only provide entities whose values can be swapped; for example, if one of the values is not
   * in the value range of the other entity's variable, then swapping would lead to an invalid
   * solution.
   *
   ...
   *
   * <p>Both entities must be different instances. For each variable in the list, after execution,
   ...
   *
   * <p>The value must not already be assigned to any list variable. This move inserts the value at
   * the given position, shifting all existing values at or after that position to the right.
   *
   ...
   * @param <Entity_> the entity type
   * @param <Value_> the variable value type
   * @return a move that, when executed, assigns the value to the list variable at the specified
   *     position
   */
  ...
  /**
   * As defined by {@link #assign(PlanningListVariableMetaModel, Object, PositionInList)}, but with
   * explicit entity and index parameters.
   */
  public static <Solution_, Entity_, Value_> Move<Solution_> assign(
      PlanningListVariableMetaModel<Solution_, Entity_, Value_> variableMetaModel,
      Value_ value,
   ...
   *
   * <p>This move removes the value at the given position, shifting all subsequent values to the
   * left. After execution, the removed value will be unassigned.
   *
   * @param variableMetaModel describes the list variable to be changed
  ...
  /**
   * As defined by {@link #unassign(PlanningListVariableMetaModel, PositionInList)}, but with
   * explicit entity and index parameters.
   */
  public static <Solution_, Entity_, Value_> Move<Solution_> unassign(
      PlanningListVariableMetaModel<Solution_, Entity_, Value_> variableMetaModel,
   ...
   *
   * <p>The element at the source position is removed and inserted at the destination position. Both
   * positions may be in the same entity or in different entities.
   *
   * <p>If the source and destination are within the same entity, the element is first removed from
   * the source position (shifting later elements left), then inserted at the destination position.
   *
   * @param variableMetaModel describes the list variable to be changed
   ...
   * @param <Entity_> the entity type
   * @param <Value_> the variable value type
   * @return a move that, when executed, relocates the element from the source position to the
  ...
  public static <Solution_, Entity_, Value_> Move<Solution_> change(
    ...
    return new ListChangeMove<>(
        variableMetaModel, sourceEntity, sourceIndex, destinationEntity, destinationIndex);
  }

  /**
   * Creates a move that swaps two elements between positions in list variables.
   *
   * <p>The element at the left position is swapped with the element at the right position. The left
   * and right positions may be in the same or different entities.
   *
   * @param variableMetaModel describes the list variable to be changed
  ...
  /**
   * As defined by {@link #swap(PlanningListVariableMetaModel, PositionInList, PositionInList)}, but
   * with explicit entity and index parameters.
   */
  public static <Solution_, Entity_, Value_> Move<Solution_> swap(
      PlanningListVariableMetaModel<Solution_, Entity_, Value_> variableMetaModel,
      Entity_ leftEntity,
