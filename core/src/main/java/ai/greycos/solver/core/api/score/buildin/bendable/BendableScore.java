...
/**
 * This {@link Score} is based on n levels of int constraints. The number of levels is bendable at
 * configuration time.
 *
...
public final class BendableScore implements IBendableScore<BendableScore> {
  ...
  public static BendableScore parseScore(String scoreString) {
    ...
    for (var i = 0; i < hardScores.length; i++) {
      hardScores[i] =
          ScoreUtil.parseLevelAsInt(BendableScore.class, scoreString, scoreTokens[0][i]);
    }
    var softScores = new int[scoreTokens[1].length];
    for (var i = 0; i < softScores.length; i++) {
      softScores[i] =
          ScoreUtil.parseLevelAsInt(BendableScore.class, scoreString, scoreTokens[1][i]);
    }
  ...
  public static BendableScore ofHard(
      int hardLevelsSize, int softLevelsSize, int hardLevel, int hardScore) {
  ...
  public static BendableScore ofSoft(
      int hardLevelsSize, int softLevelsSize, int softLevel, int softScore) {
  ...
  /**
   * @param hardLevel {@code 0 <= hardLevel <} {@link #hardLevelsSize()}. The {@code scoreLevel} is
   *     {@code hardLevel} for hard levels and {@code softLevel + hardLevelSize} for soft levels.
  ...
  /**
   * @param softLevel {@code 0 <= softLevel <} {@link #softLevelsSize()}. The {@code scoreLevel} is
   *     {@code hardLevel} for hard levels and {@code softLevel + hardLevelSize} for soft levels.
   * @return higher is better
   */
  ...
  @Override
  public BendableScore
      negate() { // Overridden as the default impl would create zero() all the time.
    var newHardScores = new int[hardScores.length];
    var newSoftScores = new int[softScores.length];
    for (var i = 0; i < newHardScores.length; i++) {
