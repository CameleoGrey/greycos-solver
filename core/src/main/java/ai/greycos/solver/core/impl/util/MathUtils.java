...
/**
 * Includes code taken from Apache's commons-math library, v3.6.1 (Apache 2.0-licensed) These
 * methods are clearly marked in comments, their modifications listed.
 */
public class MathUtils {

  public static final long LOG_PRECISION = 1_000_000L;
  private static final int FACTORIAL_MAX_N = 20;
  private static final long[] FACTORIAL_CACHE =
      new long[FACTORIAL_MAX_N - 1]; // 0 and 1 are hard-coded.
  ...
  public static long getPossibleArrangementsScaledApproximateLog(
      long scale, long base, int listSize, int partitions) {
    double result;
   ...
   * @return A value approximately equal to {@code scale * log_base(value)}, rounded to the nearest
   *     integer.
   */
  ...
  public static long factorial(int n) {
    if (n < 0) {
      throw new IllegalArgumentException("Factorial of %d is undefined.".formatted(n));
    } else if (n > FACTORIAL_MAX_N) {
      throw new IllegalArgumentException(
          "Factorial of %d is too large to fit in a long.".formatted(n));
    }
    return switch (n) {
      case 0, 1 -> 1;
      default -> {
   ...
   * n choose k}", the number of {@code k}-element subsets that can be selected from an {@code
   * n}-element set.
   *
   * <p><strong>Preconditions</strong>:
   *
   * <ul>
   *   <li>{@code 0 <= k <= n } (otherwise {@link IllegalArgumentException} is thrown)
   *   <li>The result is small enough to fit into a {@code long}. The largest value of {@code n} for
   *       which all coefficients are less than {@code Long.MAX_VALUE} is 66. If the computed value
   *       exceeds {@code Long.MAX_VALUE} a {@link ArithmeticException} is thrown.
   * </ul>
   *
   * Taken from `commons-math` 3.6.1 (Apache 2.0 licensed.) Refactored to not depend on any other
   * `commons-math` methods or types.
   *
  ...
  public static long binomialCoefficient(final int n, final int k) {
    ...
    if (n < 0) {
      throw new IllegalArgumentException(
          "must have n >= 0 for binomial coefficient (n, k), got n = %d".formatted(n));
    }
    if ((n == k) || (k == 0)) {
      return 1;
    }
  ...
  /**
   * Taken from `commons-math` 3.6.1 (Apache 2.0 licensed.) Refactored to not depend on any other
   * `commons-math` methods or types, replaced local variable types with inference.
   */
   ...
   * method which avoids division and modulo operations. See Knuth 4.5.2 algorithm B. The algorithm
   * is due to Josef Stein (1961). <br>
   * Special cases:
   *
   * <ul>
   *   <li>The result of {@code gcd(x, x)}, {@code gcd(0, x)} and {@code gcd(x, 0)} is the value of
   *       {@code x}.
   *   <li>The invocation {@code gcd(0, 0)} is the only one which returns {@code 0}.
   * </ul>
   *
   * <p>Taken from `commons-math` 3.6.1 (Apache 2.0 licensed.) Refactored to not depend on any other
   * `commons-math` methods or types, replaced local variable types with inference.
   *
  ...
  /**
   * Taken from `commons-math` 3.6.1 (Apache 2.0 licensed.) Refactored to not depend on any other
   * `commons-math` methods or types, replaced local variable types with inference and introduced
   * multiple return.
   */
  ...
  public static String approximateProblemScaleAsFormattedString(
      double approximateProblemSizeLog, Locale locale) {
    ...
    if (approximateProblemSizeLog < 10) { // log_10(10_000_000_000) = 10
      return "%s"
          .formatted(
              formatNumber(Math.pow(10d, approximateProblemSizeLog), BASIC_FORMATTER, locale));
    }
  ...
  /**
   * In order for tests to work currently regardless of the default system locale, we need to set
   * the locale to a known value before running the tests. And because the {@link DecimalFormat}
   * instances are initialized statically for reasons of performance, we cannot expect them to be in
   * the locale that the test expects them to be in. This method exists to allow for an override.
   *
  ...
  private static String formatNumber(double number, DecimalFormat decimalFormat, Locale locale) {
    ...
    try { // Slow path for corner cases where input locale doesn't match the default locale.
      ...
      decimalFormat.setDecimalFormatSymbols(
          DecimalFormatSymbols.getInstance(MathUtils.FORMATTER_LOCALE));
    }
  }
}
