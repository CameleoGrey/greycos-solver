package ai.greycos.solver.core.impl.localsearch.decider.acceptor.lateacceptance;
 ...
 *
 * <p>Caches the best-solution step index and the corresponding score level values. On each step,
 * {@link #update} refreshes the cache when the best solution has changed, and {@link
 * #isNonDominatedLevelChanged} compares the non-dominated levels (hard for {@link IBendableScore
 * IBendableScore}, hard and medium for all others) against the cached values to determine whether
 * the {@link LateAcceptanceScoreBuffer} should be reset.
 */
final class DefaultLevelScoreState<Solution_> implements LevelScoreState<Solution_> {
  ...
  public void update(LocalSearchStepScope<Solution_> stepScope) {
    ...
    if (previousBestScoreIndex != bestSolutionStepIndex) {
      ...
      this.previousBestScoreLevels =
          stepScope.getPhaseScope().getBestScore().raw().toLevelNumbers();
    }
  }

  /**
   * If non-dominated levels are updated (hard or medium), it is necessary to reset the late scores.
   * Failing to do so may cause the solver to accept poor moves that do not affect the non-dominated
   * scores but degrade the soft scores. As a result, any move that does not decrease the hard or
   * medium score but significantly worsens the soft score may be mistakenly accepted. This could
   * cause the working solution to enter a bad region and require many additional steps to escape
   * it.
   *
