...
public interface SolverJob<Solution_, ProblemId_> {
  ...
  /**
   * @return true if {@link SolverJob#terminateEarly} has been called since the underlying {@link
   *     Solver} started solving.
   */
  boolean isTerminatedEarly();

  /**
   * Waits if necessary for the solver to complete and then returns the final best {@link
   * PlanningSolution}.
   *
   ...
   * it returns {@link Duration#ZERO}. If it hasn't ended yet, it returns the time between the last
   * start and now. If it has ended already, it returns the time between the last start and the
   * ending.
   *
   ...
   * returns 0. If it hasn't ended yet, it returns the number of score calculations so far. If it
   * has ended already, it returns the total number of score calculations that occurred during
   * solving.
   *
   * @return the number of score calculations that had occurred during solving since the last
   *     (re)start, at least 0
   */
  long getScoreCalculationCount();

  /**
   * Return the number of move evaluations since the last start. If it hasn't started yet, it
   * returns 0. If it hasn't ended yet, it returns the number of moves evaluations so far. If it has
   * ended already, it returns the total number of move evaluations that occurred during solving.
   *
   * @return the number of move evaluations that had occurred during solving since the last
   *     (re)start, at least 0
   */
   ...
   * started yet, it returns 0. If it hasn't ended yet, it returns the average number of score
   * calculations per second so far. If it has ended already, it returns the average number of score
   * calculations per second during solving.
   *
  ...
  /**
   * Return the average number of move evaluations per second since the last start. If it hasn't
   * started yet, it returns 0. If it hasn't ended yet, it returns the average number of move
   * evaluations per second so far. If it has ended already, it returns the average number of move
   * evaluations per second during solving.
   *
