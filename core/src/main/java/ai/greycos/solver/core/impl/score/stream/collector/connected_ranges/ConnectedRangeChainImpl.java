package ai.greycos.solver.core.impl.score.stream.collector.connected_ranges;
...
import ai.greycos.solver.core.api.score.stream.common.ConnectedRange;
import ai.greycos.solver.core.api.score.stream.common.ConnectedRangeChain;
import ai.greycos.solver.core.api.score.stream.common.RangeGap;

import org.jspecify.annotations.NonNull;

public final class ConnectedRangeChainImpl<
        Range_, Point_ extends Comparable<Point_>, Difference_ extends Comparable<Difference_>>
    implements ConnectedRangeChain<Range_, Point_, Difference_> {

  private final NavigableMap<
          RangeSplitPoint<Range_, Point_>, ConnectedRangeImpl<Range_, Point_, Difference_>>
      startSplitPointToConnectedRange;
  private final NavigableSet<RangeSplitPoint<Range_, Point_>> splitPointSet;
  private final NavigableMap<
          RangeSplitPoint<Range_, Point_>, RangeGapImpl<Range_, Point_, Difference_>>
      startSplitPointToNextGap;
  private final BiFunction<? super Point_, ? super Point_, ? extends Difference_>
      differenceFunction;

  public ConnectedRangeChainImpl(
  ...
  void addRange(Range<Range_, Point_> range) {
    var intersectedConnectedRangeMap =
        startSplitPointToConnectedRange.subMap(
            Objects.requireNonNullElseGet(
                startSplitPointToConnectedRange.floorKey(range.getStartSplitPoint()),
                range::getStartSplitPoint),
            true,
            range.getEndSplitPoint(),
            true);

    // Case: the connected range before this range does not intersect this range
    if (!intersectedConnectedRangeMap.isEmpty()
        && intersectedConnectedRangeMap
            .firstEntry()
            .getValue()
            .getEndSplitPoint()
      ...
      intersectedConnectedRangeMap =
          intersectedConnectedRangeMap.subMap(
              intersectedConnectedRangeMap.firstKey(),
              false,
              intersectedConnectedRangeMap.lastKey(),
              true);
    }
    ...
    // connected range
    intersectedConnectedRangeMap
        .tailMap(oldStartSplitPoint, false)
        .values()
    ...
    if (oldStartSplitPoint.isAfter(firstIntersectedConnectedRange.getStartSplitPoint())) {
      ...
      startSplitPointToConnectedRange.put(
          firstIntersectedConnectedRange.getStartSplitPoint(), firstIntersectedConnectedRange);
      var nextGap =
          startSplitPointToNextGap.get(firstIntersectedConnectedRange.getStartSplitPoint());
      if (nextGap != null) {
        nextGap.setPreviousConnectedRange(firstIntersectedConnectedRange);
        nextGap.setLength(
            differenceFunction.apply(nextGap.getPreviousRangeEnd(), nextGap.getNextRangeStart()));
      }
    }
  }
  ...
  private void createNewConnectedRange(Range<Range_, Point_> range) {
    ...
    var newConnectedRange =
        ConnectedRangeImpl.getConnectedRangeStartingAt(
            splitPointSet, differenceFunction, startSplitPoint);
    ...
    if (nextConnectedRangeEntry != null) {
      ...
      var difference =
          differenceFunction.apply(newConnectedRange.getEnd(), nextConnectedRange.getStart());
    ...
    if (previousConnectedRangeEntry != null) {
      ...
      var difference =
          differenceFunction.apply(previousConnectedRange.getEnd(), newConnectedRange.getStart());
  ...
  private void removeSpannedGapsAndUpdateIntersectedGaps(
      Range<Range_, Point_> range, ConnectedRangeImpl<Range_, Point_, Difference_> connectedRange) {
    var firstGapSplitPointBeforeRange =
        Objects.requireNonNullElseGet(
            startSplitPointToNextGap.floorKey(range.getStartSplitPoint()),
            range::getStartSplitPoint);
    var intersectedRangeGapMap =
        startSplitPointToNextGap.subMap(
            firstGapSplitPointBeforeRange, true, range.getEndSplitPoint(), true);
    ...
    var connectedRangeBeforeFirstIntersectedGap =
        (ConnectedRangeImpl<Range_, Point_, Difference_>)
            (intersectedRangeGapMap.firstEntry().getValue().getPreviousConnectedRange());
    var connectedRangeAfterFinalIntersectedGap =
        (ConnectedRangeImpl<Range_, Point_, Difference_>)
            (intersectedRangeGapMap.lastEntry().getValue().getNextConnectedRange());
    ...
    if (!range
      ...
      if (!range
          .getEndSplitPoint()
          .isBefore(connectedRangeAfterFinalIntersectedGap.getStartSplitPoint())) {
        ...
        finalGap.setLength(
            differenceFunction.apply(finalGap.getPreviousRangeEnd(), finalGap.getNextRangeStart()));
        intersectedRangeGapMap.clear();
      ...
      intersectedRangeGapMap.clear();
      startSplitPointToNextGap.put(
          ((ConnectedRangeImpl<Range_, Point_, Difference_>)
                  (previousGap.getPreviousConnectedRange()))
              .getStartSplitPoint(),
          previousGap);
    } else {
  ...
  void removeRange(Range<Range_, Point_> range) {
    var connectedRangeEntry =
        startSplitPointToConnectedRange.floorEntry(range.getStartSplitPoint());
    ...
    var nextConnectedRangeEntry =
        startSplitPointToConnectedRange.higherEntry(connectedRangeEntry.getKey());
    ...
    while (iterator.hasNext()) {
      ...
      if (previousGap != null) {
        ...
        previousGap.setLength(
            differenceFunction.apply(
                previousGap.getPreviousConnectedRange().getEnd(), newConnectedRange.getStart()));
        startSplitPointToNextGap.put(
            ((ConnectedRangeImpl<Range_, Point_, Difference_>)
                    previousGap.getPreviousConnectedRange())
                .getStartSplitPoint(),
      ...
      startSplitPointToConnectedRange.put(
          newConnectedRange.getStartSplitPoint(), newConnectedRange);
    }
    ...
    if (nextConnectedRangeEntry != null && previousGap != null) {
      ...
      previousGap.setLength(
          differenceFunction.apply(
              previousConnectedRange.getEnd(), nextConnectedRangeEntry.getValue().getStart()));
      startSplitPointToNextGap.put(previousConnectedRange.getStartSplitPoint(), previousGap);
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof ConnectedRangeChainImpl<?, ?, ?> that)) return false;
    return Objects.equals(startSplitPointToConnectedRange, that.startSplitPointToConnectedRange)
        && Objects.equals(splitPointSet, that.splitPointSet)
        && Objects.equals(startSplitPointToNextGap, that.startSplitPointToNextGap);
  }
