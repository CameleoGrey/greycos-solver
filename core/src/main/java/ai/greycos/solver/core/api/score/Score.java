 ...
 *
 * <p>Implementations must be immutable, preferably a Java record or even a primitive record, if the
 * target JDK permits that.
 *
...
public interface Score<Score_ extends Score<Score_>> extends Comparable<Score_>, Serializable {
  ...
  /**
   * Returns an array of numbers representing the Score. Each number represents 1 score level. A
   * greater score level uses a lower array index than a lesser score level.
   *
   * <p>When rounding is needed, each rounding should be floored (as defined by {@link
   * Math#floor(double)}). The length of the returned array must be stable for a specific {@link
   * Score} implementation.
   *
   * <p>For example: {@code -0hard/-7soft} returns {@code new int{-0, -7}}
   */
  ...
  /**
   * Like {@link Object#toString()}, but trims score levels which have a zero weight. For example
   * {@literal 0hard/-258soft} returns {@literal -258soft}.
   *
   * <p>Do not use this format to persist information as text, use {@link Object#toString()}
