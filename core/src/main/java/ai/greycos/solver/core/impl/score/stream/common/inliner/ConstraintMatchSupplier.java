 ...
 * ConstraintMatch}, an entire {@link ConstraintJustification} object needs to be created, along
 * with the collection of indicted objects. Creating these structures every time a constraint is
 * matched would be wasteful, as the same constraint match can be undone almost immediately,
 * resulting in a lot of pointless garbage. Therefore, {@link ConstraintMatch} (along with all of
 * its supporting data structures) is only created when actually needed, and that is during score
...
public interface ConstraintMatchSupplier<Score_ extends Score<Score_>>
  ...
  static <Score_ extends Score<Score_>> ConstraintMatchSupplier<Score_> empty() {
    return (constraint, impact) ->
        new ConstraintMatch<>(constraint.getConstraintRef(), null, Collections.emptyList(), impact);
  }
  ...
  private static RuntimeException createIndictmentException(
      Constraint constraint, Exception cause, Object... facts) {
    throw new IllegalStateException(
        "Consequence of a constraint ("
            + constraint.getConstraintRef()
            + ") threw an exception collecting indicted objects from a tuple ("
            + factsToString(facts)
            + ").",
        cause);
  }
