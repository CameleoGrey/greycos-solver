package ai.greycos.solver.core.impl.score.trend;
...
/**
 * Bounds the possible {@link Score}s for a {@link PlanningSolution} as more and more variables are
 * initialized (while the already initialized variables don't change).
 *
 * @see InitializingScoreTrendLevel
 */
public record InitializingScoreTrend(InitializingScoreTrendLevel[] trendLevels) {

  public static InitializingScoreTrend parseTrend(
      String initializingScoreTrendString, int levelsSize) {
    ...
    if (!tokenIsSingle && trendTokens.length != levelsSize) {
      throw new IllegalArgumentException(
          """
              ...
              .formatted(
                  initializingScoreTrendString,
                  buildTrendPattern(levelsSize),
                  trendTokens.length,
                  levelsSize));
    }
  ...
  public static InitializingScoreTrend buildUniformTrend(
      InitializingScoreTrendLevel trendLevel, int levelsSize) {
    var trendLevels = new InitializingScoreTrendLevel[levelsSize];
  ...
  public boolean equals(Object o) {
    return o instanceof InitializingScoreTrend that && Arrays.equals(trendLevels, that.trendLevels);
  }
