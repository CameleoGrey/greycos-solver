 ...
 * Requires to specify a (sub)set of variables to swap values of, all of which must belong to the
 * same entity class.
 *
 * <p>Only provide entities whose values can be swapped; for example, if one of the values is not in
 * the value range of the other entity's variable, then swapping would lead to an invalid solution.
 * This move will skip that swap, but it is more efficient to not propose it in the first place.
 *
...
public class SwapMove<Solution_, Entity_> extends AbstractMove<Solution_> {
   ...
   * #rightEntity}. Example: with two variables v1 and v2, the list contains [left.v1, right.v1,
   * left.v2, right.v2].
   */
  ...
  protected SwapMove(
    ...
    if (leftEntity == rightEntity) {
      throw new IllegalArgumentException(
          "Swap move requires two different entities (%s).".formatted(leftEntity));
    }
  }
