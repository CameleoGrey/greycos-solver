...
/**
 * A 2-opt move for list variables, which takes two edges and swap their endpoints. For instance,
 * let [A, B, E, D, C, F, G, H] be the route assigned to an entity. Select (B, E) and (C, F) as the
 * edges to swap. Then the resulting route after this operation would be [A, B, C, D, E, F, G, H].
 * The edge (B, E) became (B, C), and the edge (C, F) became (E, F) (the first edge end point became
 * the second edge start point and vice versa). It is used to fix crossings; for instance, it can
 * change:
 *
 * <pre>{@code
 * ... -> A B <- ...
 ...
 * }</pre>
 *
 * Note the sub-path D...B was reversed. The 2-opt works by reversing the path between the two edges
 * being removed.
 *
 * <p>When the edges are assigned to different entities, it results in a tail swap. For instance,
 * let r1 = [A, B, C, D], and r2 = [E, F, G, H]. Doing a 2-opt on (B, C) + (F, G) will result in r1
 * = [A, B, G, H] and r2 = [E, F, C, D].
 *
 * @param <Solution_>
 */
public class TwoOptListMove<Solution_> extends AbstractMove<Solution_> {
  ...
  private void doTailSwap(ScoreDirector<Solution_> scoreDirector) {
    ...
    castScoreDirector.beforeListVariableChanged(
        variableDescriptor, firstEntity, firstEdgeEndpoint, firstOriginalSize);
    castScoreDirector.beforeListVariableChanged(
        variableDescriptor, secondEntity, secondEdgeEndpoint, secondOriginalSize);
    ...
    castScoreDirector.afterListVariableChanged(
        variableDescriptor, firstEntity, firstEdgeEndpoint, firstOriginalSize + tailSizeDifference);
    castScoreDirector.afterListVariableChanged(
        variableDescriptor,
        secondEntity,
        secondEdgeEndpoint,
  ...
  private void doSublistReversal(ScoreDirector<Solution_> scoreDirector) {
    ...
    if (firstEdgeEndpoint < secondEdgeEndpoint) {
      if (firstEdgeEndpoint > 0) {
        castScoreDirector.beforeListVariableChanged(
            variableDescriptor, firstEntity, firstEdgeEndpoint, secondEdgeEndpoint);
      } else {
        castScoreDirector.beforeListVariableChanged(
            variableDescriptor, firstEntity, entityFirstUnpinnedIndex, listVariable.size());
      }
      ...
      if (firstEdgeEndpoint == 0 && shift > 0) {
        if (entityFirstUnpinnedIndex == 0) {
          ...
          Collections.rotate(
              listVariable.subList(entityFirstUnpinnedIndex, listVariable.size()), shift);
        }
      }

      FlipSublistAction.flipSublist(
          listVariable, entityFirstUnpinnedIndex, firstEdgeEndpoint, secondEdgeEndpoint);
  ...
  public boolean isMoveDoable(ScoreDirector<Solution_> scoreDirector) {
    ...
    var firstListVariableTail =
        firstListVariable.subList(firstEdgeEndpoint, firstListVariable.size());
    var secondValueRange =
        valueRangeManager.getFromEntity(variableDescriptor.getValueRangeDescriptor(), secondEntity);
    var secondListVariable = variableDescriptor.getValue(secondEntity);
    var secondListVariableTail =
        secondListVariable.subList(secondEdgeEndpoint, secondListVariable.size());
  ...
  public Collection<?> getPlanningValues() {
    if (firstEntity == secondEntity) {
      ...
      if (firstEdgeEndpoint < secondEdgeEndpoint) {
        ...
        var secondHalfReversedPath =
            listVariable.subList(entityFirstUnpinnedIndex, secondEdgeEndpoint);
