...
/**
 * Selects values from the {@link ValueRangeProvider} for a {@link PlanningVariable} annotated
 * property.
 *
...
public interface ValueSelector<Solution_> extends Selector<Solution_> {
   ...
   * @return the approximate number of elements generated by this {@link Selector}, always {@code >=
   *     0}
   * @throws IllegalStateException if {@link #isCountable} returns false, but not if only {@link
   *     #isNeverEnding()} returns true
   */
  ...
  /**
   * If {@link #isNeverEnding()} is true, then {@link #iterator(Object)} will never end. This
   * returns an ending {@link Iterator}, that tries to match {@link #iterator(Object)} as much as
   * possible, but return each distinct element only once and therefore it might not respect the
   * configuration of this {@link ValueSelector} entirely.
   *
   * @param entity never null
   * @return never null
   * @see #iterator(Object)
  ...
  /**
   * Returns the selection sorter applied to the node. By default, it returns null and must be
   * overridden by the child class if necessary.
   *
