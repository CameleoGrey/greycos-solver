...
import ai.greycos.solver.core.impl.cotwin.lookup.LookUpManager;
...
import ai.greycos.solver.core.impl.cotwin.variable.listener.support.violation.SolutionTracker;
...
import ai.greycos.solver.core.impl.solver.exception.CloningCorruptionException;
import ai.greycos.solver.core.impl.solver.exception.ScoreCorruptionException;
import ai.greycos.solver.core.impl.solver.exception.UndoScoreCorruptionException;
 ...
 *
 * <p>Implementation note: Extending classes should follow these guidelines:
 *
 * <ul>
 *   <li>{@link #setWorkingSolutionWithoutUpdatingShadows(Object)} should delegate to {@link
 *       #setWorkingSolutionWithoutUpdatingShadows(Object, Consumer)}
 *   <li>before* method: last statement should be a call to the super method
 *   <li>after* method: first statement should be a call to the super method
 * </ul>
 */
public abstract class AbstractScoreDirector<
        Solution_,
  ...
  /**
   * Must never be shared between score directors, because it contains state based on the current
   * clone of the working solution. Creating these value range caches is expensive, but they are
   * only created on first access to each and operations which do not perform moves do not require
   * them.
   */
   ...
   * the {@link ProblemChangeDirector} during {@link ProblemChange problem changes}, as these calls
   * are propagated to {@link InnerVariableListener variable listeners}, which update shadow
   * variables in the {@link PlanningSolution working solution} to keep it consistent.
   *
   * @param workingSolution the working solution to set
   * @param entityAndFactVisitor maybe null; a function to apply to all problem facts and problem
   *     entities
   */
  protected void setWorkingSolutionWithoutUpdatingShadows(
    ...
    if (lookUpEnabled) {
      ...
      entityAndFactVisitor =
          entityAndFactVisitor == null
              ? workingObjectLookupVisitor
              : entityAndFactVisitor.andThen(workingObjectLookupVisitor);
    }
    // One-time check of value ranges to ensure assigned planning values are included
    // in the solution range or any entity value range.
    entityAndFactVisitor =
        entityAndFactVisitor == null
            ? this::assertValueRangeForEntity
    ...
    entityAndFactVisitor =
        entityAndFactVisitor == null
            ? entityValidator
            : entityAndFactVisitor.andThen(entityValidator);
    setWorkingEntityListDirty(workingSolution);
    // This visits all the entities.
    var initializationStatistics =
        valueRangeManager.getInitializationStatistics(entityAndFactVisitor);
    workingInitScore =
  ...
  /**
   * Note: by default does nothing. Subclasses should override this if they need to compute
   * something after shadow variables are set.
   */
  ...
  public Solution_ cloneSolution(Solution_ originalSolution) {
    ...
    if (scoreDirectorFactory.isAssertClonedSolution()) {
      ...
      solutionDescriptor.visitAllEntities(
          originalSolution, originalEntity -> originalEntityMap.put(originalEntity, null));
      solutionDescriptor.visitAllEntities(
          cloneSolution,
          cloneEntity -> {
  ...
  /**
   * This function clears all listener events that have been generated without triggering any of
   * them. Using this method requires caution because clearing the event queue can lead to
   * inconsistent states. This occurs when the shadow variables are not updated, causing constraints
  ...
  public void afterEntityAdded(EntityDescriptor<Solution_> entityDescriptor, Object entity) {
    ...
    if (!allChangesWillBeUndoneBeforeStepEnds) {
      ...
      // Therefore, we need to reset the value range state, but the working solution does not
      // change.
  ...
  public void assertWorkingScoreFromScratch(
      InnerScore<Score_> workingScore, Object completedAction) {
    assertScoreFromScratch(workingScore, completedAction, false);
  }

  @Override
  public void assertPredictedScoreFromScratch(
      InnerScore<Score_> workingScore, Object completedAction) {
  ...
  private static <Solution_> void assertValueRangeForBasicVariables(
  ...
  private static <Solution_> void assertValueRangeForListVariable(
      InnerScoreDirector<Solution_, ?> scoreDirector,
      ListVariableDescriptor<Solution_> variableDescriptor,
      Object entity,
      List<@Nullable Object> valueList) {
  ...
  private void assertScoreFromScratch(
      InnerScore<Score_> innerScore, Object completedAction, boolean predicted) {
    ...
    try (var uncorruptedScoreDirector =
      ...
      if (!innerScore.equals(uncorruptedInnerScore)) {
        String scoreCorruptionAnalysis =
            buildScoreCorruptionAnalysis(uncorruptedScoreDirector, predicted);
  ...
  public void assertExpectedUndoMoveScore(
      Move<Solution_> move,
      InnerScore<Score_> beforeMoveInnerScore,
    ...
    if (trackingWorkingSolution) {
      throw new UndoScoreCorruptionException(
          corruptionMessage,
  ...
  public SolutionTracker.SolutionCorruptionResult getSolutionCorruptionAfterUndo(
      Move<Solution_> move, InnerScore<Score_> undoInnerScore) {
    var trackingWorkingSolution = solutionTracker != null;
    if (trackingWorkingSolution) {
      solutionTracker.setAfterUndoSolution(workingSolution);
    }
  ...
  protected String buildScoreCorruptionAnalysis(
    ...
    var uncorruptedAnalysis =
        uncorruptedScoreDirector.buildScoreAnalysis(ScoreAnalysisFetchPolicy.FETCH_ALL);
    ...
    uncorruptedAnalysis
        .constraintMap()
        .forEach(
            (constraintRef, uncorruptedConstraintAnalysis) -> {
              var uncorruptedConstraintMatches =
                  emptyMatchAnalysisIfNull(uncorruptedConstraintAnalysis);
              var corruptedConstraintMatches =
                  emptyMatchAnalysisIfNull(corruptedAnalysis.constraintMap().get(constraintRef));
              if (corruptedConstraintMatches.isEmpty()) {
                missingSet.addAll(uncorruptedConstraintMatches);
              } else {
                updateExcessAndMissingConstraintMatches(
                    uncorruptedConstraintMatches,
                    corruptedConstraintMatches,
                    excessSet,
                    missingSet);
              }
            });
    ...
    corruptedAnalysis
        .constraintMap()
        .forEach(
            (constraintRef, corruptedConstraintAnalysis) -> {
              var corruptedConstraintMatches =
                  emptyMatchAnalysisIfNull(corruptedConstraintAnalysis);
              var uncorruptedConstraintMatches =
                  emptyMatchAnalysisIfNull(uncorruptedAnalysis.constraintMap().get(constraintRef));
    ...
    // If predicted, the score calculation might have happened on another thread, so a different
    // ScoreDirector
    ...
    if (!missingSet.isEmpty() || !excessSet.isEmpty()) {
      ...
      if (predicted) {
        ...
        analysis.append(
            "  Impossible state. Maybe this is a bug in the scoreDirector (%s)."
                .formatted(getClass()));
      }
    }
  ...
  private static <Score_ extends Score<Score_>>
      List<MatchAnalysis<Score_>> emptyMatchAnalysisIfNull(
          ConstraintAnalysis<Score_> constraintAnalysis) {
  ...
  private void appendAnalysis(
    ...
    if (matches.isEmpty()) {
      ...
      matches.stream()
          .sorted()
          .limit(CONSTRAINT_MATCH_DISPLAY_LIMIT)
          .forEach(
              match ->
                  analysis.append(
                      """
  ...
  private void updateExcessAndMissingConstraintMatches(
      List<MatchAnalysis<Score_>> uncorruptedList,
      List<MatchAnalysis<Score_>> corruptedList,
      Set<MatchAnalysis<Score_>> excessSet,
  ...
  private void iterateAndAddIfFound(
      List<MatchAnalysis<Score_>> referenceList,
      List<MatchAnalysis<Score_>> lookupList,
  ...
  /**
   * An abstract builder for creating instances of {@link InnerScoreDirector}. This class provides
   * methods to configure the behavior of the score director before building it. Unless the
   * appropriate withers are called, the score director will be built:
   *
   * <ul>
   *   <li>With {@link ConstraintMatchPolicy#DISABLED}.
   *   <li>With lookup disabled.
   *   <li>Expecting shadow variables in their correct state.
   * </ul>
   *
   ...
   */
  @NullMarked
  public abstract static class AbstractScoreDirectorBuilder<
      Solution_,
      Score_ extends Score<Score_>,
      Factory_ extends AbstractScoreDirectorFactory<Solution_, Score_, Factory_>,
     ...
     * Derived score directors may make choices which the main score director cannot make, such as
     * reducing logging. Derived score directors are typically used for multithreaded solving,
     * testing and assert modes. Derived score directors do not support Neighborhoods API, as they
     * are only used to calculate the score.
     *
     * @return this
     */
