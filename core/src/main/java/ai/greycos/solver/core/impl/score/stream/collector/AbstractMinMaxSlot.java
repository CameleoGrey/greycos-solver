...
public abstract class AbstractMinMaxSlot<Result_, Property_> {
  public static final class State<Result_, Property_> {
    private final Supplier<Map.Entry<Property_, SequencedMap<Result_, MutableInt>>>
        firstOrLastEntrySupplier;
    private final NavigableMap<Property_, SequencedMap<Result_, MutableInt>>
        propertyToItemCountNavigableMap;
    private final Function<? super Result_, ? extends Property_> propertyFunction;

    private State(
        boolean isMin,
        NavigableMap<Property_, SequencedMap<Result_, MutableInt>> propertyToItemCountNavigableMap,
        Function<? super Result_, ? extends Property_> propertyFunction) {
      this.firstOrLastEntrySupplier =
          isMin
              ? propertyToItemCountNavigableMap::firstEntry
              : propertyToItemCountNavigableMap::lastEntry;
  ...
  protected void addMapped(Result_ item) {
    ...
    cachedInnerMap =
        state.propertyToItemCountNavigableMap.computeIfAbsent(
            cachedKey, ignored -> new LinkedHashMap<>());
  ...
  protected void replaceWithMapped(Result_ item) {
    ...
    cachedInnerMap =
        state.propertyToItemCountNavigableMap.computeIfAbsent(
            newKey, ignored -> new LinkedHashMap<>());
    cachedCount = cachedInnerMap.computeIfAbsent(item, ignored -> new MutableInt());
    cachedCount.increment();
  }
