...
public class KOptMoveSelector<Solution_> extends GenericMoveSelector<Solution_> {
  ...
  public KOptMoveSelector(
      EntitySelector<Solution_> entitySelector,
      ValueSelector<Solution_>[] valueSelectors,
    ...
    if (!randomSelection) {
      throw new UnsupportedOperationException(
          "Non randomSelection (such as original selection) is not yet supported on "
              + KOptMoveSelector.class.getSimpleName()
              + "."); // TODO
    }
    ...
    if (!isChained) {
      throw new IllegalStateException(
          "The selector (%s)'s valueSelector's variableDescriptor (%s) must be chained (%s)."
              .formatted(this, variableDescriptor, isChained));
    }
    if (!variableDescriptor
        .getEntityDescriptor()
        .getEntityClass()
        .isAssignableFrom(entitySelector.getEntityDescriptor().getEntityClass())) {
      throw new IllegalStateException(
          "The selector ("
              + this
    ...
    for (ValueSelector<Solution_> valueSelector : valueSelectors) {
      if (valueSelector.getVariableDescriptor() != variableDescriptor) {
        throw new IllegalStateException(
            "The selector ("
                + this
                + ") has a valueSelector with a variableDescriptor ("
                + valueSelector.getVariableDescriptor()
                + ") that differs from the first variableDescriptor ("
                + variableDescriptor
                + ").");
      }
  ...
  public long getSize() {
    ...
    //            return entitySelector.getSize() * (long) Math.pow(((IterableSelector)
    // valueSelector).getSize(), K);
    //        } else {
    //        }
  }
    ...
    if (!randomSelection) {
      ...
      return new UpcomingSelectionIterator<>() {
        ...
        protected Move<Solution_> createUpcomingSelection() {
          // TODO currently presumes that entitySelector and all valueSelectors are never ending,
          // despite the hasNext() checks
          ...
          return new KOptMove<>(
              variableDescriptor, inverseVariableSupply, anchorVariableSupply, entity, values);
        }
      };
    }
  }
  ...
  public String toString() {
    return getClass().getSimpleName()
        ...
        + entitySelector
        + ", "
        + Arrays.toString(valueSelectors)
        + ")";
  }
}
