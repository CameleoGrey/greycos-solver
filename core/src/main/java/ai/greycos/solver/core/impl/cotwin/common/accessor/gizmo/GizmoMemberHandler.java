...
interface GizmoMemberHandler {
   ...
   * @return never null
   */
  static GizmoMemberHandler of(
      Class<?> declaringClass, String name, FieldDesc fieldDescriptor, boolean ignoreFinalChecks) {
    try {
      Field field = declaringClass.getField(name);
      return new GizmoFieldHandler(
          declaringClass,
          fieldDescriptor,
          ignoreFinalChecks || !Modifier.isFinal(field.getModifiers()));
    } catch (
        NoSuchFieldException
            e) { // The field is only used for its metadata and never actually called.
   ...
   * @param readMethodParameterType the method parameter type. It is null if the method has no
   *     parameter.
   * @param methodDescriptor never null, descriptor of the {@link Method} in question
   * @return never null
   */
  static GizmoMemberHandler of(
      Class<?> declaringClass, Class<?> readMethodParameterType, MethodDesc methodDescriptor) {
