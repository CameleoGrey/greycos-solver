 ...
 * PlanningSolution} which it will change when {@link #execute(MutableSolutionView)} is called. It
 * is recommended for the moves to not touch shadow variables, the solver will update shadow
 * variables after move execution is complete. If the move has to touch shadow variables, it is
 * responsible for updating them consistently across the entire dependency graph.
 *
 * <p>For tabu search, a Move should implement {@link Object#equals(Object)} and {@link
 * Object#hashCode()}, {@link #getPlanningEntities()} and {@link #getPlanningValues()}.
 *
...
public interface Move<Solution_> {
   ...
   *
   * @param solutionView Exposes all possible mutative operations on the variables. Remembers those
   *     mutative operations and can replay them in reverse order when the solver needs to undo the
   *     move. Do not store this parameter in a field.
   */
   ...
   *
   * <p>This method is only called after {@link #execute(MutableSolutionView)}, which might affect
   * the return values.
   *
  ...
  /**
   * Returns all planning values that this move is assigning to entity variables. Required for value
   * tabu.
   *
   * <p>This method is only called after {@link #execute(MutableSolutionView)}, which might affect
   * the return values.
  ...
  /**
   * Describes the move type for statistical purposes. For example, a move which changes a variable
   * "computer" on a class "Process" could be described as "ChangeMove(Process.computer)".
   *
   * <p>The format is not formalized, but it is recommended to stick to ASCII, avoiding whitespace
   * and special characters. Never parse the {@link String} returned by this method, it is only
   * intended to be used by the solver.
   *
