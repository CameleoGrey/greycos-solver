 ...
 *
 * <ul>
 *   <li>A move to unassign it.
 *   <li>A move to reassign it to another position if assigned.
 * </ul>
 *
 * To assign or reassign a value, creates:
 *
 * <ul>
 *   <li>A move for every unpinned value in every entity's list variable to assign the value before
 *       that position.
 *   <li>A move for every entity to assign it to the last position in the list variable.
 * </ul>
 *
 ...
 */
@NullMarked
public class ListChangeMoveProvider<Solution_, Entity_, Value_> implements MoveProvider<Solution_> {
  ...
  public MoveStream<Solution_> build(MoveStreamFactory<Solution_> moveStreamFactory) {
    ...
    return moveStreamFactory
        .pick(entityValuePairs)
        .pick(availableValues, NeighborhoodsJoiners.filtering(this::isValidChange))
        .asMove(
            (solutionView, targetPosition, value) -> {
              var currentPosition =
                  solutionView.getPositionOf(variableMetaModel, Objects.requireNonNull(value));
  ...
  private boolean isValidChange(
      SolutionView<Solution_> solutionView, ElementPosition targetPosition, Value_ value) {
    ...
    if (currentPosition
        instanceof
        UnassignedElement) { // Only assign the value if the target entity will accept it.
    ...
    if (currentPositionInList.entity()
        == targetPositionInList.entity()) { // The value is already in the list.
      ...
      if (valueCount == 1) { // The value is the only value in the list; no change.
        return false;
      } else if (targetPositionInList.index()
          == valueCount) { // Trying to move the value past the end of the list.
        return false;
