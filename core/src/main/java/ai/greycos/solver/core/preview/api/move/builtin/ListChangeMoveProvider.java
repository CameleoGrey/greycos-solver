...
/**
 * For each assigned value that is not pinned, creates a move to reassign it to a different position
 * in a list variable. Unassigned-to-list (assign) moves are handled by {@code
 * ListAssignMoveProvider}. List-to-unassigned (unassign) moves are handled by {@code
 * ListUnassignMoveProvider}.
 *
 * <p>To reassign a value, creates:
 *
 * <ul>
 *   <li>A move for every unpinned position in every entity's list variable to reassign the value
 *       before that position.
 *   <li>A move for every entity to reassign the value to the last position in the list variable.
 * </ul>
 *
...
public class ListChangeMoveProvider<Solution_, Entity_, Value_> implements MoveProvider<Solution_> {
  ...
  public MoveStream<Solution_> build(MoveStreamFactory<Solution_> moveStreamFactory) {
    ...
    return moveStreamFactory
        ...
        .pick(assignedValues, NeighborhoodsJoiners.filtering(this::isValidChange))
        .asMove(
            (solutionView, targetPosition, value) -> {
              var currentPosition =
                  solutionView
                      .getPositionOf(variableMetaModel, Objects.requireNonNull(value))
                      .ensureAssigned();
  ...
  private boolean isValidChange(
      SolutionView<Solution_> solutionView, PositionInList targetPosition, Value_ value) {
