...
import ai.greycos.solver.core.impl.heuristic.selector.entity.pillar.PillarSelectorFactory;
  ...
  protected MoveSelector<Solution_> buildBaseMoveSelector(
    ...
    if (leftEntitySelectorConfig != null && leftEntitySelectorConfig.getEntityClass() != null) {
      leftEntityDescriptor =
          configPolicy
              ...
              .findEntityDescriptor(leftEntitySelectorConfig.getEntityClass());
    }
    var leftVariableNameIncludeList = config.getVariableNameIncludeList();
    if (leftVariableNameIncludeList == null
        && leftEntityDescriptor != null
        && leftEntityDescriptor.hasAnyGenuineBasicVariables()
        && leftEntityDescriptor.hasAnyGenuineListVariables()) {
      // Mixed models filter out the list variable
      leftVariableNameIncludeList =
          leftEntityDescriptor.getGenuineBasicVariableDescriptorList().stream()
    ...
    if (rightEntitySelectorConfig != null && rightEntitySelectorConfig.getEntityClass() != null) {
      rightEntityDescriptor =
          configPolicy
              .getSolutionDescriptor()
              .findEntityDescriptor(rightEntitySelectorConfig.getEntityClass());
    }
    var rightVariableNameIncludeList = config.getVariableNameIncludeList();
    if (rightVariableNameIncludeList == null
        && rightEntityDescriptor != null
        && rightEntityDescriptor.hasAnyGenuineBasicVariables()
        && rightEntityDescriptor.hasAnyGenuineListVariables()) {
      // Mixed models filter out the list variable
      rightVariableNameIncludeList =
          rightEntityDescriptor.getGenuineBasicVariableDescriptorList().stream()
              .map(GenuineVariableDescriptor::getVariableName)
              .toList();
    }
    var leftPillarSelector =
        buildPillarSelector(
            leftPillarSelectorConfig,
            configPolicy,
            leftVariableNameIncludeList,
            minimumCacheType,
            randomSelection);
    ...
    var variableDescriptorList =
        deduceVariableDescriptorList(
            leftPillarSelector.getEntityDescriptor(), leftVariableNameIncludeList);
    return new PillarSwapMoveSelector<>(
        leftPillarSelector, rightPillarSelector, variableDescriptorList, randomSelection);
  }

  private PillarSelector<Solution_> buildPillarSelector(
