 ...
 * @param removedEdgeIndexToTourOrder node visit order when the tour is traveled in the successor
 *     direction. This forms a 2k-cycle representing the permutation performed by the K-opt move.
 * @param inverseRemovedEdgeIndexToTourOrder node visit order when the tour is traveled in the
 *     predecessor direction. It is the inverse of {@link
 *     KOptDescriptor#removedEdgeIndexToTourOrder} (i.e. {@link
 *     KOptDescriptor#removedEdgeIndexToTourOrder}[inverseRemovedEdgeIndexToTourOrder[i]] == i
 * @param addedEdgeToOtherEndpoint maps the index of a removed edge endpoint to its corresponding
 *     added edge other endpoint. For instance, if the removed edges are (a, b), (c, d), (e, f) and
 *     the added edges are (a, d), (c, f), (e, b), then <br>
 *     removedEdges = [null, a, b, c, d, e, f] <br>
 *     addedEdgeToOtherEndpoint = [null, 4, 5, 6, 1, 2, 3] <br>
 *     <br>
 *     For any valid removedEdges index, (removedEdges[index],
 *     removedEdges[addedEdgeToOtherEndpoint[index]]) is an edge added by this K-Opt move.
 * @param <Node_>
 */
record KOptDescriptor<Node_>(
    int k,
    Node_[] removedEdges,
    int[] removedEdgeIndexToTourOrder,
    int[] inverseRemovedEdgeIndexToTourOrder,
   ...
   *
   * @param removedEdges The edges removed from the tour. The added edges will be formed from
   *     opposite endpoints for consecutive edges.
   ...
   *     argument is between its first and last argument when the tour is taken in the successor
   *     direction.
   */
  KOptDescriptor(
      Node_[] removedEdges,
    ...
    this(
        removedEdges,
        computeInEdgesForSequentialMove(removedEdges),
        endpointToSuccessorFunction,
        betweenPredicate);
  }

  /**
   * Create as sequential or non-sequential {@link KOptDescriptor} from the given removed and added
   * edges.
   *
  ...
  KOptDescriptor(
      ...
      TriPredicate<Node_, Node_, Node_> betweenPredicate) {
    this(
        (removedEdges.length - 1) >> 1,
        removedEdges,
        new int[removedEdges.length],
        new int[removedEdges.length],
    ...
    Comparator<Integer> comparator =
        (pa, pb) ->
            pa.equals(pb)
                ? 0
                : (betweenPredicate.test(
                        removedEdges[removedEdgeIndexToTourOrder[1]],
                        removedEdges[pa],
                        removedEdges[pb])
                    ? -1
                    : 1);

    var wrappedRemovedEdgeIndexToTourOrder =
        IntStream.of(removedEdgeIndexToTourOrder).boxed().toArray(Integer[]::new);
   ...
   * <br>
   * It implements the algorithm described in the paper <a
   * href="https://dl.acm.org/doi/pdf/10.1145/300515.300516">"Transforming Cabbage into Turnip:
   * Polynomial Algorithm for Sorting Signed Permutations by Reversals"</a> which is used in the
   * paper <a href="http://webhotel4.ruc.dk/~keld/research/LKH/KoptReport.pdf">"An Effective
   * Implementation of K-opt Moves for the Lin-Kernighan TSP Heuristic"</a> (Section 5.4 "Execution
   * of a feasible move") to perform a K-opt move by performing the minimal number of list reversals
   * to transform the current route into the new route after the K-opt. We use it here to calculate
   * the {@link FlipSublistAction} list for the {@link KOptListMove} that is described by this
   * {@link KOptDescriptor}.<br>
   * <br>
   * The algorithm goal is to convert a signed permutation (p_1, p_2, ..., p_(2k)) into the identify
   * permutation (+1, +2, +3, ..., +(2k - 1), +2k). It can be summarized as:
   *
   * <ul>
   *   <li>As long as there are oriented pairs, perform the reversal that corresponds to the
   *       oriented pair with the maximal score (described in {@link
   *       #countOrientedPairsForReversal}).
   *   <li>If there are no oriented pairs, Find the pair (p_i, p_j) for which |p_j – p_i| = 1, j >=
   *       i + 3, and i is minimal. Then reverse the segment (p_(i+1) ... p_(j-1). This corresponds
   *       to a hurdle cutting operation. Normally, this is not enough to guarantee the number of
   *       reversals is optimal, but since each hurdle corresponds to a unique cycle, the number of
   *       hurdles in the list at any point is at most 1 (and thus, hurdle cutting is the optimal
   *       move). (A hurdle is an subsequence [i, p_j, p_(j+1)..., p_(j+k-1) i+k] that can be sorted
   *       so [i, p_j, p_(j+1)..., p_(j+k-1), i+k] are all consecutive integers, which does not
   *       contain a subsequence with the previous property ([4, 7, 6, 5, 8] is a hurdle, since the
   *       subsequence [7, 6, 5] does not contain all the items between 7 and 5 [8, 1, 2, 3, 4]).
   *       This create enough enough oriented pairs to completely sort the permutation.
  ...
  public <Solution_> KOptListMove<Solution_> getKOptListMove(
    ...
    var currentInverseRemovedEdgeIndexToTourOrder =
        Arrays.copyOf(
            inverseRemovedEdgeIndexToTourOrder, inverseRemovedEdgeIndexToTourOrder.length);
    ...
    while (isMoveNotDone) {
      ...
      for (var firstEndpoint = 1; firstEndpoint <= 2 * k - 2; firstEndpoint++) {
        ...
        if (secondEndpoint >= firstEndpoint + 2 && (firstEndpoint & 1) == (secondEndpoint & 1)) {
          var orientedPairCountAfterReversal =
              ((firstEndpoint & 1) == 1)
                  ? countOrientedPairsForReversal(
      ...
      if (maximumOrientedPairCountAfterReversal >= 0) {
        if ((bestOrientedPairFirstEndpoint & 1) == 1) {
          out.add(
              getListReversalMoveForEdgePair(
                  listVariableDescriptor,
                  indexVariableSupply,
                  originalToCurrentIndexList,
                  removedEdges[
                      currentRemovedEdgeIndexToTourOrder[bestOrientedPairFirstEndpoint + 1]],
                  ...
                  removedEdges[
                      currentRemovedEdgeIndexToTourOrder[bestOrientedPairSecondEndpoint + 1]]));
  ...
  /**
   * Return true if and only if performing the K-opt move described by this {@link KOptDescriptor}
   * will result in a single cycle.
   *
   * @param minK
   * @param maxK
   * @return true if and only if performing the K-opt move described by this {@link KOptDescriptor}
   *     will result in a single cycle, false otherwise.
   */
  public boolean isFeasible(int minK, int maxK) {
    if (k < minK || k > maxK) {
      throw new IllegalStateException(
          "Impossible state: The k-opt move k-value (%d) is not in the range [%d, %d]."
  ...
  private boolean isFeasible() {
    ...
    // This loop calculate the length of the cycle that the endpoint at removedEdges[2k] is
    // connected
    // to by iterating the loop in reverse. We know that the successor of removedEdges[2k] is 0,
    // which
  ...
  /**
   * Reverse an array between two indices and update its inverse array to point at the new
   * locations.
   *
   ...
   */
  private void reversePermutationPart(
    ...
    while (startInclusive < endExclusive) {
      ...
      currentRemovedEdgeIndexToTourOrder[startInclusive] =
          currentRemovedEdgeIndexToTourOrder[endExclusive];
      currentInverseRemovedEdgeIndexToTourOrder[currentRemovedEdgeIndexToTourOrder[endExclusive]] =
          startInclusive;
   ...
   * <br>
   * Let p = (p_1 ..., p_n) be a signed permutation. An oriented pair (p_i, p_j) is a pair of
   * adjacent integers, that is |p_i| - |p_j| = ±1, with opposite signs. For example, the signed
   * permutation <br>
   * (+1 -2 -5 +4 +3) <br>
   * contains three oriented pairs: (+1, -2), (-2, +3), and (-5, +4). Oriented pairs are useful as
   * they indicate reversals that cause adjacent integers to be consecutive in the resulting
   * permutation. For example, the oriented pair (-2, +3) induces the reversal <br>
   ...
   * The score of an oriented reversal is defined as the number of oriented pairs in the resulting
   * permutation. <br>
   * <br>
   * This function perform the reversal indicated by the oriented pair, count the number of oriented
   * pairs in the new permutation, undo the reversal and return the score.
   *
   ...
   */
  private int countOrientedPairsForReversal(
      int[] currentRemovedEdgeIndexToTourOrder,
      int[] currentInverseRemovedEdgeIndexToTourOrder,
      int left,
      int right) {
    int count = 0, i, j;
    reversePermutationPart(
        currentRemovedEdgeIndexToTourOrder, currentInverseRemovedEdgeIndexToTourOrder, left, right);
    for (i = 1; i <= 2 * k - 2; i++) {
  ...
  /**
   * Get a {@link FlipSublistAction} that reverses the sublist that consists of the path between the
   * start and end of the given edges.
   *
  ...
  private static <Node_> FlipSublistAction getListReversalMoveForEdgePair(
    ...
    var secondEndpoint =
        ((originalSecondEdgeStartIndex + 1) % originalToCurrentIndexList.length)
  ...
  private MultipleDelegateList<Node_> computeCombinedList(
    ...
    for (var i = 1; i < removedEdges.length; i++) {
      entityToEntityIndex.computeIfAbsent(
          inverseVariableSupply.getInverseSingleton(removedEdges[i]),
  ...
  public boolean equals(Object o) {
    return o instanceof KOptDescriptor<?> that
        ...
        && Arrays.equals(
            inverseRemovedEdgeIndexToTourOrder, that.inverseRemovedEdgeIndexToTourOrder)
  ...
  public String toString() {
    return k
        + "-opt(removed="
        + KOptUtils.getRemovedEdgeList(this)
        + "\n, added="
        + KOptUtils.getAddedEdgeList(this)
        + ")";
  }
}
