 ...
 * ConstraintMatchTotal}. Retrievable from {@link ScoreExplanation#getIndictmentMap()}.
 *
...
public interface Indictment<Score_ extends Score<Score_>> {
   ...
   *
   * <p>This is a poorly named legacy method, which does not in fact return a justification, but an
   * indicted object. Each indictment may have multiple justifications, and they are accessed by
   * {@link #getJustificationList()}.
   *
  ...
  /**
   * The object that was involved in causing the constraints to match. It is part of {@link
   * ConstraintMatch#getIndictedObjectList()} of every {@link ConstraintMatch} returned by {@link
   ...
   * {@link #getConstraintMatchSet()}. This is equivalent to retrieving {@link
   * #getConstraintMatchSet()} and collecting all {@link ConstraintMatch#getJustification()} objects
   * into a list.
   *
  ...
  /**
   * Retrieve {@link ConstraintJustification} instances associated with {@link ConstraintMatch}es in
   * {@link #getConstraintMatchSet()}, which are of (or extend) a given constraint justification
   * implementation. This is equivalent to retrieving {@link #getConstraintMatchSet()} and
   * collecting all matching {@link ConstraintMatch#getJustification()} objects into a list.
   *
   * @return guaranteed to contain unique instances
   */
  @NonNull
  default <ConstraintJustification_ extends ConstraintJustification>
      List<ConstraintJustification_> getJustificationList(
          Class<ConstraintJustification_> justificationClass) {
