...
public abstract class AbstractEventSupport<E extends EventListener> {
  ...
  /**
   * {@link EntitySelector} instances may end up here. Each instance added via {@link
   * #addEventListener(EventListener)} must appear here, regardless of whether it is equal to any
   * other instance already present. Likewise {@link #removeEventListener(EventListener)} must
   * remove elements by identity, not equality.
   *
   * <p>This list-based implementation could be called an "identity set", similar to {@link
   * IdentityHashMap}. We can not use {@link IdentityHashMap}, because we require iteration in
   * insertion order.
   */
