 ...
 * disabled constraints have already been removed.
 *
...
public record NodeNetwork(
  ...
  public Stream<BavetRootNode<?>> getRootNodes() {
    return declaredClassToNodeMap.entrySet().stream().flatMap(entry -> entry.getValue().stream());
  }
  ...
  public Stream<BavetRootNode<?>> getRootNodesAcceptingType(Class<?> factClass) {
    ...
    return declaredClassToNodeMap.entrySet().stream()
        ...
        .filter(
            tupleSourceRoot ->
                factClass == PlanningSolution.class
                    || tupleSourceRoot.allowsInstancesOf(factClass));
  }
