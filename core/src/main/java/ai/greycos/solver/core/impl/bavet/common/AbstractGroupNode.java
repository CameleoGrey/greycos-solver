...
public abstract class AbstractGroupNode<
  ...
  /**
   * Some code paths may decide to not supply a grouping function. In that case, every tuple
   * accumulates into {@link #singletonGroup} and not to {@link #groupMap}.
   */
  private final boolean hasMultipleGroups;

  /**
   * Some code paths may decide to not supply a collector. In that case, we skip the code path that
   ...
   *
   * @implNote The field is lazy initialized in order to maintain the same semantics as with the
   *     groupMap above. When all tuples are removed, the field will be set to null, as if the group
   *     never existed.
   */
  private Group<OutTuple_, ResultContainer_> singletonGroup;

  private final DynamicPropagationQueue<OutTuple_, Group<OutTuple_, ResultContainer_>>
      propagationQueue;
  ...
  public final void insert(InTuple_ tuple) {
    if (tuple.getStore(groupStoreIndex) != null) {
      throw new IllegalStateException(
          "Impossible state: the input for the tuple ("
              + tuple
  ...
  private Group<OutTuple_, ResultContainer_> getOrCreateGroup(GroupKey_ userSuppliedKey) {
    var groupMapKey =
        useAssertingGroupKey ? new AssertingGroupKey<>(userSuppliedKey) : userSuppliedKey;
  ...
  private GroupKey_ extractUserSuppliedKey(Object groupMapKey) {
    return useAssertingGroupKey
        ? ((AssertingGroupKey<GroupKey_>) groupMapKey).key()
        : (GroupKey_) groupMapKey;
  }
  ...
  private void killTuple(Group<OutTuple_, ResultContainer_> group) {
    ...
    if (killGroup) {
      ...
      if (oldGroup == null) {
        throw new IllegalStateException(
            "Impossible state: the group for the groupKey ("
                + groupKey
                + ") doesn't exist in the groupMap.\n"
  ...
  /**
   * Group key hashcode must never change once introduced to the group map. If it does,
   * unpredictable behavior will occur. Since this situation is far too frequent and users run into
   * this, we have this helper class that will optionally throw an exception when it detects this.
   */
