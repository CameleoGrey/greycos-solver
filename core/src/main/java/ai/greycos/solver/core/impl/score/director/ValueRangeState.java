...
import ai.greycos.solver.core.impl.domain.valuerange.buildin.primdouble.DoubleValueRange;
import ai.greycos.solver.core.impl.domain.valuerange.descriptor.ValueRangeDescriptor;
import ai.greycos.solver.core.impl.domain.valuerange.sort.SelectionSorterAdapter;
import ai.greycos.solver.core.impl.domain.valuerange.sort.SortableValueRange;
...
import ai.greycos.solver.core.impl.heuristic.selector.common.ReachableValues.ReachableItemValue;
import ai.greycos.solver.core.impl.heuristic.selector.common.ReachableValues.ReachableValuesIndex;
...
final class ValueRangeState<Solution_, Entity_, Value_> {
  ...
  private @Nullable ValueRangeItem<Solution_, Entity_, CountableValueRange<Value_>, Value_>
      fromSolutionItem;
  private @Nullable Map<Value_, Integer> fromSolutionValueIndexMap;

  // Entity related fields
  private @Nullable
      Map<Entity_, ValueRangeItem<Solution_, Entity_, CountableValueRange<Value_>, Value_>>
      fromEntityMap;
  private @Nullable Map<HashedValueRange<Value_>, Entity_> valueRangeDeduplicationCache;

  // ReachableValues related field
  private @Nullable ValueRangeItem<Solution_, Entity_, ReachableValues<Entity_, Value_>, Value_>
      reachableValuesItem;

  ValueRangeState(
      ValueRangeDescriptor<Solution_> valueRangeDescriptor, Solution_ cachedWorkingSolution) {
  ...
  public CountableValueRange<Value_> getFromSolution(
    ...
    // If the left sorter is null and the given sorter is not, we replace the left value with a
    // sorted one.
    if (fromSolutionItem.leftSorter() == null && sorter != null) {
      var sortedValueRange =
          sortValueRange(Objects.requireNonNull(fromSolutionItem.leftItem()), sorter);
      fromSolutionItem =
          ValueRangeItem.of(
              null,
              sortedValueRange,
      ...
      fromSolutionValueIndexMap =
          buildIndexMap(
              sortedValueRange.createOriginalIterator(),
      ...
      fromSolutionItem =
          ValueRangeItem.of(
              null,
              fromSolutionItem.leftItem(),
              fromSolutionItem.leftSorter(),
              sortedValueRange,
              sorter);
      return sortedValueRange;
    } else {
  ...
  private <Type_> @Nullable Type_ pickValueBySorter(
      ValueRangeItem<Solution_, Entity_, Type_, Value_> item,
      @Nullable SelectionSorter<Solution_, Value_> sorter,
      @Nullable BiFunction<Entity_, @Nullable SelectionSorter<Solution_, Value_>, Type_>
          placeholderFunction) {
    ...
    if (item.leftItem() == null && item.rightItem() == null && placeholderFunction != null) {
      ...
      if (placeholder == null) {
        throw new IllegalStateException(
            "Impossible state: the placeholder is null and no value ranges are found.");
      }
      return placeholderFunction.apply(placeholder, sorter);
    } else if (item.leftItem() != null
        && (sorter == null || Objects.equals(item.leftSorter(), sorter))) {
      // Return the left value if there is no sorter or if the left sorter is the same as the
      // provided one.
  ...
  private Map<Value_, Integer> getIndexMapFromSolution() {
    if (fromSolutionValueIndexMap == null) {
      // We call getFromSolution to ensure the solution-range is loaded and the related index map is
      // created
  ...
  private CountableValueRange<Value_> sortValueRange(
      CountableValueRange<Value_> originalValueRange,
      @Nullable SelectionSorter<Solution_, Value_> sorter) {
    if (sorter == null) {
    ...
    return (CountableValueRange<Value_>)
        sortableValueRange.sort(SelectionSorterAdapter.of(cachedWorkingSolution, sorter));
  }

  public CountableValueRange<Value_> getFromEntity(
      Entity_ entity, int entityCount, @Nullable SelectionSorter<Solution_, Value_> sorter) {
    ...
    if (item.leftSorter() == null && sorter != null) {
      ...
      var newItem =
          ValueRangeItem.of(
              entity,
              sortValueRange(Objects.requireNonNull(item.leftItem()), sorter),
              sorter,
              item.rightItem(),
              item.rightSorter());
      entityMap.put(entity, newItem);
  ...
  private ValueRangeItem<Solution_, Entity_, CountableValueRange<Value_>, Value_>
      buildEntityValueRangeItem(
          Entity_ entity, @Nullable SelectionSorter<Solution_, Value_> sorter) {
  ...
  /**
   * Search for an identical bitset linked to another entity. The goal is to deduplicate the value
   * ranges stored in memory.
   *
   * @param entity the entity
   * @param valueRange the entity value range
   * @return returns an existing entity with a matching value range, or returns null if it does not
   *     exist.
   */
  private @Nullable Entity_ findEntityMatch(
      Entity_ entity, CountableValueRange<Value_> valueRange) {
  ...
  private CountableValueRange<Value_> fetchValueRangeFromEntity(
      Entity_ entity, @Nullable SelectionSorter<Solution_, Value_> sorter) {
    CountableValueRange<Value_> valueRange;
    var extractedValueRange =
        valueRangeDescriptor.<Value_>extractValuesFromEntity(
            cachedWorkingSolution, Objects.requireNonNull(entity));
    if (!(extractedValueRange instanceof CountableValueRange<Value_> countableValueRange)) {
      throw new UnsupportedOperationException(
          """
  ...
  public ReachableValues<Entity_, Value_> getReachableValues(
    ...
    if (reachableValuesItem.leftSorter() == null && sorter != null) {
      ...
      var sortedValues =
          buildSortedReachableValues(
              Objects.requireNonNull(reachableValuesItem.leftItem()), sorter, true);
      reachableValuesItem =
          ValueRangeItem.of(
              null,
              sortedValues,
              sorter,
              reachableValuesItem.rightItem(),
      ...
      var sortedValues =
          buildSortedReachableValues(
              Objects.requireNonNull(reachableValuesItem.leftItem()), sorter, false);
      reachableValuesItem =
          ValueRangeItem.of(
              null,
              Objects.requireNonNull(reachableValuesItem.leftItem()),
  ...
  private ReachableValues<Entity_, Value_> buildSortedReachableValues(
      ReachableValues<Entity_, Value_> values,
      SelectionSorter<Solution_, Value_> sorter,
      boolean clear) {
  ...
  private ReachableValues<Entity_, Value_> fetchReachableValues(
      GenuineVariableDescriptor<Solution_> variableDescriptor,
      @Nullable SelectionSorter<Solution_, Value_> sorter) {
    ...
    var valueIndexMap =
        buildIndexMap(
            valueList.createOriginalIterator(),
            (int) valueList.getSize(),
            valueRangeDescriptor.isGenericTypeImmutable());
    ...
    if (valueListSize > Integer.MAX_VALUE) {
      throw new IllegalStateException(
          "The structure %s cannot be built for the entity %s (%s) because value range has a size (%d) which is higher than Integer.MAX_VALUE."
              .formatted(
                  ReachableValues.class.getSimpleName(),
                  entityDescriptor.getEntityClass().getSimpleName(),
                  variableDescriptor.getVariableName(),
                  valueListSize));
    }
    var expectedTypeOfValue =
        valueRangeDescriptor.getVariableDescriptor().getVariableMetaModel().type();
  ...
  private static <Type_> Map<Type_, Integer> buildIndexMap(
      Iterator<@Nullable Type_> allValues, int size, boolean isImmutable) {
  ...
  private List<ReachableItemValue<Entity_, Value_>> initReachableValueList(
      CountableValueRange<Value_> valueRange, int entityListSize) {
    var valuesSize = (int) valueRange.getSize();
    Iterator<@Nullable Value_> iterator = valueRange.createOriginalIterator();
    var spliterator =
        Spliterators.spliterator(iterator, valuesSize, Spliterator.ORDERED | Spliterator.IMMUTABLE);
    var idx = new MutableInt(-1);
    return StreamSupport.stream(spliterator, false)
        .filter(Objects::nonNull)
        .map(
            v ->
                new ReachableItemValue<Entity_, Value_>(
                    idx.increment(), v, entityListSize, valuesSize))
        .toList();
  }

  private static <Entity_, Value_> void loadEntityValueRange(
      int entityIndex,
      Map<Value_, Integer> valueIndexMap,
      CountableValueRange<Value_> valueRange,
      List<ReachableItemValue<Entity_, Value_>> reachableValueList) {
  ...
  private static <Value_> BitSet buildBitSetForValueRange(
      CountableValueRange<Value_> valueRange, Map<Value_, Integer> valueIndexMap) {
  ...
  private record ValueRangeItem<Solution_, Entity_, Type_, Value_>(
      @Nullable Entity_ entity,
      @Nullable Type_ leftItem,
    ...
    public static <Solution_, Entity_, Type_, Value_>
        ValueRangeItem<Solution_, Entity_, Type_, Value_> ofEntity(Entity_ entity) {
      return new ValueRangeItem<>(entity, null, null, null, null);
    }

    public static <Solution_, Entity_, Type_, Value_>
        ValueRangeItem<Solution_, Entity_, Type_, Value_> ofLeft(
            @Nullable Entity_ entity,
            Type_ leftItem,
            @Nullable SelectionSorter<Solution_, Value_> leftSorter) {
    ...
    public static <Solution_, Entity_, Type_, Value_>
        ValueRangeItem<Solution_, Entity_, Type_, Value_> of(
            @Nullable Entity_ entity,
            @Nullable Type_ leftItem,
            @Nullable SelectionSorter<Solution_, Value_> leftSorter,
            @Nullable Type_ rightItem,
  ...
  /**
   * The record holds a reference to {@link CountableValueRange}, a precomputed hash to avoid
