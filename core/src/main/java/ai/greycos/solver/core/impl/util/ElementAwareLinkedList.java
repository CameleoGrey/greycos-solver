...
/**
 * Linked list that allows to add and remove an element in O(1) time. Ideal for incremental
 * operations with frequent undo.
 *
...
public final class ElementAwareLinkedList<T> implements Iterable<T> {
  ...
  /**
   * Convenience method for where it is easy to use a non-capturing lambda. If a capturing lambda
   * consumer were to be created for this method, use {@link #iterator()} instead, which will
   * consume less memory.
   *
   * <p>For example, the following code is perfectly fine: <code>
   *     for (int i = 0; i &lt; 3; i++) {
   *         elementAwareList.forEach(entry -&gt; doSomething(entry));
   *     }
   * </code> It will create only one lambda instance, regardless of the number of iterations; it
   * doesn't need to capture any state. On the contrary, the following code will create three
   * instances of a capturing lambda, one for each iteration of the for loop: <code>
   *     for (int a: List.of(1, 2, 3)) {
   *         elementAwareList.forEach(entry -&gt; doSomething(entry, a));
   *     }
   * </code> In this case, the lambda would need to capture "a" which is different in every
   * iteration. Therefore, it will generally be better to use the iterator variant, as that will
   * only ever create one instance of the iterator, regardless of the number of iterations: <code>
   *     for (int a: List.of(1, 2, 3)) {
   *         for (var entry: elementAwareList) {
   *             doSomething(entry, a);
   *         }
   *     }
   * </code> This is only an issue on the hot path, where this method can create quite a large
   * garbage collector pressure on account of creating throw-away instances of capturing lambdas.
   *
  ...
  /**
   * Returns an iterator that will randomly iterate over the elements. This iterator is exhaustive;
   * once every element has been once iterated over, the iterator returns false for every subsequent
   * {@link Iterator#hasNext()}. The iterator does not support the {@link Iterator#remove()}
   * operation.
   *
   ...
   * In fact, Neighborhoods API is likely to only use the first few elements. Therefore, shuffling
   * the entire list would be a waste of time. Instead, we pick random index every time and keep a
