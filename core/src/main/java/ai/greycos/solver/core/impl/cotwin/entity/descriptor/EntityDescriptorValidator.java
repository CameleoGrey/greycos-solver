...
import static ai.greycos.solver.core.impl.cotwin.entity.descriptor.EntityDescriptor.VARIABLE_ANNOTATION_CLASSES;
...
public class EntityDescriptorValidator {
  ...
  private static final Class[] ADDITIONAL_VARIABLE_ANNOTATION_CLASSES = {
    PlanningPin.class, PlanningPinToIndex.class
  };

  private EntityDescriptorValidator() {}

  /**
   * Mixed inheritance is not permitted. Therefore, inheritance must consist only of classes or only
   * of interfaces.
   */
  public static void assertNotMixedInheritance(
      Class<?> entityClass, List<Class<?>> declaredInheritedEntityClassList) {
  ...
  public static void assertSingleInheritance(
      Class<?> entityClass, List<Class<?>> declaredInheritedEntityClassList) {
  ...
  /**
   * If a class declares any variable (genuine or shadow), it must be annotated as an entity, even
   * if a supertype already has the annotation.
   */
  public static void assertValidPlanningVariables(Class<?> clazz) {
    // We first check the entity class
    if (clazz.getAnnotation(PlanningEntity.class) == null
        && hasAnyGenuineOrShadowVariables(clazz)) {
      var planningVariables =
          extractPlanningVariables(clazz).stream().map(Member::getName).toList();
      throw new IllegalStateException(
          """
    ...
    for (var otherClazz : classList) {
      if (otherClazz != null
          && otherClazz.getAnnotation(PlanningEntity.class) == null
          && hasAnyGenuineOrShadowVariables(otherClazz)) {
        var planningVariables =
            extractPlanningVariables(otherClazz).stream().map(Member::getName).toList();
        throw new IllegalStateException(
            """
                                The class %s is not annotated with @PlanningEntity but defines genuine or shadow variables.
                                Maybe annotate %s with @PlanningEntity.
  ...
  private static List<Member> extractPlanningVariables(Class<?> entityClass) {
    ...
    return membersList.stream()
        .filter(
            member ->
                ConfigUtils.extractAnnotationClass(member, VARIABLE_ANNOTATION_CLASSES) != null
                    || ConfigUtils.extractAnnotationClass(
                            member, ADDITIONAL_VARIABLE_ANNOTATION_CLASSES)
                        != null)
        .toList();
  }
  ...
  public static boolean isEntityClass(Class<?> clazz) {
    return clazz.getAnnotation(PlanningEntity.class) != null
        || clazz.getSuperclass() != null
            && clazz.getSuperclass().getAnnotation(PlanningEntity.class) != null
        || Arrays.stream(clazz.getInterfaces())
            .anyMatch(i -> i.getAnnotation(PlanningEntity.class) != null);
  }
}
