...
public final class FixedVariableReferenceGraph<Solution_>
    extends AbstractVariableReferenceGraph<
        Solution_, PriorityQueue<BaseTopologicalOrderGraph.NodeTopologicalOrder>> {
  // These are immutable
  ...
  public FixedVariableReferenceGraph(
    ...
    for (var node = 0; node < nodeList.size(); node++) {
      var finalNode = node;
      cachedComponentForwardEdges[node] =
          StreamSupport.intStream(
                  () -> Spliterators.spliterator(graph.nodeForwardEdges(finalNode), 0, 0), 0, false)
              .toArray();
      ...
      if (variableReference.groupEntities() != null) {
        for (var groupEntity : variableReference.groupEntities()) {
          entityConsistencyState.setEntityIsInconsistent(
              changedVariableNotifier, groupEntity, false);
        }
      } else {
        for (var shadowEntity : outerGraph.entityToEntityId.keySet()) {
          if (variableReference
            ...
            entityConsistencyState.setEntityIsInconsistent(
                changedVariableNotifier, shadowEntity, false);
          }
        }
      }
    }
  ...
  @Override
  protected PriorityQueue<BaseTopologicalOrderGraph.NodeTopologicalOrder> createChangeTracker(
      int instanceCount) {
  ...
  void innerUpdateChanged() {
    ...
    while (!changeTracker.isEmpty()) {
      ...
      for (var shadowVariableReference : shadowVariableReferences) {
        var isVariableChanged =
            shadowVariableReference.updateIfChanged(entity, changedVariableNotifier);
