...
import static ai.greycos.solver.core.impl.score.ScoreUtil.parseLevelAsInt;
import static ai.greycos.solver.core.impl.score.ScoreUtil.parseScoreTokens;
...
/**
 * This {@link Score} is based on 2 levels of int constraints: hard and soft. Hard constraints have
...
public final class HardSoftScore implements Score<HardSoftScore> {
  ...
  public HardSoftScore multiply(double multiplicand) {
    return of(
        (int) Math.floor(hardScore * multiplicand), (int) Math.floor(softScore * multiplicand));
  }

  @Override
  public HardSoftScore divide(double divisor) {
    return of((int) Math.floor(hardScore / divisor), (int) Math.floor(softScore / divisor));
  }
