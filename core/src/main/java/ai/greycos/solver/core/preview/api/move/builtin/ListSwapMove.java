...
/**
 * Swaps two elements of a {@link PlanningListVariable list variable}. Each element is identified by
 * an entity instance and an index in that entity's list variable. The swap move has two sides
 * called left and right. The element at {@code leftIndex} in {@code leftEntity}'s list variable is
 * replaced by the element at {@code rightIndex} in {@code rightEntity}'s list variable and vice
 * versa. Left and right entity can be the same instance.
 *
...
public class ListSwapMove<Solution_, Entity_, Value_> extends AbstractMove<Solution_> {
   ...
   * #getLeftValueCache()} and {@link #getRightValueCache()} respectively. Ideally, the method would
   * first be called before the values are changed by the move, so that the {@link #toString()}
   * method shows the original values.
   */
  ...
  /**
   * Create a move that swaps a list variable element at {@code leftEntity.listVariable[leftIndex]}
   * with {@code rightEntity.listVariable[rightIndex]}.
   *
  ...
  public void execute(MutableSolutionView<Solution_> solutionView) {
    solutionView.swapValuesBetweenLists(
        variableMetaModel, leftEntity, leftIndex, rightEntity, rightIndex);
  }
  ...
  public ListSwapMove<Solution_, Entity_, Value_> rebase(Rebaser rebaser) {
    return new ListSwapMove<>(
        variableMetaModel,
        rebaser.rebase(leftEntity),
        leftIndex,
        rebaser.rebase(rightEntity),
        rightIndex);
  }
  ...
  public String toString() {
    return String.format(
        "%s {%s[%d]} <-> %s {%s[%d]}",
        getLeftValueCache(), leftEntity, leftIndex, getRightValueCache(), rightEntity, rightIndex);
  }
}
