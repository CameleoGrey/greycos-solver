...
record KOptAffectedElements(
    int wrappedStartIndex, int wrappedEndIndex, List<Range> affectedMiddleRangeList) {
  ...
  public KOptAffectedElements merge(KOptAffectedElements other) {
        ...
        for (var j = i + 1; j < listSize; j++) {
          ...
          if (leftRange.startInclusive() <= rightRange.endExclusive()) {
            if (rightRange.startInclusive() <= leftRange.endExclusive()) {
              var mergedRange =
                  new Range(
                      Math.min(leftRange.startInclusive(), rightRange.startInclusive()),
    ...
    return new KOptAffectedElements(
        newWrappedStartIndex, newWrappedEndIndex, newAffectedMiddleRangeList);
  }
