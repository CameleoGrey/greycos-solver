...
import ai.greycos.solver.core.api.domain.variable.AbstractVariableListener;
import ai.greycos.solver.core.api.domain.variable.CascadingUpdateShadowVariable;
import ai.greycos.solver.core.api.domain.variable.ListVariableListener;
 ...
 */
public final class CustomShadowVariableDescriptor<Solution_>
    extends ShadowVariableDescriptor<Solution_> {

  private final Map<Class<? extends AbstractVariableListener>, List<VariableDescriptor<Solution_>>>
      listenerClassToSourceDescriptorListMap = new HashMap<>();

  public CustomShadowVariableDescriptor(
      int ordinal,
  ...
  public void linkVariableDescriptors(DescriptorPolicy descriptorPolicy) {
    for (ShadowVariable shadowVariable :
        variableMemberAccessor.getDeclaredAnnotationsByType(ShadowVariable.class)) {
      ...
      throw new IllegalArgumentException(
          """
              ...
              .formatted(
                  ...
                  ShadowVariable.class.getSimpleName(),
                  variableMemberAccessor.getName(),
                  sourceVariableDescriptor.getSimpleEntityAndVariableName(),
                  variableListenerClass,
                  ListVariableListener.class.getSimpleName(),
                  variableListenerClass.getSimpleName(),
                  ListVariableListener.class.getSimpleName()));
    }
    if (!sourceVariableDescriptor.isListVariable()
  ...
  public Iterable<VariableListenerWithSources> buildVariableListeners(SupplyManager supplyManager) {
    return listenerClassToSourceDescriptorListMap.entrySet().stream()
        .map(
            classListEntry -> {
              AbstractVariableListener<Solution_, Object> variableListener =
                  ConfigUtils.newInstance(
                      this::toString, "variableListenerClass", classListEntry.getKey());
              return new VariableListenerWithSources(
                  (variableListener instanceof ListVariableListener<?, ?, ?>)
                      ? new LegacyCustomShadowVariableListVariableListener<>(
                          entityDescriptor.getEntityClass(),
