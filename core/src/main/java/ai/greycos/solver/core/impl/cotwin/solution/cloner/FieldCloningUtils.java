...
final class FieldCloningUtils {
  ...
  private static RuntimeException createExceptionOnRead(
      Object bean, Field field, Throwable rootCause) {
    return new IllegalStateException(
        "The class ("
            + bean.getClass()
            + ") has a field ("
            + field
            + ") which cannot be read to create a planning clone.",
        rootCause);
  }
  ...
  private static RuntimeException createExceptionOnWrite(
      Object bean, Field field, Object value, Throwable rootCause) {
    return new IllegalStateException(
        "The class ("
            + bean.getClass()
            + ") has a field ("
            + field
            + ") which cannot be written with the value ("
            + value
            + ") to create a planning clone.",
        rootCause);
  }
