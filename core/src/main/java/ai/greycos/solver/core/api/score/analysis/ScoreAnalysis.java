 ...
 * one for each constraint. Compared to {@link ScoreExplanation}, this is JSON-friendly and faster
 * to generate.
 *
 ...
 * SolutionManager#analyze(Object)} to obtain instances of this record.
 *
 ...
 *     the {@link ConstraintAnalysis} that describes the impact of that constraint on the overall
 *     score.
 *     <p>Zero-weight constraints are never included, they are excluded from score calculation in
 *     the first place. Otherwise constraints are always included, even if they have no matches,
 ...
 *           ScoreAnalysisFetchPolicy#FETCH_MATCH_COUNT}), constraint diff will only be included if
 *           it has a non-zero match count diff.
 *       <li>Otherwise (when constraint matching is fully enabled with {@link
 *           ScoreAnalysisFetchPolicy#FETCH_ALL}) the constraint diff will not be included if the
 *           diff of its constraint matches is empty. (In other words: when diffing, the analysis
 *           for a particular constraint won't be available if we can guarantee that the constraint
 *           matches are identical in both analyses.)
 *     </ul>
 *     <p>Entries in the map have a stable iteration order; items are ordered first by {@link
...
public record ScoreAnalysis<Score_ extends Score<Score_>>(
        ...
        Stream.concat(constraintMap.keySet().stream(), other.constraintMap.keySet().stream())
            ...
            .flatMap(
                constraintRef -> {
                  ...
                  var diff =
                      ConstraintAnalysis.diff(
                          constraintRef, constraintAnalysis, otherConstraintAnalysis);
                  // The following code implements logic to decide which information the user needs
                  // to see,
                  // and which is information we can safely discard.
                  // This is done so that the diff (which is likely to be serialized into JSON) is
                  // not bloated.
                  ...
                  // Can't use constraint weight; weight diff on the same constraint is zero if
                  // weight unchanged.
                  // Can't use matchCount; matchCount diff can be zero if one match was added and
                  // another removed.
                  ...
                  if (diff.matches() == null) {
                    ...
                    if (diff.matchCount() == 0) {
                      ...
                      // The result would be constraint name + zero weight + zero score + zero match
                      // count.
                      return Stream.empty();
                    } else {
                      return Stream.of(diff);
   ...
   * to identify which constraints cause that score quality. The string is built fresh every time
  ...
  /**
   * Provides a summary of the solution's score analysis using the {@link #summarize(int)} method in
   * a similar way to {@link #summarize()}. It is possible to specify the maximum number of
   * constraint matches to display per constraint by passing the desired limit as an argument.
   *
  ...
  private String buildSummary(int topLimit) {
    ...
    summary.append(
        """
                ...
                """
            .formatted(score));
    Comparator<ConstraintAnalysis<Score_>> constraintsScoreComparator =
        comparing(ConstraintAnalysis::score);
