...
public enum PreviewFeature {
   ...
   * under heavy development, entirely undocumented, and many key features are yet to be delivered.
   * Neither the API nor the feature set are complete, and any part can change or be removed at any
   * time.
   *
   * <p>Neighborhoods will eventually stabilize and be promoted from a research project to a true
   * preview feature. We only expose it now to be able to use it for experimentation and testing. As
   * such, it is an exception to the rule; this preview feature is not finished, and it is not yet
   * ready for feedback.
   */
  NEIGHBORHOODS
}
