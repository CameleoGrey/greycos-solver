...
import ai.greycos.solver.core.impl.phase.event.PhaseLifecycleSupport;
...
public abstract class AbstractPhase<Solution_> implements Phase<Solution_> {
  ...
  /**
   * Whether this phase is nested inside another phase. Nested phases, such as ruin and recreate,
   * must not notify the solver of their starting; otherwise unimproved termination of local search
   * would be reset every time the nested phase starts, effectively disabling this termination.
  ...
  protected void calculateWorkingStepScore(
  ...
  protected <Score_ extends Score<Score_>> void predictWorkingStepScore(
      AbstractStepScope<Solution_> stepScope, Object completedAction) {
    var phaseScope = stepScope.getPhaseScope();
    // There is no need to recalculate the score, but we still need to set it
    phaseScope
        .getSolutionDescriptor()
        .setScore(phaseScope.getWorkingSolution(), stepScope.<Score_>getScore().raw());
    if (assertStepScoreFromScratch) {
  ...
  private static <Solution_> void collectMetrics(AbstractStepScope<Solution_> stepScope) {
    ...
    if (solverScope.isMetricEnabled(SolverMetric.STEP_SCORE)
      ...
      SolverMetricUtil.registerScore(
          SolverMetric.STEP_SCORE, tags, scoreDefinition, tagToScoreLevels, stepScope.getScore());
    }
  }
