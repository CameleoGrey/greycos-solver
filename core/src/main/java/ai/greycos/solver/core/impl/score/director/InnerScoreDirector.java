...
public interface InnerScoreDirector<Solution_, Score_ extends Score<Score_>>
  ...
  /**
   * Sets the {@link PlanningSolution working solution} of the {@link ScoreDirector} to the given
   * {@link PlanningSolution solution}, and force updates all the shadow variables on the given
   * solution.
   *
  ...
  /**
   * Sets the {@link PlanningSolution working solution} of the {@link ScoreDirector} to the given
   * {@link PlanningSolution solution}, without updating any of the shadow variables on the given
   * solution.
   *
   * @param workingSolution never null, must never be the same instance as the best solution.
   */
   ...
   *
   * <p>A phase may not need a move repository at all, such as construction heuristics, which
   * currently does not support Neighborhoods API.
   *
   * <p>Each phase is responsible for calling this method to set its repository, and also calling it
   ...
   * is called. Until that happens, this set accumulates and could possibly act as a memory leak.
   *
   * @return true if the score director can potentially cause a memory leak due to unflushed
   *     changes.
   */
  boolean requiresFlushing();

  /**
   * Inverse shadow variables have a fail-fast for cases where the shadow variable doesn't actually
   * point to its correct inverse. This is very useful to pinpoint improperly initialized solutions.
   *
   * <p>However, {@link SolutionManager#update(Object)} exists precisely for the purpose of
   * initializing solutions. And when this API is used, the fail-fast must not be triggered as it is
   * guaranteed and expected that the inverse relationships will be wrong. In fact, they will be
   * null.
   *
   * <p>For this case and this case only, this method is allowed to return false. All other cases
   * must return true, otherwise a very valuable fail-fast is lost.
   *
   ...
   *
   * <p>Used to assert that skipping {@link #calculateScore()} (when the score is otherwise
   ...
   *     Object#toString()} is included in the exception message
   */
  void assertShadowVariablesAreNotStale(
      InnerScore<Score_> expectedWorkingScore, Object completedAction);

  /**
   * Asserts that if the {@link Score} is calculated for the current {@link PlanningSolution working
   * solution} in a fresh {@link ScoreDirector} (with no incremental calculation residue), it is
   * equal to the parameter {@link Score workingScore}.
   *
   * <p>Furthermore, if the assert fails, a score corruption analysis might be included in the
   * exception message.
   *
   ...
   * Asserts that if the {@link Score} is calculated for the current {@link PlanningSolution working
   * solution} in a fresh {@link ScoreDirector} (with no incremental calculation residue), it is
   * equal to the parameter {@link Score predictedScore}.
   *
   * <p>Furthermore, if the assert fails, a score corruption analysis might be included in the
   * exception message.
   *
  ...
  /**
   * Asserts that if the {@link Score} is calculated for the current {@link PlanningSolution working
   * solution} in the current {@link ScoreDirector} (with incremental calculation residue), it is
   * equal to the parameter {@link Score beforeMoveScore}.
   *
   * <p>Furthermore, if the assert fails, a score corruption analysis might be included in the
   * exception message.
   *
  ...
  default void beforeEntityAdded(Object entity) {
    beforeEntityAdded(
        getSolutionDescriptor().findEntityDescriptorOrFail(entity.getClass()), entity);
  }
