...
public class QueuedEntityPlacerFactory<Solution_>
  ...
  public static <Solution_> QueuedEntityPlacerConfig unfoldNew(
    ...
    var entitySelectorConfig =
        new QueuedEntityPlacerFactory<Solution_>(config).buildEntitySelectorConfig(configPolicy);
    config.setEntitySelectorConfig(entitySelectorConfig);
    var moveSelectorConfigList =
        new ArrayList<MoveSelectorConfig>(templateMoveSelectorConfigList.size());
    config.setMoveSelectorConfigList(moveSelectorConfigList);
    var leafMoveSelectorConfigList =
        new ArrayList<MoveSelectorConfig>(templateMoveSelectorConfigList.size());
  ...
  @Override
  public QueuedEntityPlacer<Solution_> buildEntityPlacer(
      HeuristicConfigPolicy<Solution_> configPolicy) {
    ...
    var moveSelectorConfigList_ =
        getMoveSelectorConfigs(configPolicy, entitySelector, entitySelectorConfig_);
  ...
  private @NonNull List<MoveSelectorConfig> getMoveSelectorConfigs(
      HeuristicConfigPolicy<Solution_> configPolicy,
  ...
  public EntitySelectorConfig buildEntitySelectorConfig(
      HeuristicConfigPolicy<Solution_> configPolicy) {
    var entitySelectorConfig = config.getEntitySelectorConfig();
    if (entitySelectorConfig == null) {
      var entityDescriptor =
          getTheOnlyEntityDescriptorWithBasicVariables(configPolicy.getSolutionDescriptor());
      entitySelectorConfig =
          getDefaultEntitySelectorConfigForEntity(configPolicy, entityDescriptor);
    } else {
      // The default phase configuration generates the entity selector config without an updated
      // version of the configuration policy.
      ...
      entitySelectorConfig =
          deduceEntitySortManner(configPolicy, entityDescriptor, entitySelectorConfig);
    }
