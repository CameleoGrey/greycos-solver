...
/**
 * An iterator for the uni-move stream which returns elements in random order. Implements sampling
 * without replacement via {@link
 * ai.greycos.solver.core.impl.bavet.common.index.UniqueRandomIterator}.
 */
@NullMarked
final class UniRandomMoveIterator<Solution_, A> implements Iterator<Move<Solution_>> {
  ...
  public UniRandomMoveIterator(
      UniMoveStreamContext<Solution_, A> context, RandomGenerator workingRandom) {
  ...
  public boolean hasNext() {
    ...
    if (tupleIterator
        == null) { // Only create a possibly expensive instance when we actually need it.
      tupleIterator =
          context.getDatasetInstance().randomIterator(Objects.requireNonNull(workingRandom));
    }
