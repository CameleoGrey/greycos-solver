...
/**
 * A class representing an int array that is dynamically allocated based on the first set index. The
 * array is only created when the first element is set and is reallocated as needed when lower
...
public final class DynamicIntArray {
  ...
  /**
   * Sets the value at the specified index. If this is the first element, the array is created. If
   * the index is lower than the current firstIndex or higher than the current lastIndex, the array
   * is reallocated with a growth strategy to reduce frequent reallocations.
   *
  ...
  /**
   * Clears the array by setting all values to 0. The array structure is preserved, only the values
   * are reset.
   */
  public void clear() {
  ...
  public enum ClearingStrategy {
    ...
    /**
     * The GC will be allowed to reclaim the array. This means that, on next access, the array will
     * have to be reallocated and gradually resized, possibly leading to excessive GC pressure.
     *
     * <p>This is the default.
     */
    FULL,
    /**
     * The array will not be returned to GC and will be filled with zeros instead. This has no
     * impact on GC, but may result in greater at-rest heap usage than strictly necessary.
     */
    PARTIAL
  }
}
