package ai.greycos.solver.core.config.partitionedsearch;
...
public class PartitionedSearchPhaseConfig extends PhaseConfig<PartitionedSearchPhaseConfig> {
  ...
  public void setSolutionPartitionerCustomProperties(
      @Nullable Map<String, String> solutionPartitionerCustomProperties) {
   ...
   * SolutionPartitioner#splitWorkingSolution(ScoreDirector, Integer)}, because otherwise some
   * partitions would never run (especially with {@link Solver#terminateEarly() asynchronous
   * termination}). If this limit (or {@link Runtime#availableProcessors()}) is lower than the
   * number of partitions, this results in a slower score calculation speed per partition {@link
   * Solver}.
   *
  ...
  public @NonNull PartitionedSearchPhaseConfig withRunnablePartThreadLimit(
      @NonNull String runnablePartThreadLimit) {
    this.setRunnablePartThreadLimit(runnablePartThreadLimit);
    return this;
  }

  public @NonNull PartitionedSearchPhaseConfig withPhaseConfigList(
      @NonNull List<@NonNull PhaseConfig> phaseConfigList) {
    this.setPhaseConfigList(phaseConfigList);
    return this;
  }

  public @NonNull PartitionedSearchPhaseConfig withPhaseConfigs(
      @NonNull PhaseConfig @NonNull ... phaseConfigs) {
    this.setPhaseConfigList(List.of(phaseConfigs));
    return this;
  }

  @Override
  public @NonNull PartitionedSearchPhaseConfig inherit(
      @NonNull PartitionedSearchPhaseConfig inheritedConfig) {
    ...
    runnablePartThreadLimit =
        ConfigUtils.inheritOverwritableProperty(
            runnablePartThreadLimit, inheritedConfig.getRunnablePartThreadLimit());
    phaseConfigList =
