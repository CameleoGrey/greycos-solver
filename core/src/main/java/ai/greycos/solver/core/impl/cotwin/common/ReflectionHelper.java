...
public final class ReflectionHelper {
   ...
   * @return null if the member is neither a field nor a getter method according to the JavaBeans
   *     standard
   */
  ...
  public static Method getDeclaredMethod(
      Class<?> containingClass, String methodName, Class<?>... parameterTypes) {
    var baseClass = containingClass;
  ...
  public static Method getSetterMethod(
      Class<?> containingClass, Class<?> propertyType, String propertyName) {
   ...
   * @return null if it doesn't exist
   */
  public static Method getDeclaredSetterMethod(
      Class<?> containingClass, Class<?> propertyType, String propertyName) {
    String setterName = PROPERTY_MUTATOR_PREFIX + capitalizePropertyName(propertyName);
  ...
  public static boolean isMethodOverwritten(Method parentMethod, Class<?> childClass) {
    ...
    try {
      leafMethod =
          childClass.getDeclaredMethod(parentMethod.getName(), parentMethod.getParameterTypes());
    } catch (NoSuchMethodException e) {
      return false;
    }
  ...
  public static void assertReadMethod(Method readMethod, boolean readMethodWithParameter) {
    ...
    if (readMethodWithParameter && readMethod.getParameterCount() > 1) {
      throw new IllegalStateException(
          "The readMethod (%s) must have only one parameter (%s)."
              .formatted(readMethod, Arrays.toString(readMethod.getParameterTypes())));
    }
    if (readMethod.getReturnType() == void.class) {
      throw new IllegalStateException(
          "The readMethod (%s) must have a non-void return type (%s)."
              .formatted(readMethod, readMethod.getReturnType()));
    }
  }

  public static void assertReadMethod(
      Method readMethod,
