...
public interface UniConstraintStream<A> extends ConstraintStream {
   ...
   * Returns a constraint stream consisting of new tuples, each made of the original fact and one
   * item from that iterable. This may produce a stream with duplicate tuples. See {@link
   ...
   * flatten(Person::getRoles)} on such stream will produce a stream of {@code [(Ann, USER), (Ann,
   * ADMIN), (Beth, USER), (Cathy, ADMIN), (Cathy, AUDITOR)]}.
   *
   ...
   * @param <ResultA_> the type of the last fact in the resulting tuples. It is recommended that
  ...
  /**
   * As defined by {@link #flatten(Function)}, only replacing the only fact in the original tuple by
   * an item from the iterable. This means the resulting stream is a {@link UniConstraintStream},
  ...
  /** As defined by {@link #penalize(Score, ToLongFunction)}, where the match weight is one (1). */
  default <Score_ extends Score<Score_>> @NonNull UniConstraintBuilder<A, Score_> penalize(
      @NonNull Score_ constraintWeight) {
    return penalize(constraintWeight, uniConstantOneLong());
  }
   ...
   *     constraintWeight
   * @see #penalizeBigDecimal(Score, Function) You may use BigDecimal instead of long.
   */
  ...
  /**
   * As defined by {@link #penalize(Score, ToLongFunction)}, with a penalty of type {@link
   * BigDecimal}.
   */
  ...
  /** As defined by {@link #reward(Score, ToLongFunction)}, where the match weight is one (1). */
  default <Score_ extends Score<Score_>> @NonNull UniConstraintBuilder<A, Score_> reward(
      @NonNull Score_ constraintWeight) {
    return reward(constraintWeight, uniConstantOneLong());
  }
   ...
   * @param matchWeigher the result of this function (matchWeight) is multiplied by the
   *     constraintWeight
   * @see #rewardBigDecimal(Score, Function) You may use BigDecimal instead of long.
   */
  <Score_ extends Score<Score_>> @NonNull UniConstraintBuilder<A, Score_> reward(
      @NonNull Score_ constraintWeight, @NonNull ToLongFunction<A> matchWeigher);

  /**
   * As defined by {@link #reward(Score, ToLongFunction)}, with a penalty of type {@link
   * BigDecimal}.
   */
  ...
  default <Score_ extends Score<Score_>> @NonNull UniConstraintBuilder<A, Score_> impact(
      @NonNull Score_ constraintWeight) {
    return impact(constraintWeight, uniConstantOneLong());
  }
   ...
   * @param matchWeigher the result of this function (matchWeight) is multiplied by the
   *     constraintWeight
   * @see #impactBigDecimal(Score, Function) You may use BigDecimal instead of long.
   */
  <Score_ extends Score<Score_>> @NonNull UniConstraintBuilder<A, Score_> impact(
      @NonNull Score_ constraintWeight, @NonNull ToLongFunction<A> matchWeigher);

  /**
   * As defined by {@link #impact(Score, ToLongFunction)}, with an impact of type {@link
   * BigDecimal}.
   */
