...
public abstract class AbstractScorer<Tuple_ extends AbstractTuple>
  ...
  protected RuntimeException createExceptionOnImpact(Tuple_ tuple, Exception cause) {
    return new IllegalStateException(
        "Consequence of a constraint ("
            + weightedScoreImpacter.getContext().getConstraint().getConstraintRef()
            + ") threw an exception processing a tuple ("
            + tuple
            + ").",
        cause);
  }
  ...
  public final String toString() {
    return getClass().getSimpleName()
        ...
        + weightedScoreImpacter.getContext().getConstraintWeight()
        + ")";
  }
}
