...
/**
 * {@link ArrayList}-backed list which allows for a cheap {@link #remove(Entry) removal of an
 ...
 *
 * <p>It uses internal state of the entry to track insertion position of the element. When an
 * element is removed, the underlying collection isn't actually touched; therefore, the insertion
 * position of later elements isn't changed. This position is called a gap. Gaps are removed (the
 * list is compacted) when {@link #forEach(Consumer)} or {@link #asList()} is called. This keeps the
 * overhead low while giving us all benefits of {@link ArrayList}, such as memory efficiency, random
 * access, and fast iteration.
 *
 * <p>This class is very thread-unsafe.
 *
 * @param <T>
 */
@NullMarked
public final class ElementAwareArrayList<T> {
  ...
  private boolean clearIfPossible() {
    if (gapCount > 0
        && lastElementPosition + 1
            == gapCount) { // All positions are gaps. Clear the list entirely.
      forceClear();
      return true;
    }
    return false;
  ...
  /**
   * Performs the given action for each element of the collection until all elements have been
   * processed.
   *
   * @param elementConsumer the action to be performed for each element; mustn't modify the
   *     collection and mustn't throw exceptions, as that'd leave the collection in an inconsistent
   *     state
   */
   ...
   * @param encounteredGaps the number of gaps encountered so far in the iteration, including the
   *     current one
   ...
   */
  private boolean clearTailGapsIfPossible(
      int currentGapPosition, int encounteredGaps, int... indexesToRemove) {
    ...
    if (remainingElements == remainingGaps) { // All remaining elements are gaps; we can stop here.
      ...
      clearGaps(
          indexesToRemove, encounteredGaps - 2); // -2 because current gap is not yet in the array.
      return true;
