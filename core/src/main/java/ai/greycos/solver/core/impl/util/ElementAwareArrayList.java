...
/**
 * {@link AbstractList} implementation which allows for a cheap {@link Entry#remove() removal of an
 * element}, while still providing fast iteration and random access. The order of iteration is
 * guaranteed to be the insertion order. {@code null} is a valid value.
 *
 * <p>It uses internal state of the entry to track insertion position of the element. When an entry
 * is removed, its slot in the underlying collection is replaced with {@code null} (a gap);
 * therefore, the insertion position of later elements isn't changed. Gaps are removed (the list is
 * fully compacted) when {@link #listIterator()} is called. {@link #get(int)} and related
 * index-based operations compact only the prefix up to the requested index. This keeps the overhead
 * low while giving us most benefits of an array-backed list.
 *
 * <p>Primary fast-path methods are {@link #addEntry(Object)} and {@link Entry#remove()}, both run
 * in O(1). All standard {@link List} methods are also available and may run in O(n) or worse.
 *
 * <p>This class is so very not thread safe. {@code modCount} is intentionally not maintained;
 * iteration is not fail-fast (matches {@link ElementAwareLinkedList}).
 *
 * @param <T>
 */
@NullMarked
public final class ElementAwareArrayList<T extends @Nullable Object> extends AbstractList<T> {
  ...
  private static final int RETAIN_THRESHOLD =
      DEFAULT_CAPACITY; // Retain backing array when length <= this.
  ...
  private int firstGapPosition =
      0; // Pessimistic lower bound: positions [0, firstGapPosition) are guaranteed gap-free and
  // positionally compact (logical i == physical i).
  private int size = 0;
  ...
  public Entry addEntry(T element) {
    ...
    if (newPosition
        == entries.length) { // Full (also covers EMPTY_ARRAY); grow on the cold path only.
  ...
  /**
   * Removes all gaps from the list in O(n), preserving insertion order. After this call, {@code
   * gapCount == 0} and every subsequent {@code get(int)} runs in O(1). No-op when the list is
  ...
  private void partialCompact(int rightBoundaryPosition) {
    ...
    var lastNonNullPosition =
        firstGapPosition - 1; // firstGapPosition non-nulls are already in place before us.
    for (var currentPosition = firstGapPosition;
        currentPosition <= lastElementPosition;
        currentPosition++) {
      ...
      if (entry == null) {
        ...
        if (encounteredGaps > 0) {
          ...
          entries[currentPosition] =
              null; // For consistency; the list is never in an invalid state.
        }
        if (lastNonNullPosition == rightBoundaryPosition) {
          ...
          // If that suffix is entirely nulls (equivalent to rightBoundaryPosition == size()-1),
          // trim it now.
  ...
  private void remove(Entry entry) {
    ...
    if (position
        == lastElementPosition) { // Removing the last element; trim and retract trailing gaps.
      lastElementPosition--;
      ...
      if (lastElementPosition
          < 0) { // List now empty: retain a small backing array, free a large one.
   ...
   *
   * @param action the action to be performed for each element; mustn't modify the list and mustn't
   *     throw exceptions, as that'd leave the list in an inconsistent state
   */
  @Override
    ...
    for (var currentPosition = 0; currentPosition <= lastElementPosition; currentPosition++) {
      elementConsumer.accept(
          ((Entry) entries[currentPosition])
              .element); // entries[i] is provably non-null (gapCount==0)
    }
  }

  /**
   * Compacts during iteration. Elements are moved to their new position (if needed) after the
   * consumer is called on them, so that the consumer sees the original insertion order. Gaps end up
   * at the end of the list, which is cleared in one go.
   *
  ...
  public boolean equals(Object o) {
    ...
    return o instanceof List<?> other && this.size() == other.size() && super.equals(other);
  }

  @Override
  public int hashCode() {
    return super
        .hashCode(); // Size not relevant; if sizes differ => lists do not equal => same hashCode is
    // fine.
  }
  ...
  private final class ElementAwareListIterator implements ListIterator<T> {
    ...
    private ElementAwareListIterator(int startingPosition) {
      ...
      if (startingPosition < 0 || startingPosition > currentSize) {
        throw new IndexOutOfBoundsException(
            "The index (%d) must be >= 0 and <= size (%d)."
                .formatted(startingPosition, currentSize));
      }
      // listIterator() compacts before construction ⟹ gapless: logical position == physical
      // position.
    ...
    public void add(T element) {
      ...
      logicalPosition++;
      // Appended entries land at physical lastElementPosition (may exceed logicalPosition when gaps
      // exist);
  ...
  public final class Entry implements ListEntry<T> {
    ...
    private int
        position; // Keeps the element's position in the list; must be kept in sync with its actual
