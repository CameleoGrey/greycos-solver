...
/**
 * Delivered when the {@link PlanningSolution best solution} changes during solving. Delivered in
 * the solver thread (which is the thread that calls {@link Solver#solve}).
 *
...
public class BestSolutionChangedEvent<Solution_> {
  ...
  public BestSolutionChangedEvent(
      @NonNull Solver<Solution_> solver,
      long timeMillisSpent,
      @NonNull Solution_ newBestSolution,
      @NonNull Score newBestScore) {
  ...
  /**
   * @return {@code >= 0}, the amount of millis spent since the {@link Solver} started until {@link
   *     #getNewBestSolution()} was found
   */
   ...
   *
   * <ul>
   *   <li>In real-time planning, not all {@link ProblemChange}s might be processed: check {@link
   *       #isEveryProblemFactChangeProcessed()}.
   *   <li>this {@link PlanningSolution} might be uninitialized: check {@link
   *       #isNewBestSolutionInitialized()}.
   *   <li>this {@link PlanningSolution} might be infeasible: check {@link Score#isFeasible()}.
   * </ul>
   */
   ...
   *
   * <p>This is useful for generic code, which doesn't know the type of the {@link PlanningSolution}
