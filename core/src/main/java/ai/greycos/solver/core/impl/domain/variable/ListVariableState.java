...
final class ListVariableState<Solution_> {
  ...
  private ExternalizedNextPrevElementVariableProcessor<Solution_>
      externalizedPreviousElementProcessor = null;
  private ExternalizedNextPrevElementVariableProcessor<Solution_> externalizedNextElementProcessor =
      null;
  ...
  public void linkShadowVariable(
    ...
    this.externalizedIndexProcessor =
        new ExternalizedIndexVariableProcessor<>(shadowVariableDescriptor);
  }

  public void linkShadowVariable(
      InverseRelationShadowVariableDescriptor<Solution_> shadowVariableDescriptor) {
    this.externalizedInverseProcessor =
        new ExternalizedListInverseVariableProcessor<>(
            shadowVariableDescriptor, sourceVariableDescriptor);
  }
  ...
  public void linkShadowVariable(
    ...
    this.externalizedNextElementProcessor =
        ExternalizedNextPrevElementVariableProcessor.ofNext(shadowVariableDescriptor);
  }

  public void initialize(
      InnerScoreDirector<Solution_, ?> scoreDirector, int initialUnassignedCount) {
    ...
    var shouldUnassignElements =
        !scoreDirector.expectShadowVariablesInCorrectState()
            && (externalizedIndexProcessor != null
                || externalizedInverseProcessor != null
                || externalizedPreviousElementProcessor != null
                || externalizedNextElementProcessor != null);
  ...
  private ChangeType processElementPosition(Object entity, Object element, int index) {
    if (requiresPositionMap) { // Update the position and figure out if it is different from
      ...
      var changeType =
          comparePositions(entity, oldPosition.getEntity(), index, oldPosition.getIndex());
      if (changeType
          .anythingChanged) { // Replace the map value in-place, to avoid a put() on the hot path.
  ...
  private static ChangeType comparePositions(
      Object entity, Object otherEntity, int index, int otherIndex) {
  ...
  public Object getPreviousElement(Object element) {
    if (externalizedPreviousElementProcessor == null) {
      ...
      return sourceVariableDescriptor.getValue(mutablePosition.getEntity()).get(index - 1);
    }
  ...
  /**
   * This class is used to avoid creating a new {@link PositionInList} object every time we need to
   * return a position. The actual value is held in a map and can be updated without doing a put()
   * operation, which is more efficient. The {@link PositionInList} object is only created when it
   * is actually requested, and stored until the next time the mutable state is updated and
   * therefore the cache invalidated.
   */
