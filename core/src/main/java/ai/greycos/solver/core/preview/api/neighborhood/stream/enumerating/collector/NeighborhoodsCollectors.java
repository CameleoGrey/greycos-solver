package ai.greycos.solver.core.preview.api.neighborhood.stream.enumerating.collector;

import java.util.List;
import java.util.function.BiFunction;
import java.util.function.Function;

import ai.greycos.solver.core.impl.neighborhood.stream.enumerating.collector.AndThenBiNeighborhoodsCollector;
import ai.greycos.solver.core.impl.neighborhood.stream.enumerating.collector.AndThenUniNeighborhoodsCollector;
import ai.greycos.solver.core.impl.neighborhood.stream.enumerating.collector.ComposeTwoBiNeighborhoodsCollector;
import ai.greycos.solver.core.impl.neighborhood.stream.enumerating.collector.ComposeTwoUniNeighborhoodsCollector;
import ai.greycos.solver.core.impl.neighborhood.stream.enumerating.collector.ToListBiNeighborhoodsCollector;
import ai.greycos.solver.core.impl.neighborhood.stream.enumerating.collector.ToListUniNeighborhoodsCollector;
...
public final class NeighborhoodsCollectors {
  ...
  /**
   * As defined by {@link #toList(UniNeighborhoodsMapper)}, but using the fact directly without
   * mapping.
   */
  ...
  /**
   * Collects all facts from a {@link UniEnumeratingStream} group into a {@link List}, applying the
   * given mapping function to each before collecting.
   *
   * @param mapper maps each tuple to a single value to collect
   */
  public static <Solution_, A, Mapped_>
      UniNeighborhoodsCollector<Solution_, A, ?, List<Mapped_>> toList(
          UniNeighborhoodsMapper<Solution_, A, Mapped_> mapper) {
    return ToListUniNeighborhoodsCollector.create(mapper);
  }

  /**
   * Collects all facts from a {@link BiEnumeratingStream} group into a {@link List}, applying the
   * given mapping function to each {@code (A, B)} pair before collecting. The mapping function also
   * has access to the working solution via {@link BiNeighborhoodsMapper}.
   *
   * @param mapper maps each tuple to a single value to collect
   */
  public static <Solution_, A, B, Mapped_>
      BiNeighborhoodsCollector<Solution_, A, B, ?, List<Mapped_>> toList(
  ...
  public static <Solution_, A, Intermediate_, Result_>
      UniNeighborhoodsCollector<Solution_, A, ?, Result_> collectAndThen(
          UniNeighborhoodsCollector<Solution_, A, ?, Intermediate_> delegate,
  ...
  public static <Solution_, A, B, Intermediate_, Result_>
      BiNeighborhoodsCollector<Solution_, A, B, ?, Result_> collectAndThen(
          BiNeighborhoodsCollector<Solution_, A, B, ?, Intermediate_> delegate,
          Function<Intermediate_, Result_> mappingFunction) {
    return new AndThenBiNeighborhoodsCollector<>(delegate, mappingFunction);
  }

  /**
   * Composes two {@link UniNeighborhoodsCollector}s into one, combining their results with the
   * given function.
   */
  ...
  /**
   * Composes two {@link BiNeighborhoodsCollector}s into one, combining their results with the given
   * function.
   */
