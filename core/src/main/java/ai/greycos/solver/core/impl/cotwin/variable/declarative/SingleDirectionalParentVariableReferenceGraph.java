...
public final class SingleDirectionalParentVariableReferenceGraph<Solution_>
  ...
  public SingleDirectionalParentVariableReferenceGraph(
      ...
      List<DeclarativeShadowVariableDescriptor<Solution_>>
          sortedDeclarativeShadowVariableDescriptors,
      TopologicalSorter topologicalSorter,
      ChangedVariableNotifier<Solution_> changedVariableNotifier,
      boolean canTerminateEarly,
      Object[] entities) {
    monitoredEntityClass =
        sortedDeclarativeShadowVariableDescriptors.get(0).getEntityDescriptor().getEntityClass();
    sortedVariableUpdaterInfos =
    ...
    var shadowEntities =
        Arrays.stream(entities)
            .filter(monitoredEntityClass::isInstance)
            .sorted(topologicalOrderComparator)
            .toArray();
  ...
  public void afterVariableChanged(VariableMetaModel<?, ?, ?> variableReference, Object entity) {
    if (!isUpdating
        && monitoredSourceVariableSet.contains(variableReference)
        && monitoredEntityClass.isInstance(entity)) {
