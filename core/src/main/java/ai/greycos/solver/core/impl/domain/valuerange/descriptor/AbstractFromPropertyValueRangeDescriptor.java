...
import ai.greycos.solver.core.impl.domain.valuerange.buildin.collection.SetValueRange;
...
public abstract non-sealed class AbstractFromPropertyValueRangeDescriptor<Solution_>
  ...
  protected AbstractFromPropertyValueRangeDescriptor(
      int ordinalId,
    ...
    ValueRangeProvider valueRangeProviderAnnotation =
        memberAccessor.getAnnotation(ValueRangeProvider.class);
    if (valueRangeProviderAnnotation == null) {
      throw new IllegalStateException(
          "The member (%s) must have a valueRangeProviderAnnotation (%s)."
  ...
  private void processValueRangeProviderAnnotation(
    ...
    countable =
        collectionWrapping || arrayWrapping || CountableValueRange.class.isAssignableFrom(type);
  }
  ...
  protected <Value_> CountableValueRange<Value_> processValueRange(
      Object valueRangeObject, Object bean) {
    ...
    if (arrayWrapping) {
      ...
      if (collection instanceof Set<Value_> set) {
        if (!(collection instanceof SortedSet<Value_>
  ...
  private <T> CountableValueRange<T> buildValueRange(Collection<T> valueCollection) {
    if (valueCollection.isEmpty()) {
      ...
      throw new IllegalArgumentException(
          "Impossible state: The collection (%s) must be a Set or a List."
