package ai.greycos.solver.core.impl.neighborhood.stream.enumerating.joiner;
...
/**
 * Combs an array of {@link BiEnumeratingJoiner} instances into a mergedJoiner and a
 * mergedFiltering.
 *
 * @param <A>
 * @param <B>
 */
@NullMarked
public record BiEnumeratingJoinerComber<Solution_, A, B>(
    DefaultBiEnumeratingJoiner<A, B> mergedJoiner,
    @Nullable BiEnumeratingPredicate<Solution_, A, B> mergedFiltering) {

  public static <Solution_, A, B> BiEnumeratingJoinerComber<Solution_, A, B> comb(
    ...
    // Make sure all indexing joiners, if any, come before filtering joiners. This is necessary for
    // performance.
    ...
    DefaultBiEnumeratingJoiner<A, B> mergedJoiner =
        DefaultBiEnumeratingJoiner.merge(defaultJoinerList);
  ...
  private static <Solution_, A, B> @Nullable BiEnumeratingPredicate<Solution_, A, B> mergeFiltering(
      List<BiEnumeratingPredicate<Solution_, A, B>> filteringList) {
    ...
    return switch (filteringList.size()) {
      ...
      default ->
          // Avoid predicate.and() when more than 2 predicates for debugging and potentially
          // performance
