  ...
  private int doComparison(Score moveScore, Number[] moveLevelNumbers) {
    ...
    if (!finalistImprovesUponReference && moveScore.compareTo(referenceScore) < 0) {
      for (var i = 0; i < referenceLevelNumbers.length; i++) {
        var moveIsHigher =
            ((Comparable) moveLevelNumbers[i]).compareTo(referenceLevelNumbers[i]) > 0;
        ...
        if (moveIsHigher) {
          if (finalistIsHigher) {
            ...
            // The move has the first level which is higher while the finalist is lower than the
            // reference
            return 1;
          }
        } else {
          if (finalistIsHigher) {
            // The finalist has the first level which is higher while the move is lower than the
            // reference
            return -1;
          } else {
