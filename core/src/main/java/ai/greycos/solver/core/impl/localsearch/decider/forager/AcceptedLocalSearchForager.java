...
import ai.greycos.solver.core.impl.localsearch.decider.forager.finalist.FinalistPodium;
...
public final class AcceptedLocalSearchForager<Solution_>
  ...
  public AcceptedLocalSearchForager(
      FinalistPodium<Solution_> finalistPodium,
    ...
    if (acceptedCountLimit < 1) {
      throw new IllegalArgumentException(
          "The acceptedCountLimit (" + acceptedCountLimit + ") cannot be negative or zero.");
    }
  ...
  private <Score_ extends Score<Score_>> void checkPickEarly(
    ...
    switch (pickEarlyType) {
      ...
      case FIRST_LAST_STEP_SCORE_IMPROVING:
        var lastStepScore =
            moveScope.getStepScope().getPhaseScope().getLastCompletedStepScope().<Score_>getScore();
