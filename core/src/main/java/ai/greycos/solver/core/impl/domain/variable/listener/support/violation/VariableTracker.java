...
import ai.greycos.solver.core.impl.domain.variable.listener.SourcedBasicVariableListener;
...
/**
 * Tracks variable listener events for a given genuine or shadow variable (except {@link
 * ai.greycos.solver.core.api.domain.variable.PlanningListVariable}).
 */
public class VariableTracker<Solution_>
  ...
  public void beforeChange(
      InnerScoreDirector<Solution_, ?> scoreDirector, BasicVariableChangeEvent<Object> event) {
    ...
    for (var changedVariable : changedVariables) {
      ...
      if (!beforeVariableChangedEntityList.contains(entity)) {
        out.add(
            "Entity ("
                + entity
                + ") is missing a beforeVariableChanged call for variable ("
      ...
      if (!afterVariableChangedEntityList.contains(entity)) {
        out.add(
            "Entity ("
                + entity
                + ") is missing a afterVariableChanged call for variable ("
                + variableDescriptor.getVariableName()
                + ").");
      }
    }
  ...
  /**
   * In order for the {@link VariableTracker} to be registered as a variable listener, it needs to
   * be passed to the {@link InnerScoreDirector#getSupplyManager()}, which requires a {@link
   * Demand}.
   *
   * <p>Unlike most other {@link Demand}s, there will only be one instance of {@link
   * VariableTracker} in the {@link InnerScoreDirector} for each variable.
   */
