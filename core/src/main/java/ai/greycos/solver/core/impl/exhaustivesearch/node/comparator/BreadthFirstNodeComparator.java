package ai.greycos.solver.core.impl.exhaustivesearch.node.comparator;
 ...
 * memory scalability.
 *
 * <p>A typical {@link ScoreBounder}'s {@link ScoreBounder#calculateOptimisticBound(ScoreDirector,
 * InnerScore)} will be weak, which results in horrible performance scalability too.
 */
public class BreadthFirstNodeComparator implements Comparator<ExhaustiveSearchNode> {
  ...
  public int compare(ExhaustiveSearchNode a, ExhaustiveSearchNode b) {
    ...
    // No point to investigating higher parent breadth index first (no impact on the churn on
    // workingSolution)
