...
public final class KOptListMove<Solution_> extends AbstractMove<Solution_> {
  ...
  KOptListMove(
    ...
    if (equivalent2Opts.isEmpty()) {
      ...
      for (var i = 1; i < equivalent2Opts.size(); i++) {
        currentAffectedElements =
            currentAffectedElements.merge(equivalent2Opts.get(i).getAffectedElements());
      }
  ...
  private KOptListMove(
    ...
    if (equivalent2Opts.isEmpty()) {
      ...
      affectedElementsInfo =
          KOptAffectedElements.forMiddleRange(
              0, computeCombinedList(listVariableDescriptor, originalEntities).size());
    } else {
      var currentAffectedElements = equivalent2Opts.get(0).getAffectedElements();
      for (var i = 1; i < equivalent2Opts.size(); i++) {
        currentAffectedElements =
  ...
  protected void doMoveOnGenuineVariables(ScoreDirector<Solution_> scoreDirector) {
    ...
    combinedList.actOnAffectedElements(
        ...
        (entity, start, end) ->
            castScoreDirector.beforeListVariableChanged(
                listVariableDescriptor, entity, start, end));
    ...
    combinedList.actOnAffectedElements(
        ...
        originalEntities,
        (entity, start, end) ->
            castScoreDirector.afterListVariableChanged(listVariableDescriptor, entity, start, end));
  }
  ...
  public boolean isMoveDoable(ScoreDirector<Solution_> scoreDirector) {
    ...
    return combinedList.isElementsFromDelegateInEntityValueRange(
        listVariableDescriptor, valueRangeManager);
  }
  ...
  public Collection<?> getPlanningValues() {
    ...
    if (affectedElementsInfo.wrappedStartIndex() != -1) {
      out.addAll(
          combinedList.subList(affectedElementsInfo.wrappedStartIndex(), combinedList.size()));
    ...
    for (var affectedRange : affectedElementsInfo.affectedMiddleRangeList()) {
      out.addAll(
          combinedList.subList(affectedRange.startInclusive(), affectedRange.endExclusive()));
    }

    return out;
  }

  private <T> void flipSublists(
      List<FlipSublistAction> actions, MultipleDelegateList<T> combinedList, int shiftAmount) {
  ...
  static <Solution_> MultipleDelegateList<?> computeCombinedList(
      ListVariableDescriptor<Solution_> listVariableDescriptor, Object[] entities) {
