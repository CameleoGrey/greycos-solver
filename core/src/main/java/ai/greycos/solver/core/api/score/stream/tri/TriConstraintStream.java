...
import static ai.greycos.solver.core.impl.util.ConstantLambdaUtils.triConstantOne;
import static ai.greycos.solver.core.impl.util.ConstantLambdaUtils.triConstantOneBigDecimal;
import static ai.greycos.solver.core.impl.util.ConstantLambdaUtils.triConstantOneLong;
...
public interface TriConstraintStream<A, B, C> extends ConstraintStream {
  ...
  /**
   * Exhaustively test each tuple of facts against the {@link TriPredicate} and match if {@link
   * TriPredicate#test(Object, Object, Object)} returns true.
   *
   * <p>Important: This is slower and less scalable than {@link
   * BiConstraintStream#join(UniConstraintStream, TriJoiner)} with a proper {@link TriJoiner}
   * predicate (such as {@link Joiners#equal(BiFunction, Function)}, because the latter applies
   ...
   *
   * <p>Important: {@link QuadConstraintStream#filter(QuadPredicate) Filtering} this is slower and
   * less scalable than a {@link #join(UniConstraintStream, QuadJoiner)}, because it doesn't apply
   ...
   */
  default <D> @NonNull QuadConstraintStream<A, B, C, D> join(
      @NonNull UniConstraintStream<D> otherStream) {
    return join(otherStream, new QuadJoiner[0]);
  }

  /**
   * Create a new {@link QuadConstraintStream} for every combination of [A, B] and C for which the
   ...
   * followed by a {@link QuadConstraintStream#filter(QuadPredicate) filter}, because it applies
   * hashing and/or indexing on the properties, so it doesn't create nor checks every combination of
   * [A, B, C] and D.
   *
   * @param <D> the type of the fourth matched fact
   * @return a stream that matches every combination of [A, B, C] and D for which the {@link
   *     QuadJoiner} is true
   */
  default <D> @NonNull QuadConstraintStream<A, B, C, D> join(
      @NonNull UniConstraintStream<D> otherStream, @NonNull QuadJoiner<A, B, C, D> joiner) {
    return join(otherStream, new QuadJoiner[] {joiner});
  }

  /**
   * As defined by {@link #join(Class, QuadJoiner)}. For performance reasons, indexing joiners must
   * be placed before filtering joiners.
   *
   * @param <D> the type of the fourth matched fact
   * @return a stream that matches every combination of [A, B, C] and D for which all the {@link
   *     QuadJoiner joiners} are true
   */
  default <D> @NonNull QuadConstraintStream<A, B, C, D> join(
      @NonNull UniConstraintStream<D> otherStream,
   ...
   *     QuadJoiner joiners} are true
   */
  <D> @NonNull QuadConstraintStream<A, B, C, D> join(
      @NonNull UniConstraintStream<D> otherStream, @NonNull QuadJoiner<A, B, C, D>... joiners);

  /**
   * Create a new {@link QuadConstraintStream} for every combination of [A, B, C] and D.
   *
   * <p>Important: {@link QuadConstraintStream#filter(QuadPredicate)} Filtering} this is slower and
   * less scalable than a {@link #join(Class, QuadJoiner)}, because it doesn't apply hashing and/or
   * indexing on the properties, so it creates and checks every combination of [A, B, C] and D.
   *
   * <p>Note that, if a legacy constraint stream uses {@link ConstraintFactory#from(Class)} as
   * opposed to {@link ConstraintFactory#forEach(Class)}, a different range of D may be selected.
  ...
  /**
   * Create a new {@link QuadConstraintStream} for every combination of [A, B, C] and D for which
   * the {@link QuadJoiner} is true (for the properties it extracts from all facts).
   *
   * <p>Important: This is faster and more scalable than a {@link #join(Class, QuadJoiner) join}
   * followed by a {@link QuadConstraintStream#filter(QuadPredicate) filter}, because it applies
   * hashing and/or indexing on the properties, so it doesn't create nor checks every combination of
   * [A, B, C] and D.
   *
   ...
   * (See {@link ConstraintFactory#from(Class)} Javadoc.)
   *
   * <p>This method is syntactic sugar for {@link #join(UniConstraintStream, QuadJoiner)}.
   *
   * <p>This method has overloaded methods with multiple {@link QuadJoiner} parameters.
   *
   * @param <D> the type of the fourth matched fact
   * @return a stream that matches every combination of [A, B, C] and D for which the {@link
   *     QuadJoiner} is true
   */
  default <D> @NonNull QuadConstraintStream<A, B, C, D> join(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D> joiner) {
   ...
   */
  <D> @NonNull QuadConstraintStream<A, B, C, D> join(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D>... joiners);
  ...
  /**
   * Create a new {@link BiConstraintStream} for every tuple of A, B and C where D exists for which
   * the {@link QuadJoiner} is true (for the properties it extracts from the facts).
   *
   * <p>This method has overloaded methods with multiple {@link QuadJoiner} parameters.
   *
   * <p>Note that, if a legacy constraint stream uses {@link ConstraintFactory#from(Class)} as
   ...
   *
   * @param <D> the type of the fourth matched fact
   * @return a stream that matches every tuple of A, B and C where D exists for which the {@link
   *     QuadJoiner} is true
   */
  default <D> @NonNull TriConstraintStream<A, B, C> ifExists(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D> joiner) {
    return ifExists(otherClass, new QuadJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifExists(Class, QuadJoiner)}. For performance reasons, indexing joiners
   * must be placed before filtering joiners.
   *
   * @param <D> the type of the fourth matched fact
   * @return a stream that matches every tuple of A, B and C where D exists for which the {@link
   *     QuadJoiner}s are true
   */
  default <D> @NonNull TriConstraintStream<A, B, C> ifExists(
      @NonNull Class<D> otherClass,
      @NonNull QuadJoiner<A, B, C, D> joiner1,
   ...
   *     QuadJoiner}s are true
   */
  <D> @NonNull TriConstraintStream<A, B, C> ifExists(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D>... joiners);
  ...
  /**
   * As defined by {@link #ifExists(UniConstraintStream, QuadJoiner)}. For performance reasons,
  ...
  /**
   * Create a new {@link BiConstraintStream} for every tuple of A, B and C where D exists for which
   * the {@link QuadJoiner} is true (for the properties it extracts from the facts). For classes
   ...
   *     QuadJoiner} is true
   */
  default <D> @NonNull TriConstraintStream<A, B, C> ifExistsIncludingUnassigned(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D> joiner) {
    return ifExistsIncludingUnassigned(otherClass, new QuadJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifExistsIncludingUnassigned(Class, QuadJoiner)}. For performance reasons,
   * indexing joiners must be placed before filtering joiners.
   *
   * @param <D> the type of the fourth matched fact
   * @return a stream that matches every tuple of A, B and C where D exists for which the {@link
  ...
  default <D> @NonNull TriConstraintStream<A, B, C> ifExistsIncludingUnassigned(
    ...
    return ifExistsIncludingUnassigned(
        otherClass, new QuadJoiner[] {joiner1, joiner2, joiner3, joiner4});
  }
   ...
   *     QuadJoiner}s are true
   */
  <D> @NonNull TriConstraintStream<A, B, C> ifExistsIncludingUnassigned(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D>... joiners);

  /**
   * Create a new {@link BiConstraintStream} for every tuple of A, B and C where D does not exist
   * for which the {@link QuadJoiner} is true (for the properties it extracts from the facts).
   *
   * <p>This method has overloaded methods with multiple {@link QuadJoiner} parameters.
   *
   * <p>Note that, if a legacy constraint stream uses {@link ConstraintFactory#from(Class)} as
   ...
   */
  default <D> @NonNull TriConstraintStream<A, B, C> ifNotExists(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D> joiner) {
    return ifNotExists(otherClass, new QuadJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifNotExists(Class, QuadJoiner)}. For performance reasons, indexing
   ...
   *     {@link QuadJoiner}s are true
   */
  <D> @NonNull TriConstraintStream<A, B, C> ifNotExists(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D>... joiners);
  ...
  /**
   * As defined by {@link #ifNotExists(UniConstraintStream, QuadJoiner)}. For performance reasons,
   * indexing joiners must be placed before filtering joiners.
   *
   * @param <D> the type of the fourth matched fact
   * @return a stream that matches every tuple of A, B and C where D does not exist for which the
  ...
  /**
   * Create a new {@link BiConstraintStream} for every tuple of A, B and C where D does not exist
   * for which the {@link QuadJoiner} is true (for the properties it extracts from the facts). For
   ...
   *     {@link QuadJoiner} is true
   */
  default <D> @NonNull TriConstraintStream<A, B, C> ifNotExistsIncludingUnassigned(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D> joiner) {
    return ifNotExistsIncludingUnassigned(otherClass, new QuadJoiner[] {joiner});
  }

  /**
   * As defined by {@link #ifNotExistsIncludingUnassigned(Class, QuadJoiner)}. For performance
   ...
   *     {@link QuadJoiner}s are true
   */
  <D> @NonNull TriConstraintStream<A, B, C> ifNotExistsIncludingUnassigned(
      @NonNull Class<D> otherClass, @NonNull QuadJoiner<A, B, C, D>... joiners);
  ...
  /**
   * Convert the {@link TriConstraintStream} to a {@link UniConstraintStream}, containing only a
   * single tuple, the result of applying {@link TriConstraintCollector}.
   *
  ...
  /**
   * Convert the {@link TriConstraintStream} to a {@link BiConstraintStream}, containing only a
   * single tuple, the result of applying two {@link TriConstraintCollector}s.
   *
  ...
  /**
   * Convert the {@link TriConstraintStream} to a {@link TriConstraintStream}, containing only a
   * single tuple, the result of applying three {@link TriConstraintCollector}s.
   *
   * @param collectorA the collector to perform the first grouping operation with See {@link
   *     ConstraintCollectors} for common operations, such as {@code count()}, {@code sum()} and
   *     others.
  ...
  /**
   * Convert the {@link TriConstraintStream} to a {@link QuadConstraintStream}, containing only a
   * single tuple, the result of applying four {@link TriConstraintCollector}s.
   *
   * @param collectorA the collector to perform the first grouping operation with See {@link
   *     ConstraintCollectors} for common operations, such as {@code count()}, {@code sum()} and
   *     others.
  ...
  /**
   * Convert the {@link TriConstraintStream} to a {@link UniConstraintStream}, containing the set of
  ...
  /**
   * Convert the {@link TriConstraintStream} to a {@link BiConstraintStream}, consisting of unique
   * tuples.
   *
   * <p>The first fact is the return value of the group key mapping function, applied on the
   * incoming tuple. The second fact is the return value of a given {@link TriConstraintCollector}
  ...
  /**
   * Convert the {@link TriConstraintStream} to a {@link TriConstraintStream}, consisting of unique
   * tuples with three facts.
   *
   * <p>The first fact is the return value of the group key mapping function, applied on the
   * incoming tuple. The remaining facts are the return value of the respective {@link
   * TriConstraintCollector} applied on all incoming tuples with the same first fact.
   *
   * @param groupKeyMapping function to convert the fact in the original tuple to a different fact
  ...
  /**
   * Combines the semantics of {@link #groupBy(TriFunction, TriFunction)} and {@link
   * #groupBy(TriConstraintCollector)}. That is, the first and second facts in the tuple follow the
   * {@link #groupBy(TriFunction, TriFunction)} semantics, and the third fact is the result of
   * applying {@link TriConstraintCollector#finisher()} on all the tuples of the original {@link
   ...
   * Combines the semantics of {@link #groupBy(TriFunction, TriFunction)} and {@link
   * #groupBy(TriConstraintCollector)}. That is, the first and second facts in the tuple follow the
   * {@link #groupBy(TriFunction, TriFunction)} semantics. The third fact is the result of applying
   * the first {@link TriConstraintCollector#finisher()} on all the tuples of the original {@link
   * TriConstraintStream} that belong to the group. The fourth fact is the result of applying the
   * second {@link TriConstraintCollector#finisher()} on all the tuples of the original {@link
   * TriConstraintStream} that belong to the group
   *
   * @param groupKeyAMapping function to convert the original tuple into a first fact
   * @param groupKeyBMapping function to convert the original tuple into a second fact
  ...
  /**
   * Combines the semantics of {@link #groupBy(TriFunction, TriFunction)} and {@link
   * #groupBy(TriConstraintCollector)}. That is, the first three facts in the tuple follow the
   * {@link #groupBy(TriFunction, TriFunction)} semantics. The final fact is the result of applying
   * the first {@link TriConstraintCollector#finisher()} on all the tuples of the original {@link
   * TriConstraintStream} that belong to the group.
   *
   * @param groupKeyAMapping function to convert the original tuple into a first fact
  ...
  <ResultA_> @NonNull UniConstraintStream<ResultA_> map(
      @NonNull TriFunction<A, B, C, ResultA_> mapping);
  ...
  /**
   * Removes duplicate tuples from the stream, according to the tuple's facts {@link
   * Object#equals(Object) equals}/{@link Object#hashCode() hashCode} methods, such that only
   * distinct tuples remain. (No two tuples will {@link Object#equals(Object) equal}.)
   *
   * <p>By default, tuples going through a constraint stream are distinct. However, operations such
   * as {@link #map(TriFunction)} may create a stream which breaks that promise. By calling this
   ...
   * TriConstraintStream} and the provided {@link UniConstraintStream}. The {@link
   * UniConstraintStream} tuples will be padded from the right by null.
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3)]}
   * and the other stream consists of {@code [C, D, E]}, {@code this.concat(other)} will consist of
   * {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3), (C, null), (D, null), (E, null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   */
  default @NonNull TriConstraintStream<A, B, C> concat(
   ...
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3)]}
   * and the other stream consists of {@code [C, D, E]}, {@code this.concat(other, a -> null, a ->
   * null)} will consist of {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3), (C, null), (D, null),
   * (E, null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   *
   ...
   */
  @NonNull TriConstraintStream<A, B, C> concat(
      @NonNull UniConstraintStream<A> otherStream,
  ...
  /**
   * Returns a new {@link TriConstraintStream} containing all the tuples of both this {@link
   * TriConstraintStream} and the provided {@link BiConstraintStream}. The {@link
   * BiConstraintStream} tuples will be padded from the right by null.
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3)]}
   * and the other stream consists of {@code [(C1, C2), (D1, D2), (E1, E2)]}, {@code
   * this.concat(other)} will consist of {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3), (C1, C2,
   * null), (D1, D2, null), (E1, E2, null)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   */
  default @NonNull TriConstraintStream<A, B, C> concat(
   ...
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3)]}
   * and the other stream consists of {@code [(C1, C2), (D1, D2), (E1, E2)]}, {@code
   * this.concat(other, (a, b) -> null)} will consist of {@code [(A1, A2, A3), (B1, B2, B3), (C1,
   ...
   */
  @NonNull TriConstraintStream<A, B, C> concat(
      @NonNull BiConstraintStream<A, B> otherStream, @NonNull BiFunction<A, B, C> paddingFunctionC);

  /**
   * Returns a new {@link TriConstraintStream} containing all the tuples of both this {@link
   * TriConstraintStream} and the provided {@link TriConstraintStream}. Tuples in both this {@link
   * TriConstraintStream} and the provided {@link TriConstraintStream} will appear at least twice.
   *
   * <p>For instance, if this stream consists of {@code [(A, 1, -1), (B, 2, -2), (C, 3, -3)]} and
   * the other stream consists of {@code [(C, 3, -3), (D, 4, -4), (E, 5, -5)]}, {@code
   * this.concat(other)} will consist of {@code [(A, 1, -1), (B, 2, -2), (C, 3, -3), (C, 3, -3), (D,
   * 4, -4), (E, 5, -5)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   */
   ...
   * TriConstraintStream} and the provided {@link QuadConstraintStream}. The {@link
   * TriConstraintStream} tuples will be padded from the right by null.
   *
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3)]}
   * and the other stream consists of {@code [(C1, C2, C3, C4), (D1, D2, D3, D4), (E1, E2, E3,
   * E4)]}, {@code this.concat(other)} will consist of {@code [(A1, A2, A3, null), (B1, B2, B3,
   * null), (C1, C2, C3, null), (C1, C2, C3, C4), (D1, D2, D3, D4), (E1, E2, E3, E4)]}.
   *
   * <p>This operation can be thought of as an or between streams.
   */
  default <D> @NonNull QuadConstraintStream<A, B, C, D> concat(
   ...
   * <p>For instance, if this stream consists of {@code [(A1, A2, A3), (B1, B2, B3), (C1, C2, C3)]}
   * and the other stream consists of {@code [(C1, C2, C3, C4), (D1, D2, D3, D4), (E1, E2, E3,
   * E4)]}, {@code this.concat(other, (a, b, c) -> null)} will consist of {@code [(A1, A2, A3,
   * null), (B1, B2, B3, null), (C1, C2, C3, null), (C1, C2, C3, C4), (D1, D2, D3, D4), (E1, E2, E3,
   * E4)]}. (Assuming that the padding function returns null for the given inputs.)
   *
   * <p>This operation can be thought of as an or between streams.
   *
   * @param paddingFunction function to find the padding for the fourth fact
   */
  <D> @NonNull QuadConstraintStream<A, B, C, D> concat(
      @NonNull QuadConstraintStream<A, B, C, D> otherStream,
   ...
   */
  <ResultD_> @NonNull QuadConstraintStream<A, B, C, ResultD_> expand(
      @NonNull TriFunction<A, B, C, ResultD_> mapping);
  ...
  /**
   * As defined by {@link #complement(Class, Function, Function)}, where the padding function pads
   * with null.
   */
   ...
   * @param paddingFunctionC function to find the padding for the third fact
   */
  default @NonNull TriConstraintStream<A, B, C> complement(
    ...
    if (firstStream instanceof AbstractConstraintStream<?> abstractConstraintStream) {
      ...
      return firstStream.concat(
          secondStream.ifNotExists(remapped, Joiners.equal()), paddingFunctionB, paddingFunctionC);
    } else {
  ...
  /**
   * As defined by {@link #penalizeLong(Score, ToLongTriFunction)}, where the match weight is one
   * (1).
   */
  default <Score_ extends Score<Score_>>
      @NonNull TriConstraintBuilder<A, B, C, Score_> penalizeLong(
          @NonNull Score_ constraintWeight) {
    return penalizeLong(constraintWeight, triConstantOneLong());
  }

  /**
   * As defined by {@link #penalizeBigDecimal(Score, TriFunction)}, where the match weight is one
   * (1).
   */
  default <Score_ extends Score<Score_>>
      @NonNull TriConstraintBuilder<A, B, C, Score_> penalizeBigDecimal(
          @NonNull Score_ constraintWeight) {
   ...
   *
   * <p>For non-int {@link Score} types use {@link #penalizeLong(Score, ToLongTriFunction)} or
   ...
   *     constraintWeight
   */
  <Score_ extends Score<Score_>> @NonNull TriConstraintBuilder<A, B, C, Score_> penalize(
      @NonNull Score_ constraintWeight, @NonNull ToIntTriFunction<A, B, C> matchWeigher);

  /** As defined by {@link #penalize(Score, ToIntTriFunction)}, with a penalty of type long. */
  <Score_ extends Score<Score_>> @NonNull TriConstraintBuilder<A, B, C, Score_> penalizeLong(
      @NonNull Score_ constraintWeight, @NonNull ToLongTriFunction<A, B, C> matchWeigher);

  /**
   * As defined by {@link #penalize(Score, ToIntTriFunction)}, with a penalty of type {@link
   * BigDecimal}.
   */
   ...
   *     constraintWeight
   * @return never null
   * @deprecated Prefer {@link #penalize(Score, ToIntTriFunction)} and {@link
   ...
   *
   * @deprecated Prefer {@link #penalizeLong(Score, ToLongTriFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  TriConstraintBuilder<A, B, C, ?> penalizeConfigurableLong(
      ToLongTriFunction<A, B, C> matchWeigher);

  /**
   * As defined by {@link #penalizeConfigurable(ToIntTriFunction)}, with a penalty of type {@link
   * BigDecimal}.
   *
   * @deprecated Prefer {@link #penalizeBigDecimal(Score, TriFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  TriConstraintBuilder<A, B, C, ?> penalizeConfigurableBigDecimal(
   ...
   * ConstraintWeightOverrides} on the {@link PlanningSolution}-annotated class
   *
   * <p>For non-int {@link Score} types use {@link #rewardLong(Score, ToLongTriFunction)} or {@link
   * #rewardBigDecimal(Score, TriFunction)} instead.
   *
   * @param matchWeigher the result of this function (matchWeight) is multiplied by the
   *     constraintWeight
   */
  <Score_ extends Score<Score_>> @NonNull TriConstraintBuilder<A, B, C, Score_> reward(
      @NonNull Score_ constraintWeight, @NonNull ToIntTriFunction<A, B, C> matchWeigher);

  /** As defined by {@link #reward(Score, ToIntTriFunction)}, with a penalty of type long. */
  <Score_ extends Score<Score_>> @NonNull TriConstraintBuilder<A, B, C, Score_> rewardLong(
      @NonNull Score_ constraintWeight, @NonNull ToLongTriFunction<A, B, C> matchWeigher);

  /**
   * As defined by {@link #reward(Score, ToIntTriFunction)}, with a penalty of type {@link
   * BigDecimal}.
   */
   ...
   *     constraintWeight
   * @return never null
   * @deprecated Prefer {@link #reward(Score, ToIntTriFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
   ...
   *
   * @deprecated Prefer {@link #rewardLong(Score, ToLongTriFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  ...
  /**
   * As defined by {@link #rewardConfigurable(ToIntTriFunction)}, with a penalty of type {@link
   * BigDecimal}.
   *
   * @deprecated Prefer {@link #rewardBigDecimal(Score, TriFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
   ...
   *     constraintWeight
   */
  <Score_ extends Score<Score_>> @NonNull TriConstraintBuilder<A, B, C, Score_> impact(
      @NonNull Score_ constraintWeight, @NonNull ToIntTriFunction<A, B, C> matchWeigher);

  /** As defined by {@link #impact(Score, ToIntTriFunction)}, with an impact of type long. */
  <Score_ extends Score<Score_>> @NonNull TriConstraintBuilder<A, B, C, Score_> impactLong(
      @NonNull Score_ constraintWeight, @NonNull ToLongTriFunction<A, B, C> matchWeigher);

  /**
   * As defined by {@link #impact(Score, ToIntTriFunction)}, with an impact of type {@link
   * BigDecimal}.
   */
   ...
   *
   * @return never null
   * @deprecated Prefer {@link #impact(Score, ToIntTriFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
   ...
   *
   * @deprecated Prefer {@link #impactLong(Score, ToLongTriFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
   ...
   *
   * @deprecated Prefer {@link #impactBigDecimal(Score, TriFunction)} and {@link
   *     ConstraintWeightOverrides}.
   */
  @Deprecated(forRemoval = true, since = "1.13.0")
  TriConstraintBuilder<A, B, C, ?> impactConfigurableBigDecimal(
      TriFunction<A, B, C, BigDecimal> matchWeigher);
  ...
  default <D> TriConstraintStream<A, B, C> ifExistsIncludingNullVars(
      Class<D> otherClass, QuadJoiner<A, B, C, D> joiner) {
    return ifExistsIncludingUnassigned(otherClass, new QuadJoiner[] {joiner});
  }
  ...
  @Deprecated(forRemoval = true, since = "1.8.0")
  default <D> TriConstraintStream<A, B, C> ifExistsIncludingNullVars(
      Class<D> otherClass, QuadJoiner<A, B, C, D> joiner1, QuadJoiner<A, B, C, D> joiner2) {
    return ifExistsIncludingUnassigned(otherClass, new QuadJoiner[] {joiner1, joiner2});
  }
   ...
   */
  @Deprecated(forRemoval = true, since = "1.8.0")
  default <D> TriConstraintStream<A, B, C> ifNotExistsIncludingNullVars(
      Class<D> otherClass, QuadJoiner<A, B, C, D> joiner) {
    return ifNotExistsIncludingUnassigned(otherClass, new QuadJoiner[] {joiner});
  }
   ...
   *
   * <p>For non-int {@link Score} types use {@link #penalizeLong(String, Score, ToLongTriFunction)}
   * or {@link #penalizeBigDecimal(String, Score, TriFunction)} instead.
   *
  ...
  default Constraint penalize(
      String constraintName, Score<?> constraintWeight, ToIntTriFunction<A, B, C> matchWeigher) {
  ...
  default Constraint penalizeLong(
      String constraintName, Score<?> constraintWeight, ToLongTriFunction<A, B, C> matchWeigher) {
   ...
   * ToLongTriFunction)} or {@link #penalizeConfigurableBigDecimal(String, TriFunction)} instead.
   *
   * @param constraintName never null, shows up in {@link ConstraintMatchTotal} during score
   *     justification
   * @param matchWeigher never null, the result of this function (matchWeight) is multiplied by the
  ...
  default Constraint penalizeConfigurable(
      String constraintName, ToIntTriFunction<A, B, C> matchWeigher) {
  ...
  default Constraint penalizeConfigurableLong(
      String constraintName, ToLongTriFunction<A, B, C> matchWeigher) {
  ...
  @Deprecated(forRemoval = true)
  default Constraint penalizeConfigurableLong(
      String constraintPackage, String constraintName, ToLongTriFunction<A, B, C> matchWeigher) {
  ...
  default Constraint penalizeConfigurableBigDecimal(
      String constraintName, TriFunction<A, B, C, BigDecimal> matchWeigher) {
   ...
   *
   * <p>For non-int {@link Score} types use {@link #rewardLong(String, Score, ToLongTriFunction)} or
   ...
   * ToLongTriFunction)} or {@link #rewardConfigurableBigDecimal(String, TriFunction)} instead.
   *
   * @param constraintName never null, shows up in {@link ConstraintMatchTotal} during score
   *     justification
   * @param matchWeigher never null, the result of this function (matchWeight) is multiplied by the
   ...
   * have positive and negative weights.
   *
   * <p>For non-int {@link Score} types use {@link #impactLong(String, Score, ToLongTriFunction)} or
   ...
   * match weight. Otherwise as defined by {@link #impact(String, Score)}.
   *
   * <p>Use {@code penalizeBigDecimal(...)} or {@code rewardBigDecimal(...)} unless you intend to
   ...
   * ToLongTriFunction)} or {@link #impactConfigurableBigDecimal(String, TriFunction)} instead.
   *
   * <p>The constraintWeight comes from an {@link ConstraintWeight} annotated member on the {@link
   * ConstraintConfiguration}, so end users can change the constraint weights dynamically. This
   * constraint may be deactivated if the {@link ConstraintWeight} is zero.
   ...
   * match.
   *
   * <p>Use {@code penalizeConfigurableBigDecimal(...)} or {@code rewardConfigurableLong(...)}
