...
public class SubChainChangeMoveSelector<Solution_> extends GenericMoveSelector<Solution_> {
  ...
  public SubChainChangeMoveSelector(
    ...
    if (subChainSelector.getVariableDescriptor() != valueSelector.getVariableDescriptor()) {
      throw new IllegalStateException(
          "The selector ("
              + this
              + ") has a subChainSelector ("
              + subChainSelector
              + ") with variableDescriptor ("
              + subChainSelector.getVariableDescriptor()
              + ") which is not the same as the valueSelector ("
              + valueSelector
              + ")'s variableDescriptor("
      ...
      if (subChainSelector.isNeverEnding()) {
        throw new IllegalStateException(
            "The selector ("
                ...
                + subChainSelector
                + ") with neverEnding ("
                + subChainSelector.isNeverEnding()
                + ").");
      }
  ...
  private class OriginalSubChainChangeMoveIterator
    ...
    protected Move<Solution_> createUpcomingSelection() {
      ...
      Move<Solution_> upcomingSelection =
  ...
  private class RandomSubChainChangeMoveIterator
    ...
    protected Move<Solution_> createUpcomingSelection() {
      ...
      return reversing
          ? new SubChainReversingChangeMove<>(
              subChain, valueSelector.getVariableDescriptor(), inverseVariableSupply, toValue)
          : new SubChainChangeMove<>(
              subChain, valueSelector.getVariableDescriptor(), inverseVariableSupply, toValue);
    }
  }
