...
public enum GraphStructure {
  ...
  /**
   * A graph structure without dynamic edges. The topological order of such a graph is fixed, since
   * edges are neither added nor removed.
   */
  NO_DYNAMIC_EDGES,

  /**
   * A graph structure where there is at most one directional parent for each graph node, and no
   * indirect parents. For example, when the only input variable from a different entity is
   * previous. This allows us to use a successor function to find affected entities. Since there is
   * at most a single parent node, such a graph cannot be inconsistent.
   */
  SINGLE_DIRECTIONAL_PARENT,

  /**
   * A graph structure that accepts all graphs that only have a single entity that uses declarative
   * shadow variables with all directional parents being the same type.
   */
  ...
  public record GraphStructureAndDirection(
  ...
  public static <Solution_> GraphStructureAndDirection determineGraphStructure(
    ...
    var multipleDeclarativeEntityClasses =
        declarativeShadowVariableDescriptors.stream()
                .map(variable -> variable.getEntityDescriptor().getEntityClass())
                .distinct()
                .count()
            > 1;

    final var arbitraryGraphStructure =
        new GraphStructureAndDirection(
            multipleDeclarativeEntityClasses
                ? ARBITRARY
                : ARBITRARY_SINGLE_ENTITY_AT_MOST_ONE_DIRECTIONAL_PARENT_TYPE,
            null,
            null);

    var rootVariableSources =
        declarativeShadowVariableDescriptors.stream()
        ...
        case GROUP -> {
          ...
          for (var entity : entities) {
            if (variableSource.rootEntity().isInstance(entity)) {
              variableSource
                  .valueEntityFunction()
                  .accept(entity, fromEntity -> groupMemberCount.increment());
            }
          }
          if (groupMemberCount.intValue() != 0) {
            ...
            if (groupParentVariableType != null && groupParentVariableType.isDirectional()) {
              ...
              } else if (!parentMetaModel.equals(
                  variableSource.variableSourceReferences().get(0).variableMetaModel())) {
    ...
    if (directionalType == null) {
      ...
      return new GraphStructureAndDirection(
          SINGLE_DIRECTIONAL_PARENT, parentMetaModel, directionalType);
    }
  }
