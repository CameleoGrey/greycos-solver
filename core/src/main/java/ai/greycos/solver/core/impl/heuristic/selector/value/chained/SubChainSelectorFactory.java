...
public class SubChainSelectorFactory<Solution_> {
  ...
  /**
   * Defaults to 1, even if it partially duplicates {@link ChangeMoveSelectorConfig}, because
   * otherwise the default would not include swapping a pillar of size 1 with another pillar of size
   * 2 or greater.
   */
  ...
  public static <Solution_> SubChainSelectorFactory<Solution_> create(
   ...
   * @return never null
   */
  public SubChainSelector<Solution_> buildSubChainSelector(
    ...
    ValueSelector<Solution_> valueSelector =
        ValueSelectorFactory.<Solution_>create(valueSelectorConfig)
            .buildValueSelector(
                configPolicy, entityDescriptor, minimumCacheType, SelectionOrder.ORIGINAL);
    return new DefaultSubChainSelector<>(
        ...
        Objects.requireNonNullElse(
            config.getMaximumSubChainSize(), DEFAULT_MAXIMUM_SUB_CHAIN_SIZE));
  }
}
