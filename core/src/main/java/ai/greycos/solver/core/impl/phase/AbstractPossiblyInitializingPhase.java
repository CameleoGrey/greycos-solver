...
public abstract class AbstractPossiblyInitializingPhase<Solution_> extends AbstractPhase<Solution_>
  ...
  protected static TerminationStatus translateEarlyTermination(
      AbstractPhaseScope<?> phaseScope,
      @Nullable TerminationStatus earlyTerminationStatus,
    ...
    if (earlyTerminationStatus == null || !hasMoreSteps) {
      ...
      //    This happens when BestScore termination is set to the same score that the last step ends
      // with.
  ...
  protected void ensureCorrectTermination(AbstractPhaseScope<Solution_> phaseScope, Logger logger) {
    ...
    if (!terminationStatus.terminated()) {
      throw new IllegalStateException(
          "Impossible state: construction heuristic phase (%d) ended, but not terminated."
