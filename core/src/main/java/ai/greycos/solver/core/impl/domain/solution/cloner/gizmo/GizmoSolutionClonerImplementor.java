...
import ai.greycos.solver.core.impl.domain.common.accessor.gizmo.GizmoClassLoader;
import ai.greycos.solver.core.impl.domain.common.accessor.gizmo.GizmoMemberDescriptor;
import ai.greycos.solver.core.impl.domain.solution.cloner.DeepCloningUtils;
import ai.greycos.solver.core.impl.domain.solution.cloner.FieldAccessingSolutionCloner;
...
public class GizmoSolutionClonerImplementor {
  ...
  /**
   * Return a comparator that sorts classes into instanceof check order. In particular, if x is a
   * subclass of y, then x will appear earlier than y in the list.
   *
   * @param deepClonedClassSet The set of classes to generate a comparator for
   * @return A comparator that sorts classes from deepClonedClassSet such that x &lt; y if x is
   *     assignable from y.
   */
  public static Comparator<Class<?>> getInstanceOfComparator(Set<Class<?>> deepClonedClassSet) {
    ...
    deepClonedClassSet.forEach(
        clazz -> {
          if (deepClonedClassSet.stream()
              .allMatch(
                  otherClazz ->
                      clazz.isAssignableFrom(otherClazz) || !otherClazz.isAssignableFrom(clazz))) {
    ...
    while (isChanged) {
      ...
      for (Class<?> clazz : deepClonedClassSet) {
        isChanged |=
            classToSubclassLevel.keySet().stream()
  ...
  protected ClonerDescriptor withFallbackClonerField(ClonerDescriptor clonerDescriptor) {
    return clonerDescriptor.withFallbackClonerField(
  ...
  /**
   * Generates the constructor and implementations of SolutionCloner methods for the given
   * SolutionDescriptor using the given ClassCreator
   */
  public static void defineClonerFor(
    ...
    defineClonerFor(
        GizmoSolutionClonerImplementor::new,
        classCreator,
        solutionDescriptor,
        solutionClassSet,
  ...
  public static void defineClonerFor(
    ...
    var clonerDescriptor =
        new ClonerDescriptor(
            solutionDescriptor,
            memoizedSolutionOrEntityDescriptorMap,
  ...
  public static ClassOutput createClassOutputWithDebuggingCapability(
      Map<String, byte[]> classBytecodeHolder) {
    return new GizmoSolutionClonerClassOutput(classBytecodeHolder);
  }

  static <T> SolutionCloner<T> createClonerFor(
      SolutionDescriptor<T> solutionDescriptor, GizmoClassLoader gizmoClassLoader) {
    ...
    gizmo.class_(
        ...
        classCreator -> {
          ...
          var deepClonedClassSet =
              GizmoCloningUtils.getDeepClonedClasses(solutionDescriptor, Collections.emptyList());
  ...
  private <T> SolutionCloner<T> createInstance(
      String className, ClassLoader gizmoClassLoader, SolutionDescriptor<T> solutionDescriptor) {
    try {
  ...
  protected void createSetSolutionDescriptor(ClonerDescriptor clonerDescriptor) {
    clonerDescriptor.classCreator.method(
        "setSolutionDescriptor",
        methodCreator -> {
          methodCreator.public_();
          methodCreator.returning(void.class);
          var solutionDescriptor =
              methodCreator.parameter("solutionDescriptor", SolutionDescriptor.class);
          methodCreator.body(
              blockCreator -> {
                blockCreator.set(
                    clonerDescriptor.fallbackClonerField,
  ...
  private static void createCloneSolutionRun(
      ...
      Set<Class<?>> solutionClassSet,
      Comparator<Class<?>> instanceOfComparator) {
    var solutionClass = clonerDescriptor.solutionDescriptor.getSolutionClass();

    clonerDescriptor.classCreator.staticMethod(
        "cloneSolutionRun",
        methodCreator -> {
          methodCreator.public_();
          ...
          methodCreator.body(
              blockCreator -> {
                ...
                var maybeClone =
                    blockCreator.localVar(
                        "existingClone",
                        solutionClass,
                        blockCreator.withMap(createdCloneMap).get(thisObj));
                blockCreator.ifNotNull(
                    maybeClone, hasCloneBranch -> hasCloneBranch.return_(maybeClone));
                ...
                for (Class<?> solutionSubclass : sortedSolutionClassList) {
                  ...
                  blockCreator.if_(
                      isSubclass,
                      isExactMatchBranch -> {
                        // Note: it appears Gizmo2 does not have a way to cast expressions, so we
                        // need to
                        //       use an ifInstanceOf to get a casted version
                        //       uncheckedCast does NOT do a checkcast, and will fail class
                        // verification
                        isExactMatchBranch.ifInstanceOf(
                            thisObj,
                            solutionSubclass,
                            (isExactMatchWithCastBranch, castedSolution) -> {
                              var solutionSubclassDescriptor =
                                  clonerDescriptor.memoizedSolutionOrEntityDescriptorMap
                                      .computeIfAbsent(
                                          solutionSubclass,
                                          key ->
                                              new GizmoSolutionOrEntityDescriptor(
                                                  clonerDescriptor.solutionDescriptor,
                                                  solutionSubclass));
                              ...
                              cloneShallowlyClonedFieldsOfObject(
                                  ...
                                  new ClonerMethodDescriptor(
                                      ...
                                      isExactMatchWithCastBranch.localVar(
                                          "cloneQueue",
                                          isExactMatchWithCastBranch.new_(
                                              ConstructorDesc.of(ArrayDeque.class)))),
                                  castedSolution,
                                  clone);
                ...
                var errorBuilder =
                    blockCreator.localVar(
                        "errorMessageBuilder",
                        blockCreator.new_(
                            ConstructorDesc.of(StringBuilder.class, String.class),
                ...
                var errorMsg =
                    blockCreator.invokeVirtual(
                        MethodDesc.of(Object.class, "toString", String.class), errorBuilder);
                var error =
                    blockCreator.new_(
                        ConstructorDesc.of(IllegalArgumentException.class, String.class), errorMsg);
                blockCreator.throw_(error);
              });
        });
  }

  private static void cloneDeepClonedFieldsOfSolution(
      ClonerDescriptor clonerDescriptor,
      GizmoSolutionOrEntityDescriptor solutionSubclassDescriptor,
      BlockCreator isSubclassBranch,
      Var thisObj,
      Var createdCloneMap,
      Var clone) {
    for (Field deeplyClonedField : solutionSubclassDescriptor.getDeepClonedFields()) {
      ...
      var fieldValue =
          isSubclassBranch.localVar(
              deeplyClonedField.getName() + "$Value",
              gizmoMemberDescriptor.readMemberValue(isSubclassBranch, thisObj));
      var cloneValue =
          isSubclassBranch.localVar(
              deeplyClonedField.getName() + "$Clone",
              deeplyClonedField.getType(),
      ...
      writeDeepCloneInstructions(
          ...
          new ClonerMethodDescriptor(
              ...
              true,
              isSubclassBranch.localVar(
                  deeplyClonedField.getName() + "$Queue", isSubclassBranch.new_(ArrayDeque.class))),
          deeplyClonedField,
      ...
      if (!gizmoMemberDescriptor.writeMemberValue(isSubclassBranch, clone, cloneValue)) {
        throw new IllegalStateException(
            "The member (%s) of class (%s) does not have a setter."
                .formatted(
                    gizmoMemberDescriptor.getName(),
                    gizmoMemberDescriptor.getDeclaringClassName()));
      }
    }
  }

  private static void cloneShallowlyClonedFieldsOfObject(
      GizmoSolutionOrEntityDescriptor solutionSubclassDescriptor,
      ClonerDescriptor clonerDescriptor,
      ClonerMethodDescriptor solutionSubclassDescriptor1,
      Var thisObj,
      Var clone) {
    for (GizmoMemberDescriptor shallowlyClonedField :
        solutionSubclassDescriptor.getShallowClonedMemberDescriptors()) {
      writeShallowCloneInstructions(
          clonerDescriptor, solutionSubclassDescriptor1, shallowlyClonedField, thisObj, clone);
    }
  }
  ...
  private static void writeShallowCloneInstructions(
    ...
    try {
      ...
      if (type == null && !isArray) {
        type =
            Class.forName(
                shallowlyClonedField.getTypeName().replace('/', '.'),
                false,
      ...
      if (type != null && !isArray) {
        entitySubclasses =
            clonerDescriptor.deepClonedClassesSortedSet.stream()
                .filter(type::isAssignableFrom)
                .toList();
      }

      var fieldValue =
          clonerMethodDescriptor.blockCreator.localVar(
              shallowlyClonedField.getName() + "$Value",
      ...
      if (!entitySubclasses.isEmpty()) {
        var cloneResultHolder =
            clonerMethodDescriptor.blockCreator.localVar(
                shallowlyClonedField.getName() + "$Clone", type, Const.ofNull(type));
      ...
      if (!shallowlyClonedField.writeMemberValue(
          clonerMethodDescriptor.blockCreator, clone, fieldValue)) {
        throw new IllegalStateException(
            "Field (%s) of class (%s) does not have a setter."
                .formatted(
   ...
   *     Var)
   */
  private static void writeDeepCloneInstructions(
      ClonerDescriptor clonerDescriptor,
      ClonerMethodDescriptor clonerMethodDescriptor,
    ...
    blockCreator.ifNull(
        toClone,
        isNullBranch ->
            isNullBranch.set(cloneResultHolder, Const.ofNull(cloneResultHolder.type())));
    ...
    blockCreator.ifNotNull(
        ...
        isNotNullBranch -> {
          ...
          if (clonerMethodDescriptor
            ...
            writeDeepCloneArrayInstructions(
                clonerDescriptor,
                clonerMethodDescriptor.withBlockCreator(isNotNullBranch),
                deeplyClonedFieldClass,
                toClone,
            ...
            var unknownClassCloneType =
                (DeepCloningUtils.isFieldDeepCloned(
                        clonerMethodDescriptor.entityDescriptor.solutionDescriptor,
                        deeplyClonedField,
  ...
  private static void writeDeepCloneInstructions(
    ...
    blockCreator.ifNull(
        toClone,
        ifNullBranch ->
            ifNullBranch.set(cloneResultHolder, Const.ofNull(cloneResultHolder.type())));
  ...
  private static void writeDeepCloneSolutionInstructions(
    ...
    clonerMethodDescriptor.blockCreator.ifNull(
        toClone,
        isNullBranch ->
            isNullBranch.set(cloneResultHolder, Const.ofNull(cloneResultHolder.type())));
    clonerMethodDescriptor.blockCreator.ifNotNull(
        toClone,
        isNotNullBranch -> {
          var clone =
              isNotNullBranch.invokeStatic(
                  ClassMethodDesc.of(
                      ClassDesc.of(
                          GizmoSolutionClonerFactory.getGeneratedClassName(
  ...
  private static void writeDeepCloneCollectionInstructions(
    ...
    var constructedCollection =
        blockCreator.localVar(
            toClone.name() + "$ConstructedCollection",
            blockCreator.invokeStatic(
                MethodDesc.of(
                    FieldAccessingSolutionCloner.class,
                    "constructCloneCollection",
                    Collection.class,
                    Collection.class),
                toClone));
    checkCastAndAssign(
        blockCreator, deeplyClonedFieldClass, cloneResultHolder, constructedCollection);

    var iterator =
        blockCreator.localVar(
            toClone.name() + "$Iterator", blockCreator.withCollection(toClone).iterator());
    blockCreator.while_(
        ...
        whileLoopBlock -> {
          ...
          if (type instanceof ParameterizedType parameterizedType) {
            ...
            if (elementClassType instanceof Class<?> class1) {
              elementClass = class1;
            } else if (elementClassType
                instanceof ParameterizedType parameterizedElementClassType) {
            ...
            throw new IllegalStateException(
                "Cannot infer element type for Collection type (%s).".formatted(type));
          }
          ...
          var next =
              whileLoopBlock.localVar(
                  toClone.name() + "$Item", whileLoopBlock.withIterator(iterator).next());
          var clonedElement =
              whileLoopBlock.localVar(
                  cloneResultHolder.name() + "$Item", elementClass, Const.ofNull(elementClass));
          writeDeepCloneInstructions(
              clonerDescriptor,
   ...
   * </pre>
   */
  private static void checkCastAndAssign(
      BlockCreator blockCreator,
      Class<?> deeplyClonedFieldClass,
      Var cloneResultHolder,
      Var constructedCollection) {
    blockCreator.ifInstanceOfElse(
        constructedCollection,
        deeplyClonedFieldClass,
        (isInstanceCreator, casted) -> {
          isInstanceCreator.set(cloneResultHolder, casted);
        },
        isNotInstanceCreator -> {
          try {
            var baseMessage =
                isNotInstanceCreator.localVar(
                    "message",
                    Const.of("Constructed type (%s) is not assignable to field type (%s)."));
            // Apparently we need to get the method from String.class so it sees the String[] as
            // varargs?
            var formattedMessage =
                isNotInstanceCreator.invokeVirtual(
                    MethodDesc.of(String.class.getMethod("formatted", Object[].class)),
                    baseMessage,
                    isNotInstanceCreator.newArray(
                        String.class,
                        isNotInstanceCreator
                            .withClass(
                                isNotInstanceCreator.withObject(constructedCollection).getClass_())
                            .getName(),
                        Const.of(deeplyClonedFieldClass.getName())));
            isNotInstanceCreator.throw_(
                isNotInstanceCreator.new_(IllegalStateException.class, formattedMessage));
  ...
  private static void writeDeepCloneMapInstructions(
    ...
    var constructedMap =
        blockCreator.localVar(
            toClone.name() + "$ConstructedMap",
            blockCreator.invokeStatic(
                MethodDesc.of(
                    FieldAccessingSolutionCloner.class, "constructCloneMap", Map.class, Map.class),
                toClone));
    ...
    var iterator =
        blockCreator.localVar(
            toClone.name() + "$EntrySet$Iterator",
    ...
    blockCreator.while_(
        ...
        whileLoopBlock -> {
          ...
          if (type instanceof ParameterizedType parameterizedType) {
            ...
            throw new IllegalStateException(
                "Cannot infer element type for Map type (%s).".formatted(type));
          }

          var entitySubclasses =
              clonerDescriptor.deepClonedClassesSortedSet.stream()
                  .filter(keyClass::isAssignableFrom)
                  .toList();
          var entry =
              whileLoopBlock.localVar(
                  toClone.name() + "$Entry", whileLoopBlock.withIterator(iterator).next());
          var toCloneValue =
              whileLoopBlock.localVar(
                  toClone.name() + "$Value",
                  whileLoopBlock.invokeInterface(
                      MethodDesc.of(Map.Entry.class, "getValue", Object.class), entry));
          var clonedElement =
              whileLoopBlock.localVar(
                  cloneResultHolder.name() + "$Element", elementClass, Const.ofNull(elementClass));
          writeDeepCloneInstructions(
              clonerDescriptor,
              clonerMethodDescriptor.withBlockCreator(whileLoopBlock),
              elementClass,
          ...
          var key =
              whileLoopBlock.localVar(
                  toClone.name() + "$Key",
                  whileLoopBlock.invokeInterface(
                      MethodDesc.of(Map.Entry.class, "getKey", Object.class), entry));
          if (!entitySubclasses.isEmpty()) {
            var keyCloneResultHolder =
                whileLoopBlock.localVar(
                    cloneResultHolder.name() + "$Key", keyClass, Const.ofNull(keyClass));
            writeDeepCloneEntityOrFactInstructions(
                clonerDescriptor,
                clonerMethodDescriptor.withBlockCreator(whileLoopBlock),
                keyClass,
                key,
                keyCloneResultHolder,
                UnhandledCloneType.DEEP);
   ...
   * </pre>
   */
  private static void writeDeepCloneArrayInstructions(
      ClonerDescriptor clonerDescriptor,
      ClonerMethodDescriptor clonerMethodDescriptor,
      Class<?> deeplyClonedFieldClass,
      Var toClone,
    ...
    blockCreator.while_(
        condition -> condition.yield(condition.lt(iterations, arrayLength)),
        whileLoopBlock -> {
          var toCloneElement =
              whileLoopBlock.localVar(toClone.name() + "$Element", toClone.elem(iterations));
          var clonedElement =
              whileLoopBlock.localVar(
                  cloneResultHolder.name() + "$Element",
                  arrayComponent,
   ...
   * </pre>
   */
  private static void writeDeepCloneEntityOrFactInstructions(
      ClonerDescriptor clonerDescriptor,
      ClonerMethodDescriptor clonerMethodDescriptor,
      Class<?> deeplyClonedFieldClass,
      Var toClone,
    ...
    var deepClonedSubclasses =
        clonerDescriptor.deepClonedClassesSortedSet.stream()
            .filter(deeplyClonedFieldClass::isAssignableFrom)
            .filter(
                type ->
                    DeepCloningUtils.isClassDeepCloned(
                        clonerMethodDescriptor.entityDescriptor.getSolutionDescriptor(), type))
            .toList();
    var currentBranch = clonerMethodDescriptor.blockCreator;
    var isHandled =
        currentBranch.localVar(cloneResultHolder.name() + "$IsHandled", Const.of(false));
    // If the field holds an instance of one of the field's declared type's subtypes, clone the
    // subtype instead.
    for (Class<?> deepClonedSubclass : deepClonedSubclasses) {
      currentBranch.ifNot(
          isHandled,
          notHandledBranch ->
              notHandledBranch.ifInstanceOf(
                  toClone,
                  deepClonedSubclass,
                  (isInstanceBranch, castedToClone) -> {
                    var cloneObj =
                        isInstanceBranch.invokeStatic(
                            ClassMethodDesc.of(
                                ClassDesc.of(
                                    GizmoSolutionClonerFactory.getGeneratedClassName(
                                        clonerMethodDescriptor.entityDescriptor
                                            .getSolutionDescriptor())),
   ...
   * </pre>
   */
  protected void handleUnknownClass(
      ClonerDescriptor clonerDescriptor,
      ClonerMethodDescriptor clonerMethodDescriptor,
      Class<?> entityClass,
      Var toClone,
  ...
  private void createDeepCloneHelperMethod(
    ...
    clonerDescriptor.classCreator.staticMethod(
        getEntityHelperMethodName(entityClass),
        methodCreator -> {
              ...
              blockCreator -> {
                var entityDescriptor =
                    clonerDescriptor.memoizedSolutionOrEntityDescriptorMap.computeIfAbsent(
                        ...
                        key ->
                            new GizmoSolutionOrEntityDescriptor(
                                clonerDescriptor.solutionDescriptor, entityClass));
                var maybeClone =
                    blockCreator.localVar(
                        "existingClone", blockCreator.withMap(cloneMap).get(toClone));
                blockCreator.ifNotNull(
                    maybeClone, hasCloneBranch -> hasCloneBranch.return_(maybeClone));
                ...
                handleUnknownClass(
                    clonerDescriptor,
                    new ClonerMethodDescriptor(
                        entityDescriptor, blockCreator, cloneMap, false, cloneQueue),
                    entityClass,
                    toClone,
                    newNoCloneBranch -> {
                      LocalVar cloneObj =
                      ...
                      var clonerMethodDescriptor =
                          new ClonerMethodDescriptor(
                              entityDescriptor, newNoCloneBranch, cloneMap, false, cloneQueue);
                      cloneShallowlyClonedFieldsOfObject(
                          entityDescriptor,
                          clonerDescriptor,
                          clonerMethodDescriptor,
                      ...
                      for (Field deeplyClonedField : entityDescriptor.getDeepClonedFields()) {
                        addDeepCloneFieldInitializerToQueue(
                            clonerDescriptor,
                            clonerMethodDescriptor,
                            deeplyClonedField,
                            toClone,
                      ...
                      newNoCloneBranch.if_(
                          isBottom,
                          bottomObjectBranch ->
                              bottomObjectBranch.while_(
                                  condition ->
                                      condition.yield(
                                          condition.logicalNot(
                                              condition.invokeVirtual(
                                                  MethodDesc.of(
                                                      ArrayDeque.class, "isEmpty", boolean.class),
                                                  cloneQueue))),
                                  queueNotEmptyBlock -> {
                                    var next =
                                        queueNotEmptyBlock.invokeVirtual(
                                            MethodDesc.of(ArrayDeque.class, "pop", Object.class),
                                            cloneQueue);
  ...
  private static void addDeepCloneFieldInitializerToQueue(
      ClonerDescriptor clonerDescriptor,
      ClonerMethodDescriptor clonerMethodDescriptor,
      Field deeplyClonedField,
      Var toClone,
    ...
    var gizmoMemberDescriptor = entityDescriptor.getMemberDescriptorForField(deeplyClonedField);
    ...
    var consumer =
        blockCreator.newAnonymousClass(
            BiConsumer.class,
            consumerClassCreator ->
                consumerClassCreator.method(
                    "accept",
                    consumerMethodCreator -> {
                      consumerMethodCreator.returning(void.class);
                      var innerCloneMapObject =
                          consumerMethodCreator.parameter("cloneMapObject", Object.class);
                      var innerCloneQueueObject =
                          consumerMethodCreator.parameter("cloneQueueObject", Object.class);
                      consumerMethodCreator.body(
                          consumerBlockCreator -> {
                            var innerCloneMap =
                                consumerBlockCreator.localVar("cloneMap", innerCloneMapObject);
                            var innerCloneQueue =
                                consumerBlockCreator.localVar("cloneQueue", innerCloneQueueObject);
                            var subfieldValue =
                                consumerBlockCreator.localVar(
                                    "toClone",
                                    gizmoMemberDescriptor.readMemberValue(
                                        consumerBlockCreator,
                            ...
                            if (!gizmoMemberDescriptor.writeMemberValue(
                                consumerBlockCreator,
    ...
    blockCreator.invokeVirtual(
        MethodDesc.of(ArrayDeque.class, "push", void.class, Object.class), cloneQueue, consumer);
  }

  protected void createAbstractDeepCloneHelperMethod(
      ClonerDescriptor clonerDescriptor, Class<?> entityClass) {
    clonerDescriptor.classCreator.staticMethod(
        getEntityHelperMethodName(entityClass),
        methodCreator -> {
  ...
  protected record ClonerMethodDescriptor(
    ...
    public ClonerMethodDescriptor withBlockCreator(BlockCreator blockCreator) {
      return new ClonerMethodDescriptor(
          entityDescriptor, blockCreator, createdCloneMap, isBottom, cloneQueue);
    }
