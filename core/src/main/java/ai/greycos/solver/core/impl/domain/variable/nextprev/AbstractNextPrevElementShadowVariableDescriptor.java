...
abstract class AbstractNextPrevElementShadowVariableDescriptor<Solution_>
  ...
  protected AbstractNextPrevElementShadowVariableDescriptor(
      int ordinal,
  ...
  private void linkShadowSources(DescriptorPolicy descriptorPolicy) {
    ...
    if (entitiesWithSourceVariable.size() > 1) {
      throw new IllegalArgumentException(
          "The entityClass ("
              ...
              + " A planning variable with the name ("
              + sourceVariableName
              + ") exists on multiple entity classes ("
    ...
    if (variableDescriptor == null) {
      throw new IllegalStateException(
          "Impossible state: variableDescriptor ("
              + variableDescriptor
              + ") is null"
              + " but previous checks indicate that the entityClass ("
              + entitiesWithSourceVariable.get(0)
              + ") has a planning variable with sourceVariableName ("
              + sourceVariableName
              + ").");
    }
    ...
    if (!variableMemberAccessor.getType().equals(sourceVariableDescriptor.getElementType())) {
      throw new IllegalStateException(
          "The entityClass ("
              ...
              + ") which is not the type of elements ("
              + sourceVariableDescriptor.getElementType()
              + ") of the source list variable ("
