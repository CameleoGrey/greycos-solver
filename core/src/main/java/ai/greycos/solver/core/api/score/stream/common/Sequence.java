...
/**
 * Represents a series of consecutive values. For instance, the list [1,2,4,5,6,10] has three
 * sequences: [1,2], [4,5,6], and [10].
 *
...
public interface Sequence<Value_, Difference_ extends Comparable<Difference_>> {
   ...
   */
  @NonNull Value_ getFirstItem();

  /**
   * @return the last item in the sequence.
   */
  @NonNull Value_ getLastItem();
   ...
   */
  @Nullable Break<Value_, Difference_> getPreviousBreak();

  /**
   * @return If this is not the last sequence, the break after it. Otherwise, null.
   */
  @Nullable Break<Value_, Difference_> getNextBreak();

  /**
   * @return items in this sequence; must not be modified by the caller.
   */
  @NonNull Collection<Value_> getItems();
