 ...
 * IndexerBackend} is always at the bottom of the hierarchy, never a parent unless it is the only
 ...
 *
 * <p>Comparison joiners result in a single indexer each, whereas equal joiners will be merged into
 * a single indexer if they are consecutive. In the latter case, a composite keyFunction is created
 * of type {@link Pair}, {@link TriTuple}, {@link Quadruple} or {@link IndexerKey}, based on the
 * length of the composite keyFunction (number of equals joiners in sequence).
 *
 * <ul>
 *   <li>Example 2: For an EQUAL+LESS_THAN joiner, there are two indexers in the chain with
 *       keyFunction length of 1 each.
 *   <li>Example 3: For an LESS_THAN+EQUAL+EQUAL joiner, there are still two indexers, but the
 *       second indexer's keyFunction length is 2.
 *   <li>Example 4: For an LESS_THAN+EQUAL+EQUAL+LESS_THAN joiner, there are three indexers in the
 *       chain, and the middle one's keyFunction length is 2.
 * </ul>
 *
 * @param <Right_>
 */
