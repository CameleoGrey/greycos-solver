...
import static ai.greycos.solver.core.impl.heuristic.selector.list.TriangularNumbers.triangularRoot;
...
final class TriangleElementFactory {
  ...
  TriangleElementFactory(
      int minimumSubListSize, int maximumSubListSize, RandomGenerator workingRandom) {
  ...
  /**
   * Produce next random element of Triangle(listSize) observing the given minimum and maximum
   * subList size.
   *
  ...
  TriangleElement nextElement(int listSize) throws IllegalArgumentException {
    ...
    // The top triangle represents all subLists of size greater or equal to maximum subList size.
    // Remove them all.
    int topTriangleSize =
        listSize <= maximumSubListSize ? 0 : nthTriangle(listSize - maximumSubListSize);
