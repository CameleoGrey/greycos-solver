...
final class DefaultPlanningSolutionDiff<Solution_> implements PlanningSolutionDiff<Solution_> {
  ...
  private final Map<Class<?>, Set<PlanningEntityDiff<Solution_, ?>>> entityDiffMap =
      new LinkedHashMap<>();

  DefaultPlanningSolutionDiff(
  ...
  void addEntityDiff(PlanningEntityDiff<Solution_, ?> entityDiff) {
    entityDiffMap
        .computeIfAbsent(entityDiff.entityMetaModel().type(), k -> new LinkedHashSet<>())
        .add(entityDiff);
  }
  ...
  String toString(int limit) {
    ...
    var entityDiffStringList =
        entityDiffMap.values().stream()
            .flatMap(Set::stream)
    ...
    return """
                ...
                """
        .formatted(
            oldSolution, newSolution, removedEntityString, addedEntityString, entityDiffString);
  }
  ...
  static String entityDiffToString(PlanningEntityDiff<?, ?> entityDiff) {
    ...
    var variableDiffString =
        variableDiffs.stream()
            .map(
                diff ->
                    "    %s (%s): %s -> %s"
                        .formatted(
                            diff.variableMetaModel().name(),
                            diff.variableMetaModel() instanceof GenuineVariableMetaModel<?, ?, ?>
                                ? "genuine"
                                : "shadow",
                            diff.oldValue(),
                            diff.newValue()))
    ...
    return """
                %s:
                %s"""
        .formatted(entity, variableDiffString);
  }
  ...
  public boolean equals(@Nullable Object o) {
    ...
    return Objects.equals(solutionMetaModel, that.solutionMetaModel)
        && Objects.equals(oldSolution, that.oldSolution)
