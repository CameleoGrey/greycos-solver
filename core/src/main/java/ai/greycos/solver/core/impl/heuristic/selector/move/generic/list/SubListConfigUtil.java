...
/**
 * Provides backward-compatible way to handle minimumSubListSize and maximumSubListSize properties
 * used on {@link SubListChangeMoveSelectorConfig} and {@link SubListSwapMoveSelectorConfig}.
 *
 * <p>If the property is used, a warning message is logged. If the corresponding property on a child
 * {@link SubListSelectorConfig} is uninitialized, it will be transferred. Thanks to this, configs
 * that used the properties before they became deprecated will continue working as if they defined
 * the properties at the child selector. The user will be warned about the deprecation.
 *
 * <p>Using both the deprecated and the new property is a mistake and will throw an exception.
 *
 * <p>This class should be removed together with its usages and tests for the property transfer and
 * wrong config detection once the deprecated properties are removed.
 */
final class SubListConfigUtil {
  ...
  private static <Config_> void transferDeprecatedProperty(
    ...
    if (moveSelectorSubListSize != null) {
      LoggerFactory.getLogger(moveSelectorConfig.getClass())
          .warn(
              "{}'s {} property is deprecated. Set {} on the child {}.",
              moveSelectorConfig.getClass().getSimpleName(),
              propertyName,
              propertyName,
              SubListSelectorConfig.class.getSimpleName());
