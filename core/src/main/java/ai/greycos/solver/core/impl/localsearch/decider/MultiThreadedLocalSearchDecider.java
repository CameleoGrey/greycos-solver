...
public class MultiThreadedLocalSearchDecider<Solution_> extends LocalSearchDecider<Solution_> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MultiThreadedLocalSearchDecider.class);
    ...
    protected ExecutorService createThreadPoolExecutor() {
        ExecutorService threadPoolExecutor = Executors.newFixedThreadPool(moveThreadCount, threadFactory);
    ...
    public void decideNextStep(LocalSearchStepScope<Solution_> stepScope) {
        ...
        do {
            ...
            if (hasNextMove) {
                ...
                operationQueue.add(new MoveEvaluationOperation<>(stepIndex, selectMoveIndex,
                    (ai.greycos.solver.core.impl.heuristic.move.Move<Solution_>) move));
                selectMoveIndex++;
                movesInPlay++;
            }
        } while (movesInPlay > 0);
        ...
        if (stepScope.getStep() != null) {
            ...
            var stepOperation = new ApplyStepOperation<>(
                    stepIndex + 1,
                    (ai.greycos.solver.core.impl.heuristic.move.Move<Solution_>) stepScope.getStep(),
    ...
    private boolean forageResult(LocalSearchStepScope<Solution_> stepScope, int stepIndex) {
        ...
        if (!result.isMoveDoable()) {
            ...
            moveScope.setInitializedScore((Score_) result.getScore().raw());
            moveScope.getScoreDirector().incrementCalculationCount();
            boolean accepted = acceptor.isAccepted(moveScope);
            moveScope.setAccepted(accepted);
            LOGGER.trace("{}        Move index ({}), score ({}), accepted ({}), move ({}).",
                    logIndentation,
                    foragingMoveIndex, moveScope.getScore().raw(), moveScope.getAccepted(),
                    foragingMove);
    ...
    public void enableAssertions(EnvironmentMode environmentMode) {
        ...
        if (environmentMode.isFullyAsserted()) {
            assertStepScoreFromScratch = true;
        }
