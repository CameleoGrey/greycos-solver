...
final class KOptListMoveIterator<Solution_, Node_>
  ...
  private @Nullable KOptDescriptor<Node_> pickKOptMove(int k) {
    ...
    if (isNodeEndpointOfList(pickedValues[1]) || isNodeEndpointOfList(pickedValues[2])) {
      return pickKOptMoveRec(
          getValuesOnSelectedEntitiesIterator(pickedValues),
          entityOrderInfo,
          pickedValues,
          2,
          k,
      ...
      return pickKOptMoveRec(
          (Iterator<Node_>) valueSelector.iterator(), entityOrderInfo, pickedValues, 2, k, true);
    }
  }

  private @Nullable KOptDescriptor<Node_> pickKOptMoveRec(
    ...
    while (remainingAttempts > 0) {
      ...
      while (nextRemovedEdgePoint
          ...
          || nextRemovedEdgePoint
              == getNodeSuccessor(newEntityOrderInfo, previousRemovedEdgeEndpoint)
          || isEdgeAlreadyAdded(
              pickedValues, previousRemovedEdgeEndpoint, nextRemovedEdgePoint, pickedSoFar - 2)
          || (isEdgeAlreadyDeleted(
                  pickedValues,
                  nextRemovedEdgePoint,
  ...
  KOptDescriptor<Node_> patchCycles(
      KOptDescriptor<Node_> descriptor,
      EntityOrderInfo entityOrderInfo,
      Node_[] oldRemovedEdges,
      int k) {
    Node_ s1, s2;
    ...
    var currentCycle =
        getShortestCycleIdentifier(
            entityOrderInfo, oldRemovedEdges, cycle, removedEdgeIndexToTourOrder, cycleCount, k);
      ...
      if (cycle[removedEdgeIndexToTourOrder[2 * i]] == currentCycle) {
        ...
        for (s1 = sStart; s1 != sStop; s1 = s2) {
          ...
          var addedEdgeToOtherEndpoint =
              Arrays.copyOf(
                  KOptDescriptor.computeInEdgesForSequentialMove(oldRemovedEdges),
                  removedEdges.length + 2 + (2 * cycleCount));
          for (var newEdge = removedEdges.length;
              newEdge < addedEdgeToOtherEndpoint.length - 2;
              newEdge++) {
            addedEdgeToOtherEndpoint[newEdge] = newEdge + 2;
          }
          addedEdgeToOtherEndpoint[addedEdgeToOtherEndpoint.length - 1] =
              addedEdgeToOtherEndpoint.length - 3;
          addedEdgeToOtherEndpoint[addedEdgeToOtherEndpoint.length - 2] =
              addedEdgeToOtherEndpoint.length - 4;
          var newMove =
              patchCyclesRec(
                  valueIterator,
                  descriptor,
  ...
  KOptDescriptor<Node_> patchCyclesRec(
      Iterator<Node_> valueIterator,
    ...
    while (s3 == getNodePredecessor(entityOrderInfo, s2)
        || s3 == getNodeSuccessor(entityOrderInfo, s2)
        || ((NewCycle =
                findCycleIdentifierForNode(
                    entityOrderInfo,
                    s3,
                    removedEdges,
    ...
    if (isEdgeAlreadyDeleted(removedEdges, s3, getNodePredecessor(entityOrderInfo, s3), k)) {
      ...
      s4 =
          workingRandom.nextBoolean()
              ? getNodeSuccessor(entityOrderInfo, s3)
              : getNodePredecessor(entityOrderInfo, s3);
    }
  ...
  int findCycleIdentifierForNode(
    ...
    for (var i = 1; i < pickedValues.length; i++) {
      if (isMiddleNodeBetween(
          entityOrderInfo, pickedValues[permutation[i - 1]], value, pickedValues[permutation[i]])) {
  ...
  int getShortestCycleIdentifier(
      EntityOrderInfo entityOrderInfo,
      Object[] removeEdgeEndpoints,
      int[] endpointIndexToCycle,
      int[] removeEdgeEndpointIndexToTourOrder,
      int cycleCount,
      int k) {
    int i;
    ...
    for (i = 0; i < 2 * k; i += 2) {
      size[endpointIndexToCycle[removeEdgeEndpointIndexToTourOrder[i]]] +=
          getSegmentSize(
              entityOrderInfo,
              removeEdgeEndpoints[removeEdgeEndpointIndexToTourOrder[i]],
    ...
    if (startIndex <= endIndex) {
      ...
      var totalRouteSize =
          offsets[offsets.length - 1]
              + listVariableDescriptor.getListSize(entities[entities.length - 1]);
