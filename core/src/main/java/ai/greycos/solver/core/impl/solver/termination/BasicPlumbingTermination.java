...
/**
 * Concurrency notes: Condition predicate on ({@link #problemChangeQueue} is not empty or {@link
 * #terminatedEarly} is true).
 */
@NullMarked
public final class BasicPlumbingTermination<Solution_>
  ...
  private final BlockingQueue<ProblemChangeAdapter<Solution_>> problemChangeQueue =
   ...
   * This method is thread-safe.
   *
   * <p>Concurrency note: unblocks {@link #waitForRestartSolverDecision()}.
   *
   ...
   *
   * <p>Concurrency note: Blocks until {@link #problemChangeQueue} is not empty or {@link
   * #terminatedEarly} is true.
   *
   ...
   */
  public synchronized boolean addProblemChanges(
      List<ProblemChangeAdapter<Solution_>> problemChangeList) {
    boolean added = problemChangeQueue.addAll(problemChangeList);
    notifyAll();
    return added;
  }

  public synchronized BlockingQueue<ProblemChangeAdapter<Solution_>>
  ...
  public synchronized boolean isSolverTerminated(SolverScope<Solution_> solverScope) {
    // Destroying a thread pool with solver threads will only cause it to interrupt those solver
    // threads,
