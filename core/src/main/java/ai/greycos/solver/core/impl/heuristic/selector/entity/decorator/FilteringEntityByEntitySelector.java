...
import ai.greycos.solver.core.impl.heuristic.selector.move.generic.SwapMoveSelector;
...
/**
 * The decorator returns a list of reachable entities for a specific entity. It enables the creation
 * of a filtering tier when using entity-provided value ranges, ensuring only valid and reachable
 * entities are returned. An entity is considered reachable to another entity if the assigned values
 * exist within their respective entity value ranges.
 *
 * <p>The decorator can only be applied to basic variables. <code>
 *
 ...
 * </code>
 *
 * <p>Let's consider the following use-cases:
 *
 * <ol>
 *   <li>e1(null) - e2(null): e2 is reachable by e1 because both assigned values are null.
 *   <li>e1(v2) - e2(v1): e2 is not reachable by e1 because its value range does not accept v2.
 *   <li>e2(v1) - e3(v4): e3 is reachable by e2 because e2 accepts v4 and e3 accepts v1.
 * </ol>
 *
 * <p>This node is currently used by the {@link SwapMoveSelector} selector. To explain its
 * functionality, let's consider how moves are generated for the basic swap type. Initially, the
 * swap move selector employs a left entity selector to choose one entity. Then, it uses a right
 * entity selector to select another entity, with the goal of swapping their values.
 *
 * <p>Based on the previously described process and the current goal of this node, we can observe
 * that once an entity is selected using the left selector, the right node can filter out all
 * non-reachable entities and generate a valid move. A move is considered valid only if both
 * entities accept each other's values. The filtering process of invalid entities allows the solver
...
public final class FilteringEntityByEntitySelector<Solution_>
  ...
  public FilteringEntityByEntitySelector(
      EntitySelector<Solution_> childEntitySelector,
      EntitySelector<Solution_> replayingEntitySelector,
  ...
  public void phaseStarted(AbstractPhaseScope<Solution_> phaseScope) {
    ...
    var basicVariableList =
        childEntitySelector.getEntityDescriptor().getGenuineBasicVariableDescriptorList().stream()
    ...
    if (basicVariableList.isEmpty()) {
      throw new IllegalStateException(
          "Impossible state: no basic variable found for the entity %s."
              .formatted(childEntitySelector.getEntityDescriptor().getEntityClass()));
    }
    this.allEntities =
        childEntitySelector.getEntityDescriptor().extractEntities(phaseScope.getWorkingSolution());
  ...
  /**
   * The expected replayed entity corresponds to the selected entity when the replaying selector has
   * the next value. Once it is selected, it will be reused until a new entity is replayed by the
   * recorder selector.
   */
  ...
  public Iterator<Object> endingIterator() {
    return new OriginalFilteringValueRangeIterator<>(
        this::selectReplayedEntity,
        childEntitySelector.endingIterator(),
  ...
  public Iterator<Object> iterator() {
    if (randomSelection) {
      if (basicVariableDescriptors.length == 1) {
        return new SingleVariableRandomFilteringValueRangeIterator<>(
            this::selectReplayedEntity,
            childEntitySelector.iterator(),
            basicVariableDescriptors,
            valueRangeManager,
            reachableValues,
            workingRandom,
  ...
  public ListIterator<Object> listIterator() {
    return new OriginalFilteringValueRangeListIterator<>(
        this::selectReplayedEntity,
        childEntitySelector.listIterator(),
        basicVariableDescriptors,
        valueRangeManager);
  }
  ...
  public ListIterator<Object> listIterator(int index) {
    return new OriginalFilteringValueRangeListIterator<>(
        this::selectReplayedEntity,
        childEntitySelector.listIterator(index),
        basicVariableDescriptors,
        valueRangeManager);
  }
  ...
  private abstract static class AbstractFilteringValueRangeIterator<Solution_, Entity_, Value_>
      implements Iterator<Object> {
    private final Supplier<Entity_> upcomingEntitySupplier;
    ...
    private Entity_ replayedEntity;

    private AbstractFilteringValueRangeIterator(
        Supplier<Entity_> upcomingEntitySupplier,
    ...
    abstract void load(Entity_ replayedEntity);

    /**
     * The other entity is reachable if it accepts all assigned values from the replayed entity, and
     * vice versa.
     */
    boolean isReachable(Entity_ otherEntity) {
      return isReachable(replayedEntity, otherEntity);
    }

    boolean isReachable(Entity_ entity, Entity_ otherEntity) {
      ...
      for (var basicVariableDescriptor : basicVariableDescriptors) {
        if (!isReachable(
            entity,
            basicVariableDescriptor.getValue(entity),
            otherEntity,
            basicVariableDescriptor.getValue(otherEntity),
    ...
    private boolean isReachable(
      ...
      return valueRangeManager
              ...
              .contains(otherValue)
          && valueRangeManager
              .getFromEntity(variableDescriptor.getValueRangeDescriptor(), otherEntity)
              .contains(value);
    }
  }

  private static class OriginalFilteringValueRangeIterator<Solution_, Entity_, Value_>
      extends AbstractFilteringValueRangeIterator<Solution_, Entity_, Value_> {

    private final Iterator<Entity_> entityIterator;
    private Entity_ selected = null;

    private OriginalFilteringValueRangeIterator(
        Supplier<Entity_> upcomingEntitySupplier,
        Iterator<Entity_> entityIterator,
        BasicVariableDescriptor<Solution_>[] basicVariableDescriptors,
        ValueRangeManager<Solution_> valueRangeManager) {
    ...
    @Override
    void load(Entity_ replayedEntity) {
      this.selected = null;
    }
  ...
  private static class OriginalFilteringValueRangeListIterator<Solution_, Entity_, Value_>
    ...
    private OriginalFilteringValueRangeListIterator(
        Supplier<Entity_> upcomingEntitySupplier,
        ListIterator<Entity_> entityIterator,
        BasicVariableDescriptor<Solution_>[] basicVariableDescriptors,
        ValueRangeManager<Solution_> valueRangeManager) {
      this.upcomingEntitySupplier = upcomingEntitySupplier;
      this.entityIterator = entityIterator;
  ...
  private static class SingleVariableRandomFilteringValueRangeIterator<Solution_, Entity_, Value_>
      extends AbstractFilteringValueRangeIterator<Solution_, Entity_, Value_> {
    ...
    private SingleVariableRandomFilteringValueRangeIterator(
        Supplier<Entity_> upcomingEntitySupplier,
        Iterator<Entity_> allEntitiesIterator,
        BasicVariableDescriptor<Solution_>[] basicVariableDescriptors,
        ValueRangeManager<Solution_> valueRangeManager,
    ...
    void load(Entity_ replayedEntity) {
      ...
      var value = basicVariableDescriptor.getValue(replayedEntity);
      if (value != null) {
        ...
        this.oneSideValidation = false;
        this.entityIterator = allEntitiesIterator;
        this.iterationBailoutSize = maxBailoutSize;
      }
    }
    ...
    public Entity_ next() {
      ...
      var bailoutSize = iterationBailoutSize;
      do {
        bailoutSize--;
    ...
    private boolean isReachable(
        Entity_ replayedEntity, Entity_ otherEntity, boolean oneSideValidation) {
      if (oneSideValidation) {
        var otherValue = basicVariableDescriptor.getValue(otherEntity);
        if (otherValue != null) {
          return reachableValues.isEntityReachable((Value_) otherValue, replayedEntity);
        } else {
  ...
  private static class RandomFilteringValueRangeIterator<Solution_, Entity_, Value_>
      extends AbstractFilteringValueRangeIterator<Solution_, Entity_, Value_> {
