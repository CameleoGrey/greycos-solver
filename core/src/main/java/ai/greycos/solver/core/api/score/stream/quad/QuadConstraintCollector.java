 ...
 *     detail)
 * @param <Result_> the type of the fact of the tuple in the destination {@link ConstraintStream} It
 *     is recommended that this type be deeply immutable. Not following this recommendation may lead
 *     to hard-to-debug hashing issues down the stream, especially if this value is ever used as a
 *     group key.
...
public interface QuadConstraintCollector<A, B, C, D, ResultContainer_, Result_> {
   ...
   *
   * @see QuadConstraintCollectorAccumulator The incremental accumulator API.
   */
  QuadConstraintCollectorAccumulator<ResultContainer_, A, B, C, D> accumulator();
