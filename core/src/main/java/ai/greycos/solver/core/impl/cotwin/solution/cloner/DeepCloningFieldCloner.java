...
import ai.greycos.solver.core.impl.cotwin.common.accessor.FieldHandle;
...
final class DeepCloningFieldCloner {
  ...
  public <C> Object clone(SolutionDescriptor<?> solutionDescriptor, C original, C clone) {
    ...
    if (deepClone(
        solutionDescriptor, original.getClass(), originalValue)) { // Defer filling in the field.
  ...
  /**
   * Obtaining the decision on whether or not to deep-clone is expensive. This method exists to
   * cache those computations as much as possible, while maintaining thread-safety.
   *
   * @param solutionDescriptor never null
  ...
  private boolean deepClone(
      SolutionDescriptor<?> solutionDescriptor, Class<?> fieldTypeClass, Object originalValue) {
     ...
     */
    boolean isValueDeepCloned =
        valueDeepCloneDecision.updateAndGet(
                old -> {
                  Class<?> originalClass = originalValue.getClass();
                  if (old == null || old.clz != originalClass) {
                    return new Metadata(
                        originalClass,
                        DeepCloningUtils.isClassDeepCloned(solutionDescriptor, originalClass));
                  } else {
                    return old;
                  }
                })
            .decision;
    if (isValueDeepCloned) { // The value has to be deep-cloned. Does not matter what the field
      // says.
      return true;
    }
    /*
    ...
    if (fieldDeepCloneDecision.get() < 0) {
      fieldDeepCloneDecision.set(
          DeepCloningUtils.isFieldDeepCloned(
                  solutionDescriptor, getFieldHandles().field(), fieldTypeClass)
              ? 1
              : 0);
    }
