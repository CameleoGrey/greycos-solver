...
import ai.greycos.solver.core.impl.score.DefaultScoreManager;

/**
 * A stateless service to help calculate {@link Score}, {@link ConstraintMatchTotal}, {@link
 * Indictment}, etc.
 *
 * <p>To create a ScoreManager, use {@link #create(SolverFactory)}.
 *
...
public interface ScoreManager<Solution_, Score_ extends Score<Score_>> {
  ...
  static <Solution_, Score_ extends Score<Score_>, ProblemId_>
    ...
    return new DefaultScoreManager<>(
        SolutionManager.<Solution_, Score_, ProblemId_>create(solverManager));
  }
  ...
  /**
   * Calculates the {@link Score} of a {@link PlanningSolution} and updates its {@link
   * PlanningScore} member.
   *
   * <p>It is equivalent to calling {@link #update(Object, SolutionUpdatePolicy)} with {@link
   * SolutionUpdatePolicy#UPDATE_SCORE_ONLY}. This policy doesn't update shadow variables, which
   * carries a performance advantage but also brings additional limitations. Please review the
   * {@link SolutionUpdatePolicy} documentation for details.
   *
   ...
   * identify which constraints or planning entities cause that score quality. In case of an {@link
   * Score#isFeasible() infeasible} solution, this can help diagnose the cause of that.
   *
   ...
   *
   * <p>It is equivalent to calling {@link #explain(Object, SolutionUpdatePolicy)} with {@link
   * SolutionUpdatePolicy#UPDATE_SCORE_ONLY}. This policy doesn't update shadow variables, which
   * carries a performance advantage but also brings additional limitations. Please review the
   * {@link SolutionUpdatePolicy} documentation for details.
   *
  ...
  /** As defined by {@link #explain(Object)}, using {@link SolutionUpdatePolicy#UPDATE_ALL}. */
