...
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getFilteringValueRangeSelector;
...
class ListSwapMoveSelectorTest {
  ...
  void originalWithEntityValueRange() {
    ...
    var filteringValueRangeSelector =
        getFilteringValueRangeSelector(
            mimicRecordingValueSelector, mimicRecordingValueSelector, false, true, false);

    var moveSelector =
        new ListSwapMoveSelector<>(mimicRecordingValueSelector, filteringValueRangeSelector, false);
  ...
  void originalWithPinningAndEntityValueRange() {
    ...
    assertAllCodesOfMoveSelectorWithoutSize(
        moveSelector, "1 {A[1]} <-> 3 {C[0]}", "3 {C[0]} <-> 1 {A[1]}");
  }
  ...
  void originalAllowsUnassignedValuesWithEntityValueRange() {
    ...
    assertAllCodesOfMoveSelectorWithoutSize(
        moveSelector, "1 {A[1]} <-> 2 {A[0]}", "2 {A[0]} <-> 1 {A[1]}");
  }
    ...
    var moveSelector =
        new ListSwapMoveSelector<>(
            ...
            mockIterableValueSelector(
                listVariableDescriptor, v2, v3, v2, v3, v2, v3, v1, v1, v1, v1),
            mockIterableValueSelector(
  ...
  void randomWithEntityValueRange() {
    ...
    var mimicRecordingValueSelector =
        getMimicRecordingIterableValueSelector(
            getEntityRangeListVariableDescriptor(scoreDirector),
            v2,
            v1,
            v3,
            v1,
    ...
    var filteringValueRangeSelector =
        getFilteringValueRangeSelector(
            mimicRecordingValueSelector, iterableValueRangeSelector, true, true, true);

    var moveSelector =
        new ListSwapMoveSelector<>(mimicRecordingValueSelector, filteringValueRangeSelector, true);
  ...
  void randomWithEntityValueRangeAndFiltering() {
    ...
    // This test validates a path in the FilteringValueRangeSelector that does not use the
    // OptimizedRandomFilteringValueRangeIterator
    {
      // The mimic recorder selector returns v2
      var mimicRecordingValueSelector =
          getMimicRecordingIterableValueSelector(
              getEntityRangeListVariableDescriptor(scoreDirector), v2, v2);

      // The nonReplaying selector returns only v3
      var iterableValueRangeSelector =
          mockIterableValueSelector(
              getEntityRangeListVariableDescriptor(scoreDirector), v3, v3, v3, v3);

      // Since OptimizedRandomFilteringValueRangeIterator is not used, the values from
      // iterableValueRangeSelector must be accounted
      var filteringValueRangeSelector =
          getFilteringValueRangeSelector(
              mimicRecordingValueSelector, iterableValueRangeSelector, true, true, false);

      var moveSelector =
          new ListSwapMoveSelector<>(
              mimicRecordingValueSelector, filteringValueRangeSelector, true);

      var solverScope = solvingStarted(moveSelector, scoreDirector, new Random(0));
      phaseStarted(solverScope, moveSelector);

      // Generate one move for v2
      assertCodesOfNeverEndingMoveSelector(moveSelector, "2 {A[0]} <-> 3 {B[0]}");
    }
    {
      // The mimic recorder selector returns v1
      var mimicRecordingValueSelector =
          getMimicRecordingIterableValueSelector(
              getEntityRangeListVariableDescriptor(scoreDirector), v1);

      // The value selector will return only v1 and nonReplaying selector will return the value v3,
  ...
  void randomWithPinningAndEntityValueRange() {
    ...
    var filteringValueRangeSelector =
        getFilteringValueRangeSelector(
            mimicRecordingValueSelector, mimicRecordingValueSelector, true, true, true);
    ...
    // over all values.
    assertCodesOfNeverEndingMoveSelector(
        moveSelector, "3 {C[0]} <-> 1 {A[1]}", "1 {A[1]} <-> 3 {C[0]}");
  }
  ...
  void randomAllowsUnassignedValues() {
    ...
    var moveSelector =
        new ListSwapMoveSelector<>(
            ...
            // to be never ending, so they must not be exhausted after the last asserted code.
            mockIterableValueSelector(
                listVariableDescriptor, v2, v3, v4, v2, v3, v4, v2, v3, v4, v1, v1, v1, v1),
            mockIterableValueSelector(
  ...
  void noReachableEntities() {
    ...
    var filteringValueRangeSelector =
        getFilteringValueRangeSelector(
            mimicRecordingValueSelector, mimicRecordingValueSelector, true, true, false);
    ...
    // The iterator is not able to find a reachable entity, but the random iterator will return has
    // next as true
