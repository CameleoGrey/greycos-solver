...
class IterableFromEntityPropertyValueSelectorTest {
  ...
  public void runOriginalSelection(SelectionCacheType cacheType) {
    ...
    var valueSelector =
        new IterableFromEntityPropertyValueSelector<>(fromEntityPropertySelector, cacheType, false);

    var solution = new TestdataEntityProvidingSolution();
    var firstEntity = new TestdataEntityProvidingEntity();
    firstEntity.setValueRange(
        List.of(new TestdataValue("jan"), new TestdataValue("feb"), new TestdataValue("mar")));
    ...
    doReturn(
            ValueRangeManager.of(
                TestdataEntityProvidingSolution.buildSolutionDescriptor(), solution))
        .when(scoreDirector)
