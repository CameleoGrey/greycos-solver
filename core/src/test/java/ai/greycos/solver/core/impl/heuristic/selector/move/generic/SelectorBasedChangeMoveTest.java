...
class SelectorBasedChangeMoveTest {
  ...
  void isMoveDoable() {
    ...
    ScoreDirector<TestdataAllowsUnassignedEntityProvidingSolution> scoreDirector =
        mock(ScoreDirector.class);

    var variableDescriptor =
        TestdataAllowsUnassignedEntityProvidingEntity.buildVariableDescriptorForValue();

    var aMove = new SelectorBasedChangeMove<>(variableDescriptor, a, v2);
    a.setValue(v1);
  ...
  void rebase() {
    ...
    assertSameProperties(
        destinationE1,
        null,
        new SelectorBasedChangeMove<>(variableDescriptor, e1, null)
            .rebase(destinationScoreDirector));
    assertSameProperties(
        destinationE1,
        destinationV1,
        new SelectorBasedChangeMove<>(variableDescriptor, e1, v1).rebase(destinationScoreDirector));
    assertSameProperties(
        destinationE2,
        null,
        new SelectorBasedChangeMove<>(variableDescriptor, e2, null)
            .rebase(destinationScoreDirector));
    assertSameProperties(
        destinationE3,
        destinationV2,
        new SelectorBasedChangeMove<>(variableDescriptor, e3, v2).rebase(destinationScoreDirector));
  }

  void assertSameProperties(
      Object entity, Object toPlanningVariable, SelectorBasedChangeMove<?> move) {
  ...
  @Test
  void getters() {
    var primaryVariableDescriptor = TestdataMultiVarEntity.buildVariableDescriptorForPrimaryValue();
    var primaryMove =
        new SelectorBasedChangeMove<>(
            primaryVariableDescriptor, new TestdataMultiVarEntity("a"), null);
    ...
    var secondaryMove =
        new SelectorBasedChangeMove<>(
            secondaryVariableDescriptor, new TestdataMultiVarEntity("b"), new TestdataValue("1"));
  ...
  void toStringTest() {
    ...
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, a, null))
        .hasToString("a {null -> null}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, a, v1))
        .hasToString("a {null -> v1}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, a, v2))
        .hasToString("a {null -> v2}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, b, null))
        .hasToString("b {v1 -> null}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, b, v1))
        .hasToString("b {v1 -> v1}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, b, v2))
        .hasToString("b {v1 -> v2}");
  }
  ...
  void toStringTestMultiVar() {
    ...
    var variableDescriptor = TestdataMultiVarEntity.buildVariableDescriptorForSecondaryValue();
    ...
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, a, v2))
        .hasToString("a {null -> v2}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, b, null))
        .hasToString("b {v3 -> null}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, b, v1))
        .hasToString("b {v3 -> v1}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, b, v2))
        .hasToString("b {v3 -> v2}");
    assertThat(new SelectorBasedChangeMove<>(variableDescriptor, c, v3))
        .hasToString("c {v4 -> v3}");
  }
