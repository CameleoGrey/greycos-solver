package ai.greycos.solver.core.testdomain.shadow;
...
public class TestdataShadowedIncrementalScoreCalculator
    implements ConstraintMatchAwareIncrementalScoreCalculator<
        TestdataShadowedSolution, SimpleScore> {
  ...
  public void resetWorkingSolution(
      @NonNull TestdataShadowedSolution workingSolution, boolean constraintMatchEnabled) {
        ...
        if (Objects.equals(right.getValue(), value)) {
          var constraintMatch =
              constraintMatchTotal.addConstraintMatch(
                  List.of(left, right), SimpleScore.ONE.negate());
          Stream.of(left, right)
              .forEach(
                  entity ->
                      indictmentMap
                          .computeIfAbsent(
                              entity, key -> new DefaultIndictment<>(key, SimpleScore.ZERO))
