...
class ConcurrentValuesShadowVariableTest {
  ...
  void simpleChain() {
    ...
    SolutionManager.updateShadowVariables(
        TestdataConcurrentSolution.class, entity, value1, value2, value3);
  ...
  private static void assertStartsAfterDuration(
      Duration duration, TestdataConcurrentValue... values) {
    for (var value : values) {
      assertThat(value.getServiceStartTime()).isEqualTo(BASE_START_TIME.plus(duration));
      assertThat(value.getServiceFinishTime())
          .isEqualTo(BASE_START_TIME.plus(duration).plusMinutes(30L));
                    ...
                    .withAssertionScoreDirectorFactory(
                        new ScoreDirectorFactoryConfig()
                            .withConstraintProviderClass(
                                TestdataConcurrentConstraintProvider.class)))
                    ...
                    .withAssertionScoreDirectorFactory(
                        new ScoreDirectorFactoryConfig()
                            .withEasyScoreCalculatorClass(
                                TestdataConcurrentAssertionEasyScoreCalculator.class)))
            .withTerminationConfig(new TerminationConfig().withMoveCountLimit(1000L));
