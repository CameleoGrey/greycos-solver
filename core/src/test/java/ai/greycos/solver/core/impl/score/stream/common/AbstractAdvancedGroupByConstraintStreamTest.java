...
import static ai.greycos.solver.core.api.score.stream.ConstraintCollectors.sum;
import static ai.greycos.solver.core.api.score.stream.ConstraintCollectors.toMap;
...
public abstract class AbstractAdvancedGroupByConstraintStreamTest
  ...
  void collectedAndFiltered() {
    ...
    assertScore(
        scoreDirector); // There is less than 10 entities, and therefore there are no penalties.
  }
  ...
  void uniGroupByRecollected() {
    ...
    assertScore(scoreDirector, assertMatchWithScore(-1, asSet(entity2.getEntityGroup())));
  }
            ...
            factory ->
                factory
                    ...
                    .join(
                        TestdataLavishEntity.class,
                        equal(
                            (a, b, c) -> a.getEntityGroup(), TestdataLavishEntity::getEntityGroup),
                        filtering(
                            (a, b, c, d) ->
                                !Objects.equals(a, d)
                                    && !Objects.equals(b, d)
  ...
  void filteredFromUniquePair() {
    ...
    // From scratch
    scoreDirector.setWorkingSolution(solution);
    assertScore(scoreDirector, assertMatchWithScore(-1, entity3, solution.getFirstEntity()));
  ...
  void reusedStreamsInJoin() { // PLANNER-2884
    ...
    ConstraintProvider cp =
        constraintFactory -> {
          UniConstraintStream<TestdataLavishEntity> reusedStream =
              constraintFactory
                  .forEach(TestdataLavishEntity.class)
