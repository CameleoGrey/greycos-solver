...
public class TestdataConcurrentValue {
  public static final LocalDateTime BASE_START_TIME =
      LocalDate.of(2025, 1, 1).atTime(LocalTime.of(9, 0));
    ...
    if (concurrentValueGroup != null) {
      for (var visit : concurrentValueGroup) {
        if (visit.isInconsistent) {
          throw new IllegalStateException(
              "Value (%s) is inconsistent when serviceStartTimeUpdater is called."
                  .formatted(visit));
        }
  ...
  boolean getExpectedInconsistent(Map<TestdataConcurrentValue, Boolean> cache) {
    ...
    if (concurrentValueGroup != null) {
      ...
      for (var visit : concurrentValueGroup) {
        ...
        if (visit != this
            && visit.previousValue != null
            && visit.previousValue.getExpectedInconsistent(cache)) {
          return true;
        }
      }
    }
  ...
  record TimeCache(
      Map<TestdataConcurrentValue, LocalDateTime> readyTimeCache,
  ...
  public LocalDateTime getExpectedServiceStartTime(TimeCache cache) {
    ...
    if (concurrentValueGroup == null) {
      ...
      var out =
          concurrentValueGroup.stream()
              .map(visit -> visit.getExpectedServiceReadyTime(cache))
