...
class SubListSwapMoveTest {
  ...
  void isMoveDoable() {
    ...
    assertThat(
            new SubListSwapMove<>(variableDescriptor, e1, 0, 3, e1, 2, 5, false)
                .isMoveDoable(scoreDirector))
        .isFalse();
    // same entity, overlap => not doable
    assertThat(
            new SubListSwapMove<>(variableDescriptor, e1, 0, 5, e1, 1, 2, false)
                .isMoveDoable(scoreDirector))
        .isFalse();
    // same entity, no overlap (with gap) => doable
    assertThat(
            new SubListSwapMove<>(variableDescriptor, e1, 0, 1, e1, 4, 5, false)
                .isMoveDoable(scoreDirector))
        .isTrue();
    // same entity, no overlap (with touch) => doable
    assertThat(
            new SubListSwapMove<>(variableDescriptor, e1, 0, 3, e1, 3, 5, false)
                .isMoveDoable(scoreDirector))
        .isTrue();
    // same entity, no overlap (with touch, right below left) => doable
    assertThat(
            new SubListSwapMove<>(variableDescriptor, e1, 2, 5, e1, 0, 2, false)
                .isMoveDoable(scoreDirector))
        .isTrue();
    // different entities => doable
    assertThat(
            new SubListSwapMove<>(variableDescriptor, e1, 0, 5, e2, 0, 1, false)
                .isMoveDoable(scoreDirector))
        .isTrue();
  }
  ...
  void doMove() {
    ...
    SubListSwapMove<TestdataListSolution> move =
        new SubListSwapMove<>(variableDescriptor, e1, 1, 3, e2, 0, 1, false);
  ...
  void doReversingMove() {
    ...
    SubListSwapMove<TestdataListSolution> move =
        new SubListSwapMove<>(variableDescriptor, e1, 0, 3, e2, 0, 2, true);
  ...
  void doMoveOnSameEntity() {
    ...
    SubListSwapMove<TestdataListSolution> move =
        new SubListSwapMove<>(variableDescriptor, e1, 0, 1, e1, 4, 7, false);
  ...
  void rebase() {
    ...
    ScoreDirector<TestdataListSolution> destinationScoreDirector =
  ...
  void tabuIntrospection_twoEntities() {
    ...
    assertThat(new SubListSwapMove<>(variableDescriptor, e1, 0, 3, e2, 0, 1, true))
        .isNotEqualTo(moveTwoEntities);
    //                                                                       ^
    assertThat(new SubListSwapMove<>(variableDescriptor, e1, 0, 3, e2, 0, 2, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                                    ^
    assertThat(new SubListSwapMove<>(variableDescriptor, e1, 0, 3, e2, 1, 1, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                                 ^
    assertThat(new SubListSwapMove<>(variableDescriptor, e1, 0, 3, e3, 0, 1, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                             ^
    assertThat(new SubListSwapMove<>(variableDescriptor, e1, 1, 4, e2, 0, 1, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                       ^  ^
    assertThat(new SubListSwapMove<>(variableDescriptor, e1, 1, 3, e2, 0, 1, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                       ^
    assertThat(new SubListSwapMove<>(variableDescriptor, e3, 0, 3, e2, 0, 1, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                    ^
    assertThat(new SubListSwapMove<>(variableDescriptor, e1, 0, 3, e2, 0, 1, false))
  ...
  void tabuIntrospection_oneEntity() {
    ...
    // Swaps on the same entity are normalized so that the lower index subList is always treated as
    // the left one.
    assertThat(new SubListSwapMove<>(variableDescriptor, e1, 0, 2, e1, 3, 4, false))
