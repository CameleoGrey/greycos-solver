...
public class TestdataShadowedFullConsistencyListVariableListener
    implements ListVariableListener<
        TestdataShadowedFullSolution, TestdataShadowedFullEntity, TestdataShadowedFullValue> {
  @Override
  public void afterListVariableElementUnassigned(
      ScoreDirector<TestdataShadowedFullSolution> scoreDirector, TestdataShadowedFullValue value) {
    // Ignore
  }

  @Override
  public void beforeListVariableChanged(
      ScoreDirector<TestdataShadowedFullSolution> scoreDirector,
  ...
  private void checkRange(TestdataShadowedFullEntity entity, int fromIndex, int toIndex) {
    if (fromIndex < 0 || toIndex > entity.valueList.size()) {
      throw new IllegalStateException(
          "Change range [%s, %s) went past bounds of entity list %s"
