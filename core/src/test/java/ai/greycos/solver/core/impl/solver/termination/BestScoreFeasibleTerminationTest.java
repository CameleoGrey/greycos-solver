...
class BestScoreFeasibleTerminationTest {
  ...
  void solveTermination() {
    ...
    doReturn(InnerScore.fullyAssigned(HardSoftScore.of(-100, -100)))
        .when(solverScope)
        .getBestScore();
  ...
  void calculateTimeGradientBendableScoreHHSSS() {
    ...
    var termination =
        new BestScoreFeasibleTermination<TestdataSolution>(scoreDefinition, new double[] {0.75});

    // Normal cases
    // Smack in the middle
    assertThat(
            termination.calculateFeasibilityTimeGradient(
