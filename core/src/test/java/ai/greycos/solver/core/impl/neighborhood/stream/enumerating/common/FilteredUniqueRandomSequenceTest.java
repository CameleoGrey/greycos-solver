...
import static ai.greycos.solver.core.impl.neighborhood.stream.enumerating.common.SelectionProbabilityTest.toEntries;
...
/**
 * The class under test uses randomness, and has a whole lot of possible corner cases. Therefore, we
 * run the same tests with a variety of random seeds to increase coverage. None of these are
 * expected to fail. If any fail, the random seed is printed in the test name for reproducibility.
 */
...
class FilteredUniqueRandomSequenceTest {
  ...
  void throwsWhenUnknowinglyEmpty(int elementCount) {
    ...
    assertThatThrownBy(() -> sequence.pick(random)).isInstanceOf(NoSuchElementException.class);
  }
}
