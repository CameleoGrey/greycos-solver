...
class SubListChangeMoveTest {
  ...
  void isMoveDoable() {
    ...
    assertThat(
            new SubListChangeMove<>(variableDescriptor, e1, 1, 2, e1, 1, false)
                .isMoveDoable(scoreDirector))
        .isFalse();
    // same entity, different index => doable
    assertThat(
            new SubListChangeMove<>(variableDescriptor, e1, 1, 2, e1, 0, false)
                .isMoveDoable(scoreDirector))
        .isTrue();
    // same entity, index + length <= list size => doable
    assertThat(
            new SubListChangeMove<>(variableDescriptor, e1, 0, 3, e1, 1, false)
                .isMoveDoable(scoreDirector))
        .isTrue();
    // same entity, index + length > list size => not doable
    assertThat(
            new SubListChangeMove<>(variableDescriptor, e1, 0, 3, e1, 2, false)
                .isMoveDoable(scoreDirector))
        .isFalse();
    // different entity => doable
    assertThat(
            new SubListChangeMove<>(variableDescriptor, e1, 1, 2, e2, 0, false)
                .isMoveDoable(scoreDirector))
        .isTrue();
  }
  ...
  void doReversingMove() {
    ...
    SubListChangeMove<TestdataListSolution> move =
        new SubListChangeMove<>(variableDescriptor, e1, 0, 3, e2, 1, true);
  ...
  void tabuIntrospection_twoEntities() {
    ...
    assertThat(new SubListChangeMove<>(variableDescriptor, e1, 1, 3, e2, 0, true))
        .isNotEqualTo(moveTwoEntities);
    //                                                                      ^
    assertThat(new SubListChangeMove<>(variableDescriptor, e1, 1, 3, e2, 1, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                                   ^
    assertThat(new SubListChangeMove<>(variableDescriptor, e1, 1, 3, e3, 0, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                               ^
    assertThat(new SubListChangeMove<>(variableDescriptor, e1, 1, 2, e2, 0, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                            ^
    assertThat(new SubListChangeMove<>(variableDescriptor, e1, 2, 4, e2, 0, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                         ^  ^
    assertThat(new SubListChangeMove<>(variableDescriptor, e3, 1, 3, e2, 0, false))
        .isNotEqualTo(moveTwoEntities);
    //                                                     ^
    assertThat(new SubListChangeMove<>(variableDescriptor, e1, 1, 3, e2, 0, false))
  ...
  void toStringTest() {
    ...
    assertThat(new SubListChangeMove<>(variableDescriptor, e1, 1, 3, e1, 0, false))
        .hasToString("|3| {e1[1..4] -> e1[0]}");
    assertThat(new SubListChangeMove<>(variableDescriptor, e1, 0, 1, e2, 1, false))
