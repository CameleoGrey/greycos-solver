...
/**
 * Covers the unified mechanism for equal-bearing joins: a single {@link FusedEqualIndex} whose
 * {@link Bucket}s co-locate matching left and right tuples (an EQUAL match is {@code
 * Objects.equals} = same bucket), built via {@link IndexerFactory#buildFusedEqualIndex()}.
 */
class FusedEqualIndexTest {

  @Test
  void singleEqualBucketSharingAndOps() {
    // equal(gender): pure equal ⇒ the composite key IS the equal key (raw gender), bucket = the
    // equal-key group.
    ...
    assertThat(fusedEqualIndex.getOrCreateBucket("F"))
        .isSameAs(bucketF); // matching keys ⇒ same bucket
  ...
  void equalPlusEqualKeyedByCompositeKey() {
    ...
    assertThat(fusedEqualIndex.getOrCreateBucket(CompositeKey.of("F", 40)))
        .isSameAs(bucket); // equal-by-value ⇒ same bucket
  ...
  void equalPlusLessThanSuffixIsPerSideAndFlipped() {
    // equal(gender) AND lessThan(age): bucket keyed by gender only; the right side carries a
    // (flipped) age suffix.
    FusedEqualIndex<UniTuple<String>, UniTuple<String>> fusedEqualIndex =
        new IndexerFactory<>(equalGenderLessThanAge()).buildFusedEqualIndex();

    var bucket = fusedEqualIndex.getOrCreateBucket(CompositeKey.of("F", 40));
    assertThat(fusedEqualIndex.getOrCreateBucket(CompositeKey.of("F", 99)))
        .isSameAs(bucket); // same gender ⇒ same bucket
    assertThat(fusedEqualIndex.getOrCreateBucket(CompositeKey.of("M", 40))).isNotSameAs(bucket);
    ...
    // A left tuple ("F", age) matches right tuples with right.age > left.age (left.age <
    // right.age).
    assertThat(collectRight(bucket, CompositeKey.of("F", 40))).containsExactly(rightAge50);
    assertThat(collectRight(bucket, CompositeKey.of("F", 20)))
        .containsExactlyInAnyOrder(rightAge30, rightAge50);
    assertThat(collectRight(bucket, CompositeKey.of("F", 60))).isEmpty();
  }

  @Test
  void hasSuffixAndIsSameBucket() {
    // equal(gender) AND lessThan(age): has a suffix ⇒ different composite keys can share a bucket
    // (same gender).
    FusedEqualIndex<UniTuple<String>, UniTuple<String>> suffixed =
        new IndexerFactory<>(equalGenderLessThanAge()).buildFusedEqualIndex();
    assertThat(suffixed.hasSuffix()).isTrue();
    // Same equal prefix (gender), different suffix (age) ⇒ same bucket: the changed-key update can
    // reuse it.
  ...
  void lazyAllocation_leftOnlyKey_rightNotAllocated() {
    ...
    var index =
        new FusedEqualIndex<String, String>(
    ...
    assertThat(bucket.isRemovable()).as("left empty + right null must be removable").isTrue();
  ...
  private static List<UniTuple<String>> collectRight(
      Bucket<UniTuple<String>, UniTuple<String>> bucket, Object key) {
    var result = new ArrayList<UniTuple<String>>();
    bucket.forEachRight(key, result::add);
    return result;
  }

  private static List<UniTuple<String>> collectLeft(
      Bucket<UniTuple<String>, UniTuple<String>> bucket, Object key) {
    var result = new ArrayList<UniTuple<String>>();
