...
public class TestdataDifferentCascadingSolution {
  ...
  public static TestdataDifferentCascadingSolution generateUninitializedSolution(
      int valueCount, int entityCount) {
    return generateSolution(valueCount, entityCount);
  }

  private static TestdataDifferentCascadingSolution generateSolution(
      int valueCount, int entityCount) {
    List<TestdataDifferentCascadingEntity> entityList =
        IntStream.range(1, entityCount + 1)
            .mapToObj(i -> new TestdataDifferentCascadingEntity("Generated Entity " + i))
            .toList();
    List<TestdataDifferentCascadingValue> valueList =
        IntStream.range(1, valueCount + 1).mapToObj(TestdataDifferentCascadingValue::new).toList();
