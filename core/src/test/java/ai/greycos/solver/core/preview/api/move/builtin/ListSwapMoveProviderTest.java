...
class ListSwapMoveProviderTest {
  ...
  void fromSolution() {
    ...
    var variableMetaModel =
        solutionDescriptor.getMetaModel().genuineEntity(TestdataListEntity.class).listVariable();
    ...
    var moveIterable =
        createMoveIterable(
            new ListSwapMoveProvider<>(variableMetaModel), solutionDescriptor, solution);
    ...
    // That makes 6 possible swap moves, including duplicates.

    var move1 =
        (ListSwapMove<TestdataListSolution, TestdataListEntity, TestdataListValue>) moveList.get(0);
    assertSoftly(
        softly -> {
          softly.assertThat(move1.getPlanningEntities()).containsOnly(e1, e2);
          softly.assertThat(move1.getPlanningValues()).containsOnly(assignedValue1, assignedValue2);
        });

    var move2 =
        (ListSwapMove<TestdataListSolution, TestdataListEntity, TestdataListValue>) moveList.get(1);
    assertSoftly(
        softly -> {
          softly.assertThat(move2.getPlanningEntities()).containsOnly(e1, e2);
          softly.assertThat(move2.getPlanningValues()).containsOnly(assignedValue1, assignedValue3);
        });

    var move3 =
        (ListSwapMove<TestdataListSolution, TestdataListEntity, TestdataListValue>) moveList.get(2);
    assertSoftly(
        softly -> {
          softly.assertThat(move3.getPlanningEntities()).containsOnly(e1, e2);
          softly.assertThat(move3.getPlanningValues()).containsOnly(assignedValue1, assignedValue2);
        });

    var move4 =
        (ListSwapMove<TestdataListSolution, TestdataListEntity, TestdataListValue>) moveList.get(3);
    assertSoftly(
        softly -> {
          softly.assertThat(move4.getPlanningEntities()).containsOnly(e2);
          softly.assertThat(move4.getPlanningValues()).containsOnly(assignedValue2, assignedValue3);
        });

    var move5 =
        (ListSwapMove<TestdataListSolution, TestdataListEntity, TestdataListValue>) moveList.get(4);
    assertSoftly(
        softly -> {
          softly.assertThat(move5.getPlanningEntities()).containsOnly(e1, e2);
          softly.assertThat(move5.getPlanningValues()).containsOnly(assignedValue1, assignedValue3);
        });

    var move6 =
        (ListSwapMove<TestdataListSolution, TestdataListEntity, TestdataListValue>) moveList.get(5);
    assertSoftly(
        softly -> {
          softly.assertThat(move6.getPlanningEntities()).containsOnly(e2);
          softly.assertThat(move6.getPlanningValues()).containsOnly(assignedValue2, assignedValue3);
        });
  }
