...
public class TestdataSeparateClassLoaderDomain {
  ...
  public static Class<?> getTestdataEntityClass() {
    try {
      return CLASS_LOADER.loadClass("ai.greycos.solver.core.testcotwin.TestdataEntity");
  ...
  public static Class<?> getTestdataValueClass() {
    try {
      return CLASS_LOADER.loadClass("ai.greycos.solver.core.testcotwin.TestdataValue");
    } catch (ClassNotFoundException e) {
      throw new RuntimeException(e);
    }
  }
  ...
  public static Class<?> getTestdataSolutionClass() {
    try {
      return CLASS_LOADER.loadClass("ai.greycos.solver.core.testcotwin.TestdataSolution");
    } catch (ClassNotFoundException e) {
      throw new RuntimeException(e);
    }
  }
      ...
      for (var i = 0; i < 3; i++) {
        var entity =
            entityClass
                ...
                .newInstance("Generated Entity %d".formatted(i));
        entityList.add(entity);
      }

      for (var i = 0; i < 3; i++) {
        var value =
            valueClass.getConstructor(String.class).newInstance("Generated Value %d".formatted(i));
        valueList.add(value);
      }
