...
final class SelectionProbabilityTest {
  ...
  void check(int n) {
    ...
    for (var trial = 0;
        trial < TRIAL_COUNT;
        trial++) { // Independent trials; each gets its own random seed.
      UniqueRandomSequence.SequenceElement<Integer> element = null;
      var sequence =
          new DefaultUniqueRandomSequence<>(sampleList); // This is the code that we test.
    ...
    var standardDeviation = new BigDecimal(statistics.getStandardDeviation()).round(context);
    var threshold =
        BigDecimal.valueOf(TRIAL_COUNT / (double) SAMPLE_COUNT)
            .multiply(BigDecimal.valueOf(0.02))
            .round(context); // 2% tolerance
    Assertions.assertThat(standardDeviation)
        .as(
            () ->
                "Standard deviation of selection counts (%s) on the %sth random sample is over %s threshold."
