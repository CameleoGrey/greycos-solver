...
class ConsecutiveSetTreeTest {
  ...
  void testNonconsecutiveNumbers() {
    ...
    assertSoftly(
        softly -> {
          ...
          softly
              .assertThat(tree.getFirstBreak())
              .usingRecursiveComparison()
              .isEqualTo(breakList.get(0));
          softly
              .assertThat(tree.getLastBreak())
              .usingRecursiveComparison()
  ...
  void testConsecutiveNumbers() {
    ...
    assertThat(breakList.get(0))
        .usingRecursiveComparison()
        .isEqualTo(getBreak(tree, breakStart3, breakEnd5));
  }
  ...
  void testTimeslotConsecutive() {
    ConsecutiveSetTree<Timeslot, OffsetDateTime, Duration> tree =
    ...
    assertThat(breakList).hasSize(1);
    assertThat(breakIterator.next()).usingRecursiveComparison().isEqualTo(getBreak(tree, t2, t3));
  }
  ...
  private <ValueType_, DifferenceType_ extends Comparable<DifferenceType_>>
      Break<ValueType_, DifferenceType_> getBreak(
          SequenceChain<ValueType_, DifferenceType_> consecutiveData,
          ValueType_ start,
          ValueType_ end) {
    for (var sequenceBreak : consecutiveData.getBreaks()) {
      if (sequenceBreak.getPreviousSequenceEnd().equals(start)
          && sequenceBreak.getNextSequenceStart().equals(end)) {
        return sequenceBreak;
      }
    }
    throw new IllegalStateException(
        "Unable to find sequence with start ("
            + start
            + ") and end ("
            + end
            + ") in ("
  ...
  private record Timeslot(OffsetDateTime from, OffsetDateTime to) {
    public Timeslot(int fromIndex, int toIndex) {
      this(
          OffsetDateTime.of(2000, 1, fromIndex + 1, 0, 0, 0, 0, ZoneOffset.UTC),
