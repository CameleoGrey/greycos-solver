...
public class MoveAsserter<Solution_> {
  ...
  public void assertMoveAndUndo(
      Solution_ solution, Move<Solution_> move, Consumer<Solution_> moveSolutionConsumer) {
    assertMove(solution, move, moveSolutionConsumer, false);
  }

  public void assertMoveAndApply(
      Solution_ solution, Move<Solution_> move, Consumer<Solution_> moveSolutionConsumer) {
      ...
      if (corruptionResult.isCorrupted()) {
        throw new AssertionError(
            """
                        ...
                        Analysis:
                        %s"""
                .formatted(move, corruptionResult.message()));
      }
      if (applyMove) {
