package ai.greycos.solver.core.testdomain.list.pinned.noshadows;
...
public class TestdataPinnedNoShadowsListSolution {
  ...
  public static TestdataPinnedNoShadowsListSolution generateInitializedSolution(
      int valueCount, int entityCount) {
    return generateSolution(valueCount, entityCount).initialize();
  }

  public static TestdataPinnedNoShadowsListSolution generateUninitializedSolution(
      int valueCount, int entityCount) {
    return generateSolution(valueCount, entityCount);
  }

  private static TestdataPinnedNoShadowsListSolution generateSolution(
      int valueCount, int entityCount) {
    List<TestdataPinnedNoShadowsListEntity> entityList =
        IntStream.range(0, entityCount)
            .mapToObj(i -> new TestdataPinnedNoShadowsListEntity("Generated Entity " + i))
            .collect(Collectors.toList());
    List<TestdataPinnedNoShadowsListValue> valueList =
