...
import static ai.greycos.solver.core.impl.solver.termination.DiminishedReturnsTermination.NANOS_PER_MILLISECOND;
...
class DiminishedReturnsTerminationTest {
  ...
  void testNoImprovementInGraceTerminates() {
    ...
    assertThat(
            termination.isTerminated(
                10 * NANOS_PER_MILLISECOND, InnerScore.fullyAssigned(SimpleScore.ZERO)))
        .isTrue();
  }
  ...
  void testTerminatesWhenScoreDoesNotImprove() {
    ...
    assertThat(
            termination.isTerminated(
                10 * NANOS_PER_MILLISECOND, InnerScore.fullyAssigned(SimpleScore.ONE)))
        .isFalse();
    ...
    assertThat(
            termination.isTerminated(
                11 * NANOS_PER_MILLISECOND, InnerScore.fullyAssigned(SimpleScore.ONE)))
        .isTrue();
  }
  ...
  void testGracePeriodAndLongInitializationPeriod() {
    ...
    assertThat(
            termination.isTerminated(
                termination.getGracePeriodStartTimeNanos() + 50_000 * NANOS_PER_MILLISECOND, score))
        .isFalse();
    ...
    assertThat(termination.isTerminated(termination.getGracePeriodStartTimeNanos(), score))
        .isFalse();
    // It can be terminated after the first step started
    assertThat(
            termination.isTerminated(
                termination.getGracePeriodStartTimeNanos() + 2_000 * NANOS_PER_MILLISECOND, score))
        .isTrue();
    ...
    // Timeout
    assertThat(termination.isTerminated(currentTime + 11_000 * NANOS_PER_MILLISECOND, score))
        .isTrue();
  }
}
