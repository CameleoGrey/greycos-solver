...
import ai.greycos.solver.core.impl.score.stream.collector.connected_ranges.ConnectedRangeTracker;
import ai.greycos.solver.core.impl.score.stream.collector.consecutive.ConsecutiveSetTree;
...
public abstract class AbstractConstraintCollectorsTest {
  ...
  protected static SequenceChain<Integer, Integer> buildSequenceChain(Integer... data) {
    return Arrays.stream(data)
        .collect(
            () ->
                new ConsecutiveSetTree<Integer, Integer, Integer>(
                    (a, b) -> b - a, Integer::sum, 1, 0),
  ...
  protected ConnectedRangeChain<Interval, Integer, Integer> buildConsecutiveUsage(
      Interval... data) {
    return Arrays.stream(data)
        .collect(
  ...
  protected ConnectedRangeChain<DynamicInterval, Integer, Integer> buildDynamicConsecutiveUsage(
      DynamicInterval... data) {
    return Arrays.stream(data)
        .collect(
            () ->
                new ConnectedRangeTracker<>(
  ...
  public static final class DynamicInterval {
    ...
    public boolean equals(Object o) {
      ...
      if (!(o instanceof DynamicInterval that)) return false;
