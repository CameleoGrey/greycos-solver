...
class ConnectedRangeTrackerTest {
  ...
  void testConsecutiveRanges() {
    ...
    assertThat(connectedRangeList.get(0))
        .containsExactly(new TestRange(0, 2), new TestRange(2, 4), new TestRange(4, 7));
    assertThat(connectedRangeList.get(0).getMinimumOverlap()).isEqualTo(1);
    assertThat(connectedRangeList.get(0).getMaximumOverlap()).isEqualTo(1);
    verifyGaps(tree);
  }
  ...
  void testOverlappingRange() {
    ...
    assertThat(connectedRanges.get(0))
        .containsExactly(a.getValue(), removedTestRange1, c.getValue());
  ...
  void verifyGaps(ConnectedRangeTracker<TestRange, Integer, Integer> tree) {
    ...
    for (int i = 0; i < connectedRangeList.size() - 1; i++) {
      assertThat(gapList.get(i).getPreviousRangeEnd())
          .isEqualTo(connectedRangeList.get(i).getEnd());
      assertThat(gapList.get(i).getNextRangeStart())
          .isEqualTo(connectedRangeList.get(i + 1).getStart());
      assertThat(gapList.get(i).getLength())
          .isEqualTo(connectedRangeList.get(i + 1).getStart() - connectedRangeList.get(i).getEnd());
    }
  }

  private static int rangeGapCompare(RangeGap<Integer, Integer> a, RangeGap<Integer, Integer> b) {
    if (a == b) {
      return 0;
    }
    if (a == null || b == null) {
    ...
    boolean out =
        Objects.equals(a.getPreviousRangeEnd(), b.getPreviousRangeEnd())
  ...
  private static int rangeClusterCompare(
      ConnectedRange<TestRange, Integer, Integer> a,
    ...
    if (!(a instanceof ConnectedRangeImpl) || !(b instanceof ConnectedRangeImpl)) {
      throw new IllegalArgumentException(
          "Expected (" + a + ") and (" + b + ") to both be ConnectedRangeImpl");
    }
    ...
    boolean out =
        first.getStartSplitPoint().compareTo(second.getStartSplitPoint()) == 0
  ...
  void testRandomRanges() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      for (int j = 0; j < 100; j++) {
        ...
        RangeSplitPoint<TestRange, Integer> endSplitPoint =
            splitPoints.floor(treeRange.getEndSplitPoint());
        ...
        RangeSplitPoint<TestRange, Integer> current =
            splitPoints.isEmpty() ? null : splitPoints.first();
        ...
        while (current != null) {
          rangeClusterList.add(
              ConnectedRangeImpl.getConnectedRangeStartingAt(
                  splitPoints, (a, b) -> a - b, current));
          if (previous != null) {
          ...
          current =
              splitPoints.higher(
                  rangeClusterList.get(rangeClusterList.size() - 1).getEndSplitPoint());
        }
  ...
  private String formatConnectedRangeTracker(
      ConnectedRangeTracker<TestRange, Integer, Integer> rangeTree) {
    List<List<TestRange>> listOfConnectedRanges = new ArrayList<>();
    for (ConnectedRange<TestRange, Integer, Integer> cluster :
        rangeTree.getConnectedRangeChain().getConnectedRanges()) {
