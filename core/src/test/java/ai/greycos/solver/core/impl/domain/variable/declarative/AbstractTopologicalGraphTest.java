...
public abstract class AbstractTopologicalGraphTest<Graph_ extends TopologicalOrderGraph> {
  ...
  private static void assertTopologicalOrder(
      TopologicalOrderGraph graph, List<Integer>... expectedTopologicalNodeOrder) {
                ...
                .withFailMessage(
                    () ->
                        "Expected topological order of %d (%d) to be greater than %d (%d) in graph %s"
                            .formatted(a, aOrder, b, bOrder, graph))
  ...
  void assertAllPermutations(
      int size,
      List<List<Integer>> graphEdges,
  ...
  void testNoLoops() {
    ...
    assertAllPermutations(
        GRAPH_SIZE,
        List.of(List.of(1, 2), List.of(3), List.of(3), List.of(4), List.of()),
        (graph, mapper, changed) -> {
  ...
  void testNoLoopsRemoveEdge() {
    ...
    assertAllPermutations(
        ...
        (graph, mapper, changed) -> {
          ...
          // it is to be after 0.
          assertTopologicalOrder(
              graph, List.of(mapper.applyAsInt(0)), List.of(mapper.applyAsInt(2)));
  ...
  void testLoops() {
    ...
    assertAllPermutations(
        GRAPH_SIZE,
        List.of(List.of(1, 2), List.of(3), List.of(3), List.of(4, 1), List.of()),
        (graph, mapper, changed) -> {
          verifyConsistent(graph);
          assertTopologicalOrder(
              graph,
          ...
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(1)))
              .containsExactlyInAnyOrder(mapper.applyAsInt(1), mapper.applyAsInt(3));
  ...
  void testConnectingTwoLoops() {
    ...
    assertSomePermutations(
        6,
        List.of(
            List.of(1, 2), List.of(3, 2), List.of(4), List.of(5, 1), List.of(5, 2, 3), List.of()),
        (graph, mapper, changed) -> {
          verifyConsistent(graph);
          assertTopologicalOrder(
              graph,
          ...
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(0)))
              .containsExactlyInAnyOrder(mapper.applyAsInt(0));
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(1)))
              .containsExactlyInAnyOrder(cycle);
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(2)))
              .containsExactlyInAnyOrder(cycle);
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(3)))
              .containsExactlyInAnyOrder(cycle);
    ...
    assertSomePermutations(
        ...
        (graph, mapper, changed) -> {
          ...
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(1)))
              .containsExactlyInAnyOrder(mapper.applyAsInt(1), mapper.applyAsInt(3));
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(2)))
              .containsExactlyInAnyOrder(mapper.applyAsInt(2), mapper.applyAsInt(4));
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(3)))
              .containsExactlyInAnyOrder(mapper.applyAsInt(1), mapper.applyAsInt(3));
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(4)))
              .containsExactlyInAnyOrder(mapper.applyAsInt(2), mapper.applyAsInt(4));
  ...
  void testConnectingTwoLoopsRemoveNonJoiningEdge() {
    ...
    assertSomePermutations(
        GRAPH_SIZE,
        List.of(List.of(1, 2), List.of(2), List.of(4, 3), List.of(4, 5), List.of(5, 1), List.of()),
        (graph, mapper, changed) -> {
          changed.clear();
          ...
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(0)))
              .containsExactlyInAnyOrder(mapper.applyAsInt(0));
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(1)))
              .containsExactlyInAnyOrder(loop);
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(2)))
              .containsExactlyInAnyOrder(loop);
          assertThat(getComponentMembers(graph, GRAPH_SIZE, mapper.applyAsInt(3)))
              .containsExactlyInAnyOrder(loop);
