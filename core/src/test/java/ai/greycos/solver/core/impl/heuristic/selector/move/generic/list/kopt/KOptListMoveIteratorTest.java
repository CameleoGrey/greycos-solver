...
public class KOptListMoveIteratorTest {
  ...
  private KOptListMoveIteratorMockData createMockKOptListMoveIterator(
  ...
  private static class KOptMoveInfo {
    ...
    public void verify(KOptListMove<?> kOptListMove) {
      ...
      assertThat((Object[]) descriptor.removedEdges())
          .containsExactly(expectedRemoveEdges.toArray());

      assertThat(descriptor.addedEdgeToOtherEndpoint())
          .containsExactly(addedEdgeIndexToOtherEndpoint);
    }
  }
   ...
   */
  private KOptMoveInfo setupValidOddSequentialKOptMove(
      KOptListMoveIteratorMockData mocks, int k, Object... entities) {
    if (entities.length != k) {
      throw new IllegalArgumentException("Expected (" + k + ") arguments");
    }
    if (k % 2 != 1) {
      throw new IllegalArgumentException(
          "Function can only be used for odd k (" + k + " is not odd).");
    }
    int randomValue = 0;
    ...
    Map<Object, Integer> entityToListSize =
        Arrays.stream(entities)
            .collect(Collectors.toMap(Function.identity(), entity -> 2, Integer::sum));
    ...
    for (Map.Entry<Object, Integer> entityAndListSize : entityToListSize.entrySet()) {
      ...
      List<Object> entityList =
          new ArrayList<>(Arrays.asList(data).subList(offset, offset + listSize));
      ...
      when(mocks.listVariableDescriptor.getUnpinnedSubListSize(entity))
          .thenReturn(entityList.size());
   ...
   * @return The expected KOptMoveInfo
   */
  private KOptMoveInfo setupValidNonsequential4OptMove(
      KOptListMoveIteratorMockData mocks, Object... entities) {
    int k = 4;
    ...
    for (Map.Entry<Object, Integer> entityAndListSize : entityToListSize.entrySet()) {
      ...
      doAnswer(
          ...
          .when(mocks.listVariableDescriptor)
          .getRandomUnpinnedElement(eq(entity), any());
      when(mocks.entityDescriptor.isMovable(any(), eq(entity))).thenReturn(true);
    ...
    List<Object> distinctEntityList =
        Arrays.stream(entities).distinct().collect(Collectors.toList());
    ...
    when(mocks.workingRandom.nextInt(
            entityToList.get(distinctEntityList.get(entityListIndex)).size()))
    ...
    out.removedEdgeList.add(
        entityToList.get(distinctEntityList.get(entityListIndex)).get(finalValueIndex));
    out.removedEdgeList.add(
        entityToList.get(distinctEntityList.get(entityListIndex)).get(finalValueIndex + 1));
    ...
    out.addedEdgeIndexToOtherEndpoint =
        new int[] {0, 8, 3, 2, 5, 4, 7, 6, 1, 12, 11, 10, 9, 15, 16, 13, 14};
    return out;
  }

  @Test
  void testSequentialKOptOnSameEntity() {
    KOptListMoveIteratorMockData mocks =
        createMockKOptListMoveIterator(2, 5, new int[] {1, 1, 1, 1});

    KOptMoveInfo kOptMoveInfo = setupValidOddSequentialKOptMove(mocks, 3, "e1", "e1", "e1");
    KOptListMove<?> kOptListMove =
        (KOptListMove<?>) mocks.kOptListMoveIterator.createUpcomingSelection();
  ...
  void testSequentialKOptOnDifferentEntities() {
    ...
    KOptMoveInfo kOptMoveInfo =
        setupValidOddSequentialKOptMove(mocks, 5, "e1", "e2", "e1", "e2", "e1");
    KOptListMove<?> kOptListMove =
        (KOptListMove<?>) mocks.kOptListMoveIterator.createUpcomingSelection();
    kOptMoveInfo.verify(kOptListMove);
  }
  ...
  void testNonsequentialKOptOnSameEntity() {
    KOptListMoveIteratorMockData mocks =
        createMockKOptListMoveIterator(2, 6, new int[] {1, 1, 1, 1});

    KOptMoveInfo kOptMoveInfo =
        setupValidNonsequential4OptMove(mocks, "e1", "e1", "e1", "e1", "e1");
    KOptListMove<?> kOptListMove =
        (KOptListMove<?>) mocks.kOptListMoveIterator.createUpcomingSelection();
    kOptMoveInfo.verify(kOptListMove);
  }
  ...
  void testNonsequentialKOptOnDifferentEntity() {
    ...
    KOptMoveInfo kOptMoveInfo =
        setupValidNonsequential4OptMove(mocks, "e1", "e2", "e1", "e2", "e1");
    KOptListMove<?> kOptListMove =
        (KOptListMove<?>) mocks.kOptListMoveIterator.createUpcomingSelection();
    kOptMoveInfo.verify(kOptListMove);
  }
