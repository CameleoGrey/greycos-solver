...
class SwapMoveDefinitionTest {
  ...
  void univariate() {
    ...
    var entityMetaModel = solutionDescriptor.getMetaModel().entity(TestdataEntity.class);
    ...
    var moveIterable =
        createMoveIterable(new SwapMoveDefinition<>(entityMetaModel), solutionDescriptor, solution);
    var moveList =
    ...
    assertSoftly(
        softly -> {
          softly.assertThat(firstMove.getPlanningEntities()).containsOnly(e1, e2);
          softly.assertThat(firstMove.getPlanningValues()).containsOnly(v1, v2);
        });

    var secondMove = moveList.get(1);
    assertSoftly(
        softly -> {
          softly.assertThat(secondMove.getPlanningEntities()).containsOnly(e2, e3);
          softly.assertThat(secondMove.getPlanningValues()).containsOnly(v2, v1);
        });
  }
  ...
  void multivariate() {
    ...
    var entityMetaModel = solutionDescriptor.getMetaModel().entity(TestdataMultiVarEntity.class);
    ...
    assertSoftly(
        softly -> {
          softly.assertThat(firstMove.getPlanningEntities()).containsOnly(e1, e2);
          softly.assertThat(firstMove.getPlanningValues()).containsOnly(v1, v2, otherV1, otherV2);
        });
    ...
    assertSoftly(
        softly -> {
          softly.assertThat(secondMove.getPlanningEntities()).containsOnly(e2, e3);
          softly.assertThat(secondMove.getPlanningValues()).containsOnly(v2, v1, otherV2, otherV1);
        });
  }
  ...
  void multivariateWithExclusions() {
    ...
    var moveIterable =
        createMoveIterable(
            new SwapMoveDefinition<>(allowedVariableMetaModels), solutionDescriptor, solution);
    var moveList =
        StreamSupport.stream(moveIterable.spliterator(), false)
            .map(m -> (SwapMove<TestdataMultiVarSolution, TestdataMultiVarEntity>) m)
            .toList();
