...
class IfExistsUniNodeActivityTest {
  ...
  private static UnindexedIfExistsUniNode<String, String> node(
      boolean shouldExist, TupleLifecycle<UniTuple<String>> downstream) {
    var tracker = mock(InTupleStorePositionTracker.class);
    return new UnindexedIfExistsUniNode<>(shouldExist, downstream, tracker);
  }

  private static void initBothSides(
      UnindexedIfExistsUniNode<String, String> node, boolean left, boolean right) {
