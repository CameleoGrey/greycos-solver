...
class ListChangeMoveTest {
  ...
  void isMoveDoable() {
    ...
    assertThat(new ListChangeMove<>(variableDescriptor, e1, 1, e1, 1).isMoveDoable(scoreDirector))
        .isFalse();
    // same entity, different index => doable
    assertThat(new ListChangeMove<>(variableDescriptor, e1, 0, e1, 1).isMoveDoable(scoreDirector))
        .isTrue();
    // same entity, index == list size => not doable because the element is first removed (list size
    // is reduced by 1)
    assertThat(new ListChangeMove<>(variableDescriptor, e1, 0, e1, 2).isMoveDoable(scoreDirector))
        .isFalse();
    // different entity => doable
    assertThat(new ListChangeMove<>(variableDescriptor, e1, 0, e2, 0).isMoveDoable(scoreDirector))
        .isTrue();
  }
  ...
  void isMoveDoableValueRangeProviderOnEntity() {
    ...
    assertThat(
            new ListChangeMove<>(otherVariableDescriptor, entity1, 1, entity2, 0)
                .isMoveDoable(otherInnerScoreDirector))
        .isFalse();
  }
  ...
  void doMove() {
    ...
    ListChangeMove<TestdataListSolution> move =
        new ListChangeMove<>(variableDescriptor, e1, 1, e2, 1);
  ...
  void tabuIntrospection_twoEntities() {
    ...
    ListChangeMove<TestdataListSolution> moveTwoEntities =
        new ListChangeMove<>(variableDescriptor, e1, 1, e2, 1);
  ...
  void tabuIntrospection_oneEntity() {
    ...
    ListChangeMove<TestdataListSolution> moveOneEntity =
        new ListChangeMove<>(variableDescriptor, e1, 0, e1, 1);
    // Do the move first because that might affect the returned values.
  ...
  void toStringTest() {
    ...
    assertThat(new ListChangeMove<>(variableDescriptor, e1, 1, e1, 0))
        .hasToString("2 {e1[1] -> e1[0]}");
    assertThat(new ListChangeMove<>(variableDescriptor, e1, 0, e2, 1))
