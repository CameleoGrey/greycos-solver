...
class TwoOptListMoveTest {
  ...
  void doMove() {
    ...
    TestdataListEntity e1 =
        TestdataListEntity.createWithValues("e1", v1, v2, v5, v4, v3, v6, v7, v8);
    ...
    // 2-Opt((v2, v5), (v3, v6))
    TwoOptListMove<TestdataListSolution> move =
        new TwoOptListMove<>(variableDescriptor, e1, e1, 2, 5);
  ...
  void isMoveDoable() {
    ...
    // 2-Opt((v2, v3), (v2, v3))
    move = new TwoOptListMove<>(variableDescriptor, e1, e1, 2, 2);
    assertThat(move.isMoveDoable(scoreDirector)).isFalse();

    // 2-Opt((v2, v3), (v3, v4))
    move = new TwoOptListMove<>(variableDescriptor, e1, e1, 2, 3);
    assertThat(move.isMoveDoable(scoreDirector)).isFalse();

    // 2-Opt((v2, v3), (v4, v5))
    move = new TwoOptListMove<>(variableDescriptor, e1, e1, 2, 4);
    assertThat(move.isMoveDoable(scoreDirector)).isTrue();

    // 2-Opt((v2, v3), (v1, v2))
    move = new TwoOptListMove<>(variableDescriptor, e1, e1, 2, 1);
    assertThat(move.isMoveDoable(scoreDirector)).isTrue();
  }
  ...
  void isMoveDoableValueRangeProviderOnEntity() {
    ...
    var e1 =
        new TestdataListEntityProvidingEntity(
            "e1", List.of(v1, v2, v3, v5, v6), List.of(v2, v1, v5));
    var e2 =
        new TestdataListEntityProvidingEntity(
            "e2", List.of(v1, v3, v4, v5, v6), List.of(v4, v3, v6));
    ...
    assertThat(
            new TwoOptListMove<>(otherVariableDescriptor, e1, e2, 1, 1)
                .isMoveDoable(otherInnerScoreDirector))
        .isTrue();
    // different entity => invalid left
    assertThat(
            new TwoOptListMove<>(otherVariableDescriptor, e1, e2, 0, 1)
                .isMoveDoable(otherInnerScoreDirector))
        .isFalse();
    // different entity => invalid right
    assertThat(
            new TwoOptListMove<>(otherVariableDescriptor, e1, e2, 1, 0)
                .isMoveDoable(otherInnerScoreDirector))
        .isFalse();
  }
  ...
  void isMoveDoableTailSwap() {
    ...
    // 2-Opt((v2, v3), (v6, v7))
    TwoOptListMove<TestdataListSolution> move =
        new TwoOptListMove<>(variableDescriptor, e1, e2, 2, 2);
    assertThat(move.isMoveDoable(scoreDirector)).isTrue();

    move = new TwoOptListMove<>(variableDescriptor, e1, e2, 1, 2);
    assertThat(move.isMoveDoable(scoreDirector)).isTrue();

    move = new TwoOptListMove<>(variableDescriptor, e1, e2, 2, 1);
    assertThat(move.isMoveDoable(scoreDirector)).isTrue();
  }
  ...
  void doMoveSecondEndsBeforeFirst() {
    ...
    TestdataListValue v8 = new TestdataListValue("8");
    TestdataListEntity e1 =
        TestdataListEntity.createWithValues("e1", v8, v7, v3, v4, v5, v6, v2, v1);
    var solution = new TestdataListSolution();
    solution.setEntityList(List.of(e1));
    solution.setValueList(List.of(v1, v2, v5, v4, v3, v6, v7, v8));
    scoreDirector.setWorkingSolution(solution);
    ...
    // 2-Opt((v6, v2), (v7, v3))
    TwoOptListMove<TestdataListSolution> move =
        new TwoOptListMove<>(variableDescriptor, e1, e1, 6, 2);
    move.doMoveOnly(scoreDirector);
  ...
  void doMoveSecondEndsBeforeFirstUnbalanced() {
    ...
    // 2-Opt((v4, v1), (v5, v2))
    TwoOptListMove<TestdataListSolution> move =
        new TwoOptListMove<>(variableDescriptor, e1, e1, 4, 1);
    move.doMoveOnly(scoreDirector);
  ...
  void doMoveFirstEndsBeforeSecondUnbalanced() {
    ...
    // 2-Opt((v4, v1), (v5, v2))
    TwoOptListMove<TestdataListSolution> move =
        new TwoOptListMove<>(variableDescriptor, e1, e1, 2, 1);
    move.doMoveOnly(scoreDirector);
  ...
  void doMoveSecondEndsBeforeFirstPinned() {
    ...
    // 2-Opt((v6, v2), (v7, v3))
    TwoOptListMove<TestdataListSolution> move =
        new TwoOptListMove<>(variableDescriptorSpy, e1, e1, 6, 2);
    move.doMoveOnly(scoreDirector);
  ...
  void rebase() {
    ...
    assertSameProperties(
        ...
        0,
        1,
        new TwoOptListMove<>(variableDescriptor, e1, e1, 0, 1).rebase(destinationScoreDirector));
  }

  static void assertSameProperties(
      Object destinationEntity, int destinationV1, int destinationV2, TwoOptListMove<?> move) {
