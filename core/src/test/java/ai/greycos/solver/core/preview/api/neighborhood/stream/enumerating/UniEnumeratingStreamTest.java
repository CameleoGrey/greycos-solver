...
class UniEnumeratingStreamTest {
  ...
  private static <Solution_, A> UniLeftDatasetInstance<Solution_, A> getDatasetInstance(
  ...
  private static <Solution_> DatasetSession<Solution_> createSession(
    ...
    var scoreDirector =
        new EasyScoreDirectorFactory<>(
                enumeratingStreamFactory.getSolutionDescriptor(), s -> SimpleScore.ZERO)
            .buildScoreDirector();
  ...
  @Test
  void forEachListVariableExcludingPinnedIncludingNull() { // Entities with planningPin true will be
    // skipped.
    var enumeratingStreamFactory =
        new EnumeratingStreamFactory<>(
            TestdataPinnedWithIndexListSolution.buildSolutionDescriptor(),
