...
class SelectorBasedListAssignMoveTest {

  private final InnerScoreDirector<TestdataListSolution, ?> innerScoreDirector =
      mock(InnerScoreDirector.class);
  private final MoveDirector<TestdataListSolution, ?> moveDirector =
      new MoveDirector<>(innerScoreDirector);
  ...
  void doMove() {
    ...
    moveDirector.execute(new SelectorBasedListAssignMove<>(variableDescriptor, v2, e1, 0));
    moveDirector.execute(new SelectorBasedListAssignMove<>(variableDescriptor, v3, e1, 1));
    moveDirector.execute(new SelectorBasedListAssignMove<>(variableDescriptor, v1, e1, 0));
  ...
  void isMoveDoableValueRangeProviderOnEntity() {
    ...
    assertThat(
            new SelectorBasedListAssignMove<>(otherVariableDescriptor, v1, e1, 0)
                .isMoveDoable(otherInnerScoreDirector))
        .isTrue();
    assertThat(
            new SelectorBasedListAssignMove<>(otherVariableDescriptor, v1, e2, 0)
                .isMoveDoable(otherInnerScoreDirector))
        .isTrue();
  }
  ...
  static void assertSameProperties(
      ...
      SelectorBasedListAssignMove<?> move) {
  ...
  void toStringTest() {
    ...
    assertThat(new SelectorBasedListAssignMove<>(variableDescriptor, v1, e1, 15))
        .hasToString("1 {null -> E1[15]}");
  }
}
