...
public class SolutionTrackerTest {
  ...
  void testGetVariableChangedViolations() {
    ...
    assertThat(SolutionTracker.getVariableChangedViolations(before, after))
        .containsExactlyInAnyOrder(
            "Actual value (null) of variable value on TestdataEntity entity ("
            ...
            "Actual value ("
                + workingSolution.getValueList().get(0)
                + ") of variable value on TestdataEntity entity ("
  ...
  void testBuildScoreCorruptionMessageGoodVariableListeners() {
    ...
    variableListeners.forEach(
        variableListener -> {
          variableListener.beforeChange(
              null, new BasicVariableChangeEvent<>(workingSolution.getEntityList().get(0)));
        });
    workingSolution.getEntityList().get(0).setValue(null);
    variableListeners.forEach(
        variableListener -> {
    ...
    variableListeners.forEach(
        variableListener -> {
          variableListener.beforeChange(
              null, new BasicVariableChangeEvent<>(workingSolution.getEntityList().get(1)));
        });
    workingSolution.getEntityList().get(1).setValue(workingSolution.getValueList().get(0));
    variableListeners.forEach(
        variableListener -> {
