...
class SetValueRangeTest {
  ...
  void createOriginalIterator() {
    ...
    assertAllElementsOfIterator(
        createRange(100, 120, 5, 7, 8).createOriginalIterator(), 100, 120, 5, 7, 8);
    ...
    assertAllElementsOfIterator(
        new SetValueRange<>(Collections.emptySet()).createOriginalIterator());
  }

  @Test
  void createRandomIterator() {
    assertElementsOfIterator(
        createRange(0, 2, 5, 10).createRandomIterator(new TestRandom(2, 0)), 5, 0);
    assertElementsOfIterator(
        createRange(100, 120, 5, 7, 8).createRandomIterator(new TestRandom(2, 0)), 5, 100);
    assertElementsOfIterator(
        createRange(-15, 25, 0).createRandomIterator(new TestRandom(2, 0)), 0, -15);
    assertElementsOfIterator(
        createRange("b", "z", "a").createRandomIterator(new TestRandom(2, 0)), "a", "b");
    assertAllElementsOfIterator(
        new SetValueRange<>(Collections.emptySet()).createRandomIterator(new Random(0)));
  }
  ...
  private static <T> SetValueRange<T> createRange(T... values) {
    var set = Arrays.stream(values).collect(Collectors.toCollection(LinkedHashSet::new));
