...
public class TestdataPinnedWithIndexListSolution {
  ...
  public static TestdataPinnedWithIndexListSolution generateInitializedSolution(
      int valueCount, int entityCount) {
    return generateSolution(valueCount, entityCount).initialize();
  }

  public static TestdataPinnedWithIndexListSolution generateUninitializedSolution(
      int valueCount, int entityCount) {
    return generateSolution(valueCount, entityCount);
  }

  private static TestdataPinnedWithIndexListSolution generateSolution(
      int valueCount, int entityCount) {
    List<TestdataPinnedWithIndexListEntity> entityList =
        IntStream.range(0, entityCount)
            .mapToObj(i -> new TestdataPinnedWithIndexListEntity("Generated Entity " + i))
            .collect(Collectors.toList());
    List<TestdataPinnedWithIndexListValue> valueList =
