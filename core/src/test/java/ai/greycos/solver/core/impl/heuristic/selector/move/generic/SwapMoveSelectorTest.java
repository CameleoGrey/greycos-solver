...
import static ai.greycos.solver.core.impl.heuristic.selector.SelectorTestUtils.mockEntitySelector;
...
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getEntityDescriptor;
import static ai.greycos.solver.core.testutil.PlannerAssert.DO_NOT_ASSERT_SIZE;
...
import ai.greycos.solver.core.testdomain.valuerange.entityproviding.multivar.TestdataAllowsUnassignedMultiVarEntityProvidingEntity;
import ai.greycos.solver.core.testdomain.valuerange.entityproviding.multivar.TestdataAllowsUnassignedMultiVarEntityProvidingSolution;
...
class SwapMoveSelectorTest {
  ...
  void originalLeftEqualsRight() {
    ...
    SwapMoveSelector moveSelector =
        new SwapMoveSelector(
            entitySelector,
            entitySelector,
  ...
  void originalLeftUnequalsRightWithEntityRange() {
    ...
    var leftEntitySelector =
        new FromSolutionEntitySelector<>(
            getEntityDescriptor(scoreDirector), SelectionCacheType.JUST_IN_TIME, false);
    ...
    assertAllCodesOfMoveSelector(
        moveSelector, expectedSize, "A<->B", "A<->C", "B<->A", "B<->C", "C<->A", "C<->B");
  }
  ...
  void originalEntitiesPinned() {
    ...
    var leftEntitySelector =
        new ManualEntityMimicRecorder<>(
            mockEntitySelector(
                TestdataAllowsUnassignedEntityProvidingEntity.buildEntityDescriptor(), e1, e2, e3));
    ...
    var rightEntitySelector =
        new FilteringEntityByEntitySelector<>(
            filteringEntitySelector, replayingEntitySelector, false);
  ...
  void randomEntitiesPinned() {
    ...
    var baseEntitySelector =
        new FromSolutionEntitySelector<>(
            getEntityDescriptor(scoreDirector), SelectionCacheType.JUST_IN_TIME, true);
    var leftEntitySelector = new ManualEntityMimicRecorder<>(baseEntitySelector);

    var replayingEntitySelector = new MimicReplayingEntitySelector<>(leftEntitySelector);
    var filteringEntitySelector =
        FilteringEntitySelector.of(
            baseEntitySelector, new EntityCodeFiltering<>(List.of("B", "C")));
    var rightEntitySelector =
        new FilteringEntityByEntitySelector<>(
            filteringEntitySelector, replayingEntitySelector, true);
    var solverScope =
        solvingStarted(
            rightEntitySelector,
            scoreDirector,
  ...
  void multiVarRandomSelectionWithEntityValueRange() {
    ...
    var e1 =
        new TestdataAllowsUnassignedMultiVarEntityProvidingEntity(
            "A", List.of(v1, v4), List.of(v1, v4));
    var e2 =
        new TestdataAllowsUnassignedMultiVarEntityProvidingEntity(
            "B", List.of(v2, v3), List.of(v2, v3));
    var e3 =
        new TestdataAllowsUnassignedMultiVarEntityProvidingEntity(
  ...
  void noReachableEntities() {
    ...
    var scoreDirector =
        mockScoreDirector(TestdataEntityProvidingSolution.buildSolutionDescriptor());
    scoreDirector.setWorkingSolution(solution);
  ...
  private static class EntityCodeFiltering<Solution_>
