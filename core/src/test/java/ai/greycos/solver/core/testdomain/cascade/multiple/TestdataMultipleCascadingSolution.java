package ai.greycos.solver.core.testdomain.cascade.multiple;
...
public class TestdataMultipleCascadingSolution {
  ...
  public static TestdataMultipleCascadingSolution generateUninitializedSolution(
      int valueCount, int entityCount) {
    return generateSolution(valueCount, entityCount);
  }

  private static TestdataMultipleCascadingSolution generateSolution(
      int valueCount, int entityCount) {
    List<TestdataMultipleCascadingEntity> entityList =
        IntStream.range(1, entityCount + 1)
            .mapToObj(i -> new TestdataMultipleCascadingEntity("Generated Entity " + i))
            .toList();
    List<TestdataMultipleCascadingValue> valueList =
        IntStream.range(1, valueCount + 1).mapToObj(TestdataMultipleCascadingValue::new).toList();
