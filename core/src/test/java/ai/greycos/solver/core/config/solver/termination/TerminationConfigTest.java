...
class TerminationConfigTest {
  ...
  /**
   * A child config without any time spent limit should inherit the limit from a parent config with
   * a limit.
   */
  @Test
  void childWithoutTimeSpentLimitShouldInheritLimitFromParent() {
    ...
    TerminationConfig parent = new TerminationConfig().withSecondsSpentLimit(1L);
  ...
  /**
   * A child config with a seconds spent limit should ignore a seconds spent limit when inheriting
   * from another config. In this case, there is a direct collision between a parameter in the child
   * and the same parameter in the parent.
   */
  @Test
  void childWithSecondsSpentLimitShouldNotInheritSecondsSpentFromParent() {
    // Given a child config with a seconds spent limit
    TerminationConfig child = new TerminationConfig().withSecondsSpentLimit(2L);
    // and a parent with a seconds spent limit
    TerminationConfig parent = new TerminationConfig().withSecondsSpentLimit(1L);
    // when the child inherits from the parent
    child.inherit(parent);
  ...
  /**
   * A child config with a time spent limit should ignore all time spent limits of a parent config.
   * In this case, there is an indirect collision between a parameter in the child and a
   * semantically equivalent parameter in the parent.
   */
  @Test
  void childWithTimeSpentLimitShouldNotInheritTimeSpentLimitFromParent() {
    ...
    TerminationConfig parent = new TerminationConfig().withMinutesSpentLimit(1L);
    // when the child inherits from the parent
    child.inherit(parent);
    // then the child should keep its limit.
    assertThat(child.getSecondsSpentLimit()).isEqualTo(2L);
  ...
  void checkMoveCountMetrics() {
    TerminationConfig parent = new TerminationConfig().withMoveCountLimit(2L);
