package ai.greycos.solver.core.testdomain.multivar;
...
public class TestdataMultivarIncrementalScoreCalculator
    implements ConstraintMatchAwareIncrementalScoreCalculator<
        TestdataMultiVarSolution, SimpleScore> {
  ...
  public void resetWorkingSolution(
      @NonNull TestdataMultiVarSolution workingSolution, boolean constraintMatchEnabled) {
  ...
  private DefaultConstraintMatchTotal<SimpleScore> update() {
    var constraintMatchTotal =
        new DefaultConstraintMatchTotal<>(
            ConstraintRef.of(getClass().getPackageName(), "testConstraint"), SimpleScore.ONE);
    ...
    for (TestdataMultiVarEntity left : workingSolution.getMultiVarEntityList()) {
      ...
      var constraintMatch =
          constraintMatchTotal.addConstraintMatch(List.of(left), SimpleScore.of(-count));
      indictmentMap
          .computeIfAbsent(left, key -> new DefaultIndictment<>(key, SimpleScore.ZERO))
