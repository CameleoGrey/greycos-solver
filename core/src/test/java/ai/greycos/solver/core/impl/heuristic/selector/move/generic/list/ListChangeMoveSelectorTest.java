...
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getAllowsUnassignedvaluesEntityRangeListVariableDescriptor;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getAllowsUnassignedvaluesListVariableDescriptor;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getEntityRangeListVariableDescriptor;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getEntityValueRangeDestinationSelector;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getIterableFromEntityPropertyValueSelector;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getListVariableDescriptor;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getMimicRecordingIterableValueSelector;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getPinnedEntityRangeListVariableDescriptor;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.getPinnedListVariableDescriptor;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.mockAllowsUnassignedValuesNeverEndingDestinationSelector;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.mockDestinationSelector;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.mockIterableValueSelector;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.mockNeverEndingDestinationSelector;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.mockPinnedEntityRangeNeverEndingDestinationSelector;
import static ai.greycos.solver.core.testdomain.list.TestdataListUtils.mockPinnedNeverEndingDestinationSelector;
import static ai.greycos.solver.core.testutil.PlannerAssert.assertAllCodesOfMoveSelector;
import static ai.greycos.solver.core.testutil.PlannerAssert.assertAllCodesOfMoveSelectorWithoutSize;
...
class ListChangeMoveSelectorTest {
  ...
  void originalWithEntityValueRange() {
    ...
    var mimicRecordingValueSelector =
        getMimicRecordingIterableValueSelector(
    ...
    var entityDescriptor =
        solutionDescriptor.findEntityDescriptor(TestdataListEntityProvidingEntity.class);
    var destinationSelector =
        getEntityValueRangeDestinationSelector(
            mimicRecordingValueSelector, solutionDescriptor, entityDescriptor, false);
    var moveSelector =
        new ListChangeMoveSelector<>(mimicRecordingValueSelector, destinationSelector, false);
    ...
    var moveSelector =
        new ListChangeMoveSelector<>(
            mockIterableValueSelector(
                getPinnedListVariableDescriptor(scoreDirector), v4, v3, v1, v2),
            mockDestinationSelector(
                ElementPosition.of(c, 0),
                ElementPosition.of(c, 1),
                ElementPosition.of(a, 2),
  ...
  void originalWithPinningAndEntityValueRange() {
    ...
    var scoreDirector =
        mockScoreDirector(TestdataListPinnedEntityProvidingSolution.buildSolutionDescriptor());
    scoreDirector.setWorkingSolution(solution);

    var mimicRecordingValueSelector =
        getMimicRecordingIterableValueSelector(
            getPinnedEntityRangeListVariableDescriptor(scoreDirector).getValueRangeDescriptor(),
            false);
    var solutionDescriptor = scoreDirector.getSolutionDescriptor();
    var entityDescriptor =
        solutionDescriptor.findEntityDescriptor(TestdataListPinnedEntityProvidingEntity.class);
    var destinationSelector =
        getEntityValueRangeDestinationSelector(
            mimicRecordingValueSelector, solutionDescriptor, entityDescriptor, false);
    var moveSelector =
    ...
    // over all values.
    assertAllCodesOfMoveSelectorWithoutSize(
        moveSelector, "1 {A[1]->A[1]}", "3 {C[0]->A[1]}", "3 {C[0]->C[0]}", "3 {C[0]->A[2]}");
  }
  ...
  void originalAllowsUnassignedValues() {
    ...
    var scoreDirector =
        mockScoreDirector(TestdataAllowsUnassignedValuesListSolution.buildSolutionDescriptor());
    scoreDirector.setWorkingSolution(solution);

    var moveSelector =
        new ListChangeMoveSelector<>(
            mockIterableValueSelector(
                getAllowsUnassignedvaluesListVariableDescriptor(scoreDirector), v3, v1, v4, v2),
            mockDestinationSelector(
                ElementPosition.of(a, 0),
                ElementPosition.of(b, 0),
                ElementPosition.of(c, 0),
  ...
  void originalAllowsUnassignedValuesWithEntityValueRange() {
    ...
    var scoreDirector =
        mockScoreDirector(TestdataListUnassignedEntityProvidingSolution.buildSolutionDescriptor());
    scoreDirector.setWorkingSolution(solution);

    var mimicRecordingValueSelector =
        getMimicRecordingIterableValueSelector(
            getAllowsUnassignedvaluesEntityRangeListVariableDescriptor(scoreDirector)
                .getValueRangeDescriptor(),
            false);
    var solutionDescriptor = scoreDirector.getSolutionDescriptor();
    var entityDescriptor =
        solutionDescriptor.findEntityDescriptor(TestdataListUnassignedEntityProvidingEntity.class);
    var destinationSelector =
        getEntityValueRangeDestinationSelector(
            mimicRecordingValueSelector, solutionDescriptor, entityDescriptor, false);
    var moveSelector =
  ...
  void random() {
    ...
    var moveSelector =
        new ListChangeMoveSelector<>(
            TestdataListUtils.mockNeverEndingIterableValueSelector(
                getListVariableDescriptor(scoreDirector), v2, v1, v3, v3, v3),
    ...
    // The moved values (2, 1, 3, 3, 3) are supplied by the source value selector and their source
    // positions
    // are deduced using inverse relation and index supplies. The destinations (B[0], A[2], ...) are
    // supplied
  ...
  void randomWithEntityValueRange() {
    ...
    var destinationSelector =
        getEntityValueRangeDestinationSelector(
            mimicRecordingValueSelector, solutionDescriptor, entityDescriptor, true);
    var moveSelector =
        new ListChangeMoveSelector<>(mimicRecordingValueSelector, destinationSelector, true);
  ...
  void randomWithEntityValueRangeAndFiltering() {
    ...
    var destinationSelector =
        getEntityValueRangeDestinationSelector(
            ...
            solutionDescriptor,
            entityDescriptor,
            IgnoreBValueSelectionFilter.class,
            true);
    var moveSelector =
        new ListChangeMoveSelector<>(mimicRecordingValueSelector, destinationSelector, true);
    ...
    // IgnoreBValueSelectionFilter is applied to the value selector used by the destination
    // selector,
  ...
  void randomWithPinning() {
    ...
    var moveSelector =
        new ListChangeMoveSelector<>(
            TestdataListUtils.mockNeverEndingIterableValueSelector(
                getPinnedListVariableDescriptor(scoreDirector), v2, v1, v4, v3, v3),
    ...
    assertCodesOfNeverEndingMoveSelector(
        moveSelector, "2 {A[1]->C[0]}", "3 {C[0]->A[2]}", "3 {C[0]->A[1]}", "2 {A[1]->C[0]}");
  }
  ...
  void randomWithPinningAndEntityValueRange() {
    ...
    assertCodesOfNeverEndingMoveSelector(moveSelector, "3 {C[0]->C[0]}", "1 {A[1]->A[2]}");
  }
  ...
  public static class IgnoreBValueSelectionFilter
      implements SelectionFilter<
          TestdataListEntityProvidingSolution, TestdataListEntityProvidingValue> {
    ...
    public boolean accept(
        ScoreDirector<TestdataListEntityProvidingSolution> scoreDirector,
