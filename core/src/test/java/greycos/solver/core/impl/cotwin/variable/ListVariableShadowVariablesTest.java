...
import greycos.solver.core.impl.heuristic.selector.move.generic.list.SubListChangeMove;
import greycos.solver.core.impl.heuristic.selector.move.generic.list.SubListSwapMove;
...
class ListVariableShadowVariablesTest {
  ...
  private final InnerScoreDirector<TestdataListSolutionWithShadowHistory, SimpleScore>
      scoreDirector =
          PlannerTestUtils.mockScoreDirector(
              variableDescriptor.getEntityDescriptor().getSolutionDescriptor());

  static TestdataListSolutionWithShadowHistory buildSolution(
      TestdataListEntityWithShadowHistory... entities) {
    var values =
        Arrays.stream(entities)
            .flatMap(e -> e.getValueList().stream())
  ...
  static void assertIndexHistory(
      TestdataListValueWithShadowHistory element, Integer... indexHistory) {
  ...
  static void assertEntityHistory(
      TestdataListValueWithShadowHistory element,
  ...
  static void assertPreviousHistory(
      TestdataListValueWithShadowHistory element,
  ...
  static void assertNextHistory(
    ...
    assertThat(element.getNextHistory()).as("History is incorrect").containsExactly(nextHistory);
  }

  void doChangeMove(
      TestdataListEntityWithShadowHistory sourceEntity,
      int sourceIndex,
      TestdataListEntityWithShadowHistory destinationEntity,
  ...
  void doSwapMove(
      TestdataListEntityWithShadowHistory leftEntity,
      int leftIndex,
      TestdataListEntityWithShadowHistory rightEntity,
      int rightIndex) {
  ...
  void addAndRemoveElement() {
    ...
    assertThat(scoreDirector.calculateScore())
        .isEqualTo(InnerScore.withUnassignedCount(SimpleScore.ZERO, 1));
  ...
  /**
   * The point of this test case is not only to verify that swapped neighbors end up with the
   * expected indices, but also to verify that listeners do not receive excessive notifications.
   * Excessive (redundant, duplicate) notifications might cause troubles for custom listeners that
   * do incremental changes on a shadow variable.
   */
  @Test
  ...
  void swapSubListsSameEntity() {
    ...
    var ann = TestdataListEntityWithShadowHistory.createWithValues("Ann", a, b, c, d, e, f, g, h);
