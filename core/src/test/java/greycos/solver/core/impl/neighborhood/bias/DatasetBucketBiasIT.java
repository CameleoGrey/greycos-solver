...
class DatasetBucketBiasIT extends AbstractBiasIT {
  ...
  void bucketsAreWeightedByRemainingSizeAtDraw(int drawIndex) {
    ...
    BiNeighborhoodsJoiner<TestdataEntity, TestdataValue> joiner =
        NeighborhoodsJoiners.greaterThanOrEqual(
            entity -> Integer.parseInt(entity.getCode()),
            value -> Integer.parseInt(value.getCode()));
    ...
    BiasReport.tally(
            "just-in-time per-A unique, draw #" + drawIndex,
            TRIAL_COUNT,
            trial -> {
