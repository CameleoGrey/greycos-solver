 ...
 * biases which surviving left is drawn next ({@code DefaultRetiringRandomIterator}'s old "snap to
 * nearest active index" correction), or it gets retired even though it still has a real match
...
class LeftRetirementBiasIT extends AbstractBiasIT {
   ...
   * retirements were fair. 4 interior entities (out of 20) are unreachable ("dead": a bucket size
   * of 0 gives them no matching value at all, per {@link BucketedFixture}), which retires them for
   * good during the warm-up phase; the measured phase then checks that doing so left every survivor
   * equally likely.
   */
  @Test
  void interiorRetirementsStayUniformOverSurvivingEntities() {
    var entityCount = 20;
    var deadIndexSet =
        Set.of(4, 8, 12, 16); // Interior; the old bug was already unbiased at the edges.
    var warmUpDrawCount = 1_000;
    var measuredDrawCount = 200_000;

    var bucketSizeList =
        IntStream.range(0, entityCount).mapToObj(i -> deadIndexSet.contains(i) ? 0 : 1).toList();
    ...
    // Give every dead entity plenty of chances to be drawn and permanently retired before
    // measuring.
    ...
    BiasReport.tally(
            "interior retirement, live entities",
            measuredDrawCount,
            draw -> {
              var move = iterator.next();
              return ((TestdataEntity) move.getPlanningEntities().getFirst()).getCode();
            })
        .expectUniform(liveEntityCodeList)
        .assertWithinSigma(SIGMA_LIMIT);
  }

  /**
   * Under {@link NeighborhoodsJoiners#filtering(BiNeighborhoodsPredicate)}, {@code
   * FilteringIterator}'s bail-out is a per-call false negative, not proof of emptiness: before the
   * fix, one such bail-out was enough to permanently retire a left that still had a real match, and
   * "hard" (1 match out of 20 values) permanently disappeared partway through this run. After the
   * fix, {@code RetiringBiWalk.PROBE_ATTEMPT_COUNT} independent probes are required before giving
   * up. "hard" must therefore still be drawn near the end of the run, not only near the start.
   */
  @Test
  void hardToMatchEntityStaysReachableThroughoutTheRun() {
    ...
    var report =
        BiasReport.tally(
            "hard/easy filtering() reachability",
            drawCount,
            draw -> {
              var move = iterator.next();
    ...
    assertThat(hardCountInTail[0])
        .as(
            "'hard' must still be drawn in the final %d draws, not permanently retired",
            tailDrawCount)
        .isPositive();
   ...
   * from {@code PairFairnessBiasIT} is a structural no-op here): "easy" matches every value, "hard"
   * matches only {@code hardMatch}.
   */
  @NullMarked
  private record PickHardOrEasy(
      PlanningVariableMetaModel<TestdataSolution, TestdataEntity, TestdataValue> variable,
      TestdataValue hardMatch)
      implements MoveProvider<TestdataSolution> {

    @Override
    public MoveStream<TestdataSolution> build(
        MoveStreamFactory<TestdataSolution> moveStreamFactory) {
      ...
      var matchesUnlessHard =
          NeighborhoodsJoiners.<TestdataSolution, TestdataEntity, TestdataValue>filtering(
              (solutionView, entity, value) ->
                  !entity.getCode().equals("hard") || value.equals(hardMatch));
