...
/**
 * Tests declarative shadow variables sourced from the elements of a planning list variable, i.e.
 * {@code @ShadowSources("values[].endTime")} on the entity declaring the list variable.
 */
class ListElementShadowVariableTest {
  ...
  void listMovesUpdateAggregates() {
    ...
    var listVariableMetaModel =
        solutionMetaModel
            .genuineEntity(TestdataListElementEntity.class)
  ...
  @Test
  void unassigningLastElementAndReassigningUpdatesBothAggregates() {
    var value = new TestdataListElementValue("v1", 2);
    var entityA = new TestdataListElementEntity("A", 3);
    var entityB = new TestdataListElementEntity("B", 10);
    entityA.setValues(new ArrayList<>(List.of(value)));
    var solution = new TestdataListElementSolution();
    solution.setEntities(List.of(entityA, entityB));
    solution.setValues(List.of(value));
    var solutionMetaModel = TestdataListElementSolution.buildMetaModel();
    var listVariable =
        solutionMetaModel
            ...
            .listVariable("values", TestdataListElementValue.class);
    var context = MoveTester.build(solutionMetaModel).using(solution);
    assertThat(entityA.getLastEndTime()).isEqualTo(5);

    context.execute(Moves.unassign(listVariable, entityA, 0));
    assertThat(entityA.getValues()).isEmpty();
    assertThat(entityA.getLastEndTime()).isEqualTo(3);
    assertThat(value.getEntity()).isNull();
    assertThat(value.getEndTime()).isNull();

    context.execute(Moves.assign(listVariable, value, entityB, 0));
    assertThat(entityB.getLastEndTime()).isEqualTo(12);
    assertThat(value.getEntity()).isSameAs(entityB);
    assertThat(entityA.getLastEndTime()).isEqualTo(3);

    context.execute(Moves.change(listVariable, entityB, 0, entityA, 0));
    assertThat(entityA.getLastEndTime()).isEqualTo(5);
    assertThat(entityB.getValues()).isEmpty();
    assertThat(entityB.getLastEndTime()).isEqualTo(10);
  }
  ...
  void basicVariableChangeRetriggersAggregateWithoutListChange() {
    ...
    var solutionMetaModel =
        TestdataMixedListElementSolution.buildSolutionDescriptor().getMetaModel();
    var durationVariableMetaModel =
        solutionMetaModel
            .genuineEntity(TestdataMixedListElementValue.class)
  ...
  private void solveWithFullAssert(TestdataListElementSolution problem) {
    ...
    for (var entity : solution.getEntities()) {
      ...
      var expectedLastEndTime =
          values.isEmpty()
              ? Integer.valueOf(entity.getStartTime())
