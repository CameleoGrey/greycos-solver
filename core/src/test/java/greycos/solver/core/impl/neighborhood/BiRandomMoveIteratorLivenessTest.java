...
/**
 * {@code BiRandomMoveIterator} must always terminate: an empty left dataset, an empty right
 * dataset, and a {@code filtering()} joiner that rejects every pair are all different ways for
 * every left tuple to end up dead, and none of them may hang. A plain, synchronous assertion is
 * enough, since each case is bounded by construction (at most {@code
 * RetiringBiWalk.PROBE_ATTEMPT_COUNT} bail-outs per left tuple, not just one); a genuine regression
 * would hang the whole build, not just fail an assertion, since the working random used here is
 * checked for thread ownership and cannot safely be driven from a JUnit timeout's watcher thread.
 */
class BiRandomMoveIteratorLivenessTest {

  private static final PlanningVariableMetaModel<TestdataSolution, TestdataEntity, TestdataValue>
      VARIABLE =
  ...
  private void assertNeverHangs(MoveProvider<TestdataSolution> moveProvider) {
    ...
    var context =
        NeighborhoodTester.build(moveProvider, TestdataSolution.buildMetaModel()).using(solution);
  ...
  /**
   * Picks a (entity, value) pair, matching nothing: either the left or the right stream is forced
   * empty by a rejecting {@code filter()}, or (if neither is) the {@code filtering()} joiner itself
   * rejects every pair.
   */
  @NullMarked
    ...
    public MoveStream<TestdataSolution> build(
      ...
      var rejectEveryPair =
          NeighborhoodsJoiners.<TestdataSolution, TestdataEntity, TestdataValue>filtering(
              (solutionView, entity, value) -> false);
