 ...
 * delegate already ends by itself) has a whole lot of possible corner cases. Therefore, we run the
 * same tests with a variety of random seeds to increase coverage. None of these are expected to
 * fail. If any fail, the random seed is printed in the test name for reproducibility.
 */
...
class FilteredUniqueIterationTest {
  ...
  void throwsWhenUnknowinglyEmpty(int elementCount) {
    ...
    var sequence =
        new FilteringIterator<>(UniqueRandomIterator.of(toEntries(list), random), filter);
