...
class UnindexedJoinBiNodeTest {
  ...
  @Mock private TupleLifecycle<BiTuple<String, String>> downstream;
  ...
  void updateLeft_refreshesLeftFact() {
    var node =
        new UnindexedJoinBiNode<String, String>(downstream, (a, b) -> true, new TestTracker());
    ...
    // every layer's propagate phase (see AbstractBavetNodeNetwork#settleLayer), so a direct node
    // test
    // must too.
