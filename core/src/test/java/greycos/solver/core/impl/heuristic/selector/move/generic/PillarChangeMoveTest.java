...
import static greycos.solver.core.testutil.PlannerAssert.assertAllCodesOfCollection;
...
class PillarChangeMoveTest {
  ...
  void doMove() {
    ...
    var abcMove = new PillarChangeMove<>(Arrays.asList(a, b, c), variableDescriptor, v2);
  ...
  void rebase() {
    ...
    assertSameProperties(
        ...
        null,
        new PillarChangeMove<>(Arrays.asList(e1, e3), variableDescriptor, null)
            .rebase(destinationScoreDirector));
    assertSameProperties(
        Arrays.asList(destinationE1, destinationE3),
        destinationV3,
        new PillarChangeMove<>(Arrays.asList(e1, e3), variableDescriptor, v3)
            .rebase(destinationScoreDirector));
    assertSameProperties(
        Arrays.asList(destinationE2),
        destinationV1,
        new PillarChangeMove<>(Arrays.asList(e2), variableDescriptor, v1)
            .rebase(destinationScoreDirector));
    assertSameProperties(
        Arrays.asList(destinationE1),
        destinationV2,
        new PillarChangeMove<>(Arrays.asList(e1), variableDescriptor, v2)
            .rebase(destinationScoreDirector));
  }

  public void assertSameProperties(
      List<Object> pillar, Object toPlanningVariable, PillarChangeMove<?> move) {
  ...
  void toStringTest() {
    ...
    assertThat(new PillarChangeMove<>(Arrays.asList(a, b), variableDescriptor, v1))
        .hasToString("[a, b] {null -> v1}");
    assertThat(new PillarChangeMove<>(Arrays.asList(a, b), variableDescriptor, v2))
    ...
    assertThat(new PillarChangeMove<>(Arrays.asList(c, d, e), variableDescriptor, v2))
        .hasToString("[c, d, e] {v1 -> v2}");
    assertThat(new PillarChangeMove<>(Arrays.asList(d), variableDescriptor, v2))
        .hasToString("[d] {v1 -> v2}");
  }
}
