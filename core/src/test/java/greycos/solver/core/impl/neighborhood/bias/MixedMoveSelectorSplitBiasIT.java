...
/**
 * Confirms that {@link MixedMoveSelector} draws from its two sides in proportion to how many movers
 * each holds, not 50/50 regardless of that count: 3 classic move selectors against 2 neighborhoods
 * must yield a 3/5-2/5 split, never 1/2-1/2.
 */
class MixedMoveSelectorSplitBiasIT extends AbstractBiasIT {
  ...
  void splitsProportionallyToMoverCount() {
    ...
    var scoreDirector =
        new EasyScoreDirectorFactory<>(
                solutionDescriptor, s -> SimpleScore.ZERO, EnvironmentMode.PHASE_ASSERT)
            .buildScoreDirector();
    ...
    var report =
        BiasReport.tally(
            "mixed classic/neighborhoods split", 100_000, i -> classify(iterator.next()));

    mixedMoveSelector.stepEnded(stepScope);
    mixedMoveSelector.phaseEnded(phaseScope);
    mixedMoveSelector.solvingEnded(solverScope);

    // 3 classic move selectors vs. 2 neighborhoods: expected share is 3/5 and 2/5, never 1/2 and
    // 1/2.
    report
        .expectWeights(Map.of("classic", 3.0 / 5, "neighborhood", 2.0 / 5))
        .assertWithinSigma(SIGMA_LIMIT);
  }

  private static @NonNull MixedMoveSelector<TestdataSolution> getMixedMoveSelector(
      SolutionDescriptor<TestdataSolution> solutionDescriptor) {
    List<MoveSelector<TestdataSolution>> classicChildMoveSelectorList =
        List.of(
            new NeverEndingDummyMoveSelector("classic-a"),
            new NeverEndingDummyMoveSelector("classic-b"),
  ...
  private static @NonNull NeighborhoodsMoveSelector<TestdataSolution> getNeighborhoodsMoveSelector(
      SolutionDescriptor<TestdataSolution> solutionDescriptor) {
    ...
    var neighborhoodProviderList =
        List.of(
            neverEndingMoveProvider("neighborhood-a"), neverEndingMoveProvider("neighborhood-b"));
  ...
  private static MoveProvider<TestdataSolution> neverEndingMoveProvider(String code) {
    return moveStreamFactory ->
        moveStreamFactory.buildMoveStream((session, random) -> neverEndingIterator(code));
  }
