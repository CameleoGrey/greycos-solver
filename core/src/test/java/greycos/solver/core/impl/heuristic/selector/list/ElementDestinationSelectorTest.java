...
import static greycos.solver.core.impl.heuristic.selector.SelectorTestUtils.mockReplayingValueSelector;
...
import static greycos.solver.core.testcotwin.list.TestdataListUtils.getPinnedAllowsUnassignedvaluesListVariableDescriptor;
...
import static greycos.solver.core.testcotwin.list.TestdataListUtils.mockIterableFromEntityPropertyValueSelector;
import static greycos.solver.core.testcotwin.list.TestdataListUtils.mockIterableValueSelector;
import static greycos.solver.core.testcotwin.list.TestdataListUtils.mockUpcomingSelectionIterator;
import static greycos.solver.core.testutil.PlannerAssert.assertAllCodesOfIterableSelector;
import static greycos.solver.core.testutil.PlannerAssert.assertAllCodesOfIterator;
import static greycos.solver.core.testutil.PlannerAssert.assertCodesOfNeverEndingIterableSelector;
import static greycos.solver.core.testutil.PlannerAssert.assertCodesOfNeverEndingIterableSelectorWithoutSize;
import static greycos.solver.core.testutil.PlannerAssert.assertCodesOfNeverEndingIterator;
import static greycos.solver.core.testutil.PlannerAssert.assertEmptyNeverEndingIterableSelector;
...
import greycos.solver.core.impl.heuristic.selector.entity.FromSolutionEntitySelector;
...
class ElementDestinationSelectorTest {
  ...
  void original() {
    ...
    assertAllCodesOfIterableSelector(
        selector, destinationSize, "A[0]", "B[0]", "C[0]", "C[1]", "A[2]", "A[1]");
  }
  ...
  void random() {
    ...
    var valueSelector =
        TestdataListUtils.mockNeverEndingIterableValueSelector(
            getListVariableDescriptor(scoreDirector), v3, v2, v1);
    var destinationSize = entitySelector.getSize() + valueSelector.getSize();
    ...
    // The destinations (A[0], C[1], ...) depend on the random number, which decides whether
    ...
    assertCodesOfNeverEndingIterableSelector(
        selector, destinationSize, "A[0]", "C[1]", "B[0]", "A[2]", "A[1]", "C[0]");
  ...
  void randomWithEntityValueRange() {
    ...
    var filteringValueRangeSelector =
        mockIterableFromEntityPropertyValueSelector(valueSelector, true);
    ...
    // Random values
    // 3 - pick a random value in ElementPositionRandomIterator and force generating a random
    // position
    ...
    checkEntityValueRange(
        ...
        new FilteringValueRangeSelector<>(
            filteringValueRangeSelector, replayinValueSelector, true, false),
        scoreDirector,
        new TestRandom(3, 1, 0),
        "C[1]");
  }

  private void checkEntityValueRange(
      FilteringEntityByValueSelector<TestdataListEntityProvidingSolution> entitySelector,
    ...
    var selector = new ElementDestinationSelector<>(entitySelector, valueSelector, true);
    var solverScope =
        solvingStarted(selector, scoreDirector, random, entitySelector, valueSelector);
    phaseStarted(solverScope, selector);
  ...
  void randomPartiallyPinnedAndUnassigned() {
    ...
    var scoreDirector =
        mockScoreDirector(TestdataPinnedUnassignedValuesListSolution.buildSolutionDescriptor());
    scoreDirector.setWorkingSolution(solution);
    ...
    assertCodesOfNeverEndingIterator(
        selector.iterator(),
        "UnassignedLocation",
        "A[1]",
        "B[0]",
  ...
  void randomUnassignedSingleEntity() {
    ...
    var valueSelector =
        mockIterableValueSelector(
            getAllowsUnassignedvaluesListVariableDescriptor(scoreDirector), unassignedValue);
    var selector = new ElementDestinationSelector<>(entitySelector, valueSelector, true);
    selector.solvingStarted(solverScope);
    selector.phaseStarted(new LocalSearchPhaseScope<>(solverScope, 0));
  ...
  void randomFullyPinned() {
    ...
    var valueSelector =
        TestdataListUtils.mockNeverEndingIterableValueSelector(
            getPinnedListVariableDescriptor(scoreDirector), v3, v2, v1);
    var selector = new ElementDestinationSelector<>(entitySelector, valueSelector, true);
  ...
  void emptyIfThereAreNoEntities() {
    ...
    var valueSelector =
        mockIterableValueSelector(
            TestdataListEntity.buildVariableDescriptorForValueList(), v1, v2, v3);
