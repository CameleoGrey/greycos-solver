...
/**
 * See {@link ActivitySupport}. The node must count every fact it has ever seen, regardless of
 * filter outcome; filter transitions during update() must not corrupt that count. Updates can
 * arrive before the activation check runs, because shadow variable initialization happens between
 * fact insertion and the first settle.
 */
