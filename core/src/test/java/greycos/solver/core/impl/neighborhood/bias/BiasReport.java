 ...
 * margin against the bound is never silently discarded on a pass; see {@link
 * AbstractBiasIT#SIGMA_LIMIT}'s javadoc for why the bound is a sigma count, not a hand-picked
 * percentage.
 *
...
final class BiasReport<Category_> {
  ...
  /**
   * Every category in {@code expectedCategoryCollection} is expected in equal share ({@code 1 /
  ...
  void assertWithinSigma(double sigmaLimit) {
    ...
    var worst =
        deviationList.stream().max(Comparator.comparingDouble(Deviation::sigma)).orElseThrow();
  ...
  /**
   * For a deliberately non-uniform fixture (no {@code expect*} call needed): asserts that {@code
   * smallCategory}'s per-member rate is at least {@code minRatio} times {@code largeCategory}'s,
   * where a category's rate is its observed count divided by the given member count (e.g. its
   * bucket size).
   */
  void assertShareRatioAtLeast(
      Category_ smallCategory,
      int smallCategorySize,
      Category_ largeCategory,
      int largeCategorySize,
      double minRatio) {
    ...
    var summary =
        "[bias] %s: %d samples; %s rate %.2f (count %d / %d), %s rate %.2f (count %d / %d)"
            .formatted(
                label,
                sampleCount,
                smallCategory,
    ...
    assertThat(smallCount)
        .as(() -> summary + "; both categories must be reachable at all")
        .isPositive();
    ...
    assertThat(smallRate)
        .as(
            () ->
                summary
                    + "; %s's per-member rate must be at least %sx %s's"
                        .formatted(smallCategory, minRatio, largeCategory))
        .isGreaterThan(largeRate * minRatio);
  }

  private record Deviation<Category_>(
      Category_ category, long observed, double expected, double sigma) {
  ...
  private List<Deviation<Category_>> deviations() {
    ...
    for (var entry : expectedShareByCategory.entrySet()) {
      ...
      var sigma =
          binomialStandardDeviation == 0
              ? 0
              : Math.abs(observed - expected) / binomialStandardDeviation;
  ...
  private String table(
      List<Deviation<Category_>> deviationList, Deviation<Category_> worst, double sigmaLimit) {
    var statistics = new SummaryStatistics();
    for (var deviation : deviationList) {
      statistics.addValue(deviation.observed());
    }
    var unreachedCategoryList =
        deviationList.stream()
            .filter(d -> d.expected() > 0 && d.observed() == 0)
            .map(d -> d.category())
            .toList();
    // Every category's expected count is (about) the same only for a uniform expectation; the
    // "expected"
    // binomial spread reported here is therefore an average across categories, informative, not
    // exact.
    var averageBinomialStandardDeviation =
        deviationList.stream()
            .mapToDouble(
                d ->
                    Math.sqrt(
                        sampleCount
                            * (d.expected() / sampleCount)
                            * (1 - d.expected() / sampleCount)))
            .average()
            .orElse(0);
    var builder = new StringBuilder();
    builder.append(
        "[bias] %s: %d samples over %d categories, limit %.1f sigma%n"
            .formatted(label, sampleCount, deviationList.size(), sigmaLimit));
    builder.append(
        "  worst deviation at %s: observed %d, expected %.1f (%+.2f%%), %.2f sigma%n"
            .formatted(
                worst.category(),
                worst.observed(),
                worst.expected(),
                worst.deviationPercent(),
                worst.sigma()));
    builder.append(
        "  spread: min %.0f, max %.0f, stdev %.1f, average expected stdev %.1f (binomial)%n"
            .formatted(
                statistics.getMin(),
                statistics.getMax(),
    ...
    builder.append(
        "  headroom: %.2f sigma to the limit; unreached categories: %s%n"
            .formatted(
                sigmaLimit - worst.sigma(),
                unreachedCategoryList.isEmpty() ? "none" : unreachedCategoryList));
    var sortedBySigmaDescending =
        deviationList.stream()
            .sorted(Comparator.comparingDouble(Deviation<Category_>::sigma).reversed())
            .toList();
    var detailList =
        sortedBySigmaDescending.size() <= MAX_DETAIL_ROW_COUNT
            ? sortedBySigmaDescending
            : sortedBySigmaDescending.subList(0, 5);
    for (var deviation : detailList) {
      builder.append(
          "    %s: observed %d, expected %.1f (%+.2f%%), %.2f sigma%n"
              .formatted(
                  deviation.category(),
                  deviation.observed(),
                  deviation.expected(),
