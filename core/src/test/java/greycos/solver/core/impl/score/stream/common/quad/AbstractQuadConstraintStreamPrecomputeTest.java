...
import greycos.solver.core.api.score.stream.PrecomputeFactory;
...
import greycos.solver.core.impl.score.stream.common.ConstraintStreamPrecomputeTest;
...
public abstract class AbstractQuadConstraintStreamPrecomputeTest
    extends AbstractConstraintStreamTest implements ConstraintStreamPrecomputeTest {
  ...
  private <A, B, C, D> void assertPrecomputeFilterChanged(
    ...
    var createMatch =
        (QuadFunction<
                ...
                TestdataLavishValueGroup,
                AssertableMatch>)
            (entity, value, matchEntityGroup, matchValueGroup) -> {
  ...
  public void filter_0_changed() {
    assertPrecomputeFilterChanged(
        (precomputeFactory, entityGroup, valueGroup) ->
            precomputeFactory
                .forEachUnfiltered(TestdataLavishEntity.class)
                .join(TestdataLavishValue.class)
                .join(TestdataLavishEntityGroup.class)
                .join(TestdataLavishValueGroup.class)
                .filter(
                    (entity, value, matchedEntityGroup, matchedValueGroup) ->
                        entity.getEntityGroup() == entityGroup
  ...
  public void filter_1_changed() {
    assertPrecomputeFilterChanged(
        (precomputeFactory, entityGroup, valueGroup) ->
            precomputeFactory
                .forEachUnfiltered(TestdataLavishValue.class)
                .join(TestdataLavishEntity.class)
                .join(TestdataLavishEntityGroup.class)
                .join(TestdataLavishValueGroup.class)
                .filter(
                    (value, entity, matchedEntityGroup, matchedValueGroup) ->
                        entity.getEntityGroup() == entityGroup
                            && value.getValueGroup() == valueGroup
                            && matchedEntityGroup == entityGroup
                            && matchedValueGroup == valueGroup),
        (value, entity, entityGroup, valueGroup) -> entity,
        (entity, value, entityGroup, valueGroup) ->
            new Quadruple<>(value, entity, entityGroup, valueGroup));
  }
    ...
    assertPrecomputeFilterChanged(
        (precomputeFactory, entityGroup, valueGroup) ->
            precomputeFactory
                ...
                .join(TestdataLavishValueGroup.class)
                .filter(
                    (value, matchedEntityGroup, entity, matchedValueGroup) ->
                        entity.getEntityGroup() == entityGroup
                            && value.getValueGroup() == valueGroup
                            && matchedEntityGroup == entityGroup
                            && matchedValueGroup == valueGroup),
        ...
        (entity, value, entityGroup, valueGroup) ->
            new Quadruple<>(value, entityGroup, entity, valueGroup));
  }
    ...
    assertPrecomputeFilterChanged(
        (precomputeFactory, entityGroup, valueGroup) ->
            precomputeFactory
                ...
                .join(TestdataLavishEntity.class)
                .filter(
                    (value, matchedEntityGroup, matchedValueGroup, entity) ->
                        entity.getEntityGroup() == entityGroup
                            && value.getValueGroup() == valueGroup
                            && matchedEntityGroup == entityGroup
                            && matchedValueGroup == valueGroup),
        ...
        (entity, value, entityGroup, valueGroup) ->
            new Quadruple<>(value, entityGroup, valueGroup, entity));
  }

  private <A, B, C, D> void assertPrecompute(
  ...
  public void ifExists() {
    ...
    assertPrecompute(
        solution,
        List.of(new Quadruple<>(entityWithGroup, value, value, value)),
        pf ->
            pf.forEachUnfiltered(TestdataLavishEntity.class)
                ...
                .ifExists(
                    ...
                    Joiners.equal((a, b, c, d) -> a.getEntityGroup(), Function.identity())));
  }
  ...
  public void flattenLast() {
    ...
    assertPrecompute(
        solution,
        List.of(
            new Quadruple<>(entityWithoutGroup, value, value, value),
  ...
  public void flattenLastNewInstances() {
    ...
    assertPrecompute(
        solution,
        List.of(
            new Quadruple<>(new ValueHolder(1), value, value, value),
  ...
  public void map() {
    ...
    solution
        .getEntityList()
        .addAll(List.of(entityWithoutGroup, entityWithGroup1, entityWithGroup2));
    solution.getEntityGroupList().add(entityGroup);
    var value = new TestdataLavishValue();
    solution.getValueList().add(value);
    ...
    assertPrecompute(
        ...
        pf ->
            pf.forEachUnfiltered(TestdataLavishEntity.class)
                ...
                .join(TestdataLavishValue.class)
                .filter(
                    (entity, joinedValue1, joinedValue2, joinedValue3) ->
                        entity.getEntityGroup() != null)
                .map(
                    (entity, joinedValue1, joinedValue2, joinedValue3) -> entity.getEntityGroup(),
  ...
  public void distinct() {
    ...
    assertPrecompute(
        solution,
        List.of(new Quadruple<>(entityGroup, value, value, value)),
        pf ->
            pf.forEachUnfiltered(TestdataLavishEntity.class)
                .join(TestdataLavishValue.class)
                .join(TestdataLavishValue.class)
