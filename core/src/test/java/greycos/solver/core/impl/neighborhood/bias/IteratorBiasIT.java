...
import greycos.solver.core.impl.bavet.common.index.UniqueRandomIterator;
...
/**
 * The bias fixes at the {@code bavet.common.index} iterator/indexer level: uniformity of {@link
 * RepeatingRandomIterator}, {@code DefaultRetiringRandomIterator} after retirement, {@link
 * UniqueRandomIterator}'s full-drain permutation order, and both flavors' multi-bucket weighting by
 * bucket size.
 */
class IteratorBiasIT extends AbstractBiasIT {

  private static final int SAMPLE_COUNT = 100;
  private static final List<Integer> SAMPLES = IntStream.range(0, SAMPLE_COUNT).boxed().toList();
  ...
  void repeatingRandomIteratorIsUniformAtDraw(int n) {
    ...
    BiasReport.tally(
        ...
        .expectUniform(SAMPLES)
        .assertWithinSigma(SIGMA_LIMIT);
  }

  /**
   * Protects {@code ComparisonIndexer}'s plain (repeating, with-replacement) flavor: it must pick
   * across every matching bucket, weighted by bucket size, not just the first bucket it encounters
   * while walking the boundary. Parameterized over both directions: LTE walks {@code comparisonMap}
   * forward, GTE walks it in {@code reverseOrder}, and the two directions share no code path other
   * than {@code collectMatchingBuckets} itself. The GTE query (10) is also the smallest bucket's
   * own key, so it additionally decides the sign-flipped {@code hasOrEquals} branch under sampling.
   */
  ...
  void comparisonIndexerRandomIteratorWeightsByBucketSize(JoinerType joinerType, int queryAge) {
    ...
    var joiner =
        (DefaultBiNeighborhoodsJoiner<TestPerson, TestPerson>)
            (joinerType == JoinerType.LESS_THAN_OR_EQUAL
    ...
    BiasReport.tally(
            "ComparisonIndexer repeating, multi-bucket by size (%s)".formatted(joinerType),
            trialCount,
  ...
  void containingAnyOfUniqueIteratorIsUniformOverOverlappingBuckets() {
    var trialCount = 200_000;
    var joiner =
        new DefaultBiNeighborhoodsJoiner<>(
            TestSkilledPerson::skills, JoinerType.CONTAINING_ANY_OF, TestSkilledPerson::skills);
    Indexer<UniTuple<String>> indexer = new IndexerFactory<>(joiner).buildIndexer(true);
    ...
    var random = new Random(0);
    BiasReport.tally(
            "ContainingAnyOfIndexer unique, overlapping buckets",
            trialCount,
  ...
  /**
   * Retiring an interior element used to oversample its surviving neighbors, since a draw that
  ...
  void retiringRandomIteratorStaysUniformAfterRetirement() {
    ...
    var retiredElementSet =
        Set.of(5, 12); // Interior retirements; the old bug was unbiased at the edges.
  ...
  /**
   * Draws (with replacement) until {@code target} is picked, then retires it. Any non-matching draw
   * along the way is left untouched, exactly as an ordinary caller that decides not to retire what
   * it just drew would leave it.
   */
   ...
   * size exactly like the first, by the same symmetry a uniformly shuffled deck has: every position
   * in the shuffle is equally likely to hold a card from any given suit, in proportion to that
   * suit's size.
   */
  @MethodSource("multiBucketUniqueRandomIteratorArguments")
  @ParameterizedTest
  void multiBucketUniqueRandomIteratorWeightsByBucketSize(
      MultiBucketFlavour flavour, int drawIndex) {
    var trialCount = 200_000;
    ...
    var root = new Random(0);
    BiasReport.tally(
            "%s multi-bucket unique, draw #%d".formatted(flavour, drawIndex),
            trialCount,
            trial -> {
              var splitRandom = splitFrom(root);
  ...
  /**
   * The two indexers whose multi-bucket {@code uniqueRandomIterator} is backed by {@code
   * MultiBucketUniqueRandomIterator}. {@code ContainingAnyOfIndexer} is deliberately excluded: its
   * buckets can overlap, so no bucket weighting can make it uniform, and it drains to a list
   * instead.
   */
  private enum MultiBucketFlavour {
    COMPARISON {
      ...
      @Override
      UniqueRandomIterator<UniTuple<String>> uniqueRandomIterator(
          Indexer<UniTuple<String>> indexer, RandomGenerator random) {
    ...
    CONTAINED_IN {
      ...
      Indexer<UniTuple<String>> buildIndexer() {
        var joiner =
            new DefaultBiNeighborhoodsJoiner<TestWorker, TestJob>(
                TestWorker::skills, JoinerType.CONTAINED_IN, TestJob::skill);
        Indexer<UniTuple<String>> indexer = new IndexerFactory<>(joiner).buildIndexer(true);
    ...
    abstract UniqueRandomIterator<UniTuple<String>> uniqueRandomIterator(
        Indexer<UniTuple<String>> indexer, RandomGenerator random);
  }

  /**
   * Only used for their accessor method references, to build a {@code ContainedInIndexer} directly
   * via {@code JoinerType.CONTAINED_IN}; no instance of either is ever constructed, since buckets
   * here are populated directly by {@link #putContainedInBucket}.
   */
