...
import greycos.solver.core.testcotwin.shadow.declarative.basicinverse.TestdataBasicInverseEntity;
import greycos.solver.core.testcotwin.shadow.declarative.basicinverse.TestdataBasicInverseGroup;
import greycos.solver.core.testcotwin.shadow.declarative.basicinverse.TestdataBasicInverseOwner;
import greycos.solver.core.testcotwin.shadow.declarative.basicinverse.TestdataBasicInverseSolution;

import org.junit.jupiter.api.Test;

/**
 * Covers the map-mode-only path of {@link
 * ChangedVariableNotifier#getCollectionInverseVariableSupply}: the "group" basic variable of {@link
 * TestdataBasicInverseEntity} has no {@code @InverseRelationShadowVariable} declared anywhere, so
 * the declarative "ownerCode" shadow (sourced from "group.owner") can only be kept correct if the
 * framework demands a fresh, non-externalized {@code BasicVariableStateSupply} to track "which
 * entities point to a given group" purely in memory.
 */
class BasicInverseShadowVariableTest {
  ...
  void declarativeShadowTracksBasicVariableChangeThroughNonExternalizedInverseSupply() {
    ...
    var solution =
        new TestdataBasicInverseSolution(
            List.of(entity), List.of(groupA, groupB), List.of(ownerX, ownerY));

    var solutionMetaModel =
        PlanningSolutionMetaModel.of(
            TestdataBasicInverseSolution.class,
            TestdataBasicInverseEntity.class,
            TestdataBasicInverseGroup.class);
    var groupVariable =
        solutionMetaModel
            .genuineEntity(TestdataBasicInverseEntity.class)
            .basicVariable("group", TestdataBasicInverseGroup.class);
    var ownerVariable =
        solutionMetaModel
            .genuineEntity(TestdataBasicInverseGroup.class)
