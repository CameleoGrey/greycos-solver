...
class PairFairnessBiasIT extends AbstractBiasIT {
  ...
  void indexedJoin_bucketsAreUniformPerPair(List<Integer> bucketSizeList) {
    ...
    BiasReport.tally(
            "indexed join, buckets %s".formatted(bucketSizeList),
            drawCount,
            draw -> iterator.next())
  ...
  /**
   * A same-shape regression guard on {@code CachedBiDatasetInstance}, unaffected by the fix since
   * it draws from all materialized pairs directly, with no per-left walk at all.
   */
  @Test
  void cachedJoin_sameSkewedShapeStaysUniformOverAllMaterializedPairs() {
    ...
    BiasReport.tally(
            "cached join, buckets %s".formatted(BUCKET_SIZE_LIST),
            drawCount,
            draw -> {
              iterator.next();
              return List.<Object>of(iterator.a(), iterator.b());
            })
        .expectUniform(expectedPairList)
        .assertWithinSigma(SIGMA_LIMIT);
  }

  /**
   * {@code JustInTimeBiDatasetInstance}'s whole-dataset {@code exhaustiveIterator()} (as opposed to
   * its per-A flavor) does not override {@code acceptLeft}, so it is unaffected by the fairness
   * fix; a full drain must still visit every pair exactly once, regardless of bucket size.
   */
  @Test
  ...
  /**
   * A filtering()-only joiner has no composite key to restrict the right side by, so {@code weight
   * == bound} for every left always: the fairness fix is a structural no-op here, deliberately.
   * This only checks reachability and a loose starvation floor, not uniformity, since the old
   * per-left bias is still present and expected.
   */
  @Test
  void filteringOnlyJoin_biasIsUnchangedButEveryPairStaysReachable() {
    ...
    var fixture = new BucketedFixture(BUCKET_SIZE_LIST);
    var iterator =
        moveIterator(new PickPair(fixture.variable, fixture.filteringJoiner()), fixture.solution);
    ...
    BiasReport.tally(
            "filtering()-only join, buckets %s".formatted(BUCKET_SIZE_LIST),
            drawCount,
            draw -> {
        ...
        .assertShareRatioAtLeast(
            smallestLabel,
            BUCKET_SIZE_LIST.getFirst(),
            largestLabel,
            BUCKET_SIZE_LIST.getLast(),
            5);
  }
}
