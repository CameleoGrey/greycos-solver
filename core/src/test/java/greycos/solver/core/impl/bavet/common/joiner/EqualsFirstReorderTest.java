...
/**
 * Verifies the equal-first reorder primitive ({@code AbstractJoiner.equalsFirstOrder} via {@code
 * DefaultBiJoiner.reorderedEqualsFirst()}) that the comber applies, including its node-sharing
 * consequence.
 */
class EqualsFirstReorderTest {
  ...
  void movesEqualToFrontCarryingMappings() {
    var joiner =
        (DefaultBiJoiner<Shift, Shift>) Joiners.lessThan(startFn).and(Joiners.equal(employeeFn));
  ...
  void alreadyEqualFirstReturnsSameInstance() {
    var joiner =
        (DefaultBiJoiner<Shift, Shift>) Joiners.equal(employeeFn).and(Joiners.lessThan(startFn));
  ...
  void equivalentJoinsInDifferentOrderShareViaEquality() {
    ...
    assertThat(equalFirst.reorderedEqualsFirst())
        .hasSameHashCodeAs(equalLast.reorderedEqualsFirst());
  }
}
