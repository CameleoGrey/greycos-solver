...
final class BavetRegressionTest extends AbstractConstraintStreamTest {
  ...
  void joinWithNullKeyFromRight() {
    ...
    assertScore(scoreDirector, assertMatch(entity1, entity1), assertMatch(entity2, entity2));
  ...
  public void filteringJoinNullConflictRight() {
    ...
    try (InnerScoreDirector<TestdataAllowsUnassignedValuesListSolution, SimpleScore> scoreDirector =
      ...
      assertScore(scoreDirector, assertMatch(value1, value1));
      ...
      assertScore(scoreDirector, assertMatch(value2, value2));
  ...
  /**
   * Like {@link #filteringJoinNullConflictRightUnassignOne()}, but the null-checking filter sits
   * behind a 3-hop self-join instead of a single bi-join. A bi-join's left input is always the root
   * tuple, which sits at layer 0 and is never itself mid-retraction when this join reads it, so a
   * single hop can't expose the race this test targets. Two hops deeper, the left input is itself a
   * join product living in a strictly later network layer -- reading it here is exactly the case
   * {@code AbstractJoinNode#prepareForSettle()} exists to close: without deferring the cross-match
   * to this node's own layer turn, the left tuple's retraction may not yet have propagated through
   * its own layer by the time this join reads it, exposing a stale, "still active" tuple to the
   * filtering predicate.
   */
  @TestTemplate
  public void filteringJoinNullConflictRightQuadJoinUnassignOne() {
    ...
    try (InnerScoreDirector<TestdataAllowsUnassignedValuesListSolution, SimpleScore> scoreDirector =
      ...
      assertScore(scoreDirector, assertMatch(value2, value2, value2, value2));
    }
  }
   ...
   * {@code AbstractJoinNode#insertOutTupleIfActiveFiltered} skips {@code testFiltering} entirely
   * for them and inserts unconditionally -- so a stale-but-"active" read here can only ever produce
   * an out-tuple that the true retraction (arriving later, deeper in the same layer chain) cleans
   * up via the retracted tuple's own out-tuple list. The final score must converge correctly
   * regardless of read timing, which is exactly why non-filtering joins skip the deferred
   * cross-match machinery entirely (see {@code AbstractJoinNode#hasDeferredWork()}).
   */
  @TestTemplate
  ...
  /**
   * Like {@link #filteringJoinNullConflictRightQuadJoinUnassignOne()}, but the trigger is an
   * in-place list reorder (an UPDATE on the index/previous/next shadow variables) instead of an
   * unassign (a RETRACT). Both values stay assigned throughout, so this exercises the deferred
   * cross-match on the update path specifically -- {@code AbstractJoinNode#innerUpdateLeft}'s
   * filtering branch enqueues the left tuple and lets {@code prepareForSettle()} re-run its
   * cross-match once every layer has settled -- rather than the insert-path deferral the
  ...
  public void filteringIfExistsNullConflictRight() {
    ...
    try (InnerScoreDirector<TestdataAllowsUnassignedValuesListSolution, SimpleScore> scoreDirector =
      ...
      assertScore(scoreDirector, assertMatch(value1));
      ...
      assertScore(scoreDirector, assertMatch(value2));
  ...
  /**
   * Like {@link #filteringIfExistsNullConflictRight()}, but the left input to ifExists is a 3-hop
   * self-join product instead of the root tuple directly — the ifExists counterpart to {@link
   * #filteringJoinNullConflictRightQuadJoinUnassignOne()}.
   */
  @TestTemplate
   ...
   * self-join chain is ifExists' *right* input instead of its left, so the stale read this time is
   * on the right tuple, going through {@code AbstractIfExistsNode#updateCounterLeft(counter,
   * rightTuple)}.
   */
  @TestTemplate
   ...
   * value, reorders the list in place -- the ifExists counterpart to {@link
   * #filteringJoinNullConflictThroughQuadJoinReorder()}. Triggers a pure UPDATE (index/
   * previous/next shadow changes only, same composite key) through the chain into the ifExists
   * node, rather than a retract+insert.
   */
  @TestTemplate
   ...
   * ifExists, exercising the inverted counter logic ({@code countRight == 0}) through the same
   * 3-hop self-join chain on the left input.
   */
  @TestTemplate
  ...
  void filteringJoinNullConflict() {
                              ...
                              (a, b) -> {
                                if (a.getValue() == null) {
                                  throw new IllegalStateException(
                                      "Impossible state: value of A is null even though forEach()"
                                          + " should have eliminated it.");
                                } else if (b.getValue() == null) {
                                  throw new IllegalStateException(
                                      "Impossible state: value of B is null even though join()'s"
                                          + " inner forEach() should have eliminated it.");
                                }
                                return true;
                              }))
                      .penalize(SimpleScore.ONE)
    ...
    entity2.setValue(null);
    scoreDirector.afterVariableChanged(entity2, "value");
    assertScore(scoreDirector, assertMatch(entity1, entity1));
                      ...
                      .join(
                          ...
                          filtering(
                              (a, b) -> {
                                if (a.getValue() == null) {
                                  throw new IllegalStateException(
                                      "Impossible state: value of A is null even though filter()"
                                          + " should have eliminated it.");
                                } else if (b.getValue() == null) {
                                  throw new IllegalStateException(
                                      "Impossible state: value of B is null even though join()'s"
                          ...
                          filtering(
                              (a, b) -> {
                                if (a.getValue() == null) {
                                  ...
                                  throw new IllegalStateException(
                                      "Impossible state: value of B is null even though"
                                          + " ifExists()'s inner forEach() should have eliminated"
                                          + " it.");
                                }
  ...
  void concatSameTupleDeadAndAlive() {
    ...
    scoreDirector.setWorkingSolution(solution);
    assertScore(scoreDirector, assertMatch(entity1), assertMatch(entity1), assertMatch(entity2));
    ...
    scoreDirector.afterVariableChanged(entity2, "value");
    assertScore(scoreDirector, assertMatch(entity1), assertMatch(entity2), assertMatch(entity2));
  ...
  void clearEvents() {
    var scoreDirector =
        (BavetConstraintStreamScoreDirector<
                TestdataListMultipleShadowVariableSolution, SimpleScore>)
            buildScoreDirector(
  ...
  /**
   * Like {@link #filteringJoinNullConflictRightUnassignOne()}, but the left input to the filtering
   * join is derived via a {@code .map()} from an earlier join's output, instead of being that
   * earlier join's product directly. Map retracts its own out-tuple only once its input tuple
   * retracts, at its own layer's turn -- the same layer-ordering shape as an intermediate join's
   * own output, just with one more node type standing between the root and the filtering join that
   * reads it.
   */
  @TestTemplate
            ...
            factory ->
                new Constraint[] {
                  factory
                      ...
                      .map(
                          (TestdataAllowsUnassignedValuesListValue a,
  ...
  /**
   * Like {@link #filteringJoinNullConflictThroughMapUnassignOne()}, but with {@code .flatten()}
   * instead of {@code .map()} in the middle of the chain. {@link
   * greycos.solver.core.impl.bavet.common.AbstractFlattenNode} retracts its own out-tuples only
   * once its input tuple retracts, at its own layer's turn -- the same layer-ordering shape as map,
   * so it should reproduce the same way.
   */
  @TestTemplate
  ...
  /**
   * Like {@link #filteringJoinNullConflictThroughMapUnassignOne()}, but with {@code .groupBy()}
   * instead of {@code .map()} in the middle of the chain. A group's out-tuple aggregates over every
   * contributor mapped into it (an N:1 relationship, unlike map/flatten's 1:1) and only retracts
   * once the group becomes empty, at its own layer's turn -- the same layer-ordering shape as map
   * and flatten, so it should reproduce the same way despite the different membership-tracking
   ...
   * mapped into a single group instead of one. Confirms {@code Group}'s contributor tracking
   * correctly identifies that the group survives when only one of many contributors dies, with the
   * dying contributor's value excluded downstream.
   */
  @TestTemplate
   ...
   * downstream once its {@code TupleList} of contributors becomes empty — exercises {@code
   * Group#isEmpty()}/{@code AbstractGroupNode#killOutTuple} for a many-contributor group, not just
   * the single-contributor case the disabled-then-fixed test above already covers.
   */
  @TestTemplate
  public void filteringJoinNullConflictThroughLargeGroupUnassignAll() {
    ...
    try (InnerScoreDirector<TestdataAllowsUnassignedValuesListSolution, SimpleScore> scoreDirector =
      ...
      // Unassign every contributor at once; the group must fully retract, with nothing left
      // downstream.
      var variableDescriptor = scoreDirector.getSolutionDescriptor().getListVariableDescriptor();
      for (var value : values) {
