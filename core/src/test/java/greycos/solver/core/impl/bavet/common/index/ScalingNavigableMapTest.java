...
class ScalingNavigableMapTest {
  ...
  /**
   * Builds a map that has treeified via churn (not via the size ceiling): fills 0..ARRAY_THRESHOLD
   * (65 keys, still array-mode), then removes and immediately re-adds key 0, CHURN_TOLERANCE times.
   * Size drops to ARRAY_THRESHOLD (still >= ARRAY_THRESHOLD) on the first removal, so every cycle
   * counts as churn at scale; after CHURN_TOLERANCE cycles the map has treeified.
   */
