 ...
 *
 * <p>These tests assert a <b>statistical</b> property (uniform, weight-proportional, or a
...
abstract class AbstractBiasIT {
   ...
   *
   * <p>These are max-of-many-categories tests, so the multiple-comparison penalty is real: at 5
   * sigma, a test over {@code k} categories has an expected false-failure rate of about {@code k *
   * 5.7e-7} per run (two-sided). At the largest fixture here (310 pairs) that is about {@code
   * 1.8e-4} per run; at 3 sigma it would be about 1 run in 120. Do not lower this to make a
   * specific case pass; a case that only passes below 5 sigma has a real unexplained bias, and
   * belongs either fixed or documented with a per-case override, not hidden by a looser global
   * constant.
   */
  static final double SIGMA_LIMIT = 5.0;

  /**
   * Builds one trial's seed from a root random, rather than from an incrementing counter ({@code
   * new Random(0)}, {@code new Random(1)}, ...): {@link Random}'s first {@code nextInt(2)} call is
   * constant across such small, close seeds (an LCG artifact), which would make a first-draw bias
   * undetectable no matter how large it is.
   */
  ...
  static Iterator<Move<TestdataSolution>> moveIterator(
      MoveProvider<TestdataSolution> moveProvider, TestdataSolution solution) {
   ...
   * route of their own.
   */
  static DefaultNeighborhoodSession<TestdataSolution> session(
  ...
  /**
   * Picks (entity, value) pairs matched by the given joiner; reused for both the indexing {@code
   * equal} and the {@code filtering()} shape.
   */
  @NullMarked
  record PickPair(
