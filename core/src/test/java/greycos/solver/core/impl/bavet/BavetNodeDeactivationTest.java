...
import greycos.solver.core.impl.bavet.uni.AbstractForEachUniNode;
...
class BavetNodeDeactivationTest {
  ...
  /**
   * Builds the network for the given constraints + solution; setWorkingSolution settles it
   * (read-only; no scoring).
   *
   * <p>Typed as {@link AbstractBavetNodeNetwork}, not the concrete
   * ConstraintStreamsBavetNodeNetwork: the seam methods getActiveNodes()/getNodes() are
   * package-private in AbstractBavetNodeNetwork, and a subclass in a different package does not
   * inherit package-private members (JLS 8.4.8), so a call on the concrete type would not compile.
   * This test lives in package greycos.solver.core.impl.bavet, so a base-typed reference resolves
   * them.
   *
   * <p>Scoring is performed by a Scorer (a TupleLifecycle) held outside the node network, so no
   * node carries StreamKind.SCORING; the activity of a constraint's terminal node already reflects
   * whether its scorer stayed active (terminal isActive() == canProduceTuples() &amp;&amp;
   * scorer.isActive()).
   */
  private static AbstractBavetNodeNetwork settle(
      ConstraintProvider provider, TestdataLavishSolution solution) {
    ...
    // setWorkingSolution -> afterSetWorkingSolution -> session.settle() (guarded); network is
    // settled on return.
  ...
  private static boolean active(AbstractBavetNodeNetwork net, StreamKind kind) {
    return net.getActiveNodes().stream().anyMatch(n -> n.getStreamKind() == kind);
  }

  private static boolean present(AbstractBavetNodeNetwork net, StreamKind kind) {
    return net.getNodes().stream().anyMatch(n -> n.getStreamKind() == kind);
  }
  ...
  private static long forEachCount(Collection<AbstractNode> nodes, Class<?> forEachClass) {
    return nodes.stream()
        .filter(
            n ->
                n instanceof AbstractForEachUniNode<?> fe
                    && fe.getForEachClass().equals(forEachClass))
        .count();
  }
  ...
  private static TestdataLavishSolution solutionWithoutExtras() {
    return TestdataLavishSolution
        .generateSolution(); // entityList populated; extraList empty by default
  }
  ...
  void emptyJoinSideDeactivatesChain() {
    ...
    assertThat(activeForEach(net, TestdataLavishEntity.class))
        .isFalse(); // feeds only the dead join
  }
  ...
  void concatWithOneEmptySideStaysActive() {
    // concat requires both sides to share arity and element type, so both forEach streams are
    // mapped to Object.
    ConstraintProvider provider =
        factory ->
            new Constraint[] {
              factory
                  .forEach(TestdataLavishExtra.class)
                  .map(extra -> (Object) extra)
                  .concat(
                      factory.forEach(TestdataLavishEntity.class).map(entity -> (Object) entity))
                  .penalize(SimpleScore.ONE)
  ...
  void chainOverEmptyClassFullyDeactivates() {
    ...
    // filter() compiles to a ConditionalTupleLifecycle WRAPPER, not a node, so there is no FILTER
    // node.
  ...
  void flattenOverEmptyClassDeactivates() {
    // FlattenLastUniNode inherits the single-input default rule (upstreamCanProduceTuples &&
    // downstream).
    ConstraintProvider provider =
        factory ->
            new Constraint[] {
    ...
    ConstraintProvider provider =
        factory -> {
          ...
          return new Constraint[] {
            base.penalize(SimpleScore.ONE).asConstraint("aliveBranch"),
  ...
  void precomputeOverEmptySourceDeactivates() {
    ConstraintProvider provider =
        factory ->
            new Constraint[] {
              factory
                  .precompute(pf -> pf.forEachUnfiltered(TestdataLavishExtra.class))
                  .penalize(SimpleScore.ONE)
