...
import static greycos.solver.core.impl.heuristic.selector.move.generic.list.kopt.KOptUtils.getMultiEntityBetweenPredicate;
import static greycos.solver.core.impl.heuristic.selector.move.generic.list.kopt.KOptUtils.getMultiEntitySuccessorFunction;
...
class KOptListMoveTest {
  ...
  void testMultiEntity3OptRebase() {
    ...
    var destinationE1 =
        TestdataListEntity.createWithValues(
            "e1", destinationV1, destinationV2, destinationV3, destinationV6);
    ...
    when(inverseVariableSupply.getInverseSingleton(destinationE1.getValueList().get(0)))
        .thenReturn(destinationE1);
    when(inverseVariableSupply.getInverseSingleton(destinationE2.getValueList().get(0)))
        .thenReturn(destinationE2);
    ...
    assertThat(destinationE2.getValueList())
        .containsExactly(destinationV2, destinationV6, destinationV4, destinationV3);
    verify(destinationScoreDirector)
        .beforeListVariableChanged(variableDescriptor, destinationE1, 0, 4);
    verify(destinationScoreDirector)
        .beforeListVariableChanged(variableDescriptor, destinationE2, 0, 2);
    verify(destinationScoreDirector)
        .afterListVariableChanged(variableDescriptor, destinationE1, 0, 2);
    verify(destinationScoreDirector)
        .afterListVariableChanged(variableDescriptor, destinationE2, 0, 4);
  }
  ...
  void test4OptLong() {
    var e1 =
        TestdataListEntity.createWithValues(
            "e1", v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12);
    var solution = new TestdataListSolution();
    solution.setEntityList(List.of(e1));
    solution.setValueList(List.of(v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12));
    scoreDirector.setWorkingSolution(solution);
    ...
    assertThat(e1.getValueList())
        .containsExactly(v1, v3, v4, v5, v10, v11, v12, v9, v8, v7, v6, v2);
  ...
  void testInverted4OptLong() {
    ...
    assertThat(e1.getValueList())
        .containsExactly(v1, v2, v5, v4, v3, v12, v11, v10, v6, v7, v8, v9);
    verify(scoreDirector).beforeListVariableChanged(variableDescriptor, e1, 0, 12);
    verify(scoreDirector).afterListVariableChanged(variableDescriptor, e1, 0, 12);
  }
  ...
  void testDoubleBridge4OptLong() {
    ...
    assertThat(e1.getValueList())
        .containsExactly(v1, v2, v9, v10, v11, v12, v6, v7, v8, v3, v4, v5);
  ...
  void isMoveDoableValueRangeProviderOnEntity() {
    ...
    var entity1 =
        new TestdataListEntityProvidingEntity(
            "e1",
            List.of(value1, value2, value3, value5, value6, value7),
            List.of(value5, value2, value3, value1));
    var entity2 =
        new TestdataListEntityProvidingEntity(
            "e2",
            List.of(value1, value3, value4, value5, value6, value7, value8),
  ...
  /**
   * Create a sequential or non-sequential k-opt from the supplied pairs of undirected removed and
   * added edges.
   *
   * @param <Solution_>
   * @param scoreDirector
   * @param listVariableDescriptor
   * @param removedEdgeList The edges to remove. For each pair {@code (edgePairs[2*i],
   *     edgePairs[2*i+1])}, it must be the case {@code edgePairs[2*i+1]} is either the successor or
   *     predecessor of {@code edgePairs[2*i]}. Additionally, each edge must belong to the given
   *     entity's list variable.
   * @param addedEdgeList The edges to add. Must contain only endpoints specified in the
   *     removedEdgeList.
   * @return A new sequential or non-sequential k-opt move with the specified undirected edges
   *     removed and added.
   */
  private static <Solution_, Value_> KOptListMove<Solution_> fromRemovedAndAddedEdges(
    ...
    return fromRemovedAndAddedEdges(
        scoreDirector,
  ...
  private static <Solution_, Value_> KOptListMove<Solution_> fromRemovedAndAddedEdges(
    ...
    if (!addedEdgeList.containsAll(removedEdgeList)) {
      throw new IllegalArgumentException(
          "addedEdgeList ("
              + addedEdgeList
              + ") is invalid; it contains endpoints "
              + "that are not included in the removedEdgeList ("
              + removedEdgeList
              + ").");
    }
