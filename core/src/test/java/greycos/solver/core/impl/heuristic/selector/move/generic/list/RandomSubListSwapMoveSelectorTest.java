...
class RandomSubListSwapMoveSelectorTest {
  ...
  void sameEntityUnrestricted() {
    ...
    var moveSelector =
        new RandomSubListSwapMoveSelector<>(
    ...
    var moveSelector =
        new RandomSubListSwapMoveSelector<>(
            new RandomSubListSelector<>(
                entitySelector,
                TestdataListUtils.mockNeverEndingIterableValueSelector(
                    listVariableDescriptor, v1, v4),
                minimumSubListSize,
                maximumSubListSize),
            new RandomSubListSelector<>(
                entitySelector,
                TestdataListUtils.mockNeverEndingIterableValueSelector(
                    listVariableDescriptor, v4, v1),
                minimumSubListSize,
                maximumSubListSize),
            false);
  ...
  void allowsUnassignedValues() {
    ...
    // The entity selector must be complete; it affects subList calculation and the move selector
    // size.
    var entitySelector = mockEntitySelector(a, b);
    var moveSelector =
        new RandomSubListSwapMoveSelector<>(
            ...
            new RandomSubListSelector<>(
                entitySelector,
                TestdataListUtils.mockNeverEndingIterableValueSelector(
                    getAllowsUnassignedvaluesListVariableDescriptor(scoreDirector), v1, v1, v1, v3),
                minimumSubListSize,
                maximumSubListSize),
            false);
    ...
    assertCodesOfNeverEndingMoveSelector(
        moveSelector,
        (long) subListCount * subListCount,
        "{A[0+2]} <-> {A[0+2]}",
        "{A[0+2]} <-> {A[0+1]}",
        "{A[0+2]} <-> {A[1+1]}",
        "{A[0+2]} <-> {B[0+1]}",
  ...
  void phaseLifecycle() {
    ...
    var leftValueSelector =
        TestdataListUtils.mockNeverEndingIterableValueSelector(listVariableDescriptor);
    ...
    var moveSelector =
        new RandomSubListSwapMoveSelector<>(
            new RandomSubListSelector<>(
                entitySelector, leftValueSelector, minimumSubListSize, maximumSubListSize),
            new RandomSubListSelector<>(
                entitySelector, rightValueSelector, minimumSubListSize, maximumSubListSize),
            false);
