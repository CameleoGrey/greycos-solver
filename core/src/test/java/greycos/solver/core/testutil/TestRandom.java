 ...
 * greycos.solver.core.impl.solver.scope.SolverScope} and never updated in later phases and steps,
 * we need to be able to reset the same random to start running a new sequence of numbers. That is
 * what {@link #reset(int...)} et al. are for.
 */
public final class TestRandom extends Random implements RandomSource {
  ...
  private BigDecimal getNextValue() {
    ...
    if (returnCount > toReturn.length) {
      throw new IllegalStateException(
          "Requested a random value past the specified collection ("
  ...
  protected int next(int bits) {
    throw new UnsupportedOperationException(
        getClass().getCanonicalName() + " does not support this method.");
  }
  ...
  public void reset(int... toReturn) {
    this.toReturn =
        Arrays.stream(toReturn).mapToObj(BigDecimal::valueOf).toArray(BigDecimal[]::new);
    this.returnCount = 0;
  }
  ...
  /**
   * Check whether the last time that {@link #nextInt(int)} was last called with the given bound
   * value. If not, throws; otherwise resets the last known bound.
   *
   * @param bound
  ...
  public void assertIntBoundJustRequested(int bound) {
    assertThat(lastRequestedIntBound)
        .as(
            "Expected bound (%s) to have just been requested, but was (%s).",
            bound, lastRequestedIntBound)
        .isEqualTo(bound);
