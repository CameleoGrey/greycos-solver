...
import greycos.solver.core.config.solver.testutil.calculator.TestdataCorruptedDifferentValuesCalculator;
import greycos.solver.core.config.solver.testutil.calculator.TestdataDifferentValuesCalculator;
import greycos.solver.core.config.solver.testutil.corruptedundoshadow.CorruptedUndoShadowEasyScoreCalculator;
import greycos.solver.core.config.solver.testutil.corruptedundoshadow.CorruptedUndoShadowEntity;
import greycos.solver.core.config.solver.testutil.corruptedundoshadow.CorruptedUndoShadowSolution;
import greycos.solver.core.config.solver.testutil.corruptedundoshadow.CorruptedUndoShadowValue;
...
class EnvironmentModeTest {
  ...
  static void setUpInputProblem() {
    ...
    inputProblem.setValueList(
        Arrays.asList(new TestdataValue("v1"), new TestdataValue("v2"), new TestdataValue("v3")));
  ...
  private static SolverConfig buildSolverConfig(EnvironmentMode environmentMode) {
    var initializerPhaseConfig =
        new CustomPhaseConfig()
            .withCustomPhaseCommandClassList(
                Collections.singletonList(TestdataFirstValueInitializer.class));
    ...
    switch (environmentMode) {
      ...
      case STEP_ASSERT ->
          assertIllegalStateExceptionWhileSolving(
              solverConfig, "Score corruption analysis could not be generated ");
      case PHASE_ASSERT ->
          assertIllegalStateExceptionWhileSolving(solverConfig, "Solver corruption was detected.");
      case NO_ASSERT, NON_REPRODUCIBLE ->
          assertThatNoException()
  ...
  private void assertIllegalStateExceptionWhileSolving(
      SolverConfig solverConfig, String... exceptionMessage) {
    assertThatExceptionOfType(IllegalStateException.class)
        .isThrownBy(() -> PlannerTestUtils.solve(solverConfig, inputProblem))
        .withMessageContainingAll(exceptionMessage);
  }

  private void assertSameScoreSeries(
      Solver<TestdataSolution> solver1, Solver<TestdataSolution> solver2) {
    ...
    assertSoftly(
        softly ->
            IntStream.range(0, NUMBER_OF_TIMES_RUN)
                .forEach(
                    i -> {
                      solver1.solve(inputProblem);
                      solver2.solve(inputProblem);
                      softly
                          .assertThat(listener.getScores())
                          .as(
                              "Score steps should be the same "
                                  + "in a reproducible environment mode.")
                          .isEqualTo(listener2.getScores());
                    }));
  }

  private void assertDifferentScoreSeries(
      Solver<TestdataSolution> solver1, Solver<TestdataSolution> solver2) {
    var listener = new TestdataStepScoreListener();
    var listener2 = new TestdataStepScoreListener();
                ...
                .forEach(
                    i -> {
                      ...
                      softly
                          .assertThat(listener.getScores())
                          .as(
                              "Score steps should not be the same in a non-reproducible environment mode. "
                ...
                .forEach(
                    i ->
                        softly
                            ...
                            .as(
                                "Random factories should generate the same results "
                                    + "in a reproducible environment mode.")
  ...
  private void setSolverConfigCalculatorClass(
      SolverConfig solverConfig, Class<? extends EasyScoreCalculator> easyScoreCalculatorClass) {
    solverConfig.setScoreDirectorFactoryConfig(
        new ScoreDirectorFactoryConfig().withEasyScoreCalculatorClass(easyScoreCalculatorClass));
  }
  ...
  public static class TestdataStepScoreListener
