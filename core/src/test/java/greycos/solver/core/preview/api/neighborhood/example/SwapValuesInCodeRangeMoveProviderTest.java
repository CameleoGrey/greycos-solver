...
class SwapValuesInCodeRangeMoveProviderTest {
  ...
  void producesASwapWithinTheOnlyRange() {
    ...
    var variableMetaModel =
        solutionMetaModel.genuineEntity(TestdataEntity.class).<TestdataValue>basicVariable();
    ...
    var context =
        NeighborhoodTester.build(
                new SwapValuesInCodeRangeMoveProvider(variableMetaModel), solutionMetaModel)
            .using(solution);
    ...
    // and compose() of a single move returns that move directly rather than wrapping it in a
    // CompositeMove.
    ...
    context.producesNoneOf(
        Moves.change(variableMetaModel, e0, v0), Moves.change(variableMetaModel, e1, v1));
  }
}
