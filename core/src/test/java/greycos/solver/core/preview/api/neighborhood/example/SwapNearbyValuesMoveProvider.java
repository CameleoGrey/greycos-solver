...
/**
 * Swaps two assigned values whose list positions are at most one index apart, in different
 * entities, as a single built-in {@link Moves#swap(PlanningListVariableMetaModel, PositionInList,
 * PositionInList) swap move} - no custom move, no composition.
 */
@NullMarked
final class SwapNearbyValuesMoveProvider
  ...
  public MoveStream<TestdataListEntityProvidingSolution> build(
    ...
    var nearbyValuePairs =
        factory
            .forEachAssignedValue(variableMetaModel)
            .asCachedDataset()
            .join(
                factory.forEachAssignedValue(variableMetaModel),
        ...
        (session, random) -> {
          ...
          return new Iterator<>() {
            ...
            public Move<TestdataListEntityProvidingSolution> next() {
              ...
              var positionA =
                  solutionView.getPositionOf(variableMetaModel, pairIterator.a()).ensureAssigned();
              var positionB =
                  solutionView.getPositionOf(variableMetaModel, pairIterator.b()).ensureAssigned();
  ...
  private boolean isNearbySwap(
      SolutionView<TestdataListEntityProvidingSolution> solutionView,
      TestdataListEntityProvidingValue valueA,
      TestdataListEntityProvidingValue valueB) {
