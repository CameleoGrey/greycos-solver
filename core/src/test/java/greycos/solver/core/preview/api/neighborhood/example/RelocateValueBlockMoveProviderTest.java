  ...
  void producesExactRelocationInRangeAndNoneOutOfRange() {
    ...
    var entityA =
        new TestdataListEntityProvidingEntity(
            "A", List.of(v0, v1, v2), new ArrayList<>(List.of(v0, v1, v2)));
    ...
    var context =
        NeighborhoodTester.build(
                new RelocateValueBlockMoveProvider(variableMetaModel), solutionMetaModel)
            .using(solution);
    ...
    context.producesNoneOf(
        new RelocateValueBlockMove(variableMetaModel, entityA, 1, 2, entityB, 0));
  }
}
