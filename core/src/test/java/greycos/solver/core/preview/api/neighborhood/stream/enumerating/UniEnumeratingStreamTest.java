...
import greycos.solver.core.preview.api.neighborhood.stream.enumerating.collector.NeighborhoodsCollectors;
import greycos.solver.core.preview.api.neighborhood.stream.enumerating.collector.UniNeighborhoodsCollector;
import greycos.solver.core.preview.api.neighborhood.stream.enumerating.collector.UniNeighborhoodsCollectorAccumulator;
...
class UniEnumeratingStreamTest {
  ...
  private static <Solution_> DatasetSession<Solution_> createSession(
      EnumeratingStreamFactory<Solution_> enumeratingStreamFactory, Solution_ solution) {
    var scoreDirector =
  ...
  void forEach_listVar() {
    var enumeratingStreamFactory =
        new EnumeratingStreamFactory<>(
            TestdataListSolution.buildSolutionDescriptor(), EnvironmentMode.PHASE_ASSERT);
    var uniDataset =
  ...
  @Test
  void forEachExcludingPinnedIncludingNull_listVar() { // Entities with planningPin true will be
    // skipped.
    var enumeratingStreamFactory =
        new EnumeratingStreamFactory<>(
            TestdataPinnedWithIndexListSolution.buildSolutionDescriptor(),
  ...
  void forEachExcludingPinned_listVarValues() {
    ...
    var enumeratingStreamFactory =
        new EnumeratingStreamFactory<>(solutionDescriptor, EnvironmentMode.PHASE_ASSERT);
    var uniDataset =
        ((AbstractUniEnumeratingStream<
                    TestdataPinnedWithIndexListSolution, TestdataPinnedWithIndexListValue>)
                enumeratingStreamFactory.forEachExcludingPinned(
  ...
  void groupBy_0Mapping1Collector() {
    ...
    assertThat(instance.iterator()).toIterable().map(UniTuple::getA).hasSize(1);
  ...
  void groupBy_1Mapping0Collector_solutionView() {
    ...
    UniNeighborhoodsMapper<TestdataSolution, TestdataEntity, TestdataValue> mapper =
        (view, entity) -> {
  ...
  private static final class NeighborhoodsUniCountCollector
    ...
    @Override
    public UniNeighborhoodsCollectorAccumulator<TestdataSolution, TestdataEntity, int[]>
        accumulator() {
      return (view, container) ->
          new UniNeighborhoodsCollectorValueHandle<TestdataEntity>() {
            @Override
