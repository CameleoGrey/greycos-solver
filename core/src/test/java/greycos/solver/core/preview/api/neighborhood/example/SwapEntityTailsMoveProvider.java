 ...
 * swaps every value from that index to the end of the list between the two entities, as a single
 * {@link SwapEntityTailsMove}.
 */
@NullMarked
final class SwapEntityTailsMoveProvider implements MoveProvider<TestdataListSolution> {
  ...
  public MoveStream<TestdataListSolution> build(MoveStreamFactory<TestdataListSolution> factory) {
    ...
    var entityPairs =
        factory
            .forEach(TestdataListEntity.class, false)
            .asCachedDataset()
            .join(
                factory.forEach(TestdataListEntity.class, false),
        ...
        (session, random) -> {
          ...
          return new Iterator<>() {
            ...
            private @Nullable Move<TestdataListSolution> draw() {
              ...
              var overlap =
                  Math.min(
                      solutionView.countValues(variableMetaModel, entityA),
              ...
              if (!destinationIterator.hasNext()) {
                return null; // No destination for entityA (should not happen once filtered
                // non-empty).
              }
  ...
  private boolean bothHaveValues(
      SolutionView<TestdataListSolution> solutionView,
      TestdataListEntity entityA,
