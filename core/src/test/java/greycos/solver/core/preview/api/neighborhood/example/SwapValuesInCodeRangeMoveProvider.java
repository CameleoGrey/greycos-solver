...
/**
 * For a randomly picked pair of distinct values, and a randomly picked range of entity codes,
 * reassigns every entity in that range currently holding one of the two values to the other, and
 * composes the result into a single move.
 */
@NullMarked
final class SwapValuesInCodeRangeMoveProvider implements MoveProvider<TestdataSolution> {
  ...
  SwapValuesInCodeRangeMoveProvider(
      PlanningVariableMetaModel<TestdataSolution, TestdataEntity, TestdataValue>
          variableMetaModel) {
  ...
  public MoveStream<TestdataSolution> build(MoveStreamFactory<TestdataSolution> factory) {
    ...
    var valueEntities =
        factory
            .forEach(TestdataValue.class, false)
            .join(
                factory.forEach(TestdataEntity.class, false),
    ...
    var valuePairs =
        factory
            ...
            .join(
                ...
                NeighborhoodsJoiners.lessThan(TestdataValue::getCode));
    ...
    var codeRanges =
        entityCodes
            .asCachedDataset()
            .join(entityCodes, NeighborhoodsJoiners.lessThan(code -> code));
    ...
    return factory.buildMoveStream(
        (session, random) -> {
          ...
          if (pairInstance.size() == 0 || rangeInstance.size() == 0) {
            return Collections
                .emptyIterator(); // Fewer than 2 values, or fewer than 2 distinct entity codes.
          }
  ...
  private void collectSwaps(
      BiDatasetInstance<TestdataValue, TestdataEntity> valueEntityInstance,
      TestdataValue from,
      TestdataValue to,
      String start,
      String end,
