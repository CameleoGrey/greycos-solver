...
/**
 * For a randomly picked ordered pair of distinct entities and a randomly picked run of consecutive
 * values in one entity's list, relocates that run into the other entity's list at a randomly picked
 * destination, as a single {@link RelocateValueBlockMove}.
 */
@NullMarked
final class RelocateValueBlockMoveProvider
    implements MoveProvider<TestdataListEntityProvidingSolution> {
  ...
  public MoveStream<TestdataListEntityProvidingSolution> build(
    ...
    var entityPairs =
        factory
            .forEach(TestdataListEntityProvidingEntity.class, false)
            .asCachedDataset()
            .join(
                factory.forEach(TestdataListEntityProvidingEntity.class, false),
    ...
    var entityDestinations =
        factory
            .forEach(TestdataListEntityProvidingEntity.class, false)
            .join(
                factory.forEachDestination(variableMetaModel),
        ...
        (session, random) -> {
          ...
          return new Iterator<>() {
            ...
            private @Nullable Move<TestdataListEntityProvidingSolution> draw() {
              ...
              // The drawn destination's entity picks the direction: the pair's other entity is the
              // source.
  ...
  private boolean isBlockInRange(
      SolutionView<TestdataListEntityProvidingSolution> solutionView,
