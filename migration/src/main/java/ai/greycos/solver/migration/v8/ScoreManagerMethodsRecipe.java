...
public class ScoreManagerMethodsRecipe extends AbstractRecipe {
    ...
    return Preconditions.check(
        ...
        new JavaIsoVisitor<>() {
          ...
          public Expression visitExpression(Expression expression, ExecutionContext ctx) {
            ...
            MatcherMeta matcherMeta =
                Arrays.stream(MATCHER_METAS)
                    .filter(m -> m.methodMatcher.matches(e))
                    .findFirst()
                    .orElse(null);
            if (matcherMeta == null) {
              return e;
            ...
            String pattern =
                "#{any("
                    + matcherMeta.classFqn
                    + ")}."
                    + (matcherMeta.methodName.contains("Summary")
                        ? "explain(#{any()}, SolutionUpdatePolicy.UPDATE_SCORE_ONLY).getSummary()"
                        : matcherMeta
                            .methodName
                            .replace(")", ", SolutionUpdatePolicy.UPDATE_SCORE_ONLY)")
                            .replace("..", "#{any()}")
                            .replace("Score(", "("));
            maybeAddImport("ai.greycos.solver.core.api.solver.SolutionUpdatePolicy");
            return JavaTemplate.builder(pattern)
                .javaParser(JAVA_PARSER)
                .imports("ai.greycos.solver.core.api.solver.SolutionUpdatePolicy")
                .build()
  ...
  private static final class MatcherMeta {
    ...
    public MatcherMeta(String method) {
      this.classFqn = "ai.greycos.solver.core.api.score.ScoreManager";
