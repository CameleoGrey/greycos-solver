package ai.greycos.solver.migration.fork;
...
public final class GreyCOSChangeDependencies extends AbstractRecipe {
  ...
  public GreyCOSChangeDependencies() {
    ...
    for (String artifactSuffix : ARTIFACT_SUFFIXES) {
      ...
      String newArtifactId = "greycos-solver-" + newArtifactSuffix;

      // Maven
      recipeList.add(
          new ChangeManagedDependencyGroupIdAndArtifactId(
              oldGroupId, oldArtifactId, newGroupId, newArtifactId, null));
      recipeList.add(
          new ChangeDependencyGroupIdAndArtifactId(
              oldGroupId, oldArtifactId, newGroupId, newArtifactId, null, null));
      // Gradle
      recipeList.add(
          new ChangeDependencyArtifactId(oldGroupId, oldArtifactId, newArtifactId, null));
    }
    // TODO Do not use "*" approach. This is a workaround for
    // https://github.com/openrewrite/rewrite/issues/2994
