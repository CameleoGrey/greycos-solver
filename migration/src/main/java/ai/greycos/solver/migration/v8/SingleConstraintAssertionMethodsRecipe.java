...
public final class SingleConstraintAssertionMethodsRecipe extends AbstractRecipe {
  ...
  public List<Recipe> getRecipeList() {
    var methodsToReorder =
        List.of(
            new String[] {
              "ai.greycos.solver.test.api.score.stream.SingleConstraintAssertion *(int, String)",
              "message,matchWeightTotal",
              "matchWeightTotal,message"
            },
            new String[] {
              "ai.greycos.solver.test.api.score.stream.SingleConstraintAssertion *(long, String)",
              "message,matchWeightTotal",
              "matchWeightTotal,message"
            },
            new String[] {
              "ai.greycos.solver.test.api.score.stream.SingleConstraintAssertion *(java.math.BigDecimal, String)",
              "message,matchWeightTotal",
              "matchWeightTotal,message"
            });
    var result = new ArrayList<Recipe>();
    methodsToReorder.forEach(
        method ->
            result.add(
                new ReorderMethodArguments(
                    method[0], method[1].split(","), method[2].split(","), null, null)));
    return result;
  }
}
