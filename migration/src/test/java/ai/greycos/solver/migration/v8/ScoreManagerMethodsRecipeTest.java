...
class ScoreManagerMethodsRecipeTest implements RewriteTest {
  ...
  public void defaults(RecipeSpec spec) {
    spec.recipe(new ScoreManagerMethodsRecipe()).parser(AbstractRecipe.JAVA_PARSER);
  }

  @Test
  void summary() {
    runTest(
        "String summary = scoreManager.getSummary(solution);",
  ...
  void explain() {
    runTest(
        "ScoreExplanation explanation = scoreManager.explainScore(solution);",
  ...
  void update() {
    runTest(
        "Object score = scoreManager.updateScore(solution);",
  ...
  private void runTest(String before, String after) {
    rewriteRun(java(wrap(before, false), wrap(after, true)));
  }

  private static String wrap(String content, boolean addImport) {
    return "import ai.greycos.solver.core.api.score.ScoreManager;\n"
        + (addImport ? "import ai.greycos.solver.core.api.solver.SolutionUpdatePolicy;\n" : "")
