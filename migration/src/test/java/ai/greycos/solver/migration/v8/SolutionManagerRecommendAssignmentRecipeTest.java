...
class SolutionManagerRecommendAssignmentRecipeTest implements RewriteTest {
  ...
  public void defaults(RecipeSpec spec) {
    spec.recipe(new SolutionManagerRecommendAssignmentRecipe()).parser(AbstractRecipe.JAVA_PARSER);
  }

  @Test
  void base() {
    runTest(
        "List<RecommendedFit> recommendations = solutionManager.recommendFit(null, null, null, null);",
  ...
  void overload() {
    runTest(
        "List<RecommendedFit> recommendations = solutionManager.recommendFit(null, null, null);",
  ...
  private void runTest(String before, String after) {
    rewriteRun(java(wrap(before, true), wrap(after, false)));
  }

  private static String wrap(String content, boolean addImport) {
    return "import ai.greycos.solver.core.api.solver.SolutionManager;\n"
        + "import ai.greycos.solver.core.api.solver.SolverFactory;\n"
        + (addImport ? "import ai.greycos.solver.core.api.solver.RecommendedFit;\n" : "")
