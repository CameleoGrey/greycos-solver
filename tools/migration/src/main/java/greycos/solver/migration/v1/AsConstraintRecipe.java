    ...
    return Preconditions.check(
        ...
        new JavaIsoVisitor<>() {
          ...
          private final Pattern uniConstraintStreamPattern =
              Pattern.compile("greycos.solver.core.api.score.stream.uni.UniConstraintStream");
          private final Pattern biConstraintStreamPattern =
              Pattern.compile("greycos.solver.core.api.score.stream.bi.BiConstraintStream");
          private final Pattern triConstraintStreamPattern =
              Pattern.compile("greycos.solver.core.api.score.stream.tri.TriConstraintStream");
          private final Pattern quadConstraintStreamPattern =
              Pattern.compile("greycos.solver.core.api.score.stream.quad.QuadConstraintStream");
          ...
          public J.MethodInvocation visitMethodInvocation(
            ...
            if (selectType.isAssignableFrom(uniConstraintStreamPattern)) {
              templateCode =
                  "#{any(greycos.solver.core.api.score.stream.uni.UniConstraintStream)}\n";
            } else if (selectType.isAssignableFrom(biConstraintStreamPattern)) {
              templateCode = "#{any(greycos.solver.core.api.score.stream.bi.BiConstraintStream)}\n";
            } else if (selectType.isAssignableFrom(triConstraintStreamPattern)) {
              templateCode =
                  "#{any(greycos.solver.core.api.score.stream.tri.TriConstraintStream)}\n";
            } else if (selectType.isAssignableFrom(quadConstraintStreamPattern)) {
              templateCode =
                  "#{any(greycos.solver.core.api.score.stream.quad.QuadConstraintStream)}\n";
            } else {
              return method;
            }
              ...
              if (!matcherMeta.matchWeigherIncluded) {
                templateCode +=
                    "."
                        ...
                        + "(#{any(greycos.solver.core.api.score.Score)})\n";
              } else {
                templateCode +=
                    "."
                        + matcherMeta.methodName
  ...
  private static class MatcherMeta {
    ...
    public MatcherMeta(String select, String method) {
      ...
      if ("ConstraintStream".equals(select)) {
        signature = "greycos.solver.core.api.score.stream.ConstraintStream";
      } else if ("UniConstraintStream".equals(select)) {
        signature = "greycos.solver.core.api.score.stream.uni.UniConstraintStream";
      } else if ("BiConstraintStream".equals(select)) {
        signature = "greycos.solver.core.api.score.stream.bi.BiConstraintStream";
      } else if ("TriConstraintStream".equals(select)) {
        signature = "greycos.solver.core.api.score.stream.tri.TriConstraintStream";
      } else if ("QuadConstraintStream".equals(select)) {
        signature = "greycos.solver.core.api.score.stream.quad.QuadConstraintStream";
      } else {
      ...
      signature +=
          " "
              + method
                  .replace(" Score", " greycos.solver.core.api.score.Score")
                  ...
                  .replace(
                      " ToIntTriFunction", " greycos.solver.core.api.function.ToIntTriFunction")
                  .replace(
                      " ToLongTriFunction", " greycos.solver.core.api.function.ToLongTriFunction")
                  .replace(" TriFunction", " greycos.solver.core.api.function.TriFunction")
                  .replace(
                      " ToIntQuadFunction", " greycos.solver.core.api.function.ToIntQuadFunction")
                  .replace(
                      " ToLongQuadFunction", " greycos.solver.core.api.function.ToLongQuadFunction")
                  .replace(" QuadFunction", " greycos.solver.core.api.function.QuadFunction");
