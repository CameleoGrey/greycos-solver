package greycos.solver.benchmark.aggregator;
...
public class BenchmarkAggregator {
  ...
  public File aggregateBenchmarks(PlannerBenchmarkConfig benchmarkConfig) {
    ...
    if (!benchmarkDirectory.exists() || !benchmarkDirectory.isDirectory()) {
      throw new IllegalArgumentException(
          "Benchmark directory does not exist: %s".formatted(benchmarkDirectory));
    }
    ...
    if (plannerBenchmarkResults.isEmpty()) {
      throw new IllegalArgumentException(
          "No benchmark results found in directory: %s".formatted(benchmarkDirectory));
    }
  ...
  public File aggregateSelectedBenchmarksInUi(
      List<SingleBenchmarkResult> selectedSingleBenchmarkResults,
  ...
  public File aggregateSelectedBenchmarks(
      PlannerBenchmarkConfig benchmarkConfig, List<String> selectedDirectoryNames) {
    File benchmarkDirectory = benchmarkConfig.getBenchmarkDirectory();
    if (!benchmarkDirectory.exists() || !benchmarkDirectory.isDirectory()) {
      throw new IllegalArgumentException(
          "No benchmark results found in directory: %s".formatted(benchmarkDirectory));
    ...
    BenchmarkResultIO benchmarkResultIO = new BenchmarkResultIO();
    List<PlannerBenchmarkResult> allPlannerResults =
        benchmarkResultIO.readPlannerBenchmarkResultList(benchmarkDirectory);
    ...
    if (selectedSingleBenchmarkResults.isEmpty()) {
      throw new IllegalArgumentException(
          "No valid benchmark results found in the selected directories: %s"
    ...
    if (desktop != null && desktop.isSupported(Action.BROWSE)) {
      try {
        ...
        throw new IllegalStateException(
            "Failed showing htmlOverviewFile (%s) in the default browser."
                .formatted(htmlOverviewFile),
            e);
      }
    } else {
      LOGGER.warn(
  ...
  private File aggregate(
    ...
    if (!benchmarkDirectory.exists()) {
      throw new IllegalArgumentException(
          "The benchmarkDirectory (" + benchmarkDirectory + ") must exist.");
    }
    if (benchmarkReportConfig == null) {
      throw new IllegalArgumentException(
          "The benchmarkDirectory (%s) must exist.".formatted(benchmarkDirectory));
    }
    if (singleBenchmarkResultList.isEmpty()) {
      throw new IllegalArgumentException(
          "The singleBenchmarkResultList (%s) must not be empty."
              .formatted(singleBenchmarkResultList));
    }
    ...
    BenchmarkReportFactory benchmarkReportFactory =
    ...
    LOGGER.info(
        "Aggregation ended: statistic html overview ({}).",
