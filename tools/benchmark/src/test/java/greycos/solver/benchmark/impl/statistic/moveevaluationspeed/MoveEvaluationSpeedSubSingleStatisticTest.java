...
final class MoveEvaluationSpeedSubSingleStatisticTest
    extends AbstractSubSingleStatisticTest<
        LongStatisticPoint, MoveEvaluationSpeedSubSingleStatistic<TestdataSolution>> {

  @Override
  protected Function<
          SubSingleBenchmarkResult, MoveEvaluationSpeedSubSingleStatistic<TestdataSolution>>
  ...
  void generateCharts() {
    ...
    doReturn(List.of(singleBenchmarkResult))
        .when(problemBenchmarkResult)
    ...
    doReturn(singleStatistic)
        .when(singleBenchmarkResult)
        .getSubSingleStatistic(any(ProblemStatisticType.class));
    doReturn(
            List.of(
                new LongStatisticPoint(Long.MIN_VALUE, Long.MIN_VALUE),
                new LongStatisticPoint(Long.MAX_VALUE, Long.MAX_VALUE)))
        .when(singleStatistic)
        .getPointList();
    var statistic = new MoveEvaluationSpeedProblemStatisticTime(problemBenchmarkResult);
    ...
    assertThat(lineChart.title()).isEqualTo("Problem_0 move evaluation speed statistic");
    assertThat(lineChart.xLabel()).isEqualTo("Time spent");
    assertThat(lineChart.yLabel()).isEqualTo("Move evaluation speed per second");
  }
}
