...
import greycos.solver.benchmark.impl.report.LineChart.Builder;
...
class LineChartTest {
  ...
  void stepSize(BigDecimal difference, BigDecimal expectedStepSize) {
    BigDecimal actualStepSize =
        LineChart.stepSize(BigDecimal.ZERO, difference).stripTrailingZeros();
    assertThat(actualStepSize).isEqualTo(expectedStepSize.stripTrailingZeros());
  }
  ...
  void testDownSamplingProcess() throws IOException {
    ...
    try (BufferedReader in =
        new BufferedReader(
            new FileReader(
                LineChartTest.class
                    .getResource("/greycos/solver/benchmark/impl/result/xValues.csv")
                    .getFile()))) {
      String str;
    ...
    try (BufferedReader in =
        new BufferedReader(
            new FileReader(
                LineChartTest.class
                    .getResource("/greycos/solver/benchmark/impl/result/yValues.csv")
                    .getFile()))) {
      String str;
      while ((str = in.readLine()) != null) {
        String[] values = str.split(",");
    ...
    /**
     * The result was created by the implementation available at:
    ...
    try (BufferedReader in =
        new BufferedReader(
            new FileReader(
                LineChartTest.class
                    .getResource("/greycos/solver/benchmark/impl/result/resultValues.csv")
                    .getFile()))) {
      String str;
      while ((str = in.readLine()) != null) {
        String[] values = str.split(",");
        for (String value : values) {
          ...
          expectedResult.add(
              new Pair<>(Double.parseDouble(expected[0]), Double.parseDouble(expected[1])));
        }
      }
    }
    ...
    LineChart<Double, Double> chart =
        builder.build("test.txt", "test", "x", "y", false, false, false);
