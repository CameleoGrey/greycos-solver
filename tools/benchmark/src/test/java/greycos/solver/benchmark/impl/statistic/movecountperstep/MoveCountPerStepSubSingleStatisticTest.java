...
public final class MoveCountPerStepSubSingleStatisticTest
    extends AbstractSubSingleStatisticTest<
        MoveCountPerStepStatisticPoint, MoveCountPerStepSubSingleStatistic<TestdataSolution>> {
  ...
  protected List<MoveCountPerStepStatisticPoint> getInputPoints() {
    return Collections.singletonList(
        new MoveCountPerStepStatisticPoint(Long.MAX_VALUE, 0, Long.MAX_VALUE));
  }

  @Override
  protected void runTest(
      SoftAssertions assertions, List<MoveCountPerStepStatisticPoint> outputPoints) {
    assertions
        .assertThat(outputPoints)
        .hasSize(1)
        .first()
        .matches(s -> s.getAcceptedMoveCount() == 0, "Accepted move counts do not match.")
        .matches(
            s -> s.getSelectedMoveCount() == Long.MAX_VALUE, "Selected move counts do not match.")
