...
public abstract class StatisticPoint {
  ...
  public static String buildCsvLineWithLongs(long timeMillisSpent, long... values) {
    Object[] array =
        Stream.concat(Stream.of(timeMillisSpent), Arrays.stream(values).boxed())
            .toArray(Object[]::new);
  ...
  public static String buildCsvLineWithStrings(long timeMillisSpent, String... values) {
    Object[] array =
        Stream.concat(Stream.of(timeMillisSpent), Arrays.stream(values)).toArray(Object[]::new);
    return buildCsvLine(array);
  }
      ...
      while (token.startsWith("\"") && !token.endsWith("\"")) {
        ...
        if (i >= tokens.length) {
          throw new IllegalArgumentException(
              "The CSV line (" + line + ") is not a valid CSV line.");
        }
        token += "," + tokens[i];
      }
