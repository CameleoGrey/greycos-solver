package ai.greycos.solver.benchmark.impl.statistic.subsingle.constraintmatchtotalbestscore;
...
import ai.greycos.solver.benchmark.impl.report.LineChart;
...
public class ConstraintMatchTotalBestScoreSubSingleStatistic<Solution_>
    extends PureSubSingleStatistic<
        Solution_, ConstraintMatchTotalBestScoreStatisticPoint, LineChart<Long, Double>> {

  private ConstraintMatchTotalBestScoreSubSingleStatistic() {
    // For JAXB.
  }

  public ConstraintMatchTotalBestScoreSubSingleStatistic(
  ...
  public void open(StatisticRegistry<Solution_> registry, Tags runTag) {
    registry.addListener(
        ...
        timeMillisSpent ->
            registry.extractConstraintSummariesFromMeters(
                SolverMetric.CONSTRAINT_MATCH_TOTAL_BEST_SCORE,
                runTag,
                constraintSummary ->
                    pointList.add(
                        new ConstraintMatchTotalBestScoreStatisticPoint(
                            timeMillisSpent,
  ...
  protected String getCsvHeader() {
    return ConstraintMatchTotalBestScoreStatisticPoint.buildCsvLine(
        "timeMillisSpent", "constraintName", "constraintMatchCount", "scoreTotal");
  }

  @Override
  protected ConstraintMatchTotalBestScoreStatisticPoint createPointFromCsvLine(
      ScoreDefinition<?> scoreDefinition, List<String> csvLine) {
    return new ConstraintMatchTotalBestScoreStatisticPoint(
  ...
  protected List<LineChart<Long, Double>> generateCharts(BenchmarkReport benchmarkReport) {
    List<LineChart.Builder<Long, Double>> builderList =
    ...
    for (int scoreLevelIndex = 0; scoreLevelIndex < builderList.size(); scoreLevelIndex++) {
      ...
      LineChart<Long, Double> chart =
          builder.build(
              "constraintMatchTotalBestScoreSubSingleStatisticChart" + scoreLevelIndex,
              subSingleBenchmarkResult.getName()
                  + " constraint match total best "
                  + scoreLevelLabel
                  + " diff statistic",
              "Time spent",
              "Constraint match total " + scoreLevelLabel,
              true,
              true,
              false);
      chartList.add(chart);
