...
/**
 * This {@link SolverRankingWeightFactory} orders a {@link SolverBenchmarkResult} by how many times
 * each of its {@link SingleBenchmarkResult}s beat {@link SingleBenchmarkResult}s of the other
 * {@link SolverBenchmarkResult}. It maximizes the overall ranking.
 *
 * <p>When the inputSolutions differ greatly in size or difficulty, this often produces a difference
 * in {@link Score} magnitude between each {@link PlanningSolution}. For example: score 10 for
 * dataset A versus 1000 for dataset B. In such cases, this ranking is more fair than {@link
...
public class TotalRankSolverRankingWeightFactory implements SolverRankingWeightFactory {
  ...
  @Override
  public Comparable createRankingWeight(
    ...
    for (SingleBenchmarkResult single : singleBenchmarkResultList) {
      List<SingleBenchmarkResult> otherSingleList =
          single.getProblemBenchmarkResult().getSingleBenchmarkResultList();
    ...
    return new TotalRankSolverRankingWeight(
        solverBenchmarkResult, betterCount, equalCount, lowerCount);
  }

  public static class TotalRankSolverRankingWeight
      implements Comparable<TotalRankSolverRankingWeight> {

    private final Comparator<TotalRankSolverRankingWeight>
        totalRankSolverRankingWeightFactoryComparator =
            Comparator.comparingInt(TotalRankSolverRankingWeight::getBetterCount)
                ...
                .thenComparing(
                    TotalRankSolverRankingWeight::getSolverBenchmarkResult,
    ...
    public TotalRankSolverRankingWeight(
        SolverBenchmarkResult solverBenchmarkResult,
        int betterCount,
        int equalCount,
        int lowerCount) {
