...
public class StepScoreProblemStatistic extends ProblemStatistic<LineChart<Long, Double>> {
  ...
  protected List<LineChart<Long, Double>> generateCharts(BenchmarkReport benchmarkReport) {
    var builderList =
        new ArrayList<LineChart.Builder<Long, Double>>(BenchmarkReport.CHARTED_SCORE_LEVEL_SIZE);
    ...
    var chartList =
        new ArrayList<LineChart<Long, Double>>(BenchmarkReport.CHARTED_SCORE_LEVEL_SIZE);
    for (var scoreLevelIndex = 0; scoreLevelIndex < builderList.size(); scoreLevelIndex++) {
      ...
      var chart =
          builderList
              .get(scoreLevelIndex)
              .build(
                  "stepScoreProblemStatisticChart" + scoreLevelIndex,
                  problemBenchmarkResult.getName() + " step " + scoreLevelLabel + " statistic",
                  "Time spent",
                  "Step " + scoreLevelLabel,
                  true,
                  true,
                  false);
      chartList.add(chart);
