...
import static ai.greycos.solver.core.impl.util.MathUtils.getSpeed;
...
import ai.greycos.solver.benchmark.impl.statistic.StatisticType;
import ai.greycos.solver.benchmark.impl.statistic.SubSingleStatistic;
import ai.greycos.solver.benchmark.impl.statistic.subsingle.constraintmatchtotalbestscore.ConstraintMatchTotalBestScoreSubSingleStatistic;
import ai.greycos.solver.benchmark.impl.statistic.subsingle.constraintmatchtotalstepscore.ConstraintMatchTotalStepScoreSubSingleStatistic;
import ai.greycos.solver.benchmark.impl.statistic.subsingle.pickedmovetypebestscore.PickedMoveTypeBestScoreDiffSubSingleStatistic;
import ai.greycos.solver.benchmark.impl.statistic.subsingle.pickedmovetypestepscore.PickedMoveTypeStepScoreDiffSubSingleStatistic;
...
/**
 * Represents 1 benchmark run for 1 Single Benchmark configuration for 1 {@link Solver}
...
public class SubSingleBenchmarkResult implements BenchmarkResult {
  ...
  @XmlElements({
    @XmlElement(
        name = "constraintMatchTotalBestScoreSubSingleStatistic",
        type = ConstraintMatchTotalBestScoreSubSingleStatistic.class),
    @XmlElement(
        name = "constraintMatchTotalStepScoreSubSingleStatistic",
        type = ConstraintMatchTotalStepScoreSubSingleStatistic.class),
    @XmlElement(
        name = "pickedMoveTypeBestScoreDiffSubSingleStatistic",
        type = PickedMoveTypeBestScoreDiffSubSingleStatistic.class),
    @XmlElement(
        name = "pickedMoveTypeStepScoreDiffSubSingleStatistic",
  ...
  public SubSingleBenchmarkResult(
  ...
  public void setPureSubSingleStatisticList(
  ...
  public void initSubSingleStatisticMap() {
    ...
    effectiveSubSingleStatisticMap =
        new HashMap<>(problemStatisticList.size() + pureSubSingleStatisticList.size());
    ...
    for (var pureSubSingleStatistic : pureSubSingleStatisticList) {
      effectiveSubSingleStatisticMap.put(
          pureSubSingleStatistic.getStatisticType(), pureSubSingleStatistic);
    }
  }
  ...
  protected static SubSingleBenchmarkResult createMerge(
    ...
    newResult.pureSubSingleStatisticList =
        new ArrayList<>(oldResult.pureSubSingleStatisticList.size());
    ...
    for (var newSubSingleStatistic : newResult.effectiveSubSingleStatisticMap.values()) {
      var oldSubSingleStatistic =
          oldResult.getSubSingleStatistic(newSubSingleStatistic.getStatisticType());
      if (!oldSubSingleStatistic.getCsvFile().exists()) {
        if (oldResult.hasAnyFailure()) {
          newSubSingleStatistic.initPointList();
          LOGGER.debug(
              "Old result ({}) is a failure, skipping merge of its sub single statistic ({}).",
              oldResult,
              oldSubSingleStatistic);
          continue;
        } else {
          throw new IllegalStateException(
              "Could not find old result's ("
                  + oldResult
                  + ") sub single statistic's ("
                  + oldSubSingleStatistic
                  + ") CSV file.");
        }
      }
