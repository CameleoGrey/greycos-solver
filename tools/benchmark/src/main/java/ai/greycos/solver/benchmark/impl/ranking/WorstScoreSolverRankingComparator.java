...
/**
 * This ranking {@link Comparator} orders a {@link SolverBenchmarkResult} by its worst {@link
 * Score}. It minimizes the worst case scenario.
 */
public class WorstScoreSolverRankingComparator implements Comparator<SolverBenchmarkResult> {
  ...
  public int compare(SolverBenchmarkResult a, SolverBenchmarkResult b) {
    ...
    for (int i = 0; i < aSize && i < bSize; i++) {
      int comparison =
          singleBenchmarkComparator.compare(
              aSingleBenchmarkResultList.get(i), bSingleBenchmarkResultList.get(i));
      if (comparison != 0) {
        return comparison;
      }
    }
