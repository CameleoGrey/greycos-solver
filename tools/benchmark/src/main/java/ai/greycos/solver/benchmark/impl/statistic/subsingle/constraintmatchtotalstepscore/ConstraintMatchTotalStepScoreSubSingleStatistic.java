package ai.greycos.solver.benchmark.impl.statistic.subsingle.constraintmatchtotalstepscore;
...
public class ConstraintMatchTotalStepScoreSubSingleStatistic<Solution_>
    extends PureSubSingleStatistic<
        Solution_, ConstraintMatchTotalStepScoreStatisticPoint, LineChart<Long, Double>> {

  private ConstraintMatchTotalStepScoreSubSingleStatistic() {
    // For JAXB.
  }

  public ConstraintMatchTotalStepScoreSubSingleStatistic(
  ...
  @Override
  protected ConstraintMatchTotalStepScoreStatisticPoint createPointFromCsvLine(
      ScoreDefinition<?> scoreDefinition, List<String> csvLine) {
    return new ConstraintMatchTotalStepScoreStatisticPoint(
  ...
  protected List<LineChart<Long, Double>> generateCharts(BenchmarkReport benchmarkReport) {
    ...
    for (int scoreLevelIndex = 0; scoreLevelIndex < builderList.size(); scoreLevelIndex++) {
      ...
      LineChart<Long, Double> chart =
          builder.build(
              "constraintMatchTotalStepScoreSubSingleStatisticChart" + scoreLevelIndex,
              subSingleBenchmarkResult.getName()
                  + " constraint match total step "
                  + scoreLevelLabel
                  + " diff statistic",
              "Time spent",
