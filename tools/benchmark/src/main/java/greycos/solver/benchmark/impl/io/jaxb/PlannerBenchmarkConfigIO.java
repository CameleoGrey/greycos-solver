package greycos.solver.benchmark.impl.io.jaxb;
...
public class PlannerBenchmarkConfigIO {
  ...
  private final GenericJaxbIO<PlannerBenchmarkConfig> genericJaxbIO =
      new GenericJaxbIO<>(PlannerBenchmarkConfig.class);
  ...
  public PlannerBenchmarkConfig read(Reader reader) {
    ...
    if (PlannerBenchmarkConfig.XML_NAMESPACE.equals(
        rootElementNamespace)) { // If there is the correct namespace, validate.
      ...
      return genericJaxbIO.readOverridingNamespace(
          document,
          new ElementNamespaceOverride(
              PlannerBenchmarkConfig.XML_ELEMENT_NAME, PlannerBenchmarkConfig.XML_NAMESPACE),
          new ElementNamespaceOverride(SolverConfig.XML_ELEMENT_NAME, SolverConfig.XML_NAMESPACE));
    } else { // If there is an unexpected namespace, fail fast.
      String errorMessage =
          String.format(
              "The <%s/> element belongs to a different namespace (%s) than expected (%s).",
