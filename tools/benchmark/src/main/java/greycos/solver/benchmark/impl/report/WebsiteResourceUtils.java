...
public final class WebsiteResourceUtils {
  ...
  private static final String RESOURCE_NAMESPACE = "/greycos/solver/benchmark/impl/report/";
  private static final String WEBJAR_RESOURCE_NAMESPACE = "META-INF/resources/webjars/greycos/";
    ...
    try (var in = resourceLoader.apply(namespace + websiteResource)) {
      if (in == null) {
        throw new IllegalStateException(
            "The websiteResource (%s) does not exist.".formatted(websiteResource));
      }
      Files.copy(in, outputFile.toPath());
    } catch (IOException e) {
      throw new IllegalStateException(
          "Could not copy websiteResource (%s) to outputFile (%s)."
              .formatted(websiteResource, outputFile),
          e);
    }
  }
