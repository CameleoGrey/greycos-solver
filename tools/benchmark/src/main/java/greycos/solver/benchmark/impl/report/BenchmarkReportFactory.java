...
import greycos.solver.benchmark.impl.ranking.TotalRankSolverRankingWeightFactory;
import greycos.solver.benchmark.impl.ranking.TotalScoreSolverRankingComparator;
import greycos.solver.benchmark.impl.ranking.WorstScoreSolverRankingComparator;
  ...
  protected void supplySolverRanking(BenchmarkReport benchmarkReport) {
    ...
    if (solverRankingType != null) {
      switch (solverRankingType) {
        ...
        default:
          throw new IllegalStateException(
              "The solverRankingType (%s) is not implemented.".formatted(solverRankingType));
      }
    }
    if (solverRankingComparatorClass != null) {
      solverRankingComparator =
          ConfigUtils.newInstance(
              config, "solverRankingComparatorClass", solverRankingComparatorClass);
    }
    if (solverRankingWeightFactoryClass != null) {
      solverRankingWeightFactory =
          ConfigUtils.newInstance(
              config, "solverRankingWeightFactoryClass", solverRankingWeightFactoryClass);
    }
