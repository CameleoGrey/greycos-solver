...
public final class MillisecondDurationNumberFormatFactory extends TemplateNumberFormatFactory {
  ...
  static final MillisecondDurationNumberFormatFactory INSTANCE =
      new MillisecondDurationNumberFormatFactory();

  @Override
  public TemplateNumberFormat get(String params, Locale locale, Environment environment)
  ...
  public static String formatMillis(Locale locale, long millis) {
    ...
    if (daysPart > 0) {
      ...
      return String.format(
          locale, "%02d:%02d:%06.3f s. (%,d ms.)", hoursPart, minutesPart, seconds, millis);
    } else if (minutesPart > 0) {
      return String.format(locale, "%02d:%06.3f s. (%,d ms.)", minutesPart, seconds, millis);
    } else {
      return String.format(locale, "%.3f s. (%,d ms.)", seconds, millis);
    }
  }
}
