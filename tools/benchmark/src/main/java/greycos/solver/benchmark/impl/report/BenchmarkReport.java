package greycos.solver.benchmark.impl.report;
...
import greycos.solver.benchmark.impl.ranking.SolverRankingWeightFactory;
import greycos.solver.benchmark.impl.result.LoggingLevel;
...
public class BenchmarkReport {
  ...
  public void setSolverRankingComparator(
  ...
  public void writeReport() {
    ...
    worstScoreDifferencePercentageSummaryChartList =
        createWorstScoreDifferencePercentageSummaryChart();
    ...
    for (ProblemBenchmarkResult<?> problemBenchmarkResult :
      ...
      if (problemBenchmarkResult.hasAnySuccess()) {
        for (ProblemStatistic<?> problemStatistic :
    ...
    if (loggingLevelGreyCOSCore == LoggingLevel.TRACE) {
      warningList.add(
          "The loggingLevel ("
              ...
              + ") of greycos.solver.core is high."
              + " This decreases performance."
  ...
  private List<BarChart<Double>> createBestScoreSummaryChart() {
    ...
    for (BarChart.Builder<Double> builder : builderList) {
      ...
      BarChart<Double> chart =
          builder.build(
              "bestScoreSummaryChart" + scoreLevelIndex,
              "Best " + scoreLevelLabel + " summary (higher is better)",
              "Data",
              "Best " + scoreLevelLabel,
              false);
      chartList.add(chart);
      scoreLevelIndex++;
    }
  ...
  private List<LineChart<Long, Double>> createBestScoreScalabilitySummaryChart() {
    ...
    for (LineChart.Builder<Long, Double> builder : builderList) {
      ...
      chartList.add(
          builder.build(
              "bestScoreScalabilitySummaryChart" + scoreLevelIndex,
  ...
  private List<BoxPlot> createBestScoreDistributionSummaryChart() {
          ...
          for (SubSingleBenchmarkResult subSingleBenchmarkResult :
            ...
            for (int i = 0; i < levelValues.length && i < CHARTED_SCORE_LEVEL_SIZE; i++) {
              if (i >= distributionLevelList.size()) {
                distributionLevelList.add(
                    new ArrayList<>(singleBenchmarkResult.getSubSingleCount()));
              }
    ...
    for (BoxPlot.Builder builder : builderList) {
      ...
      BoxPlot boxPlot =
          builder.build(
              "bestScoreDistributionSummaryChart" + scoreLevelIndex,
              "Best " + scoreLevelLabel + " distribution summary (higher is better)",
              "Data",
              "Best " + scoreLevelLabel);
  ...
  private List<BarChart<Double>> createWorstScoreDifferencePercentageSummaryChart() {
    return createScoreDifferenceSummaryChart(
        singleBenchmarkResult ->
            singleBenchmarkResult.getWorstScoreDifferencePercentage().percentageLevels(),
        scoreLevelIndex -> "worstScoreDifferencePercentageSummaryChart" + scoreLevelIndex,
        scoreLevelLabel ->
            "Worst " + scoreLevelLabel + " difference percentage" + " summary (higher is better)",
  ...
  private List<BarChart<Double>> createScoreDifferenceSummaryChart(
      Function<SingleBenchmarkResult, double[]> scoreLevelValueFunction,
      IntFunction<String> idFunction,
      Function<String, String> titleFunction,
      Function<String, String> yLabelFunction) {
    List<BarChart.Builder<Double>> builderList = new ArrayList<>(CHARTED_SCORE_LEVEL_SIZE);
    for (SolverBenchmarkResult solverBenchmarkResult :
        plannerBenchmarkResult.getSolverBenchmarkResultList()) {
      String solverLabel = solverBenchmarkResult.getNameWithFavoriteSuffix();
    ...
    for (BarChart.Builder<Double> builder : builderList) {
      ...
      BarChart<Double> chart =
          builder.build(
              idFunction.apply(scoreLevelIndex),
              titleFunction.apply(scoreLevelLabel),
              "Data",
              yLabelFunction.apply(scoreLevelLabel),
              false);
      chartList.add(chart);
      scoreLevelIndex++;
    }
  ...
  private LineChart<Long, Long> createScoreCalculationSpeedSummaryChart() {
    return createScalabilitySummaryChart(
        SingleBenchmarkResult::getScoreCalculationSpeed,
        "scoreCalculationSpeedSummaryChart",
        "Score calculation speed summary (higher is better)",
  ...
  private LineChart<Long, Long> createMoveEvaluationSpeedSummaryChart() {
    return createScalabilitySummaryChart(
        SingleBenchmarkResult::getMoveEvaluationSpeed,
        "moveEvaluationSpeedSummaryChart",
        "Move evaluation speed summary (higher is better)",
        "Move evaluation speed per second",
        false);
  }

  private BarChart<Double> createWorstScoreCalculationSpeedDifferencePercentageSummaryChart() {
    return createSummaryBarChart(
        result -> result.getWorstScoreCalculationSpeedDifferencePercentage() * 100,
        "worstScoreCalculationSpeedDifferencePercentageSummaryChart",
  ...
  private BarChart<Long> createTimeSpentSummaryChart() {
    return createSummaryBarChart(
        ...
        "Time spent summary (lower time is better)",
        "Time spent",
        true);
  }

  private <N extends Number & Comparable<N>> BarChart<N> createSummaryBarChart(
      Function<SingleBenchmarkResult, N> valueFunction,
      String id,
      String title,
      String yLabel,
      boolean timeOnY) {
  ...
  private LineChart<Long, Long> createTimeSpentScalabilitySummaryChart() {
    return createScalabilitySummaryChart(
        SingleBenchmarkResult::getTimeMillisSpent,
        "timeSpentScalabilitySummaryChart",
        "Time spent scalability summary (lower is better)",
        "Time spent",
        true);
  }

  private LineChart<Long, Long> createScalabilitySummaryChart(
      ToLongFunction<SingleBenchmarkResult> valueFunction,
      String id,
      String title,
      String yLabel,
      boolean timeOnY) {
    ...
    for (SolverBenchmarkResult solverBenchmarkResult :
      ...
      solverBenchmarkResult.getSingleBenchmarkResultList().stream()
          .filter(SingleBenchmarkResult::hasAllSuccess)
          .forEach(
              singleBenchmarkResult -> {
                long problemScale =
                    singleBenchmarkResult.getProblemBenchmarkResult().getProblemScale();
  ...
  private List<LineChart<Long, Double>> createBestScorePerTimeSpentSummaryChart() {
    ...
    for (LineChart.Builder<Long, Double> builder : builderList) {
      ...
      LineChart<Long, Double> chart =
          builder.build(
              "bestScorePerTimeSpentSummaryChart" + scoreLevelIndex,
  ...
  private void writeHtmlOverviewFile() {
    ...
    freemarkerCfg.setCustomNumberFormats(
        Map.of("msDuration", MillisecondDurationNumberFormatFactory.INSTANCE));
